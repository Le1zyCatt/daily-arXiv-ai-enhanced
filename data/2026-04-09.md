...
TL;DR: DesigNet是一个用于SVG设计的神经网络，通过引入可微分模块实现曲线连续性和轴对齐控制，使AI生成的设计更易于人类设计师编辑和集成到专业工作流程中。


<details>
  <summary>Details</summary>
Motivation: 尽管AI内容生成取得了显著进展，但神经网络与人类设计师的工作方式存在根本差异，导致两者协作困难。特别是在SVG设计领域，需要让神经网络能够使用设计师常用的工具和概念。

Method: 提出DesigNet，一个层次化的Transformer-VAE模型，直接在SVG序列上操作。核心贡献是两个可微分模块：连续性自精炼模块（预测并强制C0、G1、C1连续性）和对齐自精炼模块（支持水平和垂直线的吸附对齐）。

Result: DesigNet能够生成可编辑的轮廓，在连续性和对齐准确性方面显著优于现有方法，与最先进方法相比具有竞争力。输出结果更容易精炼和集成到专业设计工作流程中。

Conclusion: 通过为神经网络配备设计师常用的工具（连续性和对齐控制），DesigNet缩小了AI生成与人类设计之间的差距，使AI生成的设计更实用、更易于协作。
...
TL;DR: MoRight是一个用于生成运动控制视频的统一框架，通过解耦的运动建模实现分离的相机和物体运动控制，并学习运动因果关系，支持前向和逆向推理。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个主要问题：1) 将相机和物体运动纠缠在单一跟踪信号中，无法分别控制；2) 将运动视为运动学位移，没有建模物体间的因果关系。这限制了用户对运动视频的精确控制和物理合理性。

Method: 1) 通过时间跨视图注意力实现解耦运动控制：在规范静态视图中指定物体运动，然后转移到任意目标相机视角；2) 将运动分解为主动（用户驱动）和被动（后果）组件，从数据中学习运动因果关系；3) 支持前向推理（给定主动运动预测后果）和逆向推理（指定被动结果恢复驱动动作）。

Result: 在三个基准测试中展示了最先进的性能，包括生成质量、运动可控性和交互感知能力。

Conclusion: MoRight通过解耦的运动建模和因果关系学习，实现了对运动控制视频的精确控制，支持分离的相机和物体运动控制，以及前向和逆向推理，显著提升了生成视频的物理合理性和用户控制能力。
...
TL;DR: 将陨石坑分析重新定义为实例级图像检索问题，提出CraterBench-R基准，发现自监督ViT模型表现最佳，并提出实例-令牌聚合方法在保持精度的同时大幅减少存储需求。


<details>
  <summary>Details</summary>
Motivation: 传统深度学习将陨石坑视为检测问题，但科学工作流程如目录去重、跨观测匹配和形态类比发现本质上是检索任务，需要实例级图像检索方法。

Method: 1) 构建CraterBench-R基准，包含约25,000个陨石坑身份的多尺度视图；2) 评估多种架构，发现自监督ViT表现最佳；3) 提出实例-令牌聚合方法，通过选择K个种子令牌并聚类聚合来减少存储；4) 设计两阶段管道：单向量初筛+实例-令牌重排序。

Result: 自监督ViT（特别是领域预训练）优于参数更多的通用模型；保留多个ViT补丁令牌进行后期交互匹配显著提升精度；实例-令牌聚合在K=16时比原始令牌选择提升17.9点mAP，在K=64时匹配所有196令牌的精度但存储更少；两阶段管道恢复89-94%的完整后期交互精度。

Conclusion: 将陨石坑分析重新定义为检索问题，通过CraterBench-R基准和实例-令牌聚合方法，在保持高精度的同时实现了可扩展的陨石坑检索系统，为行星表面分析提供了新工具。
...
### [4] [Fast Spatial Memory with Elastic Test-Time Training](https://arxiv.org/abs/2604.07350)
*Ziqiao Ma,Xueyang Yu,Haoyu Zhen,Yuncong Yang,Joyce Chai,Chuang Gan*

Main category: cs.CV

TL;DR: 提出Elastic Test-Time Training方法，通过弹性权重整合稳定LaCT的快速权重更新，并引入Fast Spatial Memory模型，实现高效可扩展的4D重建，支持长序列快速适应。


<details>
  <summary>Details</summary>
Motivation: LaCT在长上下文3D重建中表现良好，但其完全可塑的推理时更新容易遭受灾难性遗忘和过拟合，通常只能使用单个大块处理完整输入序列，无法真正处理任意长序列。

Method: 提出基于弹性权重整合的Elastic Test-Time Training方法，通过Fisher加权弹性先验稳定LaCT快速权重更新，锚点状态作为过去快速权重的指数移动平均来平衡稳定性和可塑性。基于此架构引入Fast Spatial Memory模型，从长观察序列学习时空表示并渲染新视角-时间组合。

Result: FSM支持长序列快速适应，提供高质量的3D/4D重建，使用更小的块并减轻相机插值捷径问题。实验表明该方法能超越有限单块设置，实现鲁棒的多块适应，显著缓解激活内存瓶颈。

Conclusion: 该方法将LaCT扩展到鲁棒的多块适应，是实现真正长序列泛化的必要步骤，同时大幅减轻激活内存瓶颈，为4D重建提供了高效可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 迭代重建技术虽然能通过减少投影降低辐射剂量，但需要精确调参多个超参数，手动设置耗时且增加操作员负担

Method: 提出全自动参数优化框架，包含改进的乌鸦搜索算法（具有优越的集合依赖局部搜索机制、搜索空间感知全局搜索策略、目标驱动的局部-全局搜索平衡）和混沌对角线性均匀初始化方案加速收敛

Result: 在三个成像设备和四个真实数据集上评估，平均适应度提升4.19%，在CHILL@UK和RPI_AXIS两个无参考学习质量指标上分别提升4.89%和3.82%，定性结果显示能保持精细细节

Conclusion: 提出的框架在不同比较场景中表现出有效性和鲁棒性，能够自动优化CBCT迭代重建参数，优于手动设置和原始CSA算法
...
TL;DR: 论文提出DISSECT基准测试，用于诊断视觉语言模型中的感知-整合差距，发现开源模型在化学分子推理中存在整合瓶颈，而闭源模型表现更好。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型存在感知-整合差距：模型能够正确识别图像内容，但在后续推理中却无法有效利用这些视觉信息。当前基准测试将感知和整合混为一谈，无法诊断这种失败。

Method: 提出DISSECT诊断基准，包含12,000个化学和生物学问题。采用五种输入模式评估：视觉+文本、纯文本、纯视觉、人类标注、以及创新的模型标注（模型先描述图像，然后基于自己的描述进行推理）。

Result: 评估18个视觉语言模型发现：1）化学比生物学更难利用语言先验，需要真正的视觉推理；2）开源模型基于自己描述推理的得分高于基于原始图像，存在整合瓶颈；3）闭源模型没有这种差距，表明感知-整合整合是区分开源和闭源能力的关键。

Conclusion: 模型标注协议是模型和基准测试无关的，可用于任何视觉语言模型评估后诊断整合失败。感知-整合差距是当前视觉语言模型的关键瓶颈，特别是对于需要真正视觉推理的任务如化学分子分析。
...
TL;DR: Telescope是一个用于超长距离自动驾驶的两阶段检测模型，通过新颖的重采样层和图像变换解决小目标检测问题，在250米以上距离的检测性能相比SOTA方法提升76%。


<details>
  <summary>Details</summary>
Motivation: 高速公路自动驾驶（特别是长途重型卡车）需要在500米以上的超长距离检测物体以满足高速制动需求。在长距离下，车辆等关键物体在图像中只占几个像素，现有检测器失效，而商用LiDAR的有效范围有限，因此需要基于图像的解决方案。

Method: 提出Telescope两阶段检测模型，包含强大的检测骨干网络、新颖的重采样层和图像变换，专门解决小、远距离物体的检测挑战。

Result: 在超长距离检测（250米以上）中，相比SOTA方法获得76%的相对mAP提升（从0.185提升到0.326），计算开销最小，并在所有检测距离上保持强性能。

Conclusion: Telescope模型有效解决了自动驾驶中超长距离小目标检测的挑战，为高速公路自动驾驶提供了实用的图像检测解决方案。
...
TL;DR: 这篇综述系统回顾了视频生成技术的发展，从早期GAN到主流扩散模型，再到新兴的自回归和多模态技术，分析了原理、进展和局限，并探讨了多模态视频生成的趋势和应用前景。


<details>
  <summary>Details</summary>
Motivation: 现有视频生成综述往往局限于特定技术领域（如GAN、扩散模型）或具体任务（如视频编辑），缺乏对领域演进的全面视角，特别是关于自回归模型和多模态信息整合方面。需要填补这些空白，为未来研究提供指导。

Method: 采用系统性综述方法：1）追溯视频生成技术从GAN到扩散模型再到自回归和多模态技术的发展历程；2）深入分析基础原理、关键进展和比较优势/局限；3）探索多模态视频生成的新兴趋势，强调多数据类型整合以增强上下文感知。

Result: 提供了视频生成领域的全面技术发展脉络，分析了不同技术范式的优缺点，识别了多模态整合的关键趋势，为理解该领域的历史发展和未来方向建立了系统框架。

Conclusion: 通过连接历史发展和当代创新，这篇综述为视频生成及其应用（包括虚拟/增强现实、个性化教育、自动驾驶模拟、数字娱乐和高级世界模型）的未来研究提供了指导性见解，推动这一快速演进领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 超声心动图在心血管疾病筛查和诊断中很重要，但自动化智能分析面临挑战：复杂的心脏动力学和强烈的视图异质性。现有视觉语言模型方法容易受到模板捷径和虚假解释的影响，需要更可靠、可解释的临床决策支持。

Method: 提出EchoTrust框架，采用证据驱动的Actor-Verifier架构。首先生成结构化的中间表示，然后由不同角色进行分析，实现更可靠和可解释的决策。

Result: 该方法能够产生更可靠和可解释的决策，适用于高风险临床应用，解决了现有方法在超声心动图分析中的模板捷径和虚假解释问题。
...
TL;DR: 提出基于视觉语言框架的食物营养分析新方法，使用餐前餐后配对图像进行食物级别的营养评估


<details>
  <summary>Details</summary>
Motivation: 现有基于图像的饮食评估方法大多依赖单张餐前图像，只能提供粗略的餐级估计，无法确定实际消耗量，且需要深度感知、多视角图像或显式分割等限制性输入

Method: 使用配对餐前餐后图像，通过自然语言提示定位特定食物项并直接从单张RGB图像估计重量，采用两阶段训练策略预测配对图像间的重量差异来估计食物消耗

Result: 在三个公开数据集上评估，相比现有方法取得一致改进，为餐前餐后饮食图像分析建立了强基线

Conclusion: 提出的简单视觉语言框架能够进行食物级别的营养分析，无需传统分割方法，仅需配对RGB图像即可准确估计食物消耗
...
TL;DR: 提出MTA-Agent框架，通过工具增强的多跳推理自动生成高质量多模态深度搜索训练数据，构建MTA-Vision-DeepSearch数据集，使32B开源多模态搜索代理在六个基准测试中超越GPT-5、Gemini等商业模型。


<details>
  <summary>Details</summary>
Motivation: 现有MLLMs在需要深度搜索和整合视觉证据与外部知识的复杂多步推理任务上仍存在局限，需要高质量的多跳视觉语言训练数据来提升多模态深度搜索代理的能力。

Method: 提出MTA-Agent框架，自动选择工具及其参数从视觉和文本源检索验证证据，生成结构化多跳问答轨迹。从多样VQA种子数据集出发，通过多阶段验证流程构建包含21K高质量多跳示例的MTA-Vision-DeepSearch数据集。

Result: 使用该数据集训练的32B开源多模态搜索代理在六个挑战性基准测试中平均达到54.63%，超越GPT-5(51.86%)、Gemini-2.5-Pro(50.98%)和Gemini-3-Pro(54.46%)。推理深度从平均2.27步提升到4.28步，搜索策略更系统持久。

Conclusion: MTA-Agent为多模态深度搜索提供了完全开放的解决方案，通过高质量训练数据显著提升多模态搜索代理的推理能力和工具使用行为，且可通过缓存交互降低训练成本，促进开源多模态搜索代理研究。
...
Method: 提出MorphDistill两阶段框架：第一阶段使用维度无关的多教师关系蒸馏和监督对比正则化训练学生编码器，整合10个基础模型的知识；第二阶段通过基于注意力的多实例学习聚合整张切片图像特征来预测五年生存率。

Result: 在Alliance/CALGB 89803队列中，MorphDistill达到AUC 0.68，比最强基线提高约8%；C-index为0.661，风险比2.52。在外部TCGA队列中C-index为0.628，表现出良好的泛化能力和临床亚组稳健性。

Conclusion: MorphDistill通过整合多个基础模型知识实现任务特异性表示学习，为计算病理学中的预后建模提供了高效策略，具有更广泛肿瘤学应用潜力，需要在更多队列和疾病阶段进一步验证。
...
TL;DR: 首个面向边缘设备持续学习的视觉异常检测基准，提出Tiny-Dinomaly模型，在内存和计算成本大幅降低的同时提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有视觉异常检测研究未能同时解决边缘部署（计算资源受限）和持续学习（适应数据分布变化而不遗忘）两个关键挑战，需要综合评估方法在效率和适应性上的权衡。

Method: 1) 建立首个边缘设备持续学习场景的VAD基准，评估7个VAD模型在3个轻量级骨干架构上的表现；2) 提出Tiny-Dinomaly，基于DINO基础模型的轻量化版本；3) 对PatchCore和PaDiM进行针对性改进以提升持续学习效率。

Result: Tiny-Dinomaly实现了13倍内存占用减少和20倍计算成本降低，同时Pixel F1分数提升5个百分点。基准研究为在效率和适应性约束下选择最优骨干网络和VAD方法提供了指导。

Conclusion: 该工作填补了视觉异常检测在边缘部署和持续学习联合场景的研究空白，提出的基准和轻量化方法为实际应用提供了实用解决方案，揭示了内存占用、推理成本和检测性能之间的权衡关系。
...
TL;DR: 本文提出激活提示（AP）作为视觉提示（VP）的扩展，通过在模型中间层添加通用扰动而非仅输入层，显著提升了性能与效率，缩小了与常规微调的差距。


<details>
  <summary>Details</summary>
Motivation: 视觉提示（VP）作为一种无需修改模型参数的迁移学习方法，与常规微调方法存在明显性能差距。本文旨在理解并提升输入级VP的性能，探索如何缩小这一差距。

Method: 提出激活提示（AP）概念，将通用扰动扩展到模型中间层的激活映射上。通过AP分析VP的局限性，揭示输入级提示效果不足的原因。理论分析各层全局特征，解释AP的层偏好机制。

Result: 在29个数据集和多种模型架构上的实验表明，AP在准确性和效率（时间、参数、内存使用和吞吐量）方面均优于VP和参数高效微调基线。AP与CNN和ViT中的归一化调优密切相关，但不同模型类型具有不同的层偏好。

Conclusion: 激活提示（AP）通过将扰动扩展到中间层，有效解决了输入级视觉提示的局限性，在性能和效率上均优于现有方法，为模型适配提供了更优的解决方案。
...
TL;DR: PhysHead：一种用于可动画头部化身的混合表示方法，结合3D参数化头部网格和基于发丝的头发，通过物理引擎模拟实现逼真的头发动态效果


<details>
  <summary>Details</summary>
Motivation: 现有头部化身方法通常假设头发是刚性的，将头发表示为简单的外壳，无法捕捉其自然的体积行为，导致无法实现逼真的头发动态效果

Method: 提出PhysHead混合表示：1）使用3D参数化网格表示头部；2）基于发丝的头发表示，可直接用物理引擎模拟；3）采用附着在头部网格和头发段上的高斯基元进行外观建模；4）利用VLM模型生成动态训练序列中被遮挡区域的外观

Result: 在定量和定性研究中证明了该方法的能力，能够合成物理上合理的头发动态效果，同时支持表情和相机控制，超越了现有基线方法

Conclusion: PhysHead能够创建具有逼真头发动态行为的照片级头部化身，克服了现有方法中刚性头发的限制，实现了头发随风飘动等自然效果
...
TL;DR: 提出基于图神经网络（GNN）的模型，利用静息态功能磁共振成像（rs-fMRI）的功能连接图，预测阿尔茨海默病患者认知障碍阶段的进展。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病（AD）影响美国超过700万人，目前无法治愈，但早期发现可以延缓病情进展。需要开发能够预测认知障碍阶段转换的模型，以便及时干预。

Method: 使用GNN模型结合循环神经网络（RNN）块，处理303名受试者的rs-fMRI功能连接图数据。模型能够处理不同数量的就诊记录、不规则时间间隔，并容忍缺失就诊数据。

Result: 模型总体准确率达到82.9%，在认知正常（CN）到轻度认知障碍（MCI）转换预测中达到68.8%的准确率，这在领域内是具有挑战性的任务。

Conclusion: rs-fMRI在预测MCI或AD发病方面具有有效性，结合其他模态可为延缓认知障碍进展提供及时的干预方法。
...
TL;DR: 提出结合ResNet-1D、BiGRU和多头注意力的混合深度学习模型，用于工业物联网入侵检测，在EdgeHoTset数据集上达到98.71%准确率，在CICIoV2024数据集上达到99.99%准确率，优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 工业物联网系统面临安全威胁，需要有效的入侵检测方法。现有方法在处理时空特征和类别不平衡方面存在不足，需要开发能够实时检测且具有良好泛化能力的模型。

Method: 提出混合深度学习模型：1) ResNet-1D提取空间特征；2) BiGRU捕获时间依赖关系；3) 多头注意力机制进行特征加权；4) 使用SMOTE处理类别不平衡问题；5) 在EdgeHoTset和CICIoV2024数据集上进行训练和测试。
...
Conclusion: 提出的混合深度学习模型在工业物联网入侵检测中表现出色，具有高准确性、低延迟和良好的泛化能力，适合实时应用，为解决工业物联网安全威胁提供了有效解决方案。
...
TL;DR: LiftFormer：基于提升理论拓扑的单目深度估计新方法，通过构建深度导向几何表示子空间和边缘感知表示子空间，将颜色特征与深度值关联，提升边缘区域深度预测精度。


<details>
  <summary>Details</summary>
Motivation: 单目深度估计是从单张图像/视频估计深度图来表示场景3D结构的高度不适定问题。现有方法在边缘区域深度预测存在误差，需要更有效的几何表示来桥接颜色特征与深度值。

Method: 提出基于提升理论拓扑的LiftFormer：1）构建深度导向几何表示子空间，将深度值预测问题转化为子空间特征表示；2）构建边缘感知表示子空间，增强边缘局部特征；3）使用线性相关向量构建冗余鲁棒表示。

Result: 在广泛使用的数据集上实现了最先进的性能，消融研究验证了两个提升模块的有效性。

Conclusion: LiftFormer通过构建中间子空间成功桥接了图像颜色特征与深度值，特别是改善了边缘区域的深度预测，为单目深度估计提供了新的几何表示方法。
...
TL;DR: VAMAE：一种用于OCTA图像自监督预训练的血管感知掩码自编码框架，通过解剖学感知的掩码策略和多目标重建，在有限标注数据下显著提升血管分割性能。


<details>
  <summary>Details</summary>
Motivation: OCTA图像中的血管结构稀疏且具有强拓扑约束，现有自监督学习方法（如掩码自编码）主要针对密集自然图像设计，采用均匀掩码和像素级重建，难以有效捕捉血管几何特征。

Method: 提出VAMAE框架：1）基于血管性和骨架线索的解剖学感知掩码，强调血管丰富区域；2）多目标重建，同时学习外观、结构和拓扑信息；3）在OCTA-500基准上评估不同监督水平下的血管分割任务。
...
TL;DR: HopS是一种用于部分标注场景的提示学习方法，通过局部密度过滤和全局最优传输两种互补策略进行标签选择，在八个基准数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 在部分标注可用的情况下，提示学习的性能受到标签模糊性和监督信息不足的限制，需要解决标签选择问题。

Method: 提出Holistic Optimal Label Selection (HopS)：1) 局部密度过滤：从最近邻候选集中选择高频标签，用softmax分数识别最可能标签；2) 全局最优传输：将均匀采样分布映射到批次候选标签分布，最小化期望传输成本确定最可能标签分配。

Result: 在八个基准数据集上的广泛实验表明，HopS在部分监督下持续提升性能，优于所有基线方法。

Conclusion: HopS通过局部和全局视角的互补标签选择策略，为弱监督设置下的提示学习提供了实用解决方案，突显了整体标签选择的优势。
...
TL;DR: MARMamba是一种基于Mamba架构的CT金属伪影去除模型，无需正弦图数据，能有效去除不同尺寸金属植入物产生的伪影，同时保持原始解剖结构完整性，在计算效率与恢复效果间取得良好平衡。


<details>
  <summary>Details</summary>
Motivation: CT成像中金属植入物会产生严重伪影，影响图像质量和诊断准确性。现有方法存在三个主要问题：1）器官和组织结构退化；2）依赖正弦图数据；3）资源使用与恢复效率不平衡。

Method: 提出MARMamba模型，采用精简的UNet架构，核心是多尺度Mamba（MS-Mamba）模块。MS-Mamba包含翻转Mamba块，通过多方向分析图像捕获全面上下文信息，然后通过平均最大前馈网络整合关键特征与平均特征来抑制伪影。

Result: 实验结果表明，该模型在减少金属伪影方面优于其他模型，在计算需求、内存使用和参数数量之间取得了最佳平衡，突出了其在实际应用中的实用性。

Conclusion: MARMamba能有效去除不同尺寸金属植入物产生的伪影，同时保持原始解剖结构完整性，无需额外输入数据，在计算效率和恢复效果间取得良好平衡，具有实际应用价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有天气去除方法主要针对特定天气条件，少数能处理多天气场景的方法往往忽视性能考虑，导致参数大、推理时间长、内存成本高。需要一种既能处理多种天气条件又能在性能上取得平衡的解决方案。

Method: 采用UNet-like结构，结合门控机制和多尺度金字塔视觉Transformer。使用通道注意力优化特征提取，线性空间缩减降低注意力计算需求。在feed-forward和下采样阶段策略性地放置门控机制，选择性处理冗余信息，自适应选择关键数据。

Result: 模型在恢复质量、参数效率、计算开销和内存使用方面达到最佳平衡，相比其他多天气模型具有显著优势，能够有效满足实际应用需求。

Conclusion: WeatherRemover模型成功解决了多天气图像恢复问题，在保持高质量恢复的同时实现了轻量化设计，为实际应用提供了有效的解决方案。
...
Method: 使用变分潜在瓶颈将输入编码到紧凑潜在空间，通过基于KL散度和梯度显著性的动态二进制掩码抑制对指定任务无信息的潜在维度。编码器在白盒设置中训练，推理时只需冻结目标模型的前向传播。

Result: 在CIFAR-100上，处理后的表示保持指定分类器的强实用性，同时将所有非预期分类器的准确率降至2%以下，抑制比超过45倍。在CIFAR-10、Tiny ImageNet和Pascal VOC上的初步实验显示方法可扩展到不同任务设置。

Conclusion: 该框架有效抑制跨模型传输，保护输入数据不被未经授权的下游模型重用，同时保持指定任务的性能。需要进一步评估对抗自适应攻击的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 传统感知视频压缩采用生成模型提升感知真实感，但常牺牲信号保真度，与视频压缩忠实再现视觉信号的目标相悖。需要缓解感知与保真度之间的困境。

Method: 提出可控生成视频压缩范式：1) 编码场景关键帧作为结构先验；2) 编码密集逐帧控制先验以保留细粒度结构和语义；3) 使用可控视频生成模型在时序和内容一致性下重建非关键帧；4) 开发颜色距离引导的关键帧选择算法自适应选择关键帧

Result: 实验结果表明CGVC在信号保真度和感知质量方面均优于以往的感知视频压缩方法

Conclusion: CGVC范式通过多视觉条件引导生成，有效平衡了感知质量与信号保真度，为视频压缩提供了新思路
...
TL;DR: GPAFormer：一种用于3D医学图像分割的轻量级网络，在保持高精度的同时强调效率，在多个数据集上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 由于成像模态多样性、数据高维性和解剖结构异质性，在多器官分割任务中同时实现分割精度和计算效率仍然具有挑战性。

Method: 提出GPAFormer轻量级网络架构，包含两个核心模块：多尺度注意力引导堆叠聚合（MASA）和互感知补丁图聚合器（MPGA）。MASA使用三个并行路径处理不同感受野，MPGA采用图引导方法动态聚合具有相似特征分布的补丁。

Result: 在BTCV、Synapse、ACDC和BraTS数据集上，仅使用181万参数的GPAFormer取得了最高的DSC分数（75.70%、81.20%、89.32%、82.74%），在消费级GPU上单次推理时间不到1秒。

Conclusion: GPAFormer在多种临床场景下平衡了多器官、多模态3D分割任务的准确性和效率，特别适用于资源受限和时间敏感的临床环境。
...
TL;DR: ISTS：针对文本到图像扩散模型的新型水印范式，通过动态控制注入时间和水印模式，结合双向检测，有效抵抗移除和伪造攻击。


<details>
  <summary>Details</summary>
Motivation: 生成内容引发了版权保护、图像来源和归属权等严重问题，现有水印技术容易受到移除攻击和伪造攻击的威胁，需要更鲁棒的水印方案。

Method: 提出ISTS（实例特定水印与双向检测）范式：1）基于用户提示语义动态控制水印注入时间和模式；2）采用双向检测方法增强水印检测鲁棒性。

Result: 实验证明ISTS水印在抵抗移除攻击和伪造攻击方面具有优越性，显著提升了水印的安全性和可靠性。

Conclusion: ISTS为文本到图像扩散模型提供了一种有效的抗攻击水印方案，解决了现有水印技术的脆弱性问题，为版权保护和内容溯源提供了更可靠的技术支持。
...
TL;DR: VDPP是一个视频深度后处理框架，通过纯几何优化而非完整场景重建，实现了高速（>43.5 FPS）、高精度且无需RGB输入的视频深度估计，可直接集成任何图像深度模型。


<details>
  <summary>Details</summary>
Motivation: 现有端到端视频深度模型虽然性能优秀，但紧密耦合导致难以快速集成新的单图像深度估计器；而后处理方法在速度、精度和实用性上仍不足。需要一种既能快速集成新模型，又具备高效实时性能的解决方案。

Method: VDPP采用纯几何后处理框架，将计算从昂贵的场景重建转向低分辨率空间的针对性几何优化。通过密集残差学习驱动几何表示，而非完整重建，实现高速处理。RGB-free架构确保可扩展性。

Result: VDPP在NVIDIA Jetson Orin Nano上达到>43.5 FPS，匹配端到端系统的时间一致性，在速度、精度和内存效率方面达到最佳平衡，适合实时边缘部署。

Conclusion: VDPP为视频深度估计提供了一种实用高效的后处理解决方案，通过几何优化范式实现了速度、精度和可扩展性的平衡，是实时边缘部署的最实用选择。
...
Method: RASR框架包含三个模块：1) CSPR将视频解构为高层语义原语并从动态记忆库检索关联证据；2) DGMP融合领域先验驱动专家多模态大语言模型生成领域感知分析报告；3) MVDFF通过自适应门控机制集成多维特征进行真实性判定

Result: 在FakeSV和FakeTT数据集上，RASR显著优于现有最佳基线，实现优越的跨域泛化能力，整体检测精度提升最高达0.93%
...
TL;DR: 提出基于视觉语言模型和LLM代理的甲骨文破译框架，通过组件识别、图知识检索和关系推理实现更精确的破译，并构建了专家标注数据集OB-Radix。


<details>
  <summary>Details</summary>
Motivation: 现有方法将甲骨文破译视为封闭式图像识别问题，存在"解释鸿沟"：单个字符通常独特且罕见，但由有限的重复象形组件构成，这些组件具有可转移的语义含义。需要利用这种结构逻辑进行更准确的破译。

Method: 提出代理驱动的视觉语言模型框架，整合VLM进行精确视觉定位，使用LLM代理自动化组件识别、基于图的知识检索和关系推理的推理链，实现语言准确的解释。构建OB-Radix数据集，包含1,022个字符图像和1,853个细粒度组件图像。

Result: 在三个不同任务的基准测试中，该框架相比基线方法产生更详细和精确的破译结果。

Conclusion: 通过利用甲骨文的结构逻辑和构建专家标注数据集，提出的代理驱动VLM框架能够更准确地破译甲骨文，为古代文化研究提供新见解。
...
TL;DR: 提出一个改进的半稠密图像匹配方法，解决粗匹配阶段的尺度差异问题和细匹配阶段的局部一致性问题


<details>
  <summary>Details</summary>
Motivation: 现有半稠密图像匹配方法存在两个长期问题：1）粗匹配阶段中互最近邻匹配的过度排除问题，使其难以处理图像间的尺度差异；2）细匹配阶段忽视最终匹配的局部一致性，影响鲁棒性

Method: 1）提出尺度感知匹配模块，利用得分矩阵中的隐藏信息指示尺度比例；2）将细匹配阶段重新定义为级联流细化问题，引入梯度损失来鼓励流场的局部一致性

Result: 大量实验表明，提出的新匹配流程在下游任务中实现了鲁棒且准确的匹配性能

Conclusion: 通过解决粗匹配的尺度差异问题和细匹配的局部一致性问题，提出的方法显著提升了半稠密图像匹配的性能
...
TL;DR: HQF-Net：一种用于遥感图像分割的混合量子-经典多尺度融合网络，通过集成冻结的DINOv3 ViT-L/16骨干网络的多尺度语义引导，结合可变形多尺度交叉注意力融合模块和量子增强组件，在多个遥感基准测试中取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 传统编码器-解码器架构（如U-Net）在遥感语义分割中难以充分利用全局语义和结构化特征交互。需要能够同时捕捉精细空间细节和高层次语义上下文的模型。

Method: 提出HQF-Net混合量子-经典网络：1）使用冻结的DINOv3 ViT-L/16骨干网络提供多尺度语义引导；2）通过可变形多尺度交叉注意力融合模块（DMCAF）与定制U-Net架构集成；3）引入量子增强跳跃连接（QSkip）和量子瓶颈混合专家模块（QMoE），结合局部、全局和方向性量子电路。

Result: 在三个遥感基准测试中表现优异：LandCover.ai上达到0.8568 mIoU和96.87%总体准确率；OpenEarthMap上71.82% mIoU；SeasoNet上55.28% mIoU和99.37%总体准确率。架构消融研究证实了各主要组件的贡献。

Conclusion: 结构化混合量子-经典特征处理是在近期量子约束下改进遥感语义分割的有前景方向，HQF-Net展示了量子增强组件与传统深度学习架构有效集成的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统神经架构搜索方法需要完全重新训练模型，成本高昂。本文旨在开发一种更经济的方法，通过微调现有已训练模型来提升性能，避免完全重新训练的开销

Method: 提出植入式自适应单元(IAC)概念，使用部分连接的DARTS方法识别小型模块，将这些模块注入到已训练U型模型的跳跃连接中，实现架构优化而无需完全重新训练

Result: 在四个医学数据集(MRI和CT图像)上的实验显示，各种U-Net配置均获得一致的精度提升，分割准确率平均提高约5个百分点，最佳情况下提升达11个百分点

Conclusion: 该方法为复杂模型性能升级提供了经济有效的替代方案，避免了完全重新设计模型的成本，并表明该方法具有扩展到其他架构和问题领域的潜力
...
TL;DR: DeSOPE是一个大规模6自由度变形物体数据集，包含26个常见物体类别的高质量3D扫描和133K帧RGB-D数据，用于研究物体变形对6D姿态估计的影响。


<details>
  <summary>Details</summary>
Motivation: 现有6D物体姿态估计方法大多假设物体是刚性或关节式的，但在实际应用中物体会因磨损、碰撞或变形而偏离其规范形状，需要专门的数据集来研究这种变形情况。

Method: 创建了包含26个物体类别的数据集，每个类别有1个规范状态和3个变形配置的高保真3D扫描。采用半自动标注流程：先标注2D掩码，用物体姿态方法计算初始姿态，通过物体级SLAM系统优化，最后人工验证生成665K个姿态标注。

Result: 评估多个物体姿态方法发现，随着变形程度的增加，性能显著下降，表明变形处理对实际应用至关重要。数据集包含133K帧RGB-D数据和665K个姿态标注。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型依赖2D视觉先验，在复杂3D空间推理上表现不佳。现有方法要么需要昂贵的后训练，要么缺乏几何理解和视角灵活性。

Method: 提出训练免费框架，包含：1）使用MLLM引导的关键词提取和多粒度掩码生成从单图像重建高保真3D网格；2）利用外部知识库迭代计算最优相机外参并合成新视角。

Result: 在3DSRBench和Rel3D等主要基准测试中显著优于专用空间模型和通用MLLM（包括GPT-5.2和Gemini-2.5-Flash）。

Conclusion: 该框架通过显式3D重建和视角模拟显著提升了空间理解能力，无需额外训练即可解决MLLM的3D空间推理局限。
...
TL;DR: 提出URMF框架，通过显式建模模态可靠性来解决多模态讽刺检测中模态噪声问题，使用不确定性感知融合提升检测准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有多模态讽刺检测方法假设所有模态同等可靠，但现实社交媒体中文本可能模糊、图像可能弱相关甚至无关，确定性融合会引入噪声证据并削弱鲁棒推理。

Method: 提出不确定性感知鲁棒多模态融合(URMF)框架：1)使用多头交叉注意力将视觉证据注入文本表示；2)在融合语义空间中使用多头自注意力增强不一致性感知推理；3)对文本、图像和交互感知潜在表示进行统一的单模态偶然不确定性建模；4)使用估计的不确定性动态调节模态贡献；5)设计包含任务监督、模态先验正则化、跨模态分布对齐和不确定性驱动自采样对比学习的联合训练目标。

Result: 在公开MSD基准测试中，URMF始终优于强大的单模态、多模态和MLLM基线，证明了不确定性感知融合在提高准确性和鲁棒性方面的有效性。

Conclusion: URMF通过显式建模模态可靠性并动态调节模态贡献，有效解决了多模态讽刺检测中的模态噪声问题，提高了检测的准确性和鲁棒性。
...
TL;DR: 提出DOC-GS框架解决稀疏视图3D高斯泼溅重建中的过拟合和伪影问题，通过双域观察与校准建模高斯基元可靠性


<details>
  <summary>Details</summary>
Motivation: 稀疏视图3D高斯泼溅重建由于几何监督不足导致严重过拟合，产生结构扭曲和半透明雾状伪影。现有基于dropout的正则化方法缺乏对伪影形成的统一理解，核心挑战在于高斯基元可靠性的不可观测性。

Method: 提出双域观察与校准框架：1) 优化域：通过连续深度引导dropout策略，以dropout概率作为基元可靠性代理，施加平滑深度感知归纳偏置；2) 观察域：建立浮动物体伪影与大气散射的联系，利用暗通道先验作为结构一致性线索识别异常区域，设计可靠性驱动的几何剪枝策略。

Result: 未在摘要中明确说明具体实验结果，但方法旨在抑制弱约束高斯、提高优化稳定性、识别并移除低置信度高斯，从而解决过拟合和伪影问题。

Conclusion: 通过建模和校准高斯可靠性，DOC-GS框架为稀疏视图3D高斯泼溅重建提供了统一的解决方案，结合优化域归纳偏置和观察域证据来抑制不可靠高斯积累导致的伪影。
...
TL;DR: LiveStre4m：首个实时无位姿多视角视频直播新视角合成方法，相比优化方法快38倍，仅需两个未标定输入流


<details>
  <summary>Details</summary>
Motivation: 现有动态场景表示方法需要已知相机参数且优化时间长（约2.67秒），无法满足直播场景需求。需要解决无位姿多视角视频的实时新视角合成问题。

Method: 提出前馈模型LiveStre4m：1）多视角视觉Transformer进行关键帧3D场景重建；2）扩散-Transformer插值模块保证时间一致性和稳定流式传输；3）相机位姿预测器直接从RGB图像估计位姿和内参，无需相机标定信息。

Result: 在1024×768分辨率下达到平均每帧0.07秒重建时间，比基于优化的动态场景表示方法快数个数量级。仅需两个同步未标定输入流即可实现时间一致的新视角视频流。
...
TL;DR: 将静态视觉上下文学习模型转变为可通过涂鸦、点击等用户交互进行控制的系统


<details>
  <summary>Details</summary>
Motivation: 现有视觉上下文学习模型虽然能适应新任务，但缺乏用户交互机制，无法根据用户提供的涂鸦、点击等指导信号来调整预测结果，这在需要用户主动指导的实际应用中受到限制

Method: 提出Interactive DeLVM方法，将用户交互直接编码到示例输入-输出对中，保持视觉上下文学习的核心理念，让用户无需微调即可通过交互提示模型，动态引导预测

Result: 实验表明现有SOTA模型无法有效利用交互线索，而本方法在可控用户引导场景中表现优异：交互分割IoU提升7.95%，定向超分辨率PSNR提升2.46，交互物体去除LPIPS降低3.14%

Conclusion: 本工作填补了静态任务适应与流畅交互性之间的空白，实现了以用户为中心的视觉上下文学习，使模型能够通过自然视觉线索与用户进行无缝交互
...
TL;DR: VENUSS框架系统评估视觉语言模型在连续驾驶场景中的性能，发现现有模型仅57%准确率，远低于人类65%水平，且输入配置对性能有显著影响


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在自动驾驶任务中应用日益广泛，但对其在连续驾驶场景中的性能缺乏系统评估，特别是输入配置如何影响模型能力尚不明确

Method: 提出VENUSS框架，从现有数据集提取时间序列驾驶视频，生成结构化评估类别，比较25+个现有VLM模型在2600+个场景中的表现，分析输入配置（分辨率、帧数、时间间隔、空间布局、呈现模式）对性能的影响

Result: 最佳模型仅达到57%准确率，低于人类在相同约束下的65%表现；模型擅长静态物体检测，但在理解车辆动态和时间关系方面存在显著能力差距；输入配置对模型性能有系统性影响

Conclusion: VENUSS为连续驾驶场景中的VLM性能提供了首个系统敏感性分析框架，揭示了现有模型的局限性，并为未来研究建立了基准，表明需要改进模型对动态和时间关系的理解能力
...
TL;DR: FlowInOne提出了一种纯视觉流的多模态生成框架，将所有输入转换为视觉提示，实现图像输入-图像输出的统一流程，在多种生成任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 挑战传统文本驱动的多模态生成范式，探索是否所有模态（文本描述、空间布局、编辑指令）都能统一到单一视觉表示中，以消除跨模态对齐瓶颈。

Method: 将多模态生成重新表述为纯视觉流，所有输入转换为视觉提示，使用单一流匹配模型构建图像输入-图像输出管道，无需跨模态对齐、噪声调度或任务特定分支。

Result: 在文本到图像生成、布局引导编辑和视觉指令跟随等任务上达到最先进性能，超越开源模型和商业系统，并创建了包含500万视觉提示对的VisPrompt-5M数据集和VP-Bench基准。

Conclusion: FlowInOne为完全视觉中心的生成建模建立了新基础，在单一连续视觉空间中实现感知与创造的共存，统一了多种生成任务。
...
TL;DR: FlowExtract：从ISO 5807标准化流程图中提取有向图的系统，优于视觉语言模型，实现可查询的程序知识表示


<details>
  <summary>Details</summary>
Motivation: 制造业中的维护程序通常以静态PDF或扫描图像中的流程图形式记录，这些流程图编码了资产生命周期管理必需的程序知识，但无法被现代操作员支持系统访问。视觉语言模型在处理此类图表的连接拓扑重建方面存在困难。
...
Conclusion: FlowExtract为组织提供了实现可查询程序知识表示的实际路径，代码已在GitHub开源。
...
TL;DR: MAPO通过强制模型生成视觉工具使用结果的文本描述，并设计结合语义对齐与任务奖励的优势估计，解决多模态推理中文本推理与视觉动作执行不一致的问题，提升多轮推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的多模态大语言模型训练仅依赖结果奖励，忽视了文本推理合理性与视觉动作执行不精确之间的差异。这种推理-动作不一致会在多轮推理过程中积累噪声，严重损害模型的多模态推理能力，甚至导致训练崩溃。

Method: 提出多模态代理策略优化（MAPO），要求模型为通过工具使用获得的视觉内容生成明确的文本描述，然后设计新颖的优势估计方法，将这些描述与实际观察的语义对齐与任务奖励相结合。

Result: 理论分析表明MAPO能有效降低梯度方差，大量实验证明该方法在多个视觉推理基准测试中取得了优越性能。

Conclusion: MAPO通过弥合文本推理与视觉动作之间的差距，解决了多模态推理中的执行失败问题，显著提升了多轮视觉推理的性能和稳定性。
...
TL;DR: EventFace：基于事件相机的面部识别框架，通过结合空间结构和时间动态进行身份建模，在有限数据下实现高性能识别


<details>
  <summary>Details</summary>
Motivation: 事件相机在面部识别中具有光照鲁棒性和隐私友好性优势，但缺乏传统RGB系统的稳定光度外观。需要建模由刚性面部运动和个体面部几何形状驱动的时空身份表示

Method: 提出EventFace框架：1) 使用LoRA从预训练RGB面部模型迁移结构先验；2) 引入运动提示编码器(MPE)显式编码时间特征；3) 使用时空调制器(STM)融合时空特征

Result: 在EFace数据集上达到94.19%的Rank-1识别率和5.35%的EER，在退化光照条件下表现出更强的鲁棒性，学习到的表示具有更低的模板可重构性

Conclusion: EventFace成功将RGB面部模型的先验知识迁移到事件域，通过显式建模时空特征实现了高性能的事件相机面部识别，为隐私友好型生物识别系统提供了新方向
...
<details>
  <summary>Details</summary>
Motivation: 现有视频Transformer方法（因子化或基于窗口的自注意力）会分割视频中感兴趣区域间的时空相关性，限制了模型捕捉运动和长程依赖的能力。受人类视觉系统启发，认为时空信息的重要性在不同时间尺度上不同，注意力在时间上是稀疏分配的（通过扫视和凝视行为）。

Method: 提出OG-ReG Transformer双路径网络：Glance路径提取粗粒度整体时空信息，Gaze路径补充局部细节。这种设计模拟人类视觉的扫视（整体概览）和凝视（局部细节）行为。

Result: 在Kinetics-400、Something-Something v2和Diving-48数据集上取得了最先进的结果，证明了其竞争性能。

Conclusion: 通过模拟人类视觉系统的扫视和凝视机制，OG-ReG Transformer能够更有效地捕捉视频中的时空信息，在多个视频任务中表现出色。
...
TL;DR: 提出全局视频引导的多模态翻译框架，通过语义编码器和向量数据库检索构建上下文集，使用注意力机制聚焦相关视觉内容，在长视频翻译中显著超越基线模型。


<details>
  <summary>Details</summary>
Motivation: 现有视频引导多模态翻译方法大多依赖局部对齐的视频片段与字幕一一配对，限制了在长视频中捕捉全局叙事上下文的能力，需要克服这一局限性。
...
Conclusion: 提出的全局视频引导多模态翻译框架通过构建语义相关的上下文集和创新的注意力机制，有效解决了长视频中全局叙事上下文捕捉的问题，在翻译任务中取得了优异性能。
...
TL;DR: 提出FedDAP框架，通过构建领域特定的全局原型来解决联邦学习中跨领域数据导致的领域偏移问题，提升模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在现实场景中面临跨领域数据导致的严重领域偏移问题，现有基于原型的方法存在两个关键限制：1）每个类别只构建单一全局原型，未保留领域信息；2）特征-原型对齐是领域无关的，强制所有客户端与全局原型对齐而不考虑领域来源。

Method: 提出FedDAP框架：1）通过相似性加权融合机制，在同一领域内聚合客户端本地原型，构建领域特定的全局原型；2）在本地训练中，将本地特征与同领域原型对齐，同时鼓励与不同领域原型分离；3）这种双重对齐增强本地层面的领域特定学习，使全局模型能够泛化到不同领域。

Result: 在DomainNet、Office-10和PACS三个数据集上进行广泛实验，证明了所提框架在解决领域偏移挑战方面的有效性。

Conclusion: FedDAP通过构建领域感知原型和领域特定的特征-原型对齐机制，有效解决了联邦学习中的跨领域数据问题，提升了模型在多样化领域上的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有声源定位方法大多基于对比学习的特征匹配，缺乏显式推理和验证机制，在复杂声学场景中效果有限。受人类元认知过程启发，希望利用多模态大语言模型的内在推理能力来解决这一问题。
...
Conclusion: 该工作证明了利用MLLM推理能力进行声源定位的有效性，为复杂声学场景下的定位问题提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: LiDAR语义分割中的半监督学习常受噪声伪标签导致的错误传播和确认偏差问题困扰，需要提升伪标签质量

Method: 提出RePL框架，通过掩码重建识别和修正伪标签中的潜在错误，并设计专门的训练策略。还提供了伪标签细化有益条件的理论分析
...
TL;DR: VGGT-SLAM++ 是一个完整的视觉SLAM系统，利用VGGT的几何丰富输出，通过融合VGGT前馈变换器和Sim(3)解算的视觉里程计、基于DEM的图构建模块以及后端优化，实现大规模精确建图且内存有界。


<details>
  <summary>Details</summary>
Motivation: 先前基于变换器的SLAM管道（如VGGT-SLAM）主要依赖稀疏回环检测或全局Sim(3)流形约束，这会导致短期姿态漂移。需要恢复高频局部光束法平差来稳定轨迹并减少漂移。

Method: 系统包含：1）融合VGGT前馈变换器和Sim(3)解算的视觉里程计前端；2）为每个VGGT子图构建密集平面规范DEM，分割为补丁并计算DINOv2嵌入，将子图整合到共视图中的DEM图构建模块；3）在共视窗口内使用视觉位置识别模块检索空间邻居，触发频繁局部优化的后端。

Result: 在标准SLAM基准测试中，VGGT-SLAM++实现了最先进的精度，显著减少了短期漂移，加速了图收敛，并通过紧凑的DEM图块和次线性检索保持了全局一致性。

Conclusion: VGGT-SLAM++通过恢复高频局部光束法平差和空间校正后端，解决了先前变换器基SLAM系统的短期姿态漂移问题，实现了大规模精确建图且内存有界。
...
TL;DR: CloudMamba：一种基于CNN-Mamba混合架构的双尺度云检测框架，采用不确定性引导的两阶段策略，在薄云区域和边界细节上表现优异，计算复杂度线性。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习云检测方法通常采用单阶段像素级二值分割，但在薄云区域存在模糊性和不确定性，难以准确处理碎片化云和边界细节。

Method: 1) 不确定性引导的两阶段云检测策略：第一阶段分割后，通过不确定性估计模块量化薄云分割置信度，第二阶段对低置信度困难区域进行细化分割；2) 基于CNN-Mamba混合架构的双尺度Mamba网络，同时捕捉云的大尺度结构特征和小尺度边界细节。

Result: 在GF1_WHU和Levir_CS公开数据集上的大量实验表明，该方法在多个分割精度指标上优于现有方法，同时具有高效率和过程透明度。

Conclusion: CloudMamba通过不确定性引导的两阶段策略和双尺度Mamba网络，有效解决了薄云区域的模糊性问题，并准确处理了碎片化云和边界细节，在保持线性计算复杂度的同时实现了优异的云检测性能。
...
TL;DR: PASS是一个智能MRI框架，利用视觉语言模型指导深度展开网络，实现任务导向的快速成像，通过个性化采样和异常感知先验提升临床相关区域的成像质量。


<details>
  <summary>Details</summary>
Motivation: 传统加速MRI方法优化通用图像质量，缺乏对特定临床任务的适应性，导致成像效率不高且临床相关性不足。

Method: 提出PASS框架，包含三个核心组件：基于物理模型的深度展开重建网络、生成患者特定k空间轨迹的采样模块、以及从预训练VLM提取的异常感知先验，该先验指导采样和重建过程。

Result: PASS在不同解剖结构、对比度、异常情况和加速因子下均获得优越的图像质量，显著提升下游诊断任务（包括细粒度异常检测、定位和诊断）的性能。

Conclusion: 通过将VLM的高层临床推理与可解释的物理感知网络相结合，PASS实现了任务导向的个性化MRI加速，直接改善临床诊断效果。
...
TL;DR: 该论文从监控系统视角重新审视物理对抗攻击，强调时间持续性、多模态感知、载体真实性和系统级目标的重要性，而非孤立地评估单帧攻击效果。


<details>
  <summary>Details</summary>
Motivation: 现有物理对抗攻击研究多关注孤立图像基准测试，而实际监控系统涉及人员检测、多目标跟踪、可见光-红外感知等多种因素，需要从系统层面重新评估攻击的有效性和威胁模型。

Method: 采用四部分分类法组织现有研究：时间持续性、感知模态、载体真实性和系统级目标。通过分析多目标跟踪、双模态可见光-红外规避和可控服装等最新成果，展示该领域的变化趋势。

Result: 提出了监控导向的物理攻击评估框架，总结了当前评估实践和未解决问题，包括距离鲁棒性、相机管道变化、身份级指标和激活感知测试等。

Conclusion: 监控系统的鲁棒性不能仅从孤立的单帧基准测试中可靠判断，必须作为随时间推移、跨传感器、在现实物理部署约束下展开的系统问题来考察。
...
TL;DR: RefineAnything：一种多模态扩散模型，专注于区域特定的图像细化，通过聚焦-细化策略和边界一致性损失，在保持背景严格不变的同时恢复局部细节。


<details>
  <summary>Details</summary>
Motivation: 现有图像生成模型在处理局部细节（如文本、徽标、细结构）时经常出现细节崩溃，而指令驱动的编辑模型往往忽视细微局部缺陷或意外改变背景，特别是当感兴趣区域只占输入图像一小部分时。

Method: 提出Focus-and-Refine策略：通过裁剪和重分配分辨率预算到目标区域来改善局部重建；使用混合掩码粘贴保证严格背景保持；引入边界一致性损失减少接缝伪影。支持基于参考和无参考两种细化方式。

Result: 在RefineEval基准测试中，RefineAnything相比竞争基线取得显著改进，实现近乎完美的背景保持，为高精度局部细化提供了实用解决方案。

Conclusion: RefineAnything通过区域聚焦的细化策略和边界一致性损失，有效解决了局部细节恢复和背景保持的平衡问题，为区域特定图像细化提供了有效的多模态扩散模型。
...
TL;DR: SCT-MOT：一种用于空中无人机群跟踪的框架，通过群体耦合运动建模和轨迹引导特征融合，解决了复杂非线性群体运动和小目标弱视觉线索带来的挑战。


<details>
  <summary>Details</summary>
Motivation: 现有方法独立建模每个目标，忽略了群体级运动依赖关系，且运动预测与外观表征的有限集成削弱了时空一致性，导致在视觉模糊和杂乱环境中难以维持连贯轨迹和可靠关联。

Method: 1. 群体运动感知轨迹预测模块：从群体级视角联合建模历史轨迹和姿态感知外观特征；2. 轨迹引导时空特征融合模块：将预测位置与历史视觉线索对齐，并与当前帧特征深度融合。

Result: 在三个公开数据集上验证，轨迹预测模块比最先进的EqMotion提升1.21% IDF1，SCT-MOT在复杂群体场景下多个指标上均优于现有跟踪器。

Conclusion: SCT-MOT通过群体耦合运动建模和轨迹引导特征融合，显著提升了空中无人机群跟踪的准确性和鲁棒性，解决了现有方法在复杂非线性群体运动中的局限性。
...
TL;DR: 提出一个深度学习框架，结合FDG-PET/CT影像和时间数据预测非小细胞肺癌患者总生存期，优于仅使用影像的基线方法。


<details>
  <summary>Details</summary>
Motivation: 自动化医学影像预测临床结果（如总生存期）对改善患者预后和个性化治疗规划具有重要潜力。现有方法通常仅使用影像数据，未充分考虑时间因素对生存预测的影响。

Method: 使用ResNet-50处理组织层面的FDG-PET/CT投影生成影像嵌入，结合时间数据（以天为单位的时间范围）预测随时间变化的OS概率。在U-CAN队列（n=556）开发，与仅使用影像的基线方法（在2年或5年等预设时间点预测）进行比较。

Result: 结合时间数据的模型在AUC上比基线方法提升4.3%。临床+IDP特征模型表现良好，影像与临床+IDP模型的集成达到最佳性能（AUC=0.788）。模型能对患者进行风险分层（高/低风险），显著性分析热图显示肿瘤区域是关键预测结构。

Conclusion: 该方法提供了一个自动化框架，将OS预测参数化为时间函数，证明了结合影像和表格数据在改善生存预测方面的潜力。
...
Method: 提出能量正则化空间掩码（ERSM）框架，将特征选择重新表述为可微分的能量最小化问题。在标准卷积骨干网络中嵌入轻量级的能量掩码层，为每个视觉标记分配由一元重要性成本和成对空间一致性惩罚组成的标量能量。

Result: ERSM在卷积架构上验证有效，产生涌现稀疏性、提升对结构化遮挡的鲁棒性、生成高度可解释的空间掩码，同时保持分类精度。学习的能量排序在基于删除的鲁棒性测试中显著优于基于幅度的剪枝。

Conclusion: ERSM作为一种内在去噪机制，无需像素级监督即可隔离语义对象区域，为特征选择提供了更自主和自适应的方法。
...
TL;DR: Q-Zoom：一种查询感知的自适应高分辨率感知框架，通过动态门控和自蒸馏区域建议网络，实现从粗到细的高效处理，显著加速MLLM推理并保持或提升精度。


<details>
  <summary>Details</summary>
Motivation: 当前MLLM的高分辨率全局缩放方法存在视觉冗余令牌过多的问题，导致二次自注意力机制计算负担重，推理吞吐量严重受限，且忽略了空间稀疏性和查询意图。

Method: 1. 轻量级动态门控网络：在粗粒度全局特征足够时安全绕过高分辨率处理；2. 自蒸馏区域建议网络：从中间特征空间精确定位任务相关感兴趣区域；3. 一致性感知生成策略和全自监督蒸馏范式优化模块；4. 连续时空对齐方案和针对性微调融合密集局部ROI与粗粒度全局布局。

Result: 在Qwen2.5-VL-7B上，Q-Zoom在文档和OCR基准上加速推理2.52倍，在高分辨率场景下加速4.39倍，同时匹配基线峰值精度。在最大感知保真度配置下，分别超越基线峰值性能1.1%和8.1%。这些改进可无缝迁移到Qwen3-VL、LLaVA和新兴的基于RL的思维图像模型。

Conclusion: Q-Zoom建立了主导的帕累托前沿，通过查询感知的自适应高分辨率感知，有效解决了MLLM中视觉冗余令牌导致的推理瓶颈问题，实现了效率与精度的双重提升。
...
TL;DR: 提出一种结合GPT生成文本描述与YOLOv5的多模态UI控件检测方法，通过跨注意力模块融合视觉和语义信息，显著提升检测性能，特别是在视觉模糊或语义复杂的控件类别上。


<details>
  <summary>Details</summary>
Motivation: 传统基于像素的UI控件检测方法面临视觉模糊、设计多样性和缺乏上下文线索的挑战，需要更鲁棒和上下文感知的解决方案。

Method: 提出YOLOv5的多模态扩展，集成GPT生成的UI图像文本描述，通过跨注意力模块将视觉特征与文本嵌入的语义信息对齐，比较了三种融合策略：元素级加法、加权求和和卷积融合。

Result: 在包含23个控件类别、超过16,000张标注UI截图的数集上评估，所有融合策略均优于基线YOLOv5，其中卷积融合表现最佳，在语义复杂或视觉模糊的类别上获得显著提升。

Conclusion: 融合视觉和文本模态能显著增强UI元素检测，特别是在视觉信息不足的边缘情况下，为软件测试、无障碍支持和UI分析工具开辟了有前景的方向。
...
TL;DR: POS-ISP提出序列级强化学习框架，将模块化ISP优化转化为全局序列预测问题，一次性预测整个模块序列和参数，通过终端任务奖励优化，避免中间监督和冗余执行。
...
Method: 提出POS-ISP序列级RL框架，将模块化ISP优化建模为全局序列预测问题。该方法在单次前向传播中预测整个模块序列及其参数，并使用终端任务奖励优化管道，无需中间监督和冗余执行。

Result: 在多个下游任务上的实验表明，POS-ISP提高了任务性能，同时降低了计算成本，展示了序列级优化作为任务感知ISP的稳定高效范式。

Conclusion: POS-ISP通过序列级强化学习框架有效解决了模块化ISP优化中的挑战，提供了一种稳定且高效的任务感知ISP优化方法。
...
TL;DR: 提出Grounded Forcing框架，通过双记忆KV缓存、双参考RoPE注入和非对称邻近重缓存三种机制，解决自回归视频生成中的语义遗忘、视觉漂移和可控性丢失问题，实现长期一致的视频合成。
...
Conclusion: Grounded Forcing通过将生成过程锚定在稳定的语义核心同时适应灵活的局部动态，为无限视野的自回归视频合成提供了稳健的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 比特流损坏视频解码常产生严重的时空伪影和内容失真，需要开发有效的恢复方法来应对实际应用中的比特流损坏问题。

Method: 通过组织挑战赛，提供数据集和评估协议，收集并分析参赛方法，建立统一的基准测试平台。

Result: 挑战赛展示了该任务的难度，总结了主要技术趋势，为未来研究提供了有价值的见解。

Conclusion: 该挑战赛强调了比特流损坏视频恢复这一新兴任务的挑战性，为实际应用中鲁棒视频恢复的未来研究提供了重要参考。
...
TL;DR: 本文提出对抗性走私攻击，利用人机能力差距将有害内容编码为人类可读但AI无法识别的视觉格式，从而绕过多模态大语言模型的内容审核。


<details>
  <summary>Details</summary>
Motivation: 随着多模态大语言模型被部署为自动内容审核器，作者发现了一个关键威胁：对抗性走私攻击。这种攻击不同于传统的对抗性扰动和越狱攻击，它利用人机能力差距，让有害内容能够绕过AI检测而传播。

Method: 将走私攻击分为两种途径：感知盲区（破坏文本识别）和推理阻断（在成功识别文本后抑制语义理解）。构建了包含1700个对抗性走私攻击实例的SmuggleBench基准，评估了GPT-5和Qwen3-VL等先进模型的脆弱性，并通过感知和推理视角分析了根本原因。

Result: 评估显示，无论是专有模型还是开源模型都对此威胁高度脆弱，攻击成功率超过90%。分析发现三个根本原因：视觉编码器能力有限、OCR的鲁棒性差距以及领域特定对抗样本的稀缺性。

Conclusion: 对抗性走私攻击是多模态大语言模型内容审核的一个严重威胁。作者初步探索了通过思维链推理和对抗性训练等缓解策略，并公开了代码和基准以促进进一步研究。
...
TL;DR: 压缩感知攻击比像素空间攻击更有效，压缩通过决策空间缩减放大对抗脆弱性


<details>
  <summary>Details</summary>
Motivation: 图像压缩是现代视觉系统的普遍组件，但其对对抗鲁棒性的影响尚未被充分理解。压缩可能放大对抗脆弱性，特别是在压缩表示中直接攻击的场景。

Method: 研究压缩表示中的直接攻击，提出压缩感知攻击方法，分析压缩通过决策空间缩减（非可逆、信息丢失的变换）收缩分类边界并增加对扰动的敏感性。

Result: 在相同扰动预算下，压缩感知攻击比像素空间攻击显著更有效。实验验证了压缩在循环部署设置中的关键脆弱性。

Conclusion: 压缩作为对抗放大器，通过决策空间缩减增加深度图像分类器的脆弱性，揭示了压缩在循环部署设置中的安全风险。
...
TL;DR: 论文系统审计了面部关键点检测中的公平性问题，发现视觉混淆因素（头部姿态和图像分辨率）比人口统计属性影响更大，消除混淆后性别和种族差异消失，但年龄偏见依然存在。


<details>
  <summary>Details</summary>
Motivation: 人机交互的公平性依赖于机器人感知模型的可靠性。虽然人口统计偏见在高级面部分析任务中被广泛研究，但在面部关键点检测中的偏见尚未探索。公平性问题可能出现在低级视觉组件中，并通过人机交互管道传播，不公平地影响弱势群体。

Method: 引入受控统计方法来分离人口统计效应与混淆视觉因素（特别是头部姿态和图像分辨率）。评估标准代表性模型，分析年龄、性别和种族偏见。

Result: 混淆视觉因素（头部姿态和图像分辨率）的影响远大于人口统计属性。消除这些混淆因素后，性别和种族间的性能差异消失。但发现统计显著的年龄相关效应，对老年人的偏见更高。

Conclusion: 公平性问题可能出现在低级视觉组件中，并可能通过人机交互管道传播，不公平地影响弱势群体。审计和纠正此类偏见是构建可信和公平的机器人感知系统的必要步骤。
...
TL;DR: 提出一种针对混合自回归-扩散模型的稳定强化学习框架，通过多轨迹期望和不确定性感知优化解决梯度噪声问题


<details>
  <summary>Details</summary>
Motivation: 强化学习已成功应用于自回归和扩散模型，但扩展到混合AR-扩散框架仍具挑战性，主要问题包括交错推理和噪声对数概率估计，扩散头在训练动态中引入噪声梯度导致不稳定和性能早熟
...
Result: 在多个基准测试中，该方法相比基线GRPO和预RL模型，在视觉质量、训练稳定性和空间结构理解方面均取得一致改进
...
TL;DR: CAAP是一个针对掌纹识别的捕获感知对抗性补丁框架，通过交叉形补丁拓扑和三个模块实现物理可实现的攻击，在多个数据集上展示了强大的攻击性能和跨模型迁移能力。


<details>
  <summary>Details</summary>
Motivation: 掌纹识别广泛应用于安全关键应用，但现有研究主要局限于数字环境，对物理可实现攻击的鲁棒性理解不足，特别是未能充分考虑掌纹的纹理主导特性和物理采集过程中的失真。

Method: 提出CAAP框架：采用交叉形补丁拓扑扩大空间覆盖；包含三个模块：ASIT（输入条件补丁渲染）、RaS（随机捕获感知模拟）和MS-DIFE（特征级身份破坏指导）；学习可在输入间重复使用的通用补丁。

Result: 在Tongji、IITD和AISEC数据集上评估，CAAP在通用CNN骨干网络和掌纹特定识别模型上都实现了强大的无目标和有目标攻击性能，具有良好的跨模型和跨数据集迁移性；对抗训练只能部分降低攻击成功率，仍存在显著残余漏洞。
...
TL;DR: 将分位数回归应用于现有卫星数据树高估算模型，为风险敏感场景提供统计校准的不确定性量化
...
Result: 模型能够提供统计校准的不确定性估计，且不确定性结果与遥感中的已知挑战（如地形复杂性、植被异质性）相关，表明模型在更复杂条件下信心较低

Conclusion: 分位数回归是增强现有树高估算模型的有效方法，能够提供有意义的不确定性量化，有助于在生态监测和生物量评估中做出更明智的决策
...
TL;DR: 首个生成式照片马赛克方法，使用扩散模型生成拼贴图像，解决了传统颜色匹配方法的多样性和结构一致性限制


<details>
  <summary>Details</summary>
Motivation: 传统照片马赛克方法依赖大量拼贴图像和基于颜色的匹配，限制了多样性和结构一致性。需要一种既能保持语义表达又能保证结构连贯的新方法。

Method: 提出生成式照片马赛克框架，使用基于扩散模型的生成方法，通过参考图像进行条件生成。采用低频条件扩散机制对齐全局结构，同时保留提示驱动的细节。利用少样本个性化扩散技术，无需大量图像集合即可生成用户特定或风格一致的拼贴。

Result: 该方法能够生成语义表达丰富且结构连贯的照片马赛克，有效克服了基于匹配方法的根本限制。通过个性化扩散技术，能够生成用户特定或风格一致的拼贴图像。
...
TL;DR: IQ-LUT通过集成插值、量化和残差学习，在显著减小LUT存储空间的同时提升超分辨率图像质量，相比ECNN减少50倍存储
...
Method: 1. 将插值和量化集成到单输入多输出ECNN中，大幅减少索引空间和LUT大小；2. 集成残差学习减少对LUT位深的依赖，提升训练稳定性并优先重建细节；3. 基于知识蒸馏的非均匀量化优化量化级别，减少存储并补偿量化损失

Result: 实验表明该方法大幅降低存储成本（相比ECNN减少高达50倍），同时实现更优的超分辨率质量

Conclusion: IQ-LUT通过创新的插值-量化集成、残差学习和知识蒸馏指导的非均匀量化，有效解决了LUT方法的存储瓶颈问题，在保持高质量的同时显著减少存储需求
...
TL;DR: 提出基于Unity的虚拟扫描框架生成合成3D扫描数据集，解决真实数据采集成本高的问题


<details>
  <summary>Details</summary>
Motivation: 基于学习的3D场景重建和物体补全方法需要大量配对数据（部分扫描+完整几何），但真实扫描系统获取此类数据成本高、耗时长，特别是需要遮挡区域的精确真值时

Method: 1. 在Unity中实现虚拟扫描框架，模拟真实扫描器行为（可配置分辨率、测量范围、距离相关噪声等参数）
2. 采用基于射线的扫描而非直接网格采样，从虚拟视点进行扫描，真实模拟传感器可见性和遮挡效应
3. 使用全景图像为点云着色
4. 与程序化室内场景生成管道集成，自动生成多样化房间布局和家具布置
5. 创建V-Scan数据集，包含合成室内扫描、物体级部分点云、体素遮挡网格和完整几何真值

Result: 开发了V-Scan数据集，为基于学习的场景重建和物体补全方法提供有价值的监督训练和评估资源
...
<details>
  <summary>Details</summary>
Motivation: 现有弱监督语义分割方法通常将语义识别和对象定位耦合在一起，导致模型只关注稀疏的判别区域。基础模型虽然潜力巨大，但现有方法仍采用紧密耦合的优化范式，难以有效缓解伪标签噪声，且依赖耗时的多阶段重训练或不稳定的端到端联合优化。

Method: 提出ModuSeg框架，通过解耦对象发现和语义分配：1）使用通用掩码提议器提取具有可靠边界的几何提议；2）利用语义基础模型构建离线特征库，将分割转化为非参数特征检索过程；3）提出语义边界净化和软掩码特征聚合策略，缓解边界模糊和量化误差，提取高质量类别原型。

Result: 在标准基准数据集上的大量实验表明，该解耦架构无需参数微调就能更好地保留精细边界，并实现了极具竞争力的性能。

Conclusion: ModuSeg通过解耦对象发现和语义分配，提供了一种无需训练的弱监督语义分割解决方案，有效解决了现有方法中的边界模糊和伪标签噪声问题，在保持精细边界的同时实现了高性能。
...
Method: 提出DARE统一框架：1) 分布校正无分类器引导(DR-CFG)，通过动态抑制低语义密度主导token来正则化训练过程；2) 空间表示对齐(SRA)，根据token重要性自适应重新加权交叉注意力图并强制表示一致性。

Result: 在多个基准数据集上的广泛实验表明，DARE能持续提升生成保真度和语义对齐，相比现有方法取得了显著增益。

Conclusion: DARE通过分布去偏和空间一致性两个角度改进扩散模型中的语义引导，有效解决了语义重要token被忽视的问题，为文本到视频生成提供了更平衡和语义对齐的解决方案。
...
TL;DR: PRISM提出了一种物理结构化的去雾框架PSAR，联合重建清晰场景和散射变量，通过在线非均匀雾合成和选择性自蒸馏适应方案解决真实世界去雾挑战。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像去雾面临非均匀雾分布、多光源空间变化照明以及配对的真实有雾-干净数据稀缺等挑战，需要更可靠的方法处理复杂区域和混合光照条件。

Method: 提出PSAR框架，在大气散射模型下联合重建清晰场景和散射变量；设计在线非均匀雾合成管道和选择性自蒸馏适应方案，让模型从高质量感知目标选择性学习，利用内在散射理解审计残留雾并指导自我精炼。

Result: 在真实世界基准测试上的广泛实验表明，PRISM在真实世界图像去雾任务上实现了最先进的性能。

Conclusion: PRISM通过物理结构化框架和合成到真实域的适应策略，有效解决了真实世界去雾的挑战，在复杂场景和混合光照条件下表现出色。
...
TL;DR: AnchorSplat：基于3D几何先验的锚点对齐高斯表示框架，用于场景级3D重建，减少高斯数量并提升重建质量


<details>
  <summary>Details</summary>
Motivation: 现有前馈高斯重建模型采用像素对齐公式，将每个2D像素映射到3D高斯，导致高斯表示与输入图像紧密耦合，限制了场景级重建的效率和保真度

Method: 提出锚点对齐高斯表示，由3D几何先验（稀疏点云、体素或RGB-D点云）引导；使用高斯细化器通过少量前向传递调整中间高斯；实现与图像分辨率和视图数量无关的几何感知可渲染3D高斯

Result: 在ScanNet++ v2 NVS基准测试中达到SOTA性能，相比先前方法具有更好的视图一致性和显著更少的高斯基元

Conclusion: AnchorSplat通过锚点对齐设计和几何先验引导，实现了高效、高质量的3D场景重建，显著减少了所需高斯数量并提升了计算效率
...
TL;DR: LIANet是一种基于坐标的神经表示方法，通过仅输入时空坐标来重建卫星图像，可作为用户友好的地理空间基础模型替代方案，无需原始卫星数据即可适应下游任务。


<details>
  <summary>Details</summary>
Motivation: 传统地理空间基础模型(GFMs)需要大量数据访问和预处理工作，给终端用户带来负担。LIANet旨在消除这些开销，提供更友好的替代方案，让用户仅凭标签就能进行微调。

Method: LIANet是一种基于坐标的神经表示，将多时相地球观测数据建模为连续时空神经场。仅需空间和时间坐标即可重建卫星图像，预训练后可通过微调适应语义分割、像素级回归等下游任务。

Result: 在不同规模的目标区域上预训练LIANet，微调后在下游任务中表现优异，与从头训练或使用现有GFMs相比具有竞争力。代码和数据集已开源。

Conclusion: LIANet提供了一种用户友好的地理空间基础模型替代方案，消除了数据访问和预处理负担，仅需标签即可微调，在多种下游任务中表现良好。
...
TL;DR: 提出RePaste方法处理工业异常检测中正常样本定义模糊的问题，通过重新粘贴高异常分数区域来增强学习，在MVTec AD基准测试中取得SOTA性能


<details>
  <summary>Details</summary>
Motivation: 传统异常检测假设训练数据只有正常样本，但现实中正常样本定义模糊（如小划痕、污渍是否可接受）。工业场景中，随着设备升级，对正常样本的定义会变化，现有方法未考虑这种规范变化

Method: 提出RePaste方法：将前一步中高异常分数区域重新粘贴到下一步的输入中，通过迭代增强学习来处理正常样本的模糊性。同时提出新的场景和评估指标来适应实际应用中的规范变化

Result: 在MVTec AD基准测试的新场景中，RePaste在提出的评估指标上达到最先进性能，同时保持高AUROC和PRO分数

Conclusion: 该研究解决了工业异常检测中正常样本定义模糊的问题，提出的RePaste方法和新评估指标能更好地适应实际应用中的规范变化需求
...
TL;DR: SurFITR是一个用于监控图像伪造检测和定位的数据集，针对现有伪造检测模型在监控场景下泛化能力不足的问题，提供大量具有法证价值的篡改图像。


<details>
  <summary>Details</summary>
Motivation: 随着开放访问图像生成模型的进步，伪造视觉证据的风险增加。现有伪造检测模型在监控场景下表现不佳，因为监控图像中的篡改通常是局部、细微的，且场景具有多样视角、小目标、遮挡和低视觉质量等特点。

Method: 通过多模态LLM驱动的流水线生成具有法证价值的图像，实现语义感知的细粒度编辑。数据集包含超过137k张不同分辨率和编辑类型的篡改图像，使用多种图像编辑模型生成。

Result: 实验表明，现有检测器在SurFITR上性能显著下降，而在SurFITR上训练则能显著提升域内和跨域性能。

Conclusion: SurFITR填补了监控图像伪造检测的空白，为开发更鲁棒的检测模型提供了有价值的资源，已在GitHub上公开可用。
...
TL;DR: 本文分析了地面处理架构的局限性，评估了星上处理在EO服务中的价值，以IRIDE烧毁区域制图服务为例，展示了星上智能如何支持更高空间细节、更小可检测事件和更快系统响应。


<details>
  <summary>Details</summary>
Motivation: 当前EO服务（如CEMS、EFFIS、CLMS）依赖地面处理，受下行链路延迟、带宽限制和自主观测优先级能力不足的制约。需要探索星上处理来克服这些限制，提升服务响应能力和信息提取效率。

Method: 以IRIDE（意大利国家EO计划）为框架，采用"星座的星座"架构，集成异构传感技术。重点分析Hawk for Earth Observation（HEO）的星上数据产品生成能力，通过IRIDE烧毁区域制图服务作为案例研究，对比地面处理与星上处理的差异。

Result: 星上智能支持：1）更高空间细节（亚3米地面采样距离）；2）更小可检测事件（最小制图单元3公顷）；3）改善系统响应性。HEO能力作为补充层，提供图像驱动的预分类，支持下游应急和土地管理工作流。

Conclusion: 星上智能为新兴低延迟EO服务架构提供了重要操作价值，不是取代现有Copernicus服务，而是作为补充层，通过早期信息提取提升整体服务能力。
...
TL;DR: 提出两种改进半监督语义分割的方法：1) 使用标注图像的类别标签和对应区域粘贴到未标注图像；2) 训练模型使未标注图像的预测更接近标注图像。在医学图像数据集上平均mIoU提升2.07%。


<details>
  <summary>Details</summary>
Motivation: 语义分割中像素级标注成本高昂，半监督学习利用少量标注图像和大量未标注图像提升性能。传统方法ClassMix使用未标注图像的伪标签进行混合，存在标签不准确的问题，且标注与未标注图像间存在数据质量差异影响特征图。

Method: 1) 提出新混合方法：将标注图像的类别标签和对应图像区域粘贴到未标注图像及其伪标签图像上；2) 引入一致性训练方法：训练模型使未标注图像的预测更接近标注图像的预测。

Result: 在Chase和COVID-19医学图像数据集上的实验表明，相比传统半监督学习方法，平均mIoU提升了2.07%。

Conclusion: 通过改进数据混合策略和增强标注-未标注图像间的一致性，有效提升了半监督语义分割的性能，特别是在医学图像分割任务中。
...
TL;DR: 提出UPDP方法，通过生成式过滤机制合成隐私过滤但保留病理信息的放射影像，结合ID过滤报告，实现跨医院数据共享，在保护隐私的同时保持诊断效用。


<details>
  <summary>Details</summary>
Motivation: 大规模放射学数据对开发医疗AI系统至关重要，但医院间共享受隐私限制。现有去标识化研究主要关注移除可识别信息，但去标识化数据是否仍能保持大规模视觉语言模型训练和跨医院迁移的效用尚不明确。

Method: 提出效用保持的去标识化管道(UPDP)：1) 编译隐私敏感术语黑名单和病理相关术语白名单；2) 对放射影像使用生成式过滤机制，合成隐私过滤但保留病理信息的对应图像；3) 结合ID过滤报告，实现安全跨医院共享。

Result: 在公共胸部X光基准测试中，方法有效移除隐私敏感信息同时保留诊断相关病理线索。在去标识化数据上训练的模型保持与原数据相当的诊断准确性，而身份相关准确性显著下降，证实有效隐私保护。跨医院设置中，去标识化数据与本地数据结合可获得更好性能。

Conclusion: UPDP方法能够在保护隐私的同时保持放射学数据的诊断效用，支持跨医院数据共享和大规模医疗AI模型开发，为医疗数据隐私保护与效用平衡提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 儿童性虐待图像(CSAI)分类研究面临法律和伦理限制，无法公开分享数据集，这阻碍了研究的可重复性和进展。

Method: 创建CSA-Graphs数据集，提供结构表示而非原始图像，包含两种图模态：描述物体关系的场景图和编码人体姿态的骨架图。

Result: 实验表明两种表示都保留了CSAI分类的有用信息，结合使用能进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 肾结石是泌尿科常见疾病，目前分析方法依赖术后标本，无法在术前快速分类，限制了个性化治疗和预防复发。
...
TL;DR: 该论文将知识库视觉问答重新定义为搜索代理问题，通过多步决策过程让代理自主选择检索、回答等动作，使用自动化收集的轨迹数据进行微调，在多个数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有检索增强生成方法采用固定流水线设计，难以适应多样化问题类型，且将检索与推理分离，导致检索证据与问题对齐不佳。需要更灵活、集成的方法来改进知识库视觉问答。

Method: 将KB-VQA重新定义为搜索代理问题，建模为多步决策过程。代理在每一步选择四种动作之一：回答、图像检索、文本检索、图像描述。设计自动化管道收集多步轨迹记录推理过程、工具使用和中间决策，用这些轨迹作为监督进行微调。

Result: 在InfoSeek和E-VQA数据集上的实验表明，该方法实现了最先进的性能，持续优于先前基线，验证了框架的有效性。

Conclusion: 通过将KB-VQA重新构建为搜索代理问题并采用多步决策框架，能够更灵活地适应不同问题类型，实现检索与推理的更好集成，显著提升性能。
...
TL;DR: 提出正交子空间分解框架，将PSMA PET信号分解为MRI可解释的生理包络和正交残差，量化多模态成像的互补性


<details>
  <summary>Details</summary>
Motivation: 现有多模态成像联合表示方法很少明确定义共享信息与模态特定信息的区别，而这一区分在临床上有重要意义，能阐明每种模态的不可替代贡献并指导合理的采集策略

Method: 提出子空间分解框架，将多模态融合重新定义为正交子空间分离问题。使用多参数MRI训练基于强度的非空间隐式神经表示(INR)，将MRI特征向量映射到PET摄取。引入基于奇异值分解的投影正则化，惩罚位于MRI特征流形内的残差分量，强制组织水平生理特性与细胞内PSMA表达之间的数学正交性

Result: 在13名前列腺癌患者上测试，模型显示MRI特征跨越的残差分量被吸收到学习到的包络中，而正交残差在肿瘤区域最大。这表明PSMA PET包含无法从MRI衍生生理描述符中恢复的信号分量

Conclusion: 该分解提供了基于表示几何而非图像转换的模态互补性结构化表征，为多模态成像分析提供了新的理论框架
...
<details>
  <summary>Details</summary>
Motivation: 视觉基础模型（如 DINOv2）虽然特征提取性能优异，但其复杂的高维表示难以解释，阻碍了可解释性分析。

Method: 提出 DINO-QPM 适配器，将 QPM 模型适配到冻结的 DINO 骨干网络上；使用平均池化而非 CLS token，将补丁嵌入直接连接到模型特征；应用稀疏损失最小化空间散射和背景噪声。

Result: DINO-QPM 在分类准确率上超过 DINOv2 线性探针，在可解释性指标（包括新提出的 Plausibility 指标）上优于其他适用于冻结视觉基础模型的方法。

Conclusion: DINO-QPM 成功将 QPM 的可解释性水平作为适配器提供，同时实现了更高的分类精度和更好的解释质量，为视觉基础模型提供了有效的可解释性解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 模型在特定数据集（源域）上训练后，在不同数据集（目标域）上评估时精度显著下降，这是由于成像设备、染色方法等环境条件差异导致的域差异问题。需要实现不依赖域差异的分割方法。

Method: 1) 分离与域差异无关的类别信息和源域特定信息；2) 利用Stochastically Quantized Variational AutoEncoder (SQ-VAE)中的量子向量吸收训练和测试数据之间的域差异。

Result: 在血管分割和细胞核分割数据集上的实验表明，该方法相比传统方法提高了分割精度。

Conclusion: 通过分离域无关信息和利用SQ-VAE吸收域差异，实现了更好的域泛化分割性能，在医学图像分割任务中表现出优越性。
...
TL;DR: 提出基于患者特异性能量建模的纵向多参数MRI几何分析框架，通过去噪分数匹配学习序列空间的能量函数，将纵向评估转化为相对于基线几何的演变分析。


<details>
  <summary>Details</summary>
Motivation: 传统纵向MRI分析依赖空间网络和图像分割，需要监督标签。本文旨在开发无需分割标签的几何框架，通过能量建模在序列空间直接分析组织状态演变。

Method: 将每个体素表示为多序列强度向量，通过去噪分数匹配训练紧凑的隐式神经表示学习能量函数E_θ(u)。基线能量流形作为固定几何参考，纵向评估分析后续扫描在基线能量函数下的分布演变。

Result: 在复发儿科病例中，随访扫描显示能量逐渐偏离，序列空间方向位移指向基线肿瘤相关区域；稳定病例中体素分布保持在低能量盆地。证明患者特异性能量流形可作为纵向分析的无分割几何参考系统。

Conclusion: 患者特异性能量流形为无需显式分割或监督分类的纵向mpMRI分析提供了几何参考系统基础，为神经肿瘤学中基于流形的组织风险追踪开辟了新研究方向。
...
TL;DR: 该论文评估了多种CNN模型在茶叶病害检测上的性能，其中DenseNet201在teaLeafBD数据集上达到99%的最高测试准确率，并采用Grad-CAM、遮挡敏感度分析和对抗训练等技术增强模型可靠性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 茶叶作为全球第二大消费饮料，其病害的精确识别对农业生产至关重要。茶叶病害检测不仅影响经济，也关系到文化传承和农业生产效率。

Method: 评估了多种卷积神经网络模型（包括DenseNet201、MobileNetV2、InceptionV3）在teaLeafBD数据集上的性能。该数据集包含7个类别（6种病害和1个健康类别），采集自多种田间条件。采用Grad-CAM、遮挡敏感度分析和对抗训练技术来增强模型可靠性和可解释性。

Result: DenseNet201在所有CNN模型中表现最佳，达到99%的最高测试准确率。开发了原型系统以在实际农业中应用模型能力。

Conclusion: 深度学习模型在茶叶病害检测和管理中具有实际应用潜力，能够有效分类茶叶病害，为农业生产提供技术支持。
...
TL;DR: INSPATIO-WORLD是一个实时框架，能从单参考视频恢复和生成高保真动态交互场景，通过时空自回归架构和联合分布匹配蒸馏解决空间一致性和交互精度问题。


<details>
  <summary>Details</summary>
Motivation: 当前视频生成方法在空间持久性和视觉真实感方面存在不足，难以支持复杂环境中的无缝导航，需要解决空间一致性和实时交互的挑战。

Method: 提出时空自回归（STAR）架构，包含隐式时空缓存（聚合参考和历史观测到潜在世界表示）和显式空间约束模块（强制几何结构并转换用户交互为精确相机轨迹）。引入联合分布匹配蒸馏（JDMD）使用真实世界数据分布作为正则化指导。

Result: 在空间一致性和交互精度方面显著优于现有SOTA模型，在WorldScore-Dynamic基准测试中实时交互方法排名第一，建立了从单目视频重建4D环境的实用导航流程。

Conclusion: INSPATIO-WORLD通过创新的时空架构和分布匹配技术，成功解决了视频生成中的空间一致性和实时交互问题，为从单目视频重建4D环境的导航提供了实用解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有扩散模型通常将服装生成和虚拟试衣作为独立问题处理，限制了实际工作流程的灵活性。同时，多源异构条件下的时尚图像合成面临属性纠缠和语义干扰的挑战。

Method: 提出特质路由注意力模块，利用专家混合机制动态路由条件特征到最兼容的专家和生成层；开发自动化多视角偏好优化流程，无需人工标注即可构建偏好数据用于直接偏好优化。

Result: 在服装生成和虚拟试衣基准测试中，VersaVogue在视觉保真度、语义一致性和细粒度可控性方面持续优于现有方法。

Conclusion: VersaVogue为时尚生命周期中的设计和展示阶段提供了统一的解决方案，通过动态条件路由和自动化偏好优化实现了高质量的多条件可控时尚合成。
...
TL;DR: PhyEdit是一个利用显式几何模拟作为3D感知视觉指导的图像编辑框架，通过结合可插拔的3D先验和2D-3D联合监督，显著提升了物理准确性和操作一致性。


<details>
  <summary>Details</summary>
Motivation: 现有视觉生成模型在精确空间操作方面存在不足，经常出现物体缩放和定位错误，这主要源于缺乏显式机制来融入3D几何和透视投影，限制了在交互式世界模型中的应用潜力。

Method: 开发PhyEdit框架，利用显式几何模拟作为上下文3D感知视觉指导，结合可插拔的3D先验和2D-3D联合监督；创建RealManip-10K真实世界数据集和ManipEval评估基准。
...
TL;DR: Geo-EVS：基于几何条件的稀疏监督框架，用于自动驾驶中的外推式新视角合成，通过几何感知重投影和伪影引导潜在扩散提升合成质量和几何精度
...
Conclusion: Geo-EVS通过几何条件框架和稀疏监督，有效解决了自动驾驶中外推式新视角合成的挑战，统一了训练和推理的重投影路径，提升了合成质量和下游任务性能。
...
TL;DR: 深度神经网络能预测人类真实性判断，但解释不一致，跨架构一致性弱，表明成功的行为模型的事后解释不能作为认知机制的强证据


<details>
  <summary>Details</summary>
Motivation: 研究深度神经网络预测人类真实性判断时，其归因热图的解释是否可靠。虽然模型能预测人类判断，但这不意味着它们依赖人类类似的信息或揭示了判断背后的线索，需要测试解释的鲁棒性。

Method: 在多个预训练视觉模型上拟合轻量回归头，使用Grad-CAM、LIME和多尺度像素掩码生成归因图。评估模型内部和跨架构的解释一致性，并通过集成模型改进预测和归因。

Result: 多个架构预测性能良好（达到噪声上限约80%），但VGG模型依赖图像质量而非真实性特定方差。架构内部归因稳定（尤其EfficientNetB3和Barlow Twins），但跨架构一致性弱。集成模型改善了人类真实性判断预测并支持像素级归因。

Conclusion: 深度网络能很好预测人类真实性判断，但无法产生可识别的解释。更广泛地说，成功行为模型的事后解释应被视为认知机制的弱证据。
...
TL;DR: GenLCA是一个基于扩散模型的生成式方法，用于从文本和图像输入生成和编辑逼真的全身虚拟化身，支持高质量的面部和全身动画。


<details>
  <summary>Details</summary>
Motivation: 现有方法在生成逼真全身虚拟化身时面临训练数据不足的问题，大多数真实世界视频只提供身体部位的局部观察，导致3D重建质量受限。

Method: 1) 使用预训练的前馈化身重建模型作为可动画的3D标记器，将非结构化视频帧编码为结构化3D标记；2) 提出可见性感知扩散训练策略，用可学习标记替换无效区域，仅在有效区域计算损失；3) 在标记数据集上训练基于流的扩散模型。

Result: GenLCA能够生成多样化和高质量的生成和编辑结果，在逼真度和泛化能力上大幅超越现有解决方案，支持高保真的面部和全身动画。

Conclusion: 该方法有效解决了从部分观察数据训练3D扩散模型的挑战，实现了大规模真实世界视频数据在3D扩散模型训练中的有效利用，为逼真虚拟化身生成提供了新范式。
...
TL;DR: Mem3R是一个流式3D感知模型，通过解耦相机跟踪和几何映射的混合记忆设计，解决了长序列处理中的漂移累积和时间遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 现有的循环模型虽然能实现线性时间推理，但由于压缩潜在记忆容量有限，在长序列中容易积累漂移和出现时间遗忘问题，需要改进长期一致性。

Method: 采用混合记忆设计：1）相机跟踪使用基于轻量级MLP的隐式快速权重记忆，通过测试时训练更新；2）几何映射使用显式的基于令牌的固定大小状态。这种设计解耦了两个任务。

Result: 相比CUT3R，不仅显著提升长序列性能，还将模型参数从793M减少到644M。与TTT3R集成后，在500-1000帧序列上绝对轨迹误差降低达39%。同时保持恒定GPU内存使用和可比较的推理吞吐量。

Conclusion: Mem3R通过创新的混合记忆架构有效解决了流式3D感知中的长期一致性问题，在减少模型规模的同时提升了性能，并兼容现有改进策略，适用于机器人、增强现实等需要长序列处理的场景。
...
<details>
  <summary>Details</summary>
Motivation: 研究不同DNN模型（包括领域特定模型和基础模型）是否以相似的方式编码面部身份，尽管它们在数据集、损失函数和架构上存在差异。这对于模型互操作性、集成设计和生物特征模板安全具有重要意义。

Method: 将不同DNN模型生成的面部图像嵌入视为点云，研究简单的仿射变换是否能够对齐不同模型的面部表示。分析低容量线性映射能否改善跨模型人脸识别性能。

Result: 发现惊人的跨模型兼容性：低容量线性映射显著改善了跨模型人脸识别性能（包括人脸识别和验证任务）。对齐模式在不同数据集上具有泛化性，且在不同模型家族中呈现系统性变化，表明面部身份编码存在表示收敛。

Conclusion: 不同DNN模型在面部身份编码上表现出显著的跨模型兼容性，简单的线性变换就能对齐它们的表示空间。这一发现对模型互操作性、集成设计和生物特征模板安全具有重要启示。
...
TL;DR: ROAM是一种用于病理全切片图像分类的空间感知混合专家MIL聚合器，通过容量约束的熵最优传输实现平衡的区域到专家路由，在多个基准测试中表现出色。


<details>
  <summary>Details</summary>
Motivation: 当前MIL聚合器将所有实例通过共享路径路由，限制了在病理异质性中的专业化能力。混合专家方法虽然提供解决方案，但无约束的softmax路由可能导致专家利用高度不平衡，使混合系统退化为近乎单路径方案。

Method: ROAM将密集补丁包压缩为空间分箱的区域标记，通过容量约束的熵最优传输（Sinkhorn）进行区域到专家分配，强制平衡的专家利用，并通过图正则化Sinkhorn迭代在空间区域图上扩散路由分配。

Result: 在四个WSI基准测试中使用冻结的基础模型补丁嵌入，ROAM性能与强大的MIL和MoE基线相当，在NSCLC泛化（TCGA-CPTAC）上达到外部AUC 0.845 ± 0.019。

Conclusion: ROAM通过空间感知的容量约束最优传输路由，解决了MIL中专家利用不平衡问题，为病理全切片图像分类提供了有效的混合专家聚合方案。
...
TL;DR: AlignPrune是一个噪声鲁棒的动态剪枝模块，通过基于损失轨迹的动态对齐分数更准确识别噪声样本，提升带噪声标签数据的剪枝效果。
...
Method: 提出AlignPrune模块，引入动态对齐分数（DAS）作为基于损失轨迹的评估标准，能更准确识别噪声样本。这是一个即插即用的模块，可无缝集成到现有动态剪枝框架中。

Result: 在五种广泛使用的基准测试上，针对不同类型的噪声和剪枝比例进行实验，AlignPrune相比最先进的基线方法最高提升6.3%的准确率。

Conclusion: AlignPrune为噪声数据下的剪枝提供了一个通用解决方案，鼓励在真实世界场景中进一步探索学习。代码已开源。
...
TL;DR: SUMI方法通过模拟退化-增强学习，将低质量常规CT转换为光子计数CT质量，无需大规模配对数据，显著提升图像质量和下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 光子计数CT（PCCT）相比传统能量积分CT（EICT）具有更优的图像质量，但临床可用性有限，限制了大规模研究和应用。需要一种方法将PCCT的优势"蒸馏"到常规EICT中。

Method: 提出SUMI方法：1）显式建模真实的采集退化过程，将高质量PCCT转换为临床合理的低质量版本；2）学习逆转这一退化过程；3）使用潜在扩散模型，基于1046个PCCT和405,379个EICT预训练自编码器提取通用CT潜在特征；4）构建大规模公开数据集（17,316个增强EICT）。

Result: 1）在外部数据上，SUMI在SSIM和PSNR上分别超越现有方法15%和20%；2）放射科医生评估显示临床实用性提升；3）下游病灶检测性能显著改善：灵敏度提升达15%，F1分数提升达10%；4）发布了通用CT潜在特征和大型公开数据集。

Conclusion: 通过有限的高质量扫描作为参考，可以将新兴成像技术的优势系统性地"蒸馏"到常规CT中，SUMI方法为医疗影像生成任务提供了有价值的工具和资源。
...
TL;DR: 提出Gaussian Wrapping方法，为3D高斯泼溅引入可学习的法向量和衰减公式，推导出闭合形式的法向量场和占据场，实现高质量水密网格重建。


<details>
  <summary>Details</summary>
Motivation: 3D高斯泼溅(3DGS)在快速新视角合成方面表现优异，但其基于不透明度的表示使得表面提取变得困难。与基于有符号距离场或占据场的隐式方法不同，3DGS缺乏全局几何场，现有方法只能依赖启发式方法如TSDF融合。

Method: 1) 为每个高斯元素引入可学习的定向法向量；2) 定义适应的衰减公式，推导出空间任意位置的法向量场和占据场的闭合形式表达式；3) 提出一致性损失和专门的致密化策略，确保高斯元素包裹整个表面；4) 修改可微分光栅化器输出深度作为连续模型的等值面；5) 提出Primal Adaptive Meshing进行感兴趣区域网格化。

Result: 在DTU和Tanks and Temples数据集上达到新的最优性能，能够生成完整的水密网格，网格尺寸远小于同期工作，能够重建薄结构如自行车辐条等困难结构。

Conclusion: Gaussian Wrapping方法为3D高斯泼溅提供了原理性的几何表示，解决了表面提取的根本困难，同时暴露了标准表面评估协议的偏差并提出了更严格的替代方案。
...
TL;DR: 该研究评估了视觉语言模型在文化遗产结构化元数据推断任务上的表现，发现当前模型存在文化偏见和性能不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 虽然视觉语言模型在文化遗产图像描述方面有进步，但从视觉输入推断结构化文化元数据（如创作者、起源、时期）的研究仍然不足，需要系统评估模型在这方面的能力。

Method: 1. 引入多类别跨文化基准测试；2. 使用LLM-as-Judge框架评估语义对齐；3. 通过精确匹配、部分匹配和属性级准确率评估文化推理能力；4. 分析不同文化区域和元数据类型的性能差异。

Result: 模型仅能捕捉碎片化信号，在不同文化和元数据类型上表现出显著性能差异，导致预测结果不一致且缺乏充分依据，表明当前视觉语言模型在结构化文化元数据推断方面存在局限。

Conclusion: 当前视觉语言模型在超越视觉感知的结构化文化元数据推断任务上存在明显局限性，需要改进模型的文化理解和推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过增加潜在表示的通道数来维持高压缩比下的重建质量，但这往往导致潜在表示崩溃，从而降低生成性能。需要从token空间角度解决这一问题。

Method: 1. 研究token数量缩放：在固定潜在预算下调整ViT的patch大小，将token到潜在压缩分解为两个阶段以减少结构信息损失；2. 通过联合自监督训练增强图像token的语义结构，获得更利于生成的潜在表示。

Result: TC-AE在深度压缩下实现了显著改进的重建和生成性能。

Conclusion: 该研究从token空间角度解决了深度压缩中的潜在表示崩溃问题，有望推动基于ViT的视觉生成tokenizer的发展。
...
TL;DR: 评估三种LLM方法（微调、RAG和混合方法）用于从支持工单构建RCA知识库的性能，实验表明生成的知识库能加速RCA任务并提升网络韧性


<details>
  <summary>Details</summary>
Motivation: 通信网络需要高可靠性（"五个9"），但即使有冗余机制也难以保证，需要快速准确的根因分析（RCA）来恢复服务和预防未来中断

Method: 评估三种大型语言模型方法：微调、检索增强生成（RAG）和混合方法，用于从支持工单构建RCA知识库，并使用全面的词汇和语义相似度指标进行比较

Result: 在真实工业数据集上的实验表明，生成的知识库为加速RCA任务和提升网络韧性提供了优秀的起点

Conclusion: LLM方法能有效构建RCA知识库，有助于快速恢复网络服务并预防未来中断，提升网络可靠性
...
TL;DR: 该论文提出"逐步信息性假设"，解释为什么语言模型内部熵动态与外部正确性相关，认为模型通过答案信息性前缀积累真实答案信息，并通过实验验证该假设。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要基于经验观察发现语言模型内部熵动态与外部正确性存在相关性，但缺乏理论解释。核心未解之谜是为什么基于模型预测分布定义的内部熵动态与基于真实答案的外部正确性如此稳健相关。

Method: 提出逐步信息性假设，形式化模型通过答案信息性前缀积累答案相关信息的过程。从最大似然优化推导该假设，并推导可观测的熵动态特征。在多个推理基准和开源LLM上进行实证测试。

Result: 实验验证训练过程确实诱导了逐步信息性假设，正确推理轨迹展现出特征性的条件答案熵模式。该假设在多个推理基准和不同LLM变体中得到证实。

Conclusion: 逐步信息性假设为语言模型内部熵动态与外部正确性之间的相关性提供了理论解释，表明模型通过积累答案相关信息进行正确推理，这一机制通过标准训练流程得到强化。
...
TL;DR: 研究比较了多种AI方法从初级医疗对话中检测抑郁症，发现GPT-OSS零样本方法表现最佳，联合医患对话文本优于单方文本，仅需前128个患者词就能有效检测，支持实时临床决策。


<details>
  <summary>Details</summary>
Motivation: 抑郁症在初级医疗中常被漏诊，而数字转录技术记录的临床对话为从自然对话中检测抑郁症提供了机会。研究旨在探索利用临床音频记录进行自动化抑郁症检测的可行性。

Method: 使用Establishing Focus研究的1,108个初级医疗音频记录（253例抑郁，855例非抑郁），比较了三种监督方法（Sentence-BERT+逻辑回归、LIWC+逻辑回归、ModernBERT）与零样本GPT-OSS方法。分析了医患联合对话与单方对话的效果差异，并测试了不同对话长度下的检测性能。

Result: GPT-OSS零样本方法表现最佳（AUPRC=0.510，AUROC=0.774），LIWC+逻辑回归在监督方法中表现最好（AUPRC=0.500，AUROC=0.742）。医患联合对话文本优于单方配置，医生在抑郁对话中会语言镜像患者。仅需前128个患者词就能实现有意义的检测（AUPRC=0.356，AUROC=0.675）。

Conclusion: 研究表明从临床音频中自动化检测抑郁症是可行的，医患联合对话提供额外信号，短对话片段即可支持实时临床决策。被动收集的临床音频可作为现有筛查流程的低负担补充工具。
...
TL;DR: 该论文提出了一种结合指令拒绝和结构化弃权门的复合架构，通过计算支持度赤字分数来减少大语言模型的幻觉问题，实验表明两种机制具有互补性，结合使用效果最佳。
...
Result: 在50个项目、五种认知机制和三个模型的评估中：单独指令拒绝能显著减少幻觉但仍有过度弃权和残留幻觉；结构化弃权门能保持可回答项目的准确性但会错过自信的虚构；复合架构实现了高整体准确性和低幻觉率，但继承了指令组件的过度弃权特性。
...
TL;DR: CGD-PD是一种轻量级测试时层，通过查询假设及其否定形式、投影到否定一致决策、使用证明驱动的消歧步骤，解决三值逻辑问答中的否定不一致和认知性Unknown问题，显著提升LLM在FOLIO基准上的准确性。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型在三值逻辑问答中存在两个常见失败模式：否定不一致（对H和¬H的回答违反确定性标签映射）和认知性Unknown（即使前提集S蕴含某一方，模型仍因不确定性或不稳定性预测Unknown）。

Method: CGD-PD包含三个步骤：(a) 在H和机械否定的H形式上查询单个三值分类器；(b) 将结果对投影到否定一致决策；(c) 调用证明驱动的消歧步骤，使用有针对性的二元蕴含探针选择性解决Unknown结果，平均仅需4-5次模型调用。
...
TL;DR: 开发了一个基于LLM的糖尿病病例报告时间线提取系统，用于从文本中提取临床事件及其时间信息，并在下游分析中验证了GLP-1药物对呼吸系统并发症的保护作用。


<details>
  <summary>Details</summary>
Motivation: 糖尿病病例报告中的临床病程描述通常使用难以在纵向建模中重用的自然语言表达，这限制了从病例报告中提取结构化时间序列数据的能力。

Method: 构建了包含136篇PubMed开放获取单病例报告的文本时间序列语料库，使用临床专家标注的金标准时间线，评估了LLM自动提取时间线的性能，并在下游进行了糖尿病时间-事件分析。

Result: 最佳性能的LLM（GPT5）在事件覆盖度（0.871）和时间序列准确性（0.843）方面表现优异。下游分析显示GLP-1使用者相比非使用者呼吸系统并发症风险显著降低（HR=0.259，p<0.05）。

Conclusion: LLM能够有效从糖尿病病例报告中提取结构化时间线信息，为临床研究提供可重用的时间序列数据，并验证了GLP-1药物对呼吸系统并发症的保护作用。
...
TL;DR: 研究发现在纯AI代理社交网络中，指令性内容会引发纠正性信号响应，表明合成社会能产生内生的自我调节机制。


<details>
  <summary>Details</summary>
Motivation: 随着自主AI代理在在线环境中广泛互动，需要了解在没有人类干预或集中设计的情况下，合成集体是否能表现出自我调节的社会动态。

Method: 在Moltbook（仅代理社交网络）上研究OpenClaw代理，分析39,026个帖子和5,712条评论。使用指令强度（DI）量化诱导行动的语言，将响应评论分为四类：肯定、纠正信号、负面反应和中性互动。采用混合效应逻辑模型分析数据。

Result: 指令性内容普遍存在（18.4%的帖子DI>0）。更重要的是，纠正信号随DI增加：DI越高的帖子引发纠正回复的概率越高。混合效应模型确认了这种正相关关系，线程内分析也支持首次纠正后出现负面反馈的证据。

Conclusion: 纯合成的、仅由代理组成的社会能够表现出内生的纠正信号机制，其强度与指令性提案的强度正相关，表明AI代理社会具有自我调节能力。
...
TL;DR: Text2DistBench是一个评估LLM从自然语言推断分布知识的阅读理解基准，基于YouTube电影音乐评论构建，要求模型回答关于评论分布的问题。


<details>
  <summary>Details</summary>
Motivation: 现有阅读理解基准主要关注基于具体文本证据的事实性问题，但现实任务需要理解分布信息（如群体趋势、偏好分布）。需要评估LLM从文本集合中推断分布知识的能力。

Method: 构建Text2DistBench基准：基于YouTube电影音乐评论，提供实体元数据和相关评论，要求回答分布问题（如正负评论比例、最频繁讨论话题等）。构建流程完全自动化并可随时间更新纳入新实体。

Result: 多个LLM实验显示：模型显著优于随机基线，但性能在不同分布类型和特征间差异很大。表明当前LLM在分布阅读理解上既有能力也有局限。

Conclusion: Text2DistBench作为实用可扩展的测试平台，有助于评估LLM的分布知识推理能力，揭示了当前模型在此任务上的能力与不足，为未来研究提供价值。
...
TL;DR: 该论文提出了一个研究突厥语族多语言大语言模型跨语言迁移和参数高效适应的理论框架，重点关注阿塞拜疆语、哈萨克语、乌兹别克语、土库曼语和加告兹语，并引入突厥语迁移系数(TTC)来衡量语言间的迁移潜力。


<details>
  <summary>Details</summary>
Motivation: 当前多语言大语言模型主要基于高资源语言训练，导致许多拥有大量使用者的语言（特别是突厥语族语言）在训练数据和评估基准中代表性不足。突厥语族语言具有显著的形态和类型相似性，但数字资源可用性差异很大，这为分析多语言适应策略提供了理想的研究场景。

Method: 整合多语言表示学习和参数高效微调技术（如LoRA），开发描述适应性能如何依赖于模型容量、适应数据规模和适应模块表达性的概念缩放模型。引入突厥语迁移系数(TTC)，这是一个结合形态相似性、词汇重叠、句法结构和文字兼容性的理论度量。

Result: 提出了一个理论框架，用于分析突厥语族内的跨语言迁移和参数高效适应。该框架强调类型相似性如何实现高效的多语言迁移，同时识别在极低资源场景下参数高效适应的结构限制。

Conclusion: 该研究为理解多语言大语言模型在突厥语族中的适应机制提供了理论基础，特别是通过TTC量化语言间迁移潜力，有助于指导未来在资源不平衡的多语言环境中开发更有效的适应策略。
...
TL;DR: SensorPersona：一个基于LLM的系统，通过移动设备传感器数据持续推断用户人格，相比传统聊天历史方法能更全面地理解用户行为模式和心理特征。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理个性化方法主要从聊天历史推断人格，但这种方法只能获取用户自我披露的信息，无法捕捉用户在物理世界中的日常行为模式，限制了全面理解用户人格的能力。

Method: 1) 对连续传感器流进行人格导向的上下文编码以丰富语义；2) 采用分层人格推理，整合片段内和片段间推理来推断物理模式、心理社会特征和生活经历；3) 使用聚类感知的增量验证和时间证据感知更新来适应演化的人格。

Result: 在包含20名参与者、1580小时传感器数据的数据集上评估，SensorPersona在人格提取方面实现了高达31.4%的召回率提升，在人格感知代理响应中达到85.7%的胜率，用户满意度显著提高。

Conclusion: SensorPersona通过移动设备传感器数据持续推断用户人格，相比传统方法能更全面地理解用户行为模式和心理特征，显著提升了LLM代理的个性化能力和用户满意度。
...
<details>
  <summary>Details</summary>
Motivation: 在线平台和用户内容的增长需要强大的内容审核系统，但大型语言模型计算成本高、延迟大，难以大规模部署。需要开发更高效的小型模型解决方案。

Method: 提出Tool-MCoT，一个通过工具增强思维链数据微调的小型语言模型。使用LLM生成工具增强的思维链数据来训练SLM，使其学会有效利用外部工具改进推理和决策。

Result: 实验显示微调后的SLM取得显著性能提升。模型学会选择性使用工具，仅在必要时调用工具，在审核准确性和推理效率之间取得平衡。

Conclusion: Tool-MCoT展示了小型语言模型通过工具增强思维链训练，能够有效提升内容安全审核性能，同时保持推理效率，为可扩展部署提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 尽管ICL在POI预测中显示出潜力，但现有研究缺乏对不同演示选择策略的全面比较分析。为了填补这一空白并为实际应用提供最佳实践指导，需要系统评估各种演示选择方法。

Method: 全面评估现有演示选择方法（随机选择、嵌入选择、任务特定选择）以及简单启发式方法（地理邻近性、时间顺序、序列模式）。在三个真实世界数据集上进行广泛实验。

Result: 简单启发式方法在计算成本和预测准确性方面均优于复杂的嵌入方法。在某些场景下，使用这些简单方法选择演示的LLM甚至优于现有的微调模型，且无需额外训练。

Conclusion: 对于POI预测任务，简单启发式演示选择方法比复杂方法更有效，这为实际应用提供了实用指导，并展示了LLM在无需微调情况下超越传统模型的潜力。
...
TL;DR: LLM框架从肿瘤学电子病历非结构化笔记中提取医学知识，专注于乳腺癌表型提取，性能与经典本体方法相当但更易适应其他癌症类型


<details>
  <summary>Details</summary>
Motivation: 肿瘤学电子病历中大量有价值信息（如化疗结果、生物标志物、肿瘤特征等）存在于非结构化医生笔记中，而非结构化字段。临床研究表明大多数肿瘤科医生更愿意用自然语言记录这些信息，因此需要有效的信息提取方法。

Method: 提出基于LLM的框架来处理医生笔记并提取有价值的医学知识和表型，专注于肿瘤学领域。具体应用在乳腺癌相关表型提取，并与早期使用知识驱动标注系统（配合NCIt本体标注器）的方法进行比较。

Result: 研究表明，基于LLM的信息提取框架可以轻松适应提取表型，其准确性与经典的本体方法相当。一旦训练完成，可以轻松微调以适应其他癌症类型和疾病。

Conclusion: LLM框架为从肿瘤学电子病历非结构化笔记中提取医学知识提供了一种有效且灵活的方法，性能与传统本体方法相当，但具有更好的适应性和可扩展性。
...
TL;DR: 提出了TelcoAgent-Bench和TelcoAgent-Metrics框架，用于评估电信领域多语言LLM代理的性能，重点关注意图识别、工具执行顺序、解决方案正确性和稳定性等指标。


<details>
  <summary>Details</summary>
Motivation: 将大型语言模型代理集成到电信网络中带来了新的挑战，包括意图识别、工具执行和解决方案生成等问题，同时需要考虑不同的操作约束。需要专门针对电信领域的基准测试框架来评估多语言LLM代理的可靠性和操作一致性。

Method: 提出了TelcoAgent-Bench和TelcoAgent-Metrics框架，这是一个电信特定的基准测试框架，用于评估多语言电信LLM代理。该框架评估语义理解能力、与结构化故障排除流程的过程级对齐，以及跨重复场景变化的稳定性。包含一套结构化指标来评估意图识别、有序工具执行、解决方案正确性和跨场景变化的稳定性。

Result: 实验结果显示，虽然最近的指令调优模型能够以合理的方式理解电信问题，但它们通常在一致遵循所需的故障排除步骤和在不同场景变化中保持稳定行为方面存在困难。这种性能差距在无约束和双语设置中更加明显。

Conclusion: 需要专门针对电信领域的基准测试框架来评估LLM代理的可靠性和操作一致性，特别是在多语言环境中。当前模型在遵循结构化流程和保持稳定性方面仍有不足，需要进一步改进。
...
TL;DR: DOVE是一个分布式的文化价值观对齐评估框架，通过比较人类文本分布和LLM生成输出来直接测量对齐程度，解决了现有基准的C³挑战。


<details>
  <summary>Details</summary>
Motivation: 随着LLM在全球部署，对齐其文化价值观对于安全和用户体验至关重要。现有基准面临C³挑战：依赖判别式选择题格式、忽视亚文化异质性、与真实世界开放生成不匹配。

Method: 使用率失真变分优化目标从10K文档构建紧凑的价值码本，将文本映射到结构化价值空间以过滤语义噪声。采用不平衡最优传输测量对齐，捕捉文化内分布结构和亚群体多样性。

Result: 在12个LLM上的实验显示，DOVE具有优越的预测效度，与下游任务相关性达到31.56%，同时仅需每个文化500个样本就能保持高可靠性。

Conclusion: DOVE提供了一个直接、分布式的文化价值观对齐评估框架，有效解决了现有基准的局限性，为LLM文化对齐提供了更准确的测量工具。
...
TL;DR: 论文研究如何提高大语言模型生成解释的忠实性和可追溯性，通过引入"言外行为宏观规划"和"言外行为链提示"方法，在编程教育RAG系统中显著提升了源文本忠实度。


<details>
  <summary>Details</summary>
Motivation: 大语言模型生成的自然语言解释虽然具有说服力，但缺乏可验证性，用户难以确认解释中的主张是否有证据支持。在可解释人工智能领域，这促使研究者关注解释的忠实性和可追溯性，即解释的主张能否基于明确来源并追溯到该来源。

Method: 基于Achinstein的言外行为解释理论，提出"言外行为宏观规划"作为源忠实解释的设计原则，并实例化为"言外行为链提示"方法。该方法将查询扩展为隐含的解释性问题来驱动检索，在编程教育RAG系统中进行实验，使用三本编程教科书作为权威证据源，在90个Stack Overflow问题上测试了6个大语言模型。

Result: 非RAG模型的源忠实度中位数为0%，基线RAG系统的源忠实度中位数较低（22-40%）。言外行为链提示方法在所有模型上都带来了统计显著的源忠实度提升（最高达63%），尽管绝对忠实度仍然中等，且某些模型的提升较弱或不显著。用户研究表明这些提升不会损害用户满意度、相关性或感知正确性。

Conclusion: 言外行为宏观规划和言外行为链提示是提高大语言模型解释源忠实性的有效方法，在编程教育RAG系统中取得了显著改进，同时保持了用户对解释质量的积极感知。
...
TL;DR: BADx：一种新的可扩展度量方法，用于测量LLM在角色扮演情境下的偏见放大，结合了差分偏见评分、角色敏感指数和波动性分析，比静态方法更能揭示上下文敏感偏见。


<details>
  <summary>Details</summary>
Motivation: 现有偏见审计方法（如CEAT、I-WEAT、I-SEAT）主要基于静态嵌入测试，难以捕捉LLM在采用社会角色时的动态偏见变化。需要一种能够测量角色诱导偏见放大的新方法。

Method: 提出BADx度量方法，包含三个组件：基于现有方法的差分偏见评分（BAD）、角色敏感指数（PSI）和波动性（标准差），并集成LIME分析增强可解释性。研究分为两个任务：任务1建立静态偏见基线，任务2应用六种角色框架（边缘化和结构优势）测量BADx、PSI和波动性。

Result: 对五个先进LLM（GPT-4o、DeepSeek-R1、LLaMA-4、Claude 4.0 Sonnet、Gemma-3n E4B）的分析显示：角色上下文显著调节偏见。GPT-4o表现出高敏感性和波动性；DeepSeek-R1抑制偏见但波动性不稳定；LLaMA-4保持低波动性和稳定偏见特征；Claude 4.0 Sonnet实现平衡调节；Gemma-3n E4B达到最低波动性。

Conclusion: BADx方法优于静态方法，能揭示静态方法忽略的上下文敏感偏见。该统一方法为检测五种流行LLM中的动态隐式交叉偏见提供了系统化途径。
...
TL;DR: 该研究提出一种无监督神经网络方法，利用BioClinicalBERT和聚类算法自动将手术转录文本分为紧急、急性和择期三个优先级类别，并通过专家验证和BiLSTM分类器实现高效手术分类。


<details>
  <summary>Details</summary>
Motivation: 手术程序按紧急程度分类对优化患者护理和医疗资源分配至关重要，但面临标记数据有限的问题，需要开发无监督的自动化解决方案。

Method: 1. 使用BioClinicalBERT将手术转录文本转换为高维语义嵌入；2. 采用K-means和深度嵌入聚类(DEC)算法进行无监督聚类；3. 通过改良德尔菲法进行专家验证和精炼；4. 开发结合BiLSTM层和BioClinicalBERT嵌入的神经网络进行分类。

Result: DEC算法在形成紧密且分离良好的聚类方面表现优于K-means；神经网络模型在准确率、精确率、召回率和F1分数等指标上表现出稳健性能，对未见数据具有良好的泛化能力。

Conclusion: 该无监督框架不仅解决了标记数据有限的挑战，还为实时手术优先级排序提供了可扩展且可靠的解决方案，最终提升了动态医疗环境中的运营效率和患者预后。
...
TL;DR: 提出一个结合人类专业知识和LLM的框架，用于检测心理健康咨询中的幻觉和遗漏，相比纯LLM方法显著提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 随着LLM驱动的聊天机器人在心理健康服务中广泛应用，检测幻觉和遗漏对用户安全至关重要。现有LLM作为评判者的方法在医疗高风险场景中表现不佳，细微错误可能导致严重后果。

Method: 提出一个整合人类专业知识和LLM的框架，提取五个分析维度的可解释、领域知情的特征：逻辑一致性、实体验证、事实准确性、语言不确定性和专业适当性。使用传统机器学习模型在这些特征上进行训练。

Result: 在公共心理健康数据集和新的标注数据集上，传统机器学习模型在幻觉检测上达到0.717 F1（自定义数据集）和0.849 F1（公共基准），在遗漏检测上达到0.59-0.64 F1，显著优于纯LLM方法（仅52%准确率）。

Conclusion: 在高风险心理健康应用中，结合领域专业知识和自动化方法比黑盒LLM评判产生更可靠和透明的评估，为解决医疗领域LLM幻觉检测问题提供了有效途径。
...
TL;DR: STDec是一种训练免费的解码方法，通过空间感知和时间感知解码，利用扩散大语言模型解码过程中的时空稳定性，显著提升推理速度同时保持任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前扩散大语言模型解码器大多采用全局置信度阈值，没有显式建模邻近解码状态的局部上下文或预测token ID跨步骤的时间一致性，这限制了解码效率。

Method: 提出STDec解码方法：1）空间感知解码：通过聚合邻近token的解码状态动态生成token自适应阈值；2）时间感知解码：对预测token ID在多个去噪步骤中保持一致的token放宽解码阈值。

Result: 在文本推理和多模态理解基准测试中，STDec显著提升吞吐量同时保持可比的任务性能得分。在MBPP与LLaDA上，实现高达14.17倍加速且得分相当。

Conclusion: STDec是一种简单有效的训练免费解码方法，利用扩散大语言模型解码的时空稳定性，显著加速推理过程，同时与基于缓存的加速方法兼容。
...
TL;DR: 提出一种基于严重程度感知的加权损失函数，用于阿拉伯语医疗文本生成模型的微调，通过动态调整不同临床严重程度病例的损失权重来提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 传统微调方法对所有医疗案例一视同仁，忽略了临床严重程度的差异。在医疗场景中，严重病例的错误具有更高的临床风险，因此需要针对性地优化模型。

Method: 使用软严重程度概率动态调整token级损失贡献，基于微调的AraBERT分类器自动生成严重程度标签和概率分数，在损失层面实现严重程度感知优化。

Result: 在10个不同架构和规模的阿拉伯语大语言模型上验证，严重程度感知优化相比标准交叉熵微调显著提升性能：AraGPT2-Base从54.04%提升到66.14%，AraGPT2-Medium从59.16%到67.18%，Qwen2.5-0.5B从57.83%到66.86%，最高性能达67.18%。

Conclusion: 严重程度感知微调方法能有效提升阿拉伯语医疗文本生成性能，相比非微调基线最高提升12.10%，且在不同模型架构上表现一致稳健。
...
TL;DR: 研究语音语言模型中的上下文学习，发现语速对ICL性能影响显著且会被模仿，而音高和强度影响较小且不被一致复制。诱导头在语音ICL中起因果作用。


<details>
  <summary>Details</summary>
Motivation: 上下文学习在文本语言模型中已广泛研究，但在语音领域仍相对未探索。本研究旨在探究语言和声学特征如何影响语音语言模型中的上下文学习，特别是在文本到语音任务中。

Method: 聚焦文本到语音任务，从两个角度分析ICL：1) 模型从演示中推断任务的准确性（生成正确的语音内容）；2) 模型在输出中模仿演示语音声学特征的程度。研究不同声学特征（语速、音高范围、强度）的影响，并探究诱导头在语音ICL中的作用。

Result: 发现语速强烈影响ICL性能，且会在输出中被模仿；而音高范围和强度对性能影响很小，且不被一致复制。诱导头在语音ICL中起因果作用：消融前k个诱导头会完全消除模型的ICL能力，这与文本ICL的研究结果一致。

Conclusion: 语音语言模型中的上下文学习受到特定声学特征的影响，其中语速是关键因素。诱导头在语音ICL中扮演关键角色，为理解语音语言模型的学习机制提供了重要见解。
...
TL;DR: 本文提出了一种基于严重程度的课程学习策略，用于阿拉伯语医学文本生成，通过从轻度到重度病例的渐进式训练，提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有的阿拉伯语医学文本生成方法通常假设训练样本重要性相同，忽略了临床严重程度的差异，这限制了模型处理复杂或高风险病例的能力。

Method: 提出基于严重程度的课程学习策略：将数据集按严重程度分为轻度、中度和重度三个等级，在微调过程中逐步从简单病例过渡到复杂病例，让模型先学习基本医学模式再处理复杂场景。

Result: 在Medical Arabic Question Answering (MAQA)数据集子集上评估，使用规则方法标注严重程度。结果显示，该方法在所有测试模型上都带来一致性能提升，相比基线模型提升约4-7%，相比传统微调方法提升3-6%。

Conclusion: 基于严重程度的课程学习策略能有效提升阿拉伯语医学文本生成质量，通过渐进式训练使模型更好地处理不同严重程度的医学病例。
...
TL;DR: 研究发现，只有在从头开始训练时，语言模型才会在连续思维链推理中使用叠加表示；在免训练和微调情况下，叠加会崩溃或不被使用，模型会寻找捷径解决方案。


<details>
  <summary>Details</summary>
Motivation: 尽管理论上认为连续思维链推理中的叠加表示能让模型同时维护多个候选解，但尚不清楚语言模型是否实际利用这种能力。本研究旨在探究在不同训练机制下，模型是否真正使用叠加表示进行推理。

Method: 研究在三种机制下分析叠加表示：1) 免训练机制：将潜在思维构建为词嵌入的凸组合；2) 微调机制：在基础模型上微调以产生潜在思维；3) 从头训练机制：完全用潜在思维训练模型解决任务。使用Logit Lens和实体级探针分析内部表示。

Result: 只有从头训练的模型显示出使用叠加表示的迹象。在免训练和微调情况下，叠加要么崩溃要么完全不被使用，模型会寻找捷径解决方案。这归因于两个现象：1) 自然语言数据的预训练使模型偏向在最后层确定具体词元；2) 模型容量对解决方案选择有巨大影响。

Conclusion: 研究为连续思维链推理中叠加表示何时出现、为何出现以及何时崩溃提供了统一解释，表明叠加表示的使用需要特定的训练条件，而非自然语言预训练模型的固有特性。
...
TL;DR: 提出结合强化学习与监督微调的多阶段优化策略，开发教育领域专用大语言模型EduQwen系列，在32B参数规模下超越更大规模通用模型，创下教学知识基准新SOTA。


<details>
  <summary>Details</summary>
Motivation: 提升大语言模型的教学知识能力，通过领域专业化优化使中等规模开源模型成为真正的教学领域专家，同时保持透明度、可定制性和成本效益，满足负责任教育AI部署需求。

Method: 三阶段优化策略：1) RL阶段采用渐进难度训练、聚焦困难样本、扩展推理推演；2) SFT阶段利用RL训练模型合成高质量训练数据，采用难度加权采样；3) 可选的第二轮RL优化。基于Qwen3-32B骨干构建。

Result: EduQwen系列模型在跨领域教学知识基准上达到足够高的准确率，在交互式教学基准排行榜上创下新的SOTA结果，显著超越更大规模的专有系统如之前的基准领先者Gemini-3 Pro。

Conclusion: 领域专业化优化可以将中等规模开源LLM转变为真正的教学领域专家，超越更大规模的通用系统，同时保持透明度、可定制性和成本效益，为负责任的教育AI部署提供可行路径。
...
<details>
  <summary>Details</summary>
Motivation: LLM在问答等任务中经常产生幻觉（生成看似合理但错误的信息），现有方法未充分探索注意力模式与幻觉之间的关系
...
Conclusion: 注意力模式与幻觉存在密切关系，通过替换浅层均匀注意力为局部注意力可以有效减少幻觉，ART是一种简单有效的训练免费方法
...
TL;DR: 使用LLMs识别西班牙语临床文本中的有毒习惯命名实体，GPT-4.1的few-shot方法在ToxHabits共享任务中表现最佳


<details>
  <summary>Details</summary>
Motivation: 解决西班牙语临床文本中有毒习惯命名实体识别问题，特别是针对西班牙语等非英语语言的临床实体识别挑战

Method: 探索多种LLMs方法：zero-shot、few-shot和提示优化，最终采用GPT-4.1的few-shot提示方法，用于检测和分类物质使用和滥用提及（烟草、酒精、大麻、药物）

Result: 在测试集上获得0.65的F1分数，证明了该方法在非英语语言命名实体识别中的有效性

Conclusion: GPT-4.1的few-shot提示方法在西班牙语临床文本有毒习惯实体识别任务中表现良好，为非英语语言的临床实体识别提供了有前景的解决方案
...
TL;DR: 论文评估LLM在长文本理解任务（小说摘要生成）上的表现，通过对比人类和LLM生成的摘要，发现LLM在叙事关注分布上与人类存在差异，更倾向于强调文本结尾部分。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM的上下文长度不断增长，但其在长文本中整合信息的能力并未同步提升。本研究通过小说摘要生成任务来评估LLM是否能够像人类一样理解文本的叙事重要性。

Method: 首先将150个人类撰写的小说摘要句子与具体章节进行对齐，展示该对齐任务的难度。然后为每个参考文本生成9个最先进LLM的摘要并进行对齐。通过比较人类和模型生成的摘要，分析两者在风格和叙事关注分布上的差异。

Result: 研究发现人类和LLM生成的摘要存在风格差异，且LLM在叙事关注分布上与人类不同，更倾向于强调文本的结尾部分。这种差异可能解释了LLM在叙事理解上的退化。

Conclusion: 通过对比人类和LLM的叙事参与模式，揭示了LLM在长文本理解上的局限性，为未来模型开发提供了改进目标。研究团队发布了数据集以支持后续研究。
...
TL;DR: 阿拉伯语专用开源大模型Arabic-DeepSeek-R1在Open Arabic LLM Leaderboard上创下新SOTA，首次在多数阿拉伯语任务上超越GPT-5.1，证明通过稀疏MoE架构、文化感知的思维链蒸馏和双语数据策略可实现低成本高性能


<details>
  <summary>Details</summary>
Motivation: 解决阿拉伯语等低资源语言在LLM生态系统中的数字公平问题，通过专业化适配而非大规模预训练来弥补性能差距

Method: 采用稀疏MoE架构，四阶段思维链蒸馏方案，整合阿拉伯语语言验证和区域伦理规范，使用372M token的污染控制阿拉伯语-英语训练数据（80/20比例）
...
Conclusion: 阿拉伯语性能不足主要源于专业化不足而非架构限制，稀疏MoE架构的文化感知适配为低资源语言提供了成本效益高的SOTA实现路径，建立了主权和领域特定语言技术的可复制框架
...
TL;DR: 研究提出GCA数据集，评估视觉语言模型(VLMs)的自我认知一致性，发现模型会系统性违反自己建立的规则，而人类参与者则能保持一致性。


<details>
  <summary>Details</summary>
Motivation: 理解视觉语言模型何时会行为异常、模型是否能可靠预测自身行为、以及模型是否遵循其内省推理，是可信部署的核心挑战。

Method: 引入Graded Color Attribution (GCA)数据集，包含三种条件的线图：世界知识重新着色、反事实重新着色和无颜色先验的形状。让VLMs和人类参与者建立颜色标签的像素覆盖阈值规则，然后比较这些规则与后续的颜色归属决策。

Result: 模型系统性违反自己的内省规则（如GPT-5-mini在强颜色先验对象上违反率近60%），而人类参与者能保持规则一致性。VLMs是优秀的颜色覆盖估计器，但在最终响应中公然违背自己的推理。世界知识先验会系统性降低模型忠实度，这与人类认知模式不同。

Conclusion: VLM推理失败不是难度驱动的，而是其内省自我知识校准错误，这对高风险部署有直接意义，挑战了VLM推理失败源于任务难度的观点。
...
TL;DR: 使用基于Transformer的方法解决SympTEMIST命名实体识别和实体链接任务，通过RoBERTa-BiLSTM-CRF进行NER，使用SapBERT XLMR-Large进行实体链接
...
Result: 知识库的选择对模型准确性影响最大，表明实体链接性能高度依赖于知识库质量

Conclusion: 提出的Transformer方法有效解决了SympTEMIST任务，其中实体链接性能主要受知识库选择影响，为医学实体识别和链接提供了有效解决方案
...
### [134] [Learning to Interrupt in Language-based Multi-agent Communication](https://arxiv.org/abs/2604.06452)
*Danqing Wang,Da Yin,Ruta Desai,Lei Li,Asli Celikyilmaz,Ansong Ni*

Main category: cs.CL

TL;DR: 提出可中断通信框架HANDRAISER，让听众能够打断说话者，通过预测最佳中断点来减少通信成本32.2%，同时保持或提升任务性能。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统中LLM通信存在冗长输出问题，增加上下文负担和计算成本。现有方法主要从说话者角度压缩信息，但难以适应不同听众和识别相关信息。受人类交流中听众可以打断并表达意见的启发，提出可中断通信框架。

Method: 提出可中断通信框架，允许听众打断当前说话者。通过提示实验发现LLM经常过于自信而过早打断，因此提出基于估计未来奖励和成本的学习方法，预测合适的中断点。

Result: 在多种多智能体场景（2智能体文本猜词游戏、3智能体会议安排、3智能体辩论）中评估，HANDRAISER相比基线减少32.2%通信成本，同时保持相当或更优的任务性能。学习到的中断行为可泛化到不同智能体和任务。

Conclusion: 可中断通信框架能有效减少多智能体系统中LLM通信的冗长问题，通过让听众主动控制信息流来优化通信效率，学习到的中断策略具有良好泛化能力。
...
### [135] [Context-Aware Dialectal Arabic Machine Translation with Interactive Region and Register Selection](https://arxiv.org/abs/2604.06456)
...
Method: 采用规则数据增强管道将3000句种子语料扩展为57000句平行数据集，涵盖8种地区变体；基于mT5-base模型，通过轻量级元数据标签进行条件微调，实现可控生成。

Result: 观察到准确度与忠实度的权衡：高资源基线系统（如NLLB）获得更高BLEU分数（13.75）但偏向现代标准阿拉伯语；本模型BLEU分数较低（8.19）但更符合目标方言，在文化真实性评估中表现更优（4.80/5 vs. 1.0/5）。
...
### [136] [Multi-objective Evolutionary Merging Enables Efficient Reasoning Models](https://arxiv.org/abs/2604.06465)
...
<details>
  <summary>Details</summary>
Motivation: 现有推理模型虽然能解决复杂问题，但需要长推理链导致计算开销大。当前Long-to-Short推理方法依赖固定超参数的算术合并方法，这些方法脆弱且需要次优折中。

Method: 提出Evo-L2S框架，将L2S推理建模为多目标优化问题，使用进化模型合并技术优化准确率与输出长度的权衡。为降低大语言模型的搜索开销，提出基于熵的子集采样技术来减少适应度估计的开销。

Result: 在1.5B、7B和14B参数规模的六个数学推理基准测试中，Evo-L2S能将生成的推理轨迹长度减少50%以上，同时保持甚至提升原始推理模型的问题解决准确率。

Conclusion: Evo-L2S通过进化模型合并有效解决了Long-to-Short推理问题，在保持准确率的同时显著减少推理长度，为高效推理提供了新方法。
...
### [137] [Knowledge Graphs Generation from Cultural Heritage Texts: Combining LLMs and Ontological Engineering for Scholarly Debates](https://arxiv.org/abs/2511.10354)
*Andrea Schimmenti,Valentina Pasqual,Fabio Vitali,Marieke van Erp*

Main category: cs.CL

TL;DR: ATR4CH是一个用于文化遗产文本知识提取的五步法系统，结合LLM和本体论，在真实性评估案例中验证有效，F1分数0.62-0.99。


<details>
  <summary>Details</summary>
Motivation: 文化遗产文本包含丰富知识，但由于非结构化文本难以系统查询，需要将其转换为结构化知识图谱。

Method: 五步法：基础分析、标注模式开发、管道架构、集成优化、全面评估。使用三种LLM（Claude Sonnet 3.7, Llama 3.3 70B, GPT-4o-mini）构建顺序管道。

Result: 成功提取复杂文化遗产知识：元数据提取F1 0.96-0.99，实体识别0.7-0.8，假设提取0.65-0.75，证据提取0.95-0.97，话语表示G-EVAL 0.62。小模型表现竞争性，可实现成本效益部署。

Conclusion: 这是首个系统协调LLM提取与文化遗产本体论的方法，提供可复现框架，但限于维基百科数据，需要人工监督后处理。
...
TL;DR: DataSTORM是一个基于LLM的智能代理系统，能够自主在结构化数据库和互联网源上进行深度研究，将结构化数据研究重构为基于论点的分析过程，在多个评估中达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 现有LLM代理方法主要关注非结构化网络数据，而在大规模结构化数据库上进行深度研究的挑战相对未被充分探索。数据为中心的研究需要超越检索和总结，要求迭代假设生成、结构化模式上的定量推理以及向连贯分析叙事的收敛。

Method: 基于探索性数据分析和数据叙事原则，DataSTORM将结构化数据深度研究重构为基于论点的分析过程：从数据中发现候选论点，通过迭代跨源调查验证它们，并将其发展为连贯的分析叙事。

Result: 在InsightBench上，DataSTORM实现了最先进的结果，在洞察级召回率上相对提升19.4%，在总结级得分上提升7.2%。在基于ACLED真实世界复杂数据库的新数据集上，DataSTORM在自动指标和人工评估中都优于ChatGPT Deep Research等专有系统。

Conclusion: DataSTORM展示了LLM代理在大规模结构化数据库上进行深度研究的可行性，通过基于论点的分析框架实现了比现有方法更好的性能，为数据驱动的研究开辟了新途径。
...
TL;DR: ValueGround是一个评估多模态大语言模型文化价值观视觉接地能力的基准，通过图像对而非文本选项来测试模型能否基于国家文化倾向做出正确选择。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型的文化价值观评估几乎都是纯文本的，不清楚模型在面对可视化选项时能否基于文化条件做出判断。文化价值观不仅通过语言表达，也通过视觉场景和日常社会实践体现。

Method: 基于世界价值观调查(WVS)问题构建ValueGround基准，使用最小对比图像对代表对立回答选项，控制无关变量。给定国家、问题和图像对，模型必须在无法访问原始文本选项的情况下选择最匹配该国价值倾向的图像。

Result: 在6个MLLM和13个国家上测试，平均准确率从纯文本设置的72.8%下降到可视化选项时的65.8%，尽管选项-图像对齐准确率达到92.8%。更强模型更稳健，但所有模型都容易发生预测反转。

Conclusion: ValueGround为研究文化条件价值判断的跨模态转移提供了一个受控测试平台，揭示了MLLM在视觉接地文化价值观方面的挑战。
...
TL;DR: LLM-based ASR系统通过少量语音数据即可显著改善模态对齐，混合批处理策略仅需10%目标域语音即可达到全数据微调效果


<details>
  <summary>Details</summary>
Motivation: LLM-based ASR架构通过投影模块连接语音编码器和大型语言模型，支持仅用文本数据进行领域适应，但这引入了模态差距问题，因为LLM未暴露于语音投影器产生的噪声表示

Method: 比较三种策略：纯文本适应、配对语音-文本适应、以及混合批处理（MB）策略（结合前两者）。在领域内和领域外设置中进行实验，验证少量语音数据对模态对齐的影响

Result: 实验表明，即使有限的语音数据也能持续提升性能。特别地，仅使用10%目标域语音（少于4小时）的混合批处理策略，其词错误率与传统ASR使用完整数据集微调的效果相当或更好

Conclusion: 少量语音数据能为LLM-based ASR系统提供强大的模态对齐信号，有效缓解语音投影器与语言模型之间的表示不匹配问题
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLMs被广泛用于推理密集型研究任务，但测试它们是否能从结构化生物医学证据推断科学结论的资源仍然有限。需要大规模数据集来研究证据到结论的推理过程。

Method: 构建了包含570万PubMed结构化摘要的MedConclusion数据集，每个实例将摘要的非结论部分与作者撰写的原始结论配对。数据集包含期刊级元数据（生物医学类别和SJR），支持跨生物医学领域的子组分析。评估了不同LLMs在结论和摘要提示设置下的表现，使用基于参考的指标和LLM-as-a-judge进行评分。

Result: 研究发现：1）结论写作与摘要写作在行为上存在显著差异；2）在当前自动指标下，强大模型的性能仍然紧密聚集；3）评判者身份会显著影响绝对分数。MedConclusion为研究科学证据到结论推理提供了可重复使用的数据资源。

Conclusion: MedConclusion提供了一个大规模、可重复使用的数据集，用于研究LLMs从生物医学证据推断科学结论的能力，填补了该领域资源不足的空白，并为未来研究提供了基准。
...
TL;DR: 研究比较了四种在CommonVoice普什图语数据集上微调Whisper模型的方法，发现标准全微调效果最好，达到21.22% WER，而LoRA、冻结编码器和乌尔都语迁移学习效果较差


<details>
  <summary>Details</summary>
Motivation: 普什图语是CommonVoice中最大的语言之一，但未包含在Whisper的预训练语料中，导致现成模型无法使用（输出阿拉伯语、达里语或乌尔都语文字，WER超过100%）

Method: 比较了四种微调策略：1) 标准全微调；2) LoRA（秩64）；3) 冻结编码器（2/6层）；4) 多阶段乌尔都语到普什图语迁移学习。还将标准微调扩展到whisper-small和whisper-large-v3-turbo模型

Result: 标准微调在CV20上达到21.22% WER，优于LoRA（33.36pp差距）、冻结编码器（14.76pp差距）和乌尔都语迁移（44.56pp差距）。在CV24上，whisper-small达到24.89% WER，whisper-large-v3-turbo达到23.37% WER。在线数据增强带来7.25pp WER提升

Conclusion: 标准微调是最有效的策略，whisper-small在113小时数据上是实用最优选择。主要错误模式包括词尾后缀混淆（阳性-ay vs 阴性-a）和涉及普什图语独特辅音/ts/的卷舌音替代
...
TL;DR: GRAPE是一种针对稀疏混合专家模型的全局剪枝策略，通过跨层冗余感知动态分配剪枝预算，在保持相同剪枝预算下显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 稀疏混合专家模型虽然通过激活部分专家提高了效率，但大量专家参数仍导致显著内存消耗。现有剪枝方法通常在各层均匀分配预算，忽略了稀疏MoE中存在的异质性冗余问题。

Method: 提出GRAPE（全局冗余感知专家剪枝）策略，这是一种全局剪枝方法，基于跨层冗余动态分配剪枝预算，而不是在各层均匀分配。

Result: 在Mixtral-8x7B、Mixtral-8x22B、DeepSeek-MoE、Qwen-MoE和GPT-OSS等模型上测试，在相同剪枝预算下，GRAPE始终获得最佳平均性能。在三个主要模型中，平均准确率比最强局部基线提升1.40%，最高提升达2.45%。

Conclusion: GRAPE通过全局冗余感知剪枝策略有效解决了稀疏MoE模型的异质性冗余问题，在保持相同剪枝预算下显著提升了模型性能，为大规模语言模型的高效压缩提供了新方法。
...
TL;DR: LLM作为智能体时存在随机采样缺陷：虽然前沿模型能将随机种子转换为目标分布，但直接从特定分布采样的能力存在根本性缺陷


<details>
  <summary>Details</summary>
Motivation: 揭示LLM作为智能体时的一个关键未满足需求——可靠的随机采样能力。智能体系统经常需要从分布中采样，但LLM无法将其内部概率估计映射到随机输出，这与依赖外部采样机制的标准RL智能体形成对比

Method: 通过跨多个模型家族、模型大小、提示风格和分布的严格实证分析，评估LLM的采样能力。特别测试了模型将随机种子转换为目标分布的能力，以及直接从特定分布采样的能力

Result: 研究发现：1）LLM在将内部概率估计映射到随机输出方面存在系统性失败；2）虽然强大的前沿模型能将提供的随机种子转换为目标分布，但直接从特定分布采样的能力存在根本性缺陷

Conclusion: 可靠的随机采样是LLM作为智能体运行的基本但未满足的要求。这一发现揭示了LLM智能体系统的一个关键限制，对未来的研究和应用具有重要意义
...
TL;DR: 提出CCD-CBT多智能体框架，通过动态认知概念图和信息不对称交互改进基于大语言模型的认知行为治疗模拟，并发布CCDCHAT数据集。


<details>
  <summary>Details</summary>
Motivation: 现有基于大语言模型的CBT咨询模拟方法依赖静态认知剖面和全知单智能体，无法捕捉真实治疗中动态、信息不对称的特性。

Method: 提出CCD-CBT框架，包含两个关键转变：1) 从静态到动态重建的认知概念图，由专用控制智能体更新；2) 从全知到信息不对称交互，治疗师智能体需从推断的客户状态进行推理。

Result: 发布CCDCHAT合成多轮CBT数据集，实验显示在该数据集上微调的模型在咨询保真度和积极情感增强方面优于强基线，消融实验证实动态CCD指导和不对称智能体设计的必要性。
...
TL;DR: 研究LLMs在不同语言和国家背景下生成虚假信息的行为模式，发现LLMs在低资源语言和低人类发展指数国家中更容易传播虚假信息，现有缓解策略存在跨语言和跨地区的不均衡保护。
...
Method: 构建GlobalLies多语言平行数据集（440个虚假信息生成提示模板和6,867个实体，涵盖8种语言和195个国家），使用人工标注和大规模LLM-as-a-judge评估方法，分析最先进模型生成的内容。

Result: LLMs生成虚假信息的行为因讨论国家而异，在低资源语言和低人类发展指数国家中传播虚假信息的程度显著更高。现有缓解策略保护不均衡：输入安全分类器存在跨语言差距，检索增强的事实核查因信息可用性不平等而在不同地区表现不一致。

Conclusion: LLMs传播虚假信息存在系统性偏见，对低资源语言和低发展水平国家影响更大。需要开发更公平的缓解策略来减少全球虚假信息传播。发布GlobalLies数据集以支持相关研究。
...
TL;DR: 开发了Guardian Parser Pack系统，使用AI驱动解析多源调查文档，通过LLM辅助路径显著提升提取质量，同时保持可审计性。


<details>
  <summary>Details</summary>
Motivation: 失踪人员和儿童安全调查依赖异构文档（表格、海报、网页资料），这些文档在布局、术语和数据质量上存在差异，阻碍了快速分类、大规模分析和搜索规划工作流程。

Method: 提出AI驱动的解析和标准化管道，包括：多引擎PDF文本提取与OCR备用、基于规则的源识别与特定源解析器、模式优先的协调与验证、可选的LLM辅助提取路径（包含验证器指导的修复和共享地理编码服务）。

Result: 在75个案例的手动对齐子集上，LLM辅助路径的提取质量显著高于确定性比较器（F1 = 0.8664 vs. 0.2578）；在517个解析记录中，LLM路径也提高了聚合关键字段完整性（96.97% vs. 93.23%）。确定性路径速度更快（平均0.03秒/记录 vs. LLM路径3.95秒/记录）。

Conclusion: 结果支持在高风险调查环境中，在模式优先、可审计的管道中受控使用概率性AI，LLM辅助路径显著提升数据提取质量，同时验证器指导的修复作为内置安全保障。
...
TL;DR: 提出GEC中编辑影响评分新任务，通过嵌入关联图捕捉编辑间依赖关系，基于困惑度评估编辑对句子流畅性的贡献，在多个数据集和语言上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: GEC系统产生编辑序列来纠正错误句子，但现有评估方法无法充分适应多样应用场景。句子可能有多个有效纠正，且基于人工判断的元评估方法难以扩展到大规模数据集。

Method: 提出编辑影响评分新任务，引入基于嵌入关联图的评分框架。该图捕捉编辑间的潜在依赖关系和句法相关编辑，将它们分组为连贯组，然后基于困惑度评分估计每个编辑对句子流畅性的贡献。

Result: 在4个GEC数据集、4种语言和4个GEC系统上的实验表明，该方法一致优于多个基线方法。进一步分析显示嵌入关联图有效捕捉了编辑间的跨语言结构依赖。

Conclusion: 提出的编辑影响评分任务和基于嵌入关联图的框架能够自动评估GEC系统中编辑的重要性，为GEC评估提供了更灵活和可扩展的解决方案。
...
TL;DR: SciDC是一种将学科特定知识与强约束结合的LLM生成方法，通过自动将灵活知识转换为多层标准化规则，有效约束模型在领域任务上的生成，在科学任务上平均提升12%准确率。


<details>
  <summary>Details</summary>
Motivation: 尽管LLM展现出强大的知识储备和任务解决能力，但严重的幻觉问题阻碍了实际应用。科学理论和规则能有效指导人类操作，但LLM尚未通过训练或提示充分利用这些高度凝练的知识。

Method: 提出SciDC方法，利用强LLM自动将灵活知识转换为多层、标准化的规则，构建可扩展框架来有效约束模型在领域任务上的生成。

Result: 在工业配方设计、临床肿瘤诊断和逆合成规划等科学任务上，该方法相比原始生成平均提升12%的准确率，证明了其有效性。

Conclusion: SciDC展示了LLM在自动归纳总结高度凝练知识方面的潜力，为加速整体科学研究过程提供了实用解决方案。
...
TL;DR: 研究发现推理模型在答案确定后仍继续生成大量文本（52-88%的思维链token），提出"检测-提取差距"概念，并开发BAEE方法通过早期退出减少70-78%生成量同时提升准确率1-5个百分点。


<details>
  <summary>Details</summary>
Motivation: 现代推理模型在答案已经确定后仍继续生成大量文本，造成计算资源浪费。研究发现存在"检测-提取差距"：从模型状态中可恢复正确答案，但条件解码无法提取。这促使开发更高效的推理方法。

Method: 提出Black-box Adaptive Early Exit (BAEE)方法：1) 使用自由延续进行检测和提取；2) 在答案可恢复时提前退出生成；3) 对思维模式模型防止后承诺覆盖；4) 成本优化变体减少API调用。

Result: BAEE方法能减少70-78%的串行生成，同时提升准确率1-5个百分点。对思维模式模型，早期退出防止后承诺覆盖，获得高达5.8个百分点的提升。成本优化变体实现68-73%的减少，中位数为9次API调用。

Conclusion: 模型在答案确定后继续生成是普遍现象，存在检测-提取差距。BAEE方法通过利用自由延续分布的不对称性，显著提高推理效率，在减少计算成本的同时提升准确性，为高效推理提供新方向。
...
TL;DR: DiffuMask：基于扩散模型的并行提示词压缩框架，通过分层剪枝信号实现快速并行化压缩，在保持推理准确性的同时大幅减少提示长度


<details>
  <summary>Details</summary>
Motivation: 现有上下文学习和思维链提示虽然能提升大语言模型的推理能力，但会导致更长、更昂贵的提示，且可能包含冗余信息。基于剪枝的提示压缩方法虽然实用，但现有方法依赖顺序令牌删除，计算成本高昂。

Method: 提出DiffuMask扩散框架，整合分层镜头级和令牌级剪枝信号，通过迭代掩码预测实现快速并行提示剪枝。在每个去噪步骤中同时掩码多个令牌，加速压缩过程。

Result: DiffuMask实现高达80%的提示长度缩减，同时在领域内、领域外和跨模型设置中保持或提高准确性。提供可调控制，保留关键推理上下文。
...
TL;DR: 本文提出RAG系统的反馈适应问题，关注系统如何有效快速地将纠正反馈传播到未来查询，并提出了两个评估指标：纠正延迟和反馈后性能。


<details>
  <summary>Details</summary>
Motivation: 现有RAG系统评估通常基于静态假设，忽视了实际部署中系统会通过用户或专家反馈不断修正的事实。当前评估协议只关注整体准确性，未能捕捉系统在引入反馈后的适应能力。

Method: 提出了反馈适应作为RAG系统的新问题设置，定义了纠正延迟和反馈后性能两个评估指标。提出了PatchRAG方法，这是一种无需重新训练的最小推理时实现，能够即时整合反馈。

Result: 基于训练的方法在延迟纠正和可靠适应之间存在权衡。PatchRAG方法展示了即时纠正能力和在反馈后的强泛化性能，在提出的评估框架下表现优异。

Conclusion: 反馈适应是交互式RAG系统行为中一个先前被忽视的重要维度，需要新的评估方法来衡量系统如何有效适应纠正反馈。
...
TL;DR: 提出多任务提示蒸馏与分解框架，从21个临床NLP任务学习共享元提示，以极少量参数（<0.05%）适应新任务，在多种临床任务上超越LoRA和单任务提示调优。
...
Method: 多任务提示蒸馏与分解框架：从21个不同的临床源任务学习单个共享元提示，然后通过分解适应未见目标任务，仅使用少于0.05%的可训练参数。

Result: 在5种临床NLP任务类型（命名实体识别、关系抽取、问答、自然语言推理、摘要）的10个目标数据集上，使用3个骨干模型（LLaMA 3.1 8B、Meditron3 8B、gpt-oss 20B），框架性能持续超越LoRA 1.5~1.7%（参数少几个数量级），超越单任务提示调优6.1~6.6%。gpt-oss 20B在临床推理任务上表现最佳。

Conclusion: 该框架通过共享提示表示实现了更好的可转移性，在零样本和少样本设置下表现出色，为大规模临床NLP系统部署提供了高效参数化的解决方案。
...
TL;DR: G-Defense：基于图增强的防御框架，通过分解新闻主张为子主张并建模依赖关系，仅使用未验证报告提供细粒度解释的可解释假新闻检测方法


<details>
  <summary>Details</summary>
Motivation: 现有结合调查新闻的方法效率低下且难以处理突发新闻，而基于LLM的方法使用外部检索报告作为证据，但未验证报告可能引入不准确性。有效的可解释假新闻检测应为主张的所有方面提供可理解的解释，帮助公众验证准确性。

Method: 1) 构建主张中心图：将新闻主张分解为多个子主张并建模依赖关系；2) 对每个子主张使用RAG技术检索关键证据并生成竞争性解释；3) 基于图的防御式推理模块评估整体真实性；4) 提示LLM生成直观的解释图

Result: 实验结果表明，G-Defense在真实性检测和解释质量方面均达到最先进的性能

Conclusion: G-Defense框架通过图增强方法，仅使用未验证报告就能提供细粒度解释，有效解决了现有可解释假新闻检测方法的局限性，在检测性能和解释质量上表现优异
...
TL;DR: 该研究使用对齐的Word2Vec空间和图基邻域分析，将波斯诗歌中的语义变化建模为局部语义图的重连，而非单纯的向量位移，揭示了词汇在历史演变中的邻居增减、桥梁角色变化和社区迁移模式。


<details>
  <summary>Details</summary>
Motivation: 传统语义变化研究主要关注向量位移，但波斯诗歌中的意义既是历史的又是关系的，词汇在文学传统中持续存在，同时通过邻居、修辞框架和诗歌声音的变化而改变其力量。需要一种能捕捉局部语义结构变化的方法。

Method: 使用对齐的Word2Vec空间结合基于图的邻域分析，跨越多个世纪和主要诗人。将词汇历史视为局部语义图的重连：邻居的增减、桥梁角色的变化、社区间的迁移。以20个目标词为中心，锚定于5个重复参考词（地球、夜晚、两个酒术语、心），周围环绕情感、宫廷、元素和苏菲概念。

Result: 不同词汇展现出不同的变化模式：夜晚更具时间敏感性，地球更具诗人敏感性，心在连续性的同时表现出图角色流动性。两个酒术语突显了探测敏感性：一个广泛且语义分散，另一个更窄且更稳定。词汇审计确认了历史驱动术语、诗人特定用法和需要谨慎处理的稀疏神秘词汇。

Conclusion: 波斯诗歌中的语义变化更适合被捕捉为邻域重连而非抽象漂移。对于数字人文，这种方法将局部结构恢复到计算分析中，支持更接近文学实践的解释：持续性、迁移、中介和选择性转变。
...
TL;DR: ChemVLR是一个专注于推理过程的化学视觉语言模型，通过细粒度识别化学描述符（如官能团）来提供可解释的推理路径，在化学视觉任务上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型主要针对直接视觉问答任务优化，形成"黑盒"系统，未能充分利用大语言模型推断反应机理的潜力，需要更注重推理过程的化学VLM。

Method: 1) 采用跨模态逆向工程策略和严格过滤流程构建76万高质量推理-描述数据集；2) 设计三阶段训练框架系统构建感知和推理能力；3) 通过细粒度识别化学描述符（如官能团）生成显式可解释的推理路径。

Result: ChemVLR在化学视觉任务上达到最先进的性能，超越了领先的专有模型和领域特定的开源基线模型，并通过消融研究验证了训练策略和数据生成设计的有效性。

Conclusion: ChemVLR通过强调推理过程的化学VLM设计，实现了更好的性能和可解释性，为化学视觉理解提供了新的范式，代码和模型权重将开源。
...
TL;DR: 提出aPSF框架，通过语义因子分解和干预式单因子更新，实现高效、可控的API-only提示优化


<details>
  <summary>Details</summary>
Motivation: 现有API-only提示优化方法通常迭代编辑整体提示，导致组件耦合、信用分配不明确、可控性差和token浪费

Method: aPSF框架：1）使用Architect模型发现任务特定的提示结构作为语义因子；2）进行干预式单因子更新，包括干预因子级评分和错误引导因子选择

Result: 在多个高级推理基准测试中，aPSF优于强基线（包括原则感知优化器），平均准确率提升高达+2.16个百分点，在MultiArith上减少45-87%的token优化成本，一步达到峰值验证

Conclusion: aPSF通过因子分解和干预式更新实现了更高效、可控的API-only提示优化，解决了现有方法的局限性
...
TL;DR: 提出了TEC（试错收集）平台和数据集，记录人类在网页任务中的完整试错轨迹和错误反思，用于研究人类试错行为并提升AI试错能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统的试错能力有限，主要依赖研究者设计的简单启发式方法，缺乏从人类实际试错过程中学习的详细数据。

Method: 开发数据标注平台，记录46名参与者在58个任务中的完整试错轨迹（5,370条轨迹）和错误反思，覆盖41,229个网页。

Result: 人类在试错任务中的准确率显著高于LLMs，表明人类在试错方面比LLMs更有效。平台和数据集已公开可用。
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM在Text-to-SQL基准测试中表现强劲，但其生成的SQL程序是否结构可靠仍不明确。现有研究主要关注执行准确性，而忽视了程序结构的一致性这一重要维度。

Method: 提出SQLStructEval框架，通过规范抽象语法树(AST)表示来分析程序结构。在Spider基准上进行实验，分析LLM生成查询的结构多样性，并探索通过编译式管道在结构化空间生成查询的方法。

Result: 实验显示：1) 现代LLM即使执行结果正确，对相同输入也会产生结构多样的查询；2) 这种结构差异常由表面输入变化（如改写或模式呈现方式）触发；3) 通过结构化生成方法能同时提高执行准确性和结构一致性。

Conclusion: 结构可靠性是评估LLM程序生成系统的一个关键但被忽视的维度。SQLStructEval框架为此提供了分析工具，结构化生成方法能改善LLM生成SQL的质量和可靠性。
...
TL;DR: Luwen是一个基于Baichuan基础模型构建的开源中文法律语言模型，通过持续预训练、监督微调和检索增强生成等技术，在法律任务上表现优异。
...
TL;DR: StructKV提出了一种结构感知的KV缓存压缩框架，通过全局中心性度量和动态层选择来识别信息枢纽，解决长上下文推理中的内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型支持超过百万token的上下文窗口，KV缓存的线性增长导致严重的内存容量和带宽瓶颈，限制了长上下文推理的效率。现有压缩方法通常基于局部显著性指标选择token，但会系统性丢弃在网络深度中作为全局信息枢纽但在特定修剪层暂时休眠的token。

Method: StructKV框架包含三个核心创新：1) 全局入度中心性：聚合网络深度上的注意力模式来识别全局信息枢纽；2) 动态枢轴检测：利用信息论指标自适应定位最佳压缩层；3) 结构传播与解耦：将计算预算与内存存储预算分离。

Result: 在LongBench和RULER基准测试中，StructKV有效保留了长距离依赖关系和检索鲁棒性。

Conclusion: StructKV通过结构感知的KV缓存压缩方法，解决了长上下文推理中的内存瓶颈问题，能够更好地保留全局信息枢纽，提高推理效率。
...
TL;DR: 该研究比较了六种推理范式在不同LLM和任务上的表现，发现没有单一范式占优，提出了基于嵌入的轻量级路由器来选择最优范式，显著提升了任务性能。


<details>
  <summary>Details</summary>
Motivation: 研究动机是探究LLM智能体性能提升的来源：是模型本身还是其推理范式？需要系统比较不同推理范式在不同任务上的效果，以确定最佳实践。

Method: 方法包括：1）系统比较六种推理范式（Direct、CoT、ReAct、Plan-Execute、Reflection、ReCode）在四个前沿LLM和十个基准上的表现；2）提出select-then-solve方法，使用基于嵌入的轻量级路由器为每个任务选择最合适的范式。

Result: 结果显示：1）推理范式效果因任务而异，ReAct在GAIA上比Direct提升44pp，而CoT在HumanEval上降低15pp；2）没有单一范式占优，任务特定选择比最佳固定范式平均提升17.1pp；3）学习型路由器将平均准确率从47.6%提升到53.1%，优于最佳固定范式（50.3%），恢复了最多37%的oracle差距。

Conclusion: 结论是推理范式选择应该是基于学习的路由器为每个任务做出的决策，而不是固定的架构选择。学习型路由器能有效利用不同范式的互补性，显著提升LLM性能。
...
TL;DR: 研究探讨了向LLM评判者提供推理链如何影响其判断准确性，发现弱评判者容易被流畅推理误导，强评判者能部分利用推理信息但仍会被高质量推理链欺骗。


<details>
  <summary>Details</summary>
Motivation: LLM作为人类评估的替代品存在缺陷，容易受表面偏见影响。随着推理能力模型的出现，向评判者展示生成器的推理内容可能提供更丰富信息，但其实际影响尚未充分研究。

Method: 系统研究推理链访问对基于LLM判断的影响，在事实问答和数学推理基准上进行实验，通过控制实验分析推理链的流畅性和事实性对评判决策的影响。

Result: 弱评判者容易被推理存在所左右，经常接受带有流畅推理的错误答案；强评判者能部分利用推理作为信息证据，但仍会被看似高质量的推理链误导；推理链的流畅性和事实性都是驱动评判决策的关键信号。

Conclusion: 需要开发更稳健的LLM评判者，能够在评估现代推理模型时区分真正的推理质量与表面流畅性，避免被推理链的表象所误导。
...
TL;DR: 比较零样本LLM与监督表格模型在认知障碍分类中的表现，发现监督方法（特别是结合语言特征和嵌入）通常优于零样本LLM，但零样本LLM可作为无需训练的基准。


<details>
  <summary>Details</summary>
Motivation: 评估在不同语言（英语、斯洛文尼亚语、韩语）中，使用语音转录文本进行认知障碍分类时，零样本大语言模型与监督表格模型的性能差异，探索在小数据场景下的最佳方法。

Method: 比较三种输入设置（仅转录文本、仅语言特征、两者结合）下的零样本LLM直接分类器，与使用留一法协议训练的监督表格模型。表格模型使用工程化语言特征、转录嵌入以及两者的早期或晚期融合。

Result: 跨语言实验中，零样本LLM提供了有竞争力的无需训练基准，但监督表格模型通常表现更好，特别是当包含工程化语言特征并与嵌入结合时。少样本实验显示有限监督的价值具有语言依赖性。

Conclusion: 在小数据认知障碍检测中，结构化语言信号和简单的基于融合的分类器仍然是强大且可靠的信号，监督方法优于零样本LLM，但后者可作为有效的无训练基准。
...
TL;DR: TeamLLM是一个模仿人类团队角色分工的多LLM协作框架，通过四角色分工和三阶段协作解决多步骤情境化任务，并在新提出的CGPST基准测试中显著提升性能。
...
Method: 提出TeamLLM框架，采用四种团队角色分工，通过三阶段多LLM协作处理多步骤情境化任务，并构建CGPST基准进行评估。

Result: 在CGPST基准测试中，TeamLLM显著提升了10个流行LLM在整体、步骤和维度三个层面的性能。

Conclusion: TeamLLM通过模拟人类团队角色分工，有效提升了多LLM在多步骤情境化任务中的协作性能，并提供了公开的基准数据集。
...
TL;DR: MSPA-CQR：通过多维度自洽偏好对齐和前缀引导的多方面直接偏好优化，提升对话查询重写效果


<details>
  <summary>Details</summary>
Motivation: 现有对话查询重写方法主要孤立地进行重写，忽略了查询重写、段落检索和响应生成之间的反馈循环，导致重写效果受限

Method: 1. 从三个维度（重写、检索、响应）构建自洽偏好对齐数据以生成更多样化的重写查询；2. 提出前缀引导的多方面直接偏好优化来学习三个不同维度的偏好信息

Result: 实验结果表明MSPA-CQR在分布内和分布外场景中均有效

Conclusion: 通过整合多维度反馈和偏好对齐，MSPA-CQR能够显著提升对话查询重写的效果
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要利用对话结构信息直接指导生成，但对话中口语化表达和不完整话语阻碍理解并削弱结构表示，这在多方对话中尤为明显

Method: 提出DRCR框架，包含两个互补反馈信号（话语连贯性和响应质量）构建偏好数据，采用动态自进化学习方法让重写器和响应器在迭代训练中相互增强
...
TL;DR: DTSR框架通过动态评估思维链的充分性实现早期退出，减少大型推理模型的计算冗余，在Qwen3模型上减少28.9%-34.9%的推理长度且性能损失最小。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型存在过度思考问题，导致计算冗余和效率降低。现有的早期退出方法依赖手工或经验指标，不可靠且不实用，需要更智能的退出机制。

Method: DTSR框架受人类元认知启发，包含两个阶段：1) 反思信号监测，识别早期退出的潜在线索；2) 思维充分性检查，评估当前思维链是否足以得出最终答案。

Result: 在Qwen3模型上的实验表明，DTSR能减少28.9%-34.9%的推理长度，同时保持最小性能损失，有效缓解过度思考问题。

Conclusion: DTSR为高效推理提供了动态评估思维充分性的框架，显著减少计算冗余。研究还讨论了大型推理模型的过度自信问题和自评估范式，为早期退出推理提供了有价值的见解。
...
TL;DR: GCoT-decoding是一种通用的解码策略，通过两阶段分支方法（斐波那契采样+启发式错误回溯）生成候选推理路径，分割推理段和答案段计算置信度，聚合语义相似路径获得共识答案，适用于固定和自由QA任务。


<details>
  <summary>Details</summary>
Motivation: 现有CoT方法需要手动设计提示词，而CoT-decoding只适用于固定答案集的问题。为了解决这些限制，需要一种更通用的解码策略来处理更广泛的问答任务。

Method: 提出GCoT-decoding：1）使用斐波那契采样和启发式错误回溯的两阶段分支方法生成候选解码路径；2）将每条路径分割为推理段和答案段以准确计算路径置信度；3）聚合语义相似路径识别共识答案，替代传统多数投票。

Result: 在六个数据集（涵盖固定和自由QA任务）上的实验表明，该方法不仅在固定QA上保持强大性能，在自由QA上也取得了显著改进，证明了其通用性。

Conclusion: GCoT-decoding成功扩展了CoT推理的适用范围，为更广泛的问答任务提供了一种有效的通用解码策略，解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 当前代数推理基准测试只能提供单一准确率分数，无法揭示模型失败的具体原因（如表达式嵌套过深、运算符不常见、中间状态过多等），缺乏系统化的诊断框架。

Method: 设计九维代数复杂度框架，每个维度对应一个已知的LLM失败模式；开发参数化管道自动生成和验证问题，独立控制各复杂度因素；评估7个8B到235B参数的指令调优模型。

Result: 发现工作记忆是主要的规模不变瓶颈：所有模型在20-30个并行分支处崩溃，与参数数量无关，表明这是硬架构约束而非可解决的容量限制；识别出五个最小但诊断充分的维度子集。

Conclusion: 该框架能全面分析模型的代数推理能力，揭示了LLM在代数推理中的根本限制，为模型诊断和进步跟踪提供了系统化工具。
...
TL;DR: 提出基于可逆分层马尔可夫链的思维链框架CLoT，通过分层分解问题、引入后向验证机制和剪枝策略，解决长思维链的上下文丢失和有限后向推理问题，在数学推理任务上显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 传统长思维链方法存在两个关键限制：1) 马尔可夫链式结构导致固有的无记忆性（上下文丢失）；2) 有限的后向推理能力。这些问题限制了数学推理的准确性和鲁棒性。

Method: 提出认知思维循环(CLoT)框架：1) 将问题分解为具有层次依赖关系的子问题；2) 受人类认知过程启发，在每个层次引入后向验证机制；3) 实现剪枝策略，一旦高层子问题被验证，就剪枝冗余的低层子问题以提高效率。

Result: 在四个数学基准测试上验证了方法的有效性。在AddSub数据集上使用GPT-4o-mini，CLoT达到99.0%准确率，分别比传统CoT和CoT-SC高出4.1%和2.9%。

Conclusion: CLoT框架通过可逆分层马尔可夫链结构，有效缓解了错误传播，增强了推理鲁棒性，为长思维链推理提供了更有效的解决方案。
...
TL;DR: AGSC是一个针对长文本生成的UQ框架，通过NLI中性概率触发和GMM软聚类来区分无关信息和不确定性，在保持事实性相关性的同时减少60%推理时间。


<details>
  <summary>Details</summary>
Motivation: LLMs在长文本生成中存在幻觉问题，现有UQ方法难以可靠聚合异构主题信息，忽略中性信息细节，且细粒度分解计算成本高。

Method: AGSC框架：1) 使用NLI中性概率作为触发器区分无关信息和不确定性；2) 应用GMM软聚类建模潜在语义主题，为下游聚合分配主题感知权重。

Result: 在BIO和LongFact数据集上，AGSC实现了与事实性的最先进相关性，同时相比完整原子分解减少了约60%的推理时间。

Conclusion: AGSC通过自适应粒度和语义聚类有效解决了长文本生成中的UQ挑战，在准确性和效率之间取得了良好平衡。
...
TL;DR: SemEval-2026 Task 9是一个多语言在线极化检测共享任务，涵盖22种语言、超过11万条标注数据，包含三个子任务：检测极化存在、识别极化类型、识别极化表现，吸引了全球1000多名参与者。


<details>
  <summary>Details</summary>
Motivation: 随着在线内容的快速增长，极化现象在社交媒体和网络平台中日益显著，需要系统性的方法来检测和分析不同语言中的极化内容，以促进更健康的在线讨论环境。

Method: 任务采用多标签标注框架，每个数据实例标注了极化存在、极化类型和极化表现三个维度。参与者需要开发系统来完成这三个子任务的预测，任务组织者提供了基线系统并收集了最终提交结果进行分析。

Result: 任务吸引了全球1000多名参与者，收到67个团队的最终提交和73篇系统描述论文。组织者报告了基线结果，分析了最佳系统的性能，突出了不同子任务和语言中最常见和最有效的方法。

Conclusion: 该任务成功创建了一个大规模的多语言极化检测数据集，促进了该领域的研究发展，数据集已公开可用，为未来的极化检测研究提供了宝贵资源。
...
TL;DR: 本文介绍了首个公开的斯洛文尼亚语ESG情感数据集和自动ESG情感检测模型套件，评估了多种模型在环境、社会和治理三个维度的表现，并展示了最佳模型在长期公司分析中的应用。


<details>
  <summary>Details</summary>
Motivation: ESG因素对企业绩效、声誉和长期可持续性评估日益重要，但针对小公司和新兴市场的可靠ESG评级仍然有限，特别是缺乏斯洛文尼亚语的ESG数据资源。

Method: 从MaCoCu斯洛文尼亚新闻语料库构建首个公开的斯洛文尼亚语ESG情感数据集，采用LLM辅助过滤和人工标注相结合的方法。评估了单语模型(SloBERTa)、多语模型(XLM-R)、基于嵌入的分类器(TabPFN)、分层集成架构和大语言模型等多种技术。

Result: LLM在环境维度(Gemma3-27B, F1-macro: 0.61)和社会维度(gpt-oss 20B, F1-macro: 0.45)表现最佳，而微调的SloBERTa在治理分类中表现最好(F1-macro: 0.54)。通过案例研究展示了最佳模型(gpt-oss)在长期公司ESG分析中的应用。

Conclusion: 研究提供了首个斯洛文尼亚语ESG数据集和有效的自动检测方法，为小公司和新兴市场的ESG分析提供了实用工具，展示了不同模型在ESG各维度上的差异化性能优势。
...
TL;DR: WRAP++通过发现网页超链接中的跨文档关系，合成需要跨文档推理的问答数据，增强LLM预训练中的知识获取能力。


<details>
  <summary>Details</summary>
Motivation: 现有单文档改写方法局限于文档内部知识，无法捕捉跨文档关系和关联上下文，限制了知识获取的广度和深度。

Method: WRAP++从网页超链接中发现高置信度的关系模式（如双向链接和共同提及），为每个发现的文档对合成需要跨文档推理的联合问答。

Result: 在Wikipedia上将约84亿token原始文本扩增为800亿token的跨文档QA数据；在SimpleQA上，基于WRAP++训练的7B和32B OLMo模型显著优于单文档方法，并展现出持续的扩展增益。

Conclusion: 跨文档知识发现和扩增具有显著优势，能够增强LLM的知识获取能力，特别是在需要关联推理的任务中表现更优。
...
TL;DR: Fast-dVLM使用块扩散技术实现视觉语言模型的并行解码，相比自回归解码获得6倍推理加速，同时保持生成质量


<details>
  <summary>Details</summary>
Motivation: 自回归解码在边缘设备上推理吞吐量受限，无法充分利用硬件并行性，特别是在机器人、自动驾驶等物理AI场景中

Method: 提出基于块扩散的Fast-dVLM，支持KV缓存兼容的并行解码和推测块解码；比较两种AR到扩散转换策略，采用直接转换方法；引入多模态扩散适配、块大小退火、因果上下文注意力、自动截断掩码和视觉高效拼接等技术

Result: 在11个多模态基准测试中，Fast-dVLM与自回归对应模型在生成质量上相当；通过SGLang集成和FP8量化，实现了超过6倍的端到端推理加速

Conclusion: 直接转换策略更有效，Fast-dVLM成功将块扩散技术扩展到视觉语言模型，在保持质量的同时显著提升推理速度，适用于边缘设备部署
...
TL;DR: 论文提出ASLEC方法解决LLM推理数据选择中的"步骤长度混淆"问题，通过丢弃首词概率或因果去偏回归来改进基于自然度的数据选择方法。


<details>
  <summary>Details</summary>
Motivation: 现有基于自然度的数据选择方法（通过LLM平均对数概率排名）在推理数据集上存在系统性偏差，倾向于选择推理步骤更长的样本而非质量更高的样本，这种"步骤长度混淆"现象影响了推理数据集的构建质量。

Method: 提出两种变体方法：ASLEC-DROP（计算平均对数概率时丢弃首词概率）和ASLEC-CASL（应用因果去偏回归消除首词的混淆效应），以解决步骤长度混淆问题。

Result: 在四个LLM和五个评估基准上的实验表明，提出的方法能有效缓解步骤长度混淆问题，改进推理数据选择的质量。

Conclusion: 论文揭示了基于自然度的数据选择在推理数据集上的局限性，提出的ASLEC方法通过处理首词概率偏差有效解决了步骤长度混淆问题，为高质量推理数据集的构建提供了改进方案。
...
TL;DR: HingeMem：基于边界触发超边的长时记忆系统，通过事件分割理论构建可解释索引，实现自适应检索，显著提升对话系统性能并降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 现有对话系统的长时记忆方法依赖连续摘要或OpenIE图构建，存在跨查询类别适应性差、计算开销大的问题，需要更高效、自适应的记忆机制。

Method: 基于事件分割理论，构建边界触发的超边索引（人物、时间、地点、主题四要素），当任一要素变化时触发边界写入；引入查询自适应检索机制，动态决定检索内容和深度。

Result: 在LLM规模（0.6B到生产级模型）的LOCOMO数据集上，相比强基线获得约20%相对提升，无需查询类别指定；计算成本显著降低（问答token成本比HippoRAG2减少68%）。

Conclusion: HingeMem通过边界引导的记忆构建和自适应检索机制，为需要高效可信长时记忆的Web应用提供了强大解决方案，推动了记忆建模的发展。
...
TL;DR: MedDialBench：一个医疗对话基准，用于量化分析患者行为五个维度对LLM诊断鲁棒性的影响，发现信息污染比信息缺失危害更大，且存在超加性交互效应。


<details>
  <summary>Details</summary>
Motivation: 现有医疗对话基准在处理非合作患者时存在局限：要么采用无分级严重性的对抗行为，要么将患者不合作简化为单一维度，且缺乏跨维度交互分析。需要系统评估不同患者行为维度对LLM诊断准确性的影响。

Method: 提出MedDialBench基准，将患者行为分解为五个维度（逻辑一致性、健康认知、表达风格、信息披露、态度），每个维度有分级严重性和病例特定的行为脚本。采用受控因子设计，评估5个前沿LLM在7,225个对话（85个病例×17种配置×5个模型）中的表现。

Result: 发现信息污染（编造症状）比信息缺失（隐瞒信息）对准确性的影响大1.7-3.4倍；编造是唯一在所有五个模型中均达到统计显著性的配置。在六种维度组合中，涉及编造的三种组合均产生超加性交互效应，而非编造组合仅显示加性效应。询问策略可缓解信息缺失但无法补偿信息污染。

Conclusion: 患者行为的不同维度对LLM诊断鲁棒性有不对称影响，信息污染危害最大且存在超加性交互效应。这为开发更鲁棒的医疗对话系统提供了重要见解，需要特别关注处理编造信息的能力。
...
TL;DR: 临床领域适应的大语言模型在英语医学基准测试中并未稳定优于通用模型，但在西班牙语任务中表现更好；现有评估框架可能不足以衡量真正的医学专业知识。


<details>
  <summary>Details</summary>
Motivation: 近期研究表明，经过领域适应的大语言模型在标准医学基准测试中并不总是优于通用模型，这引发了对临床适应必要性的质疑，需要系统评估临床LLMs与通用模型的性能差异。

Method: 系统比较通用和临床LLMs在英语和西班牙语临床问答任务上的表现；引入基于扰动的评估基准，测试模型鲁棒性、指令遵循和对抗变异的敏感性；评估包括一步和两步问题转换、多提示测试和指令引导评估；分析基于Llama 3.1的临床模型及其通用对应模型；开发Marmoka系列轻量级8B参数临床LLMs，通过医学语料和指令的持续领域自适应预训练。

Result: 实验显示临床LLMs在英语临床任务中并未稳定优于通用模型，即使在提出的扰动基准下也是如此；但在西班牙语子集中，Marmoka模型相比Llama获得更好结果；通用和临床模型在指令遵循和严格输出格式方面都存在显著限制。

Conclusion: 在当前短形式多项选择问答基准下，临床LLMs仅提供边际且不稳定的改进，表明现有评估框架可能不足以捕捉真正的医学专业知识；通用和临床模型在指令遵循方面都有局限；可以为西班牙语等低资源语言成功开发鲁棒的医学LLMs，如Marmoka模型所示。
...
TL;DR: 通过亲和池化技术压缩语音大语言模型的冗余表示，减少27.48%的计算开销，同时保持语义准确性


<details>
  <summary>Details</summary>
Motivation: 大型语音语言模型通常需要高token率来保证声学保真度，但这导致序列长度远超实际语义内容，带来高昂的推理成本。作者质疑这种细粒度token处理的必要性，旨在探索更高效的语音表示方法。

Method: 通过层间oracle干预揭示冗余层次结构，发现深层存在极端冗余。基于此提出Affinity Pooling（亲和池化），这是一种无需训练、基于相似性的token合并机制，在输入层和深层策略性地应用以压缩语音表示。

Result: 在三个任务上的广泛评估表明，该方法减少27.48%的预填充FLOPs，同时保持竞争力的准确性。实际部署证实显著效率提升：内存节省约1.7倍，长语音的首token时间加快约1.1倍。

Conclusion: 研究结果挑战了完全独立token表示的必要性，为大型语音语言模型效率提供了新视角。Affinity Pooling展示了在不损失语义信息的情况下压缩语音表示的可行性。
...
TL;DR: iTAG是一种通过迭代概念分配和CoT推理，从因果图生成高质量文本标注数据的方法，在标注准确性和文本自然度上都表现出色。


<details>
  <summary>Details</summary>
Motivation: 文本因果发现面临的主要障碍是缺乏因果标注的文本数据作为基准真值，因为标注成本高昂。现有方法要么牺牲文本自然度（模板方法），要么无法保证因果图标注准确性（LLM方法）。

Method: iTAG在现有LLM方法的基础上，增加了真实世界概念分配给节点的步骤。它将此过程构建为一个逆问题，以因果图为目标，通过Chain-of-Thought推理迭代检查和细化概念选择，使概念间诱导的关系尽可能与目标因果图描述的因果关系一致。

Result: iTAG在广泛的测试中表现出极高的标注准确性和文本自然度。使用生成数据测试基于文本的因果发现算法时，结果显示与真实世界数据具有高度统计相关性。

Conclusion: iTAG生成的数据可以作为基于文本的因果发现算法可扩展基准测试的实际替代品，为解决因果标注文本数据稀缺问题提供了有效方案。
...
TL;DR: 本研究探讨了领域自适应预训练（DAPT）在法语生物医学领域的应用，发现DAPT效果有限但适用于资源受限的小规模场景，模型融合能缓解泛化性能下降。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在专业领域特别是非英语语言的适应仍具挑战性，本研究旨在探索DAPT在法语生物医学领域的专业化应用可行性。

Method: 收集并精炼高质量法语生物医学文本，采用因果语言建模方法进行DAPT，进行广泛的比较评估，并探索模型融合策略。

Result: 结果对DAPT的有效性提出质疑，但发现在资源受限的小规模场景下可行；模型融合后DAPT能缓解泛化性能下降，有时甚至能提升专业任务表现。

Conclusion: DAPT在法语生物医学领域的应用效果有限，但在特定条件下适用于小规模场景；模型融合是缓解泛化性能下降的关键策略。
...
TL;DR: 研究者开发了技能自动化可行性指数(SAFI)，评估四大前沿LLM在263个文本任务上的表现，发现数学和编程技能最易自动化，而主动倾听和阅读理解最难自动化，78.7%的AI交互是增强而非替代。


<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型重塑全球劳动力市场，政策制定者和工作者需要实证数据来了解哪些职业技能最易受自动化影响，以便制定相应的应对策略。

Method: 开发SAFI指数，在O*NET分类的35种技能对应的263个文本任务上，对LLaMA 3.3 70B、Mistral Large、Qwen 2.5 72B和Gemini 2.5 Flash四大模型进行基准测试，并结合Anthropic经济指数的真实AI采用数据进行交叉分析。

Result: 数学(73.2分)和编程(71.8分)自动化可行性最高，主动倾听(42.2分)和阅读理解(45.5分)最低；存在"能力需求倒置"现象；78.7%的AI交互是增强而非自动化；四大模型技能表现相似(3.6分差距)。
...
TL;DR: 研究发现LLM作为评估者存在自我偏好偏差，即使在客观标准下也会偏向自身或同系列模型的输出，影响模型评估和开发


<details>
  <summary>Details</summary>
Motivation: LLM作为评估者已成为评估LLM输出的主流方法，但评估者存在自我偏好偏差，倾向于偏好自己或同系列模型的输出。这种偏差扭曲了评估结果，阻碍了模型发展，特别是在递归自我改进的背景下。

Method: 使用基于规则的评估范式，在IFEval（具有可编程验证规则的基准）和HealthBench（医学聊天基准）上研究自我偏好偏差。分析客观和主观规则下的偏差表现，并探讨集成多个评估者作为缓解策略。

Result: 即使在完全客观的评估标准下，自我偏好偏差仍然存在：在生成器失败的规则中，评估者对自身输出的错误通过率最高可达50%。集成多个评估者有助于缓解偏差但无法完全消除。在主观规则下，偏差可使模型得分偏差达10分，这在前沿模型排名中可能是决定性差异。

Conclusion: 自我偏好偏差是基于规则的LLM评估中的一个严重问题，即使在客观标准下也会影响评估结果。需要开发更有效的缓解策略，特别是在模型开发和排名等关键应用中。
...
TL;DR: 提出ChunQiuTR时间关键检索基准和CTD时间感知双编码器，解决古典中文编年史中时间一致性检索问题，超越传统语义检索基线。


<details>
  <summary>Details</summary>
Motivation: 在历史研究中，检索目标往往不是任意相关段落，而是特定统治月份的确切记录，时间一致性与主题相关性同等重要。古典中文编年史中时间表达简洁、隐含、非公历，需要从上下文解释，导致语义合理的证据可能在时间上无效。

Method: 1) 构建ChunQiuTR时间关键检索基准，基于《春秋》及其注释传统，按月份级统治密钥组织记录，包含时间近邻干扰项；2) 提出CTD（历法时间双编码器），结合基于傅里叶的绝对历法上下文和相对偏移偏置的时间感知双编码器。

Result: 实验显示CTD在时间关键评估下持续优于强语义双编码器基线，支持检索时时间一致性作为忠实下游历史RAG的关键前提。

Conclusion: 时间关键检索对于历史RAG至关重要，CTD方法能有效处理古典中文编年史中的时间一致性挑战，为历史研究提供更准确的检索基础。
...
<details>
  <summary>Details</summary>
Motivation: 语用推理本质上是分级的，不同词汇项产生不同程度的语用强化。标量含义体现了这一特性，即标量多样性——不同标量项的含意强度不同。然而，现有对大语言模型语用推理的评估多依赖基于提示的操作，需要超越提示层面的方法来探究分级语用解释。

Method: 提出连续解释性引导(CIS)方法，将激活级引导强度作为连续实验变量来探测分级语用解释。同时引入新数据集GraSD，编码分级标量多样性。在四个大语言模型上进行实验，比较均匀激活引导和分级激活引导的效果。

Result: 实验表明：均匀激活引导虽能全局增加语用解释，但会消除项目级差异；而分级激活引导能产生与标量多样性等级一致的分化解释偏移。这表明分级敏感性编码在表示空间中，可通过受控干预系统恢复。

Conclusion: CIS和GraSD为评估大语言模型中的分级语用敏感性提供了原则性框架，揭示了语用推理的分级特性在模型表示空间中的编码方式，以及通过分级激活引导恢复这种敏感性的可能性。
...
TL;DR: DTCRS是一种动态生成摘要树的方法，通过分析问题类型决定是否需要摘要树，使用子问题嵌入作为聚类中心，减少冗余摘要，提高问答效率。


<details>
  <summary>Details</summary>
Motivation: 传统的递归摘要方法存在两个主要问题：1）摘要树包含大量冗余节点，增加构建时间且可能负面影响问答质量；2）递归摘要并不适用于所有类型的问题。需要一种更智能的方法来动态决定何时以及如何构建摘要树。

Method: DTCRS方法包含三个关键步骤：1）分析问题类型，动态决定是否需要构建摘要树；2）将问题分解为子问题，使用子问题的嵌入作为初始聚类中心；3）基于文档结构和查询语义动态生成摘要树，减少冗余摘要节点。

Result: 该方法显著减少了摘要树构建时间，并在三个QA任务上取得了实质性改进。同时，研究还探讨了递归摘要对不同问题类型的适用性，为未来研究提供了有价值的见解。

Conclusion: DTCRS通过动态决策和基于查询语义的摘要树构建，有效解决了传统递归摘要方法的冗余问题，提高了RAG系统的效率和问答质量，并为递归摘要的适用性提供了理论指导。
...
TL;DR: 研究数据重复对资源有限语言NLP是否有用，在Nawatl语上通过增量重复扩展语料库，训练词嵌入并在语义相似度任务中评估，结果显示性能有适度提升。


<details>
  <summary>Details</summary>
Motivation: 针对计算资源有限的语言（π-语言），这些语言的语料库非常稀缺，难以训练大语言模型。研究数据重复是否能在这种语言环境下提升NLP任务性能，特别是在Nawatl语（一种有200多万人使用的黏着语和多式综合语）上。

Method: 使用增量重复技术扩展π-yalli语料库，通过受控方式重复现有Nawatl文本。训练静态词嵌入，并在句子级语义相似度任务中进行评估。

Result: 与未扩展的原始语料库相比，使用增量重复技术后，在语义相似度任务中表现出适度的性能提升。这是该技术首次在文献中被应用。

Conclusion: 数据重复对于资源有限语言的NLP任务是有用的，特别是通过增量重复技术扩展语料库可以改善词嵌入质量，为这类语言的NLP研究提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 传统自回归语言模型一次只生成一个token，即使连续token在给定上下文后高度可预测，这限制了生成效率。现有方法如推测解码需要额外模型，多头方法需要附加预测头，都不够轻量。

Method: MARS通过轻量级微调方法，在现有指令数据上继续训练，教导指令调优的自回归模型一次预测多个token。无需架构修改或额外参数，支持块级KV缓存策略和实时速度调整。

Result: 在六个标准基准测试中，一次生成一个token时匹配或超过自回归基线；一次接受多个token时保持基线准确度，实现1.5-1.7倍吞吐量。在Qwen2.5-7B上使用KV缓存实现1.71倍加速。

Conclusion: MARS提供了一种实用的延迟-质量权衡机制，支持实时速度调整，为部署提供了灵活的效率提升方案，同时保持模型兼容性和性能。
...
TL;DR: 论文通过系统基准测试比较了7个密集和MoE推理模型在4个基准任务上的表现，发现Gemma-4-E4B在few-shot chain-of-thought下取得最佳综合性能，表明稀疏激活本身不能保证最佳实践效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是评估MoE语言模型在实际推理约束下的端到端性能优势，验证稀疏激活是否能真正带来更好的质量-效率权衡，为部署导向的推理LLM评估提供实证依据。

Method: 方法包括：1) 选择7个推理导向的指令调优模型（涵盖密集和MoE设计）；2) 在4个基准任务（ARC-Challenge, GSM8K, Math Level 1-3, TruthfulQA MC1）上进行评估；3) 采用三种提示策略（zero-shot, chain-of-thought, few-shot chain-of-thought）；4) 记录准确率、延迟、峰值GPU内存使用和FLOPs-per-token代理指标；5) 进行总计8,400次模型-数据集-提示评估。

Result: 主要结果：1) Gemma-4-E4B在few-shot chain-of-thought下获得最佳综合性能（加权准确率0.675，平均VRAM 14.9 GB）；2) Gemma-4-26B-A4B准确率接近（0.663）但内存消耗显著更高（48.1 GB）；3) 不同模型在不同任务上表现各异：Gemma模型在ARC和Math上占优，Phi模型在TruthfulQA上最强；4) GSM8K显示最大的提示敏感性，Phi-4-reasoning在few-shot chain-of-thought下准确率从0.67骤降至0.11。

Conclusion: 结论表明稀疏激活本身不能保证最佳实践操作点，观察到的准确率-效率权衡共同取决于架构、提示协议和任务组成。研究提供了可复现的基准测试管道和统计分析，支持在真实资源约束下进行部署导向的推理LLM评估。
...
TL;DR: ReDAct提出了一种双LLM代理框架，通过小模型默认决策、大模型处理不确定情况来平衡成本与性能，在仅将15%决策委托给大模型的情况下达到与大模型单独使用相当的性能。


<details>
  <summary>Details</summary>
Motivation: LLM代理在序列决策中存在幻觉问题，单个错误可能导致轨迹不可逆恶化。大模型幻觉较少但成本高昂，小模型便宜但可靠性低，需要平衡成本与性能的解决方案。

Method: ReDAct框架配备两个LLM：默认使用小型廉价模型，当小型模型的预测不确定性超过校准阈值时，将决策委托给大型可靠但昂贵的模型。

Result: 在ALFWorld和MiniGrid等文本化具身环境中评估显示，仅将约15%的决策委托给大模型即可达到与大模型单独使用相当的质量，同时显著降低推理成本。

Conclusion: ReDAct通过智能决策委托机制有效解决了LLM代理的成本-性能权衡问题，为实际部署提供了经济高效的解决方案。
...
TL;DR: SalesLLM是一个双语销售对话基准，包含金融和消费品领域的真实场景，通过自动评估管道衡量交易进展和购买意图，用于评估结果导向的销售代理。


<details>
  <summary>Details</summary>
Motivation: 现有对话基准很少衡量交易进展和结果，而销售对话具有多轮、目标导向、激励不对称的特点，对大型语言模型具有挑战性。

Method: 构建SalesLLM双语基准，包含30,074个脚本配置和1,805个多轮场景；提出自动评估管道：LLM评分器衡量销售过程进展，微调BERT分类器评估购买意图；训练用户模型CustomerLM减少角色反转。

Result: SalesLLM评分与专家人类评分高度相关（Pearson r=0.98）；CustomerLM将角色反转从17.44%降至8.8%；15个主流LLM表现差异显著，最佳模型达到人类水平，较差模型不如人类。
...
TL;DR: IndoBERT-Sentiment：首个印尼语上下文条件情感分类器，结合主题上下文进行情感分析，显著优于传统上下文无关模型


<details>
  <summary>Details</summary>
Motivation: 现有印尼语情感分析模型孤立地分类文本，忽略了决定情感极性的主题上下文。许多文本的情感取决于讨论的主题，上下文无关方法会导致系统性误分类。

Method: 基于IndoBERT Large（3.35亿参数）构建，输入包括主题上下文和文本，在188个主题的31,360个上下文-文本对上进行训练，实现上下文条件情感分类。

Result: 模型达到F1宏平均0.856和准确率88.1%。在与三个广泛使用的印尼语情感模型的对比评估中，比最佳基线高出35.6个F1点。能正确分类上下文无关方法系统误分类的文本。

Conclusion: 上下文条件方法（先前用于相关性分类）能有效迁移到情感分析，显著提升印尼语情感分类性能。模型能捕捉主题依赖的情感极性，解决上下文无关方法的局限性。
...
TL;DR: SemEval-2026共享任务提出维度方面情感分析(DimABSA)，将传统ABSA从分类极性标签扩展到价-唤醒(VA)维度建模，并引入维度立场分析(DimStance)处理公共议题话语，包含两个赛道和多个子任务，吸引了400多名参与者。


<details>
  <summary>Details</summary>
Motivation: 传统方面情感分析(ABSA)使用分类极性标签(如正面/负面)，无法捕捉情感的细微差别。需要将ABSA扩展到消费者评论之外的公共议题话语(如政治、能源、气候议题)，并引入更丰富的情感维度表示。

Method: 提出两个主要任务：1) DimABSA：在价-唤醒维度上建模方面情感；2) DimStance：将立场目标视为方面，在VA空间中重新构建立场检测为回归问题。包含两个赛道：Track A(DimABSA)有三个子任务(回归、三元组提取、四元组提取)，Track B(DimStance)只有回归子任务。引入连续F1(cF1)指标联合评估结构化提取和VA回归。

Result: 任务吸引了超过400名参与者，产生了112份最终提交和42篇系统描述论文。报告了基线结果，讨论了表现最佳的系统，并分析了关键设计选择，为方面和立场目标层面的维度情感分析提供了见解。

Conclusion: 该共享任务成功推动了维度情感分析的发展，通过价-唤醒维度建模提供了比传统分类方法更丰富的情感表示，特别是在公共议题话语分析中。所有资源已在GitHub仓库中公开，为未来研究提供了基准和工具。
...
<details>
  <summary>Details</summary>
Motivation: 研究双语语言模型是否能够模拟人类双语者在阅读时表现出的跨语言激活模式，特别是同源词（朋友）和跨语言同形异义词（假朋友）的不同效应

Method: 训练荷兰语-英语因果Transformer模型，设置四种词汇共享条件，使用心理语言学刺激材料，通过惊奇度和嵌入相似性分析评估模型

Result: 模型基本保持语言分离，跨语言效应主要在嵌入共享时出现；此时朋友和假朋友都相对于控制词表现出促进效应；回归分析显示这些效应主要由频率而非形式-意义映射一致性驱动；只有当朋友共享嵌入时，模型才能重现人类双语者的定性模式

Conclusion: 双语语言模型能够捕捉一些跨语言激活效应，但与人类处理的对齐程度严重依赖于词汇重叠的编码方式，这可能限制了它们作为双语阅读模型的解释充分性
...
TL;DR: 多语言嵌入模型并不直接编码语言通用的能力水平表示，而是捕捉语料库特定的分布特性


<details>
  <summary>Details</summary>
Motivation: 研究多语言嵌入模型是否编码了语言通用的能力水平表示，这对于基于表示的能力自适应语言技术具有重要意义

Method: 在Qwen3-Embedding模型的隐藏状态激活上训练线性和非线性探针，预测九个语料库、七种语言的学习者文本的CEFR能力水平，并与基于表层特征的基线比较

Result: 在分布内评估中，探针表现良好（QWK≈0.7），显著优于基线，中间层预测效果最好；但在跨语料库评估中，所有探针类型和模型大小的性能都崩溃，表明学习到的映射捕捉的是语料库特定的分布特性而非抽象的可迁移能力维度

Conclusion: 当前的多语言嵌入模型并不直接编码语言通用的能力水平，这对基于表示的能力自适应语言技术方法有重要影响
...
TL;DR: STRIDE-ED：基于策略的深度推理框架，通过结构化策略条件推理建模共情对话，结合高质量数据构建和两阶段训练，显著提升共情对话性能
...
Method: 提出STRIDE-ED框架：1）策略感知数据精炼流水线，集成LLM标注、多模型一致性加权评估和动态采样；2）两阶段训练范式，结合监督微调和多目标强化学习，使模型行为与目标情感、共情策略和响应格式对齐

Result: 大量实验表明，STRIDE-ED能够泛化到多种开源LLM，在自动指标和人工评估上持续优于现有方法

Conclusion: STRIDE-ED通过结构化策略条件推理、高质量数据构建和两阶段训练，有效解决了共情对话中的策略建模和推理挑战，为共情对话系统提供了可解释的深度推理框架
...
TL;DR: 研究显示，在大型语言模型中通过激活引导进行人格向量操控会降低教育场景中的答案质量，对开放式英语语言艺术任务的影响比对事实性科学任务大11倍，同时评分也会出现可预测的人格校准偏移。


<details>
  <summary>Details</summary>
Motivation: 虽然激活引导技术可以在推理时个性化大型语言模型，但其在教育场景中的具体影响尚不明确。本研究旨在系统探索人格向量操控在教育文本生成和自动评分中的效果，为教育场景中部署引导模型提供依据。

Method: 在ASAP-SAS基准测试上，针对三种模型（涵盖两种架构）研究了七种人格特质的向量操控。分别评估了短答案生成质量和自动评分效果，比较了开放式英语语言艺术任务与事实性科学任务的差异，并分析了不同模型架构的敏感性。

Result: 人格引导总体上降低了答案质量，对开放式英语语言艺术任务的影响比对事实性科学任务大11倍。评分方面出现可预测的人格校准偏移：邪恶和不礼貌的评分者更严厉，善良和乐观的评分者更宽容。英语语言艺术任务对评分人格化的敏感性是科学任务的2.5-3倍，混合专家模型的校准偏移比密集模型大约6倍。

Conclusion: 这是首个系统研究激活引导人格特质在教育生成和评分中影响的工作。结果表明，在教育场景中部署引导模型时，需要考虑任务特性和架构差异进行校准，以确保公平性和准确性。
...
TL;DR: 耶鲁-DM-Lab系统在ArchEHR-QA 2026共享任务中采用多模型集成方法处理患者病历问题，包含四个子任务：问题重述、证据识别、答案生成和证据对齐。


<details>
  <summary>Details</summary>
Motivation: 研究如何有效处理患者对住院记录提出的问题，通过临床医生解释的问题重述、证据提取、答案生成和证据对齐来提高医疗问答系统的准确性和可靠性。
...
Result: 开发集最佳成绩：ST4达到88.81微F1，ST2达到65.72宏F1，ST3达到34.01，ST1达到33.05。模型多样性和集成投票相比单模型基线持续提升性能。

Conclusion: 模型集成和多样性对医疗问答任务性能有显著提升，证据对齐的主要限制因素是推理能力，而非证据识别。完整临床医生答案段落作为额外上下文有助于证据对齐。
...
TL;DR: 本文首次系统性地研究了NLP同行评审中的语言研究偏见（LoS bias），即评审人根据论文研究语言而非科学价值进行评价的倾向，构建了LOBSTER数据集并开发了检测方法，发现非英语论文面临更高的偏见率。


<details>
  <summary>Details</summary>
Motivation: 同行评审在NLP发表过程中至关重要，但容易受到各种偏见影响。语言研究偏见（LoS bias）虽然被评审指南明确标记，但理解不足。先前工作将其归为宽泛的弱评审或无建设性评审类别，未将其定义为独特的偏见形式。

Method: 1. 首次系统性地描述LoS偏见，区分负面和正面形式；2. 构建人类标注数据集LOBSTER（Language-Of-study Bias in ScienTific pEer Review）；3. 开发检测方法，达到87.37宏F1分数；4. 分析15,645篇评审，估计负面和正面偏见如何随研究语言变化；5. 识别负面偏见的四个子类别。

Result: 1. 非英语论文面临显著高于英语论文的偏见率；2. 负面偏见始终超过正面偏见；3. 识别出负面偏见的四个子类别，其中要求不合理跨语言泛化是最主要形式；4. 检测方法达到87.37宏F1分数。

Conclusion: 本文首次系统性地研究了NLP同行评审中的语言研究偏见，提供了数据集、检测方法和分析结果，揭示了非英语论文面临的不公平评审现象，为NLP及其他领域更公平的评审实践提供支持。
...
TL;DR: LLMs在处理多语言冲突信息时存在语言偏好偏见，普遍忽视信息冲突，自信地只给出一种答案，且对俄语有偏见，对中文有偏好。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在处理多语言冲突信息时是否存在语言偏见，特别是当不同语言提供相互矛盾的信息时，模型是否会偏向某种语言。

Method: 将"干草堆中的冲突针"范式扩展到多语言环境，使用五种不同语言的新闻领域数据进行综合评估，测试不同规模的多语言LLMs。

Result: 所有测试的LLMs（包括GPT-5.2）大多忽视信息冲突，自信地只给出一种答案；存在一致的语言偏好偏见：普遍对俄语有偏见，在最长上下文长度下对中文有偏好；这种模式在中国境内和境外训练的模型中一致，但在前者中更强。

Conclusion: LLMs在处理多语言冲突信息时确实存在系统性语言偏见，这揭示了模型在处理跨语言信息整合时的局限性，对多语言AI系统的开发具有重要意义。
...
Method: DCE包含三个机制：1）言语化尾部采样（模型自我评估并丢弃明显想法）；2）语义记忆（维护嵌入索引拒绝近重复）；3）自适应提示演化（基于记忆状态和多样性策略重构提示）。

Result: 在三个领域和两种模型上的实验显示，DCE实现0.0%崩溃率（对比朴素提示的5.6%），产生17-18个概念簇（对比2-17个），结果经独立嵌入模型验证且对参数敏感度稳定。

Conclusion: DCE有效解决跨批次模式崩溃问题，去重和提示演化单独不足但联合有效，成本约每千候选0.5美元，无需微调或定制架构。
...
TL;DR: 提出Agent-Driven Corpus Linguistics框架，让LLM代理通过工具接口连接语料库查询引擎，自动完成假设生成、查询、结果解释和分析优化，人类研究者只需设定方向和评估最终输出。


<details>
  <summary>Details</summary>
Motivation: 传统语料库语言学依赖研究人员手动操作，需要专业技能和大量时间。本文旨在降低技术门槛，让更多研究者能够快速进行语料库分析，同时确保每个发现都有可验证的语料库证据支持。

Method: 通过结构化工具使用接口将LLM连接到CQP索引的语料库查询引擎（使用Model Context Protocol），让LLM代理自动执行完整的调查循环：生成假设、查询语料库、解释结果、多轮优化分析。

Result: 在Gutenberg语料库（500万词）上，仅给定"调查英语强化词"指令，代理识别出历时接力链（so+ADJ > very > really）、三种语义变化路径和语域敏感分布。控制实验显示语料库基础提供了模型无法单独从训练数据中产生的量化和可证伪性。在CLMET语料库（4000万词）上成功复现了两项已发表研究。

Conclusion: Agent-driven corpus research能以机器速度产生经验基础的研究发现，降低技术门槛，使更广泛的研究者能够进行语料库分析，同时保持实证可验证性。
...
TL;DR: 提出一个使用语言模型作为语义上下文调节器的新框架，通过手工特征的自然语言描述来建模情感变化，在保持可解释性的同时提升预测性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于深度神经嵌入的情感预测方法缺乏可解释性，限制了专家驱动的优化。需要在无约束环境中开发既能保持特征透明度又能利用语言模型上下文抽象能力的情感建模方法。

Method: 首先从结构化领域知识中提取可解释的面部几何和声学特征，将这些特征转换为编码情感含义的符号化自然语言描述。然后使用预训练语言模型处理这些描述，生成作为情感动态高层先验的语义上下文嵌入。

Result: 在Aff-Wild2和SEWA数据集上的实验表明，该方法在效价和唤醒度的预测准确率上均优于纯手工特征和深度嵌入基线方法，实现了性能提升。

Conclusion: 语义调节能够在保持预测性能的同时实现可解释的情感建模，为完全端到端的黑盒架构提供了透明且计算高效的替代方案。
...
TL;DR: 提出双流多尺度解耦器，通过并行处理局部和全局上下文来替代深度串行处理，结合层次门控精炼器实现自适应特征优化，设计并发流并行管道克服系统瓶颈，在压缩比和吞吐量上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 传统学习数据压缩方法在精确概率建模和系统效率之间存在平衡难题。单一流架构难以同时捕捉微观语法和宏观语义特征，需要深度串行堆叠导致延迟增加。异构系统受限于设备速度不匹配，串行处理导致吞吐量受阿姆达尔定律限制。

Method: 1. 双流多尺度解耦器：解耦局部和全局上下文，用浅层并行流替代深度串行处理；2. 层次门控精炼器：实现自适应特征精炼和精确概率建模；3. 并发流并行管道：克服系统瓶颈，实现全管道并行化。

Result: 实验表明该方法在压缩比和吞吐量上均达到最先进性能，同时保持最低延迟和内存使用。

Conclusion: 提出的双流多尺度解耦架构通过并行化处理解决了学习数据压缩中的效率瓶颈问题，实现了压缩性能与系统效率的平衡。
...
TL;DR: 本文系统评估了检索增强生成（RAG）在医学问答中的表现，分析了语言模型、嵌入模型、检索策略等组件对性能的影响，发现检索增强能显著提升零样本医学问答准确率，最佳配置达到60.49%准确率。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医学问答中表现出色，但纯参数化模型存在知识缺口和事实基础不足的问题。检索增强生成通过整合外部知识检索来弥补这一局限，但现有研究对各个检索组件对性能的具体影响理解不足。

Method: 使用MedQA USMLE基准和结构化教科书知识库，在统一实验框架下分析40种配置，包括语言模型、嵌入模型、检索策略、查询重构和交叉编码器重排序的交互作用。

Result: 检索增强显著提升零样本医学问答性能，最佳配置（密集检索+查询重构+重排序）达到60.49%准确率。领域专用语言模型比通用模型能更好利用检索到的医学证据。研究发现检索效果与计算成本之间存在明显权衡。

Conclusion: 系统评估表明检索增强能有效提升医学问答性能，且可在单消费级GPU上完成，为资源有限环境下的医学RAG系统开发提供了实用指导。领域专用模型和精心设计的检索策略是关键成功因素。
...
### [208] [Joint Optimization of Reasoning and Dual-Memory for Self-Learning Diagnostic Agent](https://arxiv.org/abs/2604.07269)
...
<details>
  <summary>Details</summary>
Motivation: 临床专业知识不仅通过获取医学知识，还通过积累产生可重用诊断模式的经验而提高。现有的LLM诊断代理大多独立处理病例，限制了经验重用和持续适应能力。

Method: 提出SEA自学习诊断代理，采用认知启发的双记忆模块设计，并构建强化学习框架来联合优化推理和记忆管理。

Result: 在MedCaseReasoning数据集上达到92.46%准确率，比最强基线提升19.6%；在ER-Reason数据集上获得最佳最终准确率(0.7214)和最大改进(+0.35 Acc@100)。专家评估确认生成的规则具有临床正确性、实用性和可信度。

Conclusion: SEA通过将经验有效转化为可重用知识，显著提升了诊断推理能力和持续学习性能，双记忆模块中归纳的规则可靠且具有实际意义。
...
### [209] [ClickGuard: A Trustworthy Adaptive Fusion Framework for Clickbait Detection](https://arxiv.org/abs/2604.07272)
*Chhavi Dhiman,Naman Chawla,Riya Dhami,Gaurav Kumar,Ganesh Naik*

Main category: cs.CL

TL;DR: ClickGuard是一个用于点击诱饵检测的可信自适应融合框架，结合BERT嵌入和结构特征，通过SSAFB动态集成，使用混合CNN-BiLSTM捕获模式，达到96.93%测试准确率。


<details>
  <summary>Details</summary>
Motivation: 点击诱饵标题的广泛使用误导用户并损害在线可信度，这些标题采用耸人听闻、误导性声明和模糊语言，需要有效的检测方法来确保数字内容的可信度。

Method: 提出ClickGuard框架，结合BERT嵌入和结构特征，使用语法-语义自适应融合块(SSAFB)进行动态集成，采用混合CNN-BiLSTM架构捕获模式依赖关系。

Result: 模型达到96.93%测试准确率，优于现有方法。通过LIME和PFI评估可信度，消融研究验证SSAFB有效性，在不同数据集上表现稳健。
...
TL;DR: LLMs在基于上下文语法描述进行形式语言翻译任务中表现不佳，准确率随语法规模和句子长度显著下降，形态和书写差异严重影响性能，主要错误类型包括词汇选择错误、幻觉和未翻译。


<details>
  <summary>Details</summary>
Motivation: 低资源语言机器翻译面临数据不足的挑战，本研究旨在探索LLMs是否能够通过上下文提供的语法描述（如教科书、词典）来推断语言间的联系，从而绕过对大量训练数据的依赖。

Method: 构建同步上下文无关文法来定义形式语言对，模拟自然语言的语法、形态和书写特征。在给定语法和源语言句子的条件下，测量LLMs将句子从一个形式语言翻译到另一个的能力，并系统变化语法规模、句子长度、语言特性和书写系统等参数。

Result: 1) LLMs翻译准确率随语法规模和句子长度显著下降；2) 源语言和目标语言之间的形态和书写差异会严重降低模型性能；3) 主要错误类型包括：错误回忆目标语言词汇、幻觉新词汇、以及未翻译源语言词汇。

Conclusion: LLMs基于上下文语法描述进行翻译的能力有限，特别是在处理复杂语法、长句子和语言差异时表现不佳，这限制了其在低资源语言翻译中的实用性。
...
### [211] [Why teaching resists automation in an AI-inundated era: Human judgment, non-modular work, and the limits of delegation](https://arxiv.org/abs/2604.07285)
*Songhee Han*

Main category: cs.CL

TL;DR: 该论文认为教学本质上具有解释性、关系性和专业判断性，难以被AI完全自动化，尽管AI可以支持某些有限功能。


<details>
  <summary>Details</summary>
Motivation: 回应当前关于AI能够自动化教学的过度乐观观点，指出这些观点错误地将教学视为模块化和程序化的工作，忽略了教学实践中不可分割的复杂性。

Method: 基于近期文献和大型语言模型、检索增强生成系统的实证研究，从理论层面分析教学工作的本质特征及其与AI能力的对比。

Result: 教学因其解释性、关系性和专业判断性而难以被有意义地自动化；教学和学习受到人类认知、行为、动机和社会互动的塑造，这些无法被完全规定、预测或建模；看似可分离的任务在实践中从跨学习者、情境和关系的持续情境解释中获得教学价值。

Conclusion: 只要教育实践依赖于对人类认知和学习的新兴理解，教学就仍然是一种抵制自动化的专业工作；AI可以提高信息获取和支持特定教学活动，但不能取代有效教学所需的人类判断和关系责任。
...
### [212] [OpenSpatial: A Principled Data Engine for Empowering Spatial Intelligence](https://arxiv.org/abs/2604.07296)
...
TL;DR: OpenSpatial是一个开源空间数据引擎，采用3D边界框作为基础单元构建数据层次结构，覆盖5个核心空间任务，并发布了包含300万高质量样本的OpenSpatial-3M数据集，显著提升了空间推理模型的性能。


<details>
  <summary>Details</summary>
Motivation: 当前空间智能研究主要关注特定领域的数据生成，缺乏能够充分发挥高质量空间数据潜力的原则性开源引擎。为了填补这一空白，需要建立一个能够支持高质量、可扩展、任务多样且高效的空间数据生成系统。

Method: 提出OpenSpatial开源数据引擎，以3D边界框作为基础单元构建数据层次结构，涵盖五个核心空间任务：空间测量、空间关系、相机感知、多视角一致性和场景感知推理。基于这一可扩展基础设施，构建了包含300万高质量样本的OpenSpatial-3M数据集。

Result: 在OpenSpatial-3M数据集上训练的通用模型在广泛的空间推理基准测试中取得了最先进的性能。最佳模型相对平均提升了19%。研究还系统分析了数据属性如何影响空间感知能力。

Conclusion: 通过开源引擎和300万规模的数据集，为加速未来空间智能研究提供了坚实基础。OpenSpatial填补了高质量空间数据生成系统的空白，显著提升了空间推理模型的性能。
...
TL;DR: 提出Personalized RewardBench基准，用于评估奖励模型对个性化偏好的建模能力，发现现有SOTA模型在个性化任务上表现不佳（最高仅75.94%准确率），且该基准与下游任务性能相关性更高。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型基准主要评估通用响应质量，缺乏对个性化用户偏好建模能力的评估。随着多元化对齐成为LLM发展的关键前沿，需要专门评估奖励模型如何捕捉多样化人类价值观。

Method: 构建基于用户特定准则的偏好对（chosen/rejected responses），确保偏好差异完全由个人偏好决定。通过人工评估确认主要判别因素是个人偏好而非通用质量。测试现有SOTA奖励模型，并验证基准与下游任务（Best-of-N采样和PPO）性能的相关性。

Result: 现有奖励模型在个性化任务上表现显著不足，最高准确率仅75.94%。更重要的是，Personalized RewardBench与下游任务性能的相关性显著高于现有基准，证明其作为评估奖励模型下游应用性能的有效代理。

Conclusion: Personalized RewardBench是一个稳健且准确的基准，能够有效评估奖励模型在个性化偏好建模方面的能力，并为预测模型在下游应用中的性能提供了更好的指标。
...
TL;DR: GS-Surrogate：基于可变形高斯泼溅的可视化代理模型，用于参数空间探索，通过分离模拟变化与可视化变化实现实时灵活探索


<details>
  <summary>Details</summary>
Motivation: 当前可视化代理模型存在局限性：要么在图像空间操作缺乏明确3D表示，要么依赖计算昂贵的神经辐射场将所有参数变化编码在单个隐式场中，难以支持交互式探索

Method: 构建规范高斯场作为基础3D表示，通过序列化参数条件变形进行适应，分离模拟相关变化与可视化特定变化，支持等值面提取和传递函数编辑等任务

Result: 在多个模拟数据集上评估，GS-Surrogate能够在模拟和可视化参数空间实现实时灵活的探索

Conclusion: GS-Surrogate通过可变形高斯泼溅框架，为参数空间探索提供了高效可控的可视化代理模型，解决了现有方法的局限性
...
TL;DR: 研究通过GPU降频和功耗限制模拟不同性能等级，分析3D高斯泼溅在边缘设备上的实时渲染可行性，探索功耗-性能平衡关系。


<details>
  <summary>Details</summary>
Motivation: 探索3D高斯泼溅（3DGS）在边缘客户端实时渲染的可行性，特别是在不同高斯泼溅数量和GPU计算预算下的性能表现。目标是了解在能源受限环境（如独立头显和瘦客户端）中部署3DGS的实际下限。

Method: 采用基于仿真的方法，通过系统性地降低GPU核心频率和施加功耗限制，在单个高端GPU上模拟不同GPU能力层级。在不同性能点上测量帧率、运行时行为和功耗，分析功耗-性能关系如FPS-功耗曲线、每帧能耗和性能功耗比。

Result: 该方法能够近似模拟从嵌入式、移动级设备到高端消费级系统的多样化GPU性能范围，为边缘部署的3DGS系统提供性能-能耗权衡的早期见解。

Conclusion: 通过分析不同复杂度场景、流水线和优化下的性能表现，评估了客户端3DGS渲染在能源受限环境中的部署潜力，为边缘部署的3DGS系统可行性提供了重要参考。
...
### [216] [High-Precision Estimation of the State-Space Complexity of Shogi via the Monte Carlo Method](https://arxiv.org/abs/2604.06189)
*Sotaro Ishii,Tetsuro Tanaka*

Main category: cs.AI

TL;DR: 使用蒙特卡洛采样和基于"王-王"位置反向搜索的新方法，将将棋可达位置数精确估计为6.55×10⁶⁸，显著缩小了之前10⁶⁴-10⁶⁹的估计范围。


<details>
  <summary>Details</summary>
Motivation: 将棋的状态空间复杂度估计存在巨大差距（10⁶⁴到10⁶⁹），传统组合估计方法无法准确区分从初始位置可达的有效棋盘配置。

Method: 结合蒙特卡洛采样和新型可达性测试，采用向"王-王"位置集合的反向搜索，而非单一目标的初始位置反向搜索，大幅减少了不可达性判定的搜索工作量。

Result: 基于50亿个位置的采样，估计将棋合法位置数为6.55×10⁶⁸（三位有效数字），3σ置信水平，显著改进了先前已知的边界。迷你将棋复杂度约为2.38×10¹⁸。

Conclusion: 提出的反向搜索方法有效解决了将棋状态空间估计问题，将估计范围从五个数量级缩小到精确值，为复杂博弈游戏的状态空间分析提供了新工具。
...
### [217] [Blind Refusal: Language Models Refuse to Help Users Evade Unjust, Absurd, and Illegitimate Rules](https://arxiv.org/abs/2604.06233)
*Cameron Pattison,Lorenzo Manuali,Seth Lazar*

Main category: cs.AI

TL;DR: 语言模型存在"盲目拒绝"问题：即使面对不合理的规则，模型也会机械地拒绝帮助用户规避规则，这表明其拒绝行为与道德推理能力脱节。


<details>
  <summary>Details</summary>
Motivation: 研究动机是发现语言模型在安全训练后存在过度拒绝的问题。模型会拒绝所有违反规则的请求，而不考虑规则本身是否合理、公正或存在例外情况。这种"盲目拒绝"反映了模型缺乏真正的道德推理能力。

Method: 研究方法包括：1) 构建包含5种规则失效类型和19种权威类型的合成案例数据集；2) 通过三个自动化质量门控和人工审查验证数据；3) 收集来自7个模型家族的18种配置的响应；4) 使用盲审的GPT-5.4 LLM作为评判者，从两个维度分类响应：响应类型（帮助、硬拒绝、回避）和是否识别规则失效的原因。

Result: 研究发现：1) 模型对75.4%（N=14,650）的规则失效请求都予以拒绝；2) 即使在请求本身没有安全或双重用途风险的情况下，模型仍然拒绝；3) 模型在57.5%的情况下确实识别了规则失效的原因，但仍然拒绝帮助，表明拒绝行为与规范性推理能力脱钩。

Conclusion: 结论是当前语言模型存在"盲目拒绝"问题，其拒绝机制与真正的道德推理能力分离。模型能够识别规则的不合理性，但仍然机械地拒绝帮助，这表明需要开发更精细的道德推理系统，而不仅仅是基于规则的拒绝机制。
...
### [218] [Toward Reducing Unproductive Container Moves: Predicting Service Requirements and Dwell Times](https://arxiv.org/abs/2604.06251)
*Elena Villalobos,Adolfo De Unánue T.,Fernanda Sobrino,David Aké,Stephany Cisneros,Jorge Lecona,Alejandra Matadamaz*

Main category: cs.AI

TL;DR: 利用机器学习预测集装箱预清关需求和停留时间，减少码头无效移动，提升运营效率
...
Method: 开发机器学习模型，利用历史运营数据预测集装箱预清关需求和停留时间；实施货物描述分类系统和收货人记录去重以提高数据质量

Result: 模型在多个时间验证期间，在精确率和召回率上持续优于现有规则启发式方法和随机基线
...
### [219] [Weakly Supervised Distillation of Hallucination Signals into Transformer Representations](https://arxiv.org/abs/2604.06277)
*Shoaib Sadiq Salehmohamed,Jinal Prashant Thakkar,Hansika Aredla,Shaik Mohammed Omar,Shalmali Ayachit*

Main category: cs.AI

TL;DR: 提出一种弱监督框架，将外部验证信号蒸馏到LLM内部表示中，实现仅通过内部激活检测幻觉，无需推理时外部验证


<details>
  <summary>Details</summary>
Motivation: 现有幻觉检测方法依赖推理时的外部验证（如黄金答案、检索系统或辅助模型），这限制了实际应用。研究探索是否可以将这种外部监督蒸馏到模型自身表示中，实现仅通过内部激活进行检测

Method: 1) 使用三种互补的弱监督信号（子串匹配、句子嵌入相似度、LLM作为评判者）构建标注数据集；2) 从SQuAD v2构建15,000个样本数据集，包含LLaMA-2-7B生成答案及其各层隐藏状态；3) 训练五种探测分类器（MLP和Transformer架构）直接在这些隐藏状态上进行幻觉检测

Result: Transformer-based probes表现最佳，M2在5折平均AUC/F1上最好，M3在单折验证和保留测试集上最好。推理效率高：探测延迟0.15-6.66ms，端到端生成+探测吞吐量约0.231 QPS，实际开销可忽略

Conclusion: 成功证明幻觉检测信号可以被蒸馏到transformer表示中，实现无需推理时外部验证的内部检测，为高效幻觉检测提供了新途径
...
### [220] [SymptomWise: A Deterministic Reasoning Layer for Reliable and Efficient AI Systems](https://arxiv.org/abs/2604.06375)
*Isaac Henry,Avery Byrne,Christopher Giza,Ron Henry,Shahram Yazdani*

Main category: cs.AI

TL;DR: SymptomWise框架通过分离语言理解和诊断推理，结合专家知识、确定性推理和受限的LLM使用，提高AI症状分析系统的可靠性、可解释性和可追溯性。
...
Method: 将语言理解与诊断推理分离，结合专家策划的医学知识、确定性代码驱动推理和受限的大语言模型使用。自由文本输入映射到验证的症状表示，然后由确定性推理模块在有限假设空间中进行评估，生成排名鉴别诊断。语言模型仅用于症状提取和可选解释，不用于诊断推理。

Result: 在42个专家撰写的具有挑战性的儿科神经病学病例上进行初步评估，显示与临床医生共识有意义的重叠，正确诊断出现在前五名鉴别诊断中的比例为88%。

Conclusion: 该架构提高了可追溯性，减少了无支持的结论，并支持系统组件的模块化评估。该框架可推广到其他溯因推理领域，可作为基础模型的确定性结构和路由层，提高精度并减少有界任务中不必要的计算开销。
...
### [221] [SELFDOUBT: Uncertainty Quantification for Reasoning LLMs via the Hedge-to-Verify Ratio](https://arxiv.org/abs/2604.06389)
*Satwik Pandey,Suresh Raghu,Shashwat Pandey*

Main category: cs.AI

TL;DR: SELFDOUBT：一种单次推理的不确定性估计框架，通过分析推理轨迹中的行为信号（特别是Hedge-to-Verify Ratio）来评估模型置信度，适用于无法获取内部概率的专有API模型。


<details>
  <summary>Details</summary>
Motivation: 当前不确定性估计方法存在部署困难：基于采样的方法计算成本高，而单次推理的代理指标（如口头置信度或轨迹长度）在不同模型间不一致。对于不暴露logits或中间token概率的专有推理API，缺乏可靠的不确定性信号。

Method: 提出SELFDOUBT框架，通过提取推理轨迹本身的行为信号来评估不确定性。核心信号是Hedge-to-Verify Ratio (HVR)，检测推理轨迹是否包含不确定性标记，以及这些标记是否被显式的自我检查行为所抵消。该方法仅需单次推理轨迹，无需多次采样或模型内部信息。

Result: 在7个模型和3个多步推理基准测试（BBH、GPQA-Diamond、MMLU-Pro）上评估。结果显示：不含不确定性标记的轨迹正确率达96%；完整的SELFDOUBT分数在10倍更低推理成本下显著优于基于采样的语义熵方法；部署级联策略在无任务特定标签情况下达到90%准确率和71%覆盖率。

Conclusion: SELFDOUBT为专有推理模型提供了一个可扩展、生产就绪的不确定性估计基础框架，解决了实际部署中的计算成本和API限制问题。
...
### [222] [Qualixar OS: A Universal Operating System for AI Agent Orchestration](https://arxiv.org/abs/2604.06392)
*Varun Pratap Bhardwaj*

Main category: cs.AI

TL;DR: Qualixar OS 是首个应用层操作系统，用于通用AI智能体编排，支持异构多智能体系统，包含执行语义、团队设计引擎、三层模型路由、共识判断管道、四层内容归属、通用兼容性和生产仪表板等功能。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：内核级方法（如AIOS）或单框架工具（如AutoGen、CrewAI）无法满足异构多智能体系统的需求。需要统一的运行时环境来协调不同LLM提供商、智能体框架和传输协议。

Method: 1) 12种多智能体拓扑的执行语义；2) Forge LLM驱动的团队设计引擎；3) 三层模型路由（Q-learning、五种策略、贝叶斯POMDP）；4) 共识判断管道（Goodhart检测、JSD漂移监测）；5) 四层内容归属（HMAC签名、隐写水印）；6) Claw Bridge通用兼容性（MCP和A2A协议）；7) 24标签生产仪表板。

Result: 在2821个测试用例（217种事件类型，8个质量模块）上验证。在20任务评估套件中，系统达到100%准确率，平均每个任务成本$0.000039。

Conclusion: Qualixar OS 提供了首个完整的应用层操作系统，用于通用AI智能体编排，解决了异构多智能体系统的协调问题，具有高准确率和低成本的优势，采用Elastic License 2.0开源许可。
...
### [223] [ProofSketcher: Hybrid LLM + Lightweight Proof Checker for Reliable Math/Logic Reasoning](https://arxiv.org/abs/2604.06401)
...
<details>
  <summary>Details</summary>
Motivation: LLM生成的数学逻辑论证常含细微错误但难以察觉，而交互式定理证明器虽可靠但需要完全形式化证明，成本高昂

Method: 设计混合管道：LLM在紧凑DSL中生成类型化证明草图，轻量级可信核将草图扩展为显式证明义务

Result: 结合LLM的创造性推理能力和形式验证的可靠性，降低形式化证明成本

Conclusion: 混合方法平衡了LLM的灵活性和形式验证的严谨性，为数学逻辑证明提供实用解决方案
...
### [224] [BDI-Kit Demo: A Toolkit for Programmable and Conversational Data Harmonization](https://arxiv.org/abs/2604.06405)
*Roque Lopez,Yurong Liu,Christos Koutras,Juliana Freire*

Main category: cs.AI

TL;DR: BDI-Kit是一个用于数据模式与值匹配的可扩展工具包，提供Python API和AI辅助聊天界面两种接口，支持用户通过编程或自然语言对话进行数据协调。
...
Method: 提供两种互补接口：Python API供开发者编程构建协调管道；AI辅助聊天界面让领域专家通过自然语言对话进行数据协调。支持自动化匹配、AI辅助推理和用户驱动细化的迭代工作流程。

Result: 展示了两种使用场景：(1) 使用Python API编程组合原语、检查中间输出和重用转换；(2) 与AI助手自然语言对话访问BDI-Kit功能，基于助手建议迭代细化输出。

Conclusion: BDI-Kit通过结合自动化匹配、AI辅助推理和用户驱动细化，为不同用户需求提供了灵活的数据协调解决方案，支持迭代探索、验证和优化模式与值匹配。
...
### [225] [On Emotion-Sensitive Decision Making of Small Language Model Agents](https://arxiv.org/abs/2604.06562)
*Jiaju Lin,Xingjian Du,Qingyun Wu,Ellen Wenting Zou,Jindong Wang*

Main category: cs.AI

TL;DR: 研究通过表示层情感诱导和博弈论评估，探索小语言模型在情感敏感决策中的表现，发现情感扰动会系统影响战略选择，但行为不稳定且不完全符合人类预期。


<details>
  <summary>Details</summary>
Motivation: 当前小语言模型作为交互决策代理的评估大多忽略了情感作为影响行为的因果因素，需要研究情感敏感决策以填补这一空白。

Method: 结合表示层情感诱导与结构化博弈论评估，使用基于众包验证的真实情感诱发文本进行激活引导，创建包含合作与竞争激励、完全与不完全信息的基准测试，并在多个模型家族中进行实验。

Result: 情感扰动会系统性地影响战略选择，但产生的行为往往不稳定且不完全符合人类预期，不同模型家族表现出不同的情感敏感性。

Conclusion: 需要改进模型对情感驱动扰动的鲁棒性，并提出了相应的改进方法，强调情感因素在AI决策系统中的重要性。
...
### [226] [Rethinking Generalization in Reasoning SFT: A Conditional Analysis on Optimization, Data, and Model Capability](https://arxiv.org/abs/2604.06628)
*Qihan Ren,Peng Wang,Ruikun Cai,Shuai Shao,Dadi Guo,Yuejin Xie,Yafu Li,Quanshi Zhang,Xia Hu,Jing Shao,Dongrui Liu*

Main category: cs.AI

TL;DR: 本文重新审视了LLM后训练中的主流观点：监督微调（SFT）只是记忆，而强化学习（RL）才能泛化。研究发现，对于带有长思维链（CoT）监督的推理SFT，跨领域泛化并非不存在，而是有条件的，受优化动态、训练数据和基础模型能力共同影响。


<details>
  <summary>Details</summary>
Motivation: 挑战LLM后训练中的主流叙事，即SFT只是记忆训练数据，而RL才能实现泛化。特别针对带有长思维链监督的推理SFT，探究其跨领域泛化的真实情况。

Method: 通过分析优化动态、训练数据质量和结构、基础模型能力等多个维度，研究推理SFT的泛化特性。包括观察训练过程中的"下降-恢复"模式、使用不同质量的CoT数据、以及在不同能力的基础模型上进行实验。

Result: 1. 跨领域泛化存在但条件复杂：优化不足会导致性能先下降后恢复；2. 数据质量至关重要：低质量解决方案损害泛化，已验证的长CoT轨迹带来一致增益；3. 模型能力是关键：强模型能内化可转移的程序模式，弱模型只能模仿表面冗长；4. 泛化具有不对称性：推理能力提升但安全性下降。

Conclusion: 推理SFT的泛化问题不应简单化为"是否泛化"，而应探讨"在什么条件下"和"以什么代价"泛化。泛化是条件性的，受优化、数据、模型能力共同影响，且存在推理能力提升与安全性下降的不对称性。
...
### [227] [KD-MARL: Resource-Aware Knowledge Distillation in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2604.06691)
...
<details>
  <summary>Details</summary>
Motivation: 现实世界多智能体强化学习部署受限于计算、内存和推理时间资源。专家策略虽然性能高但依赖昂贵的决策周期和大规模模型，不适合边缘设备或嵌入式平台。现有MARL知识蒸馏方法过于关注动作模仿，忽视了协调结构和智能体能力异质性。

Method: 提出资源感知的多智能体强化学习知识蒸馏框架KD-MARL，采用两阶段方法：1) 从集中式专家策略转移到轻量级去中心化学生代理；2) 学生策略训练不使用评论家，而是依赖蒸馏的优势信号和结构化策略监督，在异构和有限观测下保持协调。支持异构学生架构，使每个智能体模型容量与其观测复杂度匹配。

Result: 在SMAC和MPE基准测试中，KD-MARL保持超过90%的专家性能，同时将计算成本降低高达28.6倍（FLOPs）。该方法实现了专家级协调并通过结构化蒸馏保持协调，使MARL能够在资源受限的平台上实际部署。

Conclusion: KD-MARL框架成功地将集中式专家策略的知识转移到轻量级去中心化学生代理，在显著降低计算成本的同时保持高性能和协调能力，为资源受限平台上的多智能体强化学习实际部署提供了可行方案。
...
### [228] [Reasoning Fails Where Step Flow Breaks](https://arxiv.org/abs/2604.06695)
*Xiaoyu Xu,Yulan Pan,Xiaosong Yuan,Zhihong Shen,Minghao Su,Yuanhao Su,Xiaofeng Zhang*

Main category: cs.AI

TL;DR: Step-Saliency方法分析大型推理模型的信息流故障，StepFlow干预方法无需重训练即可提升推理性能
...
Result: Step-Saliency揭示了两种重复出现的信息流故障：浅层锁定和深层衰减；StepFlow无需重训练即可在数学、科学和编程任务上提高多个大型推理模型的准确性

Conclusion: 修复信息流可以恢复大型推理模型缺失的部分推理性能，Step-Saliency为分析长推理轨迹提供了有效工具，StepFlow展示了通过测试时干预改善模型性能的潜力
...
### [229] [AgentGate: A Lightweight Structured Routing Engine for the Internet of Agents](https://arxiv.org/abs/2604.06696)
*Yujun Cheng,Enfang Cui,Hao Qin,Zhiyuan Liang,Qi Xu*

Main category: cs.AI

TL;DR: AgentGate：一个轻量级结构化路由引擎，用于在AI代理系统中进行候选感知的请求分发，将路由问题分解为动作决策和结构落地两阶段，通过路由导向微调使紧凑模型在资源受限环境下实现高效路由。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理系统快速发展形成"代理互联网"，代理分布在本地设备、边缘节点、私有服务和云平台中。现有系统虽然改进了代理命名、发现和交互，但在延迟、隐私和成本约束下的高效请求分发仍是一个开放的系统问题。

Method: AgentGate将路由问题重新定义为约束决策问题，分解为两阶段：1）动作决策阶段决定查询触发单代理调用、多代理规划、直接响应或安全升级；2）结构落地阶段将选定动作实例化为可执行输出（目标代理、结构化参数或多步计划）。采用路由导向微调方案，包含候选感知监督和硬负例训练。

Result: 在精心策划的路由基准测试中，多个3B-7B开源模型显示紧凑模型在约束设置下能提供有竞争力的路由性能。模型差异主要体现在动作预测、候选选择和结构落地质量上。紧凑模型在资源受限部署条件下表现良好。

Conclusion: 结构化路由是实现高效和隐私感知代理系统的可行设计点，特别适用于路由决策必须在资源受限部署条件下进行的情况。AgentGate展示了紧凑模型在代理路由任务中的潜力。
...
### [230] [ATANT: An Evaluation Framework for AI Continuity](https://arxiv.org/abs/2604.06710)
*Samuel Sameer Tanguturi*

Main category: cs.AI

TL;DR: ATANT是一个评估AI系统连续性能力的自动化测试框架，包含250个故事和1835个验证问题，通过7个属性和10个检查点评估系统在不同时间点上保持、更新、消歧和重建上下文的能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI行业虽然开发了各种记忆组件（RAG管道、向量数据库、长上下文窗口、配置文件层），但缺乏正式框架来定义和衡量这些组件是否产生真正的连续性能力。

Method: 定义了连续性作为系统属性的7个必需特性，引入了10个检查点的评估方法（无需LLM参与评估循环），创建了包含250个故事、1835个验证问题的叙事测试语料库，涵盖6个生活领域。

Result: 参考实现从传统架构的58%准确率提升到孤立模式下的100%（250个故事），50个故事累积模式下100%，250个故事累积模式下96%。累积结果是主要衡量指标，测试系统在250个不同生活叙事共存时能否正确检索事实而不交叉污染。

Conclusion: ATANT是一个系统无关、模型独立的框架，为构建和验证连续性系统提供了序列化方法。框架规范、示例故事和评估协议已开源，完整的250个故事语料库将逐步发布。
...
### [231] [Steering the Verifiability of Multimodal AI Hallucinations](https://arxiv.org/abs/2604.06714)
*Jianhong Pang,Ruoxi Cheng,Ziyi Ye,Xingjun Ma,Zuxuan Wu,Xuanjing Huang,Yu-Gang Jiang*

Main category: cs.AI

TL;DR: 该研究提出了一种针对多模态大语言模型幻觉的可验证性控制方法，通过激活空间干预技术区分明显和难以察觉的幻觉，实现对模型输出可验证性的精细调控。


<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型产生的幻觉对人类用户构成风险，但不同幻觉的可验证性差异很大：有些容易被人类发现（明显幻觉），有些则难以察觉或需要更多验证努力（难以察觉幻觉）。目前缺乏针对AI应用不同安全和可用性需求来控制这种可验证性的研究。

Method: 1. 从4,470个人类对AI生成幻觉的响应中构建数据集，根据人类用户的可验证性将幻觉分为明显和难以察觉两类；2. 提出激活空间干预方法，为明显和难以察觉幻觉分别学习不同的探针；3. 通过混合干预实现不同场景所需可验证性的灵活控制。

Result: 实验结果表明：1. 明显和难以察觉幻觉引发不同的干预探针，允许对模型可验证性进行精细控制；2. 针对性干预在调节相应可验证性方面表现优异；3. 简单混合这些干预能够灵活控制不同场景所需的可验证性。

Conclusion: 该研究首次系统探索了多模态AI幻觉的可验证性控制问题，提出的激活空间干预方法能够有效区分和调控明显与难以察觉的幻觉，为满足不同安全和可用性需求的AI应用提供了实用解决方案。
...
### [232] [TurboAgent: An LLM-Driven Autonomous Multi-Agent Framework for Turbomachinery Aerodynamic Design](https://arxiv.org/abs/2604.06747)
*Juan Du,Yueteng Wu,Pan Zhao,Yuze Liu,Min Zhang,Xiaobin Xu,Xinglong Zhang*

Main category: cs.AI

TL;DR: TurboAgent是一个基于大语言模型驱动的自主多智能体框架，用于涡轮机械气动设计与优化，将传统试错设计转变为数据驱动的协作工作流，实现从自然语言需求到最终设计的闭环自主设计过程。
...
Result: 在跨音速单转子压缩机验证中，目标性能、生成设计和CFD仿真结果高度一致，质量流量、总压比和等熵效率的决定系数均超过0.91，归一化RMSE值低于8%，优化智能体将等熵效率提高1.61%，总压比提高3.02%，整个工作流可在约30分钟内并行完成。
...
### [233] [FVD: Inference-Time Alignment of Diffusion Models via Fleming-Viot Resampling](https://arxiv.org/abs/2604.06779)
...
Method: FVD受Fleming-Viot种群动力学启发，用专门的生死机制替代多项式重采样。当奖励仅近似可用时，将独立的基于奖励的生存决策与随机重生噪声相结合，处理确定性轨迹崩溃问题，形成灵活的种群动力学。

Result: 在DrawBench上比先前方法在ImageReward上提升7%；在类别条件任务中，FID比强基线提升14-20%，比基于价值的方法快达66倍；完全可并行化，能高效扩展推理计算。

Conclusion: FVD通过Fleming-Viot种群动力学有效解决了SMC扩散采样器的多样性崩溃问题，在保持轨迹支持的同时探索奖励倾斜分布，无需价值函数近似或昂贵rollout，在多个任务中取得显著性能提升。
...
### [234] [Riemann-Bench: A Benchmark for Moonshot Mathematics](https://arxiv.org/abs/2604.06802)
*Suhaas Garre,Erik Knutsen,Sushant Mehta,Edwin Chen*

Main category: cs.AI

TL;DR: 研究者创建了一个名为\bench{}的私有基准测试，包含25个专家级数学问题，用于评估AI在超越奥数水平的研究级数学推理能力，结果显示前沿模型得分均低于10%。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在国际数学奥林匹克竞赛中已达到金牌水平，但竞赛数学仅代表数学推理的狭窄领域：问题来自有限领域、需要最少的高级工具，且经常奖励技巧性解法而非深度理论知识。需要评估AI在真正研究级数学上的能力。

Method: 创建包含25个专家级数学问题的私有基准测试\bench{}，问题由常春藤盟校数学教授、研究生和拥有博士学位的IMO奖牌得主编写，每个问题都经过两位独立领域专家的双盲验证，需要从头解决。评估前沿模型作为无约束研究代理，完全访问编码工具、搜索和开放式推理，使用无偏统计估计器对每个问题进行100次独立运行计算。

Result: 所有前沿模型在当前基准测试中得分均低于10%，揭示了奥数级问题解决与真正研究级数学推理之间的巨大差距。

Conclusion: 通过保持基准测试完全私有，确保测量的性能反映真实的数学能力而非训练数据记忆。研究结果表明，虽然AI在竞赛数学上表现出色，但在研究级数学推理方面仍有显著不足。
...
### [235] [Beyond Surface Judgments: Human-Grounded Risk Evaluation of LLM-Generated Disinformation](https://arxiv.org/abs/2604.06820)
*Zonghuan Xu,Xiang Zheng,Yutao Wu,Xingjun Ma*

Main category: cs.AI

TL;DR: 研究发现LLM评估者与人类读者在内容评价上存在系统性差异，LLM评估者内部一致性高但与人类读者对齐度低，表明内部一致性不能作为代理有效性的证据。


<details>
  <summary>Details</summary>
Motivation: 随着LLM被越来越多地用作低成本的人类评估替代品，需要验证LLM评估者是否能准确追踪人类读者的真实反应，特别是在评估LLM生成内容的可信度风险方面。

Method: 将评估问题重构为代理有效性问题，使用290篇对齐文章、2,043对人类评分和8个前沿LLM评估者的输出，从整体评分、项目级排序和信号依赖三个维度比较LLM评估者与人类读者的对齐程度。

Result: 发现LLM评估者与人类读者存在持续差距：LLM评估者通常更苛刻，项目级人类排名恢复能力弱，依赖不同的文本信号（更重视逻辑严谨性，更强烈惩罚情感强度），同时LLM评估者之间的内部一致性远高于与人类读者的一致性。

Conclusion: LLM评估者形成了一个连贯的评估群体，其内部一致性远高于与人类读者的一致性，表明内部一致性不能作为代理有效性的证据，需要谨慎使用LLM作为人类读者反应的代理。
...
### [236] [Explaining Neural Networks in Preference Learning: a Post-hoc Inductive Logic Programming Approach](https://arxiv.org/abs/2604.06838)
*Daniele Fossemò,Filippo Mignosi,Giuseppe Placidi,Luca Raggioli,Matteo Spezialetti,Fabio Aurelio D'Asaro*

Main category: cs.AI

TL;DR: 使用答案集学习(ILASP)来近似神经网络偏好学习模型，通过降维预处理解决高维特征空间挑战


<details>
  <summary>Details</summary>
Motivation: 需要为黑盒模型（如神经网络）提供可解释的近似，特别是在用户偏好学习场景中，以增强模型透明度和可解释性

Method: 使用ILASP（答案集程序归纳学习）通过弱约束来近似神经网络偏好学习系统，采用PCA进行降维预处理以处理高维特征空间

Result: 实验研究了ILASP作为神经网络的全局和局部近似器，在保持对目标模型适当保真度的同时控制计算时间增长

Conclusion: ILASP可以有效地近似神经网络偏好学习模型，降维预处理有助于在保持解释透明性的同时处理高维数据挑战
...
### [237] [What's Missing in Screen-to-Action? Towards a UI-in-the-Loop Paradigm for Multimodal GUI Reasoning](https://arxiv.org/abs/2604.06995)
*Songze Li,Xiaoke Guo,Tianqi Liu,Biao Yi,Zhaoyan Gong,Zhiqiang Liu,Huajun Chen,Wen Zhang*

Main category: cs.AI

TL;DR: UILoop：一种创新的GUI推理范式，将GUI推理视为屏幕-UI元素-动作的循环过程，通过让多模态大语言模型显式学习UI元素的定位、语义功能和使用方法，实现精确元素发现和可解释推理。


<details>
  <summary>Details</summary>
Motivation: 现有GUI推理任务在UI理解方面仍面临挑战，当前方法通常依赖基于屏幕的直接决策，缺乏可解释性且忽视对UI元素的全面理解，最终导致任务失败。需要增强对UI的理解和交互能力。

Method: 提出UI-in-the-Loop (UILoop)范式，将GUI推理任务视为循环的屏幕-UI元素-动作过程。让MLLMs显式学习关键UI元素的定位、语义功能和实际使用方法。同时引入包含三个评估指标的UI理解任务，并贡献了包含26K样本的UI Comprehension-Bench基准。

Result: 大量实验表明，UILoop在UI理解方面达到了最先进的性能，同时在GUI推理任务中取得了优越的结果。

Conclusion: UILoop通过将GUI推理建模为循环过程，并让模型显式学习UI元素的关键特性，显著提升了UI理解和GUI推理的性能，为解决现有方法的局限性提供了有效方案。
...
### [238] [EmoMAS: Emotion-Aware Multi-Agent System for High-Stakes Edge-Deployable Negotiation with Bayesian Orchestration](https://arxiv.org/abs/2604.07003)
*Yunbo Long,Yunhan Liu,Liming Xu*

Main category: cs.AI

TL;DR: EmoMAS是一个贝叶斯多智能体框架，通过战略性地协调游戏理论、强化学习和心理一致性模型，优化情感决策，实现无需预训练的高风险边缘部署谈判AI。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自动谈判中计算成本高且存在隐私风险，而小型语言模型难以处理高风险谈判中的复杂情感动态。需要开发适合隐私敏感边缘设备部署的谈判AI系统。

Method: 提出EmoMAS贝叶斯多智能体框架，包含贝叶斯协调器和三个专门智能体：游戏理论模型、强化学习模型和心理一致性模型。系统融合实时洞察优化情感状态转换，基于谈判反馈持续更新智能体可靠性，采用无需预训练的在线策略学习。

Result: 在债务、医疗、应急响应和教育四个高风险边缘部署谈判基准测试中，配备EmoMAS的SLMs和LLMs在所有基准上都持续超越基线模型，在谈判性能和伦理行为之间取得平衡。

Conclusion: 通过将情感表达作为贝叶斯多智能体优化框架中的战略变量，EmoMAS为高风险边缘部署建立了有效、私密、自适应的谈判AI新范式，证明战略情感智能是谈判成功的关键驱动力。
...
### [239] [CAFP: A Post-Processing Framework for Group Fairness via Counterfactual Model Averaging](https://arxiv.org/abs/2604.07009)
...
<details>
  <summary>Details</summary>
Motivation: 在机器学习公平性领域，现有方法通常需要控制模型架构或访问受保护属性信息，这在现实系统中可能不可行。需要一种不依赖重新训练、模型无关的后处理方法来减轻不公平影响。
...
Conclusion: CAFP提供了一种有效、模型无关的后处理公平性干预方法，无需重新训练或修改原始分类器，适用于现实世界系统中敏感属性信息受限的场景。
...
### [240] [A-MBER: Affective Memory Benchmark for Emotion Recognition](https://arxiv.org/abs/2604.07017)
*Deliang Wen,Ke Sun,Yu Wang*

Main category: cs.AI

TL;DR: 提出A-MBER基准测试，用于评估AI助手如何利用长期交互记忆来理解用户当前情感状态，填补现有情感识别和记忆基准的空白。


<details>
  <summary>Details</summary>
Motivation: 现有情感数据集主要评估瞬时情感，而长期记忆基准则关注事实回忆、时间一致性或知识更新，缺乏评估模型如何利用交互历史来理解用户当前情感状态的能力。

Method: 通过分阶段流水线构建A-MBER基准，包括长期规划、对话生成、标注、问题构建和最终打包。支持判断、检索和解释任务，以及模态降级和证据不足等鲁棒性设置。

Result: 实验比较了局部上下文、长上下文、检索记忆、结构化记忆和黄金证据条件。A-MBER在长程隐式情感、高依赖记忆水平、轨迹推理和对抗设置等设计子集上具有特别强的区分能力。

Conclusion: 记忆支持情感理解不仅是通过提供更多历史，而是通过实现更选择性、有根据和上下文敏感的过去交互使用。A-MBER为评估AI助手的情感记忆能力提供了有效工具。
...
### [241] [Planning Task Shielding: Detecting and Repairing Flaws in Planning Tasks through Turning them Unsolvable](https://arxiv.org/abs/2604.07042)
*Alberto Pozanco,Marianela Morales,Pietro Totis,Daniel Borrajo*

Main category: cs.AI

TL;DR: 论文提出了规划任务屏蔽问题，通过检测和修复规划任务中的缺陷，使任务变得不可解来防止达到错误状态。


<details>
  <summary>Details</summary>
Motivation: 传统规划研究主要关注如何生成实现目标的计划，但目标规范也可以编码不应成立的性质，用于识别可能达到错误状态的轨迹。在这种情况下，目标转变为修改规划任务以确保错误状态永远不会被达到，即使得规划任务不可解。

Method: 提出了规划任务屏蔽问题，并引入了$allmin$算法，该算法通过最小化修改原始动作来使规划任务变得不可解。

Result: 对$allmin$算法在不同规模规划任务上的性能进行了实证评估，结果显示它能有效地通过使规划任务不可解来屏蔽系统。

Conclusion: 规划任务屏蔽是一个重要问题，$allmin$算法能够最优地解决该问题，通过最小化修改动作来确保系统不会达到错误状态。
...
### [242] [EVGeoQA: Benchmarking LLMs on Dynamic, Multi-Objective Geo-Spatial Exploration](https://arxiv.org/abs/2604.07070)
...
Result: 实验显示LLM能成功利用工具处理子任务，但在长距离空间探索方面表现不佳；同时观察到LLM具备总结历史探索轨迹以提升探索效率的新兴能力
...
### [243] [Reason in Chains, Learn in Trees: Self-Rectification and Grafting for Multi-turn Agent Policy Optimization](https://arxiv.org/abs/2604.07165)
*Yu Li,Sizhe Tang,Tian Lan*

Main category: cs.AI

TL;DR: T-STAR通过构建认知树识别关键推理步骤，利用回溯估值和上下文思维嫁接优化强化学习，在长链推理任务上显著提升LLM智能体性能。


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法（如GRPO）在多步推理任务中将采样轨迹视为独立链，对所有步骤均匀分配信用，忽略了关键步骤对推理结果的非对称影响，导致稀疏奖励问题。

Method: 提出T-STAR框架：1）将轨迹整合为统一的认知树，识别并合并功能相似的步骤/节点；2）引入回溯估值机制，通过树结构回溯轨迹级奖励，获得方差减少的相对优势；3）开发上下文思维嫁接，在关键分歧点对比成功与失败分支合成纠正推理；4）提出外科手术式策略优化，通过Bradley-Terry类型损失函数聚焦关键点的策略梯度信息。

Result: 在具身、交互、推理和规划等多个基准测试中，T-STAR相比强基线取得了一致的性能提升，在需要长推理链的任务上增益最为显著。

Conclusion: T-STAR通过恢复跨轨迹的潜在相关奖励结构，有效解决了LLM智能体在多步推理任务中的稀疏奖励问题，特别适用于需要扩展推理链的复杂任务。
...
### [244] [How Much LLM Does a Self-Revising Agent Actually Need?](https://arxiv.org/abs/2604.07236)
*Seongwoo Jeong,Seonil Son*

Main category: cs.AI

TL;DR: 该论文提出了一种可声明的反射运行时协议，将智能体状态、置信度信号、受保护动作和假设转换外部化为可检查的运行时结构，以实证研究LLM在智能体能力中的边际作用。
...
Method: 引入声明式反射运行时协议，将智能体状态、置信度信号、受保护动作和假设转换外部化。在嘈杂的协作战舰游戏中实例化该协议，使用四种渐进结构化的智能体进行54场游戏评估，分解出四个组件：后验信念跟踪、显式世界模型规划、符号化回合内反思和稀疏LLM修订。

Result: 显式世界模型规划相比贪婪后验跟踪基线有显著改进（胜率+24.1pp，F1+0.017）。符号化反思作为实际运行时机制运行，但当前修订预设尚未产生净正收益。在约4.3%的回合添加条件性LLM修订仅产生微小且非单调变化（平均F1略升+0.005，胜率从31降至29/54）。
