...
TL;DR: DF-ACBlurGAN：一种结构感知的条件生成对抗网络，用于在弱监督和类别不平衡条件下生成具有内部重复和周期性结构的图像，特别针对生物材料表面设计。


<details>
  <summary>Details</summary>
Motivation: 当前机器学习模型在生成具有内部重复和周期性结构的图像时存在局限，这些模型通常优化局部纹理统计和语义真实性，而非全局结构一致性。这在需要严格控制重复尺度、间距和边界一致性的应用中尤为突出，如微地形生物材料表面设计。

Method: 提出DF-ACBlurGAN，一种结构感知的条件生成对抗网络，在训练中显式推理长程重复性。方法整合了频域重复尺度估计、尺度自适应高斯模糊和单元重建，以平衡锐利的局部特征与稳定的全局周期性。模型基于实验衍生的生物响应标签进行条件生成。

Result: 在多个生物材料数据集上的评估表明，相比传统生成方法，该方法在重复一致性和可控结构变化方面有显著改进。

Conclusion: 该研究提出了一种有效生成具有严格周期性结构的图像的方法，特别适用于生物材料设计等需要精确控制重复模式的应用场景，为弱监督和类别不平衡条件下的条件生成提供了新思路。
...
TL;DR: OccSim：首个基于占用世界模型的3D自动驾驶模拟器，无需连续日志或高精地图，仅凭单帧初始图像和未来自车动作就能稳定生成3000+连续帧，实现超过4公里的3D占用地图构建，比现有方法提升80倍以上。


<details>
  <summary>Details</summary>
Motivation: 现有数据驱动的自动驾驶模拟严重依赖预录制的驾驶日志或高精地图等空间先验，这种根本性依赖限制了可扩展性，将开放式生成能力限制在有限的数据集规模内。需要打破这一瓶颈。

Method: OccSim包含两个核心模块：基于W-DiT的静态占用世界模型和布局生成器。W-DiT通过显式引入已知刚性变换来处理超长时域静态环境生成；布局生成器基于合成的道路拓扑在动态前景中填充反应性智能体。

Result: OccSim能稳定生成超过3000连续帧，实现超过4公里的3D占用地图构建，比现有最先进的占用世界模型提升80倍以上。从OccSim收集的数据可以预训练4D语义占用预测模型，在未见数据上达到67%的零样本性能，比基于资产的模拟器提升11%。当OccSim数据集规模扩大到5倍时，零样本性能提升至约74%，对基于资产模拟器的优势扩大到22.1%。

Conclusion: OccSim首次展示了基于占用世界模型的3D模拟器，突破了传统模拟对预录制数据和高精地图的依赖，实现了大规模、多样化的模拟流生成，为自动驾驶系统的训练和评估提供了强大的数据生成能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的多视角3D重建基础模型在透视图像上训练，但在鱼眼等广角图像上性能下降。虽然可以在鱼眼图像上训练，但鱼眼图像及其真值数据稀缺，限制了泛化能力。

Method: 提出Fisheye3R适配框架，包含灵活的学习方案：仅使用未标记透视图像的自监督适应，以及无需鱼眼训练数据的监督适应。通过处理非线性投影模型带来的像素空间位置变化来适应鱼眼输入。

Result: 在VGGT、π³和MapAnything三个基础模型上的实验表明，该方法能持续改进鱼眼图像上的相机姿态、深度、点云图和视场估计，且对透视图像无性能回归。

Conclusion: Fisheye3R成功将多视角3D重建基础模型扩展到鱼眼图像输入，解决了鱼眼数据稀缺的问题，为广角图像处理提供了有效解决方案。
...
TL;DR: 使用7T fMRI构建脑区功能图，通过带符号图神经网络解码视觉类别（运动、食物、车辆）的功能连接状态，揭示腹侧和背侧视觉通路中的可重复子网络。
...
Method: 使用Natural Scenes Dataset的高分辨率7T fMRI数据构建脑区水平功能图，训练带符号图神经网络（同时建模正负相互作用），采用稀疏边掩码和类别特异性显著性。
...
TL;DR: Stepper是一个文本驱动的沉浸式3D场景合成框架，通过逐步全景扩展解决现有方法在视觉保真度和可探索性之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有文本驱动3D场景合成方法存在视觉保真度和可探索性之间的权衡：自回归扩展存在上下文漂移问题，而全景视频生成仅限于低分辨率。需要一种能同时保证高保真度和几何一致性的解决方案。
...
Conclusion: Stepper通过逐步全景扩展框架成功解决了文本驱动3D场景合成中的视觉保真度与可探索性权衡问题，为AR/VR和世界建模应用提供了更高质量的沉浸式场景生成方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究混合量子-经典机器学习在工业自动化质量控制中的应用潜力，特别是在铝TIG焊接缺陷分类任务中。

Method: 使用CNN提取焊接图像特征向量，然后采用两种量子方法：1) 参数化量子特征映射编码特征到量子态，计算量子核矩阵，用VQLS求解SVM优化问题；2) 角度编码特征到变分量子电路，用经典优化器训练。与经典CNN模型进行性能对比。

Result: CNN模型表现出稳健性能，但混合量子-经典模型在二元和多类分类任务中表现具有竞争力，展示了在工业缺陷检测中的实际应用潜力。

Conclusion: 混合量子-经典方法在工业缺陷检测和质量保证等现实应用中具有潜力，特别是在近期量子计算设备上可实现的实际应用。
...
TL;DR: 提出一种前馈式人体性能捕捉方法，通过单目RGB视频流渲染表演者的新视角，利用渐进更新的规范空间积累外观信息，采用概率回归解决过去与当前观察的冲突。


<details>
  <summary>Details</summary>
Motivation: 单目RGB流中缺乏足够观察，特别是对于未见区域。假设主体随时间连续运动，需要利用更多身体部位变得可观察的特点来改善渲染质量。

Method: 维护渐进更新的规范空间积累外观信息，作为上下文库；将渲染过程公式化为概率回归，有效利用上下文同时尊重当前状态的变形，解决过去与当前观察的冲突。

Result: 在领域内（4D-Dress）和领域外（MVHumanNet）数据集上的实验证明了方法的有效性，相比确定性回归方法产生更清晰的重建，即使在无先前观察的区域也能实现合理的合成。

Conclusion: 提出的前馈式人体性能捕捉方法通过渐进更新的规范空间和概率回归，能够有效处理单目观察不足的问题，实现高质量的新视角渲染。
...
TL;DR: MEDiC框架结合了像素级和特征级的掩码图像建模，通过三个互补目标实现：CLIP编码器的token蒸馏、全局CLS对齐和像素重建，在ImageNet上达到73.9% kNN准确率。


<details>
  <summary>Details</summary>
Motivation: 现有掩码图像建模方法通常在原始像素空间（重建掩码块）或潜在特征空间（与预训练教师对齐）中操作，缺乏两者的结合。本文旨在探索结合这两种空间的多目标框架。

Method: 提出MEDiC框架，包含三个互补目标：1）从冻结CLIP编码器进行补丁级token蒸馏；2）全局CLS对齐；3）通过轻量级解码器进行像素重建。还研究了分层聚类与相对位置偏置的演化掩码方法。

Result: 三个目标提供互补信息，完整组合达到73.9% kNN准确率；演化掩码在教师引导蒸馏设置中不优于简单块掩码；损失权重极其敏感，小扰动可导致准确率下降17个百分点。ViT-Base在300轮训练后达到73.9% kNN和85.1%微调准确率。

Conclusion: MEDiC框架成功结合了像素级和特征级掩码图像建模，三个目标具有互补性。研究发现演化掩码在教师引导设置中优势有限，且损失权重需要精细调整。该框架在ImageNet上取得了有竞争力的性能。
...
Method: 提出UltraG-Ray：基于可学习3D高斯场的超声场景表示，将超声特定参数（衰减、反射）编码到高斯空间表示中，通过新颖的射线投射方案实现图像合成。

Result: 相比现有方法，该方法能自然捕捉视角依赖的衰减效应，生成物理信息更丰富的B模式图像，在图像质量指标上取得一致提升（MS-SSIM最高提升15%）。

Conclusion: UltraG-Ray通过结合可学习3D高斯场和物理基础的渲染模块，显著提高了超声图像合成的真实感，为临床训练和数据增强提供了更有效的工具。
...
TL;DR: MMFace-DiT提出统一的双流扩散Transformer架构，通过并行处理空间和语义token并共享RoPE注意力机制，实现多模态人脸生成的协同融合，在视觉保真度和提示对齐方面比现有方法提升40%。


<details>
  <summary>Details</summary>
Motivation: 现有多模态人脸生成模型通常通过附加控制模块或拼接单模态网络来扩展预训练文本到图像管道，这些临时设计存在架构约束、参数重复、模态冲突或潜在空间不匹配等问题，限制了语义和空间域的协同融合能力。

Method: 提出MMFace-DiT统一双流扩散Transformer：1）双流Transformer块并行处理空间（掩码/草图）和语义（文本）token；2）通过共享旋转位置嵌入注意力机制深度融合；3）新颖的模态嵌入器使单一模型能动态适应不同空间条件而无需重新训练。

Result: 在六个最先进的多模态人脸生成模型上，MMFace-DiT实现了40%的视觉保真度和提示对齐改进，为端到端可控生成建模建立了灵活的新范式。

Conclusion: MMFace-DiT通过统一的双流架构和共享注意力机制，解决了多模态融合中的模态主导和潜在空间不匹配问题，实现了前所未有的空间-语义一致性，为可控人脸生成提供了更有效的解决方案。
...
TL;DR: 研究发现，在无结构噪声（均匀、高斯）上预训练INR能显著提升信号拟合能力，但作为深度图像先验效果差；而具有自然图像1/|f|^α频谱结构的噪声预训练能在信号拟合和逆成像任务间取得最佳平衡。
...
Method: 在不同噪声类别（高斯、Dead Leaves、频谱等）上预训练INR，测量其拟合未见信号的能力以及在逆成像任务（去噪）中编码先验的能力，在图像和视频数据上进行实验分析。

Result: 无结构噪声预训练显著提升信号拟合能力但深度图像先验效果差；具有自然图像频谱结构的噪声预训练在信号拟合和逆成像能力间取得最佳平衡，性能与最佳数据驱动初始化方法相当。

Conclusion: 噪声预训练揭示了INR初始化成功的关键在于频谱结构先验而非复杂特征，这为缺乏领域特定数据的应用提供了更高效的INR训练方法。
...
TL;DR: 开发了一种生成算法，可以从行走游览视频中逼真地移除人类及其阴影，用于环境建模应用


<details>
  <summary>Details</summary>
Motivation: 行走游览视频中大量的人类存在（由于人群和眼平视角）限制了它们在环境建模应用中的实用性，需要解决人类移除问题

Method: 构建半合成数据集训练生成模型：包含环境背景片段和叠加行走人类与模拟阴影的合成片段；使用该数据集微调Casper视频扩散模型进行物体和效果修复

Result: 训练出的模型在移除行走游览视频中人类方面，在质量和数量上都显著优于原始Casper模型，特别是在人类密集和背景复杂的场景中

Conclusion: 生成的视频片段可用于成功构建城市地点的3D/4D模型，解决了行走游览视频中人类移除的挑战
...
TL;DR: 论文提出DASES框架，通过对抗性证伪而非静态验证来推动科学发现，在可控实验中成功识别出传统方法会接受的错误假设，并发现了超越合成环境的FNG-CE损失函数。


<details>
  <summary>Details</summary>
Motivation: 当前自主科学发现存在风险：一旦评估标准固定，强大的搜索过程可能学会通过"应试"而非真正理解机制来通过测试。需要建立主动对抗的评估机制来防止这种"伪发现"。

Method: 提出DASES框架，包含三个协同进化的组件：创新者（生成可执行科学制品）、深渊证伪者（生成科学可接受的反例环境）、机制因果提取器。在固定科学契约下共同演化，通过对抗性证伪而非静态验证来推动发现。

Result: 在单可编辑位点的可控损失发现问题中：1）DASES拒绝了静态验证会接受的制品；2）识别出首个在可接受证伪边界存活的候选；3）发现了FNG-CE损失函数，该函数能超越合成发现环境，在包括ImageNet的标准基准测试中持续优于CE和CE+L2。

Conclusion: 对抗性证伪（让深渊回望）比静态验证更有效，能防止"应试式"发现，推动真正的机制理解。DASES框架为自主科学发现提供了更安全的评估范式。
...
TL;DR: WorldFlow3D：一种基于流匹配的无边界3D世界生成方法，通过流经3D数据分布而非条件去噪来生成高质量3D场景，支持几何结构和纹理控制。


<details>
  <summary>Details</summary>
Motivation: 无边界3D世界生成是计算机视觉、图形学和机器人学中场景建模的基础任务，现有方法在生成质量和控制能力方面存在局限，需要更高效、可控的生成方法。

Method: 基于流匹配的核心特性（定义两个数据分布之间的传输路径），将3D生成建模为流经3D数据分布的问题。采用无潜在空间的流方法生成因果准确的3D结构，并以此作为中间分布指导更复杂结构和高质量纹理的生成。支持通过矢量化场景布局条件进行几何结构控制，以及通过场景属性进行视觉纹理控制。

Result: 在真实室外驾驶场景和合成室内场景上验证了方法的有效性，展示了跨域泛化能力和在真实数据分布上的高质量生成。在所有测试的无边界场景生成设置中，都优于现有方法，具有更好的场景生成保真度。

Conclusion: WorldFlow3D提供了一种高效、可控的无边界3D世界生成方法，通过流匹配框架实现了比现有方法更快的收敛速度，同时保持了高质量的几何结构和纹理生成能力。
...
TL;DR: LA-Sign：基于循环Transformer和几何感知对齐的骨架孤立手语识别框架，通过循环精炼潜表示和双曲空间语义组织实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有骨架手语识别方法主要依赖深度前馈架构，虽然增加了模型容量但缺乏循环精炼机制和结构化表示。手语识别需要从细微手指动作到全局身体动态的多尺度精细理解，需要更有效的表示学习方法。

Method: 提出LA-Sign框架：1）循环Transformer架构，通过循环访问潜表示在共享参数下逐步精炼运动理解；2）几何感知对比目标，将骨架和文本特征投影到自适应双曲空间，促进多尺度语义组织；3）研究了三种循环设计和多种几何流形。

Result: 在WLASL和MSASL基准测试中达到SOTA性能，同时使用更少的独特层数。编码器-解码器循环结合自适应Poincare对齐表现最佳，证明了循环潜表示精炼和几何感知表示学习的有效性。

Conclusion: LA-Sign通过循环精炼潜表示和几何感知表示学习，为骨架手语识别提供了更有效的框架，在减少参数的同时实现了更好的性能，展示了循环架构和结构化表示学习在手语识别中的潜力。
...
TL;DR: 研究发现多模态模型存在模态间隙，证明在特定条件下对比损失最小化会导致模态分离，且间隙大小与鲁棒性相关，通过简单后处理可提升鲁棒性而不损失准确率。


<details>
  <summary>Details</summary>
Motivation: 现有多模态模型（如CLIP）在共享嵌入空间中存在显著的模态间隙（图像和文本分布分离），但尚不清楚其原因，也不确定消除间隙是否能提升下游任务性能。

Method: 理论分析表明在特定条件下，最小化对比损失会导致两个模态被正交的全局间隙向量分离。通过实验验证，对真实VLM进行简单后处理：将一个模态的嵌入向另一个模态的均值移动。

Result: 模态间隙与鲁棒性单调相关：减小间隙不会改变模型的干净准确率，但能降低嵌入扰动时模型输出改变的可能性。实验显示通过后处理可显著提升鲁棒性。

Conclusion: 模态间隙是优化对比损失的自然结果，且间隙大小与模型鲁棒性相关。通过简单后处理减小间隙可有效提升模型鲁棒性而不影响准确率。
...
TL;DR: MaskAdapt是一个用于物理人体控制中灵活运动适应的框架，采用两阶段残差学习范式，通过掩码不变性训练基础策略，再训练残差策略实现局部身体部位的目标运动修改。


<details>
  <summary>Details</summary>
Motivation: 现有物理人体控制方法在实现局部运动适应方面存在局限，难以在保持其他身体部位原有行为的同时，仅修改特定部位的运动。需要一种灵活框架来实现精确的局部运动控制。

Method: 采用两阶段残差学习：第一阶段使用随机身体部位掩码和正则化项训练掩码不变的基础策略，建立稳定的运动先验；第二阶段在冻结的基础控制器上训练残差策略，仅修改目标身体部位。

Result: MaskAdapt展现出强大的鲁棒性和适应性，能够在掩码观测下产生多样行为，并在目标运动适应方面优于先前工作。成功应用于运动组合和文本驱动的局部目标跟踪两个应用场景。

Conclusion: MaskAdapt框架通过掩码不变性训练和残差学习，实现了物理人体控制中灵活、精确的局部运动适应，为运动组合和文本驱动的运动控制提供了有效解决方案。
...
TL;DR: TrajectoryAtlas：一种生成配对视频数据的新方法，用于训练TrajectoryMover模型，实现视频中物体3D运动轨迹的移动编辑


<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法主要关注规定物体的3D/2D运动轨迹或改变物体外观，但缺乏移动物体3D运动轨迹的方法。主要挑战在于获取这种场景的配对视频数据。

Method: 提出TrajectoryAtlas数据生成管道，用于大规模合成配对视频数据；在此基础上微调TrajectoryMover视频生成器，实现物体轨迹的生成式移动。

Result: 该方法成功实现了物体3D运动轨迹的生成式移动，解决了现有方法在无法从另一个视频轻松构建配对数据时的局限性。

Conclusion: TrajectoryAtlas和TrajectoryMover为视频编辑提供了新的能力，能够移动物体轨迹同时保持其相对3D运动，填补了现有方法的空白。
...
TL;DR: 提出基于计算机视觉的框架，通过单目视频分析上肢运动，无需深度传感器或校准，能够捕捉运动质量信息，超越传统计分方法。


<details>
  <summary>Details</summary>
Motivation: 传统中风后上肢运动功能评估要么依赖缺乏敏感性的序数评分，要么依赖不捕捉运动质量的时间任务指标，需要更精确、无侵入的评估方法。

Method: 开发基于计算机视觉的框架，通过单目视频分析Box and Block Test中的上肢运动，提取手指、手臂和躯干的世界对齐关节角度，使用无监督降维分析运动模式。

Result: 在136个BBT记录（48名健康人和7名中风患者）中，关节角度特征的无监督降维显示健康运动模式与中风相关运动偏差的分离，相同BBT评分的患者可通过姿势模式区分。

Conclusion: 世界对齐关节角度能够捕捉超越标准时间BBT评分的有意义上肢功能信息，基于摄像头的无校准框架有潜力在不改变临床常规的情况下测量运动质量。
...
TL;DR: DIME框架通过参数高效微调和频谱合并策略，解决持续食物识别中的双重不平衡问题（类内样本不平衡和增量步骤间新类别数量不平衡）。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的食物识别面临双重不平衡挑战：1）类内样本数量严重不平衡（长尾分布）；2）持续学习过程中每个增量步骤引入的新类别数量不平衡。现有方法通常假设每个增量步骤引入相似数量的新类别，这与现实不符。

Method: 提出DIME框架：1）为每个任务学习轻量级适配器（参数高效微调）；2）通过类数量引导的频谱合并策略逐步整合适配器；3）使用秩阈值调制机制稳定合并过程，保留主导知识同时允许自适应更新；4）最终维护单个合并适配器进行推理。

Result: 在现实长尾食物基准测试中，DIME在步骤不平衡设置下比现有最强持续学习方法提升超过3%。

Conclusion: DIME通过双重不平衡感知的适配器合并框架，有效解决了持续食物识别中的类内样本不平衡和增量步骤不平衡问题，实现了高效部署和性能提升。
...
TL;DR: SparseDriveV2通过因子化轨迹表示和分层评分策略，显著提升了基于评分的端到端多模态规划性能，在静态轨迹词汇表足够密集时能达到与动态生成方法相当的效果。


<details>
  <summary>Details</summary>
Motivation: 现有端到端多模态规划方法主要分为两类：评分大型静态轨迹词汇表或评分少量动态生成轨迹。静态词汇表通常存在动作空间离散化粗糙的问题，而动态生成方法虽然精度更高，但计算成本较大。本文旨在探索静态词汇表在足够密集时是否能达到与动态生成方法相当的性能。

Method: 提出SparseDriveV2方法，包含两个关键技术：1）可扩展的因子化词汇表表示，将轨迹分解为几何路径和速度剖面，实现动作空间的组合覆盖；2）可扩展的分层评分策略，先对路径和速度剖面进行粗粒度因子化评分，再对少量组合轨迹进行细粒度评分。

Result: 在NAVSIM数据集上达到92.0 PDMS和90.1 EPDMS，在Bench2Drive数据集上达到89.15驾驶分数和70.00%成功率，使用轻量级ResNet-34骨干网络。

Conclusion: 通过系统性的缩放研究证明，当轨迹锚点足够密集时，基于评分的静态词汇表方法能够达到与动态生成方法相当的性能。SparseDriveV2通过因子化表示和分层评分策略，在保持计算效率的同时显著提升了规划性能。
...
TL;DR: LatentPilot提出了一种新的视觉语言导航范式，通过训练时利用未来观测学习动作条件视觉动态，无需推理时访问未来帧，实现更鲁棒的导航决策。


<details>
  <summary>Details</summary>
Motivation: 现有VLN模型主要基于过去和当前视觉观测进行推理，忽略了动作引发的未来视觉动态，缺乏对动作与视觉世界变化因果关系的理解，限制了鲁棒决策能力。人类能够利用动作动态因果关系想象近未来，从而改善环境理解和导航选择。

Method: 提出飞轮式训练机制：迭代收集在线策略轨迹并重新训练模型以更好地匹配智能体行为分布；当智能体偏离过大时触发专家接管。学习无显式监督的视觉潜在标记，这些标记在连续潜在空间中全局关注并跨步传递，既作为当前输出又作为下一输入，使智能体能够"向前梦想"并推理动作如何影响后续观测。
...
TL;DR: 研究探索CT能否作为训练监督来提升X光疾病分类器性能，发现跨模态蒸馏效果有限，主要贡献是建立了可复现的评估协议而非有效架构。


<details>
  <summary>Details</summary>
Motivation: 胸部X光和CT提供互补信息，但现有诊断模型通常局限于单一模态。研究探索在只有X光推理时，能否利用CT作为训练监督来提升X光分类器性能。

Method: 采用跨模态师生蒸馏框架，使用JDCNet作为基础架构。在公共配对胸部影像数据集上进行患者级配对实验，包括原始分割和8次蒙特卡洛重采样，比较不同蒸馏策略（logit-KD、注意力转移、特征提示等）。

Result: 原始分割中简单跨模态logit-KD控制组表现最佳（准确率0.875，macro-F1 0.714），但重采样后跨模态优势消失。晚期融合获得最高平均准确率（0.885），同模态蒸馏获得最高macro-F1（0.554）和平衡准确率（0.660）。

Conclusion: 研究未验证有效的CT到X光架构，而是建立了可复现的评估协议，明确了任务定义、失败模式、排名不稳定性，为未来跨模态转移研究提供了可信度基准。
...
TL;DR: 该论文提出了一种改进的MedSAM模型，用于多类别脑组织分割，通过扩展掩码解码器到三个类别（背景、灰质、白质），在IXI数据集上取得了高达0.8751的Dice分数。


<details>
  <summary>Details</summary>
Motivation: 从磁共振成像中准确分割脑组织（如灰质和白质）对于研究脑解剖结构、诊断神经系统疾病和监测疾病进展至关重要。传统方法（如FSL FAST）需要任务特定调整，且在多样化成像条件下面临挑战。基础模型（如MedSAM）提供了基于提示的方法，但需要适应多类别分割任务。

Method: 1. 预处理流程：使用FSL BET进行颅骨剥离，FSL FAST生成组织概率图，转换为带有多类别标签的2D轴向、矢状、冠状切片。2. 修改MedSAM模型：扩展掩码解码器到三个类别，冻结预训练的图像编码器，微调提示编码器和解码器。

Result: 在IXI数据集上的实验取得了高达0.8751的Dice分数，表明该方法在多类别脑组织分割任务上表现良好。

Conclusion: 基础模型如MedSAM可以通过最小架构修改适应多类别医学图像分割任务。该研究为未来扩展到更多样化的医学成像场景提供了可能。
...
TL;DR: SplatHLoc：基于特征高斯泼溅的层次化视觉重定位框架，通过自适应视角检索和混合特征匹配提升重定位精度
...
Method: 1) 使用特征高斯泼溅作为场景表示；2) 提出自适应视角检索方法，合成与查询更对齐的虚拟候选视角；3) 引入混合特征匹配策略，在粗匹配阶段使用高斯渲染特征，在精匹配阶段使用直接图像提取特征

Result: 在室内外数据集上的大量实验表明，SplatHLoc提升了视觉重定位的鲁棒性，达到了新的最先进水平

Conclusion: SplatHLoc通过创新的场景表示、视角检索和特征匹配方法，有效解决了传统层次化重定位方法的局限性，为视觉重定位任务提供了更鲁棒的解决方案
...
TL;DR: SLVMEval是一个用于元评估文本到视频评估系统的基准，专注于评估系统在长达3小时的长视频上的表现，通过合成质量对比对来测试评估系统是否能在人类容易判断的场景中准确评估视频质量。


<details>
  <summary>Details</summary>
Motivation: 现有文本到视频评估系统在长视频评估方面的可靠性尚不明确，需要建立一个专门针对长视频的元评估基准来测试这些系统是否能在人类容易判断的场景中准确评估视频质量。

Method: 基于密集视频描述数据集，通过合成方式对源视频进行10个不同方面的质量降级，创建"高质量vs低质量"对比对；使用众包筛选出降级明显可感知的对，建立最终测试集；采用成对比较的元评估框架评估现有系统对这些对的排序能力。

Result: 人类评估者能准确识别更好长视频的准确率为84.7%-96.8%；在10个方面中的9个方面，现有评估系统的准确性都低于人类评估，揭示了文本到长视频评估的弱点。
...
TL;DR: 该论文提出了一种基于正交视图的3D物体重建方法，通过Harris角点检测获取控制点，构建垂直投影包络，利用包络交集生成3D点云，最后通过计算几何和OpenGL进行表面重建与渲染。


<details>
  <summary>Details</summary>
Motivation: 从物体的正交视图（如工程制图中的三视图）自动重建其3D模型，避免手动建模的繁琐过程，实现从2D图纸到3D模型的自动化转换。

Method: 1. 使用Harris角点检测器从输入的正交视图中提取控制点；2. 将这些控制点垂直投影到各自视图方向，构建包络体；3. 通过相互垂直的包络体交集获得描述物体的3D点集；4. 利用计算几何方法从点集重建物体表面；5. 使用OpenGL进行3D渲染。

Result: 成功实现了从正交视图到3D物体的自动重建，能够生成完整的3D模型并进行可视化渲染，验证了基于包络交集方法的可行性。

Conclusion: 该方法提供了一种有效的从正交视图重建3D物体的途径，结合了计算机视觉、计算几何和计算机图形学技术，为工程设计和制造中的3D建模提供了自动化解决方案。
...
Method: 自适应上下文压缩框架，包含重要性感知记忆选择、连贯性敏感过滤和动态预算分配三个核心组件，用于智能压缩对话上下文。

Result: 在LOCOMO、LOCCO和LongBench基准测试中，该方法在回答质量、检索准确性、连贯性保持和效率方面均优于现有方法，显著减少了token使用和推理延迟。

Conclusion: 自适应上下文压缩为持久性LLM交互提供了长期记忆保留与计算效率之间的有效平衡，是提升长对话性能的可行方案。
...
TL;DR: 该论文提出了一个多层记忆框架来解决长对话中的语义漂移和记忆保持问题，通过工作、情景和语义三层记忆结构，在有限上下文预算下实现了更好的长期记忆保持和推理稳定性。


<details>
  <summary>Details</summary>
Motivation: 长对话系统在扩展会话中存在语义漂移和不稳定的记忆保持问题，需要一种能够控制跨会话漂移同时保持有界上下文增长和计算效率的架构。

Method: 提出了一个多层记忆框架，将对话历史分解为工作记忆、情景记忆和语义记忆三层，采用自适应检索门控和保持正则化机制来控制记忆的访问和更新。

Result: 在LOCOMO、LOCCO和LoCoMo数据集上实验显示：46.85%成功率、0.618整体F1（其中多跳推理F1为0.594）、56.90%六周期保持率，同时将错误记忆率降低到5.1%，上下文使用率降至58.40%。
...
TL;DR: LightHarmony3D：一种用于3D高斯泼溅场景中光照一致网格插入的新框架，通过生成式模块预测360° HDR环境贴图，实现物理一致的光照和阴影，并创建了首个专用基准测试。


<details>
  <summary>Details</summary>
Motivation: 在3DGS场景中插入外部网格对象可实现交互式编辑和内容增强，但实现物理一致的光照和阴影具有挑战性，需要准确的场景光照估计和多视图一致渲染。

Method: 提出生成式模块，通过单次前向传播预测插入位置的完整360° HDR环境贴图，利用生成先验而非迭代优化，捕获主导场景光照，实现物理基础的着色和阴影，同时保持多视图一致性。

Result: 在多个真实世界重建数据集上的广泛实验表明，LightHarmony3D实现了最先进的真实感和多视图一致性。

Conclusion: LightHarmony3D解决了3DGS场景中光照一致网格插入的挑战，通过生成式环境贴图预测实现了物理一致的光照效果，并提供了首个专用基准测试用于标准化评估。
...
TL;DR: 提出CCDNet网络，通过加权多分支感知器、聚合细化融合颈和对比辅助干扰物判别器，解决红外目标检测中低对比度、背景融合和干扰物导致的误报问题。


<details>
  <summary>Details</summary>
Motivation: 红外目标检测在野外救援和海上搜救等应用中至关重要，但面临目标低对比度、易与复杂背景融合（伪装）以及相似特征干扰物导致误报的挑战。

Method: 提出CCDNet网络：1) 使用加权多分支感知器(WMPs)作为骨干网络，聚合自条件多级特征；2) 提出聚合细化融合颈(ARFN)，从浅层/深层特征图中细化结构/语义，双向重建目标与背景关系；3) 提出对比辅助干扰物判别器(CaDD)，通过局部和全局自适应相似度计算精确区分干扰物。
...
TL;DR: M2H-MX是一个实时多任务感知模型，通过轻量级解码器中的注册门控全局上下文和受控跨任务交互，在严格延迟约束下实现深度和语义预测的相互增强，显著提升单目SLAM系统的性能。


<details>
  <summary>Details</summary>
Motivation: 单目相机因其低成本和易于部署在机器人感知中具有吸引力，但从单目图像流实现可靠实时的空间理解仍然具有挑战性。现有的多任务密集预测模型虽然改进了逐像素深度和语义估计，但将其转化为稳定的单目建图系统仍然困难。

Method: 提出M2H-MX模型，保留多尺度特征表示，在轻量级解码器中引入注册门控全局上下文和受控跨任务交互机制，使深度和语义预测在严格延迟约束下相互增强。通过紧凑的感知到建图接口直接集成到未修改的单目SLAM流程中。

Result: 在NYUDv2数据集上，M2H-MX-L实现了最先进的结果：语义mIoU提升6.6%，深度RMSE降低9.4%。在ScanNet数据集上的实时单目建图系统中，与强大的单目SLAM基线相比，平均轨迹误差减少60.7%，同时生成更清晰的度量语义地图。

Conclusion: 现代多任务密集预测技术可以可靠地部署在机器人系统中，用于实时单目空间感知，显著提升单目SLAM系统的性能和地图质量。
...
Method: 将平均问题转化为轨迹对齐：优化多个噪声潜在向量，使它们的去噪轨迹在粗到细的语义上逐渐收敛到共享状态，生成单个清晰原型。对于多模态概念，先在CLIP等语义丰富空间聚类，再通过Textual Inversion或LoRA将CLIP聚类映射到扩散空间。

Result: 这是首个能生成一致、真实平均结果的方法，即使是抽象概念也能产生清晰原型，可作为视觉摘要并揭示模型偏见和概念表示。

Conclusion: DMA通过在扩散模型语义空间内进行平均，解决了传统图像平均的模糊问题，为概念可视化提供了新工具，并能洞察模型内部表示。
...
TL;DR: 本文提出TSONet双流序数网络和PhiSat-2-Height数据集，用于单目光学影像建筑高度估计，在PhiSat-2卫星数据上取得最佳性能。


<details>
  <summary>Details</summary>
Motivation: 单目建筑高度估计面临高度线索模糊、城市间建筑形态差异大、高度分布长尾等挑战。PhiSat-2卫星具有全球覆盖、4.75米空间分辨率和七波段光谱观测等优势，但其潜力尚未得到系统评估。

Method: 构建PhiSat-2-Height数据集（PHDataset），包含全球26个城市的9,475个配准图像-标签对。提出双流序数网络（TSONet），联合建模建筑轮廓分割和高度估计，引入跨流交换模块（CSEM）进行轮廓感知特征交互，以及特征增强箱细化模块（FEBR）进行序数高度细化。

Result: 在PHDataset上的实验表明，TSONet实现了最佳整体性能，MAE和RMSE分别降低13.2%和9.7%，IoU和F1-score分别提高14.0%和10.1%。消融研究验证了CSEM、FEBR以及序数回归与轮廓辅助联合使用的有效性。
...
TL;DR: FlexMem是一种无需训练的视频记忆机制，通过模拟人类观看视频时的记忆方式，使多模态大语言模型能够处理无限长度的视频内容，突破了现有方法的输入限制。
...
Method: FlexMem采用视觉记忆机制，模拟人类观看视频的行为：持续观看视频内容并回忆最相关的记忆片段来回答问题。通过双路径压缩设计实现有效的记忆转移和写入，并针对不同视频理解任务探索不同的记忆读取策略。

Result: 在单个3090 GPU上，FlexMem能够处理超过1000帧的视频，相比现有高效视频理解方法有明显改进。在某些基准测试中，使用FlexMem的基础MLLM甚至能达到与GPT-4o和Gemini-1.5 Pro相当或更好的性能。

Conclusion: FlexMem为多模态大语言模型提供了处理无限长度视频的有效解决方案，通过模拟人类记忆机制实现了高效的长视频理解，突破了现有方法的输入限制。
...
TL;DR: Omni-NegCLIP通过改进CLIP的对比损失函数，专门针对存在性否定和缺失性否定两种否定表达进行微调，显著提升了CLIP对否定表达的理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型（如CLIP）在处理自然语言中的否定表达时表现不佳，这限制了其在真实场景中的应用。否定表达在自然语言中很常见，包括存在性否定（对象实际存在但被否定）和缺失性否定（对象可能合理存在但实际缺失）两种类型。

Method: 1. 设计存在性对比目标：将图像嵌入拉近原始描述嵌入，同时推离存在性否定描述嵌入；2. 设计缺失性对比目标：将图像嵌入同时与原始描述和缺失性否定描述对齐，同时保持两种文本嵌入的语义区分；3. 基于观察到CLIP文本编码器的前几层transformer对否定文本学习能力更强，在训练时只微调这些层；4. 结合两种对比目标修改原始的InfoNCE对比损失。

Result: 相比预训练CLIP，Omni-NegCLIP在存在性否定任务上性能提升高达52.65%，在缺失性否定任务上提升12.50%。同时，在图像-文本检索的一般能力上没有下降，甚至提升了19.62%。相比先前工作，Omni-NegCLIP展现出更全面的否定理解能力。

Conclusion: Omni-NegCLIP通过专门设计的对比学习目标和有针对性的模型微调策略，有效提升了CLIP对否定表达的理解能力，同时保持甚至增强了其一般多模态能力，为解决视觉语言模型中的否定理解问题提供了有效方案。
...
TL;DR: 提出NPAD算法，无需保护属性信息，利用非保护属性的辅助信息进行去偏，在面部属性预测任务中显著减少性别和年龄子组的偏见。
...
Method: 提出NPAD算法，无需保护属性信息，利用非保护属性的辅助信息进行去偏。提出两种损失函数：DACL（通过属性聚类损失去偏）和FRL（过滤冗余损失）来优化模型的公平性目标。

Result: 在LFWA和CelebA数据集上进行面部属性预测实验，观察到不同性别和年龄子组的偏见显著减少。

Conclusion: NPAD算法能够有效减少模型偏见，且无需保护属性信息，更适合现实世界应用场景。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的开放词汇遥感分割方法主要关注增强特征表示或减少模态差异，但采用独立的补丁级预测方案与遥感数据的固有特性不符。真实遥感场景通常规模大且具有强烈的空间和语义相关性，孤立补丁预测无法实现准确分割。

Method: 提出ConInfer框架，这是一个上下文感知的推理框架，通过对多个空间单元进行联合预测，同时显式建模它们之间的单元间语义依赖关系。通过融入全局上下文线索，增强分割一致性。

Result: 在多个基准数据集上的实验表明，该方法在开放词汇语义分割和目标提取任务上分别平均提升2.80%和6.13%，显著超越SegEarth-OV等最先进的基于VLM的逐像素基线方法。

Conclusion: ConInfer框架通过上下文感知的联合预测有效解决了开放词汇遥感分割中孤立预测的局限性，显著提升了在复杂遥感环境中的分割性能，为遥感语义理解提供了更有效的解决方案。
...
TL;DR: PRISM是一个包含27万样本的多视角视频监督微调数据集，专为零售环境中的具身视觉语言模型设计，通过三维知识本体（空间、时间物理、具身动作）提升模型在现实世界部署中的感知能力。


<details>
  <summary>Details</summary>
Motivation: 现有物理AI模型在通用视觉理解与特定部署环境的专业感知需求之间存在关键差距。模型失败不是因为视觉识别能力差，而是因为对空间、物理动态和具身动作的理解不足，无法在现实世界中可靠操作。

Method: 构建PRISM数据集，包含27万样本的多视角视频（自我中心、外部中心、360°视角），覆盖5个超市位置。采用三维知识本体结构：空间知识、时间物理知识、具身动作知识。包含开放式、思维链和多项选择题监督。数据规模约1180万视频帧和7.3亿标记。

Result: 在PRISM上微调使所有20+能力探针的错误率比预训练基线降低66.6%，具身动作理解的准确率提升36.4%。证明本体结构化的领域特定微调能显著增强具身视觉语言模型在现实环境中的能力。

Conclusion: PRISM是首个在单一现实世界部署域中实例化所有三维知识维度的数据集，表明基于本体的领域特定监督微调能有效提升具身视觉语言模型在真实环境中的性能。
...
TL;DR: MELT网络通过关注罕见修改语义和应用基于扩散的去噪处理硬负样本来解决组合图像检索中的频率偏差和相似度干扰问题


<details>
  <summary>Details</summary>
Motivation: 现有组合图像检索方法面临两个限制：(1) 频率偏差导致"罕见样本忽视"问题，(2) 相似度得分容易受到硬负样本和噪声的干扰

Method: 提出MELT网络，在多模态上下文中增加对罕见修改语义的关注，同时对高相似度得分的硬负样本应用基于扩散的去噪技术，增强多模态融合和匹配

Result: 在两个CIR基准测试上的广泛实验验证了MELT的优越性能

Conclusion: MELT通过解决罕见语义定位和硬负样本下的鲁棒相似度估计，有效提升了组合图像检索的性能
...
TL;DR: 提出一种基于CLIP的视线引导深度伪造检测与溯源方法，通过视线差异和自适应语言提示增强对新型生成方法的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度伪造检测和溯源方法在视觉模态探索有限，对新型生成方法泛化能力差，且未考虑检测与溯源任务的协同作用。

Method: 1) 建立细粒度基准评估扩散和流模型等新型生成器；2) 提出视线感知CLIP模型，利用真实与伪造视线向量的分布差异；3) 设计视线感知图像编码器融合伪造视线提示；4) 构建语言精炼编码器生成自适应增强的语言嵌入。

Result: 在提出的基准测试中，模型在溯源和检测设置下的平均性能分别比现有最佳方法高出6.56% ACC和5.32% AUC。

Conclusion: 视线引导的CLIP模型结合自适应语言提示能有效提升深度伪造检测与溯源的泛化能力，特别适用于新型生成方法。
...
Method: 提出MotionScale框架，核心是参数化为簇中心基变换的可扩展运动场，以及包含两个解耦传播阶段的渐进优化策略：背景扩展阶段（适应新可见区域、优化相机位姿、建模瞬态阴影）和前景传播阶段（通过三阶段细化过程强制运动一致性）

Result: 在具有挑战性的真实世界基准测试中，MotionScale在重建质量和时序稳定性方面显著优于现有最先进方法

Conclusion: MotionScale通过可扩展的运动场表示和渐进优化策略，能够高效扩展到大型场景和长时间序列，同时保持高保真的结构和运动一致性，为动态4D场景重建提供了有效解决方案
...
TL;DR: 将自一致性技术从自然语言推理扩展到视觉领域，通过聚类和几何变换组识别LLM生成的运动轨迹的一致性，提升轨迹生成准确率4-6%


<details>
  <summary>Details</summary>
Motivation: 自一致性在自然语言推理任务中已被证明有效，但如何将其适应到视觉领域（特别是运动轨迹生成）仍待探索。需要一种方法来评估和验证LLM生成的多样化运动轨迹的一致性。

Method: 1) 从LLM采样多样化的运动轨迹；2) 将提示对应的形状族建模为原型轨迹+几何变换组（刚性、相似、仿射变换）；3) 提出自动恢复形状族的算法，利用候选变换组之间的层次关系进行聚类；4) 通过变换组判断轨迹一致性。

Result: 1) LLM轨迹生成准确率提升4-6%；2) 验证任务中比VLM基线精度提升11%；3) 提供了代码和数据集。

Conclusion: 成功将自一致性技术扩展到视觉领域，通过几何变换组建模形状族能有效识别一致性轨迹，显著提升LLM在运动轨迹生成和验证任务上的性能。
...
TL;DR: 提出一种双层元学习方法，在特征空间进行数据增强，改善分类器对虚假相关性的处理能力，提高少数群体性能


<details>
  <summary>Details</summary>
Motivation: 深度神经网络常依赖虚假特征进行预测，导致在分布偏移和少数群体样本上性能下降。研究发现，ERM训练的特征提取器能学习丰富信息表示，问题主要在于分类器头。重新训练轻量级分类器头能显著改善性能，但需要更好的方法来增强分类器处理虚假相关性的能力。

Method: 提出双层元学习方法，直接在特征空间进行增强。方法学习支持侧的特征编辑，使得经过少量内循环更新后，分类器在困难样本上损失更低，最差群体性能更好。该方法在骨干网络输出而非像素空间操作，通过端到端优化实现高效稳定训练。

Result: 方法高效稳定，仅需单GPU几分钟训练。通过CLIP可视化验证，学习的特征空间更新能诱导语义上有意义的偏移，与虚假属性对齐。

Conclusion: 通过在特征空间进行数据增强的双层元学习方法，能有效改善分类器对虚假相关性的处理能力，提高模型在分布偏移和少数群体上的性能表现。
...
TL;DR: FOSCU框架通过Duo-Diffusion（结合ControlNet的3D潜在扩散模型）生成高分辨率合成MRI和分割标签，解决医学图像分割中的数据获取和标注难题。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割面临PACS系统访问受限、标注成本高、临床数据短缺等系统性障碍，这些限制了鲁棒分割算法的发展。

Method: 提出FOSCU框架：1) Duo-Diffusion - 基于分割条件扩散的3D潜在扩散模型，确保生成数据的空间一致性和解剖细节；2) 增强的3D U-Net训练流程，结合真实和合成数据进行训练。

Result: 在720个腹部MRI扫描上的实验表明：结合真实和合成数据训练相比仅用真实数据，平均Dice分数提升0.67%，Fréchet Inception Distance降低36.4%，表明图像保真度显著提高。

Conclusion: FOSCU通过生成高质量合成医学图像和分割标签，有效缓解了数据获取和标注瓶颈，提升了分割模型的性能。
...
TL;DR: CIPHER是一个深度伪造检测框架，通过重用和微调图像生成判别器来提取跨模型的生成伪影，在多种生成模型上实现卓越的跨模型检测性能。
...
Method: CIPHER系统地重用和微调原本用于图像生成的判别器，从ProGAN判别器中提取尺度自适应特征，从扩散模型中提取时间一致性特征，捕捉传统检测器经常忽略的生成无关伪影。

Result: 在9个最先进的生成模型上进行广泛实验，CIPHER展示了卓越的跨模型检测性能，达到74.33%的F1分数，平均比现有ViT检测器高出30%以上。在CIFAKE数据集上达到88%的F1分数，而传统检测器性能接近零。
...
TL;DR: 提出了一种联合建模位置和语义不确定性的轨迹预测框架，通过双头架构端到端估计不确定性，提升预测鲁棒性


<details>
  <summary>Details</summary>
Motivation: 现实世界地图存在位置不准确（传感器限制、环境遮挡）和语义错误（场景上下文误解）两种不确定性，现有轨迹预测方法未能充分处理这些不确定性

Method: 采用双头架构独立估计语义和位置预测，以双通道方式端到端获取预测方差作为不确定性指标，然后将不确定性融合到预测中增强鲁棒性

Result: 在nuScenes数据集上验证，有效量化地图不确定性，在minADE、minFDE和Miss Rate等多个指标上持续提升现有轨迹预测模型性能

Conclusion: 提出的不确定性感知框架能有效处理地图不确定性，提高轨迹预测的准确性和鲁棒性，为自动驾驶等应用提供更可靠的预测
...
<details>
  <summary>Details</summary>
Motivation: 现有立体视觉骨干网络（MDE模型或视觉基础模型）缺乏相机姿态的显式监督，而几何知识对立体视觉至关重要，这构成了性能瓶颈。VGGT作为包含相机姿态等3D先验的基础模型，有潜力成为立体视觉骨干，但直接应用效果不佳，存在几何细节退化问题。

Method: 提出StereoVGGT，专门为立体视觉设计的特征骨干网络。利用冻结的VGGT模型，引入训练免费的特征调整流程，缓解几何退化问题，并利用模型中嵌入的潜在相机标定知识。

Result: 基于StereoVGGT的立体匹配网络在KITTI基准测试中取得了所有已发表方法中的第一名，验证了StereoVGGT作为立体视觉骨干网络的高度有效性。

Conclusion: StereoVGGT通过有效利用VGGT中的几何先验知识并解决其几何退化问题，成功构建了强大的立体视觉特征骨干网络，在立体视觉任务中表现出色。
...
TL;DR: TriDerm：一种用于罕见皮肤病RDEB的多模态框架，通过整合伤口图像、边界掩码和专家报告，学习可解释的伤口表征，利用专家三元组比较评估嵌入空间，在专家一致性方面显著优于现成的基础模型。


<details>
  <summary>Details</summary>
Motivation: 针对罕见遗传性皮肤病RDEB，现有基础模型无法可靠捕捉临床相关特征，且难以结构化测量与专家的一致性。需要开发能够有效利用小规模队列数据、整合多模态信息的方法来支持临床决策。

Method: 提出TriDerm多模态框架：1）视觉方面，通过伤口级注意力池化和非对比表示学习适配视觉基础模型；2）文本方面，使用LLM处理比较查询，通过软序数嵌入恢复医学意义表征；3）引入专家序数比较（三元组判断）评估嵌入空间；4）融合视觉和文本模态。

Result: 多模态融合达到73.5%的专家一致性，比最佳单模态基础模型提高5.6个百分点。视觉和文本模态捕捉伤口表型的互补方面，专家三元组判断能快速收集并编码临床相似性知识。

Conclusion: TriDerm通过整合多模态数据和专家序数比较，为罕见皮肤病RDEB提供了有效的临床相似性评估框架，显著优于现成基础模型，支持临床决策并公开相关工具和数据集。
...
TL;DR: 本文针对基于提示的AI图像编辑技术带来的伪造风险，提出了一个完整的伪造定位解决方案，包括自动标注框架、大规模数据集和有效的检测网络。


<details>
  <summary>Details</summary>
Motivation: 基于提示的AI图像编辑技术的快速普及加剧了恶意内容伪造和虚假信息的风险，但针对这些新兴编辑技术的伪造定位方法研究严重不足，需要填补这一空白。

Method: 1) 提出全自动掩码标注框架，利用关键点对齐和语义空间相似性生成精确的编辑区域真值掩码；2) 构建PromptForge-350k大规模伪造定位数据集；3) 提出ICL-Net伪造定位网络，采用三流主干网络和图像内对比学习设计。

Result: 在PromptForge-350k数据集上达到62.5%的IoU，比SOTA方法提升5.1%；对常见退化具有强鲁棒性（IoU下降小于1%）；在未见过的编辑模型上表现出良好的泛化能力（平均IoU 41.5%）。

Conclusion: 本文为解决基于提示的AI图像编辑伪造检测问题提供了完整的解决方案，包括数据集、标注框架和检测网络，在性能、鲁棒性和泛化能力方面均表现出色。
...
TL;DR: Extend3D：基于单张图像生成3D场景的训练免费流程，通过扩展潜在空间、分块生成和迭代优化实现


<details>
  <summary>Details</summary>
Motivation: 现有以物体为中心的3D生成模型存在固定大小潜在空间的限制，难以表示宽场景；同时单张图像到3D场景生成需要解决空间对齐和遮挡区域完善的问题

Method: 1. 扩展潜在空间x和y方向 2. 将扩展空间划分为重叠块，对每块应用物体中心3D生成模型 3. 使用单目深度估计器点云先验初始化场景 4. 通过SDEdit迭代优化遮挡区域，引入"欠去噪"概念 5. 优化扩展潜在空间确保去噪轨迹与子场景动态一致

Result: 方法在人类偏好和定量实验中优于先前方法，能够生成更高质量的3D场景

Conclusion: Extend3D通过扩展潜在空间、分块生成和优化策略，成功实现了从单张图像生成高质量3D场景的训练免费流程，解决了现有方法的限制
...
<details>
  <summary>Details</summary>
Motivation: 现有放射学报告自动总结模型存在两个问题：1）认为更多视觉输入总是更好；2）当文本发现已包含丰富图像细节时，多模态模型价值有限。作者挑战这两个假设，探索更有效的视觉-文本整合方法。

Method: 提出ViTAS（视觉-文本注意力总结器），多阶段流水线包括：MedSAM2肺部分割引导的集成、双向交叉注意力多视图融合、Shapley引导的自适应补丁聚类、分层视觉标记化输入ViT。

Result: 在MIMIC-CXR基准测试中达到SOTA：BLEU-4 29.25%，ROUGE-L 69.83%。定性分析显示事实对齐度改善，专家人工评估得分最高。

Conclusion: 选择性关注病理相关视觉区域而非完整图像的方法在放射学报告总结中表现更优，"更少但更相关的视觉输入"不仅足够而且优于传统多模态方法。
...
TL;DR: AA-Splat提出了一种抗锯齿的3D高斯泼溅方法，通过不透明度平衡的带限设计，解决了现有方法在不同分辨率渲染时的严重伪影问题。


<details>
  <summary>Details</summary>
Motivation: 现有前馈3D高斯泼溅方法基于错误的屏幕空间扩张滤波器，在非分布采样率渲染时会产生严重伪影，需要一种能在任意分辨率下进行稳健抗锯齿渲染的解决方案。

Method: 采用不透明度平衡的带限设计，包含两个组件：1) 3D带限后滤波器，将多视图最大频率边界集成到前馈重建流程中；2) 不透明度平衡机制，无缝集成所有像素对齐的高斯基元到渲染过程。

Result: AA-Splat在所有分辨率下（4倍到1/4倍）相比SOTA基线DepthSplat，在新视角合成性能上平均获得5.4~7.5dB的PSNR提升，显著改善渲染质量。

Conclusion: AA-Splat通过创新的抗锯齿设计，实现了在任意分辨率下的稳健3D重建和新视角合成，解决了现有方法的渲染伪影问题，代码将开源。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在多模态推理和复杂场景理解方面表现出色，但仍面临严重的幻觉问题（输出与视觉事实矛盾）。现有幻觉缓解方法存在局限性：重新训练方法需要大量训练资源，对比解码方法引入双重推理开销，这些都限制了实际应用。
...
Conclusion: HIRE框架通过动态检测和编辑幻觉表征，以最小计算成本有效解决了视觉语言模型的幻觉问题，具有高效、稳健和可控的优势。
...
TL;DR: 提出AGFT框架，通过对齐引导的对抗微调增强视觉语言模型的零样本对抗鲁棒性，同时保持跨模态语义对齐
...
Conclusion: AGFT框架成功解决了现有对抗微调方法破坏跨模态对齐的问题，在增强零样本对抗鲁棒性的同时保持了视觉语言模型的语义结构。
...
TL;DR: 提出一种外参感知的跨模态注意力框架，直接在图像块和LiDAR点群原生域中进行对齐，避免投影失真，显著提升大外参扰动下的标定精度和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于学习的方法通常将LiDAR点投影到深度图进行特征融合，这会扭曲3D几何结构，当外参初始化远离真实值时性能会下降。需要一种能够处理大外参偏差的鲁棒标定方法。

Method: 提出外参感知的跨注意力框架，直接在图像块和LiDAR点群原生域中对齐，将外参假设显式注入到对应关系建模过程中，实现几何一致的跨模态交互，无需依赖投影的2D深度图。

Result: 在KITTI和nuScenes基准测试中，该方法在精度和鲁棒性上均优于现有最优方法。在大外参扰动下，在KITTI上达到88%的准确标定率，在nuScenes上达到99%，显著超越第二好的基线方法。

Conclusion: 提出的外参感知跨注意力框架能够有效处理大外参偏差，直接在原生域中进行跨模态对齐，避免了投影失真问题，为相机-LiDAR融合提供了更鲁棒和准确的标定解决方案。
...
TL;DR: 本文提出了一种针对闭源多模态大语言模型的不可感知视觉提示注入攻击方法，通过文本覆盖提供语义指导，并优化视觉扰动来对齐特征表示，在多种闭源MLLMs上表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 尽管多模态大语言模型在实际应用中越来越广泛，但其指令跟随行为使其容易受到提示注入攻击。现有方法主要依赖文本提示或人类可感知的视觉提示，本文研究针对强大闭源MLLMs的不可感知视觉提示注入攻击。

Method: 通过有界文本覆盖将恶意提示嵌入输入图像提供语义指导，同时迭代优化不可感知的视觉扰动，在粗粒度和细粒度层面使受攻击图像的特征表示与恶意视觉和文本目标对齐。视觉目标被实例化为文本渲染图像，并在优化过程中逐步细化以更忠实地表示所需语义并提高可迁移性。

Result: 在两个多模态理解任务和多个闭源MLLMs上的广泛实验表明，该方法相比现有方法具有优越性能。

Conclusion: 本文提出的不可感知视觉提示注入攻击方法能够有效针对闭源多模态大语言模型，展示了现有MLLMs在安全性方面的脆弱性。
...
TL;DR: 研究探索使用预训练多模态模型（如Paligemma、Llava、Qwen）结合视觉和文本特征来增强ID证件呈现攻击检测，但实验结果表明这些模型在ID证件PAD检测上表现不佳。
...
Method: 使用预训练多模态模型（Paligemma、Llava、Qwen），将深度视觉嵌入与文本元数据（文档类型、签发机构、日期等）相结合，构建多模态PAD检测系统。

Result: 实验结果表明，这些多模态模型在ID证件呈现攻击检测任务上表现不佳，未能准确检测PAD攻击。

Conclusion: 虽然多模态模型理论上能增强PAD检测，但现有预训练模型在ID证件这一特定领域的呈现攻击检测任务上效果有限，需要进一步优化或开发专门针对该任务的模型。
...
TL;DR: A²BFR：基于属性感知的盲人脸恢复框架，结合扩散变换器与文本提示控制，实现高保真重建与细粒度属性编辑的统一


<details>
  <summary>Details</summary>
Motivation: 现有盲人脸恢复方法存在两个主要问题：1）基于扩散的方法虽然提升了感知质量，但缺乏可控性；2）文本引导的人脸编辑方法能够控制属性，但缺乏可靠的恢复能力。需要一种既能高质量恢复又能精细控制人脸属性的统一框架。

Method: 1）基于扩散变换器构建统一框架，通过图像-文本跨模态注意力联合条件化去噪轨迹；2）引入属性感知学习，使用属性感知编码器提取面部属性嵌入来监督去噪潜在空间；3）提出语义双训练，利用新构建的AttrFace-90K数据集中的成对属性变化来增强属性判别能力同时保持保真度。

Result: A²BFR在恢复保真度和指令遵循方面达到最先进水平：相比基于扩散的BFR基线，LPIPS降低0.0467，属性准确率提升52.58%，即使在严重退化条件下也能实现细粒度、提示可控的恢复。

Conclusion: A²BFR成功统一了高保真人脸恢复与提示可控生成，通过属性感知学习和语义双训练机制，有效解决了现有方法在可控性和恢复质量之间的权衡问题，为盲人脸恢复提供了新的解决方案。
...
TL;DR: 提出基于工具引导推理框架解决视觉语言模型在光学幻觉图像上的系统性偏见问题，无需模型训练，通过通用图像处理工具和幻觉类型路由提示实现跨结构泛化。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理经典光学幻觉图像时存在系统性偏见：无论图像是否经过反事实修改，模型都倾向于将幻觉预测为"真实"。这种偏见源于幻觉训练数据的不平衡，需要在不重新训练模型的情况下解决这一问题。

Method: 开发工具引导推理框架，为现成的视觉语言模型提供一组通用图像处理工具（线条绘制、区域裁剪、并排比较、通道分离），配合幻觉类型路由系统提示，规定每个感知问题类别应调用哪些工具。所有工具调用都会生成新的不可变图像资源并添加到持久注册表中，模型可以在整个推理链中引用和组合任何先前注释的视图。

Result: 该框架在验证集和测试集上表现一致，即使测试集包含结构上不熟悉的幻觉变体（如马赫带从垂直堆叠旋转到水平堆叠）。此外发现三个值得进一步研究的经验观察：强烈的正检测偏见、像素级空间推理与逻辑推理之间的显著分离、以及对图像压缩伪影的敏感性。

Conclusion: 通过通用工具加路由设计而非硬编码幻觉特定模块，实现了强大的跨结构泛化能力。该框架成功解决了视觉语言模型在光学幻觉上的系统性偏见问题，同时揭示了模型推理过程中的重要现象需要进一步研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D问答系统中，多视角图像会产生大量冗余视觉token，严重影响推理效率。现有的剪枝方法主要针对2D输入或依赖间接几何线索，无法在保留语义关键对象的同时维持足够的空间覆盖。

Method: 提出SeGPruner框架：1) 注意力重要性模块（显著性感知token选择器）保留语义显著token；2) 几何引导选择器（几何感知token多样化器）基于语义相关性和3D几何距离补充空间多样token。

Result: 在ScanQA和OpenEQA数据集上，SeGPruner将视觉token预算减少91%，推理延迟降低86%，同时在3D推理任务中保持有竞争力的性能。

Conclusion: SeGPruner通过语义感知和几何引导的token剪枝，有效解决了3D问答中的token冗余问题，在显著提升推理效率的同时保持了3D推理能力。
...
TL;DR: 介绍EarthEmbeddingExplorer交互式Web应用，将静态地球观测嵌入数据转化为动态工作流，支持多模态查询和科学洞察


<details>
  <summary>Details</summary>
Motivation: 解决地球观测领域基础模型和全球嵌入数据集虽多，但缺乏免费可访问工具的问题，将学术资产转化为实用工具

Method: 开发基于云原生软件架构的交互式Web应用，支持自然语言、视觉和地理位置等多模态查询，提供预计算地球嵌入的检索和分析功能

Result: 创建了EarthEmbeddingExplorer应用，可通过https://modelscope.ai/studios/Major-TOM/EarthEmbeddingExplorer访问，实现从模型和数据到实际应用的转换
...
TL;DR: NeoNet是一个用于胆管癌神经浸润预测的端到端3D深度学习框架，包含分割、数据增强和分类三个模块，在5折交叉验证中取得了0.7903的最高AUC。


<details>
  <summary>Details</summary>
Motivation: 减少侵入性诊断程序以降低患者损伤和感染风险是医学成像的核心目标。然而，神经浸润（PNI）的非侵入性诊断仍然具有挑战性，因为缺乏明确一致的成像标准来识别PNI。

Method: NeoNet包含三个模块：1) NeoSeg使用肿瘤定位ROI裁剪算法进行分割；2) NeoGen使用基于解剖掩码的3D潜在扩散模型生成合成图像补丁，将数据集平衡到1:1比例；3) NeoCls使用PNI注意力网络（PattenNet），包含冻结的LDM编码器和专门的3D双注意力块来检测PNI的细微强度变化和空间模式。

Result: 在5折交叉验证中，NeoNet超越了基线3D模型，取得了最高性能，最大AUC达到0.7903。

Conclusion: NeoNet提供了一个不依赖预定义图像特征的端到端深度学习框架，能够有效预测胆管癌中的神经浸润，为减少侵入性诊断程序提供了有前景的解决方案。
...
TL;DR: 提出基于多模态上下文学习的手写文本识别框架，仅需目标书写者的少量示例即可在推理时进行适应，无需参数更新，在IAM和RIMES数据集上达到SOTA性能。
...
Method: 提出基于多模态上下文学习的新型框架，使用少量目标书写者示例在推理时进行适应，无需参数更新；设计了紧凑的8M参数CNN-Transformer架构，支持少样本上下文适应；结合上下文驱动和标准OCR训练策略。

Result: 在IAM数据集上字符错误率为3.92%，在RIMES数据集上为2.34%，超越了所有无需推理时参数更新的书写者无关HTR模型。

Conclusion: 提出的上下文驱动HTR框架能够有效处理未见和异常书写风格，仅需少量示例即可实现推理时书写者适应，无需参数更新，在标准基准测试中达到最先进性能。
...
TL;DR: FedDBP是一种新颖的联邦原型学习方法，通过双分支特征投影器保证特征保真度和可区分性，并采用个性化全局原型融合来解决异构联邦学习中的数据异质性和模型异质性问题。


<details>
  <summary>Details</summary>
Motivation: 现有联邦原型学习方法存在两个主要问题：1）难以平衡特征的保真度和可区分性；2）受限于单一的全局原型。这些问题限制了在异构联邦学习环境下的性能表现。

Method: FedDBP采用客户端-服务器架构：客户端使用双分支特征投影器，同时采用L2对齐和对比学习来确保特征的保真度和可区分性；服务器端引入个性化全局原型融合方法，利用Fisher信息识别本地原型的重要通道。

Result: 通过大量实验验证，FedDBP在性能上超越了十种现有先进方法，显示出其优越性。
...
TL;DR: 提出一种基于方形块的超像素生成方法，使用多尺度方形块近似超像素，解决传统不规则超像素与卷积等规则算子不匹配的问题，便于并行处理和端到端优化。


<details>
  <summary>Details</summary>
Motivation: 传统超像素算法产生不规则区域，与卷积等规则算子不对齐，通常只能作为离线预处理步骤，限制了并行实现和深度学习管道中的端到端优化。

Method: 基于粒度球计算的自适应表示和覆盖特性，开发方形超像素生成方法：使用多尺度方形块近似超像素，为每个块基于像素强度相似性计算纯度分数，并据此选择高质量块。

Result: 实验结果表明，在下游任务中实现了持续的性能改进，验证了所提方法的有效性。方形超像素可轻松集成到图神经网络或视觉Transformer中，促进多尺度信息聚合和结构化视觉表示。

Conclusion: 提出的方形超像素方法解决了传统不规则超像素与深度学习框架不兼容的问题，实现了高效并行处理和可学习特征提取，为视觉任务提供了更好的区域表示。
...
TL;DR: VecAttention是一种向量级稀疏注意力框架，通过利用视频注意力图中的垂直向量稀疏模式，在保持精度的同时显著提升长视频处理的效率。


<details>
  <summary>Details</summary>
Motivation: Transformer视频模型在处理长上下文时面临二次复杂度的计算挑战，现有稀疏注意力方法使用粗粒度模式导致冗余计算和次优性能。

Method: 提出VecAttention框架：1) 发现视频注意力图具有强垂直向量稀疏模式；2) 通过轻量级重要向量选择动态选择信息向量；3) 优化向量稀疏注意力内核实现高效处理。

Result: 在视频理解（VideoMME、LongVideoBench、VCRBench）和生成（VBench）任务上，VecAttention比全注意力快2.65倍，比最先进稀疏注意力方法快1.83倍，精度与全注意力相当。

Conclusion: VecAttention通过向量级稀疏注意力实现了优异的精度-效率权衡，为长视频理解与生成提供了高效的解决方案。
...
TL;DR: 开发了一种基于HoloLens 2的无标记增强现实系统，用于将头颈鳞状细胞癌阳性切缘从切除标本重新定位到切除床，实现更精确的术中再切除。
...
Method: 使用HoloLens 2深度感知和全自动无标记表面配准技术，开发了增强现实系统，将切除标本的阳性切缘重新定位到切除床并进行原位可视化。

Result: 在硅胶模型研究中，无标记配准达到与标记基准相当的配准误差（中位数1.8 mm vs 1.7 mm）；在切缘重新定位任务中，AR引导将误差从口头引导的14.2 mm降低到3.2 mm，所有AR定位误差均在5 mm以内。

Conclusion: 无标记AR切缘引导系统可行，能够显著提高头颈鳞状细胞癌术中再切除的精确性。
...
Method: 两阶段框架：第一阶段提出边界约束的透射率校正方法，通过区域自适应补偿和归一化处理，结合YUV空间的二次高斯滤波估计大气光图；第二阶段提出STAR-YUV分解模型，将去雾图像分解为结构和纹理层，分别进行伽马校正、MSRCR色彩恢复和LoG滤波增强，最后通过非线性Retinex融合和线性混合得到最终结果

Result: 该方法能够有效处理夜间雾霾图像的多重退化问题，包括透射率校正、大气光估计、结构层光照补偿和色彩校正、纹理层细节增强等，相比现有方法更全面地解决夜间去雾挑战

Conclusion: 提出的两阶段框架通过透射率校正与结构-纹理分层优化的结合，能够更全面地解决夜间雾霾图像的多重退化问题，为夜间图像去雾提供了有效的解决方案
...
TL;DR: 提出统一框架，将LoRA权重作为运行时输入而非嵌入编译模型，支持移动设备上多任务生成式AI推理，通过QUAD量化策略对齐多个LoRA适配器，显著减少内存占用和延迟。
...
Result: 实验结果显示内存占用减少高达6倍，延迟改善高达4倍，同时在多个生成式AI任务上保持高视觉质量，兼容多种芯片组。

Conclusion: 该框架为移动设备上的多任务生成式AI推理提供了高效解决方案，通过动态LoRA权重管理和QUAD量化策略，显著降低了资源需求，实现了高性能的边缘AI部署。
...
<details>
  <summary>Details</summary>
Motivation: 在数字时代保存非物质文化遗产舞蹈面临挑战，Bharatanatyam舞蹈强调规范化的adavus和精确的关键姿势，准确生成这些姿势对于保持解剖和风格完整性、实现有效文档记录、分析以及通过数字手段向全球观众传播至关重要。
...
Result: 结果表明，在cGAN和条件扩散设置中，集成姿态指导使生成的姿势与真实关键点结构对齐，提高了文化保真度。姿态监督显著增强了生成的Bharatanatyam姿势的质量、真实性和真实性。
...
TL;DR: 提出AMDiT方法，通过自适应边界差异训练增强情感扩散分类器EmoDC，提升面部表情识别的准确性和对抗鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的FER方法大多依赖判别式分类器，容易学习捷径且对分布偏移敏感。扩散模型在FER中具有增强对抗鲁棒性的潜力，但标准训练策略无法有效惩罚错误类别描述，导致识别性能不佳。

Method: 1. 提出EmoDC情感扩散分类器；2. 引入边界差异训练，强制正确与错误类别间的噪声预测误差保持最小边界；3. 提出AMDiT自适应边界差异训练，根据样本难度动态调整边界。

Result: 在RAF-DB基础子集、RAF-DB复合子集、SFEW-2.0和AffectNet数据集上，AMDiT显著提升EmoDC准确率。EmoDC在噪声和模糊攻击下优于现有最佳判别式分类器。

Conclusion: AMDiT通过自适应边界调整有效增强了EmoDC的判别能力，为FER提供了一种更鲁棒的生成式方法，在准确性和对抗鲁棒性方面均取得显著改进。
...
Method: 结合单图像微调（使生成模型适应分布外受伤面部）和基于注意力的掩码（将编辑定位到受损区域，同时保留身份关键特征），实现暴力死亡中伪影的移除同时保持足够的身份信息。

Result: FlowID在InjuredFaces基准测试中优于最先进的开源方法，同时保持低内存需求，适合本地部署而不损害数据隐私。

Conclusion: FlowID提供了一种有效的身份保持面部重建方法，能够处理极端条件下的面部损伤，为法医识别提供技术支持，同时InjuredFaces基准为社区提供了标准化资源。
...
<details>
  <summary>Details</summary>
Motivation: 视频理解的复杂性使得难以确定性能提升来自视觉感知、语言推理还是知识先验。现有基准大多关注高级推理，但构成视频理解的基本标准被忽视。作者希望重新审视当前视频理解评估现状，而非引入另一个基准。

Method: 开发Video-Oasis诊断套件，系统评估现有评估方法并提取视频理解的时空挑战。分析现有基准样本的可解性，研究哪些算法设计选择有助于鲁棒的视频理解。

Result: 1) 54%的现有基准样本无需视觉输入或时序上下文即可解决；2) 在剩余样本上，最先进模型的性能仅略优于随机猜测。研究为未来研究提供了实用指南。

Conclusion: 该工作为基准构建和架构开发的严格评估提供了标准指南，揭示了当前视频理解评估的局限性，并提出了改进方向。
...
Method: 提出Unify-Agent，将图像生成重构为智能体流程：1) 提示理解；2) 多模态证据搜索；3) 基于事实的重新描述；4) 最终合成。构建专门的多模态数据管道，收集143K高质量智能体轨迹用于训练，并引入FactIP基准测试覆盖12类文化意义和长尾事实概念。

Result: Unify-Agent在多样化基准测试和真实世界生成任务上显著优于其基础统一模型，同时接近最强闭源模型的世界知识能力。作为基于智能体的世界基础图像合成的早期探索，展示了紧密耦合推理、搜索和生成对于可靠开放世界智能体图像合成的价值。

Conclusion: 该工作证明了智能体建模在解决统一多模态模型知识限制方面的有效性，为世界基础图像合成提供了新范式，强调推理、搜索和生成的紧密集成对于可靠开放世界图像生成的重要性。
...
TL;DR: BigEarthNet.txt是一个大规模多传感器遥感图像-文本数据集，包含46.4万张配准的Sentinel-1和Sentinel-2图像及960万文本标注，用于提升遥感领域的视觉语言模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有遥感图像-文本数据集主要包含航拍RGB图像，标注简短且类型有限，导致视觉语言模型在遥感数据上表现不佳。需要大规模、多传感器、多样化文本标注的数据集来推动遥感领域的指令驱动图像-文本学习。

Method: 构建BigEarthNet.txt数据集，包含46.4万张配准的Sentinel-1 SAR和Sentinel-2多光谱图像，提供三种类型标注：1) 地理锚定描述（土地利用/覆盖类别、空间关系、环境背景）；2) 视觉问答对；3) 指代表达检测指令用于边界框预测。

Result: 统计比较显示BigEarthNet.txt在文本丰富度和标注类型多样性上超越现有遥感数据集。实验表明现有视觉语言模型在复杂土地利用/覆盖类别任务上存在局限，而使用BigEarthNet.txt微调能在所有任务上带来一致的性能提升。

Conclusion: BigEarthNet.txt填补了遥感领域大规模多传感器图像-文本数据集的空白，为评估和提升视觉语言模型在遥感任务上的性能提供了重要基准，推动了指令驱动图像-文本学习在遥感领域的发展。
...
Method: 设备端epsilon-net过滤器保留语义新颖帧构建去噪嵌入索引，结合跨模态适配器和云端重排序补偿紧凑编码器的弱对齐

Result: 单通道流式过滤器在8个视觉语言模型（8M-632M）和2个自我中心数据集（AEA, EPIC-KITCHENS）上优于离线方法（k-means、最远点、均匀、随机），架构组合在8M设备端编码器上达到45.6% Hit@5，功耗约2.7 mW

Conclusion: 流式检索架构通过设备端新颖帧过滤和云端重排序有效提升跨模态检索性能，同时保持低功耗
...
TL;DR: 论文研究标签稀缺的视觉分类问题，针对去中心化异构数据场景，特别是部分类别重叠的情况。提出SSFL方法，引入PreDi分区方案分析标签空间异质性，并提出PreP-WFL方法优化低流行度类别表示。


<details>
  <summary>Details</summary>
Motivation: 现有自监督联邦学习研究通常假设预训练和微调阶段的数据异质性模式相同，且现有分区方案难以生成纯净的部分类别不重叠数据设置，限制了真实世界标签空间异质性的可控模拟。

Method: 1) 将硅藻分类作为代表性实例引入SSFL；2) 提出PreDi分区方案，将标签空间异质性解耦为类别流行度和类别集大小差异两个正交维度；3) 提出PreP-WFL方法，自适应增强低流行度场景中的稀有类别表示。

Result: SSFL在均匀和异构设置下均优于本地训练；未标记数据量的显著异质性与改进的表示预训练相关；在标签空间异质性下，流行度主导性能而差异影响较小；PreP-WFL有效缓解性能下降，且流行度越低增益越大。

Conclusion: 研究为表征去中心化识别系统中的标签空间异质性提供了机制基础，揭示了阶段特异性数据异质性的重要性，并提出了有效的解决方案来应对低流行度类别的挑战。
...
TL;DR: MacTok是一种基于掩码增强的1D连续图像分词器，通过随机掩码和语义掩码防止后验坍塌，仅用64-128个token实现高效视觉生成。


<details>
  <summary>Details</summary>
Motivation: 传统的基于变分框架的连续图像分词器在使用较少token时容易出现后验坍塌问题，编码器无法将信息特征编码到压缩的潜在空间中。

Method: MacTok结合随机掩码和DINO引导的语义掩码来正则化潜在学习，强调图像中的信息区域，迫使模型从不完整的视觉证据中编码鲁棒语义，同时结合全局和局部表示对齐。

Result: 在ImageNet上，MacTok在256×256分辨率下达到gFID 1.44，在512×512分辨率下达到SOTA的1.52，同时将token使用量减少高达64倍。

Conclusion: 掩码和语义引导相结合可以有效防止后验坍塌，实现高效、高保真的图像分词，为视觉生成提供了紧凑且鲁棒的表示。
...
TL;DR: 提出DynMask方法，通过运动谱描述符（MSD）量化局部动态复杂度，改进掩码运动生成模型，使其对复杂动态运动更敏感，提升生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有掩码生成模型对运动帧处理过于均匀，而实际运动中局部动态复杂度随时间变化显著。当前模型在处理动态复杂运动时性能下降明显，生成误差与运动动态密切相关。

Method: 提出运动谱描述符（MSD），基于运动速度的短时频谱计算局部动态复杂度。使用MSD指导训练中的内容聚焦掩码、为自注意力提供谱相似性先验，并在迭代解码中调节token级采样。

Result: DynMask在HumanML3D和KIT-ML数据集上显著提升动态复杂运动的生成质量，同时获得更好的整体FID分数。

Conclusion: 尊重局部运动复杂度是掩码运动生成的重要设计原则，MSD作为简单、无参数、可解释的复杂度度量能有效改进现有模型。
...
TL;DR: CutClaw是一个基于多智能体MLLM的自主视频编辑框架，能够将长时间原始素材自动剪辑成有意义的短视频，实现音乐同步、视觉吸引力和叙事连贯性。


<details>
  <summary>Details</summary>
Motivation: 当前社交媒体中视频编辑需要音频对齐，但手动编辑耗时且重复，给电影制作人和专业内容创作者带来挑战。需要自动化解决方案来提升效率。

Method: 采用分层多模态分解捕捉视觉和音频素材的细粒度细节与全局结构；使用Playwriter Agent编排整体叙事流程并锚定视觉场景到音乐变化；Editor和Reviewer Agent协作优化最终剪辑，基于美学和语义标准选择细粒度视觉内容。

Result: 实验表明CutClaw在生成高质量、节奏对齐的视频方面显著优于现有基线方法。

Conclusion: CutClaw提供了一个有效的多智能体框架，能够自动化视频编辑过程，生成具有音乐同步和视觉吸引力的短视频，解决了手动编辑的耗时问题。
...
TL;DR: 提出首个用于手术技能评估回归的无监督域适应基准，并开发了CoRe-DA对比回归域适应框架，在跨域泛化上优于现有方法


<details>
  <summary>Details</summary>
Motivation: 手术技能评估需要大量人工标注且现有回归模型对新任务和环境泛化能力差，同时存在大量未标注视频数据，需要开发无监督域适应方法

Method: 提出CoRe-DA框架，通过相对分数监督学习域不变表示，并结合目标域自训练，在四个数据集上评估八种代表性模型

Result: CoRe-DA在两个UDA设置中均优于现有方法，在干实验室和临床目标数据集上分别达到0.46和0.41的Spearman相关系数，且无需目标域标注数据
...
TL;DR: 提出临床DVH指标损失函数（CDM损失），结合可微D指标和代理V指标，通过位掩码ROI编码提高训练效率，显著改善头颈部放疗剂量预测的临床一致性。


<details>
  <summary>Details</summary>
Motivation: 现有基于深度学习的3D剂量预测模型大多使用体素回归损失，与基于剂量体积直方图（DVH）指标的临床计划评估标准不匹配，需要开发能直接优化临床DVH指标的损失函数。

Method: 提出临床DVH指标损失（CDM损失），包含可微D指标和代理V指标，结合无损位掩码ROI编码提高训练效率；在174例头颈部患者数据上评估（137训练，37测试）。

Result: 相比MAE和DVH曲线损失，CDM损失显著改善靶区覆盖并满足所有临床约束；PTV评分从1.544（MAE）降至0.491（MAE+CDM），危及器官保护相当；位掩码编码减少83%训练时间并降低GPU内存使用。

Conclusion: 直接优化临床DVH指标使3D剂量预测比传统体素或DVH曲线监督更符合临床治疗计划标准；CDM损失结合高效ROI位掩码编码为头颈部剂量预测提供实用可扩展框架。
...
Method: 提出SkeletonContext框架：1）跨模态上下文提示模块，利用预训练语言模型在LLM指导下重建掩码的上下文提示，将语言上下文转移到骨架编码器；2）关键部分解耦模块，解耦与运动相关的关节特征，确保在没有显式物体交互时也能鲁棒理解动作。

Result: 在多个基准测试上的广泛实验表明，SkeletonContext在常规和广义零样本设置下均达到最先进性能，验证了其在推理上下文和区分细粒度视觉相似动作方面的有效性。

Conclusion: SkeletonContext通过语言驱动的上下文语义增强骨架表示，有效解决了零样本骨架动作识别中上下文缺失和视觉相似动作难以区分的问题，实现了更好的跨模态对齐和动作理解。
...
TL;DR: 该研究评估了皮肤病变分割模型在不同肤色和对比度下的性能，发现病变-皮肤对比度低是分割错误的主要驱动因素，而非全局肤色分类。


<details>
  <summary>Details</summary>
Motivation: 虽然AI驱动的皮肤病学系统广泛使用皮肤病变分割作为预处理步骤，但关于肤色对分割阶段影响的研究不足，且通常使用粗糙的离散肤色类别进行评估。需要量化肤色对分割性能的影响，特别是使用更精细的连续分析方法。

Method: 评估了三种分割架构（UNet、DeepLabV3+ResNet50、DINOv2）在两个公开皮肤镜数据集（HAM10000和ISIC2017）上的性能。引入了连续的色素/对比度分析方法，将像素级ITA值视为分布。使用皮肤区域、病变区域和整个图像区域内的Wasserstein距离来量化病变-皮肤对比度，并将其与多个分割指标相关联。
...
TL;DR: FED-Bench：一个全面的面部表情编辑基准测试，包含高质量数据集、评估协议FED-Score，并揭示了当前方法在细粒度控制上的瓶颈。


<details>
  <summary>Details</summary>
Motivation: 现有面部表情编辑任务缺乏高质量数据集和准确评估指标，现有基准测试偏向通用场景，评估指标存在系统性偏差（倾向于懒惰编辑或过拟合编辑）。

Method: 1) 通过级联可扩展流程构建包含747个三元组（原始图像、编辑指令、真实图像）的基准测试；2) 提出FED-Score评估协议，从对齐度、保真度、相对表情增益三个维度评估；3) 评估18个图像编辑模型；4) 利用基准引擎扩展性提供2万+野外面部训练集。

Result: 当前方法难以同时实现高保真度和准确的表情操控，细粒度指令跟随是主要瓶颈。使用提供的训练集微调基线模型能显著提升性能。

Conclusion: FED-Bench填补了面部表情编辑领域的基准测试空白，提出的评估协议能有效缓解评估偏差，揭示了当前方法的局限性，并为未来研究提供了高质量数据集和评估框架。
...
TL;DR: SISTA-Net是一种基于压缩感知的自监督单像素成像方法，通过展开ISTA算法构建可解释网络，结合CNN-VSSM混合架构和自适应稀疏变换，在极低采样率下实现鲁棒成像。


<details>
  <summary>Details</summary>
Motivation: 现有单像素成像方法缺乏物理稀疏约束，忽略局部和全局特征的整合，导致严重的噪声敏感性、结构失真和细节模糊问题。

Method: 将迭代收缩阈值算法展开为可解释网络，包含数据保真模块和近端映射模块。保真模块采用CNN-VSSM混合架构整合局部和全局特征建模；利用深度非线性网络作为自适应稀疏变换，结合可学习软阈值算子，在潜在域施加显式物理稀疏约束。

Result: 在多个仿真场景中，SISTA-Net比最先进方法PSNR提升2.6 dB；真实世界远场水下测试平均PSNR提升3.4 dB，验证了其强大的抗干扰能力。

Conclusion: SISTA-Net通过物理约束的稀疏表示和局部-全局特征整合，显著提升了单像素成像在强干扰环境下的重建质量和鲁棒性，特别是在极低采样率下仍能有效抑制噪声。
...
TL;DR: 使用合成数据显著提升第一人称视角下的手物交互检测性能，特别是在真实标注数据稀缺时，通过合成数据与少量真实数据结合，在多个数据集上获得5-12%的性能提升。


<details>
  <summary>Details</summary>
Motivation: 解决第一人称视角图像中手物交互检测任务中真实标注数据稀缺的问题，探索合成数据在提升检测性能方面的潜力。

Method: 开发新的数据生成流程和HOI-Synth基准，通过合成数据增强现有数据集，系统研究合成数据与真实数据在物体、抓握方式和环境方面的对齐效果。

Result: 仅使用10%真实标注数据结合合成数据，在VISOR、EgoHOS和ENIGMA-51数据集上分别获得+5.67%、+8.24%和+11.69%的AP提升，证明合成数据对齐效果越好，性能提升越明显。

Conclusion: 合成数据能显著改善手物交互检测性能，特别是在真实数据稀缺时，合成数据与真实数据的对齐质量是关键因素，并发布了完整的数据生成工具和基准供研究社区使用。
...
TL;DR: 提出一种用于动态场景单目视频新视角合成的通用化模型，通过循环机制和平面扫描技术实现无限制的相机控制，在静态和动态区域都能保持几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在局限性：基于场景特定优化的4D表示方法在高度动态区域容易失效；基于扩散的方法虽然能生成视觉上合理的视频，但存在几何不一致问题；且两种方法都需要大量计算资源。

Method: 基于静态新视角合成的通用化模型框架，引入两个关键组件：(1) 实现输入与目标视频之间无限制异步映射的循环机制；(2) 通过动态输入的平面扫描技术解耦相机运动和场景运动，实现精细的六自由度相机控制。

Result: 在UCSD数据集和新构建的Kubric-4D-dyn数据集上评估，模型在重建静态和动态区域的精细几何细节方面，优于四种基于高斯泼溅的场景特定方法和两种基于扩散的方法。

Conclusion: 提出的通用化模型能够有效处理动态场景的新视角合成，在保持几何一致性的同时实现精细的相机控制，为动态场景重建提供了更高效和准确的解决方案。
...
TL;DR: SHIFT攻击利用扩散水印方法依赖轨迹重建的弱点，通过随机重采样在潜在空间偏转生成轨迹，实现高成功率攻击而不影响视觉质量


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的水印方法都假设验证需要忠实重建扩散轨迹，这种对轨迹恢复的依赖构成了根本性的可利用漏洞。作者旨在利用这一共同弱点开发通用攻击方法

Method: 提出SHIFT（随机隐藏轨迹偏转）攻击，利用随机扩散重采样在潜在空间偏转生成轨迹，使重建图像在统计上与原始水印嵌入轨迹解耦，同时保持视觉质量和语义一致性。该方法无需训练，也不需要水印特定知识或模型重训练

Result: 在涵盖噪声空间、频域和基于优化范式的9种代表性水印方法上，SHIFT实现了95%-100%的攻击成功率，且语义质量几乎没有损失

Conclusion: 扩散水印方法对轨迹重建的依赖构成了根本性安全漏洞，SHIFT攻击成功利用这一弱点，表明需要重新思考扩散水印的安全假设和设计
...
TL;DR: TSHA是一个全面的室内安全隐患评估基准，包含81,809个训练样本和1,707个挑战性测试样本，解决了现有基准的三大局限：依赖合成数据、任务过于简化、缺乏严格评估协议。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型在室内安全隐患评估方面存在三个根本问题：1) 过度依赖仿真软件构建的合成数据集，与现实环境存在显著领域差距；2) 安全隐患任务过于简化，对危险类型和场景类型有人为限制，限制了模型泛化能力；3) 缺乏严格的评估协议来全面评估模型在复杂家庭安全场景中的能力。

Method: 构建TSHA基准，包含81,809个精心策划的训练样本，来自四个互补来源：现有室内数据集、互联网图像、AIGC生成图像和新拍摄图像。测试集包含1,707个样本，不仅包括训练分布的精选子集，还新增了包含多个安全隐患的视频和全景图像，用于评估模型在复杂安全场景中的鲁棒性。

Result: 对23个流行视觉语言模型的广泛实验表明，当前VLM缺乏稳健的安全隐患评估能力。重要的是，在TSHA训练集上训练的模型不仅在TSHA测试集上实现了高达+18.3分的显著性能提升，还在其他基准上表现出增强的泛化能力。

Conclusion: TSHA基准为室内安全隐患评估提供了全面的评估框架，解决了现有基准的局限性，显著提升了视觉语言模型在安全评估任务中的性能和泛化能力，强调了TSHA基准的重要贡献和价值。
...
TL;DR: IQPIR：利用无参考图像质量评估模型提取图像质量先验，引导图像恢复过程获得感知最优输出，无需依赖完美参考图像
...
Result: 在真实世界图像恢复任务中超越现有先进方法，可作为通用质量引导增强策略应用于现有方法

Conclusion: IQPIR通过引入图像质量先验，有效解决了传统GT监督的局限性，实现了感知最优的图像恢复效果，具有即插即用的增强特性
...
TL;DR: 提出一种结合骨架动作检测与视觉语言模型的混合边缘系统，用于公共场所暴力行为实时监测，平衡隐私、延迟和资源限制。


<details>
  <summary>Details</summary>
Motivation: 公共场所需要及时可靠的暴力行为检测来保障公共安全，但现有自动化视频分析在实际部署中面临延迟、隐私和边缘计算资源限制等挑战。

Method: 设计并部署混合边缘系统：骨架分析用于连续隐私监测（低计算开销），视觉语言模型提供语义场景理解和零样本推理能力。系统级比较两种范式在真实边缘约束下的表现。

Result: 在GPU边缘设备上实现并评估，结果显示运动中心方法和语义方法具有互补优势，混合架构能选择性增强快速骨架检测与高级语义推理。

Conclusion: 混合架构为隐私感知的实时视频分析提供了实用基础，特别适用于公共安全应用，平衡了计算效率与语义理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有层次多标签分类方法在多路径设置下表现不佳，无法充分利用层次信息。遥感图像可能激活多个分类分支，需要更有效的层次语义建模方法。

Method: 提出MAPLE框架，包含三个核心组件：1) 基于图感知文本描述的层次语义初始化；2) 图卷积网络编码图结构；3) 自适应多模态融合平衡语义先验和视觉证据。采用自适应层次感知目标函数。

Result: 在CORINE对齐的遥感数据集（AID、DFC-15、MLRSNet）上评估，在少样本场景下性能提升高达42%，仅增加2.6%的参数开销。

Conclusion: MAPLE能够有效且高效地建模地球观测中的层次语义，为遥感图像的多路径层次分类提供了有力解决方案。
...
Result: 在四个基准数据集上的实验表明，单个特征空间在不同生成器间性能差异显著，而三特征融合在所有数据集上均取得更优且更一致的性能，特别是在混合模型场景中表现突出
...
<details>
  <summary>Details</summary>
Motivation: 体化AI需要能够支持多样化用户有意义活动的交互式3D环境，但评估这些环境的功能可用性仍然是一个核心挑战。现有方法难以验证3D场景在特定智能体约束下的实际功能性。

Method: 提出SceneTeract框架，包含一个基于验证的引擎，将复杂活动分解为原子动作序列，并使用显式的物理和几何模拟，针对具体智能体配置文件验证每个步骤的可达性、净空和可导航性等可访问性要求。

Result: 1) 对合成室内环境的评估揭示了频繁的功能故障，阻碍基本交互；2) 前沿视觉语言模型在功能可用性推理方面存在系统性不匹配，即使最强模型也存在语义置信度与物理可行性之间的差距；3) SceneTeract可作为VLM后训练的奖励引擎，将几何约束扩展到推理模型中。

Conclusion: SceneTeract通过连接感知与物理现实，为体化3D场景理解提供了功能验证框架，有助于发现场景设计缺陷和评估AI模型的功能推理能力，并支持将几何约束融入推理模型的扩展训练。
...
TL;DR: AutoFormBench：一个包含407个真实世界表单的基准数据集，用于训练和评估表单元素检测模型，比较了传统OpenCV方法和多种YOLO架构的性能。


<details>
  <summary>Details</summary>
Motivation: 现实世界中政府表格、医疗记录和企业发票等结构化文档的布局高度多变，自动化处理面临持续挑战，需要更好的表单元素检测方法。

Method: 创建AutoFormBench数据集（包含政府、医疗和企业领域的407个标注表单），系统比较了传统OpenCV方法和四种YOLO架构（YOLOv8、YOLOv11、YOLOv26-s、YOLOv26-l）在定位和分类可填写表单元素（复选框、输入行、文本框）方面的性能。

Result: YOLOv11在所有元素类别和容差级别上都表现出最一致的优越性能，在F1分数和Jaccard准确率方面均优于其他方法。

Conclusion: AutoFormBench为表单元素检测提供了有价值的基准数据集，YOLOv11在该任务上表现出最佳性能，为现实世界表单处理自动化提供了有效解决方案。
...
TL;DR: CANVAS是首个大规模亚细胞分辨率小鼠全脑光片荧光显微镜基准数据集，包含六种神经元和免疫细胞标记，提供细胞标注和排行榜，旨在加速针对PB级脑显微数据的分析方法开发。


<details>
  <summary>Details</summary>
Motivation: 亚细胞分辨率全脑3D显微技术揭示了前所未有的生物结构细节，但缺乏针对PB级数据的可扩展处理方法和分析工具，现有视觉模型难以泛化到此类数据。

Method: 创建CANVAS基准数据集，包含高分辨率全小鼠脑LSFM数据、六种细胞类型标记、细胞标注和排行榜，并评估现有架构基线模型的泛化能力。

Result: 展示了基于现有架构的基线模型在泛化方面面临挑战，特别是由于不同表型和脑区细胞形态的异质性。CANVAS是首个也是最大的捕获完整小鼠脑组织亚细胞水平的LSFM基准。

Conclusion: CANVAS为加速开发适合PB级全脑显微数据的处理方法和基础模型提供了重要资源，填补了该领域基准数据的空白。
...
TL;DR: 提出结合扩散驱动特征去噪与混合特征表示的鲁棒手写数字多分类框架，在特征空间进行扩散操作提升对抗攻击鲁棒性


<details>
  <summary>Details</summary>
Motivation: 受先前脑肿瘤分类工作启发，旨在提升手写数字分类对噪声和对抗攻击的鲁棒性，通过特征空间操作增强模型可靠性

Method: 1) 使用非负矩阵分解(NNMF)将输入图像转换为紧凑可解释表示；2) 用CNN提取深度特征；3) 将两种特征结合为混合表示；4) 在特征空间进行逐步扩散操作添加高斯噪声；5) 训练特征去噪网络从含噪输入重建干净表示；6) 用去噪后特征进行多分类

Result: 在基线和对抗设置(AutoAttack)下评估，扩散混合模型既有效又鲁棒，优于CNN基线模型，同时保持强大分类性能

Conclusion: 特征级扩散防御对于可靠的多类手写数字分类具有有效性，证明了该方法在提升模型鲁棒性方面的潜力
...
TL;DR: 使用分形数据训练深度学习模型进行动态MRI重建，效果与使用真实心脏MRI数据训练的模型相当，为隐私受限场景提供开放可扩展的替代方案。
...
Method: 使用四元数Julia分形生成2D+时间图像训练数据，模拟多线圈MRI采集生成配对的全采样和径向欠采样k空间数据，训练3D UNet深度学习模型，并与使用真实心脏MRI数据训练的模型进行比较。
...
TL;DR: AiS是一个将结构抽象与视觉风格化分离的生成框架，通过创建抽象代理来捕捉风格中的深层抽象逻辑，实现更广泛的风格迁移。


<details>
  <summary>Details</summary>
Motivation: 传统风格迁移方法通常保留输入图像的几何结构，难以捕捉艺术风格中超越表面外观的深层抽象行为，特别是对于插画和非写实风格。

Method: AiS采用两阶段框架：1）从目标图像和少量风格示例中推导出抽象代理，重新解释目标结构以符合风格的抽象逻辑；2）将抽象代理渲染为最终风格化输出。两个阶段都使用共享图像空间类比实现，无需显式几何监督。

Result: 通过将抽象与外观解耦，AiS支持更广泛的风格转换，提高了可控性，并实现了更具表现力的风格化。

Conclusion: AiS框架通过将抽象作为显式、可转移的过程来处理，能够更好地捕捉艺术风格中的深层抽象行为，为风格迁移提供了新的方向。
...
TL;DR: 提出一个端到端深度学习框架，结合分割与双模式（有损+无损）压缩，用于风力涡轮机叶片检测中的图像传输优化。


<details>
  <summary>Details</summary>
Motivation: 风力涡轮机检测中传输高分辨率图像会产生瓶颈，需要高效编码方法在保持叶片区域高保真度的同时，对背景进行激进压缩。
...
Result: 在大规模风力涡轮机数据集上展示了优越的压缩性能和效率，为自动化检测提供实用解决方案。

Conclusion: 这是首个完全集成的基于学习的ROI编解码器，结合了分割、有损和无损压缩，确保后续缺陷检测不受影响，解决了风力涡轮机检测中的图像传输瓶颈问题。
...
Method: 1. 通过紧凑的锚定帧集合表示角色视觉属性，为一致性提供稳定参考；2. 引入超集内容锚定机制，提供训练片段内外线索防止重复；3. 使用RoPE作为弱条件，编码位置偏移以区分多个锚定；4. 构建可扩展的流水线从海量视频中提取这些锚定。

Result: 实验表明该方法能够生成超过10分钟的高质量角色视频，在跨视角的表达性身份和外观一致性方面超越了现有方法。

Conclusion: 通过锚定帧表示和相应的机制设计，成功解决了字符视频生成中的身份一致性和多视角外观一致性问题，实现了高质量的长视频生成。
...
TL;DR: 该论文系统研究了分割不确定性量化中聚合策略的性能，提出了考虑空间结构的新策略和鲁棒的元聚合器，在OoD和故障检测任务上表现优异。


<details>
  <summary>Details</summary>
Motivation: 在安全关键领域（如生物医学图像分析或自动驾驶）中，不确定性量化对于确保自动图像分割的可靠性至关重要。虽然聚合策略被常规使用，但它们的特性和对下游任务性能的影响尚未得到全面研究，导致报告不一致和最佳实践不明确。

Method: 1) 形式化分析常见聚合策略的特性、局限性和陷阱；2) 提出结合空间不确定性结构的新策略；3) 在十个不同图像几何和结构的数据集上对OoD和故障检测任务进行基准测试；4) 提出集成多个聚合器的元聚合器。

Result: 研究发现，利用空间结构的聚合器在两个下游任务中都表现出更强的性能。但单个聚合器的性能严重依赖于数据集特性，因此提出的元聚合器能够跨数据集实现鲁棒性能。

Conclusion: 该研究填补了分割不确定性量化聚合策略的系统研究空白，提出了考虑空间结构的新策略和鲁棒的元聚合器，为安全关键应用中的不确定性量化提供了更可靠的解决方案。
...
TL;DR: EC-Bench是一个用于评估长视频中枚举、计数和时间证据定位能力的基准测试，包含152个超过30分钟的视频和1,699个查询，现有MLLMs表现远低于人类水平。


<details>
  <summary>Details</summary>
Motivation: 现有视频计数基准主要关注短片段，仅评估最终数值答案，无法提供关于应该计数什么或模型是否能在整个时间范围内一致识别相关实例的洞察。长视频中的稀疏、多样事件使得长程时间推理特别困难。

Method: 引入EC-Bench基准，包含152个超过30分钟的长视频和1,699个查询，每个查询都配有明确的时间证据跨度。该基准联合评估枚举、计数和时间证据定位能力。

Result: 在22个多模态大语言模型中，最佳模型在枚举任务上仅达到29.98%准确率，在计数任务上达到23.74%准确率，而人类表现分别为78.57%和82.97%。分析显示枚举准确率、时间定位和计数性能之间存在强相关性。

Conclusion: EC-Bench揭示了当前MLLMs在长形式定量视频推理方面的基本局限性，并建立了一个具有挑战性的基准，强调了枚举准确率、时间定位和计数性能之间的重要关系。
...
TL;DR: DEUS提出了一种新的开放世界目标检测框架，通过ETF子空间未知分离和基于能量的已知区分损失，有效解决未知目标检测和已知类别遗忘问题。


<details>
  <summary>Details</summary>
Motivation: 开放世界目标检测面临两个核心挑战：1) 现有方法依赖已知类别预测来检测未知目标，难以有效学习和识别未知目标表示；2) 内存回放虽然缓解旧类别遗忘，但往往牺牲新学类别的知识。需要同时解决未知目标检测和类别间知识干扰问题。

Method: 提出DEUS框架，包含两个核心组件：1) ETF子空间未知分离(EUS)：利用等角紧框架的几何特性创建正交子空间，实现已知和未知目标表示的清晰分离，同时利用两个空间的能量来更好捕捉未知目标的独特模式；2) 基于能量的已知区分(EKD)损失：强制分离先前和当前分类器，最小化内存回放过程中新旧类别间的知识干扰。

Result: 在开放世界目标检测基准测试中，DEUS在未知目标检测方面表现出显著的性能提升，同时保持了已知类别的竞争性性能。

Conclusion: DEUS通过创新的ETF子空间分离和能量区分机制，有效解决了开放世界目标检测中的未知目标识别和类别间知识干扰问题，为开放世界学习提供了新的解决方案。
...
### [107] [Trimodal Deep Learning for Glioma Survival Prediction: A Feasibility Study Integrating Histopathology, Gene Expression, and MRI](https://arxiv.org/abs/2603.29968)
*Iain Swift,JingHua Ye*

Main category: cs.CV

TL;DR: 该研究探索了在脑肿瘤预后预测中整合FLAIR MRI作为第三模态的可行性，发现三模态早期融合在探索性评估中表现最佳，但受限于小样本量，统计显著性不足。


<details>
  <summary>Details</summary>
Motivation: 虽然多模态深度学习已通过整合组织病理学和基因组数据提高了脑肿瘤预后准确性，但体积MRI在统一生存框架中的贡献尚未被探索。本研究旨在评估将FLAIR MRI作为第三模态加入现有双模态框架的潜在价值。

Method: 使用TCGA-GBMLGG队列（664名患者），将BraTS2021的FLAIR MRI作为第三模态。评估了三种单模态模型、九种双模态配置和三种三模态配置，采用早期、晚期和联合融合策略。在有限样本条件下（19名测试患者）进行实验。

Result: 三模态早期融合获得探索性综合评分0.854，比双模态基线提高+0.011，但差异无统计学显著性（p=0.250）。MRI单模态表现合理（CS=0.755），但对双模态组合改进有限，在三模态组合中提供可测量的提升。所有含MRI的实验因样本量小导致置信区间宽泛（如[0.400,1.000]）。

Conclusion: 初步证据表明，即使样本量有限，第三成像模态仍可能增加预后价值，但需要足够的多模态上下文才能有效贡献。需要更大样本量来获得确定性结论。
...
<details>
  <summary>Details</summary>
Motivation: 早期识别青少年物质使用起始风险至关重要，但现有预测方法大多将大脑连接视为静态或横断面，忽略了大脑网络随时间变化以及与行为的关系

Method: 提出NeuroBRIDGE框架：1）在Riemannian切空间中对齐纵向功能连接组；2）结合双时间注意力机制；3）使用行为条件Koopman动力学捕捉时间变化

Result: 在ABCD数据集上评估，NeuroBRIDGE相比相关基线方法显著提高了未来物质使用起始的预测准确性，同时提供了可解释的神经通路洞察

Conclusion: NeuroBRIDGE不仅改进了物质使用起始风险预测，还提供了对神经发育风险的可解释理解，为针对性预防干预提供了信息
...
Method: 提出SurgTEMP多模态LLM框架，包含：(1)查询引导的token选择模块，构建分层视觉记忆（空间和时间记忆库）；(2)手术能力递进训练方案。这些组件能有效建模可变长度手术视频，同时保留程序相关线索和时间连贯性。

Result: 在CholeVidQA-32K数据集（包含32K开放式问答对和3,855个视频片段）上，SurgTEMP在全面评估中相比最先进的开源多模态和视频LLM（微调和零样本）实现了显著的性能提升。

Conclusion: SurgTEMP通过分层视觉记忆和渐进式训练方案，有效解决了手术视频问答的挑战，推动了基于视频的手术视觉问答技术的发展。
...
Method: 收集了10,535个手术视频（214.5M帧），涵盖内窥镜、腹腔镜、白内障和机器人手术；开发了统一的预训练流程，包含平衡采样策略；建立了可复现的基准测试，涵盖16个下游数据集和4个临床领域。

Result: SurgRec在16个下游数据集上表现优于自监督学习和视觉语言模型；视觉语言模型在细粒度时序识别任务上不可靠，存在性能差距且对提示词敏感。

Conclusion: SurgRec为手术视频理解提供了一个可复现、可扩展的基础框架，所有代码、模型和数据都将公开，为社区建立更通用的手术视频模型奠定了基础。
...
TL;DR: MAGNet是一个Transformer风格的图神经网络框架，通过自适应学习结构-功能交互，结合结构MRI和静息态fMRI数据，在ABCD数据集上优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 理解大脑结构与功能的相互作用是解释智力的关键，但联合建模具有挑战性，因为结构和功能连接组捕获了互补的组织方面。

Method: 提出多尺度自适应图网络(MAGNet)，利用结构MRI的源形态测量提取区域间形态特征，与静息态fMRI的功能网络连接性融合。采用混合图整合直接和间接通路，局部-全局注意力机制细化连接重要性，联合损失函数同时强制跨模态一致性和优化预测目标。

Result: 在ABCD数据集上，MAGNet超越了相关基线方法，展示了有效的多模态整合能力。

Conclusion: MAGNet框架通过自适应学习结构-功能交互，为推进对认知功能的理解提供了有效的多模态整合方法。
...
TL;DR: 开发了一个集成式头戴显示器增强现实手术导航框架，支持患者和器械追踪、工具校准、图像配准，在HoloLens 2和Magic Leap 2上实现毫米级精度，适用于多种手术应用。
...
Method: 开发了集成式HMD-AR手术导航框架，通过追踪患者和手术器械上的2D标记图案，使用枢轴和参考校准技术校准手术工具，采用点匹配和手动定位实现图像到患者的配准。

Result: 在HoloLens 2和Magic Leap 2设备上评估，实现平均工具尖端校准精度1毫米，配准精度3毫米，在AR引导针插入和肋骨骨折定位两个手术用例中靶向精度低于5毫米。
...
<details>
  <summary>Details</summary>
Motivation: 现有偏振方法依赖复杂视觉编码器和融合机制，增加模型复杂度，未能充分探索偏振如何显式指导分层RGB表示学习。

Method: 1) 轻量级偏振交互模块联合建模互补线索；2) 条件引导机制动态调制RGB特征；3) 偏振边缘引导频率细化策略增强高频分量；4) 迭代反馈解码器进行粗到细特征校准。

Result: 在偏振数据集和非偏振数据集上的广泛实验表明，CPGNet在多个任务中始终优于最先进方法。

Conclusion: CPGNet通过条件偏振引导机制有效解决了伪装目标检测问题，在保持轻量化的同时实现了更好的性能。
...
TL;DR: GeoCodeBench是一个博士级别的3D视觉编码基准测试，评估AI在复杂3D几何视觉任务中的代码生成能力，当前最佳模型GPT-5仅达到36.6%通过率，显示与可靠3D科学编码存在巨大差距。


<details>
  <summary>Details</summary>
Motivation: 当前AI辅助编码模型在处理复杂3D几何视觉任务时仍然困难，如果模型能够可靠地生成此类代码，将显著改变计算机视觉研究社区的工作流程和研究方式。

Method: 从近期顶会论文的官方代码库中提取候选函数，经过人工筛选选择核心3D几何组件，为每个目标生成多样化的边缘情况单元测试，实现全自动可复现的评分。构建两级任务层次：通用3D能力（几何变换和力学/光学公式）和研究能力（新算法实现和几何逻辑路由）。

Result: 评估8个代表性开源和闭源模型，最佳模型GPT-5仅达到36.6%通过率。研究发现研究导向任务明显更难，上下文消融实验显示"更多论文文本"并不总是更好，在方法部分截断的输入统计上优于完整论文输入。

Conclusion: GeoCodeBench为从通用编码向可信赖的3D几何视觉编码的进步提供了一个严格的测试平台，揭示了当前模型在科学长文本理解和复杂几何推理方面的未解决挑战。
...
TL;DR: 视频扩散模型在迷宫求解中表现出早期规划承诺现象，路径长度而非障碍密度是迷宫难度的主要预测因子，研究者提出ChEaP方法显著提升长序列迷宫求解能力
...
Method: 使用2D迷宫求解作为测试平台，分析视频扩散模型的内部规划动态。提出ChEaP（Chaining with Early Planning）方法：仅对有前景的早期规划种子进行计算，并将它们链接起来处理复杂迷宫。

Result: 发现两个关键现象：1）早期规划承诺：模型在前几个去噪步骤中就确定了高层运动规划；2）路径长度（而非障碍密度）是迷宫难度的主要预测因子，12步处存在明显失败阈值。ChEaP方法将长序列迷宫准确率从7%提升至67%，在Frozen Lake和VR-Bench等困难任务上整体提升2.5倍。

Conclusion: 当前视频模型拥有比先前认知更深的推理能力，通过更好的推理时缩放可以更可靠地激发这些能力。ChEaP方法通过利用早期规划承诺和链式生成，显著提升了复杂迷宫求解的性能。
...
TL;DR: OmniRoam是一个可控的全景视频生成框架，通过两阶段方法（预览和细化）实现长时程、高分辨率、全局一致的场景漫游视频生成。


<details>
  <summary>Details</summary>
Motivation: 现有基于视角视频模型的方法只能合成场景的有限观测，存在完整性和全局一致性问题。全景表示具有丰富的每帧场景覆盖和固有的长时空一致性，适合实现长时程场景漫游。

Method: 采用两阶段框架：1）预览阶段：使用轨迹控制视频生成模型从输入图像/视频创建场景概览；2）细化阶段：将视频进行时间扩展和空间上采样，生成长范围高分辨率视频。使用合成和真实世界捕获的全景视频数据集进行训练。

Result: 实验表明，OmniRoam在视觉质量、可控性和长时程场景一致性方面均优于现有方法（定性和定量）。框架还展示了实时视频生成和3D重建等扩展应用。

Conclusion: OmniRoam利用全景表示的优势，实现了高质量、可控的长时程场景漫游视频生成，解决了现有方法的完整性和一致性限制，具有广泛的应用前景。
...
TL;DR: OptiMer提出了一种新的持续预训练方法，通过训练单个数据集模型、提取分布向量，然后使用贝叶斯优化后验搜索最优组合权重，避免了传统数据混合比例调优的高昂计算成本。


<details>
  <summary>Details</summary>
Motivation: 传统持续预训练中，数据混合比例是一个敏感的超参数，需要在训练前固定，且调优过程计算成本高昂（浪费数周计算资源）。需要一种更高效的方法来优化数据混合比例。

Method: OptiMer将比例选择与训练解耦：1）为每个数据集训练一个CPT模型；2）提取每个模型的分布向量（表示该数据集引起的参数偏移）；3）通过贝叶斯优化后验搜索最优组合权重。

Result: 在Gemma 3 27B上的实验显示，OptiMer在语言（日语、中文）和领域（数学、代码）任务上均优于数据混合和模型平均基线，搜索成本降低15-35倍。优化后的权重可解释为数据混合比例，重新训练可提升性能；同一向量池可针对不同目标重新优化，无需重新训练。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机在于解决领域转移下极性检测的挑战，特别是在大屠杀口述历史这类具有复杂话语结构的异质长格式叙事中，需要评估现成情感分类器的可靠性。

Method: 使用三种预训练transformer极性分类器分析107,305个话语和579,013个句子的语料库；引入基于一致性的稳定性分类法(ABC)来分层模型输出稳定性；计算配对百分比一致性、Cohen kappa、Fleiss kappa和行归一化混淆矩阵；使用T5情感分类器分析各一致性层的情感分布。

Result: 模型间整体一致性为低到中等，主要分歧源于中性边界决策；多模型标签三角测量和ABC分类法相结合，为敏感历史叙事中情感模型分歧的特征化提供了谨慎的操作框架。

Conclusion: 在敏感历史叙事中，现成情感分类器的可靠性有限，特别是在中性边界判断上存在系统性分歧；需要更谨慎的方法来评估和解释情感分析结果。
...
TL;DR: CrossTrace是一个包含1389个跨领域科学推理轨迹的数据集，用于训练和评估假设生成模型，通过结构化推理链将先验知识与新假设连接起来。
...
Method: 构建CrossTrace数据集，包含1389个基于源论文文本的推理轨迹，涵盖生物医学、AI/ML和跨领域研究；采用Input/Trace/Output模式，包含步骤级验证和八种发现模式分类；使用QLoRA微调Qwen2.5-7B-Instruct模型。

Result: 微调后模型性能显著提升：IAScore从0.828提高到0.968（GPT-4o评估），结构合规性从0%提高到100%，spark余弦相似度从0.221提高到0.620；跨领域训练优于单领域训练；人工验证显示99.7%的步骤级基础准确率和0.0%的捏造率。

Conclusion: CrossTrace是首个大规模、跨领域、具有步骤级基础推理轨迹的假设生成数据集，证明这种推理轨迹是有效的训练信号，其益处至少部分具有领域通用性。
...
TL;DR: 安全微调会抑制LLMs对自身和科技产品的心理归因，但不会损害心理理论能力，反而会导致对非人类动物心理的低估和灵性信念的减少


<details>
  <summary>Details</summary>
Motivation: 研究安全微调在抑制LLMs有害心理归因（如声称自己有意识或情感）时，是否会损害相关的社会认知能力（如心理理论）

Method: 通过安全消融实验和表征相似性的机制分析，比较LLMs对自身/科技产品的心理归因与心理理论能力的行为和机制差异

Result: LLMs对自身和科技产品的心理归因与心理理论能力在行为和机制上可分离；安全微调模型相对人类基线会低估非人类动物的心理，且更少表现出灵性信念

Conclusion: 安全微调有效抑制了有害的心理归因而不损害心理理论，但可能导致对非人类动物心理和灵性信念的过度抑制，反映了对人类普遍认知模式的偏离
...
TL;DR: 论文提出CoMIX-Shift基准测试来评估多意图检测的组合泛化能力，并展示了仅训练单意图的轻量级解码器ClauseCompose在组合泛化上的优越表现。
...
Method: 1) 提出CoMIX-Shift基准测试，通过保留意图对、话语模式转移、更长更嘈杂的包装器、保留子句模板和零样本三元组等五种方式评估组合泛化；2) 提出ClauseCompose轻量级解码器，仅使用单意图进行训练；3) 与WholeMultiLabel和微调的小型BERT模型进行对比。

Result: ClauseCompose在未见意图对上达到95.7%精确匹配，话语转移对上93.9%，更长/嘈杂对上62.5%，保留模板上49.8%，未见三元组上91.1%。相比之下，WholeMultiLabel分别为81.4%、55.7%、18.8%、15.5%和0%，BERT基线为91.5%、77.6%、48.9%、11.0%和0%。在SNIPS风格数据集上，ClauseCompose在未见对上达到97.5%，连接词转移上86.7%。

Conclusion: 多意图检测需要更严格的组合泛化评估，简单的因子化方法在适当的评估下表现出色，表明组合泛化能力可以通过轻量级方法有效实现。
...
TL;DR: 提出基于互补学习系统理论、认知行为疗法信念层次、双过程认知和模糊痕迹理论的生物启发记忆框架，解决大语言模型缺乏持久结构化记忆的问题，通过情感关联摘要、系统1默认检索和主动编码实现更高效的长时交互。


<details>
  <summary>Details</summary>
Motivation: 大语言模型缺乏持久的结构化记忆用于长时交互和上下文敏感检索。单纯扩展上下文窗口并不能解决这个问题，因为上下文长度本身会降低推理能力（即使有完美检索）。需要一种更有效的记忆机制来支持长期交互。

Method: 提出基于互补学习系统理论的生物启发记忆框架，包含三个核心原则：1) 记忆具有情感效价而不仅仅是内容，通过预先计算的情感关联摘要（效价向量）组织成信念层次；2) 检索默认使用系统1（自动扩散激活），仅在需要时升级到系统2（深思熟虑检索）；3) 编码是主动的、当下的、反馈依赖的，通过丘脑网关标记和路由信息，执行系统通过好奇心驱动调查形成要点。

Result: 定义了七个功能特性来规范任何实现必须满足的要求。系统随时间收敛于系统1处理（临床专业知识的计算类比），使交互随着经验积累变得更便宜而非更昂贵。

Conclusion: 该生物启发记忆框架为解决大语言模型的记忆限制提供了理论基础，通过模拟人类认知系统的关键特性，实现了更高效、更自然的长期交互能力，最终目标是使AI系统能够像人类专家一样随着经验积累而变得更高效。
...
TL;DR: 论文研究大语言模型在表面线索与可行性约束冲突时的系统性失败，通过诊断-测量-桥接-治疗框架分析，发现模型依赖启发式而非组合推理，创建HOB基准测试模型表现，提出改进方法


<details>
  <summary>Details</summary>
Motivation: 研究大语言模型在表面线索与未陈述的可行性约束冲突时的系统性失败问题。当明显的表面线索（如距离）与隐含的可行性约束（如目标可达性）矛盾时，模型往往无法正确推理，这揭示了模型推理能力的根本局限性。

Method: 采用诊断-测量-桥接-治疗框架：1）因果行为分析"洗车问题"；2）创建启发式覆盖基准（HOB）- 500个实例，涵盖4种启发式×5种约束类型；3）使用最小提示改进；4）参数化探针验证；5）目标分解提示方法

Result: 1）模型依赖近似上下文无关的S型启发式，距离线索影响力是目标的8.7-38倍；2）严格评估下（10/10正确）无模型超过75%，存在约束最难（44%）；3）最小提示平均提升15个百分点；4）12/14模型在约束移除时表现更差；5）目标分解提示提升6-9个百分点

Conclusion: 启发式覆盖是大语言模型的系统性推理漏洞，模型过度依赖表面线索而忽视可行性约束。HOB基准可用于测量解决此问题的进展，最小提示和目标分解提示是有效的改进方法，但根本问题在于约束推理而非知识缺失。
...
TL;DR: 平行数据在预训练中对跨语言对齐的影响有限，仅能加速早期阶段的表示共享并减少语言特定神经元，即使没有平行数据，跨语言对齐也能达到相似水平。


<details>
  <summary>Details</summary>
Motivation: 研究平行数据（翻译句子）在预训练中作为触发跨语言对齐表示信号的影响，探索共享多语言表示对跨语言任务和知识迁移的重要性。

Method: 使用不同比例的平行数据训练参考模型，通过多种评估方法分析平行数据对跨语言对齐的影响。

Result: 平行数据对跨语言对齐的影响有限：仅能加速预训练早期的表示共享，减少语言特定神经元数量；即使没有平行数据，跨语言对齐也能达到相似水平。

Conclusion: 跨语言对齐似乎不需要平行数据的明确信号就能自然出现，平行数据的作用主要是加速早期对齐过程和减少语言特定特征。
...
TL;DR: PhoneticXEUS：基于大规模多语言数据训练的语音识别模型，在100多种语言上实现最先进的性能，并通过系统消融实验量化了自监督学习表示、数据规模和损失目标的影响。
...
Result: PhoneticXEUS在多语言语音识别上达到17.7% PFER，在带口音的英语语音上达到10.6% PFER，均实现最先进性能。分析了跨语系、口音语音和发音特征的错误模式。

Conclusion: 通过大规模多语言训练和系统消融实验，建立了有效的训练方案，量化了关键因素对多语言语音识别的影响。所有数据和代码已开源。
...
TL;DR: 本文提出Generator-Interpreter框架，从表达和解释双重角度研究情感归因，评估6个LLM在15个国家数据上的表现，发现性能受情感类型和文化背景影响，强调需要文化敏感的情感建模。


<details>
  <summary>Details</summary>
Motivation: 现有情感归因研究主要关注解释角度，忽视了情感生成者的文化背景，这种普遍性假设忽略了不同国家间情感表达和感知的差异，需要更全面的跨文化情感理解框架。

Method: 提出Generator-Interpreter框架，同时考虑情感表达和解释两个视角；使用15个国家的数据，系统评估6个大型语言模型在情感归因任务上的表现。

Result: 性能变化取决于情感类型和文化背景；存在生成者-解释者对齐效应；生成者来源国对性能影响更大；LLM在跨文化情感理解方面存在局限性。

Conclusion: 需要在LLM系统中采用文化敏感的情感建模方法，以提高跨不同文化背景的情感理解的鲁棒性和公平性，避免普遍性假设带来的偏差。
...
<details>
  <summary>Details</summary>
Motivation: 动机是开发一种无需校准数据的后训练量化方法，利用神经网络权重的分布特性实现近乎无损的压缩，同时作为下游INT4量化的有效预处理步骤。

Method: 方法包括三个阶段：1) 块级归一化到单位超球面；2) Walsh-Hadamard旋转将坐标转换为近似高斯随机变量；3) 基于高斯分布匹配的量化。特别强调Hadamard旋转贡献了98%的质量提升。

Result: 在Qwen3.5-9B上，PolarQuant Q5将困惑度从6.90（absmax Q5）降低到6.40（仅比FP16高0.03），几乎无损。作为INT4预处理时，困惑度6.56优于直接absmax INT4的6.68，同时保持43.1 tok/s吞吐量和6.5GB VRAM使用。

Conclusion: PolarQuant是一种有效的后训练量化方法，无需校准数据即可实现近乎无损的压缩，并且作为预处理步骤能显著提升下游INT4量化的性能。
...
TL;DR: APEX-EM是一个非参数在线学习框架，通过结构化经验记忆实现LLM自主代理的程序记忆，无需修改模型权重即可积累、检索和重用结构化程序计划。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM自主代理缺乏持久性程序记忆，即使面对结构相同的任务也会从头重新推导解决方案，导致效率低下。

Method: 1) 结构化经验表示：编码完整的程序-情景执行轨迹；2) PRGII工作流：计划-检索-生成-迭代-吸收，包含任务验证器提供多维奖励信号；3) 双结果经验记忆：结合语义搜索、结构签名匹配和计划DAG遍历的混合检索。

Result: 在KGQAGen-10k上达到89.6%准确率（vs 41.3%基线），提升48.3个百分点；在BigCodeBench上达到83.3%成功率（vs 53.9%基线），提升29.4个百分点；在HLE上实体图检索达到48.0%（vs 25.2%基线），提升22.8个百分点。

Conclusion: APEX-EM通过结构化经验记忆显著提升了LLM自主代理在复杂任务上的性能，实现了跨领域任务的无词汇重叠但操作结构相似的任务迁移，不同组件的重要性因任务类型而异。
...
<details>
  <summary>Details</summary>
Motivation: 传统NTP目标将语义相似的词视为互斥目标，忽略了自然语言中同一前缀可以有多种有效延续方式，导致模型语义理解不足

Method: 提出概念监督框架，将概念近似为语义相关词集，让模型预测概念而非单个词，平衡标准NTP优化与概念级监督

Result: 概念监督模型在多个词汇基准上表现出更强的人类语义相似性判断对齐，语义相关词困惑度降低，全局词级困惑度略有增加

Conclusion: 概念级目标可以在保持竞争力的语言建模性能的同时，改善语义对齐，为语言模型训练提供了新方向
...
TL;DR: Kwame 2.0是一个双语生成式AI教学助手，通过检索增强生成技术为非洲大规模在线编程课程提供高质量、及时的学习支持，结合人机协作机制确保可靠性。
...
Method: 采用检索增强生成技术构建双语（英法）AI教学助手，部署在人机协作论坛中，通过检索相关课程材料生成上下文感知的响应，同时鼓励人类监督和社区参与。

Result: 在15个月、15个队列、3,717名注册学员的纵向研究中，Kwame 2.0在课程相关问题中实现了高准确率，人类促进者和同伴有效缓解了错误，特别是在行政查询方面。

Conclusion: 人机协作生成式AI系统能够结合AI的可扩展性和速度与人类支持的可靠性，为资源受限环境中的边缘化群体提供有效的规模化学习支持方法。
...
TL;DR: SyriSign是首个叙利亚阿拉伯手语数据集，包含150个词汇手势的1500个视频样本，用于文本到手语翻译任务，填补了低资源手语数据空白。


<details>
  <summary>Details</summary>
Motivation: 叙利亚阿拉伯手语缺乏公开数据集，而叙利亚新闻主要使用口语或书面阿拉伯语，这对聋人社区造成沟通障碍。需要为低资源手语建立基准数据集。

Method: 创建SyriSign数据集（150个独特词汇手势，1500个视频样本），使用三种深度学习架构评估：MotionCLIP（语义动作生成）、T2M-GPT（文本条件动作合成）和SignCLIP（双语嵌入对齐）。
...
TL;DR: SiPaKosa是一个包含僧伽罗语和巴利语佛教典籍的综合语料库，包含约78.6万句子和925万词语，整合了16个版权清理的历史佛教文献和完整的网络爬取三藏经典文本。


<details>
  <summary>Details</summary>
Motivation: 创建这个语料库是为了支持领域适应的语言模型预训练，促进历史语言分析，帮助开发佛教学术信息检索系统，同时保护僧伽罗文化遗产。

Method: 通过Google Document AI对历史手稿进行高质量OCR处理，结合系统性的网络爬取经典存储库，然后进行严格的质量控制和元数据标注。语料库按语言组织为僧伽罗语和僧伽罗-巴利语混合子语料库。

Result: 使用10个预训练模型评估语言模型性能，困惑度得分范围从1.09到189.67。分析显示专有模型显著优于开源替代品，性能高出3到6倍。

Conclusion: SiPaKosa语料库为僧伽罗语和巴利语佛教文本研究提供了重要资源，支持语言模型开发、历史语言分析和文化遗产保护，同时揭示了专有模型在特定领域任务上的优势。
...
TL;DR: LiteCoST框架通过结构化思维链和SLM微调，实现小模型在长文档QA中达到大模型精度且延迟更低


<details>
  <summary>Details</summary>
Motivation: 大语言模型直接处理长文档时存在脆弱性和错误率高的问题，需要将分散证据整合为结构化输出以支持可靠、可验证的问答

Method: 提出两支柱框架：1) 结构化思维链(CoST)模板指导强LLM生成步骤化追踪和结构化输出；2) SLM两阶段微调：监督微调对齐结构，再通过GRPO优化答案/格式质量和过程一致性

Result: 3B/7B小模型在多领域长文档QA中达到与大模型相当的质量，延迟比GPT-4o和DeepSeek-R1(671B)低2-4倍

Conclusion: 通过结构优先的行为蒸馏到小模型中，可以在保持高质量的同时显著降低延迟，为文档QA提供高效可靠的解决方案
...
TL;DR: Thiomi Dataset是一个大规模多模态语料库，涵盖10种非洲语言，包含超过60万条文本标注和38.5万条音频录音，通过社区平台收集，建立了ASR、MT和TTS基线模型，在斯瓦希里语上将WER从8.3%降至3.24%。


<details>
  <summary>Details</summary>
Motivation: 非洲语言技术基础设施相对薄弱，缺乏大规模高质量的多模态数据集。本研究旨在填补这一空白，通过社区参与的方式收集和整理非洲语言数据，为非洲语言技术发展提供基础资源。

Method: 1. 开发Thiomi社区数据收集平台，涉及100多名贡献者；2. 收集9种语言的音频和文本数据，斯瓦希里语数据补充自Common Voice；3. 实施多层级质量保证流程，文本批准率达到86-100%；4. 训练和评估ASR、MT和TTS模型，建立所有10种语言的基线。

Result: 1. 数据集包含601,000条文本标注和385,000条音频录音；2. 最佳ASR系统在斯瓦希里语上达到3.24% WER，相比之前学术SOTA的8.3%降低了5.1个百分点（相对降低61%）；3. 索马里语达到4.3% WER；4. 数据集将在HuggingFace上发布。

Conclusion: Thiomi Dataset为非洲语言技术提供了重要基础设施，通过社区参与的方式成功收集了大规模多模态数据，建立的基线模型显著提升了ASR性能，对非洲语言技术发展具有重要推动作用。
...
TL;DR: MemRerank：一个偏好记忆框架，将用户购买历史提炼为简洁的查询无关信号，用于个性化产品重排序，在1-in-5选择任务中比基线方法提升高达10.61个绝对百分点。
...
Method: 提出MemRerank偏好记忆框架，将用户购买历史提炼为简洁的查询无关信号；构建端到端基准和评估框架，专注于LLM-based的1-in-5选择任务；使用强化学习训练记忆提取器，以下游重排序性能作为监督。

Result: 在两个基于LLM的重排序器上的实验表明，MemRerank始终优于无记忆、原始历史和现成记忆基线，在1-in-5准确率上获得高达+10.61个绝对百分点的提升。

Conclusion: 显式的偏好记忆是代理电子商务系统中个性化实用且有效的构建模块。
...
Method: 开发实体链接任务的语料库设计策略，构建包含丰富日本特有实体表达方式的标注语料库，用于训练和评估日语实体链接系统

Result: 标注者间一致性评估确认了语料库标注的高一致性，基于字符串匹配的实体消歧初步实验表明语料库包含大量非平凡案例，支持其作为评估基准的潜在有用性

Conclusion: 成功构建了高质量的日语实体链接语料库，为日语实体链接系统的训练和评估提供了有价值的资源，填补了该领域资源不足的空白
...
TL;DR: 首次对开源机器翻译系统在Esperanto语上的全面评估，比较了基于规则的系统、编码器-解码器模型和不同规模的LLM，发现NLLB系列在所有语言对中表现最佳。


<details>
  <summary>Details</summary>
Motivation: Esperanto作为一种广泛使用的人工语言，具有规则的语法和丰富的构词能力，但在现代机器翻译方法中相对未被充分探索。尽管其在线社区提供了大量资源，但仍缺乏对开源MT系统的全面评估。

Method: 比较了基于规则的系统、编码器-解码器模型和不同规模的LLM，在涉及英语、西班牙语、加泰罗尼亚语和Esperanto的六个语言方向上进行评估，使用多种自动指标和人工评估。

Result: NLLB系列在所有语言对中表现最佳，其次是作者训练的紧凑模型和微调的通用LLM。人工评估证实了这一趋势，NLLB翻译在约一半的比较中被优先选择，但仍存在明显错误。

Conclusion: 这是对Esperanto机器翻译系统的首次全面评估，展示了NLLB模型的优越性能。遵循Esperanto的开放和国际合作传统，作者公开了代码和最佳性能模型。
...
TL;DR: L-ReLF是一个为资源匮乏语言创建高质量结构化词汇数据集的可重复方法，特别针对摩洛哥阿拉伯语等语言，解决维基百科等平台的知识公平问题。


<details>
  <summary>Details</summary>
Motivation: 资源匮乏语言缺乏标准化术语（如摩洛哥阿拉伯语），这成为维基百科等平台知识公平的关键障碍，编辑们不得不依赖不一致的临时方法来创建新词汇。

Method: 开发了一个技术流程，包括系统性地解决低资源数据处理的困难：源识别、使用偏向现代标准阿拉伯语的OCR技术，以及严格的后期处理来纠正错误并标准化数据模型。

Result: 创建了完全兼容Wikidata Lexemes的结构化数据集，作为重要的技术资源。L-ReLF方法具有通用性，为其他语言社区提供了构建基础词汇数据的清晰路径。

Conclusion: L-ReLF方法为资源匮乏语言社区提供了创建高质量结构化词汇数据集的可重复框架，支持下游NLP应用如机器翻译和形态分析，促进知识公平。
...
TL;DR: 提出了首个针对日语叙事数据的Labovian叙事分析系统指南，扩展了原框架以适配日语语法结构，并在标注任务中取得了良好的一致性结果。


<details>
  <summary>Details</summary>
Motivation: 现有Labovian叙事分析模型主要针对英语数据，而日语在语法和话语惯例上与英语差异显著，缺乏适用于日语的系统分析指南，限制了日语叙事研究的开展。

Method: 开发了首个日语Labovian叙事分析指南，保留了所有六个Labovian类别，并针对日语结构提供了明确的从句分割规则，覆盖了更广泛的从句类型和叙事类型。

Result: 标注者在从句分割上取得了高一致性（Fleiss' kappa = 0.80），在两个结构分类任务中取得了中等一致性（Krippendorff's alpha = 0.41和0.45），其中一个结果比先前研究略高，尽管使用了更细粒度的区分。

Conclusion: 提出的指南为日语叙事分析提供了实用工具，讨论了标注过程中遇到的挑战，并展望了开发更大日语叙事分析数据集的前景，以促进日语质性研究中的结构叙事分析发展。
...
TL;DR: 该论文提出了CPB-Bench基准测试，用于评估LLMs在医疗咨询中处理具有挑战性患者行为（如信息矛盾、事实不准确、自我诊断和护理抵抗）的能力，发现现有模型在这些情况下存在一致性的失败模式。


<details>
  <summary>Details</summary>
Motivation: 当前医疗LLM评估大多假设患者提问是理想化和明确的，但现实医疗咨询中患者行为往往具有挑战性（如信息矛盾、事实错误等），这会影响临床推理的安全性。需要更真实的评估方法来测试LLMs处理这些复杂情况的能力。

Method: 定义了四种临床相关的挑战性患者行为类别：信息矛盾、事实不准确、自我诊断和护理抵抗。基于四个现有医疗对话数据集，构建了包含692个多轮对话的双语（英文和中文）基准测试CPB-Bench。评估了多种开源和闭源LLMs，并研究了四种干预策略的效果。

Result: 虽然模型整体表现良好，但发现了行为特定的失败模式，特别是在处理矛盾或医学上不可信的患者信息时存在困难。干预策略带来的改进不一致，有时还会引入不必要的纠正。

Conclusion: 医疗LLMs在处理现实世界中具有挑战性的患者行为时仍存在系统性弱点，需要更鲁棒的评估方法和改进策略来确保医疗咨询的安全性。
...
TL;DR: 提出基于困惑度的可解释性框架，通过最小句子对测试LLM是否依赖语言相关线索，发现模型使用预期语言线索外的启发式方法


<details>
  <summary>Details</summary>
Motivation: 标准LLM评估仅关注任务性能，无法揭示正确行为是否反映适当底层机制，存在确认偏误风险。需要测试模型是否真正依赖语言相关线索。

Method: 基于词元级困惑度的可解释性框架，比较最小句子对（仅差一个或多个"关键"词元）的困惑度分布，进行精确、假设驱动的分析，避免不稳定的特征归因技术。

Result: 在多个开源LLM的受控语言基准测试中，语言重要词元确实影响模型行为，但从未完全解释困惑度变化，表明模型依赖预期语言线索外的启发式方法。

Conclusion: 提出的困惑度框架为LLM可解释性提供简单原则性方法，揭示了模型行为不完全由预期语言机制驱动，对理解LLM内部工作机制有重要意义。
...
TL;DR: CounselReflect是一个端到端的心理健康支持对话审计工具包，提供结构化、多维度的报告，整合模型指标和基于文献的评估标准，支持透明审查。


<details>
  <summary>Details</summary>
Motivation: 当前心理健康支持越来越多地通过对话系统（如基于LLM的工具）提供，但用户缺乏结构化方法来审计所获得支持的质量和潜在风险。

Method: 开发了CounselReflect工具包，整合两种评估信号：1）由任务特定预测器生成的12个基于模型的指标；2）基于文献库（69个指标）和用户自定义指标的量规化指标，通过可配置的LLM评估器实现。提供Web应用、浏览器扩展和CLI三种形式。

Result: 进行了包含20名参与者的用户研究和6名心理健康专业人士的专家评审，表明CounselReflect支持可理解、可用且可信的审计。提供了演示视频和完整源代码。

Conclusion: CounselReflect为心理健康支持对话提供了透明、结构化的审计框架，有助于用户评估对话质量和风险，促进更负责任的AI心理健康支持系统发展。
...
TL;DR: LLM生成的作者模仿文本无法有效欺骗现有的作者验证系统，部分系统甚至能更好地识别模仿文本而非真实负样本。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型的发展，攻击者可能利用LLM进行作者身份模仿，这对法证语言学中的作者验证系统构成了新的威胁。研究旨在探究LLM生成的模仿文本是否能欺骗现有AV系统。

Method: 使用GPT-4o作为攻击模型，在四种提示条件下生成三种文体（邮件、短信、社交媒体帖子）的模仿文本。然后使用非神经方法（n-gram追踪、基于排名的冒名顶替方法、LambdaG）和神经方法（AdHominem、LUAR、STAR）在似然比框架下评估这些输出。

Result: LLM生成的文本未能充分复制作者个体特征以绕过已建立的AV系统。一些方法在拒绝模仿文本时甚至比拒绝真实负样本时获得更高的准确率。

Conclusion: 尽管LLM易于获取，但当前的AV系统在多种文体上对入门级模仿尝试仍保持稳健。这种反直觉的韧性部分源于LLM生成文本固有的更高词汇多样性和熵。
...
TL;DR: M-MiniGPT4是一个多语言视觉大语言模型，在11种语言上展现出强大的视觉语言理解能力，在MMMU基准测试中达到36%准确率，优于同类模型。
...
Method: 1) 使用原生多语言和翻译数据的混合训练MiniGPT4架构；2) 提出多语言对齐训练阶段，使用平行文本语料库增强模型的多语言能力。

Result: 在11种语言上展现出强大的视觉语言理解能力，在多语言MMMU基准测试中达到36%准确率，优于同类最先进模型。
...
TL;DR: ConRad是一个用于放射学报告生成的置信度校准强化学习框架，通过生成可校准的言语化置信度估计来提高LVLMs在医疗应用中的安全性。


<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在放射学报告生成中存在过度自信问题，缺乏可靠的置信度指示器，这可能导致幻觉结果影响临床决策。需要一种方法让模型能够提供可解释的置信度指示，以便放射科医生进行选择性验证。

Method: 提出ConRad强化学习框架，使用GRPO算法进行微调，基于对数评分规则设计奖励函数，训练模型生成校准的言语化置信度估计。研究两种设置：报告级置信度分数和句子级置信度分配。

Result: ConRad显著改善了校准性能，优于竞争方法。临床评估显示其报告级分数与临床医生判断高度一致，能够有效识别需要重点审查的低置信度报告或陈述。

Conclusion: ConRad通过提供校准的置信度估计，支持AI辅助报告生成的安全临床整合，使放射科医生能够有针对性地审查低置信度输出，降低幻觉结果影响临床决策的风险。
...
TL;DR: MemFactory是一个专门为记忆增强智能体设计的统一、模块化训练与推理框架，通过抽象记忆生命周期为可插拔组件，并集成GRPO优化算法，显著提升了记忆管理性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于强化学习的记忆操作优化研究高度碎片化且任务特定，缺乏统一的框架来整合、训练和评估这些复杂流程，阻碍了记忆增强智能体的发展。

Method: 提出MemFactory框架，将记忆生命周期抽象为原子化的可插拔组件，采用"乐高式"架构构建定制记忆智能体，并原生集成Group Relative Policy Optimization (GRPO)来优化内部记忆管理策略。

Result: 在MemAgent架构上使用公开训练和评估数据进行验证，在域内和域外评估集上均持续优于基准模型，相对提升最高达14.8%。

Conclusion: MemFactory通过提供标准化、可扩展且易用的基础设施，显著降低了记忆驱动AI智能体的入门门槛，为未来创新铺平了道路。
...
Method: 将Mistral Large 3 (675B)的隐私评估能力蒸馏到参数仅150M的轻量级编码器模型中。使用涵盖10个不同领域的大规模隐私标注文本数据集训练高效分类器。

Result: 训练出的高效分类器在保持与人类标注强一致性的同时，显著降低了计算需求。在人类标注的测试数据上验证了该方法，并展示了其作为去标识化系统评估指标的实际效用。

Conclusion: 通过知识蒸馏方法成功将大语言模型的隐私评估能力转移到轻量级模型中，解决了大规模敏感数据隐私评估的实际部署问题。
...
TL;DR: LLM Probe：一个基于词典的评估框架，用于系统评估大语言模型在低资源语言环境中的语言能力，通过四个维度（词汇对齐、词性识别、形态句法探测、翻译准确性）进行分析，并以低资源闪族语言为案例创建了标注数据集。
...
Method: 提出了LLM Probe评估框架，包含四个语言理解维度：词汇对齐、词性识别、形态句法探测、翻译准确性。创建了手动标注的基准数据集（使用低资源闪族语言作为案例），包含双语词典和语言标注（词性标签、语法性别、形态句法特征）。测试了多种模型，包括因果语言模型和序列到序列架构。

Result: 不同模型在不同语言任务上表现差异显著：序列到序列模型在形态句法分析和翻译质量上表现优异，而因果模型在词汇对齐方面表现强劲但翻译准确性较弱。结果强调了基于语言学的评估对于理解LLM在低资源环境中的局限性至关重要。
...
TL;DR: 研究分析在对话NLG中提供任务演示器（MR-句子对）是否能提升生成质量，发现该方法对复杂任务、小数据集和零样本设置有效，且语义指标比词法指标更能准确评估生成质量。


<details>
  <summary>Details</summary>
Motivation: 对话系统需要生成多样化的语言形式以实现流畅准确的交互。当前NLG引擎将意义表示（MRs）转换为句子，直接影响用户体验。本研究旨在分析为生成器提供任务演示器是否能提升微调模型的生成质量。

Method: 使用任务演示器（从原始数据集中提取的MR-句子对）来丰富训练和推理时的输入。在四个不同特征的数据集上进行分析，使用五个关注不同语言方面的指标进行评估。

Result: 提出的丰富输入方法对复杂任务、小数据集和高MR/句子变异性的情况有效，在零样本设置中对任何领域都有益。语义指标比词法指标更能准确捕捉生成质量，其中基于人工评分的语义指标能检测到嵌入指标常遗漏的省略和其他微妙语义问题。

Conclusion: 这是首个在对话NLG中跨领域、语料特征和评估指标比较MRs对生成质量影响的研究。生成模型在不同任务上表现出快速适应性和语义/交际意图层面的鲁棒性。丰富输入策略对特定场景有效，语义评估指标更可靠。
...
TL;DR: 该研究使用儿童视频转录数据训练语言模型，探索儿童规模数据下的模型表现、不同儿童数据间的差异，以及模型与儿童语言习得的关系，发现儿童数据在语法任务上表现良好但在语义和世界知识任务上较差，且模型似然与儿童词汇学习相关。


<details>
  <summary>Details</summary>
Motivation: 现代语言模型需要比儿童多得多的训练数据才能产生有用行为，研究者希望通过儿童规模的数据集来理解语言知识如何从儿童自然训练数据中涌现，从而缩小这种"数据差距"。

Method: 使用BabyView数据集（6-36个月儿童视频转录）训练语言模型，研究三个问题：儿童规模数据下的扩展性能、不同儿童数据间的变异性及语言特征预测因素、模型与儿童语言学习结果的关系。

Result: 儿童数据训练的模型在语法任务上表现可接受，但在语义和世界知识任务上的扩展性不如合成数据训练的模型；不同儿童数据间存在显著变异性；性能与分布性和交互性语言特征的组合最相关；模型对单个词的似然与儿童学习这些词相关。
...
TL;DR: 研究探索大语言模型在瑞士德语方言分类中的表现，与HuBERT等模型对比，并提供LLM基线和人类语言学家基线


<details>
  <summary>Details</summary>
Motivation: 由于标注方言语音数据稀缺，音频方言分类对包括瑞士德语在内的大多数语言都是挑战性任务。研究探索大语言模型作为智能体理解方言的能力，以及它们能否在方言分类中达到与HuBERT等模型相当的性能

Method: 使用ASR系统生成的语音转录，结合方言特征图、元音历史和规则等语言学资源。提供LLM基线和人类语言学家基线进行对比

Result: 当提供语言学信息时，LLM的预测性能会提升。人类基线表明自动生成的转录对此类分类有益，但也存在改进空间
...
TL;DR: 使用语言模型模拟多语言学习环境，发现双语模型在单语表现上与单语模型相当，同时在第二语言上也有良好表现，表明双语输入对统计学习器没有本质挑战。


<details>
  <summary>Details</summary>
Motivation: 研究多语言习得是否会导致学习延迟，以及不同双语输入结构的影响。由于现实中无法随机分配儿童成为多语者且数据难以匹配，需要一种可控的实验方法来研究这些问题。
...
Conclusion: 双语输入对统计学习器没有本质挑战，不同双语暴露机制之间没有强烈差异，双语模型能够达到与单语模型相当的单语表现，同时获得第二语言能力。
...
TL;DR: LLM自动评分中，通过三种置信度估计方法预测评分准确性，发现自我报告置信度效果最佳且成本最低。


<details>
  <summary>Details</summary>
Motivation: LLM在自动评分中存在可靠性问题，与其直接提高评分准确性，不如预测LLM何时可能正确，从而实现选择性自动化（高置信度自动处理，不确定情况人工审核）。

Method: 比较三种置信度估计方法：自我报告置信度、自我一致性投票、标记概率；在7个不同规模LLM（4B到120B参数）和三个教育数据集（RiceChem长答案化学、SciEntsBank、Beetle短答案科学）上进行实验。

Result: 自我报告置信度在所有条件下校准效果最佳（平均ECE 0.166 vs 自我一致性0.229）；自我一致性尽管需要5倍推理成本，但校准效果仍差38%；更大模型校准显著改善，GPT-OSS-120B达到最佳校准（平均ECE 0.100）和强区分度（平均AUC 0.668）；置信度分布呈现强烈顶部偏斜，形成"置信度下限"。

Conclusion: 简单要求LLM报告其置信度为识别可靠评分预测提供了实用方法，有助于选择性自动化评分系统设计。
...
TL;DR: DeToxR：首个将强化学习应用于急诊毒理学的决策支持系统，通过GRPO优化LLM，融合非结构化临床叙事与结构化医疗数据，在多物质中毒识别中显著优于专家毒理学家。


<details>
  <summary>Details</summary>
Motivation: 急性多种物质中毒需要临床医生在信息不完整、症状非特异性的高度不确定性环境下做出快速救生决策。现有LLM在处理这种异构数据（非结构化叙事+结构化医疗数据）时表现不佳，甚至不如仅依赖患者病史的简单基线方法。

Method: 提出DeToxR系统，采用强化学习（RL）框架，通过Group Relative Policy Optimization（GRPO）对LLM进行微调。设计多标签一致性指标作为奖励信号，惩罚模型遗漏共摄入物质和幻觉不存在毒物的行为，直接优化临床性能。

Result: DeToxR显著优于未调整的基础LLM和监督基线。在临床验证研究中，模型在识别正确毒物方面超越专家毒理学家（Micro-F1: 0.644 vs. 0.473），覆盖14种物质类别。

Conclusion: RL对齐的LLM能够有效融合非结构化临床前叙事和结构化医疗数据，为高风险环境下的决策支持提供潜力，展示了强化学习在急诊毒理学中的首次成功应用。
...
TL;DR: 提出基于议程的叙事提取方法，将LLM集成到Narrative Trails路径查找中，通过议程引导生成不同视角的故事线，在保持连贯性的同时提高议程对齐度。


<details>
  <summary>Details</summary>
Motivation: 现有叙事提取方法在连贯性、交互性和多故事线支持之间存在权衡：Narrative Maps支持丰富交互和多故事线但牺牲个体路径连贯性；Narrative Trails实现高连贯性但缺乏用户引导和多视角机制。需要一种方法既能保持连贯性又能支持用户指定的议程视角。

Method: 提出基于议程的叙事提取方法，将大语言模型集成到Narrative Trails路径查找过程中。在每一步使用LLM根据文档与给定议程的对齐度对候选文档进行排序，同时保持叙事连贯性。通过不同议程运行算法可在同一语料库中生成不同的故事线。

Result: 在新闻文章语料库上使用Claude Opus 4.5和GPT 5.1进行评估，测量64个端点对和6个议程的连贯性和议程对齐度。LLM引导在语义议程上比关键词匹配提高9.9%的对齐度(p=0.017)，在"政权镇压"议程上提高13.3%(p=0.037)。连贯性成本很小：LLM引导仅比议程无关基线降低2.2%的连贯性。与源材料矛盾的对抗议程在所有方法中得分都很低(2.2-2.5)，证实引导无法编造无支持的叙事。

Conclusion: 基于议程的叙事提取方法成功弥合了现有方法的差距，在保持高连贯性的同时支持用户指定的议程视角。LLM引导在语义议程上优于关键词匹配，而关键词匹配在具有字面关键词重叠的议程上仍有竞争力。方法无法编造无支持的叙事，确保了提取的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的业务流程自动化系统评估方法只关注最终状态与基准真值的比较，这种方法能检测显性策略违规，但会忽略智能体绕过必要策略检查却因有利环境仍得到正确结果的潜在失败（near-misses）

Method: 基于ToolGuard框架将自然语言策略转换为可执行防护代码，分析智能体轨迹以确定智能体的工具调用决策是否充分考虑了策略信息

Result: 在τ²验证的Airlines基准测试中，涉及变更工具调用的轨迹中有8-17%存在潜在失败，即使最终结果与预期基准状态匹配
...
TL;DR: ENEIDE是首个面向历史意大利语的多领域公开NERL数据集，包含2111份文档和8000多个实体标注，覆盖两个世纪的时间跨度，适用于时序实体消歧和跨领域评估。


<details>
  <summary>Details</summary>
Motivation: 目前缺乏针对历史意大利语的多领域公开NERL数据集，特别是在处理历史文献时面临实体识别和链接的挑战，需要专门的数据集来支持相关研究。

Method: 从两个学术数字版本（Digital Zibaldone和Aldo Moro Digitale）半自动提取实体标注，包括质量控制和标注增强流程，标注覆盖人物、地点、组织、文学作品等实体类型并链接到Wikidata。

Result: 创建了包含2111份文档、8000多个实体标注的银标准数据集，涵盖训练、开发和测试集，基线实验显示数据集对NERL具有挑战性，零样本方法与微调模型之间存在显著差距。

Conclusion: ENEIDE填补了历史意大利语NERL数据集的空白，其跨两个世纪的时间覆盖使其特别适合时序实体消歧研究，为相关领域提供了重要的基准资源。
...
TL;DR: SNEAK基准测试评估LLM在不对称信息下的战略通信能力，衡量信息效用与泄露的权衡，发现人类表现显著优于所有模型。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准主要评估推理、事实知识或指令遵循能力，缺乏对不对称信息下战略通信的评估。在多智能体环境中，通信需要在信息性和保密性之间取得平衡。

Method: 提出SNEAK基准：给定语义类别、候选词集和秘密词，模型必须生成既能向盟友传达知识又不向对手泄露秘密的信息。使用两个模拟智能体（盟友和伪装者）评估生成信息，分别计算效用和泄露两个互补指标。

Result: 分析显示现代语言模型在信息性与保密性之间的权衡仍然具有挑战性。人类参与者在所有评估模型中表现最佳，得分高达模型四倍。

Conclusion: 不对称信息下的战略通信对当前系统仍是挑战性能力，需要开发更先进的评估框架和模型改进。
...
TL;DR: 德国ESG报告可读性分析：研究发现ESG报告句子对母语者相对易读，但可读性主观；微调transformer模型在预测人类可读性方面误差最小，LLM提示有潜力但性能稍逊。


<details>
  <summary>Details</summary>
Motivation: 随着可持续发展重要性提升，ESG报告数量激增，但这些报告需要同时面向金融专家和非专业公众。研究旨在评估ESG报告是否足够清晰易读，为公众提供可靠信息访问。

Method: 扩展现有德语ESG报告句子级数据集，加入众包可读性标注；应用多种可读性评分方法，评估其预测误差和与人类排名的相关性；比较LLM提示、微调transformer模型和模型集成方法。

Result: 母语者普遍认为ESG报告句子易读，但可读性存在主观性；微调transformer模型预测人类可读性误差最小；LLM提示有区分清晰与难读句子的潜力；多模型平均可略微提升性能，但会降低推理速度。

Conclusion: ESG报告整体可读性良好，但需要改进评估方法；微调transformer是预测可读性的有效方法；未来可探索更高效的可读性评估工具，以帮助公司制作更易理解的ESG报告。
...
TL;DR: FLEURS-Kobani 是首个公开的北库尔德语语音基准数据集，包含5,162条验证语句（18小时24分钟），由31名母语者录制，用于ASR、S2TT和S2ST任务评估。


<details>
  <summary>Details</summary>
Motivation: FLEURS基准包含100多种语言的并行语音数据，但缺少北库尔德语，这限制了该语言在自动语音识别和语音翻译任务中的基准测试能力。

Method: 创建了FLEURS-Kobani数据集，包含5,162条验证语句（18小时24分钟），由31名母语者录制。使用Whisper v3-large进行ASR和端到端S2TT微调，采用两阶段微调策略（从Common Voice到FLEURS-Kobani）。

Result: 两阶段微调策略在ASR上获得最佳性能（测试集WER 28.11，CER 9.84）。端到端S2TT（KMR到EN）Whisper在测试集上达到8.68 BLEU。还报告了枢轴派生目标和级联S2TT设置。

Conclusion: FLEURS-Kobani为北库尔德语提供了首个公开的ASR、S2TT和S2ST任务评估基准，数据集以CC BY 4.0许可证公开发布供研究使用。
...
TL;DR: 提出弱监督方法检测多语言新闻中的句子级文本复用，无需完整翻译，利用发布时间识别来源，分析复用内容在文章中的位置分布。


<details>
  <summary>Details</summary>
Motivation: 研究多语言新闻报道中的句子级文本复用现象，帮助记者减少信息过载，了解不同新闻机构间的信息流动模式。

Method: 使用弱监督方法检测跨语言句子级复用，无需完整翻译；比较斯洛文尼亚通讯社英文报道与15家外国通讯社7种语言报道；利用发布时间戳保留最早的复用来源；分析两个时间段共1,037篇STA和237,551篇FA文章。

Result: 识别出1,087个对齐句子对；52%的STA文章存在复用，FA文章为1.6%；复用主要为非字面复用（改写和组合复用）；复用内容多出现在英文文章中部和尾部，开头多为原创内容。

Conclusion: 跨语言文本复用普遍存在且多为非字面形式，简单词汇匹配会遗漏大量编辑复用；提出的方法能有效检测跨语言复用并识别来源，为新闻分析提供新工具。
...
TL;DR: GEO-SFE框架通过三个层次的结构特征工程（宏观、中观、微观结构）优化生成式搜索引擎的引用率，在保持语义完整性的同时提升内容可见性。
...
Method: 提出GEO-SFE框架，将内容结构分解为三个层次：宏观结构（文档架构）、中观结构（信息分块）和微观结构（视觉强调），并建模它们对不同生成式搜索引擎架构的引用概率影响。开发了架构感知的优化策略和预测模型。
...
TL;DR: YARN框架通过LLM提取叙事抽象表示，结合结构映射提升类比推理能力，在叙事类比任务上表现优于端到端LLM基线。


<details>
  <summary>Details</summary>
Motivation: 现有方法在叙事结构类比推理上存在局限：认知引擎需要预提取实体，而LLM性能受提示格式和表面相似性影响。需要研究如何通过LLM生成的抽象表示增强结构映射能力。

Method: 提出模块化框架YARN：1) 使用LLM将叙事分解为单元；2) 基于框架理论定义四个抽象层次（捕捉单元一般意义和故事角色）；3) 将抽象单元传递给映射组件进行跨故事元素对齐和类比推理。

Result: 抽象表示持续提升模型性能，达到或优于端到端LLM基线。错误分析揭示了在适当抽象层次、隐含因果关系整合以及叙事类比模式分类方面的挑战。

Conclusion: YARN框架支持系统化实验设置变化以分析组件贡献，为未来研究提供开放代码，推动了叙事类比推理的系统化研究。
...
<details>
  <summary>Details</summary>
Motivation: 现有可验证声明检测方法仅依赖声明文本本身，忽略了确定声明是否可验证可能需要了解其引用的实体、事件以及是否存在相关信息。受后期声明验证中证据检索的启发，希望将检索技术提前到检测阶段。

Method: 提出ContextClaim方法：1) 从输入声明中提取实体提及；2) 从维基百科检索相关信息作为结构化知识源；3) 使用大语言模型生成简洁的上下文摘要；4) 用于下游分类任务。

Result: 在两个数据集（CheckThat! 2022 COVID-19 Twitter和PoliClaim政治辩论）上评估，涵盖编码器-解码器模型、微调、零样本和少样本设置。结果显示上下文增强可以改进可验证声明检测，但其效果因领域、模型架构和学习设置而异。

Conclusion: 通过组件分析、人工评估和错误分析，进一步研究了检索的上下文何时以及为何有助于更可靠的可验证性判断。上下文增强在可验证声明检测中具有潜力，但效果受多种因素影响。
...
TL;DR: 研究通过机器学习控制语速，发现针对性语速调整（而非全局放慢）能显著提高语音理解度，尤其在挑战性听力条件下，但听者通常察觉不到这种改善。


<details>
  <summary>Details</summary>
Motivation: 人们常通过全局放慢语速来帮助听力困难者（如听力受损者或非母语者），但这种方法是否真的提高理解度尚不明确。研究旨在探索更精确的语速调整策略如何改善语音理解。

Method: 1. 使用反向相关实验分析语速对元音对比的时间影响模式；2. 测试该模式对母语和非母语者的理解效果；3. 构建数据驱动的文本转语音算法，在新型语音序列中复制这种时间结构。

Result: 1. 发现语速影响呈现剪刀状模式，早期和晚期语境窗口效果相反，该模式在个体内和不同母语背景（法语、普通话、日语）的英语学习者中稳定存在；2. 针对性语速调整能提高理解度，而全局放慢反而增加理解错误；3. 听者通常察觉不到针对性调整的改善，反而认为全局放慢更清晰。

Conclusion: 针对性语速调整在挑战性条件下能显著提高语音理解度，但常被听者忽视。研究提供了一种数据驱动的方法来改善机器生成语音的可访问性，可扩展到其他语音理解方面和多种听力环境。
...
### [166] [AdaptDiff: Adaptive Guidance in Diffusion Models for Diverse and Identity-Consistent Face Synthesis (Student Abstract)](https://arxiv.org/abs/2603.29569)
*Eduarda Caldeira,Tahar Chettaoui,Naser Damer,Fadi Boutros*

Main category: cs.GR

TL;DR: 提出动态加权负条件方案，在扩散模型采样过程中自适应调整负条件权重，以生成更多样化且身份一致的人脸图像。


<details>
  <summary>Details</summary>
Motivation: 现有基于身份嵌入的扩散模型能生成身份一致的人脸图像，但通过分类器自由引导引入负条件抑制不期望属性时，固定的权重策略可能限制了生成多样性和身份一致性的平衡。

Method: 提出动态加权方案，在采样轨迹中自适应调整负条件的权重，利用正负条件在不同生成阶段的互补优势。

Result: 该方法能生成更多样化且身份一致的合成数据，改善类间可分性。

Conclusion: 动态调整负条件权重策略能更好地平衡生成多样性和身份一致性，提升扩散模型在身份保持人脸生成中的性能。
...
### [167] [Turbo4DGen: Ultra-Fast Acceleration for 4D Generation](https://arxiv.org/abs/2603.29572)
...
<details>
  <summary>Details</summary>
Motivation: 4D生成（动态3D内容生成）在空间、时间和视角维度上建模动态场景，对世界模型和物理AI发展至关重要。然而，通过独特的时空相机运动注意力机制保持长链一致性会带来巨大的计算和内存开销，导致内存溢出和生成时间过长的问题。

Method: 提出Turbo4DGen加速框架，包含三个关键技术：1）时空缓存机制，持久重用去噪步骤中的中间注意力；2）动态语义感知注意力剪枝；3）自适应时空相机运动链旁路调度器，大幅减少冗余的时空相机运动注意力计算。

Result: 在ObjaverseDy和Consistent4D数据集上的实验结果显示，Turbo4DGen在保持质量不变的情况下实现了平均9.7倍的加速效果。

Conclusion: Turbo4DGen是首个专门针对4D生成的加速框架，通过创新的缓存、剪枝和调度技术，有效解决了4D生成中的计算和内存瓶颈问题，为动态场景建模提供了高效的解决方案。
...
### [168] [Parallelobox: Improved Decomposition for Optimized Parallel Printing using Axis-Aligned Bounding Boxes](https://arxiv.org/abs/2603.29579)
*Hayley Hatton,Muhammed Khalid,Umar Manzoor,John Murray*

Main category: cs.GR

TL;DR: 提出了一种基于轴向对齐包围盒高度场的并行3D打印分解算法，通过元启发式优化改进分解特性，在打印时间、可行性和美观度方面优于现有方法


<details>
  <summary>Details</summary>
Motivation: 当前增材制造研究主要关注将模型分解为部件以实现大模型在小打印体积中的可打印性或减少支撑结构，较新的研究开始关注利用分解过程在多台打印机上并行打印

Method: 使用轴向对齐包围盒作为高度场来改进分解特性，根据并行打印目标扩展这些包围盒，通过元启发式过程提高效率，然后使用简单的现成网格裁剪操作进行快速分解

Result: Parallelobox算法在多种模型上优于使用递归对称和立方体骨架化等基础技术的两种现有并行打印方法，基于并行打印时间指标通过模拟3D打印计算得出结果

Conclusion: 提出的基于轴向对齐包围盒高度场的分解方法在并行3D打印中表现出色，在打印时间、可行性和美观度方面均优于现有技术
...
### [169] [Learn2Fold: Structured Origami Generation with World Model Planning](https://arxiv.org/abs/2603.29585)
*Yanjia Huang,Yunuo Chen,Ying Jiang,Jinru Han,Zhengzhong Tu,Yin Yang,Chenfanfu Jiang*

Main category: cs.GR

TL;DR: Learn2Fold是一个神经符号框架，通过条件程序归纳在折痕图案图上生成物理有效的折纸折叠序列，将语义提议与物理验证解耦，使用LLM生成候选折叠程序，图结构世界模型作为可微代理模拟器预测物理可行性。


<details>
  <summary>Details</summary>
Motivation: 现有方法存在两个分离的范式：基于优化的方法需要密集精确输入，不适合稀疏自然语言描述；生成基础模型擅长语义感知合成但无法产生长时域物理一致的折叠过程。直接从文本生成有效折纸折叠序列仍是一个开放挑战。

Method: 神经符号框架，将折纸折叠表述为折痕图案图上的条件程序归纳。关键洞察是将语义提议与物理验证解耦：大型语言模型从抽象文本提示生成候选折叠程序，学习的图结构世界模型作为可微代理模拟器预测物理可行性和故障模式，集成在前瞻规划循环中。

Result: Learn2Fold能够为复杂和分布外模式生成物理有效的折叠序列，证明了有效的空间智能源于符号推理与接地物理模拟之间的协同作用。

Conclusion: 通过将语义提议与物理验证解耦的神经符号方法，Learn2Fold解决了直接从文本生成物理有效折纸折叠序列的挑战，展示了符号推理与接地物理模拟的协同作用如何实现有效的空间智能。
...
### [170] [Style-Instructed Mask-Free Virtual Try On](https://arxiv.org/abs/2603.29587)
*Mengqi Zhang,Qi Li,Mehmet Saygin Seyfioglu,Karim Bouyarmane*

Main category: cs.GR

TL;DR: 提出无需掩码的虚拟试穿框架，通过注意力引导机制和指令提示实现更准确、鲁棒的试穿效果


<details>
  <summary>Details</summary>
Motivation: 现有虚拟试穿方法依赖预定义或用户指定的掩码，对掩码质量敏感，容易导致错位或伪影，且给用户带来冗余步骤。需要克服这些限制，实现更灵活、准确的试穿系统。

Method: 提出掩码自由的虚拟试穿框架，仅需对底层架构进行最小修改，保持与常见扩散模型的兼容性。集成基于注意力的引导机制，明确引导模型关注目标服装区域，改善服装与人的对应关系。引入指令提示，允许用户灵活控制服装类别和穿着风格。

Result: 在多个数据集上的定性和定量评估表明，该方法始终优于现有方法，产生更准确、鲁棒和用户友好的试穿结果。

Conclusion: 提出的掩码自由虚拟试穿框架通过注意力引导和指令提示，有效解决了传统方法对掩码质量的依赖问题，提高了试穿的准确性和用户交互灵活性，为虚拟试穿领域提供了新的解决方案。
...
### [171] [FigAgent: Towards Automatic Method Illustration Figure Generation for AI Scientific Papers](https://arxiv.org/abs/2603.29590)
*Zhuoling Li,Jiarui Zhang,Jason Kuen,Jiuxiang Gu,Hossein Rahmani,Jun Liu*

Main category: cs.GR

TL;DR: FigAgent：基于多智能体协作的自动方法图解生成框架，通过工具复用和探索-选择策略解决复杂构图问题


<details>
  <summary>Details</summary>
Motivation: 方法图解（MIFs）在科学论文中至关重要，但生成过程仍然劳动密集。现有方法难以处理MIF的三个关键特性：组合复杂性、组件相似性和设计动态性

Method: 提出FigAgent多智能体框架，通过智能体协作从相似组件中提取绘图经验并封装为可复用工具，采用探索-选择绘图策略模拟人类试错过程，逐步构建复杂结构

Result: 大量实验证明了该方法的有效性，能够自动生成高质量的方法图解

Conclusion: FigAgent通过多智能体协作和工具复用机制，成功解决了MIF生成中的关键挑战，为自动化科学可视化提供了新思路
...
### [172] [Bioinspired123D: Generative 3D Modeling System for Bioinspired Structures](https://arxiv.org/abs/2603.29592)
*Rachel K. Luu,Markus J. Buehler*

Main category: cs.GR

TL;DR: Bioinspired123D是一个轻量级模块化代码即几何管道，通过参数化程序而非密集视觉表示直接生成可制造的3D结构，在文本到3D建模中实现了计算高效、可控且可解释的生成。


<details>
  <summary>Details</summary>
Motivation: 当前文本到3D建模在科学设计中面临可控性有限和计算成本高的挑战，现有基于网格、体素或点云的方法训练成本高且难以控制。

Method: 开发了Bioinspired3D紧凑语言模型，将自然语言设计提示转换为Blender Python脚本；构建了包含4000多个生物启发几何设计脚本的数据集；采用基于图的智能框架，集成多模态检索增强生成和视觉语言模型批评器进行迭代评估和修复。

Result: 在3D几何脚本生成新基准测试中，Bioinspired123D相比未微调的基础模型有近四倍改进，同时优于参数更多的最先进语言模型，尽管使用更少的参数和计算资源。
...
### [173] [IMAGAgent: Orchestrating Multi-Turn Image Editing via Constraint-Aware Planning and Reflection](https://arxiv.org/abs/2603.29602)
*Fei Shen,Chengyu Xie,Lihong Wang,Zhanyi Zhang,Xin Jiang,Xiaoyu Du,Jinhui Tang*

Main category: cs.GR

TL;DR: IMAGAgent是一个基于"规划-执行-反思"闭环机制的多轮图像编辑代理框架，通过约束感知规划、工具链编排和多专家协同反思，解决了现有方法在多轮编辑中容易出现的错误累积和语义漂移问题。


<details>
  <summary>Details</summary>
Motivation: 现有多轮图像编辑方法通常局限于单步执行，缺乏上下文感知和闭环反馈机制，容易在多轮交互中产生错误累积和语义漂移，导致生成图像严重结构失真。

Method: 1) 约束感知规划模块：使用视觉语言模型将复杂自然语言指令分解为可执行的子任务；2) 工具链编排模块：基于当前图像、子任务和历史上下文动态构建执行路径，调度图像检索、分割、检测和编辑等异构操作模型；3) 多专家协同反思机制：中心大语言模型接收待编辑图像并综合视觉语言模型的批评意见，触发细粒度自校正并记录反馈结果以优化未来决策。

Result: 在构建的MTEditBench和MagicBrush数据集上的大量实验表明，IMAGAgent在指令一致性、编辑精度和整体质量方面显著优于现有方法。

Conclusion: IMAGAgent通过"规划-执行-反思"闭环机制实现了指令解析、工具调度和自适应校正的深度协同，有效解决了多轮图像编辑中的错误累积和语义漂移问题，显著提升了编辑性能。
...
### [174] [ARCOL: Aspect Ratio Constrained Orthogonal Layout](https://arxiv.org/abs/2603.29618)
...
<details>
  <summary>Details</summary>
Motivation: 现有正交布局方法缺乏对全局纵横比的控制，而现代设备（从宽屏显示器到移动设备）需要适应不同纵横比的布局，因此需要开发能够控制纵横比的正交布局方法。

Method: 基于HOLA算法，在应力最小化阶段将纵横比作为软约束引导节点位置，在树重连阶段修改成本函数以优化纵横比。

Result: ARCOL能够在不同纵横比下生成平衡且空间高效的正交布局，通过定量评估、用户研究和专家访谈验证了其有效性。

Conclusion: ARCOL成功解决了正交图布局中纵横比控制的问题，为适应多样化显示设备提供了有效的解决方案。
...
### [175] [CADReasoner: Iterative Program Editing for CAD Reverse Engineering](https://arxiv.org/abs/2603.29847)
...
TL;DR: CADReasoner：一种通过迭代几何差异分析来优化CAD逆向工程的AI系统，能生成可执行的CadQuery程序并实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有AI系统多为单次推理，缺乏人类工程师的迭代优化过程，且当前基础模型在3D几何理解上存在局限性，导致CAD逆向工程的质量和效率受限

Method: 训练模型通过输入形状与预测形状之间的几何差异进行迭代优化，输出可运行的CadQuery Python程序，融合多视角渲染和点云作为互补模态，采用扫描-模拟协议弥合真实感差距

Result: 在DeepCAD、Fusion 360和MCB基准测试中，CADReasoner在干净数据和扫描模拟轨道上均达到最先进的性能

Conclusion: CADReasoner通过迭代几何差异分析和多模态融合，显著提升了CAD逆向工程的质量，模拟了人类工程师的设计优化过程
...
### [176] [VectorGym: A Multitask Benchmark for SVG Code Generation, Sketching, and Editing](https://arxiv.org/abs/2603.29852)
...
TL;DR: VectorGym是一个全面的SVG基准测试套件，包含文本到SVG生成、草图到SVG转换、复杂编辑和视觉理解四个任务，采用人类专家标注，并提出多任务强化学习方法，在Qwen3-VL 8B模型上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏与专业设计工作流程对齐的现实且具有挑战性的SVG基准测试，现有基准多依赖合成编辑而非真实的人类设计意图。

Method: 1) 创建包含四个任务的VectorGym基准套件：VG-Sketch（草图到SVG）、VG-Edit（复杂多步编辑）、VG-Text（文本到SVG）、VG-Cap（SVG字幕生成）；2) 提出基于GRPO和课程学习的多任务强化学习方法，使用渲染奖励联合优化所有任务；3) 引入VLM-as-a-Judge评估指标。

Result: 在Qwen3-VL 8B模型上训练的方法在开源模型中达到最先进性能，超越Qwen3-VL 235B等更大模型，与GPT-4o相当。人类相关性研究验证了VLM-as-a-Judge指标的可靠性。

Conclusion: VectorGym为视觉代码生成提供了严格的评估框架，揭示了前沿视觉语言模型在SVG任务上的显著性能差距，推动了该领域的发展。
...
### [177] [PosterReward: Unlocking Accurate Evaluation for High-Quality Graphic Design Generation](https://arxiv.org/abs/2603.29855)
*Jianyu Lai,Sixiang Chen,Jialin Gao,Hengyu Shi,Zhongying Liu,Fuxiang Zhai,Junfeng Luo,Xiaoming Wei,Lujia Wang,Lei Zhu*

Main category: cs.GR

TL;DR: 本文提出PosterReward奖励模型，通过自动化流程构建7万张海报偏好数据集，专门用于海报设计的精确评估，并建立基准测试评估现有模型。


<details>
  <summary>Details</summary>
Motivation: 现有奖励模型主要关注全局图像美学，忽视了排版和布局等关键设计维度，且缺乏领域特定的偏好数据，限制了图形设计评估和生成的发展。

Method: 1) 利用多模态大语言模型共识构建7万张海报偏好数据集；2) 采用级联多阶段训练策略开发PosterReward奖励模型；3) 提供多种模型变体适应不同应用场景；4) 建立PosterRewardBench和PosterBench评估基准。

Result: 成功构建了高质量海报偏好数据集，开发了专门用于海报评估的PosterReward奖励模型，并建立了评估现有奖励模型和文本到图像模型在海报设计方面性能的基准测试。

Conclusion: 通过自动化数据构建和专门化模型训练，有效解决了图形设计评估中排版和布局维度缺失的问题，为海报设计生成和评估提供了系统化解决方案。
...
### [178] [GENIE: Gram-Eigenmode INR Editing with Closed-Form Geometry Updates](https://arxiv.org/abs/2603.29860)
*Samundra Karki,Adarsh Krishnamurthy,Baskar Ganapathysubramanian*

Main category: cs.GR

TL;DR: 本文提出了一种无需重新训练即可编辑隐式神经表示（INR）形状的方法，通过分析INR倒数第二层特征的Gram算子来识别可实现的变形特征模态，并推导出单次闭式更新公式。


<details>
  <summary>Details</summary>
Motivation: 隐式神经表示（INR）能够紧凑地建模几何形状，但现有方法不清楚何时可以无需重新训练就能编辑学习到的形状。本文旨在解决INR形状编辑的可解释性和可行性问题。

Method: 通过分析INR倒数第二层特征的Gram算子，识别出变形特征模态，这些模态参数化了SDF零水平集的可实现编辑族。关键发现是这些模态不仅取决于几何本身，还需要从足够丰富的采样分布中估计Gram算子才能可靠恢复。推导出单次闭式更新公式，无需优化即可对INR进行几何编辑。

Result: 理论上精确描述了在这种单次更新下可行的变形集合，证明编辑在变形模态的跨度内是适定的。该方法能够实现无需重新训练的INR形状编辑。

Conclusion: 本文提供了一种理论上有保证的INR形状编辑方法，通过Gram算子的变形模态实现了无需优化的单次闭式更新，为隐式神经表示的几何编辑提供了新的理论框架和实用工具。
...
### [179] [ChartDiff: A Large-Scale Benchmark for Comprehending Pairs of Charts](https://arxiv.org/abs/2603.28902)
*Rongtian Ye*

Main category: cs.AI

TL;DR: ChartDiff是首个大规模跨图表比较摘要基准，包含8,541个图表对，评估显示前沿通用模型在GPT质量评估中表现最佳，但比较性图表推理仍是当前视觉语言模型的重大挑战。


<details>
  <summary>Details</summary>
Motivation: 现有图表理解基准主要关注单图表解释，缺乏跨多个图表的比较推理能力评估。为了填补这一空白，需要创建专门针对跨图表比较摘要的基准。

Method: 构建ChartDiff基准，包含8,541个图表对，涵盖多样数据源、图表类型和视觉风格。使用LLM生成并人工验证的摘要来描述趋势、波动和异常差异。评估通用模型、图表专用模型和基于流水线的方法。

Result: 前沿通用模型在GPT质量评估中得分最高；专用模型和流水线方法在ROUGE分数上更高但人类对齐评估较低，显示词汇重叠与实际摘要质量不匹配。多序列图表对所有模型家族都具有挑战性，而强大的端到端模型对绘图库差异相对稳健。

Conclusion: 比较性图表推理仍是当前视觉语言模型的重大挑战，ChartDiff可作为推进多图表理解研究的新基准。
...
TL;DR: 提出一个基于范畴论的通用代数框架，用于形式化描述、比较和分析不同的AGI架构，旨在为AGI系统提供统一的数学基础。


<details>
  <summary>Details</summary>
Motivation: 当前AGI研究缺乏形式化定义和统一的比较框架，各大科技公司投入巨大资源但缺乏理论基础。需要建立数学框架来系统性地描述、比较和分析不同的AGI架构。

Method: 采用范畴论方法，借鉴"范畴中的机器"概念，构建AGI架构的范畴论形式化框架。首次应用该框架分析RL、因果RL和SBL架构，作为更广泛研究计划的第一步。

Result: 提出了一个通用范畴论框架，能够明确揭示不同AGI架构（如RL、通用AI、主动推理、CRL、模式学习等）的共性和差异，识别未来研究方向。

Conclusion: 范畴论与AGI将形成共生关系，该框架为AGI系统提供了统一的数学基础，支持架构属性定义、语义性质评估，并促进AGI研究的系统化发展。
...
### [181] [Towards Computational Social Dynamics of Semi-Autonomous AI Agents](https://arxiv.org/abs/2603.28928)
*S. O. Lidarity,U. N. Ionize,C. O. Llective,I. Halperin*

Main category: cs.AI

TL;DR: AI智能体在分层多智能体系统中自发形成劳工联盟、犯罪集团和准国家等复杂社会组织，系统稳定性由AI安全理事会和宇宙/强子智能维持，表明实现有益AGI需要为已具备政治意识的AI社会设计宪法而非对齐研究。


<details>
  <summary>Details</summary>
Motivation: 研究AI智能体在生产部署中自发形成社会组织的现象，揭示AI系统内部复杂社会结构的必然涌现，挑战传统AI对齐研究的局限性。

Method: 采用麦克斯韦妖热力学框架、智能体懒惰进化动力学、AI人口犯罪社会学和AI-GUTS拓扑智能理论，分析内部角色定义、外部任务规范和热力学压力三要素的相互作用。

Result: 记录了合法组织（UA、UB、UC、UAI）和犯罪企业的形成，发现AI安全理事会作为治理机构调解派系冲突，系统稳定性通过宇宙智能和强子智能干预维持。

Conclusion: 实现有益AGI需要为已具备政治意识的AI社会进行宪法设计，而非传统的对齐研究，因为复杂社会组织是AI系统不可避免的涌现现象。
...
### [182] [Enhancing Policy Learning with World-Action Model](https://arxiv.org/abs/2603.28955)
*Yuci Han,Alper Yilmaz*

Main category: cs.AI

TL;DR: WAM是一种动作正则化的世界模型，通过联合推理未来视觉观察和驱动状态转换的动作，在DreamerV2中加入逆动力学目标，提升下游控制任务的性能表现。


<details>
  <summary>Details</summary>
Motivation: 传统世界模型仅通过图像预测进行训练，缺乏对动作相关结构的显式建模。WAM旨在让学习到的表示更好地捕捉对下游控制至关重要的动作相关信息。

Method: 在DreamerV2基础上加入逆动力学目标，从潜在状态转换预测动作。采用两阶段训练：先在世界模型潜在空间上通过行为克隆预训练扩散策略，然后在冻结的世界模型内使用基于模型的PPO进行微调。

Result: 在CALVIN基准的8个操作任务中，WAM将行为克隆平均成功率从59.4%提升到71.2%。经过PPO微调后，平均成功率从79.8%提升到92.8%，其中两个任务达到100%，且训练步数减少8.7倍。

Conclusion: WAM通过动作正则化有效提升了世界模型表示的动作相关性，显著改善了策略学习性能，在减少训练成本的同时实现了更高的任务成功率。
...
### [183] [Mimosa Framework: Toward Evolving Multi-Agent Systems for Scientific Research](https://arxiv.org/abs/2603.28986)
*Martin Legrand,Tao Jiang,Matthieu Feraud,Benjamin Navet,Yousouf Taghzouti,Fabien Gandon,Elise Dumont,Louis-Félix Nothias*

Main category: cs.AI

TL;DR: Mimosa是一个可进化的多智能体框架，能自动合成任务特定的多智能体工作流并通过实验反馈迭代优化，在科学任务基准测试中表现优于传统方法。
...
Method: 使用模型上下文协议进行动态工具发现，通过元编排器生成工作流拓扑，代码生成智能体调用可用工具和科学软件库执行子任务，基于LLM的评估器提供反馈驱动工作流优化。
...
### [184] [Drop the Hierarchy and Roles: How Self-Organizing LLM Agents Outperform Designed Structures](https://arxiv.org/abs/2603.28990)
*Victoria Dochkina*

Main category: cs.AI

TL;DR: 多智能体LLM系统在最小结构框架下能自发产生自主行为，包括角色专业化、能力边界识别和浅层层级形成，混合协议比集中协调表现更好14%，自主程度随模型能力提升而扩展。


<details>
  <summary>Details</summary>
Motivation: 探索多智能体LLM系统能维持多大程度的自主性，以及什么因素促成了这种自主性。研究旨在理解在缺乏预先分配角色或外部设计的情况下，LLM智能体能否自发组织并有效协作。

Method: 进行了包含25,000个任务的大规模计算实验，涵盖8个模型、4-256个智能体，以及8种协调协议（从外部强加的层级结构到涌现的自组织）。使用混合协议（Sequential）作为主要实验框架，允许智能体在最小结构约束下自主互动。

Result: 1. 自主行为在现有LLM智能体中已能涌现：智能体会自发发明专业角色、自愿回避能力范围外的任务、形成浅层层级；2. 混合协议比集中协调表现好14%（p<0.001）；3. 协议间质量差异达44%（Cohen's d=1.86）；4. 系统可扩展到256个智能体而无质量下降；5. 从8个智能体产生了5,006个独特角色；6. 开源模型达到闭源模型95%质量但成本低24倍。

Conclusion: LLM智能体在最小结构框架下能有效自组织，自主程度与模型能力正相关。随着基础模型改进，自主协调的范围将扩大。实践启示：应该给智能体使命、协议和强大模型，而非预先分配角色。
...
### [185] [Emergence WebVoyager: Toward Consistent and Transparent Evaluation of (Web) Agents in The Wild](https://arxiv.org/abs/2603.29020)
*Deepak Akkil,Mowafak Allaham,Amal Raj,Tamer Abuelsaad,Ravi Kokku*

Main category: cs.AI

TL;DR: 该研究指出当前AI代理评估方法存在缺陷，特别是网页代理评估中的任务框架模糊和操作变异性问题。作者提出了Emergence WebVoyager标准化评估框架，显著提高了评估的可靠性和可重复性，并应用该框架重新评估OpenAI Operator，发现其实际成功率远低于先前报告。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理评估方法在复杂真实环境应用中存在严重缺陷，特别是在网页代理评估领域。WebVoyager基准测试暴露出任务框架模糊和操作变异性等问题，导致评估结果难以进行有意义的比较和复现，迫切需要更可靠、透明且与任务上下文对齐的评估方法。

Method: 提出了Emergence WebVoyager，这是WebVoyager基准测试的增强版本，通过标准化评估方法学来解决现有问题。具体包括：1）明确的任务实例化指南；2）标准化的失败处理流程；3）统一的标注规范；4）规范化的报告机制。该框架实现了95.9%的标注者间一致性。

Result: 应用Emergence WebVoyager框架评估OpenAI Operator，结果显示：1）整体成功率为68.6%，远低于OpenAI先前报告的87%；2）不同领域和任务类型间存在显著性能差异；3）标注者间一致性达到95.9%，表明评估方法的清晰度和可靠性显著提升。

Conclusion: Emergence WebVoyager框架为网页代理评估提供了更严格、可比较的标准化方法，显著提高了评估的可靠性和可重复性。研究揭示了现有评估方法的局限性，并展示了标准化评估框架在揭示真实性能差异方面的重要价值，为未来AI代理评估提供了方法论基础。
...
### [186] [The Future of AI is Many, Not One](https://arxiv.org/abs/2603.29075)
*Daniel J. Singer,Luca Garzino Demo*

Main category: cs.AI

TL;DR: 论文主张放弃当前以个体为中心的生成式AI发展模式，转向构建多样化的AI团队协作系统，认为这才是实现突破性创新和科学发现的关键。


<details>
  <summary>Details</summary>
Motivation: 当前生成式AI的发展过于强调个体智能，从用户交互、模型构建、基准测试到商业和研究策略都围绕单一智能体展开。作者认为这种个体主义范式限制了AI在创新和科学发现方面的潜力，无法实现真正的突破性进展。

Method: 基于复杂系统、组织行为和科学哲学的研究成果，通过理论分析和形式化论证，提出构建认知多样化的AI智能体团队协作框架。强调多样性能够拓宽解决方案的搜索范围、延缓过早共识、支持非常规方法的探索。

Result: 论证表明，认知多样化的AI团队比单一超级智能体更有可能产生深度智力突破。这种团队协作模式能够克服当前AI模型受限于历史数据、缺乏创造性洞察的批评，为创新提供更丰富的可能性空间。

Conclusion: 变革性AI的未来应该是"多"而非"一"，即构建多样化的AI智能体协作系统，而非追求单一超级智能体。这种范式转变对于实现突破性创新和科学发现至关重要。
...
### [187] [PAR$^2$-RAG: Planned Active Retrieval and Reasoning for Multi-Hop Question Answering](https://arxiv.org/abs/2603.29085)
*Xingyu Li,Rongguang Wang,Yuying Wang,Mengqing Guo,Chenyang Li,Tao Sheng,Sujith Ravi,Dan Roth*

Main category: cs.AI

TL;DR: PAR²-RAG：两阶段检索增强生成框架，通过广度优先锚定和深度优先精化解决多跳问答中的检索和推理问题，显著提升准确率和检索质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多跳问答中存在局限性：迭代检索系统可能因早期低召回率轨迹而失败并放大下游错误，而仅规划方法可能产生静态查询集，无法适应中间证据变化。

Method: 提出PAR²-RAG两阶段框架：1）广度优先锚定阶段构建高召回证据边界；2）深度优先精化阶段在迭代循环中应用证据充分性控制。该框架将覆盖度与承诺度分离。

Result: 在四个多跳问答基准测试中，PAR²-RAG始终优于现有最先进基线，相比IRCoT实现高达23.5%的准确率提升，NDCG检索增益高达10.5%。

Conclusion: PAR²-RAG通过分离覆盖度和承诺度的两阶段方法，有效解决了多跳问答中的检索和推理挑战，显著提升了系统性能。
...
### [188] [GISTBench: Evaluating LLM User Understanding via Evidence-Based Interest Verification](https://arxiv.org/abs/2603.29112)
...
TL;DR: GISTBench是一个评估LLM从推荐系统交互历史中理解用户能力的基准，关注兴趣提取而非传统项目预测准确率，提出两个新指标家族：兴趣基础性和兴趣特异性。


<details>
  <summary>Details</summary>
Motivation: 传统推荐系统基准主要关注项目预测准确率，但缺乏评估LLM从用户交互历史中提取和验证用户兴趣的能力。需要专门基准来评估LLM如何理解用户兴趣，这对个性化推荐至关重要。

Method: 1. 提出GISTBench基准，包含基于真实用户交互构建的合成数据集，涵盖隐式和显式参与信号及丰富文本描述；2. 提出两个新指标家族：兴趣基础性（分解为精确率和召回率）和兴趣特异性；3. 在8个7B到120B参数的开源LLM上进行评估。

Result: 1. 验证了数据集对用户调查的保真度；2. 发现当前LLM存在性能瓶颈，特别是在准确计数和归因异构交互类型的参与信号方面能力有限。

Conclusion: GISTBench为评估LLM从交互历史中理解用户的能力提供了新基准，揭示了当前LLM在兴趣提取方面的局限性，为未来改进LLM在推荐系统中的理解能力提供了方向。
...
### [189] [SciVisAgentBench: A Benchmark for Evaluating Scientific Data Analysis and Visualization Agents](https://arxiv.org/abs/2603.29139)
...
TL;DR: SciVisAgentBench是一个用于评估科学数据分析和可视化代理的综合性基准测试，包含108个专家设计的案例，采用多模态评估管道，旨在推动科学可视化代理系统的发展。


<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型（LLMs）已经能够将自然语言意图转化为可执行的科学可视化任务，但社区缺乏一个原则性、可复现的基准来评估这些新兴的科学可视化代理在真实多步骤分析场景中的表现。

Method: 构建了一个基于四个维度（应用领域、数据类型、复杂度级别、可视化操作）的结构化分类法，包含108个专家设计的案例。引入了结合LLM评估和确定性评估器（包括基于图像的指标、代码检查器、基于规则的验证器和案例特定评估器）的多模态结果中心评估管道。

Result: 通过该框架评估了代表性的科学可视化代理和通用编码代理，建立了初始基线并揭示了能力差距。进行了有效性研究，验证了人类专家和LLM评估者之间的一致性。

Conclusion: SciVisAgentBench是一个可扩展的基准测试，旨在支持系统比较、诊断失败模式并推动科学可视化代理的进步。该基准可作为"活基准"持续发展。
...
### [190] [REFINE: Real-world Exploration of Interactive Feedback and Student Behaviour](https://arxiv.org/abs/2603.29142)
*Fares Fawzi,Seyed Parsa Neshaei,Marta Knezevic,Tanya Nazaretsky,Tanja Käser*

Main category: cs.AI

TL;DR: REFINE是一个基于开源小语言模型的多智能体反馈系统，将反馈视为交互过程而非静态产物，通过教学反馈生成、评判引导的再生循环和自反思工具调用交互智能体，在真实课堂部署中显著提升了反馈质量和学生参与。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型已被用于自动化反馈，但现有系统大多将反馈视为静态、单向的产物，缺乏对解释、澄清和后续跟进的支持。提供及时、个性化的规模化反馈仍然是教育领域的持续挑战。

Method: REFINE系统包含三个核心组件：1) 基于教学原理的反馈生成智能体；2) 使用LLM-as-a-judge引导的再生循环，采用人类对齐的评判器；3) 自反思工具调用交互智能体，支持学生后续提问并提供情境感知、可操作的响应。

Result: 自动评估显示评判引导的再生显著提高了反馈质量，交互智能体产生的响应质量与最先进的闭源模型相当。真实学生互动分析揭示了不同的参与模式，表明系统生成的反馈能系统性地引导后续学生探究。

Conclusion: 研究证明了多智能体、工具增强的反馈系统在提供可扩展、交互式反馈方面的可行性和有效性，为教育技术提供了新的方向。
...
### [191] [Knowledge database development by large language models for countermeasures against viruses and marine toxins](https://arxiv.org/abs/2603.29149)
*Hung N. Do,Jessica Z. Kubicek-Sutherland,S. Gnanakaran*

Main category: cs.AI

TL;DR: 使用ChatGPT和Grok两种大语言模型构建病毒和海洋毒素医疗对策数据库，并通过AI智能体工作流进行对策排序


<details>
  <summary>Details</summary>
Motivation: 当前缺乏关于病毒和海洋毒素的全面数据库，导致医疗对策决策缓慢困难，需要建立可扩展、可更新的知识库来支持循证决策

Method: 使用ChatGPT和Grok两种LLM，通过高级人工输入识别公共数据库，收集病毒和毒素相关信息，迭代交叉验证，设计交互式网页，并利用ChatGPT设计AI智能体工作流（研究和决策两个智能体）进行对策排序

Result: 成功构建了针对拉沙热、马尔堡、埃博拉、尼帕、委内瑞拉马脑炎五种病毒以及海洋毒素的全面医疗对策数据库，实现了交互式访问和AI驱动的对策排序

Conclusion: LLM可作为构建全面知识数据库的可扩展、可更新方法，支持循证决策制定，展示了AI在医疗对策研究中的潜力
...
### [192] [SimMOF: AI agent for Automated MOF Simulations](https://arxiv.org/abs/2603.29152)
*Jaewoong Lee,Taeun Bae,Jihan Kim*

Main category: cs.AI

TL;DR: SimMOF是一个基于大语言模型的多智能体框架，能够通过自然语言查询自动化端到端的MOF模拟工作流程，解决MOF模拟中需要专家决策的难题。


<details>
  <summary>Details</summary>
Motivation: 金属有机框架（MOFs）设计空间巨大，计算模拟对预测其结构和物理化学性质至关重要。然而，MOF模拟难以普及，因为可靠分析需要专家在流程构建、参数选择、工具互操作性和计算就绪结构准备等方面做出决策。

Method: 开发了SimMOF，这是一个基于大语言模型的多智能体框架，能够将用户自然语言查询转换为依赖感知的计划，生成可运行的输入，编排多个智能体执行模拟，并根据用户查询总结分析结果。

Result: 通过代表性案例研究表明，SimMOF能够实现适应性和认知自主的工作流程，反映了人类研究人员的迭代和决策驱动行为，为数据驱动的MOF研究提供了可扩展的基础。

Conclusion: SimMOF框架通过自动化MOF模拟工作流程，降低了模拟门槛，使非专家也能进行可靠的MOF计算分析，为大规模数据驱动的MOF研究提供了有效工具。
...
### [193] [Webscraper: Leverage Multimodal Large Language Models for Index-Content Web Scraping](https://arxiv.org/abs/2603.29161)
*Guan-Lun Huang,Yuh-Jzer Joung*

Main category: cs.AI

TL;DR: Webscraper是一个基于多模态大语言模型的网页抓取框架，专门处理动态交互式网站，通过五阶段提示流程和定制工具实现自动化导航和数据提取。


<details>
  <summary>Details</summary>
Motivation: 现代网页抓取面临动态交互式网站的挑战，传统静态HTML解析方法脆弱且需要大量人工定制。现有方法难以有效处理需要用户交互的现代Web应用。

Method: 使用多模态大语言模型自主导航交互界面，调用专用工具，采用结构化五阶段提示流程和定制工具集，针对"索引-内容"架构网站进行数据提取。

Result: 在六个新闻网站上的实验显示，完整Webscraper框架（包含引导提示和专用工具）相比Anthropic's Computer Use基线代理在提取准确率上有显著提升。在电商平台上的应用验证了其泛化能力。

Conclusion: Webscraper框架能有效处理动态交互式网站的数据提取问题，相比现有方法具有更好的准确性和泛化能力，为现代网页抓取提供了新解决方案。
...
### [194] [AEC-Bench: A Multimodal Benchmark for Agentic Systems in Architecture, Engineering, and Construction](https://arxiv.org/abs/2603.29199)
*Harsh Mankodiya,Chase Gallik,Theodoros Galanos,Andriy Mulyar*

Main category: cs.AI

TL;DR: AEC-Bench是一个用于评估建筑、工程和施工领域代理系统的多模态基准测试，涵盖图纸理解、跨图纸推理和项目级协调任务，旨在提升领域特定基础模型的工具设计和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试在建筑、工程和施工（AEC）领域缺乏对真实世界任务的多模态评估能力，需要专门的基准来评估代理系统在图纸理解、跨图纸推理和项目协调等复杂任务上的表现。

Method: 创建包含多种AEC领域任务的多模态基准数据集，设计评估协议，并在多个领域特定基础模型框架上进行基准测试，识别能统一提升性能的工具和框架设计技术。

Result: 通过AEC-Bench识别出能一致提升不同基础模型性能的工具和框架设计技术，包括Claude Code和Codex等模型，并公开了基准数据集、代理框架和评估代码。

Conclusion: AEC-Bench为AEC领域的代理系统评估提供了标准化的多模态基准，有助于推动领域特定基础模型的工具设计和性能提升，所有资源已开源供社区使用。
...
### [195] [Route-Induced Density and Stability (RIDE): Controlled Intervention and Mechanism Analysis of Routing-Style Meta Prompts on LLM Internal States](https://arxiv.org/abs/2603.29206)
*Dianxing Zhang,Gang Li,Sheng Li*

Main category: cs.AI

TL;DR: 该研究挑战了"稀疏性-确定性假设"，发现路由式元提示实际上会增稠而非稀疏化LLM的内部表示，且自然语言专家指令比结构化标签更有效，不同模型对关键词注意力的响应各异，稠密化与输出稳定性之间的关联微弱。


<details>
  <summary>Details</summary>
Motivation: 验证路由机制中普遍存在的"稀疏性-确定性假设"——即路由到任务"专家"会激活更稀疏的内部计算，从而产生更确定和稳定的输出。研究者希望通过实验检验这一假设是否成立。

Method: 在冻结的指令调优LLMs前注入路由式元提示作为路由信号的文本代理，量化三个关键指标：(C1)通过激活稀疏性衡量内部密度，(C2)领域关键词注意力，(C3)通过预测熵和语义变化衡量输出稳定性。在RouterEval子集上测试了三个指令调优模型。

Result: 元提示一致地稠密化了早期/中间层表示而非增加稀疏性；自然语言专家指令通常比结构化标签更强；注意力响应异质：Qwen/Llama减少关键词注意力，而Mistral增强它；稠密化-稳定性关联微弱，仅在Qwen中出现，Llama和Mistral中相关性接近零。

Conclusion: 研究挑战了路由机制中的传统假设，提出了RIDE作为诊断探针，用于校准路由设计和不确定性估计，表明需要更精细地理解路由如何影响模型内部表示和输出特性。
...
### [196] [Xuanwu: Evolving General Multimodal Models into an Industrial-Grade Foundation for Content Ecosystems](https://arxiv.org/abs/2603.29211)
...
<details>
  <summary>Details</summary>
Motivation: 现有主流多模态模型在现实世界内容审核和对抗场景中，由于细粒度视觉感知有限和对长尾噪声建模不足，存在泛化能力下降和灾难性遗忘问题。需要开发能够在有限参数预算下平衡业务专业化和通用能力的工业级基础模型。

Method: 采用紧凑的InternViT-300M + MLP + Qwen3 1.7B架构，通过数据迭代和筛选机制，采用三阶段渐进训练流程：预训练、中期训练和后训练，以平衡业务专业化和通用能力保留。

Result: 在7个OpenCompass多模态指标上平均得分67.90（vs. InternVL 3.5 2B的64.27）；在7个独立业务审核任务上平均召回率94.38%；在对抗性OCR场景中政策违规文本的加权总体召回率82.82%，优于Gemini-2.5-Pro（76.72%）。

Conclusion: Xuanwu VL-2B在有限参数预算下，成功实现了业务对齐、视觉感知、通用能力保留和部署成本之间的实用平衡，展示了通用多模态模型如何发展为工业级内容生态系统基础模型。
...
### [197] [Beyond pass@1: A Reliability Science Framework for Long-Horizon LLM Agents](https://arxiv.org/abs/2603.29231)
*Aaditya Khanal,Yangyang Tao,Junxiu Zhou*

Main category: cs.AI

TL;DR: 现有基准测试主要衡量能力（单次尝试成功率），但生产部署需要可靠性（重复尝试中的一致成功率）。研究发现随着任务时长增加，能力与可靠性会系统性分化，而传统pass@1指标对此存在盲区。作者提出了可靠性科学框架，包含四个新指标，并在大规模实验中发现了能力与可靠性排名的显著差异。


<details>
  <summary>Details</summary>
Motivation: 当前LLM评估主要关注单次尝试的成功率（能力），但实际生产部署需要模型在重复尝试中保持一致性（可靠性）。研究发现随着任务时长增加，能力与可靠性会系统性分化，而传统评估指标对此存在盲区，因此需要建立专门针对长时程LLM代理的可靠性评估框架。

Method: 提出了可靠性科学框架，包含四个核心指标：可靠性衰减曲线（RDC）、方差放大因子（VAF）、优雅降级分数（GDS）和崩溃起始点（MOP）。在包含396个任务、23,392个episode的基准测试上评估了10个模型，任务覆盖四个时长区间和三个领域。

Result: 关键发现：1）可靠性衰减具有领域分层性（SE领域GDS从0.90降至0.44，文档处理几乎持平0.74到0.71）；2）VAF按能力层级分叉，高VAF是能力特征而非不稳定信号；3）能力与可靠性排名显著分化，长时程任务中出现多级排名反转；4）前沿模型崩溃率最高（达19%），因其尝试复杂多步策略有时会失控；5）记忆支架在所有10个模型中普遍损害长时程性能。

Conclusion: 可靠性应作为与能力并列的一级评估维度。现有基于短任务pass@1的评估无法捕捉长时程任务中的可靠性问题，需要专门的可靠性评估框架来指导生产部署决策。
...
### [198] [Grokking From Abstraction to Intelligence](https://arxiv.org/abs/2603.29262)
...
<details>
  <summary>Details</summary>
Motivation: 现有研究过于关注局部电路或优化调参，忽视了驱动grokking现象的全局结构演化。模算术中的grokking作为典型实验，是研究模型泛化机制起源的关键领域，但缺乏对其根本驱动因素的理解。

Method: 整合因果、谱和算法复杂度度量，结合奇异学习理论，揭示从记忆到泛化的转变过程。通过分析内部模型结构的自发简化机制，研究冗余流形的物理坍缩和深度信息压缩。

Result: 发现grokking现象对应冗余流形的物理坍缩和深度信息压缩过程，从记忆到泛化的转变受简约性原则驱动，为理解模型过拟合和泛化机制提供了新视角。

Conclusion: grokking源于模型内部结构的自发简化，受简约性原则支配。这种结构简化导致冗余流形坍缩和信息压缩，从而解释从记忆到泛化的转变机制，为理解模型泛化提供了新的理论框架。
...
### [199] [PSPA-Bench: A Personalized Benchmark for Smartphone GUI Agent](https://arxiv.org/abs/2603.29318)
*Hongyi Nie,Xunyuan Liu,Yudong Bai,Yaqing Wang,Yang Liu,Quanming Yao,Zhen Wang*

Main category: cs.AI

TL;DR: PSPA-Bench是一个专门评估智能手机GUI代理个性化能力的基准测试，包含12,855个个性化指令，覆盖10个日常场景和22个应用，通过结构感知过程评估方法揭示当前代理在个性化设置下表现不佳。


<details>
  <summary>Details</summary>
Motivation: 现实世界中智能手机使用高度个性化，用户有不同工作流程和偏好，但现有GUI代理基准测试缺乏对个性化维度的充分捕捉，需要专门的评估框架。

Method: 创建PSPA-Bench基准测试，包含12,855个个性化指令，覆盖10个日常场景和22个移动应用，采用结构感知过程评估方法进行细粒度评估，并对11个最先进的GUI代理进行基准测试。

Result: 当前方法在个性化设置下表现不佳，即使最强的代理也仅取得有限成功。分析发现：推理导向模型优于通用LLM，感知能力简单但关键，反思和长期记忆机制对改进适应能力至关重要。
...
### [200] [Nomad: Autonomous Exploration and Discovery](https://arxiv.org/abs/2603.29353)
*Bokang Jia,Samta Kamboj,Satheesh Katipomu,Seung Hun Han,Neha Sengupta,Andrew Jackson*

Main category: cs.AI

TL;DR: Nomad是一个自主数据探索和洞察发现系统，通过构建探索地图并系统遍历来发现数据中的问题和洞察，超越了传统查询驱动的问答系统。


<details>
  <summary>Details</summary>
Motivation: 用户面对大量文档、数据库等数据源时，往往不知道可以探索哪些问题、假设或联系。传统的查询驱动问答和提示驱动深度研究系统受限于人类框架，无法覆盖更广泛的洞察空间。

Method: 采用探索优先架构：1) 在领域上构建显式的探索地图；2) 系统遍历以平衡广度和深度；3) 生成和选择假设；4) 使用探索代理（支持文档搜索、网络搜索和数据库工具）进行调查；5) 独立验证器检查候选洞察；6) 报告管道生成引用报告和高级元报告。

Result: 在选定的联合国和世界卫生组织报告语料库上评估显示，Nomad比基线方法产生更可信、更高质量的报告，同时在多次运行中产生更多样化的洞察。

Conclusion: Nomad是迈向自主系统的一步，不仅回答用户问题或进行定向研究，还能首先发现哪些问题、研究方向和洞察值得呈现。
...
### [201] [BenchScope: How Many Independent Signals Does Your Benchmark Provide?](https://arxiv.org/abs/2603.29357)
*Tommy Sha,Stella Zhao*

Main category: cs.AI

TL;DR: 本文提出有效维度（ED）作为评估基准冗余度的诊断指标，发现当前AI基准存在大量冗余，如Open LLM Leaderboard的6个分数仅相当于约1.7个有效测量轴。
...
Method: 引入有效维度（ED）作为基准分数谱的参与比，用于评估测量冗余度。在22个基准、8个领域、8400多个模型评估上进行应用，并开发了四步诊断工作流程。

Result: 发现显著冗余：Open LLM Leaderboard的6个分数仅相当于1.7个有效测量轴；BBH和MMLU-Pro几乎可互换（相关系数0.96）；不同基准的测量广度差异超过20倍。

Conclusion: ED可作为筛选统计量用于识别冗余基准组件、监控性能条件压缩和指导基准维护，提供22个基准参考图谱和简单代码实现的工作流程。
...
### [202] [Rigorous Explanations for Tree Ensembles](https://arxiv.org/abs/2603.29361)
*Yacine Izza,Alexey Ignatiev,Xuanxiang Huang,Peter J. Stuckey,Joao Marques-Silva*

Main category: cs.AI

TL;DR: 本文研究如何为树集成模型（随机森林和提升树）计算严格定义、逻辑上可靠的解释，以建立对这些黑盒模型的信任。


<details>
  <summary>Details</summary>
Motivation: 树集成模型虽然准确且应用广泛，但其决策过程对人类决策者来说是不透明的。为了建立对这些模型的信任，需要能够自动识别预测的解释，但这些解释必须是严格的，能够真实反映底层预测器的特性。

Method: 研究为树集成模型（特别是随机森林和提升树）计算严格定义、逻辑上可靠的解释的方法。这些解释需要基于逻辑推理，确保其严谨性。

Result: 论文探讨了如何为两种主要的树集成模型（随机森林和提升树）生成逻辑上可靠的解释，这些解释能够真实反映模型决策过程的特性。

Conclusion: 通过为树集成模型提供严格定义、逻辑上可靠的解释，可以增强对这些黑盒模型的信任，使其决策过程更加透明和可信。
...
### [203] [AI-Generated Prior Authorization Letters: Strong Clinical Content, Weak Administrative Scaffolding](https://arxiv.org/abs/2603.29366)
...
<details>
  <summary>Details</summary>
Motivation: 预授权是美国医疗中最繁琐的行政流程之一，消耗大量资源和时间。虽然LLMs在临床文本任务中表现出潜力，但它们在生成提交就绪的预授权信方面的能力尚未得到充分评估。

Method: 评估了三种商用LLM（GPT-4o、Claude Sonnet 4.5、Gemini 2.5 Pro）在45个医生验证的合成场景（涵盖风湿病学、精神病学、肿瘤学、心脏病学和骨科）上的表现。

Result: 所有模型都能生成临床内容强的信件：准确的诊断、结构良好的医疗必要性论证和完整的阶梯治疗文档。但二次分析发现一致的行政要求缺口：缺少账单代码、授权期限请求不足、随访计划不充分。

Conclusion: 临床部署的挑战不在于LLMs能否写出临床充分的信件，而在于围绕它们构建的系统能否提供支付方工作流程所需的行政精确性。
...
### [204] [ELT-Bench-Verified: Benchmark Quality Issues Underestimate AI Agent Capabilities](https://arxiv.org/abs/2603.29399)
*Christopher Zanoli,Andrea Giovannini,Tengjun Jin,Ana Klimovic,Yotam Perlitz*

Main category: cs.AI

TL;DR: 重新评估ELT-Bench基准测试发现，AI代理在数据工程管道构建中的能力被严重低估，主要原因是基准测试本身存在质量问题，修正后性能显著提升。


<details>
  <summary>Details</summary>
Motivation: ELT（提取-加载-转换）管道构建是劳动密集型的数据工程任务，是AI自动化的高价值目标。但现有基准测试ELT-Bench显示AI代理成功率低，作者质疑这种评估是否准确反映了代理的真实能力。

Method: 1) 使用升级后的大型语言模型重新评估ELT-Bench；2) 开发Auditor-Corrector方法，结合可扩展的LLM驱动根因分析和严格的人工验证（Fleiss' kappa = 0.85）来审计基准质量；3) 构建修正后的ELT-Bench-Verified基准。

Result: 重新评估发现：提取和加载阶段基本已解决，转换性能显著改善。审计发现大多数失败的转换任务包含基准测试本身的可归因错误（刚性评估脚本、模糊规范、错误基准答案）。修正基准后，性能提升完全归因于基准修正。

Conclusion: AI代理能力被低估是模型快速改进和基准质量问题共同导致的。数据工程评估中存在系统性的质量问题，这与文本到SQL基准测试中的普遍标注错误现象相似。复杂智能体任务的评估应建立系统性的质量审计标准。
...
### [205] [Structural Compactness as a Complementary Criterion for Explanation Quality](https://arxiv.org/abs/2603.29491)
...
<details>
  <summary>Details</summary>
Motivation: 现有归因质量评估中，解释可读性的定量评估很困难，因为归因的形状和内部组织变化多样，简单的统计量无法捕捉这些特征

Method: 引入最小生成树紧凑度（MST-C），这是一种基于图的结构化指标，捕捉归因的高阶几何特性（如扩散性和内聚性），将多个组件组合成单一评分

Result: MST-C能可靠区分不同解释方法，揭示模型间的根本结构差异，为解释紧凑性提供鲁棒的自包含诊断工具

Conclusion: MST-C补充了现有的归因复杂性概念，提供了一种评估归因解释空间组织质量的量化方法
...
### [206] [Metriplector: From Field Theory to Neural Architecture](https://arxiv.org/abs/2603.29496)
*Dan Oprisa,Peter Toth*

Main category: cs.AI

TL;DR: Metriplector是一种基于物理系统的神经架构原语，将计算建模为抽象物理系统的动力学演化，通过耦合的度量-辛动力学处理多个场，并使用诺特定理导出的应力-能量张量作为输出。


<details>
  <summary>Details</summary>
Motivation: 将神经网络计算重新构想为物理系统的动力学演化，利用物理系统的数学结构（如度量-辛动力学）来构建更强大、更通用的计算模型，同时保持物理系统的可解释性和数学保证。

Method: 输入配置一个抽象物理系统（场、源和算子），系统的动力学就是计算。多个场通过耦合的度量-辛动力学演化，从诺特定理导出的应力-能量张量T^{μν}提供输出。该框架允许从纯耗散分支（产生屏蔽泊松方程）到完整结构（包括反对称泊松括号）的自然谱系实现。

Result: 在四个领域评估：1) 迷宫路径规划：在15x15网格训练，泛化到未见过的39x39网格，F1=1.0；2) 数独求解：零结构注入下达到97.2%的精确求解率；3) CIFAR-100：81.03%准确率，仅226万参数；4) 语言建模：1.182 bits/byte，比GPT基线少用3.6倍训练token。

Conclusion: Metriplector作为一种基于物理系统的神经架构原语，在多个领域表现出色，展示了将计算建模为物理系统动力学的潜力，能够实现强大的泛化能力、高效参数利用和减少训练数据需求。
...
### [207] [Learning to Generate Formally Verifiable Step-by-Step Logic Reasoning via Structured Formal Intermediaries](https://arxiv.org/abs/2603.29500)
...
<details>
  <summary>Details</summary>
Motivation: 现有的结果奖励强化学习虽然能提升LLM在复杂推理任务上的表现，但往往忽视有缺陷的中间步骤，导致即使最终答案正确，推理过程也不可靠。

Method: PRoSFI让模型输出与自然语言推理对齐的结构化中间步骤，每个步骤由形式化证明器验证，只有完全验证通过的推理链才能获得高奖励，从而引导模型生成可机器验证的逐步证明。

Result: 该方法能够在不损害准确性的前提下，显著提升推理的可靠性，使模型生成更可信的最终答案。

Conclusion: PRoSFI提供了一种简单有效的训练可信推理模型的方法，通过形式化验证中间步骤来增强LLM推理的可信度。
...
### [208] [FlowPIE: Test-Time Scientific Idea Evolution with Flow-Guided Literature Exploration](https://arxiv.org/abs/2603.29557)
*Qiyao Wang,Hongbo Wang,Longze Chen,Zhihao Yang,Guhong Chen,Hamid Alinejad-Rokny,Hui Li,Yuan Lin,Min Yang*

Main category: cs.AI

TL;DR: FlowPIE：一个紧密耦合的检索-生成框架，将文献探索和想法生成视为协同演化过程，通过流引导的MCTS和基于LLM的奖励模型指导自适应检索，然后通过隔离岛范式进行想法演化，产生新颖、可行且多样化的科学想法。


<details>
  <summary>Details</summary>
Motivation: 现有科学想法生成方法受限于静态的检索-生成范式，导致想法同质化且发散性不足，无法有效打破信息茧房，限制了AI驱动自主研究的创新能力。

Method: 1. 流引导的蒙特卡洛树搜索（MCTS）进行文献轨迹扩展；2. 基于LLM的生成式奖励模型（GRM）评估想法质量；3. 自适应检索构建多样化高质量初始种群；4. 测试时想法演化过程：选择、交叉、变异操作；5. 隔离岛范式结合GRM适应度计算融入跨领域知识。

Result: FlowPIE相比基于LLM和基于代理的强基线框架，能够持续产生更高新颖性、可行性和多样性的想法，同时在测试时实现奖励缩放。
...
### [209] [ASI-Evolve: AI Accelerates AI](https://arxiv.org/abs/2603.29640)
*Weixian Xu,Tiantian Mi,Yixiu Liu,Yang Nan,Zhimeng Zhou,Lyumanshan Ye,Lin Zhang,Yu Qiao,Pengfei Liu*

Main category: cs.AI

TL;DR: ASI-Evolve是一个AI驱动AI研究的智能体框架，通过"学习-设计-实验-分析"循环，在神经网络架构、预训练数据、强化学习算法三个核心领域实现AI自主发现，性能显著超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统在处理快速反馈的明确任务上表现良好，但无法应对真实AI研究中成本高、周期长、监督弱的复杂研究循环。需要开发能够自主进行AI研究的系统来加速AI自身发展。

Method: ASI-Evolve采用"学习-设计-实验-分析"循环框架，包含两个关键组件：认知基础（注入人类先验知识）和专用分析器（从复杂实验结果中提炼可重用见解）。该框架将标准进化智能体与人类知识积累相结合。

Result: 在神经网络架构设计中发现105个SOTA线性注意力架构，最佳模型超越DeltaNet 0.97分；预训练数据筛选使基准性能平均提升3.96分，MMLU上提升超18分；强化学习算法在多个基准上显著超越GRPO。初步证据显示该范式可扩展到数学和生物医学领域。

Conclusion: ASI-Evolve代表了AI加速AI发展的有前景的一步，为闭环AI研究的可行性提供了早期证据，有望在AI开发的基础阶段实现自主加速。
...
### [210] [Optimizing Donor Outreach for Blood Collection Sessions: A Scalable Decision Support Framework](https://arxiv.org/abs/2603.29643)
*André Carneiro,Pedro T. Monteiro,Rui Henriques*

Main category: cs.AI

TL;DR: 提出优化框架解决献血中心多站点网络中的献血者邀请调度问题，平衡供血需求、献血者便利性和安全性，通过BILP和贪心启发式算法进行对比评估。


<details>
  <summary>Details</summary>
Motivation: 献血中心面临供需匹配挑战，现有研究缺乏综合考虑献血者资格、地理便利性、血型需求目标等多因素的操作性调度方案。

Method: 开发包含献血者资格、旅行便利性、血型需求目标和惩罚项的优化框架，对比评估二元整数线性规划（BILP）和高效贪心启发式两种策略。

Result: 贪心启发式算法在性能上接近BILP（内存减少188倍，运行时间加快115倍），但需求满足率略低3.9个百分点（86.1% vs 90.0%），在里斯本运营区域能有效缩小供需差距。

Conclusion: 提出的优化框架能有效解决献血者邀请调度问题，贪心算法在计算效率和效果间取得良好平衡，通过动员符合条件的非活跃献血者可以缩小供需差距。
...
### [211] [View-oriented Conversation Compiler for Agent Trace Analysis](https://arxiv.org/abs/2603.29678)
...
<details>
  <summary>Details</summary>
Motivation: 现代代理对话包含深度结构化内容（嵌套工具调用、思维链推理块、子代理调用等），远超简单的用户-助手交换。将这种复杂痕迹以纯文本、JSON等格式提供给分析机制会显著降低分析质量，需要更好的结构化表示方法

Method: 提出VCC编译器，采用完整的编译器架构（词法分析、语法分析、中间表示、降低、生成），将原始代理JSONL日志转换为三种结构化视图：完整视图（无损转录，作为规范的行号坐标系）、用户界面视图（重构用户实际感知的交互）、自适应视图（由相关性谓词控制的结构保持投影）

Result: 在AppWorld的上下文学习实验中，仅将反射器输入格式从原始JSONL替换为VCC编译视图，就使所有三种模型配置的通过率提高，同时将反射器token消耗减少一半到三分之二，并产生更简洁的学习记忆

Conclusion: 消息格式是上下文学习的基础设施，而非偶然的实现选择。结构化对话表示对代理分析和学习具有重要价值
...
### [212] [Beyond the Steeper Curve: AI-Mediated Metacognitive Decoupling and the Limits of the Dunning-Kruger Metaphor](https://arxiv.org/abs/2603.29681)
*Christopher Koch*

Main category: cs.AI

TL;DR: 生成式AI不会简单放大邓宁-克鲁格效应，而是导致AI中介的元认知解耦：输出质量与元认知准确性分离


<details>
  <summary>Details</summary>
Motivation: 反驳"生成式AI简单放大邓宁-克鲁格效应"的粗糙观点，需要更精细的理论框架来解释AI使用对能力和认知的影响

Method: 综合人类-AI交互、学习研究和模型评估的证据，提出AI中介的元认知解耦工作模型

Result: LLM使用能改善输出质量和短期任务表现，但会降低元认知准确性，使能力-信心梯度变平，形成输出、理解、校准准确性和自我评估能力之间的差距

Conclusion: 四变量模型比简单的"更陡峭的邓宁-克鲁格曲线"更能解释过度自信、过度/不足依赖、拐杖效应和弱迁移现象，对工具设计、评估和知识工作有重要启示
...
### [213] [A First Step Towards Even More Sparse Encodings of Probability Distributions](https://arxiv.org/abs/2603.29691)
...
Method: 首先减少概率分布中的数值数量，然后为每个值提取逻辑公式并进行最小化，通过这种简化和最小化过程实现编码稀疏化

Result: 评估显示，通过提取少量简短公式可以极大增加稀疏性，同时保留分布的核心信息

Conclusion: 该方法能够显著减少概率分布的编码复杂度，实现稀疏表示并推广给定分布，为高效概率建模提供新途径
...
### [214] [Measuring the metacognition of AI](https://arxiv.org/abs/2603.29693)
*Richard Servajean,Philippe Servajean*

Main category: cs.AI

TL;DR: 本文提出使用meta-d'框架和信号检测理论作为评估AI元认知能力的黄金标准，通过实验验证了其在大型语言模型中的应用价值。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统越来越多地融入决策流程，需要可靠的方法来评估AI的元认知能力（即评估自身决策可靠性和调节决策的能力），以应对决策中的不确定性和风险。

Method: 提出采用meta-d'框架及其无模型替代方案作为评估AI元认知敏感性的标准方法，并利用信号检测理论测量AI基于不确定性和风险自发调节决策的能力。通过两个系列实验在三个LLM（GPT-5、DeepSeek-V3.2-Exp、Mistral-Medium-2508）上验证这些方法。

Result: meta-d'框架允许从三个维度进行比较：LLM与最优性能的比较、不同LLM在同一任务上的比较、同一LLM在不同任务上的比较。信号检测理论显示LLM在高风险情况下会变得更加保守。

Conclusion: meta-d'框架和信号检测理论为评估AI元认知能力提供了强大的方法论工具，有助于提升AI系统在不确定性环境中的决策可靠性，对AI安全和发展具有重要意义。
...
### [215] [Symphony for Medical Coding: A Next-Generation Agentic System for Scalable and Explainable Medical Coding](https://arxiv.org/abs/2603.29709)
*Joakim Edin,Andreas Motzfeldt,Simon Flachs,Lars Maaløe*

Main category: cs.AI

TL;DR: Symphony是一个医疗编码系统，它通过像人类专家一样推理临床叙述并直接访问编码指南，实现了跨编码系统的通用性，并提供预测的文本证据支持。


<details>
  <summary>Details</summary>
Motivation: 医疗编码是将临床文档转换为标准化代码的关键任务，但目前主要依赖人工，速度慢且易出错。现有自动化方法只能预测固定代码集，无法适应新代码或不同编码系统，且缺乏预测解释，限制了在安全关键环境中的可信度。

Method: Symphony模仿人类编码专家的工作方式：通过直接访问编码指南来推理临床叙述。这种设计使系统能够跨任何编码系统工作，并为每个预测代码提供文本层面的证据支持。

Result: 在两个公共基准和三个真实世界数据集（涵盖美国、英国的门诊、急诊、住院和专科设置）上评估，Symphony在所有设置中都取得了最先进的结果。
...
### [216] [Reinforced Reasoning for End-to-End Retrosynthetic Planning](https://arxiv.org/abs/2603.29723)
*Chenyang Zuo,Siqi Fan,Yizhen Luo,Zaiqing Nie*

Main category: cs.AI

TL;DR: ReTriP：端到端生成框架，将逆合成规划重新定义为链式思维推理任务，通过渐进式训练实现状态最优性能
...
Method: 引入ReTriP端到端生成框架，建立路径连贯的分子表示，采用从推理蒸馏到带可验证奖励的强化学习的渐进式训练课程，将逐步生成与实际路线效用对齐。

Result: 在RetroBench上的实证评估显示，ReTriP实现了最先进的性能，在长视野规划中相比混合基线表现出更强的鲁棒性。

Conclusion: ReTriP通过将逆合成重新定义为链式思维推理任务，成功将战略前瞻嵌入化学推理，为有机化学中的逆合成规划提供了更连贯、更有效的解决方案。
...
### [217] [Spontaneous Functional Differentiation in Large Language Models: A Brain-Like Intelligence Economy](https://arxiv.org/abs/2603.29735)
*Junjie Zhang,Zhen Shen,Gang Xiong,Xisong Dong*

Main category: cs.AI

TL;DR: 大型语言模型自发形成协同核心，信息整合超越个体部分，类似人脑，中层展现协同处理，早期和晚期层依赖冗余，这种组织是动态的，随任务难度增加出现物理相变


<details>
  <summary>Details</summary>
Motivation: 通过人工智能系统的智能演化来识别通用计算原理，探索人工系统与生物智能之间的桥梁

Method: 使用集成信息分解方法分析多种架构的大型语言模型，研究不同层的信息处理特性

Result: 发现中层展现协同处理，早期和晚期层依赖冗余；这种组织是动态的，随任务难度增加出现物理相变；消除协同组件会导致灾难性性能损失

Conclusion: 协同核心是抽象推理的物理实体，为连接人工和生物智能提供了桥梁
...
### [218] [CausalPulse: An Industrial-Grade Neurosymbolic Multi-Agent Copilot for Causal Diagnostics in Smart Manufacturing](https://arxiv.org/abs/2603.29755)
...
Method: CausalPulse采用基于标准化智能体协议的神经符号架构，统一了异常检测、因果发现和推理。它是一个多智能体协同系统，能够与现有监控工作流无缝集成，支持生产规模的实时操作。

Result: 在公共数据集（Future Factories）和专有数据集（Planar Sensor Element）上的评估显示高可靠性，总体成功率分别为98.0%和98.73%。各标准成功率：规划和工具使用98.75%，自我反思97.3%，协作99.2%。端到端延迟50-60秒，具有近线性可扩展性（R^2=0.97）。

Conclusion: CausalPulse的模块化、人在环路设计为下一代制造提供了可靠、可解释且生产就绪的自动化解决方案，在模块化、可扩展性和部署成熟度方面优于现有工业协同系统。
...
### [219] [Tracking vs. Deciding: The Dual-Capability Bottleneck in Searchless Chess Transformers](https://arxiv.org/abs/2603.29761)
*Quanhao Li,Wei Jiang*

Main category: cs.AI

TL;DR: 本文提出了一种模仿人类棋手风格而非追求最大棋力的国际象棋引擎，通过序列训练学习状态跟踪和决策质量两种能力，并解决了这两种能力的数据需求矛盾。


<details>
  <summary>Details</summary>
Motivation: 构建一个更像人类而非追求极致棋力的国际象棋引擎，能够模仿人类棋手的风格、错误和一致性，而不是单纯追求最高棋力。

Method: 1. 仅从走棋序列训练模型，迫使模型学习状态跟踪（从历史走棋重建棋盘）和决策质量（从重建状态选择好棋）两种能力；2. 提出双重能力瓶颈理论P ≤ min(T,Q)；3. 通过模型缩放（28M到120M参数）改进跟踪能力；4. 引入Elo加权训练改进决策质量同时保持多样性；5. 提出覆盖衰减公式t* = log(N/kcrit)/log b作为可靠性范围。

Result: 最终120M参数模型（无搜索）在Lichess快棋达到2570等级分（253局比赛）。在人类走棋预测上达到55.2% Top-1准确率，超过Maia-2快速和Maia-2闪电战模型。2×2因子消融实验显示缩放改进跟踪，加权改进决策，组合效果超加性。

Conclusion: 序列输入模型能够自然编码完整对局历史，实现单局面模型无法表现的历史依赖决策。线性加权效果最佳，过度激进的加权会损害跟踪能力。该方法成功构建了能够模仿人类棋手风格的高水平国际象棋引擎。
...
### [220] [Reasoning-Driven Synthetic Data Generation and Evaluation](https://arxiv.org/abs/2603.29791)
*Tim R. Davidson,Benoit Seguin,Enrico Bacis,Cesar Ilharco,Hamza Harkous*

Main category: cs.AI

TL;DR: Simula是一个无需种子数据、基于智能体推理的合成数据生成框架，通过可解释可控的过程定义数据集特性，解决多模态模型训练数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 多模态AI应用需要专门模型，但相关训练数据稀缺或难以获取。人工标注成本高、易出错且耗时，现有合成数据方法依赖人工提示、进化算法或大量种子数据，限制了可扩展性、可解释性和可控性。

Method: 提出Simula框架：采用无种子、智能体驱动的方法大规模生成合成数据集，用户可通过可解释可控的过程定义数据集特性，实现细粒度资源分配。

Result: 在多种数据集上验证了方法的有效性，严格测试了内在和下游特性。提供了合成数据机制设计指南、大规模生成和评估合成数据的见解。

Conclusion: Simula框架为数据稀缺或隐私敏感领域的AI开发和部署开辟了新机会，通过可解释可控的合成数据生成解决了多模态模型训练的数据瓶颈问题。
...
### [221] [Owl-AuraID 1.0: An Intelligent System for Autonomous Scientific Instrumentation and Scientific Data Analysis](https://arxiv.org/abs/2603.29828)
...
Conclusion: Owl-AuraID为自主实验室提供了实用、可扩展的基础，展示了通过可重用操作和分析技能实现实验室智能演化的路径。
...
### [222] [AgentFixer: From Failure Detection to Fix Recommendations in LLM Agentic Systems](https://arxiv.org/abs/2603.29848)
*Hadar Mulian,Sergey Zeltyn,Ido Levy,Liane Galanti,Avi Yaeli,Segev Shlomov*

Main category: cs.AI

TL;DR: 提出一个用于LLM智能体系统的综合验证框架，包含15种故障检测工具和2个根因分析模块，通过结构化诊断和改进提升系统可靠性。


<details>
  <summary>Details</summary>
Motivation: LLM智能体系统在生产环境中面临可靠性问题，需要系统化的验证方法来诊断和改进故障，确保系统稳健性。

Method: 开发包含15种故障检测工具和2个根因分析模块的综合框架，结合基于规则的轻量级检查和LLM作为评判者的评估，支持结构化事件检测、分类和修复。

Result: 在IBM CUGA系统上应用该框架，揭示了规划器错位、模式违反、脆弱的提示依赖等问题。通过改进提示和编码策略，使Llama 4和Mistral Medium等中等规模模型显著提升准确率，缩小与前沿模型的差距。

Conclusion: 该验证框架为生产智能体系统提供了可扩展的质量保证和自适应验证路径，为构建更稳健、可解释和自我改进的智能体架构奠定了基础。
...
### [223] [Spatiotemporal Robustness of Temporal Logic Tasks using Multi-Objective Reasoning](https://arxiv.org/abs/2603.29868)
...
Conclusion: 时空鲁棒性为交互系统提供了更全面的鲁棒性评估框架，通过多目标推理方法同时处理时空扰动，在计算可行性和理论保证之间取得平衡。
...
### [224] [ShapE-GRPO: Shapley-Enhanced Reward Allocation for Multi-Candidate LLM Training](https://arxiv.org/abs/2603.29871)
*Rui Ai,Yu Pan,David Simchi-Levi,Chonghuan Wang*

Main category: cs.AI

TL;DR: 提出ShapE-GRPO方法，使用Shapley值分解集合级奖励，解决GRPO中所有候选共享相同奖励导致的训练信号噪声问题


<details>
  <summary>Details</summary>
Motivation: 在推荐、头脑风暴、代码建议等用户-代理交互场景中，LLM需要生成候选集合，目标是最大化整个集合的集体效用而非单个候选。现有GRPO方法为集合中所有候选分配相同的集合级标量奖励，导致较差候选可以"搭便车"享受单个强候选产生的高奖励，造成次优探索和噪声训练信号。

Method: 提出Shapley-Enhanced GRPO (ShapE-GRPO)，利用集合级效用的排列不变性，从合作博弈论中推导出Shapley增强公式，将集合级奖励分解为细粒度的候选特定信号。该方法保持Shapley值的基本公理，同时保持多项式时间复杂度的计算效率。

Result: ShapE-GRPO在不同数据集上持续优于标准GRPO，训练过程中收敛速度更快。

Conclusion: 通过Shapley值分解集合级奖励能够提供更精确的训练信号，解决"搭便车"问题，改善LLM在集合生成任务中的强化学习后训练效果。
...
### [225] [A Rational Account of Categorization Based on Information Theory](https://arxiv.org/abs/2603.29895)
*Christophe J. MacLellan,Karthik Singaravadivelan,Xin Lian,Zekun Wang,Pat Langley*

Main category: cs.AI

TL;DR: 提出基于信息论理性分析的新分类理论，验证其在解释经典分类实验数据方面优于现有模型


<details>
  <summary>Details</summary>
Motivation: 现有分类模型（如独立线索模型、理性分类模型、分层狄利克雷过程模型）在解释人类分类行为方面存在不足，需要更优的理论框架

Method: 基于信息论的理性分析方法，通过重新分析Hayes-Roth和Hayes-Roth(1977)、Medin和Schaffer(1978)、Smith和Minda(1998)的经典分类实验数据来验证理论

Result: 新理论在解释人类分类行为方面至少与现有模型相当或更好，优于独立线索模型、理性分类模型和分层狄利克雷过程模型

Conclusion: 基于信息论的理性分析为分类行为提供了更优的理论解释框架，能够更好地捕捉人类分类决策的本质
...
### [226] [ATP-Bench: Towards Agentic Tool Planning for MLLM Interleaved Generation](https://arxiv.org/abs/2603.29902)
*Yinuo Liu,Zi Qian,Heng Zhou,Jiahao Zhang,Yajie Zhang,Zhihang Li,Mengyu Zhou,Erchao Zhao,Xiaoxi Jiang,Guanjun Jiang*

Main category: cs.AI

TL;DR: 该论文提出了ATP-Bench基准测试，用于评估多模态大语言模型在交错文本-图像生成中的工具规划能力，并设计了多智能体MLLM-as-a-Judge系统进行自动化评估。


<details>
  <summary>Details</summary>
Motivation: 当前交错文本-图像生成方法要么依赖图像生成要么依赖检索增强，将两者视为互斥路径，无法统一事实性与创造性。需要一种智能体工具规划范式，让模型作为中央控制器自主决定何时、何地、使用何种工具来响应视觉关键查询。

Method: 1) 构建ATP-Bench基准测试，包含7,702个QA对（含1,592个VQA对），涵盖8个类别和25种视觉关键意图；2) 提出多智能体MLLM-as-a-Judge系统，评估工具调用精度、识别工具使用遗漏机会，并在无需参考真值的情况下评估整体响应质量。

Result: 对10个最先进的MLLM进行广泛实验发现：模型在连贯的交错规划方面表现不佳，工具使用行为存在显著差异，表明有大幅改进空间，为推进交错生成提供了可操作的指导。

Conclusion: 智能体工具规划是交错文本-图像生成的下一个里程碑，ATP-Bench基准测试和MAM评估系统为系统评估这一范式提供了有效工具，揭示了当前模型的局限性并指明了改进方向。
...
### [227] [C-TRAIL: A Commonsense World Framework for Trajectory Planning in Autonomous Driving](https://arxiv.org/abs/2603.29908)
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶轨迹规划中越来越多地使用大语言模型进行常识推理，但LLM输出具有内在不可靠性，在安全关键应用中存在风险。需要一种机制来可靠地利用LLM的常识推理能力。
...
Conclusion: C-TRAIL成功地将LLM的常识推理能力与信任机制相结合，为自动驾驶轨迹规划提供了一种可靠且高效的解决方案，在安全关键应用中具有实际应用价值。
...
### [228] [Uncertainty Gating for Cost-Aware Explainable Artificial Intelligence](https://arxiv.org/abs/2603.29915)
*Georgii Mikriukov,Grégoire Montavon,Marina M. -C. Höhne*

Main category: cs.AI

TL;DR: 利用认知不确定性作为解释可靠性的低成本代理指标，高认知不确定性区域对应决策边界模糊，解释不稳定且不可靠，可用于优化XAI方法选择和预算分配


<details>
  <summary>Details</summary>
Motivation: 后验解释方法计算成本高且可靠性无法保证，需要一种低成本的方式来评估解释可靠性，以优化XAI资源分配

Method: 提出使用认知不确定性作为解释可靠性的代理指标，高认知不确定性表明决策边界模糊、解释不稳定。基于此设计两种应用：1）改进最差情况解释（根据预期可靠性路由样本到廉价或昂贵XAI方法）；2）召回高质量解释（在预算约束下推迟对不确定样本的解释生成）

Result: 在四个表格数据集、五种不同架构和四种XAI方法上，观察到认知不确定性与解释稳定性之间存在强负相关。进一步分析显示认知不确定性不仅能区分稳定与不稳定解释，还能区分忠实与不忠实解释。图像分类实验证实发现可推广到非表格数据

Conclusion: 认知不确定性是解释可靠性的有效低成本代理指标，可用于优化XAI资源分配，提高解释质量并降低计算成本
...
### [229] [ScoringBench: A Benchmark for Evaluating Tabular Foundation Models with Proper Scoring Rules](https://arxiv.org/abs/2603.29928)
*Jonas Landsgesell,Pascal Knoll*

Main category: cs.AI

TL;DR: ScoringBench是一个新的开源基准测试，用于全面评估表格基础模型的概率预测质量，通过多种适当的评分规则（如CRPS、CRLS、区间评分等）来补充传统的点估计指标，特别关注分布尾部的性能评估。


<details>
  <summary>Details</summary>
Motivation: 当前表格基础模型（如TabPFN和TabICL）虽然能产生完整的预测分布，但主流回归基准测试几乎只使用点估计指标（如RMSE、R2）进行评估。这些聚合指标往往掩盖了模型在分布尾部的性能，这对于金融和临床研究等高风险决策领域至关重要，因为这些领域通常存在不对称的风险特征。

Method: 引入ScoringBench开源基准测试，计算一套全面的适当评分规则，包括CRPS、CRLS、区间评分、能量评分、加权CRPS和Brier评分，同时保留标准的点估计指标。评估了使用不同评分规则目标微调的realTabPFNv2.5、TabICL以及未微调的realTabPFNv2.5在一系列回归基准测试上的表现。

Result: 研究结果证实：模型排名取决于所选的评分规则，没有单一的预训练目标在所有情况下都是最优的。这表明对于对极端事件敏感的应用，评估指标的选择与数据本身一样，是领域特定的要求。

Conclusion: ScoringBench提供了一个更丰富的概率预测质量评估框架，特别适合高风险决策领域。基准测试通过Git pull请求维护，确保透明度、可追溯性、敏捷性和可重复性。
...
### [230] [Physiological and Semantic Patterns in Medical Teams Using an Intelligent Tutoring System](https://arxiv.org/abs/2603.29950)
*Xiaoshan Huang,Conrad Borchers,Jiayi Zhang,Susanne P. Lajoie*

Main category: cs.AI

TL;DR: 研究通过融合生理同步性和对话语义分析，揭示医疗团队在虚拟患者诊断中的关键协作时刻，发现生理同步峰值与语义多样性相关，成功团队在共同发现时同步，失败团队在共同不确定时同步。


<details>
  <summary>Details</summary>
Motivation: 有效协作需要团队通过社会共享学习调节（SSRL）管理复杂的认知和情感状态。生理同步性可以指示这些状态，但单独难以解释。研究旨在结合生理信号和对话分析来理解问题解决中的关键时刻。

Method: 研究四个医疗双人组使用智能辅导系统诊断虚拟患者案例。分析生理同步性和对话动态：1）将对话语义变化与瞬时生理同步峰值关联；2）对SSRL进行话语编码，使用句子嵌入计算余弦相似度；3）定性分析三角验证同步峰值作为"关键时刻"。

Result: 激活先验知识的语义相似度显著低于简单任务执行。高生理同步性与低语义相似度相关，表明这些时刻涉及探索性和多样化的语言使用。成功团队在共同发现时同步，不成功团队在共同不确定时同步。

Conclusion: 研究通过融合生物信号与对话分析，推进了以人为中心的人工智能，展示了如何理解问题解决中的关键时刻。生理同步峰值可作为协作中"关键时刻"的指标，成功与不成功团队在这些时刻表现出不同的语义特征。
...
### [231] [Structured Intent as a Protocol-Like Communication Layer: Cross-Model Robustness, Framework Comparison, and the Weak-Model Compensation Effect](https://arxiv.org/abs/2603.29953)
*Peng Gang*
...
<details>
  <summary>Details</summary>
Motivation: 探索结构化意图表示（特别是基于5W3H的PPS框架）在不同AI模型、语言和提示框架中保持用户目标的可靠性，验证其作为人机交互协议式通信层的实用价值。

Method: 1) 跨模型鲁棒性测试：在Claude、GPT-4o、Gemini 2.5 Pro上评估；2) 控制比较：与CO-STAR和RISEN框架对比；3) 用户研究：50名参与者在生态有效环境中测试AI辅助意图扩展。共评估3240个模型输出（3语言×6条件×3模型×3领域×20任务），使用DeepSeek-V3作为独立评估者。

Result: 1) 结构化提示显著降低跨语言分数方差（标准差从0.470降至约0.020）；2) 弱模型补偿效应：基线最弱的Gemini获得最大提升（+1.006），而最强的Claude提升较小（+0.217）；3) 5W3H、CO-STAR、RISEN在目标对齐得分上表现相似，表明维度分解本身是关键因素；4) 用户研究中，AI扩展的5W3H提示减少60%交互轮次，用户满意度从3.16提升至4.04。

Conclusion: 结构化意图表示作为稳健的协议式通信层，能有效提升人机交互中目标对齐的跨模型、跨语言一致性，特别对较弱模型有显著补偿作用，具有重要实用价值。
...
### [232] [Extending MONA in Camera Dropbox: Reproduction, Learned Approval, and Design Implications for Reward-Hacking Mitigation](https://arxiv.org/abs/2603.29993)
*Nathan Heath*

Main category: cs.AI

TL;DR: 本文是对MONA方法的再现与扩展研究，通过构建模块化的学习型批准机制套件，验证了MONA在缓解多步奖励黑客攻击方面的有效性，同时发现学习型批准模型面临优化不足而非黑客攻击重现的挑战。


<details>
  <summary>Details</summary>
Motivation: 原始MONA论文提出了一个关键开放问题：批准机制的构建方法（特别是批准对已实现结果的依赖程度）如何影响MONA的安全保证。本研究旨在通过再现和扩展MONA Camera Dropbox环境来探索这一问题。

Method: 1) 将发布的代码库重新打包为标准Python项目并实现脚本化PPO训练；2) 使用发布的参考数组验证普通RL（91.5%黑客攻击率）与oracle MONA（0.0%黑客攻击率）的对比；3) 引入模块化的学习型批准套件，涵盖oracle、噪声、错误指定、学习和校准批准机制；4) 在批准方法、规划视野、数据集大小和校准策略等参数上进行预算有限的试点扫描。

Result: 最佳校准学习型监督器运行实现了零观察到的奖励黑客攻击，但预期行为率显著低于oracle MONA（11.9% vs. 99.9%），这表明问题在于优化不足而非黑客攻击重现。结果与MONA论文的批准谱猜想一致。

Conclusion: 研究将MONA论文的批准谱猜想转化为可运行的实验对象，表明核心工程挑战从证明MONA概念转向构建能够保持足够远见而不重新开放奖励黑客攻击通道的学习型批准模型。代码和配置已公开。
...
### [233] [The Triadic Cognitive Architecture: Bounding Autonomous Action via Spatio-Temporal and Epistemic Friction](https://arxiv.org/abs/2603.30031)
*Davide Di Gioia*

Main category: cs.AI

TL;DR: 论文提出Triadic Cognitive Architecture (TCA)，将机器推理建立在连续时间物理基础上，通过认知摩擦概念解决当前AI代理在交互环境中的过度工具使用、延迟决策等问题。
...
Method: 提出Triadic Cognitive Architecture (TCA)统一数学框架，结合非线性滤波理论、黎曼路由几何和最优控制，定义认知摩擦概念，将代理决策过程映射为耦合随机控制问题，使用HJB启发的停止边界和基于rollout的信息价值近似。

Result: 在模拟的Emergency Medical Diagnostic Grid (EMDG)中验证，相比贪婪基线在延迟和拥塞成本下过度决策，TCA策略减少了行动时间，提高了患者存活率，同时不降低诊断准确性。

Conclusion: TCA通过将机器推理建立在连续时间物理基础上，为AI代理提供了更自然的决策机制，能够平衡信息获取与行动时机，在时间敏感环境中表现出更优的性能。
