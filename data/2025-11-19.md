...
Motivation: 现有的隐式场参数化方法缺乏确保表面与输入数据紧密拟合的显式机制，需要解决点云信息可靠性平衡的问题。

Method: 提出新的能量方程来平衡点云信息可靠性，并引入学习三维信息的新型卷积网络，采用神经Galerkin方法进行求解。
...
Method: 在CARLA模拟器中构建nuScenes风格的BEV感知数据集，保持与nuScenes格式完全兼容，数据集规模与nuScenes相当但类别分布更均衡，可直接用于闭环仿真部署。

Result: nuCarla提供了高性能BEV骨干网络，实现了最先进的检测结果，作为开放基准加速闭环端到端开发。

Conclusion: nuCarla通过提供数据和模型作为开放基准，显著加速了闭环端到端自动驾驶的发展，为可靠和安全感知研究铺平了道路。
...
TL;DR: 提出一个解决开放集识别中过自信问题的框架，通过扰动不确定性估计和两阶段未知检测来改善已知类和未知类的区分。


<details>
  <summary>Details</summary>
Motivation: 开放集识别中，当未知样本与已知类语义相似时，模型会过度自信地将它们分类为已知类，导致决策边界模糊。

Method: 框架包含两个组件：基于扰动的不确定性估计模块（通过参数扰动生成多样化预测）和两阶段未知检测模块（利用估计的不确定性改进分类）。

Result: 在三个公共数据集上的实验结果表明，该框架优于现有的开放集识别方法。

Conclusion: 所提出的框架能有效缓解开放集识别中的过自信问题，提升性能。
...
TL;DR: 提出了一种新颖的对象感知时序建模方法，用于少样本视频目标检测，通过选择性传播高置信度对象特征来提升时序一致性和检测精度。


<details>
  <summary>Details</summary>
Motivation: 解决少样本视频目标检测中面临的时序一致性挑战（如遮挡和外观变化）以及新对象泛化问题，避免依赖计算昂贵的区域提议方法。

Method: 采用过滤机制选择性传播高置信度对象特征跨帧，结合少样本训练的检测和分类头，实现高效特征传播而不依赖显式对象管提议。

Result: 在5-shot设置下，AP指标在多个数据集上显著提升：FSVOD-500提升3.7%，FSYTV-40提升5.3%，VidOR提升4.3%，VidVRD提升4.5%。1-shot、3-shot和10-shot配置也均有改进。

Conclusion: 该方法在少样本视频目标检测中实现了稳健的时序一致性，显著提升了检测性能，且不依赖复杂的对象管提议机制。
...
TL;DR: 提出了一种基于流匹配的图像融合方法，通过直接概率传输从源模态到融合图像分布，提高采样效率和结构一致性。使用多模型融合结果作为先验，结合任务感知选择和融合精炼模块提升伪标签质量，并集成弹性权重巩固和体验回放机制支持多任务学习。
...
Method: 采用流匹配范式实现直接概率传输，收集多个SOTA模型融合结果作为先验，使用任务感知选择函数筛选可靠伪标签，引入融合精炼模块分解增强退化组件，集成弹性权重巩固和体验回放机制支持多任务学习。

Result: 在多种融合任务上取得竞争性性能，显著提高采样效率，保持轻量级模型设计。

Conclusion: 该方法为图像融合提供了高效统一的解决方案，在保持性能的同时显著提升采样效率，并支持多任务学习场景。
...
TL;DR: 提出了一种基于扩散模型的两阶段无轨迹碰撞检测框架，通过生成合理的未来路段地图来检测交通事故，无需依赖车辆轨迹数据。


<details>
  <summary>Details</summary>
Motivation: 实时碰撞检测对于主动安全管理策略和提升交通效率至关重要。传统方法受限于轨迹获取和车辆跟踪的局限性，因此直接使用记录个体交通动态数据的路段地图进行碰撞检测。

Method: 两阶段框架：第一阶段使用Mapfusion扩散模型，通过噪声到正常的过程生成未来路段地图，结合时序嵌入组件捕获地图序列的时间动态，并利用ControlNet融入背景上下文增强生成控制；第二阶段通过比较监控路段地图与扩散模型生成结果来检测碰撞。

Result: 在非碰撞车辆运动数据上训练的Mapfusion能够基于学习到的运动模式生成真实的道路路段演化地图，在不同采样间隔下保持鲁棒性。真实世界碰撞实验表明该方法能有效准确检测碰撞。

Conclusion: 提出的两阶段无轨迹碰撞检测方法能够有效利用路段地图数据，在无需车辆轨迹的情况下实现准确的实时碰撞检测，为主动交通安全管理提供了新思路。
...
TL;DR: 提出ADATG方法，通过希尔伯特编码和自适应两网格训练策略，专门针对地震数据的高低频特征进行基础模型预训练


<details>
  <summary>Details</summary>
Motivation: 现有视觉transformer在处理地震数据时忽略了内在模式，无法有效捕捉高频和低频地震信息，而地震数据具有独特的频率特征需要专门处理

Method: 采用频谱分解分离高低频分量，使用分层希尔伯特编码表示数据，并提出自适应训练策略：先关注粗粒度信息，再逐步细化到细粒度特征

Result: 大量实验证明了该训练方法的有效性和效率

Conclusion: 研究表明基于地震图像高低频特征的数据编码和训练策略对于增强视觉地震基础模型预训练具有重要意义
...
TL;DR: 提出了一种基于面部微动态分析的被动式痴呆筛查方法，通过分析眨眼动态、嘴部运动、注视变化和头部微调等面部运动学特征，无需语音或文本即可进行早期神经认知变化检测。


<details>
  <summary>Details</summary>
Motivation: 现有痴呆筛查方法主要依赖语音或脚本化访谈，限制了在临床环境外的应用，且预测结果与语言和转录紧密耦合。本文旨在开发一种无需语言干预、可在自然环境中大规模应用的被动式筛查方法。

Method: 通过稳定面部信号，将微运动转换为可解释的面部微动态时间序列，平滑处理后提取短窗口的紧凑统计特征。分析运动在多个通道间的分布而非幅度，使预测更加透明。

Result: 在YT DemTalk数据集上，轻量级分类器取得了AUROC 0.953、平均精度0.961、F1分数0.851和准确率0.857的优异性能，其中注视不稳定性和嘴部/下颌动态是最具信息量的线索。

Conclusion: 面部微动态分析为语言无关的痴呆筛查提供了有效方法，能够在自然环境中实现大规模被动式检测，具有良好的跨设备、跨主题和跨文化适应性。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频生成模型基准主要关注保真度或对齐度，无法评估链式帧推理能力，这阻碍了对模型核心认知能力的系统理解和改进指导。

Method: 基于认知科学和实际AI应用，将链式帧推理分解为六个认知维度（从感知逻辑到抽象规划）和24个子任务，采用多源数据策展、最小提示协议和混合VLM辅助评估。

Result: 对最先进系统的实验揭示了视觉质量与推理深度之间的显著差距，建立了基线和诊断工具。
...
TL;DR: 该论文提出了基于排序的损失函数来解决热图式人体姿态估计中的三个主要问题，包括MSE损失的局限性、热图的空间和类别不平衡，以及损失函数与评估指标之间的差异。


<details>
  <summary>Details</summary>
Motivation: 热图式人体姿态估计方法存在三个主要问题：MSE损失无法有效改善关节定位、热图的空间和类别不平衡、损失函数与评估指标mAP之间的不匹配。
...
Conclusion: 该研究首次提出了与评估指标mAP对齐的损失函数，在一维和二维热图上均表现出有效性，在多个数据集上验证了方法的优越性。
...
TL;DR: 提出碰撞声源分割任务CS3，通过音频条件在视频中分割产生碰撞声音的物体，针对第一人称视角视频设计弱监督方法，在两个新基准上取得显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 受人类多感官感知能力启发，旨在从碰撞声音识别产生声音的物体，解决第一人称视角中视觉场景杂乱、物体小、交互短暂等挑战。

Method: 使用弱监督的音频条件分割方法，结合基础模型CLIP和SAM2，并融入第一人称线索（如手中物体）来定位可能的碰撞声源物体。

Result: 在两个新基准EPIC-CS3和Ego4D-CS3上，分别比竞争基线方法在mIoU指标上提升3倍和4.7倍。

Conclusion: 提出的CS3任务和方法能有效从碰撞声音中分割相关物体，为多模态感知研究提供了新方向。
...
TL;DR: 提出几何表示回归(GRR)方法，通过预测解耦的几何表示（射线方向和点云图）来估计相机位姿，替代传统的绝对位姿回归黑盒方法。
...
Method: 将APR重新定义为逆向渲染过程，预测世界坐标系中的两个解耦几何表示：射线方向估计相机旋转，点云图估计相机平移，通过可微分求解器恢复最终位姿。

Result: 在7-Scenes和Cambridge Landmarks数据集上达到最先进性能，解耦旋转和平移预测显著提升性能。

Conclusion: 建模逆向渲染过程是实现可泛化绝对位姿估计的更稳健路径，几何先验的引入有效提升了位姿估计的鲁棒性。
...
TL;DR: 提出了H-CNN-ViT模型用于膀胱癌复发预测，该模型通过分层门控注意力机制融合CNN和ViT的局部与全局特征，在多序列MRI数据集上达到78.6%的AUC性能。


<details>
  <summary>Details</summary>
Motivation: 膀胱癌复发率高（达78%），术后监测至关重要。多序列增强MRI是常用检测方法，但术后组织变化使判读困难，且缺乏专门用于复发评估的MRI数据集。

Method: 提出H-CNN-ViT分层门控注意力多分支模型，通过选择性加权融合CNN（局部特征）和ViT（全局特征），独立处理每个模态以捕获各自独特特性。

Result: 在自建的多序列多模态MRI数据集上，H-CNN-ViT达到78.6%的AUC，超越了现有最先进模型。

Conclusion: 该工作不仅提供了专门用于膀胱癌复发预测的MRI数据集基准，还证明了H-CNN-ViT模型在复发检测中的有效性，为AI辅助诊断提供了新方案。
...
TL;DR: 提出了QwenCLIP框架，用基于大语言模型的嵌入模块替换CLIP的文本编码器，通过可学习提示增强跨模态对齐，解决了CLIP在长文本医学报告表示方面的限制。


<details>
  <summary>Details</summary>
Motivation: CLIP的文本编码器仅支持77个token，无法有效处理信息丰富的长文本放射学报告；现有的医学领域特定编码器也受限于512token的输入长度和较浅的语义理解。
...
Conclusion: QwenCLIP通过结合LLM的扩展上下文能力，有效解决了医学领域长文本表示问题，提升了医学视觉语言任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统迁移学习方法在医学图像高维背景下存在局限，无法充分利用解剖学特征，需要改进特征选择和表示能力。

Method: 集成EfficientNet和VGG19，加入伪牛顿增强层进行特征权重智能调整，稀疏诱导层去除冗余特征，形成多层框架。

Result: 模型性能显著提升：精确度0.9，召回率0.861，F1分数0.88，损失0.18，准确率88.1%，优于基线模型EfficientNet。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手工制作的提示且性能不稳定，限制了在真实场景中的鲁棒性和泛化能力。作者发现当前开源VLM在高级决策任务中存在关键局限性。

Method: 利用VLM的强大场景理解能力为现有视觉基准添加结构化、语言丰富的场景描述，构建多模态交互架构融合视觉和语言特征，并设计后处理精炼模块提升预测可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 医学解剖分割领域的数据缩放规律尚未充分探索，而拓扑同构的解剖结构图像为变形增强策略提供了理论基础，旨在提高数据利用效率并减少标注和计算成本。

Method: 分析了15个语义任务和4种成像模态的缩放规律；评估了随机弹性变形和配准引导变形的增强策略；提出了基于图像配准的测地子空间生成微分同胚映射的新增增强方法。

Result: 更大数据集显著改善分割性能，遵循相似缩放趋势；配准和生成变形增强方法都显著提高数据利用效率；提出的生成变形方法实现了更优性能和加速收敛，无需额外数据。

Conclusion: 这项工作为理解医学图像分割的可扩展性和拓扑变化影响提供了见解，有助于开发更高效的模型，减少标注和计算成本。
...
Method: 基于Hema-Former多模态模块，在46个公开数据集上构建，包含超过70万张图像和2.1万个问答对，在检测、分类、分割、形态学、掩码语言建模和视觉问答等不同任务和粒度层次上桥接视觉和文本表示。
...
<details>
  <summary>Details</summary>
Motivation: 短暂沟壑是农业领域最受关注的土壤侵蚀现象之一，但由于其短暂的时间周期和缺乏准确的标注数据，传统计算机视觉和机器学习方法难以自动检测。
...
Conclusion: 该方法有效解决了短暂沟壑自动检测的挑战，为土壤侵蚀监测提供了可行的弱监督解决方案。
...
TL;DR: 提出一个基于压缩视频运动向量的可扩展框架，用于评估生成视频的时间真实性，通过运动向量统计差异量化运动质量，并展示了运动向量融合能提升下游分类任务性能。


<details>
  <summary>Details</summary>
Motivation: 当前生成视频模型在时间真实性方面存在弱点，大多数评估指标主要关注空间外观而对运动敏感性有限，需要一种能够有效评估时间行为的框架。

Method: 使用从压缩视频流（H.264、HEVC）提取的运动向量作为轻量级运动描述符，通过计算真实视频与生成视频运动向量统计的KL、JS和Wasserstein散度来量化真实性，并探索运动向量与RGB融合的多种方法。

Result: 在GenVidBench数据集上的实验显示，Pika和SVD在熵基散度上最接近真实视频，VC2和Text2Video-Zero在MV-sum统计上表现最佳，CogVideo在所有指标上偏差最大。运动向量融合显著提升了分类模型在真实与生成视频判别任务上的准确率。

Conclusion: 压缩域运动向量为诊断生成视频运动缺陷和增强判别模型的时间推理提供了有效的时序信号，是评估视频生成模型时间真实性的有力工具。
...
TL;DR: SAE-MCVT是首个可扩展的实时多摄像头车辆跟踪框架，通过边缘设备处理视频流并仅传输轻量级元数据到中央工作站，实现城市规模部署。
...
Method: 系统包含多个边缘设备与中央工作站。边缘端处理RTSP视频流，进行目标检测、跟踪、地理映射和特征提取，仅传输车辆位置和深度外观特征。中央端基于自监督相机链接模型学习相邻相机间的时空关系进行跨摄像头关联。

Result: 在RoundaboutHD数据集上，SAE-MCVT在2K 15FPS视频流上保持实时操作，IDF1得分为61.2。
...
Result: 初步结果显示，这些模型在安全推理方面表现不佳，揭示了人类学习与模型解释之间的性能差距。
...
TL;DR: 本文提出了一种基于课程学习的相对策略优化方法（CuRPO），通过将CoT长度和gIoU奖励作为复杂度指标，从简单到困难逐步训练视觉定位模型，解决了传统强化学习微调CoT推理在复杂场景下性能下降的问题。


<details>
  <summary>Details</summary>
Motivation: 研究发现基于强化学习的CoT推理微调在视觉定位任务中会随着CoT输出变长或复杂而性能下降，且增加数据集规模并不总能提升性能。这促使作者开发一种能够渐进式处理数据复杂度的训练策略。

Method: 提出Curriculum-based Relative Policy Optimization (CuRPO)方法，使用CoT长度和广义交并比(gIoU)奖励作为复杂度指标，将训练数据从简单到困难进行结构化组织，实现渐进式学习。

Result: 在RefCOCO、RefCOCO+、RefCOCOg和LISA数据集上的实验表明，CuRPO显著优于现有方法，在RefCOCO上实现了高达+12.52 mAP的提升，且在少样本学习场景下表现出优异的效率和鲁棒性。

Conclusion: CuRPO通过课程学习策略有效解决了CoT推理在复杂视觉定位任务中的性能退化问题，为处理模糊和复杂文本描述的视觉任务提供了高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决视频衍生帧数据集中由于连续帧之间的视觉相似性导致的信息泄漏问题，确保数据集划分更具代表性和可靠性。

Method: 使用聚类方法对视觉相似的帧进行分组，然后在训练集、验证集和测试集之间进行划分，避免相似帧出现在不同集合中。

Result: 该方法能够产生更具代表性、平衡和可靠的数据集划分，有效减少信息泄漏。

Conclusion: 基于聚类的帧选择策略是解决视频衍生帧数据集中信息泄漏问题的有效方法，能够提高数据集划分的质量和机器学习模型的评估可靠性。
...
TL;DR: 通过在程序生成的无视觉语义内容数据上预训练视觉Transformer，可以显著提高数据效率、收敛速度和下游性能，1%的程序数据相当于28%的ImageNet数据效果


<details>
  <summary>Details</summary>
Motivation: 探索通过程序生成的无视觉语义内容数据来为视觉Transformer注入通用的归纳偏置，以提升其跨模态的泛化能力

Method: 使用形式语法等简单算法生成程序数据，在视觉Transformer的视觉块嵌入机制之前进行预训练，鼓励模型内化抽象计算先验，然后进行标准的基于图像的训练

Result: 在ImageNet-1k上，仅用1%的训练预算用于程序数据预训练，最终准确率提高超过1.7%，1%的程序数据效果相当于28%的ImageNet数据
...
<details>
  <summary>Details</summary>
Motivation: 目标是学习图像域上的连续标量场，使其分割能够为图像中的细胞生成稳健的实例分割结果。

Method: 使用泊松偏微分方程和热方程稳态解的扩散场作为标量场，仅通过最小化场残差进行训练，无需正则化，采用分水岭方法进行分割。

Result: 在公共数据集上取得了有竞争力的结果，能够减少训练数据中异常值的不利影响，并保持清晰的细胞边界。

Conclusion: 这种新颖、简单但具有几何洞察力的方法能够实现优秀的细胞分割结果，具有实现简化、训练和推理时间短、能耗低等优势。
...
TL;DR: EchoAgent是一个用于心脏超声分析的框架，通过大型语言模型协调专业视觉工具，实现结构化、可解释的自动化分析，支持视频级推理和指南测量分析。


<details>
  <summary>Details</summary>
Motivation: 当前深度学习模型无法支持心脏超声所需的视频级推理和基于指南的测量分析，需要开发能够提供结构化、可解释自动化的解决方案。

Method: EchoAgent在大型语言模型控制下协调专业视觉工具，执行时间定位、空间测量和临床解释。关键贡献是测量可行性预测模型，确定解剖结构是否可靠可测量，实现自主工具选择。

Result: EchoAgent在时空视频分析复杂度增加的情况下仍能获得准确、可解释的结果，输出基于视觉证据和临床指南，支持透明度和可追溯性。

Conclusion: 这项工作证明了通过任务特定工具和全视频级自动化，实现基于指南的心脏超声视频分析代理推理的可行性，为心脏超声可信AI设定了新方向。
...
TL;DR: CrossTrainer提出了一种可迁移的视频技能评估方法，通过发现跨运动项目的通用技能属性（如平衡、控制、手部位置），训练多模态语言模型为新颖视频生成可操作的反馈和熟练度评级。


<details>
  <summary>Details</summary>
Motivation: 当前模型局限于单个运动项目，且专家级监督成本高昂、稀缺。为了缩小这一差距，需要探索可迁移的视频表示方法，以应对长尾运动项目的技能评估需求。

Method: CrossTrainer方法发现跨运动边界的技能属性，然后训练多模态语言模型，为新颖视频生成可操作的反馈（如"多抬手以产生更多力量"）和熟练度评级（如早期专家）。

Result: 在跨运动（迁移）和运动内（领域内）设置下的多个数据集上验证，相对于现有技术实现了高达60%的相对增益。

Conclusion: 通过抽象出指示人类技能的共享行为，所提出的视频表示方法比现有技术显著更好地泛化，丰富了当今的多模态大语言模型。
...
TL;DR: 提出基于卷积字典特征解耦的双提示专家网络(CD-DPE)，用于多对比度MRI超分辨率重建，通过分离跨对比度和内部对比度特征，结合频率提示和自适应路由提示来提升重建质量。


<details>
  <summary>Details</summary>
Motivation: 多对比度MRI超分辨率重建面临不同模态间对比度差异的挑战，导致参考图像纹理难以有效指导目标图像重建，造成特征整合不理想。

Method: 使用迭代卷积字典特征解耦模块(CD-FDM)分离特征，提出双提示特征融合专家模块(DP-FFEM)，通过频率提示选择相关参考特征，自适应路由提示确定最佳融合方法。

Result: 在公开多对比度MRI数据集上的实验表明，CD-DPE在重建精细细节方面优于现有最优方法，且在未见数据集上表现出强泛化能力。
...
TL;DR: RISE是首个基于单静态雷达的室内场景理解基准和系统，通过利用多径反射的几何线索，实现了布局重建和物体检测，在保护隐私的同时解决了光学传感器在室内环境中的遮挡问题。


<details>
  <summary>Details</summary>
Motivation: 解决室内场景理解中光学传感器存在的遮挡和隐私风险问题，利用毫米波雷达的穿透性和隐私保护特性，但需要克服其低空间分辨率的限制。
...
TL;DR: 开发了一个2.5D上下文感知模型，用于自动识别MRI切片的解剖平面方向（轴向、冠状、矢状），准确率达到99.49%，比2D模型减少60%错误。该模型在脑肿瘤检测任务中通过门控策略将准确率从97.0%提升到98.0%，减少33.3%的误诊。


<details>
  <summary>Details</summary>
Motivation: 人类能轻松识别MRI切片的解剖平面方向，但自动化系统难以完成此任务。缺失平面方向元数据会导致分析复杂化、增加数据集合并时的域偏移，并降低诊断分类器的准确性。

Method: 采用2.5D上下文感知模型，利用多切片信息避免单一切片的模糊性，实现稳健的特征学习。在3D切片序列和静态2D图像上训练模型，并使用门控策略基于不确定性分数选择性地使用元数据增强预测。

Result: 2.5D模型准确率达到99.49%，比2D参考模型（98.74%）提升显著，错误减少60%。在脑肿瘤检测任务中，准确率从97.0%提升到98.0%，误诊减少33.3%。
...
TL;DR: LINGUAL框架通过自然语言指令和上下文学习，将专家指令转换为可执行程序，自动完成医学图像分割任务，在主动领域自适应中实现与主动学习基线相当或更优的性能，同时减少约80%的标注时间。


<details>
  <summary>Details</summary>
Motivation: 医学图像分割中的主动学习仍然具有挑战性、劳动密集型和认知要求高，因为医学图像通常具有模糊和模糊的边界。传统主动学习中，标注工作量与感兴趣区域大小相关，大区域认知负担轻但标注成本高，小区域需要更精细的精度和专家更多注意力。语言指导提供了一种有效的替代方案，需要最少的专家努力，同时绕过分割中精确边界描绘的认知要求高的任务。

Method: 引入LINGUAL框架：接收专家的自然语言指令，通过上下文学习将其转换为可执行程序，并自动执行相应的子任务序列，无需任何人工干预。

Result: 在主动领域自适应（ADA）中，LINGUAL实现了与主动学习基线相当或更优的性能，同时估计标注时间减少了约80%。

Conclusion: LINGUAL框架通过语言指导有效解决了医学图像分割中的标注挑战，显著减少了专家工作量，同时保持了高质量的分割性能。
...
Method: 基于自监督模型对RandomResizedCrop增强的鲁棒性，WaRPAD首先定义基础评分函数，量化图像嵌入对通过Haar小波分解提取的高频方向扰动的敏感性。为模拟裁剪增强的鲁棒性，将图像缩放到模型输入大小的倍数，分割成小块，计算每个块的基础评分，最后对所有块的评分取平均得到最终检测分数。

Result: 在真实数据集和23种不同生成模型生成的图像上验证，WaRPAD始终实现竞争性性能，并对测试时损坏表现出强鲁棒性。由于RandomResizedCrop不变性是自监督模型的常见训练方案，WaRPAD可跨自监督模型应用。

Conclusion: WaRPAD提供了一种有效的无需训练AI生成图像检测方法，利用自监督模型的固有特性，在多种数据集和生成模型上表现出色，具有很好的通用性和鲁棒性。
...
TL;DR: FashionMAC是一个基于扩散模型的无需变形的服装图像生成框架，能够高质量地生成可控的时尚展示图像，通过直接外推分割出的服装来保持细节，并利用区域自适应解耦注意力机制实现细粒度控制。


<details>
  <summary>Details</summary>
Motivation: 解决现有服装图像生成方法中存在的两个关键问题：(1) 服装纹理变形问题，由于需要进行服装变形而导致细节失真；(2) 缺乏对生成模型外观的细粒度控制机制。

Method: 提出FashionMAC框架，核心思想是消除服装变形的需求，直接外推从着装人物分割出的服装区域。采用区域自适应解耦注意力(RADA)机制和链式掩码注入策略，自适应预测每个细粒度文本属性对应的生成区域。

Result: 大量实验验证了该框架相比现有最先进方法的优越性能，能够更好地保持服装细节并实现细粒度控制。

Conclusion: FashionMAC通过消除服装变形和引入RADA机制，成功解决了服装细节保持和细粒度控制的问题，为时尚图像生成提供了高质量的解决方案。
...
TL;DR: 提出了一种基于潜在扩散模型的洪水地图超分辨率方法，能够在保持精度的同时显著减少计算时间，提高泛化能力并增强可解释性。
...
Method: 利用潜在扩散模型对粗网格洪水地图进行超分辨率处理，结合物理信息输入，通过迁移学习加速新地理区域的适应。

Result: 实验结果显示，潜在扩散模型在不牺牲精度的情况下大幅减少了计算时间，且在跨物理位置具有优越的泛化能力。

Conclusion: 该方法实现了高保真洪水地图的实时生成，解决了机器学习黑盒问题，增强了可解释性，适用于实时洪水风险管理。
...
TL;DR: 提出了一种基于无人机图像自动检测、定位和分类混凝土桥梁结构缺陷的新方法，该方法结合显著性分析和YOLOX深度学习检测器，在准确性和计算效率方面表现优异。
...
Method: 采用两阶段框架：第一阶段使用显著性分析进行缺陷区域提议；第二阶段在显著性增强图像上应用基于YOLOX的深度学习检测器，通过边界框级亮度增强处理显著缺陷区域。
...
Result: SCAR在指令编辑和可控生成基准测试中实现了优越的视觉保真度和语义对齐，超越了先前的基于AR的方法，同时保持可控性。

Conclusion: SCAR基于向量量化预填充的灵活性和通用性，克服了其语义限制和高成本问题，可推广到next-token和next-set两种AR范式，且只需最小架构改动。
...
TL;DR: CORE提出了一种基于对象中心表示的视觉令牌压缩方法，通过高效分割解码器生成对象掩码作为语义先验，指导视觉令牌合并为紧凑的对象中心表示，并在极端压缩下仍能保持97.4%的基线性能。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在处理高分辨率图像时面临二次增长的视觉令牌带来的计算和内存成本问题，现有令牌压缩方法缺乏高层语义理解，导致次优合并、信息冗余或上下文丢失。

Method: CORE使用高效分割解码器生成对象掩码作为高层语义先验，指导视觉令牌合并为紧凑的对象中心表示，并采用质心引导排序机制恢复合并令牌的空间顺序。

Result: 在六个权威基准测试中建立了固定率压缩的新SOTA，在自适应率设置下实现了显著的效率提升，即使在仅保留2.2%视觉令牌的极端压缩下，仍能保持97.4%的基线性能。

Conclusion: CORE证明了对象中心表示在高效有效的LVLM处理中的优越性，为解决视觉令牌压缩问题提供了新范式。
...
TL;DR: 提出ZS-TMS新范式，通过预训练生成引擎直接合成任务特定分类器的全部参数，仅需单张示例图像和临床文本描述，无需训练即可部署使用。
...
Conclusion: 为AI诊断工具在数据极度有限的罕见疾病领域的快速开发和部署开辟了新途径。
...
TL;DR: 开发并验证了一个全自动深度学习管道，用于在三维CT扫描上测量肩胛盂骨缺损，该方法比传统手动测量更高效且一致性更好。


<details>
  <summary>Details</summary>
Motivation: 当前测量肩胛盂骨缺损的手动和半自动方法耗时且存在观察者间变异性，需要更可靠、高效的自动化测量工具。

Method: 采用三阶段算法：1) U-Net分割肩胛盂和肱骨；2) 神经网络预测肩胛盂边缘点；3) 主成分分析、投影和圆拟合计算骨缺损百分比。

Result: 自动测量与共识读数高度一致，且超过外科医生间一致性（ICC 0.84 vs 0.78），在骨缺损分类中低危和高危病例无相互误分类。

Conclusion: 该方法是一种时间高效且临床可靠的工具，适用于肩关节不稳的术前规划和筛查显著肩胛盂骨缺损患者。
...
TL;DR: 提出了DEER-3D框架，通过错误驱动的3D场景编辑生成针对性反事实数据，解决3D-LLMs在空间定位中的偏见问题，显著提升模型在3D环境中的语言-视觉空间定位能力。


<details>
  <summary>Details</summary>
Motivation: 现有3D-LLMs在准确将语言与3D环境中的视觉和空间元素进行定位方面存在局限，这主要源于训练数据侧重于语言推理而非空间理解，且缺乏足够的3D资源，导致固有的定位偏见未能解决。

Method: 提出DEER-3D框架，采用"分解、诊断评估、编辑、重训练"的工作流程。当识别到3D-LLM的定位失败时，首先诊断具体的谓词级错误（如属性或空间关系），然后执行最小化的谓词对齐3D场景编辑（如重新着色或重新定位），生成针对性的反事实监督数据用于迭代模型微调。

Result: 在多个3D定位和场景理解基准测试中评估编辑流程，通过迭代精化在所有评估数据集上一致展示出改进效果。

Conclusion: DEER-3D强调了针对性、错误驱动的场景编辑在弥合3D LLMs中语言推理能力与空间定位之间差距的有效性。
...
TL;DR: 提出GCA-ResUNet，一种将分组坐标注意力集成到ResNet-50残差块中的高效分割网络，在保持计算效率的同时提升全局建模能力。


<details>
  <summary>Details</summary>
Motivation: UNet类网络难以捕捉长距离依赖，Transformer变体需要大量计算和训练数据。需要一种既能捕获全局上下文又保持计算效率的解决方案。

Method: 在ResNet-50残差块中集成分组坐标注意力(GCA)，通过分组坐标建模联合编码跨通道和空间位置的全局依赖关系。

Result: 在Synapse数据集上Dice得分为86.11%，在ACDC数据集上达到92.64%，超越多个先进基线，同时保持快速推理和良好计算效率。

Conclusion: GCA为增强卷积架构的全局建模能力提供了实用方法，能够实现高精度且资源高效的医学图像分割。
...
<details>
  <summary>Details</summary>
Motivation: 传统多阶段"检索-匹配"流水线在跨视角物体地理定位中由于视角和尺度差异大、背景干扰复杂，容易产生累积误差。

Method: 采用全Transformer架构，使用Swin-Transformer联合编码无人机和卫星图像特征，引入网格级稀疏专家混合(GMoE)来捕获跨模态和视图内依赖关系，采用无锚检测头进行坐标回归。

Result: 在IoU=0.25和mIoU指标上达到领先性能（测试集分别为87.51%、62.50%和61.45%），显著优于DetGeo等方法。

Conclusion: 消融研究表明共享编码、查询引导融合和网格级稀疏专家混合具有互补增益，证明了所提方法的有效性。
...
TL;DR: 提出BCE3S方法，通过二元交叉熵的三部分协同学习解决长尾识别问题，提升特征的类内紧凑性和类间分离性，平衡分类器向量分离性。
...
Method: BCE3S包含三个组件：基于BCE的联合学习优化分类器和样本特征；基于BCE的对比学习进一步提升特征的类内紧凑性；基于BCE的均匀学习平衡分类器向量分离性并与联合学习交互增强特征特性。

Result: 在CIFAR10-LT、CIFAR100-LT、ImageNet-LT和iNaturalist2018等多个长尾数据集上实现了最先进的性能。

Conclusion: BCE3S方法能够有效提升长尾识别中特征的紧凑性和分离性，同时平衡分类器的分离性，在各种长尾数据集上表现出色。
...
TL;DR: FAPE-IR是一个频率感知规划和执行框架，使用冻结的多模态大语言模型作为规划器生成频率感知的修复计划，指导基于扩散的执行器进行图像修复。


<details>
  <summary>Details</summary>
Motivation: 现有的一体化图像修复方法依赖任务特定设计或潜在路由策略，难以适应现实世界中复杂的多种退化情况。

Method: 使用冻结MLLM作为规划器分析退化图像并生成频率感知修复计划，通过LoRA-MoE模块在扩散执行器中动态选择高频或低频专家，结合对抗训练和频率正则化损失。
...
Result: RIVER在提出的RVEBenchmark上表现最佳，并在VegGIE和FiVE两个额外基准上达到最先进性能，超越了六个基线方法。

Conclusion: RIVER成功解决了推理视频编辑任务，能够理解隐式查询并通过多跳推理执行视频编辑，为复杂视频编辑任务提供了有效解决方案。
...
TL;DR: 提出了一种实时自监督单目深度估计方法RTS-Mono，采用轻量级编码器-解码器架构，在保持高性能的同时大幅减少计算资源需求，实现了在边缘设备上的实时推理。


<details>
  <summary>Details</summary>
Motivation: 解决现有自监督单目深度估计模型计算资源消耗大、难以在实际场景中部署的问题，需要在保持性能的同时提高计算效率。
...
Conclusion: RTS-Mono成功解决了自监督单目深度估计在真实世界部署中的效率和性能平衡问题，为自动驾驶和机器人导航提供了实用的实时深度估计解决方案。
...
TL;DR: 提出A²GC-VPR方法，通过非对称聚合和几何约束改进视觉地点识别，解决了传统方法中特征与聚类中心分布差异的问题。
...
Method: 采用行列归一化平均与独立边缘校准，实现非对称匹配；通过可学习坐标嵌入融合几何约束，增强空间感知能力。

Result: 在MSLS、NordLand和Pittsburgh数据集上表现出优越性能，验证了方法在提升匹配准确性和鲁棒性方面的有效性。

Conclusion: A²GC-VPR通过非对称聚合和几何约束成功解决了视觉地点识别中的分布差异问题，显著提升了匹配性能。
...
TL;DR: 提出了CViT，一种轻量级视觉Transformer架构，通过CCFFN设计提高参数和计算效率，在保持精度的同时显著降低计算量、内存和能耗。
...
Method: 提出Cascaded-ViT架构，采用新颖的Cascaded-Chunk Feed Forward Network设计，通过分割输入特征来提高效率。
...
TL;DR: Coffee是一种通过语言描述来防止文本到图像扩散模型在微调过程中学习到不期望概念的方法，无需额外训练即可灵活调整不期望概念。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型在微调时容易学习到不期望概念的问题，这对于偏见缓解、防止恶意适应、属性解缠等下游任务至关重要。

Method: 通过保持用户提示嵌入与不期望概念的对齐来正则化适应过程，使用语言描述指定不期望概念，无需额外训练。

Result: 实验结果表明Coffee能有效防止模型在微调过程中学习到指定的不期望概念，且性能优于现有方法。

Conclusion: Coffee提供了一种灵活且有效的解决方案，通过语言描述来控制扩散模型微调过程，防止学习不期望概念。
...
TL;DR: 提出了MP-PVIR框架，通过多视角视频分析和行为阶段分割，将行人-车辆事故转化为结构化诊断报告，提升交通安全性分析。


<details>
  <summary>Details</summary>
Motivation: 现有视频系统只能检测事故是否发生，但无法深入分析行人行为的不同认知阶段，且缺乏多视角整合能力。

Method: 四阶段框架：事件触发多视角视频采集、行人行为阶段分割、阶段特定多视角推理、分层合成和诊断推理。使用两个专用VLM模型进行阶段分割和多视角分析。

Result: TG-VLM阶段分割mIoU达0.4881，PhaVR-VLM字幕评分33.063，问答准确率最高64.70%。在Woven Traffic Safety数据集上有效将视频数据转化为可操作见解。

Conclusion: MP-PVIR成功将多视角视频数据转化为结构化诊断报告，推进了车辆-基础设施协同系统中的AI驱动交通安全分析。
...
TL;DR: 提出了ConvNN框架，将卷积和自注意力统一在k近邻聚合框架下，揭示了它们都是邻域选择和聚合的特殊情况，只是分别基于空间邻近性和特征相似性。


<details>
  <summary>Details</summary>
Motivation: 尽管CNN和Transformer重塑了计算机视觉，但两者通常被视为根本不同的架构。本文认为卷积和自注意力实际上可以统一在同一个框架下。

Method: 引入卷积最近邻（ConvNN）框架，作为卷积层和注意力层的即插即用替代品，能够系统探索这两个极端之间的中间谱系。

Result: 在CIFAR-10和CIFAR-100分类任务上验证：VGG中的混合分支结合空间邻近性和特征相似性选择提高了准确率；ViT中的ConvNN优于标准注意力和其他注意力变体。

Conclusion: 本文提供了一个统一框架，消解了卷积和注意力之间的明显区别，为设计更原则化和可解释的视觉架构提供了启示。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视频时刻检索方法主要依赖粗粒度时间理解和单一视觉模态，在复杂视频中性能受限。为了解决这个问题，需要集成音频线索并利用镜头级时间结构来提升性能。

Method: 提出SMART框架，整合音频和视觉特征，应用镜头感知令牌压缩选择性地保留高信息令牌以减少冗余，同时优化提示设计以更好地利用音频-视觉线索。
...
TL;DR: iGaussian是一个两阶段前馈框架，通过直接3D高斯反演实现实时相机姿态估计，相比基于优化的方法速度提升10倍，在移动机器人上达到2.87 FPS跟踪速度。
...
Method: 采用两阶段前馈框架：首先使用高斯场景先验的姿态回归网络回归粗略6DoF姿态，然后通过特征匹配和多模型融合进行细化。关键创新包括无需可微渲染的跨相关模块和加权多视图预测器。

Result: 在NeRF Synthetic、Mip-NeRF 360和T&T+DB数据集上表现优异，中值旋转误差降至0.2°，在移动机器人上实现2.87 FPS跟踪速度。

Conclusion: iGaussian通过直接3D高斯反演实现了实时相机姿态估计，在精度和速度方面均显著优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 毫米波信号能够穿透常见遮挡物并反射隐藏物体，这为在遮挡条件下进行3D重建提供了可能，可应用于机器人、增强现实和物流等领域。

Method: 采用三阶段流程：提出候选几何表面、使用基于transformer的形状补全模型（专为毫米波信号设计）、进行熵引导的表面选择。该方法完全使用合成点云进行训练。

Result: 与最先进基线方法相比，Wave-Former将召回率从54%提升到72%，同时保持85%的高精度。

Conclusion: Wave-Former展示了在完全遮挡条件下进行高精度3D形状重建的能力，并具有良好的泛化性能。
...
TL;DR: 本文提出了O3SLM模型，通过构建大规模图像-草图-指令三元组数据集，解决了大视觉语言模型在理解抽象手绘草图方面的局限性，在多个草图任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有大视觉语言模型在解释抽象视觉输入（特别是手绘草图）方面能力有限，主要瓶颈是缺乏同时建模草图、真实图像和自然语言指令的大规模数据集。

Method: 提出了两个关键贡献：(1) 构建大规模图像-草图-指令三元组数据集，支持预训练和指令调优；(2) 在该数据集上训练O3SLM模型。

Result: 在多个草图任务（目标定位、计数、图像检索、视觉问答）上的综合评估表明，O3SLM实现了最先进的性能，在草图理解和推理方面显著优于现有LVLMs。

Conclusion: O3SLM通过专门的数据集和训练方法，有效提升了大视觉语言模型对抽象手绘草图的理解能力。
...
TL;DR: 提出RiSe框架解决多模态人脸反欺诈在跨域场景中的泛化问题，通过非对称不变风险最小化和多模态协同解耦来应对表示不变风险和协同不变风险。
...
Conclusion: RiSe框架有效解决了多模态人脸反欺诈的跨域泛化问题，通过处理表示和协同不变风险提升了模型在未知域的表现。
...
TL;DR: MVI-Bench是首个专门评估误导性视觉输入如何削弱大型视觉语言模型鲁棒性的综合基准，包含1,248个专家标注的VQA实例，并提出了细粒度鲁棒性评估指标MVI-Sensitivity。
...
Method: 基于基本视觉基元，设计了三个层次误导性视觉输入分类：视觉概念、视觉属性和视觉关系，构建了六个代表性类别和1,248个VQA实例。

Result: 对18个最先进LVLMs的实证结果揭示了它们对误导性视觉输入的明显脆弱性。

Conclusion: MVI-Bench的深入分析为开发更可靠和鲁棒的LVLMs提供了可操作的见解。
...
TL;DR: 提出了一种对象级令牌压缩策略，通过自适应令牌合并来减少MLLMs中的图像令牌数量，仅使用10%的令牌即可达到原始模型96%的性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型中补丁级令牌化导致的二次增长问题，该方法与人类视觉认知系统一致，减少幻觉和计算冗余。

Method: 采用对象级令牌合并策略进行自适应令牌压缩，与人类视觉系统保持一致。

Result: 在多个综合基准测试中，平均仅使用10%的令牌即可达到原始模型96%的性能，在压缩比和性能平衡方面优于相关工作。

Conclusion: 提出的自适应令牌压缩方法在显著减少计算和内存负担的同时，保持了高性能，验证了与人类视觉系统的一致性。
...
TL;DR: DoGCLR是一种基于博弈论的自监督对比学习框架，用于骨架动作识别。它通过时空双权重定位机制识别关键运动区域，并采用熵驱动的优势策略管理负样本，在多个数据集上取得了优于现有方法的效果。


<details>
  <summary>Details</summary>
Motivation: 现有的自监督对比学习方法在处理骨架动作识别时存在两个问题：对所有骨架区域统一处理导致运动信息丢失，以及使用FIFO队列存储负样本导致非最优的负样本选择。

Method: 提出DoGCLR框架：1）将正负样本构建建模为动态优势博弈，达到语义保持和判别强度的平衡；2）时空双权重定位机制识别关键运动区域并指导区域增强；3）熵驱动优势策略管理记忆库，保留高熵负样本并替换低熵负样本。
...
Conclusion: DoGCLR通过博弈论框架和熵驱动的负样本管理策略，在骨架动作识别任务中表现出色，特别是在更具挑战性的场景中展现了强大的鲁棒性。
...
TL;DR: UniSER是一个基础性的多功能模型，能够在一个框架内处理由软效应（如镜头光晕、雾霾、阴影、反射）引起的多种图像退化问题，显著优于专业模型和通用模型。


<details>
  <summary>Details</summary>
Motivation: 现有方法针对各种软效应退化问题开发高度专业化的模型，缺乏可扩展性且未能利用这些恢复问题的共享本质。而最近的通用模型需要详细提示，在这些细粒度任务上往往无法实现稳健的去除或保持场景身份。

Method: 利用软效应的共同本质（即半透明遮挡），策划大规模3.8M对数据集确保稳健性和泛化能力，包括新颖的物理合理数据填补公共基准的关键空白，并定制训练流程微调扩散变换器从多样化数据中学习稳健的恢复先验，集成细粒度掩码和强度控制。

Result: UniSER在野外环境中实现了稳健、高保真度的恢复，显著优于专业模型和通用模型。

Conclusion: 通过利用软效应的共同本质和协同方法，UniSER能够在一个统一框架内有效处理多种图像退化问题，展示了基础性多功能模型的潜力。
...
TL;DR: 提出UMEG-Net用于少样本精确事件检测，通过统一多实体图网络整合人体骨架和运动物体关键点，结合GCN和多尺度时间移位，并使用多模态蒸馏提升性能。


<details>
  <summary>Details</summary>
Motivation: 精确事件检测面临快速连续事件、运动模糊和细微视觉差异的挑战，现有方法依赖大量标注数据和像素/姿态输入，在少样本条件下表现不佳，而获取大规模标注数据集实际困难。

Method: UMEG-Net整合人体骨架和运动特定物体关键点到统一图中，采用基于GCN和多尺度时间移位的时空提取模块，并通过多模态蒸馏将关键点图知识迁移到视觉表示。

Result: 在少样本设置下实现鲁棒性能，显著优于基线模型，为少样本精确事件检测提供可扩展有效解决方案。

Conclusion: UMEG-Net通过统一图网络和多模态蒸馏，在有限标注数据下实现了优异的少样本精确事件检测性能。
...
TL;DR: 提出GloTok图像分词器，利用全局关系信息建模更均匀的语义分布，通过代码本直方图关系学习和残差学习模块，实现高质量图像重建和生成。
...
Method: 1. 代码本直方图关系学习：将预训练模型在整个数据集上建模的语义转移到语义代码本；2. 残差学习模块：恢复细粒度细节以最小化量化带来的重建误差。
...
TL;DR: 提出了MCFormer网络，通过建模多尺度相关性来抑制船舶重识别中的异常样本影响，包含全局和局部相关性模块，在三个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有船舶重识别方法多从行人重识别算法直接迁移而来，无法有效处理船舶图像特有的问题，如更大的类内差异和更严重的局部缺失，导致同一身份中出现异常样本。

Method: 提出多尺度相关性感知Transformer网络(MCFormer)，包含全局相关性模块(GCM)构建全局相似性亲和矩阵，以及局部相关性模块(LCM)挖掘和对齐正样本的局部特征，通过动态记忆库补偿缺失区域。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据集多为人工驾驶或未明确驾驶模式收集，无法真实评估自动驾驶车辆的黑盒控制行为安全性，需要专门由自动驾驶模式收集的数据集。

Method: 从市场多个量产自动驾驶车型收集100+小时自然驾驶数据，分割为32,727个关键帧，包含同步相机图像、高精度GNSS/IMU数据、车辆轨迹和详细2D标注。

Result: 数据集提供丰富的场景属性标注，端到端运动规划模型在自动驾驶帧上的平均位移误差为1.4米，每周新增10+小时数据。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型只能产生描述性输出，无法执行复杂的视觉任务。Orion旨在通过工具驱动的代理框架实现从被动视觉理解到主动视觉智能的转变。

Method: 使用代理框架协调多种专门的计算机视觉工具，包括目标检测、关键点定位、全景分割、OCR和几何分析等，结合神经感知与符号执行。
...
### [67] [Hierarchical Semantic Learning for Multi-Class Aorta Segmentation](https://arxiv.org/abs/2511.14187)
*Pengcheng Shi*
...
<details>
  <summary>Details</summary>
Motivation: 主动脉血管病理需要及时干预，现有方法在血管结构分割中忽视分层解剖关系且难以处理严重的类别不平衡问题。

Method: 采用课程学习策略和分形softmax进行分层语义学习，通过从简单到复杂逐步学习解剖约束，并使用两阶段推理策略加速模型。

Result: 在验证集上，分层语义损失使nnU-Net ResEnc M的Dice得分提升11.65%；在测试集上比基线模型高5.6%的Dice得分，推理速度提升达5倍。

Conclusion: 该方法在主动脉血管分割中显著提升了准确性和效率，适合实时临床应用，相关代码已公开。
...
### [68] [GEN3D: Generating Domain-Free 3D Scenes from a Single Image](https://arxiv.org/abs/2511.14291)
*Yuxin Zhang,Ziyu Lu,Hongbo Duan,Keyu Fan,Pengting Luo,Peiyu Zhuang,Mengyu Yang,Houde Liu*

Main category: cs.CV

TL;DR: Gen3d是一种从单张图像生成高质量3D场景的新方法，通过RGBD图像生成初始点云，维护和扩展世界模型，最终优化高斯溅射表示来完成3D场景重建。


<details>
  <summary>Details</summary>
Motivation: 现有的神经3D重建方法依赖密集多视角捕获，限制了其广泛应用。3D场景生成对于推进具身AI和世界模型至关重要，需要多样化的高质量场景进行学习和评估。

Method: 从RGBD图像生成初始点云，维护和扩展世界模型，通过优化高斯溅射表示来最终完成3D场景重建。

Result: 在多样化数据集上的广泛实验表明，该方法在生成世界模型和合成高保真、一致的新视角方面具有强大的泛化能力和优越性能。

Conclusion: Gen3d方法能够从单张图像生成高质量、广范围、通用的3D场景，在3D场景生成任务中表现出色。
...
TL;DR: DetGain是一种专门用于目标检测的在线数据筛选方法，通过估计每张图像对数据集级AP的边际扰动来选择信息丰富的训练样本。
...
Method: 通过建模全局分数分布，DetGain高效估计全局AP变化并计算师生贡献差距，在每个迭代中选择信息丰富的样本。该方法与架构无关且侵入性最小。

Result: 在COCO数据集上使用多个代表性检测器进行的实验显示准确率持续提升，在低质量数据下表现出强鲁棒性，并能与知识蒸馏技术有效结合进一步提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决医学图像分割中数据隐私和标注成本问题，同时应对联邦半监督学习中伪标签可靠性差和客户端计算资源受限的挑战。

Method: 结合双重知识蒸馏和自适应一致性机制，利用大容量分割基础模型指导轻量级客户端模型训练，提升伪标签质量。

Result: 在皮肤病变和息肉分割任务上，无论是同构还是异构设置，SAM-Fed均优于现有最先进的联邦半监督学习方法。

Conclusion: SAM-Fed框架有效解决了联邦半监督学习中模型容量与设备限制的矛盾，显著提升了医学图像分割性能。
...
TL;DR: InstantViR是一个实时视频重建框架，通过将双向视频扩散模型蒸馏为因果自回归学生模型，在单次前向传播中完成视频恢复，实现35+ FPS的实时处理速度。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型方法要么存在时间伪影，要么迭代后验采样速度太慢，无法满足实时视频处理的需求。

Method: 将预训练的视频扩散模型（教师）蒸馏为因果自回归学生模型，使用LeanVAE替换VAE进行高效潜在空间处理，无需外部配对数据。

Result: 在随机修复、高斯去模糊和超分辨率任务中，InstantViR达到或超过扩散基线的重建质量，在A100 GPU上运行速度超过35 FPS，比迭代视频扩散求解器快100倍。
...
### [72] [LSP-YOLO: A Lightweight Single-Stage Network for Sitting Posture Recognition on Embedded Devices](https://arxiv.org/abs/2511.14322)
...
Method: 基于YOLOv11-Pose设计LSP-YOLO，集成PConv和SimAM构建Light-C3k2轻量模块；在识别头中通过逐点卷积将关键点直接映射到姿势类别，采用中间监督实现姿态估计与分类的高效融合。

Result: 在包含5000张图像的数据集上，最小模型LSP-YOLO-n在PC上达到94.2%准确率和251FPS，模型大小仅1.9MB；在SV830C + GC030A平台上实现实时高精度推理。
...
TL;DR: Bapp是一款基于Flutter框架和Google ML Kit开发的移动应用，用于实时分析眼睑运动，在临床验证中表现出高精度（98.3%准确率），为眼睑运动监测提供了便携、客观的解决方案。
...
Method: 使用Flutter框架和Google ML Kit开发移动应用Bapp，通过45个真实患者视频进行临床验证，由眼科专家手动标注眨眼作为基准真值，采用精确率、召回率和F1分数等标准指标评估性能。

Result: Bapp在验证中表现出98.4%的精确率、96.9%的召回率和98.3%的整体准确率，证实了其作为眼睑运动监测工具的可靠性。

Conclusion: Bapp为正常和异常眼睑运动监测提供了便携、可访问且客观的工具，是传统手动眨眼计数的有前景替代方案，支持临床环境中的连续眼部健康监测和术后评估。
...
TL;DR: 提出了一种基于测量约束采样的盲人脸恢复方法，能够根据不同的文本提示生成多样化的高质量人脸重建结果。
...
Method: 将盲人脸恢复构建为测量约束的生成任务，通过粗恢复的受控退化构建逆问题，在文本到图像扩散中进行后验引导采样，包括确保结果与输入结构对齐的前向测量和产生投影空间的反向测量。

Result: 实验表明该方法能够生成与提示对齐的结果，并在盲人脸恢复任务上优于现有方法。
...
TL;DR: 提出了一种自回归扩散模型，用于语音驱动的3D面部动画，通过流式处理音频输入解决长序列处理和延迟问题。
...
Result: 实现了对可变长度音频的灵活处理，实现了与音频时长无关的低延迟，并开发了实时交互演示。

Conclusion: 该方法能够实现高质量、低延迟的实时3D面部动画合成，解决了现有方法的局限性。
...
### [76] [Cheating Stereo Matching in Full-scale: Physical Adversarial Attack against Binocular Depth Estimation in Autonomous Driving](https://arxiv.org/abs/2511.14386)
*Kangqiao Zhao,Shuo Huai,Xurui Song,Jun Luo*

Main category: cs.CV

TL;DR: 提出首个针对立体匹配模型的纹理化物理对抗攻击，使用3D全局伪装纹理而非局部2D补丁，在自动驾驶场景中有效欺骗立体深度估计模型。
...
Method: 采用3D物理对抗示例，提出3D立体匹配渲染模块处理相机视差，并开发新颖的融合攻击通过细粒度优化将目标无缝融入环境。

Result: 广泛评估显示，该方法能成功欺骗立体模型产生错误的深度信息。

Conclusion: 该方法在保持视觉一致性的同时，显著提升了攻击的隐蔽性和致命性，优于现有的隐藏攻击方法。
...
<details>
  <summary>Details</summary>
Motivation: 当前方法过度依赖隐式网络建模时空关系和运动特征，陷入被动学习陷阱，导致冗余单调的3D坐标信息获取，缺乏主动引导的显式学习机制。

Method: 设计数据感知模块将姿态投影到商空间，解耦运动几何与坐标冗余；引入网络感知模块通过恢复性学习主动学习时空依赖关系；采用辅助学习网络从扰动信息中主动适应学习。

Result: 在H3.6M、CMU Mocap和3DPW数据集上分别实现了16.3%、13.9%和10.1%的性能提升，达到新的最先进水平。

Conclusion: APS方法能够有效增强主动感知能力，是模型无关的，可与不同预测模型集成，显著提升了3D人体运动预测的性能。
...
### [78] [Enhancing Generalization of Depth Estimation Foundation Model via Weakly-Supervised Adaptation with Regularization](https://arxiv.org/abs/2511.14238)
...
<details>
  <summary>Details</summary>
Motivation: 虽然基础模型在零样本深度估计方面取得了进展，但当下游任务有数据可用时，能否进一步提升性能？

Method: 采用密集自训练目标作为结构自监督，引入语义感知分层归一化，使用成对顺序深度标注进行弱监督，并应用权重正则化损失

Result: 在多样化和具有挑战性的场景下，WeSTAR在现实和损坏的分布外数据集上持续改善泛化能力，在广泛基准测试中达到最先进性能

Conclusion: WeSTAR框架有效提升了深度估计基础模型在未见领域的鲁棒性和泛化性能
...
TL;DR: AVI是一个无需训练的视频理解框架，通过检索-感知-审查的三阶段推理过程，结合结构化视频知识库和开源模型集成，实现高效视频理解。
...
Method: 提出三阶段推理流程（检索-感知-审查）、结构化视频知识库（实体图）、多粒度集成工具和开源模型集成。

Result: 在LVBench、VideoMME-Long、LongVideoBench和Charades-STA等基准测试中取得有竞争力的性能，并提供更好的可解释性。

Conclusion: AVI框架通过系统级设计实现了无需训练的高效视频理解，在保持性能的同时提供了更好的可解释性和灵活性。
...
Conclusion: 提出的GNSS-free协同感知框架能够有效解决GNSS拒止环境下的协同感知挑战，通过LiDAR定位和时空对齐实现鲁棒的协同环境感知。
...
TL;DR: 提出了ManipBench大规模图像篡改检测基准和ManipShield统一检测模型，解决了现有基准内容多样性不足、生成模型覆盖窄和可解释性差的问题。
...
Conclusion: ManipBench基准和ManipShield模型为AI编辑图像检测提供了全面的解决方案，将推动篡改检测技术的发展。
...
### [82] [Deep Learning-Based Regional White Matter Hyperintensity Mapping as a Robust Biomarker for Alzheimer's Disease](https://arxiv.org/abs/2511.14588)
*Julia Machnio,Mads Nielsen,Mostafa Mehdipour Ghazi*

Main category: cs.CV

TL;DR: 提出了一个深度学习框架用于白质高信号（WMH）的精确分割和定位，发现区域WMH体积比全局病变负荷在疾病分类中表现更好，结合脑萎缩指标可进一步提高诊断性能。


<details>
  <summary>Details</summary>
Motivation: 虽然WMH自动分割方法已有进展，但大多数只提供全局病变负荷，忽略了不同白质区域的空间分布差异，而区域WMH量化可能对神经退行性疾病诊断有重要价值。

Method: 开发深度学习框架进行WMH分割和定位，在公共数据集和ADNI队列中评估，量化解剖定义区域内的WMH负荷，并结合脑结构体积评估诊断价值。

Result: 预测的病变负荷与参考WMH估计一致，区域WMH体积在疾病分类中持续优于全局病变负荷，结合脑萎缩指标后AUC值可达0.97，前部白质束等特定区域与诊断状态可重复相关。

Conclusion: 区域WMH量化具有附加价值，将局部病变指标与萎缩标记结合可能增强神经退行性疾病的早期诊断和分层能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统张量奇异值分解方法存在两个关键限制：1) 潜在张量近似粗糙，无法准确捕获空间局部高频信息；2) 变换矩阵由固定基原子组成，无法精确捕获模式3纤维上的局部高频信息。

Method: 使用定制的2D高斯泼溅生成潜在张量，1D高斯泼溅生成变换矩阵，两者在该表示框架中不可或缺且互补，具有强大的表示能力，特别是对局部高频信息。

Result: 在多项多维图像恢复实验中，GSLR始终优于最先进方法，特别是在捕获局部高频信息方面表现突出。

Conclusion: GSLR框架通过高斯泼溅技术有效解决了传统t-SVD方法的局限性，为多维图像提供了紧凑且连续的表示方法，在局部高频信息捕获方面具有显著优势。
...
### [84] [CCSD: Cross-Modal Compositional Self-Distillation for Robust Brain Tumor Segmentation with Missing Modalities](https://arxiv.org/abs/2511.14599)
*Dongqing Xie,Yonghuang Wu,Zisheng Ai,Jun Min,Zhencun Jiang,Shaojin Geng,Lei Wang*

Main category: cs.CV

TL;DR: 提出了一种跨模态组合自蒸馏框架CCSD，用于处理脑肿瘤MRI分割中任意模态缺失的情况，通过分层模态自蒸馏和渐进模态组合蒸馏提升模型鲁棒性。
...
Conclusion: CCSD框架能够灵活处理任意模态组合输入，有效解决了临床中模态缺失对脑肿瘤分割的挑战。
...
Method: 提出VLM3D框架，从视觉语言模型的Yes/No对数概率中提取双查询评判信号，评估语义保真度和几何一致性。该指导信号可应用于优化型管线的奖励目标和前馈型管线的测试时指导模块。

Result: 在标准基准测试中，VLM3D显著优于现有方法；作为测试时指导模块，能够主动纠正SOTA原生3D模型迭代采样过程中的严重空间错误。

Conclusion: VLM3D建立了一个原则化且可泛化的路径，将视觉语言模型丰富的、基于语言理解的语义和空间知识注入到多样化的3D生成管线中。
...
### [86] [MRI Embeddings Complement Clinical Predictors for Cognitive Decline Modeling in Alzheimer's Disease Cohorts](https://arxiv.org/abs/2511.14601)
*Nathaniel Putera,Daniel Vilet Rodríguez,Noah Videcrantz,Julia Machnio,Mostafa Mehdipour Ghazi*

Main category: cs.CV

TL;DR: 该研究评估了表格数据和基于MRI的表示方法在预测阿尔茨海默病认知衰退中的贡献，发现临床特征在识别高风险极端病例方面表现最佳，而基于transformer的MRI嵌入在区分认知稳定个体方面更有效。
...
Method: 引入基于动态时间规整聚类的轨迹感知标记策略，通过无监督重建在协调和增强的MRI数据上训练3D Vision Transformer，获得解剖结构保持的嵌入，然后使用传统机器学习分类器和深度学习头评估预训练编码器嵌入。
...
TL;DR: 该论文提出了一种基于中间域代理的跨域少样本学习方法，通过源域特征嵌入构建代码本，重构目标域特征，并利用中间域的视觉风格和语义内容属性进行快速域对齐。


<details>
  <summary>Details</summary>
Motivation: 解决跨域少样本学习中的语义不匹配、大域差异和数据稀缺三重挑战，不同于现有专注于广义表示的方法，探索中间域代理的构建和利用。

Method: 使用源域特征嵌入构建中间域代理作为代码本，重构目标域特征，并基于中间域的视觉风格和语义内容属性开发快速域对齐方法。

Result: 在8个跨域少样本学习基准测试中超越了最先进模型。

Conclusion: 通过中间域重构和目标特征变换的协同学习，能够有效提升跨域少样本学习的性能。
...
### [88] [NeuralBoneReg: A Novel Self-Supervised Method for Robust and Accurate Multi-Modal Bone Surface Registration](https://arxiv.org/abs/2511.14286)
...
Result: 在三个多模态数据集上评估，NeuralBoneReg达到或超过现有方法，在UltraBones100k上平均RRE/RTE为1.68°/1.86 mm，在UltraBones-Hip上为1.88°/1.89 mm，在SpineDepth上为3.79°/2.45 mm。

Conclusion: NeuralBoneReg在解剖结构和模态上展现出强大的泛化能力，为计算机辅助骨科手术提供了稳健准确的跨模态配准。
...
### [89] [Improving segmentation of retinal arteries and veins using cardiac signal in doppler holograms](https://arxiv.org/abs/2511.14654)
*Marius Dubosc,Yann Fischer,Zacharie Auray,Nicolas Boutry,Edwin Carlinet,Michael Atlan,Thierry Geraud*

Main category: cs.CV

TL;DR: 提出了一种利用标准分割架构进行多普勒全息图中动静脉分割的简单有效方法，通过结合脉冲分析特征使传统U-Net能够利用时间动态信息
...
Conclusion: 时间分辨预处理能够释放深度学习在多普勒全息术中的全部潜力，为视网膜血流动力学的定量探索开辟新前景
...
### [90] [Iterative Diffusion-Refined Neural Attenuation Fields for Multi-Source Stationary CT Reconstruction: NAF Meets Diffusion Model](https://arxiv.org/abs/2511.14310)
...
Conclusion: Diff-NAF能够通过迭代细化逐步提高投影完整性和重建保真度，最终在超稀疏视角条件下获得高质量的CT重建结果。
...
### [91] [Dental3R: Geometry-Aware Pairing for Intraoral 3D Reconstruction from Sparse-View Photographs](https://arxiv.org/abs/2511.14315)
*Yiyi Miao,Taoyu Wu,Tong Chen,Ji Jiang,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: 提出Dental3R方法，从稀疏口腔照片进行无姿态、图引导的3D重建，解决传统方法在远程正畸中的限制，通过几何感知配对策略和小波正则化实现高质量重建。


<details>
  <summary>Details</summary>
Motivation: 传统口腔内扫描在远程正畸中不可用，而稀疏智能手机图像重建面临大视角基线、光照不一致和镜面反射等挑战，导致重建质量下降。

Method: 使用几何感知配对策略(GAPS)选择高价值图像对，基于恢复的姿态和点云训练3D高斯泼溅模型，采用小波正则化目标保持细节。
...
### [92] [Step by Step Network](https://arxiv.org/abs/2511.14329)
*Dongchen Han,Tianzhu Ye,Zhuofan Xia,Kaiyi Chen,Yulin Wang,Hanting Chen,Gao Huang*

Main category: cs.CV

TL;DR: 提出Step by Step Network (StepsNet)解决深层网络的两个关键障碍：捷径退化和宽度限制，通过通道维度特征分离和逐步学习实现更好的深度扩展


<details>
  <summary>Details</summary>
Motivation: 虽然理论上深度网络具有指数级能力提升，但现有残差架构在继续加深时难以实现理论容量改进，需要更先进设计来释放深层网络潜力

Method: 沿通道维度分离特征，通过堆叠宽度递增的块让模型逐步学习，缓解捷径退化和宽度限制问题

Result: 在图像分类、目标检测、语义分割和语言建模等任务中一致优于残差模型
...
### [93] [ArchMap: Arch-Flattening and Knowledge-Guided Vision Language Model for Tooth Counting and Structured Dental Understanding](https://arxiv.org/abs/2511.14336)
*Bohan Zhang,Yiyi Miao,Taoyu Wu,Tong Chen,Ji Jiang,Zhuoxiao Li,Zhe Tang,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: ArchMap是一个无需训练的知识引导框架，用于3D口腔扫描的结构化理解，通过几何感知的牙弓展平模块和牙科知识库实现稳健的牙齿计数、分区、牙列阶段分类和临床状况识别。
...
Method: 提出几何感知的牙弓展平模块将原始3D网格标准化为空间对齐的多视图投影，构建包含层次牙齿本体论、牙列阶段策略和临床语义的牙科知识库来约束符号推理空间。
...
### [94] [Silhouette-to-Contour Registration: Aligning Intraoral Scan Models with Cephalometric Radiographs](https://arxiv.org/abs/2511.14343)
*Yiyi Miao,Taoyu Wu,Ji Jiang,Tong Chen,Zhe Tang,Zhengyong Jiang,Angelos Stefanidis,Limin Yu,Jionglong Su*

Main category: cs.CV

TL;DR: DentalSCR是一个用于口腔内扫描模型与侧位头颅X光片3D-2D配准的稳定轮廓引导框架，通过构建统一解剖坐标系和表面DRR投影，使用对称双向Chamfer距离优化，在临床条件下实现高精度配准。


<details>
  <summary>Details</summary>
Motivation: 传统基于强度的配准方法在真实临床条件下（如投影放大、几何畸变、低对比度牙冠等）表现不稳定，容易导致配准失败或解剖学上不合理的对齐。

Method: 构建U-Midline Dental Axis统一解剖坐标系，通过表面DRR投影生成X光样投影，使用对称双向Chamfer距离在分层粗到细调度下优化2D相似变换。

Result: 在34个专家标注临床案例上评估，显著减少了标志点误差（特别是后牙区域），下颌骨误差分布更集中，曲线级别的Chamfer和Hausdorff距离表现优异。
...
### [95] [ARC-Chapter: Structuring Hour-Long Videos into Navigable Chapters and Hierarchical Summaries](https://arxiv.org/abs/2511.14349)
*Junfu Pu,Teng Wang,Yixiao Ge,Yuying Ge,Chen Li,Ying Shan*

Main category: cs.CV

TL;DR: ARC-Chapter是首个基于百万级长视频章节数据训练的大规模视频分章模型，通过双语、时序定位和分层章节标注，在视频内容结构化方面取得显著性能提升。
...
Method: 通过结构化流程构建了双语英中章节数据集，统一了ASR转录、场景文本和视觉描述，形成从短标题到长摘要的多级标注；设计了GRACE评估指标，结合多对一段落重叠和语义相似度。

Result: ARC-Chapter在F1分数上比之前最佳方法提升14.0%，在SODA分数上提升11.3%，建立了新的最先进水平；在YouCook2等下游任务的密集视频描述任务中也表现出优异的迁移能力。

Conclusion: 数据规模（数据量和标注强度）的扩展带来了明确的性能提升，ARC-Chapter展示了大规模视频章节数据训练的有效性，为长视频内容结构化提供了有力解决方案。
...
TL;DR: 提出Image-Based Gaussian Splatting方法，通过结合标准3DGS渲染的基础颜色和从邻近训练图像学习得到的残差，提升渲染质量，特别是高频细节和视角相关效果。


<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅方法使用低阶球谐函数，难以捕捉空间变化的颜色和视角相关效果（如镜面高光）。现有方法要么使用全局纹理图（对复杂场景效果不佳），要么使用每个高斯的纹理图（存储开销大）。

Method: 将每个像素颜色建模为标准3DGS渲染的基础颜色与从邻近训练图像推断的学习残差的组合，促进准确表面对齐并实现高频细节和准确视角相关效果的渲染。

Result: 在标准NVS基准测试中，该方法在渲染质量上显著优于先前的高斯泼溅方法，且不增加存储占用。

Conclusion: Image-Based Gaussian Splatting是一种高效替代方案，利用高分辨率源图像进行精细细节和视角特定颜色建模，显著提升渲染质量。
...
Method: 设计双分支架构，清晰分支指导模糊分支：1) 轻量级特征恢复网络，使用清晰分支特征作为监督；2) 频率结构引导模块，丰富目标结构信息；3) 特征一致性自监督损失。

Result: 在IRBlurUAV基准测试中，JFD3实现了优越的检测性能，同时保持实时效率。

Conclusion: JFD3框架通过联合特征域去模糊与检测，有效提升了红外无人机目标在运动模糊条件下的检测能力。
...
TL;DR: 提出了一种肩部姿态评估算法，用于自动评估生物识别证件照片中肩部姿态是否符合国际标准要求。


<details>
  <summary>Details</summary>
Motivation: 生物识别身份证件的国际标准要求严格的姿态合规性，特别是肩部的正面展示，但现有自动化质量评估方法缺乏对此特定属性的定量评估手段。

Method: 使用常见姿态估计框架提供的两个肩部标志点的3D坐标，量化肩部偏航角和滚转角，开发了肩部姿态评估算法。

Result: 在121张肖像图像数据集上评估，SPE得分与人工标注标签显示出强Pearson相关性（r≈0.80），使用改进的错误与丢弃方法验证了其筛选不合格样本的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的驾驶代理缺乏强制执行交通规则的显式机制，且在检测小型安全关键物体（如交通灯和标志）方面存在困难。

Method: TLS-Assist将检测结果转换为结构化自然语言消息并注入LLM输入，强制关注安全关键提示。该框架即插即用、模型无关，支持单视角和多视角相机设置。

Result: 在CARLA的LangAuto基准测试中，TLS-Assist相比LMDrive和BEVDriver分别提升了14%和7%的驾驶性能，同时持续减少了交通灯和标志违规。

Conclusion: TLS-Assist通过显式交通规则执行机制有效增强了LLM驾驶代理的安全性和可靠性，是一个实用的增强框架。
...
TL;DR: BEDLAM2.0是一个新的数据集，用于改进从视频中推断3D人体运动的方法，特别是在世界坐标系中估计人体运动时。它比BEDLAM数据集更丰富多样，包含更多真实的相机运动、人体形状、动作、服装、头发、3D环境和鞋子。


<details>
  <summary>Details</summary>
Motivation: 从视频中推断3D人体运动是一个具有挑战性的问题，许多应用需要将人体运动估计在世界坐标系中。现有方法在这方面进展有限，主要是因为缺乏包含真实人体和相机运动的地面真实数据的丰富视频数据。

Method: 开发了BEDLAM2.0数据集，通过增加相机和相机运动的多样性和真实性，提高人体形状、动作、服装、头发和3D环境的多样性和真实性，并添加了BEDLAM中缺失的鞋子。

Result: BEDLAM2.0在训练估计世界坐标系中人体运动的方法时显著优于BEDLAM。比较在BEDLAM和BEDLAM2.0上训练的最先进方法，发现BEDLAM2.0显著提高了准确性。

Conclusion: BEDLAM2.0是一个重要的资源，对于训练3D人体姿态和运动回归器，特别是在世界坐标系中估计人体运动的方法，提供了显著改进的数据支持。
...
TL;DR: 提出了一种基于序数回归的糖尿病视网膜病变检测框架，在APTOS-2019数据集上实现了0.8992的QWK分数，创下新基准。
...
Result: 在APTOS数据集上获得了0.8992的QWK分数，在临床分级一致性方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 解决领域增量学习中传统方法面临的两难困境：统一视觉空间导致领域间干扰和语义扭曲，而隔离领域特定参数则造成知识碎片化。

Method: 使用语言锚定的视觉对齐方法，通过基于类名语义相似度的文本参考锚点来指导每个新领域的视觉表示，保持一致的相对几何结构。

Result: 在标准领域增量学习基准测试中，LAVA相比现有最先进方法取得了显著的性能提升。

Conclusion: LAVA通过语言锚定的相对对齐方法有效解决了领域增量学习中的知识保留和知识重用问题，为跨领域知识迁移提供了新思路。
...
TL;DR: 提出了一种名为Cranio-ID的新框架，用于颅骨到面部的跨模态匹配，通过YOLO-pose模型自动标注2D颅骨X射线图像和光学图像中的标志点，并使用图表示和跨注意力机制进行语义对应匹配。
...
Method: 1. 使用训练的YOLO-pose模型在2D颅骨X射线图像和光学图像上自动标注标志点；2. 将标志点表示为图结构，使用跨注意力和最优传输框架进行跨模态图匹配。

Result: 在S2F和CUHK数据集上的实验表明，该框架在可靠性和准确性方面均有显著提升，在法医学的颅骨到面部和素描到面部匹配中表现有效。

Conclusion: Cranio-ID框架提供了一种可靠且准确的颅骨标志点自动标注和跨模态匹配方法，在法医学应用中有重要价值。
...
TL;DR: 该研究通过模拟婴儿视觉发育阶段（灰度到彩色、模糊到清晰、保持时间连续性）训练自监督学习模型，发现这种'视觉饮食'策略能增强模型的鲁棒性，并表现出与生物发育相似的模式。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿视觉发育阶段的生态优势，为理解稳健视觉智能的出现提供逆向工程框架。

Method: 使用CATDiet策略训练自监督学习模型，包括灰度到彩色(C)、模糊到清晰(A)、保持时间连续性(T)，并在10个数据集上建立综合评估基准。

Result: 所有CATDiet变体在物体识别方面表现出增强的鲁棒性，模型展现出与生物发育对齐的模式，包括神经可塑性变化和类似婴儿视觉悬崖反应的行为。

Conclusion: 早期婴儿视觉体验的发育进程为理解机器中稳健视觉智能的出现提供了强大的逆向工程框架。
...
TL;DR: DIR-TIR框架通过对话精炼器和图像精炼器模块，实现交互式对话文本到图像检索，显著提高目标图像命中精度。
...
Method: 使用两个专门模块：对话精炼器主动询问用户提取关键信息生成精确描述；图像精炼器识别生成图像与用户意图之间的感知差距，减少视觉语义差异。

Result: 在多样化图像数据集上的综合实验表明，该对话式方法显著优于仅使用初始描述的基线方法，模块协同集成实现了更高的检索精度和更好的交互体验。

Conclusion: DIR-TIR框架通过多轮对话提供了优越的可控性和容错性，相比传统单查询方法显著提高了目标图像检索的准确性。
...
TL;DR: CompEvent是一个基于复数神经网络的框架，通过全流程融合事件数据和RGB帧来实现低光视频去模糊，在时间对齐和空间-频率学习方面具有创新性。
...
Method: 提出CompEvent框架，包含复数时间对齐GRU（使用复数卷积和GRU迭代处理视频和事件流）和复数空间-频率学习模块（在空间和频域进行统一复数信号处理）。

Result: CompEvent在解决低光视频去模糊任务上优于现有最先进方法。

Conclusion: 通过复数神经网络的全流程时空融合，能够最大化模态间的互补学习，显著增强低光视频去模糊能力。
...
TL;DR: 提出了一种物理引导的残差学习框架，通过预测BedMachine先验上的床厚度残差，从观测表面重建冰下床地形，在格陵兰岛两个子区域取得了优于其他方法的测试精度和结构保真度。
...
Method: 使用DeepLabV3+解码器和标准编码器（如ResNet-50），结合轻量级物理和数据项进行训练，包括多尺度质量守恒、流向总变分、拉普拉斯阻尼、厚度非负性、斜坡先验一致性项，以及由置信度图调制的雷达点掩码Huber拟合。

Result: 在两个格陵兰子区域，该方法在测试核心上实现了强大的精度和高结构保真度，优于U-Net、Attention U-Net、FPN和普通CNN。

Conclusion: 残差先验设计结合物理约束，产生了空间连贯、物理合理的床地形，适用于领域偏移下的操作制图。
...
TL;DR: Gaussian Spatial Transport (GST) 是一个利用高斯泼溅技术实现图像坐标空间到标注图概率度量传输的新框架，通过贝叶斯概率计算传输计划，并推导出用于标准网络优化的损失函数，显著提高了训练效率。
...
Method: 提出基于高斯泼溅的方法估计像素-标注对应关系，利用贝叶斯概率计算传输计划，并推导出衡量传输后差异的损失函数。

Result: 在人群计数和地标检测等代表性计算机视觉任务上的广泛实验验证了该方法的有效性。

Conclusion: GST框架相比传统最优传输方案，消除了训练过程中的迭代传输计划计算，显著提高了效率，同时保持了良好的性能。
...
TL;DR: SEED-SR是一种新的农场边界分割方法，通过条件潜在扩散模型和地理空间基础模型，在分割感知的潜在空间执行超分辨率，实现20倍尺度因子的分割图生成。


<details>
  <summary>Details</summary>
Motivation: 解决小农户农场边界分割的挑战：高分辨率图像更新频率低，低分辨率图像特征不足；现有超分辨率方法会平滑关键特征且无法满足大尺度因子需求。

Method: 使用条件潜在扩散模型和大型多光谱、多源地理空间基础模型，在分割感知的潜在空间执行超分辨率，而非像素空间。

Result: 在两个大型真实数据集上，实例分割指标相对提升25.5%，语义分割指标相对提升12.9%，优于现有最先进的参考超分辨率方法。

Conclusion: SEED-SR通过在分割感知潜在空间执行超分辨率，成功实现了20倍尺度因子的高质量农场边界分割，显著提升了分割性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决当前自动驾驶系统在泛化能力方面的限制，特别是处理未见场景或不同传感器配置时的性能下降问题。现有方法存在系统不一致或计算成本过高的问题。

Method: 引入RSD框架和RiskHead模块，从视觉语言模型中蒸馏因果风险估计到BEV特征，生成可解释的风险注意力图，使BEV特征学习更丰富的风险注意力表示。

Result: 在Bench2Drive基准测试中，RSD显著提升了在复杂和不可预测驾驶条件下的性能，感知和规划能力都得到明显改善。

Conclusion: RSD通过风险注意力蒸馏有效增强了自动驾驶系统的泛化能力，使系统能够更好地处理空间边界和风险对象，更接近人类驾驶行为。
...
TL;DR: 提出PAMM框架，利用状态空间模型增强多任务密集预测中的任务间交互，通过双状态空间参数专家整合任务特定参数先验，并在NYUD-v2和PASCAL-Context基准测试中验证有效性。
...
Method: 基于解码器的PAMM框架，利用状态空间模型的丰富可扩展参数，采用双状态空间参数专家整合任务特定参数先验，并通过多方向希尔伯特扫描构建多角度特征序列。

Result: 在NYUD-v2和PASCAL-Context基准测试上的广泛实验证明了该方法的有效性。

Conclusion: PAMM框架能够有效增强多任务密集预测中的任务间交互，通过状态空间模型和参数先验整合实现精确的多任务交互。
...
<details>
  <summary>Details</summary>
Motivation: LDCT图像质量因低剂量辐射而下降，现有方法往往过度平滑导致关键细节丢失，需要一种能够增强或保留感知相关特征的方法来帮助放射科医生诊断。

Method: 提出D-PerceptCT架构，包含视觉双路径提取器（整合预训练DINOv2模型的语义先验和局部空间特征）和全局-局部状态空间块（捕获长距离信息和多尺度特征），并设计深度感知相关性损失函数。

Result: 在Mayo2016数据集上的实验表明，D-PerceptCT在LDCT增强方面优于现有最先进方法，能更好地保留结构性和纹理信息。

Conclusion: D-PerceptCT通过结合人类视觉系统原理和语义感知，有效提升了LDCT图像质量，为放射科诊断提供了更清晰的图像。
...
TL;DR: 提出使用空中自然图像作为明确参考目标，通过图像转换构建合成数据集，解决水下图像恢复中高质量配对数据稀缺的问题。


<details>
  <summary>Details</summary>
Motivation: 当前水下图像恢复方法受限于高质量配对数据集的稀缺，现有基准数据集缺乏全局一致的颜色和真实监督信号。

Method: 基于非配对图像转换建立生成式数据框架，生成覆盖6种代表性水下退化类型的大规模合成数据集。

Result: 在6种代表性网络架构和3个独立测试集上的实验表明，使用合成数据训练的模型在颜色恢复和泛化性能上达到或优于现有基准。

Conclusion: 该研究为水下图像恢复和增强提供了可靠且可扩展的数据驱动解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频VAE忽略了帧间内容的相似性，导致潜在建模冗余。需要开发能够有效利用视频内容相似性的紧凑潜在表示方法。

Method: 将视频内容显式解耦为关键帧、运动和残差三个组件，为每个组件设计专用编码器，使用共享3D解码器保持时空一致性，采用解耦适应策略进行顺序训练。

Result: 大量定量和定性实验表明，DeCo-VAE在视频重建性能上表现优异。

Conclusion: 通过显式解耦视频内容和专用组件建模，DeCo-VAE能够实现紧凑且高效的视频潜在表示。
...
TL;DR: 提出了一种在共享空间中表示多个SDF的方法，结合泛化学习和过拟合学习策略的优势，以更紧凑的潜在表示恢复高保真几何细节。


<details>
  <summary>Details</summary>
Motivation: 现有的神经SDF在单个形状或场景上表现良好，但在分析多个具有高保真几何细节的SDF时存在障碍，主要由于潜在空间编码信息有限和几何细节丢失。

Method: 结合泛化学习和过拟合学习策略，提出新的采样策略来采样训练查询，提高训练效率并消除其他SDF影响造成的伪影。
...
TL;DR: 提出了一种无需物体先验的动态手-物体交互场景建模方法，通过交互感知的高斯模型和动态场来同时重建几何和外观，在动态3D高斯泼溅框架下实现了最先进的性能。
...
Method: 使用交互感知的手-物体高斯模型，引入可优化参数采用分段线性假设；将手部信息融入物体变形场构建交互感知动态场；采用渐进式策略分别处理动态区域和静态背景；设计显式正则化来稳定表示。

Result: 实验表明该方法超越了现有的基于动态3D-GS的方法，在重建动态手-物体交互方面达到了最先进的性能。

Conclusion: 提出的交互感知动态3D高斯泼溅方法能够有效建模复杂的手-物体交互场景，无需物体先验即可实现高质量的几何和外观重建。
...
TL;DR: 提出了ForensicFlow三模态取证框架，融合RGB、纹理和频率证据进行视频Deepfake检测，在Celeb-DF数据集上达到0.9752 AUC，优于单流基线方法。
...
Method: 采用三模态框架：RGB分支（ConvNeXt-tiny）提取全局视觉不一致性；纹理分支（Swin Transformer-tiny）检测细粒度融合伪影；频率分支（CNN+SE）识别周期性频谱噪声。使用注意力机制进行时间池化和自适应分支融合。

Result: 在Celeb-DF数据集上达到AUC 0.9752、F1-Score 0.9408、准确率0.9208，优于单流基线方法。消融实验验证了分支协同作用，Grad-CAM确认了取证关注点。

Conclusion: 这种全面的特征融合方法对细微伪造提供了优越的鲁棒性，多模态融合策略显著提升了Deepfake检测性能。
...
Method: 开发基于聚类的可解释性技术，通过聚类分析展示模型的全局行为模式，同时提供细粒度信息，结果聚类可进行可视化。

Result: 在前列腺癌检测模型上评估了该技术的性能，证明了其有效性，能够帮助理解模型并增强对其操作的信心。

Conclusion: 该方法能够促进数字病理模型在临床实践中的更快采用，通过提供全局视角和细粒度信息来增强模型的可解释性和可信度。
...
TL;DR: OmniZip是一个无需训练的音频引导音频-视觉令牌压缩框架，通过动态视频令牌剪枝和交错时空压缩方案，实现3.42倍推理加速和1.4倍内存减少。


<details>
  <summary>Details</summary>
Motivation: 现有令牌压缩方法尚未满足联合压缩多模态令牌的需求，而处理音频-视频令牌序列在OmniLLMs中形成了显著的计算瓶颈。

Method: 首先识别显著音频令牌，计算音频保留分数来捕捉信息密度，动态指导视频令牌剪枝；采用交错时空方案压缩视频令牌。

Result: 实现了3.42倍推理加速和1.4倍内存减少，同时保持性能且无需训练。

Conclusion: OmniZip有效解决了多模态令牌压缩问题，在保持性能的同时显著提升了计算效率。
...
TL;DR: XAttn-BMD是一个多模态深度学习框架，通过髋部X光图像和临床元数据预测股骨颈骨密度，使用双向交叉注意力机制整合多模态特征，在骨密度估计任务中表现优于基线模型。
...
Method: 提出XAttn-BMD框架，使用新颖的双向交叉注意力机制动态整合X光图像和临床元数据特征，采用加权平滑L1损失函数处理骨密度不平衡问题。

Result: 在Hertfordshire队列研究数据上的实验表明，该模型在回归泛化性和鲁棒性方面优于基线模型。交叉注意力融合比简单特征拼接减少MSE 16.7%、MAE 6.03%，增加R2分数16.4%。

Conclusion: 该研究证明了多模态数据通过交叉注意力机制整合的有效性，模型在临床相关骨密度阈值下的二分类筛查性能显示了其在真实场景中的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有的预测算法要么独立处理每个切片忽略3D结构，要么3D感知方法不可生成且扩展性差，需要一种可扩展的3D组织转录组学分析方法。

Method: 在共享特征空间中检索相邻切片上的形态对应斑点，将跨切片上下文融合到轻量级ControlNet中；引入结合学习ZINB先验和空间经验先验的3D一致先验；使用全局注意力块实现线性扩展。
...
TL;DR: 提出BioST-GCN双流模型，结合姿态和生物力学信息进行跌倒预测，在模拟数据上表现优异但存在模拟-现实差距，零样本泛化性能显著下降。
...
Result: 在模拟数据集上比基准模型提升5.32%和2.91% F1分数，但零样本泛化到未见受试者时性能降至35.9%。
...
TL;DR: 提出了一种名为\net{}的方法，通过立体几何-纹理对齐和伪特征增强几何一致性，在稀疏视图场景下实现更准确详细的表面重建和高质量新视角合成。


<details>
  <summary>Details</summary>
Motivation: 现有方法在稀疏视图场景下容易过拟合，导致表面重建质量下降和新视角合成性能降低。扁平高斯原语的各向异性加剧了稀疏视图下的过拟合问题。

Method: 引入立体几何-纹理对齐来连接渲染质量和几何估计，同时提出伪特征增强几何一致性，通过结合训练视图和未见视图来增强多视图几何一致性。

Result: 在DTU、BlendedMVS和Mip-NeRF360数据集上的广泛实验表明，该方法达到了最先进的性能。

Conclusion: 该方法能够重建更准确详细的表面，同时保持高质量的新视角渲染，有效缓解了稀疏监督导致的过拟合问题。
...
TL;DR: SLAM-AGS是一个用于计算细胞学的多任务预训练框架，通过联合优化弱监督相似性目标和自监督对比目标，在低见证率下显著提升下游任务性能。


<details>
  <summary>Details</summary>
Motivation: 计算细胞学面临两个主要挑战：实例级标签获取成本高且不可靠，以及见证率极低。需要开发能够在低监督信号下稳定学习的方法。

Method: 提出SLAM-AGS框架，联合优化弱监督相似性目标（在幻灯片负补丁上）和自监督对比目标（在幻灯片正补丁上），使用自适应梯度手术处理任务梯度冲突，防止模型崩溃。

Result: 在公开骨髓细胞学数据集上，模拟见证率从10%降至0.5%时，SLAM-AGS在袋级F1分数和前400个阳性细胞检索方面优于其他预训练方法，在低见证率下增益最大。
...
TL;DR: RepAir是一个用于3D气道分割的三阶段框架，结合nnU-Net网络和基于解剖学的拓扑校正，能生成更完整和一致的气道树结构。
...
Conclusion: RepAir框架能产生更完整和一致的气道分割结果，同时保持高分割精度。
...
TL;DR: 评估多种水印技术对对抗攻击的鲁棒性及其保持文本质量和写作风格的能力，发现这些技术能保持语义但偏离原始写作风格，且易受对抗攻击影响，特别是回译攻击。


<details>
  <summary>Details</summary>
Motivation: 为减轻大语言模型生成文本的潜在危害，研究人员提出水印技术，但现有技术常降低文本质量且易被对抗攻击移除，导致LLM创作者对广泛采用水印技术持抵制态度。

Method: 通过比较释义和回译（英语→其他语言→英语）攻击来评估多种水印技术的鲁棒性，并使用语言指标来评估其保持未加水印文本质量和写作风格的能力。

Result: 水印技术能够保持语义，但偏离未加水印文本的写作风格，且易受对抗攻击影响，特别是回译攻击。

Conclusion: 当前水印技术在保持语义方面表现良好，但在保持写作风格和对抗攻击鲁棒性方面存在不足，需要进一步改进以促进其广泛采用。
...
<details>
  <summary>Details</summary>
Motivation: 提升文本嵌入模型的语义推理能力，激活预训练期间学习到的语义推理潜力。
...
TL;DR: 本文提出了两种无需训练的QE-informed错误修正方法：一种是选择多个LLM生成的最佳翻译候选，另一种是基于QE解释替换错误子串。第一种方法在WMT 2025任务中获胜。


<details>
  <summary>Details</summary>
Motivation: 虽然联合训练QE和APE系统能提升性能，但APE系统存在过度修正MT输出的问题。本文旨在探索无需训练的简单方法来避免过度修正。

Method: 提出两种方法：1) QE-informed重翻译：从不同LLM生成的多个候选中选择质量最高的翻译；2) 基于QE解释的替换：根据提供的QE解释，指示LLM替换错误子串，并使用条件启发式最小化编辑次数。

Result: 两种方法的Delta COMET得分分别为0.0201和-0.0108，第一种方法在子任务排行榜上获得获胜位置。

Conclusion: 无需训练的QE-informed重翻译方法在避免过度修正方面表现优异，优于基于QE解释的替换方法，在WMT 2025任务中取得最佳成绩。
...
TL;DR: LLM框架能够解析电商搜索查询中的最高级表达，通过提取结构化提示来改进搜索性能，并在保持效率的同时将语义理解转移到轻量级模型。


<details>
  <summary>Details</summary>
Motivation: 带有最高级的搜索查询需要在多个维度比较候选结果，这需要语言理解和领域知识，而现有方法难以有效处理这类语义复杂性。

Method: 开发了一个框架，将查询分解为属性-值提示，这些提示与检索过程同时生成，并能够高效集成到排序管道中，同时将LLM的理解转移到轻量级模型以解决延迟问题。

Result: 方法在MAP上提升了10.9个百分点，在MRR上提升了5.9个百分点，显著优于基线方法。

Conclusion: 该研究展示了如何表示和传递最高级语义，在解决实际部署约束的同时，推进了检索系统中的语言理解能力。
...
TL;DR: MoRA-RAG是一个基于检索增强生成的知识驱动LLM框架，能够将灾后勘察报告转化为结构化知识库，用于多灾害推理，准确率高达94.5%，显著减少幻觉现象。
...
Method: 提出MoRA-RAG框架，集成混合检索机制动态路由跨灾害特定数据库的查询，使用智能分块保持检索中的上下文连贯性，并包含验证循环评估证据充分性、优化查询和启动针对性搜索。
...
TL;DR: 提出HiEAG框架，通过分层证据增强生成来改进多模态语境外虚假信息检测，强调外部一致性检查的重要性


<details>
  <summary>Details</summary>
Motivation: 现有OOC虚假信息检测方法主要关注内部一致性，忽略了图像-文本对与外部证据之间的外部一致性

Method: 使用分层证据增强生成框架，集成检索、重排序和重写三个模块，利用多模态大语言模型的知识进行外部一致性检查

Result: 在不同基准数据集上的实验结果表明，HiEAG在所有样本的准确率上超越了之前的最先进方法

Conclusion: HiEAG框架通过分层证据增强生成有效提升了多模态语境外虚假信息检测的性能，证明了外部一致性检查的重要性
...
TL;DR: 构建平衡的多标签情感数据集，并开发增强的多标签分类模型，显著提升情感分类性能
...
Method: 整合GoEmotions原始数据、使用RoBERTa-base-GoEmotions模型标注的Sentiment140样本，以及GPT-4 mini生成的人工标注文本，构建平衡数据集；开发结合FastText嵌入、卷积层、双向LSTM和注意力机制的多标签分类模型
...
TL;DR: 提出了一种名为Stealth Fine-Tuning的攻击方法，能够以低成本（仅需499个样本和3小时A100训练）有效绕过推理增强视觉语言模型的安全对齐防御，同时保持模型的一般推理能力。
...
Method: 通过段级干扰引发有害推理轨迹，将自生成的输出作为监督微调数据，采用轮次加权损失设计，实现轻量级、分布一致的微调方法。

Result: 在实验中，该方法在AdvBench和多个通用基准测试上表现优异，攻击成功率比IDEATOR高出38.52%，同时保持原始表示分布和一般推理能力。

Conclusion: Stealth Fine-Tuning是一种低成本、高效的方法，能够有效绕过对齐防御，暴露了推理增强视觉语言模型安全对齐的脆弱性。
...
<details>
  <summary>Details</summary>
Motivation: 临床文本自动ICD编码面临极端长尾分布的挑战，数千个罕见和零样本ICD代码在数据集中严重不足，导致宏观F1分数较低

Method: 构建基于罕见代码的现实多标签代码集，利用真实世界共现模式、ICD描述、同义词、分类法和相似临床笔记生成结构化提示，生成90,000条合成笔记覆盖7,902个ICD代码

Result: 在原始和扩展数据集上微调PLM-ICD和GKI-ICD模型，实验显示该方法适度提高了宏观F1分数，同时保持强微观F1分数，优于先前SOTA
...
TL;DR: HyperABSA：一种动态超图框架，通过样本特定的层次聚类构建方面-意见结构，改进了基于图的ABSA方法，在三个基准数据集上表现优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有的基于图的ABSA方法仅建模成对依赖关系，需要为不同关系视图构建多个图，这引入了冗余、参数开销和融合过程中的错误传播，在短文本、低资源设置下限制了鲁棒性。

Method: 提出动态超图框架HyperABSA，通过样本特定的层次聚类诱导方面-意见结构，并引入新颖的加速-回退截止方法来自适应确定层次聚类的粒度级别。

Result: 在三个基准数据集（Lap14、Rest14、MAMS）上的实验显示，相对于强大的图基线方法取得了持续改进，特别是与RoBERTa主干网络配对时获得了显著提升。

Conclusion: 动态超图构建是ABSA任务的高效、强大替代方案，并具有扩展到其他短文本NLP任务的潜力。
...
TL;DR: 提出一种结合Transformer关系抽取和知识图谱匹配的方法，用于回答填空式选择题，同时保持输出过程的可追溯性。


<details>
  <summary>Details</summary>
Motivation: 利用Transformer关系抽取动态生成知识图谱来表示输入句子的含义，解决传统知识图谱构建成本高且静态的问题，同时处理关系抽取方法可能生成错误信息的问题。

Method: 通过关系抽取将问题句子转换为关系图，然后在封闭世界假设下与事实正确的知识图谱进行验证，以评估句子的真实性。

Result: 实验结果显示该方法能正确回答约70%的问题，同时提供过程可追溯性，且问题类别对准确率有显著影响。

Conclusion: 该方法成功结合了动态知识图谱生成和验证，在保持可解释性的同时实现了较高的问答准确率。
...
Conclusion: 选择性弱到强泛化有助于超对齐，能避免不必要的弱监督使用。
...
TL;DR: 提出了首个基于符号语言学知识的幻觉定位框架，通过分析符号触发词在模型各层的处理过程来追踪幻觉发展。研究发现符号语义处理在早期层（2-4层）就出现严重不稳定性，特别是否定词引发灾难性方差，表明幻觉本质上是符号语言处理失败而非一般生成问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法无法理解符号性幻觉的来源，缺乏针对符号触发词（修饰语、否定、数字、例外、命名实体）如何驱动幻觉的系统性分析框架。

Method: 提出符号定位框架，利用符号语言学和语义知识，分析五个模型在HaluEval和TruthfulQA数据集上对符号触发词的处理过程，通过注意力方差分析追踪幻觉在各层的发展。

Result: 符号触发词的注意力方差在早期层（2-4层）爆炸性增长至临界不稳定状态，否定词引发灾难性方差水平。尽管模型规模增大，幻觉率仍保持高位（78.3%-83.7%），深层中符号语义触发词的注意力急剧下降。

Conclusion: 幻觉本质上是符号语言处理失败，而非一般生成问题。符号语义知识是理解和定位LLM中幻觉机制的关键。
...
TL;DR: DeepEL是一个将大语言模型深度整合到实体链接所有阶段的框架，通过自验证机制利用全局上下文信息纠正预测，在10个基准数据集上显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只将LLMs应用于实体链接的孤立阶段，未能充分利用其在整个过程中的能力，且孤立消歧无法达到最优性能。

Method: 提出DeepEL框架，将LLMs整合到实体链接的每个阶段，并引入基于全局上下文信息的自验证机制，使LLMs能够纠正自身预测并识别同一句子中实体间的连贯关系。

Result: 在10个基准数据集上，DeepEL显著优于现有最先进方法，整体F1分数平均提升2.6%，在领域外数据集上提升4%。

Conclusion: 深度整合LLMs能有效推进实体链接技术的最先进水平，自验证机制对于提升性能至关重要。
...
TL;DR: 提出ArbESC+多系统方法用于阿拉伯语语法纠错，通过组合多个模型生成修正建议，使用分类器选择最佳修正，在QALB数据集上取得优于单个模型的性能。
...
Conclusion: 这是首个整合阿拉伯语语言错误修正的多系统方法，为开发先进的阿拉伯语文本处理工具提供了实用步骤。
...
TL;DR: 构建大规模音乐相关语料库和领域优先数据管道，通过参考模型软评分进行质量控制和动态优化，提出MusicSimpleQA基准评估事实性，为音乐领域大模型提供可扩展的数据训练框架。
...
Method: 构建40B tokens的音乐相关自然语言语料库，采用领域优先数据管道进行过滤、清洗和去重，集成多源音乐文本与元数据，使用参考模型进行token级软评分和动态优化。

Result: 开发了MusicSimpleQA基准用于事实性评估，建立了可扩展的数据训练框架和可重用的评估工具。

Conclusion: 这项工作通过正确的语料库和训练目标，为音乐领域大模型的发展提供了有效的数据训练框架和评估方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有电影配音方法简化了工作流程，忽略了导演与演员之间的关键互动，而真实工作流程中导演会指导演员内化情感线索。

Method: 构建多模态参考素材库，使用情感相似性检索增强策略，开发渐进式图基语音生成方法，模拟演员内化情感知识的过程。

Result: 在V2C Animation基准数据集上的主客观评估验证了有效性，实现了情感表达能力的全面提升。

Conclusion: Authentic-Dubber成功模拟了真实配音工作流程，通过导演-演员交互学习显著改善了电影配音的情感表现力。
...
Result: 开源ASR在自发语音中表现良好但在嘈杂非母语对话中下降；多模态LLM对口音更鲁棒但在领域特定命名实体上表现不佳；专有模型在清晰语音中准确率高但存在国家和领域差异。
...
<details>
  <summary>Details</summary>
Motivation: 现有压缩方法忽视了训练过程中的熵冲突现象：压缩训练降低熵导致推理变短但探索受限，而精度目标增加熵使推理链变长，导致模型陷入局部困境

Method: 采用熵引导的训练框架：熵下降时引导模型进行简洁推理步骤；熵上升时在紧凑推理模式下加强探索以提高鲁棒性

Result: 在六个数学基准测试中，方法将推理长度压缩到原始的20%，同时保持甚至超过了基线准确率

Conclusion: 熵冲突是推理模型压缩中的关键问题，熵引导训练能有效解决此冲突，实现高效的推理压缩
...
TL;DR: 该论文提出通过预测语言化概率分布来增强LLM置信度估计的方法，该方法要求模型考虑答案空间中的所有候选答案而非单一猜测，从而促进深度推理。
...
Conclusion: 预测语言化概率分布能有效促进深度推理以进行置信度估计，该方法具有普适性和鲁棒性。
...
TL;DR: AraLingBench是一个全面人工标注的阿拉伯语语言能力评估基准，包含150个专家设计的多选题，涵盖语法、形态学、拼写、阅读理解和句法五个核心类别，用于评估35个阿拉伯语和双语大语言模型。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在阿拉伯语知识基准测试中得分很高，但可能只是表面熟练度，缺乏真正的语言掌握能力，需要通过专门的基准来评估其深层语法和句法推理能力。

Method: 创建包含150个专家设计多选题的基准，涵盖五个语言核心类别，评估35个阿拉伯语和双语LLMs，分析其结构语言理解能力。

Result: 当前模型在表面水平上表现出色，但在深层语法和句法推理方面存在困难，许多模型通过记忆或模式识别而非真实理解来获得高分。

Conclusion: AraLingBench揭示了知识基准高分与真正语言掌握之间的持续差距，为开发阿拉伯语LLMs提供了诊断框架，评估代码已在GitHub公开。
...
Result: 专有LLMs冲突检测能力强，开源模型中仅DeepSeek-R1表现类似；DeepSeek-R1和Claude-4.5-Sonnet分别以91.5%和87.3%的F1得分排名前二；LLMs很少明确通知用户冲突或请求澄清。

Conclusion: 当前LLMs在冲突处理方面存在关键缺陷，在设计指令遵循型LLMs时，这是未来需要改进的重要领域。
...
TL;DR: 本文提出了一种解决LLMs在化学领域应用中的"tokenization瓶颈"的方法，通过统一自然语言和分子结构的表示，在预训练LLM中扩展化学相关词汇并进行领域特定预训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在化学领域的应用经常受到"tokenization瓶颈"的阻碍，即基于通用文本训练的tokenizer会将化学表示（如SMILES）分割成语义无意义的子token。

Method: 采用目标词汇扩展策略，在预训练LLM的词汇表中添加化学相关token，然后在化学领域文本上进行持续预训练以整合新知识。

Result: 该方法在多个下游化学任务上表现出优越性能，证明了策略的有效性。

Conclusion: 通过统一自然语言和分子结构的表示，可以显著改善LLMs在化学领域的应用效果。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准测试在大型语言模型快速发展下面临性能饱和问题，高难度基准往往存在学科范围狭窄、答案格式简化、易受数据污染等问题，与真实科学探究存在保真度差距。

Method: ATLAS由领域专家开发，涵盖数学、物理、化学、生物、计算机科学、地球科学和材料科学七个核心科学领域，采用原创问题设计、跨学科整合、高保真答案和严格质量控制。

Result: 初步结果显示ATLAS能有效区分领先模型的先进科学推理能力。

Conclusion: ATLAS计划发展成为一个长期、开放、社区驱动的平台，为人工通用智能的进展提供可靠评估标准。
...
TL;DR: 提出标准化上下文校准(NCC)方法，解决LLMs中多令牌类标签的长度偏差问题，在多个数据集和模型上显著提升性能，F1得分最高提升10%。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在处理候选选项预测时存在标签偏差，现有校准方法忽视了多令牌类标签带来的偏差，特别是标签长度偏差问题。

Method: 提出标准化上下文校准(NCC)，在完整标签级别进行标准化和校准，扩展偏差缓解到更广泛任务如多项选择题回答。

Result: NCC在多个数据集和模型上实现统计显著改进，F1得分最高提升10%，结合上下文学习时对少样本示例选择不敏感，需要更少示例达到竞争性能。

Conclusion: 缓解完整标签偏差对于提高基于LLM方法的性能和鲁棒性至关重要，特别是在类标签自然包含多个令牌的现实应用中。
...
Result: 在八种越狱攻击策略上有效增强安全性，防止对良性查询的过度拒绝；在三个微调攻击数据集上实现稳健防御，同时保持良性微调的效用增益。

Conclusion: TSSF框架能同时缓解越狱和微调攻击，为教育LLM提供有效的安全防护，在安全性和实用性之间取得良好平衡。
...
TL;DR: MedBench v4是一个全国性的医疗AI基准测试平台，包含70多万个专家策划任务，评估了15个前沿模型，发现基础LLM在安全性和伦理方面表现较差，而基于相同骨干的智能体显著提升了端到端性能。


<details>
  <summary>Details</summary>
Motivation: 需要反映真实临床工作流程和安全约束的评估框架来评估医疗大语言模型、多模态模型和智能体的进展。

Method: 构建包含70多万个任务的云基准测试基础设施，任务经过多阶段细化和多轮临床医生评审，开放式回答通过校准到人类评分的LLM-as-a-judge进行评分。

Result: 基础LLM平均得分54.1/100（最佳：Claude Sonnet 4.5，62.5/100），安全性和伦理得分较低（18.4/100）；多模态模型表现更差（平均47.5/100）；基于相同骨干的智能体显著改善性能（平均79.8/100）。

Conclusion: 多模态推理和基础模型安全性仍存在差距，但治理感知的智能体编排可以显著提升临床准备度而不牺牲能力，该平台为医院、开发者和政策制定者提供了实用的医疗AI审计参考。
...
<details>
  <summary>Details</summary>
Motivation: 解决心理健康资源获取障碍，弥补专业治疗数据不足，通过AI技术提供可访问的心理支持，同时为研究提供合成治疗对话数据。

Method: 系统包含三个核心组件：基于检索增强生成的个性化对话助手、基于用户档案的合成治疗对话生成器、以及使用CrewAI实现的健康AI规划器生成周度自我护理计划。

Result: 系统在精心设计的健康场景中进行了评估，使用了自动LLM判断和人类用户研究，展示了其功能性和有效性。

Conclusion: 这项工作强调了NLP研究人员与心理健康专业人士跨学科合作的机会，推动人类-AI互动在健康领域的负责任创新。
...
TL;DR: 本文系统化地将MDP框架扩展到LLM智能体，并提出了Agent-R1训练框架，在Multihop QA基准任务上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前RL在LLM智能体训练中的应用仍处于早期阶段，缺乏专门针对LLM智能体定制的RL方法和灵活可扩展的训练框架。
...
TL;DR: LiveRAG是一个包含895个合成问答对的公开基准数据集，用于系统评估基于检索增强生成（RAG）的问答系统。该数据集源自SIGIR'2025 LiveRAG挑战赛，包含真实答案、支持证据、难度评分和区分度评分。


<details>
  <summary>Details</summary>
Motivation: 随着RAG在生成式AI解决方案中日益重要，需要系统评估其有效性的基准数据集。现有评估方法缺乏标准化和系统性。

Method: 基于SIGIR'2025 LiveRAG挑战赛使用的数据集构建，添加了真实答案、支持证据，并应用项目反应理论模型计算难度和区分度评分。

Result: 分析显示基准问题具有多样性、难度范围广泛，能有效区分不同系统的能力。数据集包含895个合成问答对。

Conclusion: LiveRAG基准将有助于推进RAG研究、进行系统评估，并开发更稳健的问答系统。
...
TL;DR: 本文探讨了文档级声明提取的评估方法，通过声明对齐和相似度计算来比较模型提取与人工标注的声明集，并在捷克和斯洛伐克新闻评论数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 文档级声明提取在事实核查领域仍具挑战性，现有的评估方法关注有限，需要可靠框架来评估模型提取性能并衡量标注者间一致性。

Method: 研究声明对齐技术，通过计算对齐分数来比较两个声明集的相似度，识别最佳对齐和评估方法。
...
TL;DR: 利用历史报纸的头版预告作为自然生成的摘要数据源，开发了自动收集方法，并创建了首个希伯来语多文档摘要数据集HEBTEASESUM。
...
Conclusion: 历史报纸的头版预告是获取多语言摘要数据的有效来源，自动收集方法可扩展至资源匮乏的语言。
...
TL;DR: 使用电子健康记录数据动态追踪AKI患者的临床演变，通过聚类分析识别15种不同的AKI后临床状态，发现每种状态都有不同的CKD发展概率，为高风险患者识别提供数据驱动方法。


<details>
  <summary>Details</summary>
Motivation: 急性肾损伤患者发展为慢性肾病的风险很高，但识别最高风险患者仍然具有挑战性，需要开发更好的风险分层方法。

Method: 使用电子健康记录数据，通过聚类分析识别AKI后临床状态，采用多状态模型估计状态间转移概率和CKD进展风险，通过生存分析识别不同AKI亚群中的CKD风险因素。

Result: 在20,699名AKI患者中，3,491人发展为CKD；识别出15种不同的AKI后临床状态，75%患者在研究期间保持单一状态或仅发生一次转移；发现传统和新型CKD风险因素在不同临床状态中影响各异。
...
TL;DR: 比较人类标注与多种大语言模型在政治偏见检测中的表现，发现RoBERTa与人类标注最一致，GPT在零样本设置下表现最佳


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在政治偏见检测任务中与人类判断的一致性程度，目前相关研究相对不足

Method: 构建手动标注的新闻文章数据集，评估标注一致性、偏见极性和模型间一致性，量化人类与模型在偏见感知上的差异

Result: 传统基于Transformer的模型中，RoBERTa与人类标签对齐度最高；生成模型如GPT在零样本设置下与人类标注整体一致性最强；微调后的RoBERTa模型在所有基线中准确率最高

Conclusion: 人类与LLMs在政治偏见感知上存在系统性差异，需要结合人类可解释性和模型可扩展性的混合评估框架
...
TL;DR: 多智能体系统结合视觉语言模型（VLM）通过将图表作为可验证检查点，实现自主科学发现，在宇宙学和天体化学案例中展示错误纠正和实时数据分析能力。


<details>
  <summary>Details</summary>
Motivation: 提升端到端自主科学发现的效率和准确性，通过VLM评估图表并生成领域特定评分标准，使智能体能够自我纠错并实时引导探索性数据分析。

Method: 使用VLM作为评判者，评估图表与动态生成的领域特定评分标准的一致性，使多智能体系统能够纠正自身错误并实时调整数据分析方向。

Result: 在10个数据驱动发现任务基准测试中，VLM增强系统达到0.7-0.8的pass@1分数，显著优于仅代码（0.2-0.3）和代码加文本（0.4-0.5）的基线方法。

Conclusion: VLM引导的多智能体系统能够有效提升自主科学发现的性能，提供可审计的推理轨迹，增强可解释性，并在无需人工干预的情况下适应新数据集。
...
TL;DR: 开发了RareSeek R1系统，通过领域专业临床语料库和临床验证推理集，结合指令调优、思维链学习和图基础检索，在罕见病诊断中实现最先进性能，与经验丰富的医生表现相当。
...
Method: 组装大型领域专业临床语料库和临床验证推理集，通过分阶段指令调优、思维链学习和图基础检索开发RareSeek R1。

Result: 在多中心电子健康记录叙述和公共基准测试中，RareSeek R1达到最先进准确性、稳健泛化能力，在噪声或重叠表型下保持稳定性。增强检索在叙述与优先变异配对时产生最大收益。人类研究显示与经验丰富医生表现相当。
...
TL;DR: 本研究通过定量建模验证了语言处理中的比较幻觉现象可以用贝叶斯推理和噪声通道理论来解释，模型成功预测了幻觉强度的细微差异和代词效应。


<details>
  <summary>Details</summary>
Motivation: 比较幻觉现象（如"More students have been to Russia than I have"）中人们会系统性地误判句子可接受性，需要验证噪声通道理论能否解释这种现象。

Method: 结合统计语言模型与人类行为数据，推导出合理解释的后验概率定量模型，直接预测幻觉强度。

Result: 模型不仅解释了比较幻觉效应的细微梯度，还解释了先前未解释的代词与完整名词短语than从句主语引起的效应差异。

Conclusion: 研究支持噪声通道推理作为语言处理现象的统一计算层面理论，为理解语言幻觉和非幻觉情境提供了统一框架。
...
### [164] [B2F: End-to-End Body-to-Face Motion Generation with Style Reference](https://arxiv.org/abs/2511.13988)
*Bokyung Jang,Eunho Jung,Yoonsang Lee*

Main category: cs.GR

TL;DR: B2F是一个生成与身体动作对齐的面部运动的模型，通过解耦内容和风格表示，使用对齐和一致性目标来生成与身体动作一致且符合风格参考的面部动画。


<details>
  <summary>Details</summary>
Motivation: 人类运动自然地整合了身体动作和面部表情，形成统一的感知。如果虚拟角色的面部表情与身体动作不协调，会削弱角色作为统一整体的感知。
...
Result: 评估显示B2F生成富有表现力和吸引力的面部动画，与身体动作和风格意图同步，减轻了不匹配线索带来的感知不协调，并在不同角色和风格上具有泛化能力。

Conclusion: B2F成功生成了与身体动作对齐且符合风格参考的面部动画，解决了虚拟角色面部表情与身体动作不协调的问题，提升了角色的整体感知一致性。
...
### [165] [FreeMusco: Motion-Free Learning of Latent Control for Morphology-Adaptive Locomotion in Musculoskeletal Characters](https://arxiv.org/abs/2511.14205)
*Minkwan Kim,Yoonsang Lee*

Main category: cs.GR

TL;DR: FreeMusco是一个无需运动数据的框架，通过联合学习潜在表示和控制策略来实现肌肉骨骼角色的能量感知和形态自适应运动控制。


<details>
  <summary>Details</summary>
Motivation: 解决在无法收集运动数据的情况下模拟角色运动的问题，利用肌肉骨骼模型作为强先验，实现无需运动捕捉的通用运动控制。

Method: 使用基于模型的强化学习，结合控制、平衡和生物力学目标，引入时间平均损失公式来捕捉自然步态的周期性结构，并通过随机化目标姿态和能量水平来鼓励行为多样性。

Result: 框架在人类、非人类和合成形态上都能泛化，自然出现不同的能量高效策略（如四足步态和双足步态），支持目标导航和路径跟随等下游任务。

Conclusion: 证明了无需运动捕捉也能实现多功能和自适应的运动控制，为在数据收集不可行的情况下模拟角色运动提供了新方向。
...
### [166] [Imagine in Space: Exploring the Frontier of Spatial Intelligence and Reasoning Efficiency in Vision Language Models](https://arxiv.org/abs/2511.13782)
*Xiaoxing Lian,Aidong Yang,Jun Zhu,Peng Wang,Yue Zhang*

Main category: cs.AI

TL;DR: SpatiaLite是一个合成基准测试，用于评估视觉语言模型的空间推理能力，发现当前先进VLMs主要依赖语言表示，在视觉中心任务上表现不足且效率低下，并提出了IDF框架来改进。


<details>
  <summary>Details</summary>
Motivation: 当前先进视觉语言模型在逻辑推理、问题解决等方面表现出色，但在空间推理（如心理旋转、导航、空间关系理解）方面仍面临重大挑战，需要系统评估其空间推理机制。

Method: 引入SpatiaLite合成基准测试，联合测量空间推理准确性和效率；提出Imagery Driven Framework (IDF)用于数据合成和训练，隐式构建内部世界模型。

Result: 发现三个关键发现：1）先进VLMs主要依赖语言表示，在视觉中心任务上表现不足；2）空间推理机制效率低下，token使用随复杂度快速增长；3）IDF框架能有效构建内部世界模型。
...
### [167] [KANGURA: Kolmogorov-Arnold Network-Based Geometry-Aware Learning with Unified Representation Attention for 3D Modeling of Complex Structures](https://arxiv.org/abs/2511.13798)
*Mohammad Reza Shafie,Morteza Hajiabadi,Hamed Khosravi,Mobina Noori,Imtiaz Ahmed*

Main category: cs.AI

TL;DR: 提出了KANGURA框架，基于Kolmogorov-Arnold网络进行三维几何感知学习，用于优化微生物燃料电池阳极结构设计。


<details>
  <summary>Details</summary>
Motivation: 现有预测模型难以捕捉复杂几何依赖关系来优化微生物燃料电池阳极结构，影响能量转换效率。

Method: 采用KAN网络进行函数分解，通过几何解耦表示学习分离结构变化，结合统一注意力机制动态增强关键几何区域。

Result: 在ModelNet40基准数据集上达到92.7%准确率，优于15个SOTA模型；在真实MFC阳极结构问题上达到97%准确率。

Conclusion: KANGURA为三维几何建模提供了鲁棒框架，为先进制造和质量驱动工程应用中复杂结构优化开辟了新可能。
...
### [168] [When AI Does Science: Evaluating the Autonomous AI Scientist KOSMOS in Radiation Biology](https://arxiv.org/abs/2511.13825)
*Humza Nusrat,Omar Nusrat*

Main category: cs.AI

TL;DR: 评估KOSMOS自主AI科学家在辐射生物学中的三个假设：DDR-p53相关性假设被否定，OGT关联性弱，CDO1是明显异常值，12基因签名预测效果一般。AI科学家能产生有用想法但需要严格验证。


<details>
  <summary>Details</summary>
Motivation: 评估自主AI科学家KOSMOS在辐射生物学问题上的表现，检验其生成假设和发现的能力，强调需要适当的零模型进行严格审计。

Method: 使用简单随机基因零基准评估KOSMOS在三个辐射生物学问题上的表现：DDR-p53相关性、OGT和CDO1预测辐射响应模块、12基因签名预测前列腺癌生存。

Result: DDR-p53假设不支持（rho=-0.40,p=0.76）；OGT关联弱（r=0.23,p=0.34）；CDO1是明显异常值（r=0.70,p=0.0039）；12基因签名一致性指数0.61（p=0.017）但效应大小不唯一。

Conclusion: KOSMOS产生了一个有充分支持的发现、一个可能但不确定的结果和一个错误假设，表明AI科学家能生成有用想法但需要严格的零模型审计。
...
### [169] [Causal computations in Semi Markovian Structural Causal Models using divide and conquer](https://arxiv.org/abs/2511.13852)
*Anna Rodum Bjøru,Rafael Cabañas,Helge Langseth,Antonio Salmerón*

Main category: cs.AI

TL;DR: 本文研究了将Bjøru等人的反事实概率边界计算方法从马尔可夫模型扩展到半马尔可夫结构因果模型的方法，解决了外生变量影响多个内生变量时的挑战。


<details>
  <summary>Details</summary>
Motivation: Bjøru等人的方法仅适用于马尔可夫模型，而半马尔可夫模型能够表示更复杂的混杂关系，因此需要扩展该方法以处理更一般的因果模型。

Method: 通过最小示例说明扩展挑战，提出替代解决方案策略，并进行理论和计算评估。

Result: 开发了适用于半马尔可夫SCMs的反事实概率边界计算方法，能够处理外生变量影响多个内生变量的情况。

Conclusion: 成功将反事实概率边界计算扩展到半马尔可夫模型，为更一般的因果推理问题提供了解决方案。
...
### [170] [Jailbreaking Large Vision Language Models in Intelligent Transportation Systems](https://arxiv.org/abs/2511.13892)
*Badhan Chandra Das,Md Tasnim Jawad,Md Jueal Mia,M. Hadi Amini,Yanzhao Wu*

Main category: cs.AI

TL;DR: 本文系统分析了智能交通系统中大型视觉语言模型在精心设计的越狱攻击下的脆弱性，提出了一种基于图像排版操纵和多轮提示的新型越狱攻击方法，并设计了多层响应过滤防御技术。


<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型在智能交通系统中应用广泛，但极易受到越狱攻击，这可能导致模型生成有害内容，对交通安全构成威胁。

Method: 构建交通相关有害查询数据集；开发基于图像排版操纵和多轮提示的越狱攻击；提出多层响应过滤防御技术；使用GPT-4和人工验证评估攻击和防御效果。

Result: 实验表明提出的越狱攻击方法能有效绕过模型安全机制，而多层防御技术能显著降低模型生成有害内容的概率。与现有技术相比，该方法在智能交通系统环境中展现出更严重的风险。

Conclusion: 智能交通系统中的大型视觉语言模型存在严重安全漏洞，需要加强安全防护措施，提出的防御技术能有效缓解越狱攻击风险。
...
### [171] [CORGI: Efficient Pattern Matching With Quadratic Guarantees](https://arxiv.org/abs/2511.13942)
*Daniel Weitekamp*
...
<details>
  <summary>Details</summary>
Motivation: 实时AI系统和数据库查询需要高效规则匹配，但传统方法在复杂匹配场景下会出现指数级时间和空间需求，导致系统性能下降甚至崩溃

Method: CORGI采用两步法：前向构建基础关系图，后向迭代生成匹配，避免传统RETE算法中β内存收集部分匹配的问题

Result: 在性能评估中，CORGI在简单组合匹配任务上显著优于SOAR和OPS5的RETE实现

Conclusion: CORGI算法通过消除完全冲突集的内存开销，为规则匹配系统提供了二次时间和空间复杂度的性能保证
...
<details>
  <summary>Details</summary>
Motivation: 由于实际捕捉事故触发场景几乎不可能，获取工作场所危险检测所需的真实图像数据集存在困难。
...
Conclusion: 该框架能够有效生成逼真的工作场所危险场景图像，为视觉模型训练提供了可行的数据解决方案。
...
TL;DR: 本文综述并实验验证了AI代理在音乐分析和教育中的应用，展示了从基于规则的模型到深度学习、多代理架构和RAG框架的发展历程，并通过双案例方法评估其教学意义。


<details>
  <summary>Details</summary>
Motivation: 整合AI代理在音乐分析和教育中的历史发展，评估其教学意义，并解决透明度、文化偏见和混合评估指标等关键挑战。

Method: 采用双案例方法：(1)在中学教育中使用生成式AI平台培养分析和创造技能；(2)设计用于符号音乐分析的多代理系统，实现模块化、可扩展和可解释的工作流程。

Result: 实验结果表明，AI代理在音乐模式识别、作曲参数化和教育反馈方面有效提升性能，在可解释性和适应性方面优于传统自动化方法。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型中的知识是静态的，难以适应不断变化的信息，现有方法在处理需要多步推理的复杂多跳问题时，面临可扩展性和检索效率的挑战。
...
TL;DR: Syn-STARTS框架使用LLM生成大规模分类案例，解决了真实世界数据收集困难的问题，生成的分类案例在质量上与人工整理的数据集难以区分。


<details>
  <summary>Details</summary>
Motivation: 在重大伤亡事件中，分类决策至关重要，但这类事件发生频率低，难以收集足够真实数据用于AI模型开发。

Method: 开发Syn-STARTS框架，利用大型语言模型生成分类案例，并与人工整理的TRIAGE数据集进行质量对比验证。

Result: 生成的分类案例在质量上与人工整理的数据集难以区分，且在绿色、黄色、红色、黑色四个分类等级上的评估结果高度稳定。

Conclusion: 合成数据在开发用于严重和关键医疗情境的高性能AI模型方面具有巨大潜力。
...
### [176] [AISAC: An Integrated multi-agent System for Transparent, Retrieval-Grounded Scientific Assistance](https://arxiv.org/abs/2511.14043)
*Chandrachur Bhattacharya,Sibendu Som*

Main category: cs.AI

TL;DR: AISAC是一个用于科学工程工作流的多智能体系统，基于LangGraph、FAISS和SQLite技术构建，具备透明性、溯源跟踪和科学适应性。


<details>
  <summary>Details</summary>
Motivation: 开发一个集成化的科学助手系统，支持透明的工作流、完整的溯源记录，并能适应不同科学领域的需求。

Method: 采用Router-Planner-Coordinator工作流架构，结合FAISS向量搜索和SQLite持久化存储，使用提示工程定义智能体角色并强制结构化JSON输出。

Result: 已在阿贡国家实验室的多个研究领域成功应用，包括废物转化产品和能源过程安全等专业部署，展示了跨领域适用性。

Conclusion: AISAC提供了一个可配置、透明且可追溯的多智能体科学助手框架，能够有效支持科学工程工作流。
...
### [177] [Making Evidence Actionable in Adaptive Learning](https://arxiv.org/abs/2511.14052)
*Amirreza Mehrabi,Jason W. Morphew,Breejha Quezada,N. Sanjay Rebello*

Main category: cs.AI

TL;DR: 该研究提出了一个由教师主导的反馈循环系统，将概念级评估证据转化为经过验证的微干预，通过三个保障机制（充分性、注意力预算、多样性）实现自适应学习。


<details>
  <summary>Details</summary>
Motivation: 解决自适应学习系统中诊断精准但干预薄弱的问题，避免帮助时机不当或内容不匹配的情况。

Method: 将干预分配形式化为带约束的二元整数规划，包含覆盖度、时间、难度窗口、先决条件和反冗余等约束条件。采用贪心选择、基于梯度的松弛方法和混合方法三种求解器。

Result: 在模拟和1204名学生的物理课程部署中，两种求解器都能在有限观看时间内为几乎所有学习者实现完整的技能覆盖。基于梯度的方法比贪心方法减少了约12%的冗余覆盖，并在难度上更加协调。
...
### [178] [Collaborative QA using Interacting LLMs. Impact of Network Structure, Node Capability and Distributed Data](https://arxiv.org/abs/2511.14098)
*Adit Jain,Vikram Krishnamurthy,Yiming Zhang*

Main category: cs.AI

TL;DR: 本文研究了LLM网络中的集体幻觉传播问题，通过结合平均场动力学和随机效用模型，分析了LLM网络在协作问答中如何传播幻觉，并提出了控制幻觉传播的理论框架。


<details>
  <summary>Details</summary>
Motivation: LLM在缺乏直接证据时容易产生幻觉，而在交互网络中这种幻觉会传播并影响原本准确的LLM，因此需要研究网络中的集体幻觉传播机制。

Method: 结合平均场动力学和随机效用模型，构建生成模型，将LLM建模为具有潜在状态（真实/幻觉）的节点，分析有向网络中的信息扩散。

Result: 为具有两种潜在状态的LLM网络提出了固定点存在性和唯一性的充分条件，分析了激励对固定点行为的影响，并在100个开源LLM网络上进行了实验验证。

Conclusion: 该研究为理解和控制LLM网络中的集体幻觉传播提供了理论框架和实验验证，有助于改善协作问答系统的可靠性。
...
### [179] [APD-Agents: A Large Language Model-Driven Multi-Agents Collaborative Framework for Automated Page Design](https://arxiv.org/abs/2511.14101)
...
<details>
  <summary>Details</summary>
Motivation: 移动应用页面布局设计耗时且需要专业技能，现有设计软件需要大量培训，跨页面协作设计还需要额外时间对齐标准和确保样式一致性。

Method: 提出APD-agents多智能体框架，包含OrchestratorAgent、SemanticParserAgent、PrimaryLayoutAgent、TemplateRetrievalAgent和RecursiveComponentAgent，通过智能体协作将用户描述转换为结构化数据并生成页面布局。
...
### [180] [PRISM: Prompt-Refined In-Context System Modelling for Financial Retrieval](https://arxiv.org/abs/2511.14130)
*Chun Chet Ng,Jia Yu Lim,Wei Zeng Low*

Main category: cs.AI

TL;DR: 提出了PRISM框架，这是一个无需训练的方法，通过系统提示、上下文学习和轻量级多代理系统来提升金融信息检索性能，在FinAgentBench数据集上取得了0.71818的NDCG@5分数。
...
Conclusion: PRISM的模块化、仅推理设计使其适用于现实世界用例，为金融信息检索提供了实用的解决方案。
...
TL;DR: 提出R3双进程思考框架，将大语言模型的泛化能力与VLN专用知识结合，通过Runner、Ruminator和Regulator三个模块实现高效导航。


<details>
  <summary>Details</summary>
Motivation: 解决VLN任务中LLM方法在空间理解、计算成本和推理延迟方面的不足，同时发挥LLM的常识知识和推理能力。

Method: 采用双进程框架：Runner是轻量级专家模型负责常规导航；Ruminator使用多模态LLM进行结构化推理；Regulator根据三个标准监控进度并控制思维模式。
...
### [182] [Beyond Accuracy: A Multi-Dimensional Framework for Evaluating Enterprise Agentic AI Systems](https://arxiv.org/abs/2511.14136)
*Sushant Mehta*

Main category: cs.AI

TL;DR: 提出CLEAR框架解决现有AI代理基准测试忽视企业关键需求的问题，包括成本效率、可靠性和运营稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有AI代理基准测试主要评估任务完成准确率，但忽视了企业部署所需的关键要求，如成本效率、可靠性和运营稳定性。

Method: 通过系统分析12个主要基准测试和对最先进代理的实证评估，提出CLEAR（成本、延迟、效能、保证、可靠性）整体评估框架。

Result: 在300个企业任务上评估6个领先代理，发现仅优化准确率的代理比成本感知替代方案贵4.4-10.8倍。专家评估确认CLEAR能更好预测生产成功率（ρ=0.83 vs 准确率评估的ρ=0.41）。
...
### [183] [HFL-FlowLLM: Large Language Models for Network Traffic Flow Classification in Heterogeneous Federated Learning](https://arxiv.org/abs/2511.14199)
...
<details>
  <summary>Details</summary>
Motivation: 解决5G和物联网环境下传统集中式机器学习在分布式数据和隐私保护方面的不足，以及现有联邦学习方法成本高、泛化能力差的问题。

Method: 提出HFL-FlowLLM框架，首次将大语言模型应用于异构联邦学习中的网络流量分类任务。

Result: 相比最先进的异构联邦学习方法，平均F1分数提升约13%；相比现有大语言模型联邦学习框架，在增加参与客户端数量时平均F1分数提升达5%，同时训练成本降低约87%。

Conclusion: HFL-FlowLLM在现代通信网络安全中具有巨大潜力和实用价值，展示了强大的性能和鲁棒性。
...
### [184] [Do Large Language Models (LLMs) Understand Chronology?](https://arxiv.org/abs/2511.14214)
*Pattaraphon Kenny Wongchamcharoen,Paul Glasserman*

Main category: cs.AI

TL;DR: 测试大语言模型对时间顺序的理解能力，发现模型在复杂时间排序任务中存在局限性，但增加推理预算能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 在金融和经济应用中，大语言模型经常被用于处理时间序列数据，但模型是否真正理解时间顺序这一基本问题尚未得到充分验证。

Method: 设计了一系列时间顺序任务，包括简单时间排序、条件排序（先过滤后排序）和时代错误检测，测试了GPT-4.1、Claude-3.7 Sonnet和GPT-5在不同推理强度下的表现。

Result: 随着序列长度增加，精确匹配率急剧下降，但排名相关性保持较高；条件排序中失败主要来自过滤步骤；时代错误检测相对最容易；增加推理预算能显著提升GPT-5的性能。

Conclusion: 当前大语言模型在时间顺序任务上存在局限性，但通过分配明确的推理预算可以改善性能，这对金融领域的实时应用具有重要意义。
...
### [185] [Listen Like a Teacher: Mitigating Whisper Hallucinations using Adaptive Layer Attention and Knowledge Distillation](https://arxiv.org/abs/2511.14219)
*Kumud Tripathi,Aditya Srinivas Menon,Aman Gaurav,Raj Prakash Gohil,Pankaj Wasnik*

Main category: cs.AI

TL;DR: 提出两阶段架构来减少Whisper模型在噪声条件下的幻觉错误：第一阶段使用自适应层注意力增强编码器鲁棒性，第二阶段通过多目标知识蒸馏抑制幻觉。


<details>
  <summary>Details</summary>
Motivation: Whisper模型在噪声环境下经常出现幻觉错误，而现有方法主要关注音频预处理或转录后处理，对模型本身的修改研究不足。
...
### [186] [DevPiolt: Operation Recommendation for IoT Devices at Xiaomi Home](https://arxiv.org/abs/2511.14227)
...
Result: 在实验中显著优于基线方法，所有指标平均提升69.5%。在小米家庭应用中部署一个季度，服务255,000用户，在线实验显示独立访客设备覆盖率提升21.6%，页面浏览接受率提升29.1%。

Conclusion: DevPiolt通过系统化的方法有效解决了物联网设备操作推荐的关键挑战，在实际部署中验证了其有效性和实用性。
...
### [187] [Enhancing Regional Airbnb Trend Forecasting Using LLM-Based Embeddings of Accessibility and Human Mobility](https://arxiv.org/abs/2511.14248)
...
<details>
  <summary>Details</summary>
Motivation: Airbnb等短租平台扩张对当地住房市场造成冲击，导致租金上涨和住房负担问题。准确预测区域Airbnb市场趋势可为政策制定者提供关键洞察以缓解这些影响。

Method: 使用滑动窗口方法预测1-3个月趋势，将结构化表格数据转换为基于提示的LLM输入生成区域嵌入，然后输入到RNN、LSTM、Transformer等时间序列模型中捕捉复杂时空动态。
...
### [188] [PathMind: A Retrieve-Prioritize-Reason Framework for Knowledge Graph Reasoning with Large Language Models](https://arxiv.org/abs/2511.14256)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的KGR方法存在两个关键局限：1）不加区分地提取推理路径，可能引入无关噪声误导LLMs；2）动态探索推理路径需要高检索需求和频繁LLM调用。
...
Conclusion: PathMind通过选择性引导LLMs使用重要推理路径，有效解决了现有方法的局限性，在知识图谱推理任务中实现了更忠实、可解释和高效的推理。
...
<details>
  <summary>Details</summary>
Motivation: 在数据驱动时代，全自动端到端数据分析对于发现可操作洞察至关重要。现有数据洞察代理在领域知识利用、分析深度和代码生成准确性方面存在不足，需要改进。
...
<details>
  <summary>Details</summary>
Motivation: 检验LLMs自动生成优化和约束编程模型的能力是否源于数据污染而非真正推理，因为许多标准问题可能已包含在训练数据中。
...
Conclusion: LLMs在模型生成方面的成功可能更多源于训练数据中的问题暴露，而非真正的推理能力，对上下文和语言变化敏感表明其理解有限。
...
TL;DR: 本研究探讨了在LLM对齐过程中考虑多元社会价值观的重要性，发现人口统计特征和设计参数对模型行为有显著影响，并比较了不同优化方法的效果。
...
Method: 收集美国和德国参与者的对齐数据，在不同社会群体偏好下微调多个大语言模型，并比较评级尺度、分歧处理方法和优化技术的影响。

Result: 发现系统性人口统计效应：男性参与者评分毒性低18%；保守派和黑人参与者评分情感意识高27.9%和44%。技术设计选择显示强烈影响：保留评分者分歧比多数投票毒性减少53%；5点量表比二元格式减少22%；DPO在多值优化中优于GRPO。
...
### [192] [A Neuro-Symbolic Framework for Reasoning under Perceptual Uncertainty: Bridging Continuous Perception and Discrete Symbolic Planning](https://arxiv.org/abs/2511.14533)
*Jiahao Wu,Shengwen Yu*

Main category: cs.AI

TL;DR: 提出了一种神经符号框架，通过显式建模和传播从感知到规划的不确定性，连接连续感知信号和离散符号推理。在桌面机器人操作任务中验证了有效性，在多个基准测试中表现优于POMDP基线。


<details>
  <summary>Details</summary>
Motivation: 解决在不确定性下运行的AI系统中，连接连续感知信号和离散符号推理这一基本挑战，为这两个抽象层次提供原则性连接。

Method: 结合基于transformer的感知前端和图神经网络关系推理，从视觉观察中提取概率符号状态，并使用不确定性感知的符号规划器在置信度低时主动收集信息。

Result: 在10,047个PyBullet生成的场景中，翻译器输出具有校准置信度的概率谓词（总体F1=0.68）。嵌入规划器后，在Simple Stack、Deep Stack和Clear+Stack基准测试中分别达到94%/90%/88%的成功率（平均90.7%），比最强POMDP基线高出10-14个百分点，规划时间在15毫秒内。

Conclusion: 该框架是通用的，可应用于任何需要从感知输入到符号规划进行不确定性感知推理的领域，并通过概率图模型分析建立了校准不确定性与规划收敛之间的定量联系。
...
### [193] [Rate-Distortion Guided Knowledge Graph Construction from Lecture Notes Using Gromov-Wasserstein Optimal Transport](https://arxiv.org/abs/2511.14595)
*Yuan An,Ruhma Hashmi,Michelle Rogers,Jane Greenberg,Brian K. Smith*

Main category: cs.AI

TL;DR: 提出基于率失真理论和最优传输几何的知识图谱构建与优化框架，用于从教育材料自动生成高质量选择题


<details>
  <summary>Details</summary>
Motivation: 将非结构化教育材料（如讲义和幻灯片）转换为捕捉关键教学内容的知识图谱仍然很困难

Method: 将讲义内容建模为度量-测度空间，使用融合Gromov-Wasserstein耦合对齐候选知识图谱以量化语义失真，通过精炼操作（添加、合并、拆分、移除、重连）最小化率失真拉格朗日量

Result: 应用于数据科学讲义的实验显示，从优化后知识图谱生成的选择题在15个质量标准上一致优于从原始笔记生成的选择题
...
### [194] [AutoTool: Efficient Tool Selection for Large Language Model Agents](https://arxiv.org/abs/2511.14650)
