...
TL;DR: 本文开发了一个分析框架来研究AI能力在有限时间内无界增长的条件，提出了基于可观测指标的测试方法来验证或排除AI奇点的可能性，并提供了实际可部署的安全控制措施。


<details>
  <summary>Details</summary>
Motivation: 研究AI系统在什么可测量的条件下会出现能力无界增长（失控增长），以及在什么条件下可以排除这种可能性，用可测试的条件取代推测。

Method: 开发了一个递归自我改进的分析框架，将能力增长与资源建设和部署策略联系起来，基于物理和信息论限制定义服务包络，建立内生增长模型来区分超线性和次临界状态。

Result: 推导出将可观测序列映射为失控与非奇异行为证书的决策规则，提供了基于功率限制、吞吐量节流和评估门等实际可实施的安全控制措施。

Conclusion: 该框架用可测试条件和可部署控制措施取代了对AI奇点的推测，为认证或排除AI奇点提供了理论基础和实践工具。
...
Method: 使用GPT-4o辅助的视觉语言流水线进行标注，结合人工验证，提供属性共现图和JSONL记录，并使用LoRA调优的Qwen2-VL-2B模型进行基准测试。

Result: 发布了包含标注和分析脚本的完整数据集，支持可解释的数据中心化工作流程，为未来智能车辆提供基准。

Conclusion: CAR-Scenes为自动驾驶视觉语言模型提供了全面的训练和评估框架，促进了可解释的场景理解能力发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有的文本到图像模型数据归属方法需要大量计算资源，使得它们在现实应用中不实用。

Method: 将缓慢的基于反学习的归属方法提炼到特征嵌入空间，结合高效索引和搜索方法，无需运行昂贵的归属算法即可找到高影响力图像。

Result: 在MSCOCO训练的中等规模模型和LAION训练的大型Stable Diffusion模型上均取得良好效果，比现有方法快2,500-400,000倍，仅需几秒钟。
...
Method: 通过德尔菲法达成关于通用工作流程描述符的共识，开发ColoWorkflow工具，并由独立评分者应用于多中心腹腔镜和机器人结直肠手术视频数据集。

Result: 达成了10个程序无关阶段和34个程序特定步骤的共识，工具在54个手术视频中表现出广泛适用性，评分者间信度中等（阶段K=0.71，步骤K=0.66）。

Conclusion: ColoWorkflow为微创结直肠手术建立了可复制的视频性能评估框架，支持跨机构基准测试和AI驱动的工作流程识别发展，有望标准化培训、加速能力获取和推进数据驱动的质量改进。
...
Method: 1) 基于扩散的训练-测试时增强策略重构多模态土地覆盖分布；2) 多模态小波解缠模块学习跨域不变特征；3) 共享和专用类别文本作为语言输入；4) 空间-频率感知图像编码器实现局部-全局特征重建；5) 多尺度空间-频率特征对齐模块构建统一语义空间。

Result: 大量实验表明，FVMGN相比最先进方法具有出色的多模态泛化能力。

Conclusion: 提出的FVMGN方法在遥感多模态泛化任务中表现优异，通过频率域分析和多模态特征对齐有效解决了遥感图像分类中的跨模态泛化问题。
...
Method: GFT使用轻量级图卷积网络从transformer的初始token化输入中学习动态图，通过跳跃连接和高效的跨注意力模块将图特征传递到更深层。

Result: 在物体分类和分割任务上的大量实验表明，GFT在相同领域内与现有方法竞争，同时减少了可训练参数。

Conclusion: GFT是一种针对点云数据的参数高效微调方法，能够显著减少计算和内存成本，同时保持性能。
...
TL;DR: 提出了一种基于LRP的剪枝方法，在少量数据情况下实现更高剪枝率同时保持更好的模型精度
...
Method: 使用层间相关性传播(LRP)技术进行模型剪枝，在少量数据情况下实现高效剪枝

Result: 相比现有方法，该方法在保持更好模型精度的同时实现了更高的剪枝率
...
Method: 短窗口滑动学习框架：将视频分割为1-2秒片段，使用大语言模型自动标注构建数据集，充分利用所有帧保持时间连续性

Result: 在RWF-2000数据集上达到95.25%准确率，在UCF-Crime长视频数据集上提升至83.25%准确率，显示出强泛化能力

Conclusion: 该方法在实时暴力检测中表现出色，具有强泛化能力和实时适用性，适用于智能监控系统
...
TL;DR: 提出MCN-CL方法，通过三重查询机制和难负样本挖掘解决多模态情感识别中的类别不平衡、动态面部动作单元建模复杂性和模态异构性问题，在IEMOCAP和MELD数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 多模态情感识别面临三大挑战：类别分布不平衡、动态面部动作单元时间建模复杂性、模态异构性导致的特征融合困难。随着社交媒体中多模态数据的爆炸式增长，急需构建高效的跨模态融合框架。

Method: 提出多模态交叉注意力网络和对比学习(MCN-CL)，使用三重查询机制和难负样本挖掘策略，在去除特征冗余的同时保留重要情感线索。

Result: 在IEMOCAP和MELD数据集上的实验结果显示，该方法优于最先进方法，加权F1分数分别提高了3.42%和5.73%。

Conclusion: MCN-CL方法有效解决了多模态情感识别中的模态异构性和类别不平衡问题，取得了显著的性能提升。
...
TL;DR: 提出了一种竞争性强且计算效率高的概率降雨临近预报方法，使用预训练的卫星视觉编码器结合轻量级概率头来预测4小时累积降雨量的离散经验CDF。


<details>
  <summary>Details</summary>
Motivation: 开发更准确和计算效率高的概率降雨临近预报方法，以改进短期天气预测能力。

Method: 将视频投影器(V-JEPA Vision Transformer)和轻量级概率头连接到预训练的卫星视觉编码器(DINOv3-SAT493M)，将编码器标记映射到离散经验CDF上，通过连续排名概率得分(CRPS)进行端到端优化。

Result: 在Weather4Cast 2025基准测试中，该方法取得了有前景的性能，CRPS为3.5102，相比最佳3D-UNET基线提高了约26%的效果增益。

Conclusion: 所提出的方法在概率降雨临近预报任务中表现出色，显著优于传统的3D-UNET方法，具有更好的预测准确性和计算效率。
...
TL;DR: 提出了一种名为YOLO-Drone的改进无人机目标检测模型，通过在YOLOv11的Head网络中引入GhostHead网络，在VisDrone数据集上实现了精度、召回率、F1分数和mAP(0.5)的全面提升，同时提升了推理速度。
...
Method: 基于YOLOv11n模型，在其Head网络中引入GhostHead网络进行改进，构建YOLO-Drone模型，使用VisDrone数据集进行实验验证。

Result: 相比原始YOLOv11，精度提升0.4%，召回率提升0.6%，F1分数提升0.5%，mAP(0.5)提升0.5%，推理速度也有显著改善。相比其他模型，在mAP(0.5)上优于YOLOv8(0.1%)、YOLOv9(0.3%)和YOLOv10(0.6%)。

Conclusion: YOLO-Drone是一个高性能的无人机目标检测模型，在准确性和速度方面均优于YOLOv11及其他主流检测模型。
...
TL;DR: PhaseWin是一种新颖的相位窗口搜索算法，用于对象级基础模型的忠实区域归因，通过分阶段粗到细搜索取代传统贪婪选择，在保持高忠实度的同时显著降低计算成本。
...
Method: 采用分阶段粗到细搜索策略，结合自适应剪枝、窗口化细粒度选择和动态监督机制，以近似贪婪行为但大幅减少模型评估次数。

Result: PhaseWin仅使用20%的计算预算就实现了超过95%的贪婪归因忠实度，在目标检测和视觉定位任务中持续优于其他归因基线方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的负向提示方法包含广泛的非ID特征，可能捕获重叠或误导性信息，导致次优结果。需要更精确地描述类别边界特征。

Method: 使用LLM初始化类别特定的正负提示，优化后正提示关注类内特征，负提示强调类别边界特征。采用基于图的结构聚合语义监督并传播到视觉分支。

Result: 在CIFAR-100和ImageNet-1K两个基准测试上，针对8个OOD数据集和5种不同LLM的广泛实验表明，该方法优于最先进的基线方法。

Conclusion: 所提出的正负提示监督方法能有效提升OOD检测性能，通过精确描述类别边界特征来改善视觉-语言模型的OOD检测能力。
...
TL;DR: 本研究评估了YOLOv11n和YOLOv12n两种轻量级模型在面部表情识别任务中的性能，发现在干净数据集上YOLOv12n表现更佳，而在噪声数据上YOLOv11n具有更高精度。


<details>
  <summary>Details</summary>
Motivation: 在非约束真实环境中，面部表情识别仍具挑战性，需要平衡性能与效率的轻量级模型。

Method: 将FER2013和KDEF两个分类数据集转换为目标检测格式，使用YOLOv11n和YOLOv12n模型进行评估，采用mAP 0.5、精确率、召回率和混淆矩阵等指标。

Result: YOLOv12n在干净KDEF数据集上获得95.6的mAP 0.5，在FER2013上mAP为63.8；YOLOv11n在FER2013上精确率达65.2，假阳性更少。

Conclusion: 轻量级YOLO模型能有效平衡敏感性与精确性，适用于实时资源受限的情感感知AI应用。
...
TL;DR: 提出HCD框架解决异构架构知识蒸馏问题，通过互补特征映射和子logit解耦蒸馏实现高效知识迁移


<details>
  <summary>Details</summary>
Motivation: 传统知识蒸馏方法主要针对同构架构，在异构架构（如ViT到ResNet18）中由于空间特征表示差异面临挑战，现有异构方法计算成本高或过度依赖logit对齐

Method: HCD框架包含互补特征映射器（CFM）将师生特征映射到共享logits，子logit解耦蒸馏（SDD）将共享logits分解为n个子logits，并使用正交损失确保多样性
...
TL;DR: 提出了FedBCS方法解决联邦学习中医疗数据特征异质性问题，通过频域自适应风格重校准和上下文感知双级原型对齐，实现更鲁棒的域不变原型学习。


<details>
  <summary>Details</summary>
Motivation: 现有联邦学习方法在处理医疗数据特征异质性时存在两个主要局限：1）不完整的上下文表示学习，仅关注最终层特征；2）层间风格偏差累积，忽略中间层的域特定偏差。

Method: FedBCS方法包含：频域自适应风格重校准来解耦内容-风格表示并学习最优风格参数；上下文感知双级原型对齐，从编码器和解码器的不同层提取域不变原型并与上下文信息融合。
...
<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在3D相关任务（如空间认知和物理理解）上表现不佳，这限制了其在机器人和具身智能等现实应用中的潜力。主要原因是3D任务与2D训练之间的模态差距。

Method: 设计了3D沙盒重建和感知流水线，包括四个阶段：使用抽象控制生成多视图先验、代理高程、多视图投票和聚类、以及3D感知推理。
...
TL;DR: DEFT-LLM是一个用于微表情识别的多模态大语言模型，通过多专家解耦实现运动语义对齐，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决微表情识别中的两个核心挑战：(1)静态外观和动态运动线索的纠缠阻碍模型关注细微运动；(2)现有数据集文本标签与面部肌肉运动不完全对应，存在语义鸿沟。

Method: 提出DEFT-LLM框架，包含Uni-MER运动驱动指令数据集和三个专家架构，将面部动态解耦为结构、动态纹理和运动语义三个独立可解释的表示。

Result: 在多个具有挑战性的MER基准测试中展示了最先进的性能，在局部面部运动的可解释建模方面具有特别优势。

Conclusion: 通过将指令对齐知识集成到DEFT-LLM中，该方法为微表情注入了有效的物理先验，同时利用大语言模型的跨模态推理能力，能够精确捕捉细微的情感线索。
...
Method: 使用视频图生成器将视频帧转换为结构化图，构建前向、后向和无向图；设计带双阈值图卷积机制的图内关系推理模块；提出语言引导的跨模态嵌入模块；将摘要生成建模为伯努利分布的混合模型，使用EM算法求解。

Result: 在多个基准测试中优于现有方法，推理时间和模型参数分别减少87.8%和91.7%。

Conclusion: LGRLN方法在视频摘要任务中表现出色，能够有效捕捉视频的语义结构，同时显著降低了计算复杂度。
...
TL;DR: 提出了TG-DFER框架，通过文本引导的弱监督方法改进动态面部表情识别，结合语义指导和连贯时序建模来解决MIL方法中的视觉多样性和时序复杂性挑战。


<details>
  <summary>Details</summary>
Motivation: 解决动态面部表情识别中的多对一标注问题，以及MIL方法面临的视觉表达多样性和时序动态复杂性挑战。

Method: 使用视觉语言预训练模型提供语义指导，引入视觉提示对齐文本情感标签与视觉特征，设计多粒度时序网络捕捉短期面部动态和长期情感流动。

Result: TG-DFER在弱监督下实现了更好的泛化能力、可解释性和时序敏感性。

Conclusion: 文本引导的弱监督框架有效提升了动态面部表情识别的性能，通过语义指导和连贯时序建模解决了MIL方法的局限性。
...
TL;DR: ERMoE是一种稀疏MoE架构，通过将专家重新参数化到学习到的正交特征基中，并用特征基得分（输入特征与专家基的余弦相似度）替代学习到的门控逻辑，解决了路由不稳定和专家利用不足的问题。


<details>
  <summary>Details</summary>
Motivation: 解决MoE架构中的两个核心挑战：路由器逻辑与专家内部结构不对齐导致的路由不稳定和专家利用不足，以及负载不平衡造成的瓶颈问题。标准解决方案（如辅助负载平衡损失）会削弱专家专业化并损害下游性能。

Method: 将每个专家重新参数化到学习到的正交特征基中，用特征基得分（输入特征与专家基的余弦相似度）替代学习到的门控逻辑，实现内容感知路由，将token分配直接与专家的表示空间相关联。

Result: 在ImageNet分类和跨模态图像-文本检索基准（如COCO、Flickr30K）上达到最先进精度，自然产生更平坦的专家负载分布。3D MRI变体（ERMoE-ba）将脑年龄预测准确率提高超过7%，并产生解剖学上可解释的专家专业化。

Conclusion: ERMoE为稀疏专家模型引入了一种新的架构原则，直接解决路由不稳定性，并在可扩展、可解释的专业化方面实现改进性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的CLIP模型在持续学习环境中面临挑战，特别是学习新类别的任务特定软提示时会产生严重的分类器偏差，因为文本原型会过度拟合到最近的类别。

Method: DMC采用两阶段框架：解耦视觉编码器适应和文本软提示优化，每个阶段训练时冻结另一个模态；DMC-OT进一步引入最优传输引导的校准策略来对齐内存统计量，并采用任务特定提示设计增强任务间可分性。

Result: 在CIFAR-100、Imagenet-R、CUB-200和UCF-101上的广泛实验表明，DMC和DMC-OT都达到了最先进的性能，DMC-OT进一步将准确率平均提高了1.80%。

Conclusion: 提出的解耦框架有效解决了CLIP在类增量学习中的分类器偏差问题，DMC-OT通过统计量校准进一步提升了性能，为跨模态持续学习提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频LLMs存在时间不一致性问题：帧时序的微小偏移会翻转注意力并抑制相关帧。这源于多模态RoPE扩展导致的逆傅里叶时间核产生帧级波动，扰乱了本应由原始查询键内积控制的注意力。

Method: 提出PAS方法：在不同注意力头应用相反的相位偏移，然后聚合它们的输出。该方法保持每个头的频谱幅度，同时通过聚合有效平滑时间核并降低相位敏感性，而不改变位置编码结构。

Result: 在多个视频理解基准测试中，在相同token预算下表现出持续改进，计算开销可忽略不计。PAS为视频LLMs提供了即插即用的鲁棒时间编码升级方案。

Conclusion: PAS通过平滑RoPE时间核实现了对微小时间偏移的Lipschitz稳定性，多相位平均在保持每个头频谱的同时衰减了高频波动，为视频LLMs提供了有效的时间编码鲁棒性解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型在零样本视觉任务中直接回答开放式查询效果不佳，需要一种通用的推理时设计方法来提升性能，而无需任务特定的训练。

Method: 工作流包含两个步骤：量化将开放式查询转换为具有明确候选答案的多项选择题；二值化将每个候选答案转换为真/假问题，通过确定性解析选择答案。

Result: 在指代表达式定位、空间推理和BLINK-Jigsaw等任务上，量化到多项选择题带来显著提升，真/假二值化进一步提供一致的性能增益，表明该方法具有通用性。

Conclusion: 该工作流提供了一种简单统一的推理时设计方法，强调推理时设计而非任务特定训练，为现有视觉语言模型提供了实用的零样本视觉性能提升路径。
...
TL;DR: 本文重新审视自回归模型在无损图像压缩中的应用，通过分层并行化和渐进适应框架，使纯自回归方法重新成为高性能且实用的解决方案。


<details>
  <summary>Details</summary>
Motivation: 自回归模型虽然是无损图像压缩的理论性能基准，但因计算成本过高而被认为不实用。本文旨在重新思考这一范式，证明精心设计的自回归框架可以在小参数数量和竞争性编码速度下显著优于现有方法。

Method: 提出分层并行自回归卷积网络（HPAC），采用分层因子化结构和内容感知卷积门控来高效捕捉空间依赖关系。引入缓存后选择推理（CSI）和自适应聚焦编码（AFC）优化实用性，并通过空间感知率引导渐进微调（SARP-FT）实现渐进适应策略。

Result: 在多样化数据集（自然、卫星、医学图像）上的实验验证了该方法实现了新的最先进压缩性能，为学习型无损压缩设立了新基准。

Conclusion: 精心设计的自回归框架可以在小参数数量和竞争性编码速度下提供显著优于现有方法的性能增益，重新确立了纯自回归作为顶级性能且实用的解决方案。
...
TL;DR: CLUE是一个生成模型框架，能够在无需额外数据的情况下，通过固定格式提示实现多样且稳定的图像生成，特别适用于医学等数据有限的领域。
...
Result: 在耳炎数据集上，FID降至9.30（原46.81），召回率提升至70.29%（原49.60%）。仅使用合成数据训练的分类器F1分数达83.21%（原73.83%）。
...
Method: 提出PROMISE框架，将多模态提示学习融入层次对比学习，配备专门设计的提示注意力机制，动态生成缺失模态下的鲁棒一致表示。

Result: 在基准数据集上的广泛实验和消融研究显示，PROMISE相比当前最先进的多模态方法具有优越性能。

Conclusion: PROMISE通过创新的提示注意力层次对比学习，有效弥合完整与不完整数据之间的表示差距，显著提升了模态缺失情况下的跨模态表示鲁棒性。
...
Method: 构建EmoVid数据集，包含情感标签、视觉属性和文本描述；开发基于Wan2.1模型的情感条件视频生成技术；分析视觉特征与情感感知的空间和时间模式。
...
Method: 采用结构化因果图显式建模人口统计学混杂因素，利用do-演算和碰撞结构从虚假的人口统计学相关性中解耦疾病相关信号，实现因果感知的特征聚合。

Result: 在三个基准数据集上达到最先进性能（CAMELYON16：ACC/AUC/F1 0.939/0.983/0.946），公平性显著提升——人口统计学差异方差平均降低65%以上，在生存预测任务中也表现优异。

Conclusion: MeCaMIL为数字病理学提供了一个公平、可解释且临床可操作的AI框架，因果图结构对性能至关重要，替代设计会导致准确率下降和公平性恶化。
...
TL;DR: 提出Draft and Refine (DnR)框架，通过问题条件化的利用率指标量化大视觉语言模型对视觉证据的依赖程度，并利用外部视觉专家提供针对性反馈来减少幻觉现象。
...
Method: DnR框架包含：1）构建问题条件化相关性图来定位问题特定线索；2）通过相关性引导的概率掩码测量依赖度；3）利用外部视觉专家输出（如边界框或掩码）作为视觉线索，重新查询模型选择利用率提升最大的响应。

Result: 在VQA和图像描述基准测试中，DnR框架实现了准确率的一致提升和幻觉现象的减少。
...
TL;DR: VisMem是一个受人类认知记忆理论启发的视觉语言模型框架，通过动态潜在视觉记忆来解决视觉处理瓶颈问题，在多个视觉基准测试中平均性能提升11.8%。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在处理复杂视觉任务时存在"视觉处理瓶颈"，容易失去视觉证据的根基并在长时间生成中缺乏情境化视觉体验。
...
TL;DR: SP-Guard是一种针对扩散式文本到图像模型的安全防护方法，通过估计提示词危害性并应用选择性引导掩码，仅对图像中的不安全区域进行引导，在提高生成安全性的同时最小化对内容的意外改变。


<details>
  <summary>Details</summary>
Motivation: 现有的推理时引导方法缺乏自适应性和选择性，无法根据提示词调整引导强度，也无法仅针对图像中的不安全区域进行引导，导致安全防护效果有限。

Method: 通过估计提示词的危害程度，并应用选择性引导掩码，仅对图像中被识别为不安全的区域进行引导，实现自适应和选择性的安全防护。
...
TL;DR: AirCopBench是首个评估多模态大语言模型在具身空中协作感知中表现的基准测试，包含14.6k+问题，涵盖4个任务维度，在40个MLLMs上评估显示协作感知任务存在显著性能差距。


<details>
  <summary>Details</summary>
Motivation: 当前缺乏评估多智能体协作感知的基准测试，而多无人机系统相比单传感器设置具有更好的覆盖范围、鲁棒性和协作能力，现有基准主要针对使用高质量单智能体图像的基础感知任务，无法评估MLLMs在更复杂的自我中心协作场景中的表现。

Method: 构建AirCopBench基准，包含来自模拟器和真实世界数据的14.6k+问题，涵盖场景理解、对象理解、感知评估和协作决策四个关键任务维度，通过模型、规则和人工方法在严格质量控制下生成大规模问题。
...
TL;DR: EmbryoDiff是一个基于扩散模型的两阶段框架，用于胚胎发育阶段的细粒度识别，通过多焦点特征融合和混合语义边界条件实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型未能利用胚胎发育的分布先验来提高准确性，且依赖单焦点信息导致胚胎表示不完整，容易在细胞遮挡下产生特征模糊。
...
TL;DR: 深度学习模型可以从正常胸部X光片中预测患者的健康保险类型（社会经济地位的强代理指标），准确率显著（AUC约0.67-0.68），表明医疗AI能够学习到隐藏在医学图像中的社会不平等信号。


<details>
  <summary>Details</summary>
Motivation: 揭示医学AI模型如何无意中编码社会不平等信息，挑战医疗图像是中性生物数据的假设，重新定义医疗AI公平性的目标。

Method: 使用最先进的架构（DenseNet121、SwinV2-B、MedMamba）在MIMIC-CXR-JPG和CheXpert数据集上训练模型预测健康保险类型，采用基于斑块的遮挡分析来定位信号来源。

Result: 模型能够从正常胸部X光片中显著预测保险类型，即使在控制年龄、种族和性别后信号仍然存在，信号分布在胸廓上部和中部区域而非局部化。

Conclusion: 深度学习网络可能内化了临床环境、设备差异或护理路径的微妙痕迹，学习社会经济隔离本身，这要求重新思考医疗AI公平性，不仅要平衡数据集，还要解构嵌入临床数据本身的社会指纹。
...
TL;DR: 提出了一种混合粒度缓存方法(HGC)，通过在不同计算阶段采用不同粒度的缓存策略来减少可控生成模型的计算开销，在保持视觉质量的同时显著提升生成效率。


<details>
  <summary>Details</summary>
Motivation: 现有的可控生成模型需要同时处理控制条件和内容生成，计算需求大，导致生成效率普遍较低。

Method: 采用混合粒度缓存策略：1）粗粒度缓存（块级）-基于特征重用在编码器-解码器块间动态绕过冗余计算；2）细粒度缓存（提示级）-在模块内重用跨注意力图并扩展到相邻步骤的相应模块计算。

Result: 在四个基准数据集上验证有效，特别是在COCO-Stuff分割基准上，计算成本(MACs)显著降低63%（从18.22T到6.70T），语义保真度损失控制在1.5%以内。
...
TL;DR: 本文提出MPCGNet用于息肉分割，通过耦合门机制解决小息肉漏检、边界模糊和图像噪声问题，在多个数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决息肉分割中的三个关键挑战：小息肉易漏检、息肉边界模糊、结肠镜图像噪声影响分割结果。

Method: 提出三个模块：CGMFE模块用耦合门提取多尺度特征并抑制噪声；WCAD解码器模块通过窗口交叉注意力恢复细节；DFA模块通过特征聚合和重要性选择减少小息肉丢失。
...
Method: 将CheXpert、SIIM-ACR、BraTS 2020和MosMedData四个数据集重新构建为统一的视觉问答格式，使用MAT框架评估模型在相关、不相关和零重叠三种设置下的表现。

Result: 在相关分割上模型达到83.2%分类准确率和0.75分割cIoU，但在不相关和零重叠条件下性能显著下降；跨任务迁移中分割性能提升7% cIoU。
...
TL;DR: 提出了SemanticNN语义编解码器，能够在严格的计算和通信约束下实现压缩和弹性的协作推理卸载，显著减少特征传输量56.82-344.83倍，同时保持优越的推理精度。


<details>
  <summary>Details</summary>
Motivation: 随着物联网的快速发展，在极弱嵌入式设备上集成AI引起了广泛关注，但这类设备的资源限制和不可靠的网络条件需要具有错误弹性的设备-边缘协作系统。

Method: 提出SemanticNN语义编解码器，包含BER感知解码器适应动态信道条件、基于软量化的编码器学习紧凑表示、特征增强学习训练策略，以及XAI不对称补偿解决编码器-解码器能力不匹配问题。

Result: 在STM32上使用三种模型和六个数据集进行实验，结果表明在不同传输错误率下，SemanticNN显著减少特征传输量56.82-344.83倍，同时保持优越的推理精度。

Conclusion: SemanticNN通过语义级正确性容忍比特级错误，在严格的计算和通信约束下实现了高效的设备-边缘协作推理卸载。
...
TL;DR: 提出了H²ARN网络解决文本-3D检索中的层次表示崩溃和冗余引起的显著性稀释问题，通过在双曲空间中嵌入数据和构建层次排序损失来提升检索性能。
...
Method: H²ARN在洛伦兹模型双曲空间中嵌入文本和3D数据，利用双曲空间的指数体积增长特性保持层次距离。使用层次排序损失构建收缩蕴含锥，确保匹配的3D实例落在锥内，同时结合实例级对比损失分离非匹配样本。针对RISD，提出贡献感知双曲聚合模块，利用洛伦兹距离评估局部特征相关性并进行加权聚合。

Result: 发布了扩展的T3DR-HIT v2基准数据集，包含8,935个文本-3D对，是原始大小的2.6倍，涵盖细粒度文化文物和复杂室内场景。

Conclusion: H²ARN通过双曲空间嵌入和层次对齐推理有效解决了文本-3D检索中的层次表示崩溃和冗余问题，提升了模型的检索性能。
...
TL;DR: PINGS-X是一个基于轴向对齐时空高斯表示的4D流MRI超分辨率框架，通过归一化高斯溅射、轴向对齐高斯和合并过程，显著减少了训练时间并提高了超分辨率精度。


<details>
  <summary>Details</summary>
Motivation: 4D流MRI需要高时空分辨率来早期检测心血管疾病，但高分辨率会导致扫描时间过长。现有的PINNs方法训练过程缓慢且需要为每个患者单独训练，限制了实际应用。

Method: 提出PINGS-X框架，使用轴向对齐时空高斯表示建模高分辨率血流速度。包括三个创新：归一化高斯溅射（具有收敛保证）、轴向对齐高斯（简化高维数据训练）、高斯合并过程（防止退化解并提高计算效率）。

Result: 在计算流体动力学和真实4D流MRI数据集上的实验表明，PINGS-X显著减少了训练时间，同时实现了优越的超分辨率精度。

Conclusion: PINGS-X通过创新的高斯表示方法，有效解决了4D流MRI超分辨率中的训练效率问题，在保持精度的同时大幅提升了训练速度。
...
Method: 首先通过奇异值分解提取主要风格方向，然后将主题LoRA投影到其正交零空间中，并引入软投影机制来控制主题保真度和风格一致性之间的权衡。

Result: 实验表明NP-LoRA在融合质量上持续优于强基线方法，在DINO和CLIP指标以及人类和LLM偏好评分上表现更好，且无需重新训练即可广泛应用于不同主干网络和LoRA对。
...
TL;DR: 提出多参考生成式图像合成方法，通过校准前景参考图像的全局和局部特征，使其与背景信息兼容，实现细节保持和前景姿态/视角调整的平衡。
...
Method: 扩展现有生成式合成模型为多参考版本，允许使用任意数量的前景参考图像，并校准参考特征的全局和局部信息以兼容背景。

Result: 在MVImgNet和MureCom数据集上的大量实验表明，校准后的参考特征能显著提升生成模型的性能。

Conclusion: 多参考生成式图像合成方法通过特征校准有效解决了细节保持与姿态调整的平衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 扩散变换器在视频生成中质量优异但存在二次方注意力复杂度问题，现有加速方法面临动态稀疏模式计算开销大或静态模式次优的权衡。

Method: 利用扩散注意力稀疏模式的时序一致性特性，提出LiteAttention方法，通过进化计算跳过机制，早期识别非重要tile并在后续步骤中跳过其注意力计算。

Result: 在FlashAttention基础上实现高度优化的LiteAttention内核，在生成视频扩散模型上实现显著加速，且无质量损失。

Conclusion: LiteAttention结合了动态方法的适应性和静态方法的效率，有效解决了扩散变换器的注意力计算瓶颈问题。
...
TL;DR: 这篇论文对2016-2025年间糖尿病视网膜病变(DR)的深度学习研究进行了系统性综述，整合了50多项研究和20多个数据集，分析了自监督学习、领域泛化、联邦学习等技术进展，并指出了多中心验证和临床信任等开放性问题。


<details>
  <summary>Details</summary>
Motivation: 糖尿病视网膜病变是可预防性失明的主要原因，早期检测对减少全球视力丧失至关重要。深度学习在过去十年中已经改变了DR筛查，但需要系统性地总结技术进展和临床转化障碍。

Method: 采用系统性综述方法，整合分析50多项研究和20多个数据集，考察自监督学习、半监督学习、领域泛化、联邦训练和混合神经符号模型等方法论进展。

Result: 提供了跨数据集的性能基准表，展示了各种方法在DR检测中的表现，同时识别了评估协议、报告标准和可重复性方面的挑战。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法仅通过监督微调进行实例级图像-文本对齐，无法建立解剖学基础的对齐，导致在模板化报告生成中质量不佳

Method: 采用浅到深策略：从粗粒度放射图像-报告配对开始，引入参考报告进行实例级指导，最终使用关键短语在特定解剖细节上建立基础；使用基于记忆的适配器实现特征共享
...
TL;DR: 比较扩散模型和自回归变换器在3D形状生成和补全任务中的表现，发现扩散模型在连续潜在空间中表现更好，而自回归模型在相同离散潜在空间下可达到或超越扩散模型性能。


<details>
  <summary>Details</summary>
Motivation: 当前3D生成模型缺乏统一评估标准，且条件信息（如部分3D数据）的使用不够充分，需要系统比较不同生成模型在形状建模和补全任务中的表现。

Method: 采用去噪扩散概率模型和自回归因果变换器，适配于生成形状建模和补全任务，包括基线判别模型和广泛消融研究。

Result: 扩散模型在连续潜在空间中优于判别模型和自回归方法，在单张噪声深度图像的多模态形状补全任务中达到最先进性能；在相同离散潜在空间下，自回归模型可匹配或超越扩散模型性能。
...
TL;DR: 提出了Phys-Liquid物理信息数据集，包含97,200张模拟图像和3D网格，用于透明液体几何和体积属性估计，通过四阶段重建流程验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 自主机器人执行精确液体操作任务时，容器移动引起的动态表面变形使得透明液体的几何和体积属性估计变得困难，现有数据集缺乏真实物理模拟数据。

Method: 开发了包含多种实验室场景、光照条件、液体颜色和容器旋转的物理信息数据集，并提出四阶段重建流程：液体分割、多视角掩码生成、3D网格重建和真实世界缩放。
...
TL;DR: 提出了一种高效计算基于B样条表示的3D体积层析投影的方法，使用光线追踪算法计算任意投影几何下的3D线积分，并通过神经网络高效计算基函数贡献。


<details>
  <summary>Details</summary>
Motivation: 传统基于体素的方法在重建质量上存在局限，需要一种更高效的3D体积层析投影计算方法。

Method: 使用线性组合的移位B样条表示3D体积，开发光线追踪算法计算任意投影几何下的3D线积分，并利用神经网络高效计算基函数贡献。

Result: 在数据充足的正定情况下，无需正则化即可实现比传统体素方法更高的重建质量。

Conclusion: 该方法在3D体积层析投影计算中比传统体素方法更高效且重建质量更高。
...
TL;DR: 提出了SaTformer视频变换器模型，使用全时空注意力机制从卫星辐射数据中预测极端降水，在NeurIPS Weather4Cast 2025累积降雨挑战赛中获第一名。


<details>
  <summary>Details</summary>
Motivation: 传统数值天气预报模型在临近预报（0-4小时）时性能下降，且计算成本高。AI天气预报方法虽然成功，但视频理解架构在天气预报中的应用仍待探索。

Method: 构建基于全时空注意力的视频变换器SaTformer，将降水回归问题重构为分类问题，并使用类别加权损失处理长尾数据分布。

Result: 在NeurIPS Weather4Cast 2025累积降雨挑战赛中获第一名。

Conclusion: SaTformer成功展示了视频变换器架构在极端降水预报中的有效性，为AI天气预报提供了新思路。
...
TL;DR: 本研究首次系统评估了数字重建X光片作为冠状动脉钙化检测的替代训练域，通过生成合成DRR图像并评估多种训练策略，证明了DRR可作为大规模CAC检测的标签丰富基础。


<details>
  <summary>Details</summary>
Motivation: CT是冠状动脉钙化检测的金标准但成本高，而胸片虽便宜但缺乏可靠标签，限制了深度学习发展。DRR通过将CT体积投影为X光样图像同时继承精确标签，提供了可扩展的替代方案。

Method: 使用667个CT扫描生成合成DRR，评估模型容量、超分辨率保真度增强、预处理和训练策略，包括轻量CNN从头训练、超分辨率与对比度增强配对、课程学习等。

Result: 最佳配置达到平均AUC 0.754，与或超过先前基于胸片的研究。轻量CNN从头训练优于大型预训练网络，超分辨率与对比度增强配对带来显著提升。

Conclusion: DRR为CAC检测提供了可扩展、标签丰富的基础，为未来向真实胸片的迁移学习和域适应奠定了基础。
...
Conclusion: PRISMA高光谱数据结合少样本学习为森林健康监测提供了显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 将强化微调(RFT)扩展到大型视频语言模型(LVLMs)面临挑战，需要专门的方法来处理视频推理的复杂性。

Method: 两阶段框架：1) SFT阶段使用三步流水线生成高质量的过程感知链式思维数据集；2) RL阶段引入过程感知组相对策略优化(PA-GRPO)算法，为感知和推理提供独立奖励。

Result: 在7个视频推理和理解基准测试中的6个达到最先进性能，消融研究验证了过程感知建模和PA-GRPO的有效性。

Conclusion: VideoP2R成功地将过程感知强化微调应用于视频语言模型，证明了感知输出对下游推理的信息充分性。
...
TL;DR: 本文综述了24篇关于AI生成媒体检测的研究，分析了现有方法的局限性，并建议未来研究方向应关注多模态深度学习模型，以提供更鲁棒和通用的检测能力。
...
Result: 现有检测方法（如CNN、ViT）在视觉、空间或时间异常检测方面存在不足，难以泛化到未见数据，对多模态数据和高度修改内容效果不佳。
...
Conclusion: LVGM成功实现了基于笔画的矢量中文字符生成，生成结果得到专家和相关人员验证，并发布了包含907,267个样本的大规模中文SVG数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有KBVQA方法要么使用MLLMs的隐式知识，要么通过检索增强生成使用显式知识，但它们的推理过程仍然是隐式的，缺乏明确的推理轨迹。

Method: 1. 通过提示冻结的7B规模MLLM完成问题与真实答案之间的推理过程，构建Hindsight-Zero训练数据；2. 将Hindsight-Zero自蒸馏为思维链生成器和知识生成器；3. 使用KEPO优化知识生成器，偏好低置信度但有用的知识。

Result: 在OK-VQA和A-OKVQA上的实验验证了HinD的有效性，仅使用7B规模MLLM的推理就能实现优越性能。
...
TL;DR: OT-ALD是一种基于最优传输理论的图像到图像翻译框架，解决了DDIB方法存在的翻译效率低和潜在分布不匹配问题，在保持循环一致性的同时提升了翻译效率和质量。
...
Method: 使用最优传输理论计算源域和目标域潜在分布之间的映射，将映射后的分布作为目标域反向扩散过程的起点，消除潜在分布不匹配。

Result: 在三个高分辨率数据集的四个翻译任务上，OT-ALD相比最佳基线模型平均提升采样效率20.29%，降低FID分数2.6。

Conclusion: OT-ALD有效平衡了更快的图像翻译和更好的图像质量，消除了潜在分布不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有轨迹预测方法缺乏对智能体响应延迟的显式建模，这会影响预测系统的因果连续性并导致不合理的轨迹。不同智能体对轨迹变化事件具有不同的延迟偏好。

Method: 受声学混响曲线启发，提出混响变换和Rev模型，使用两个可学习的混响核来模拟和预测每个智能体的延迟偏好及其随机性。
...
TL;DR: MultiTypeFCDD是一个轻量级卷积框架，用于可解释的多类型异常检测，仅需图像级标签即可学习生成多通道热图，每个通道对应特定异常类型，无需为每个对象类别训练单独模型。


<details>
  <summary>Details</summary>
Motivation: 现有可解释异常检测方法通常只能识别异常但无法区分异常类型，且需要为每个对象类别训练和维护单独模型。识别异常类型对于准确诊断和成本节约决策至关重要，而现有大型视觉语言模型计算密集、内存占用大，不适合实时或嵌入式系统。

Method: 提出MultiTypeFCDD框架，使用图像级标签学习生成多通道热图，每个通道对应特定异常类型。作为单一统一框架，能够跨多个对象类别区分异常类型。
...
TL;DR: CATS-V2V是首个针对复杂恶劣交通场景的V2V协同感知真实世界数据集，包含10种天气光照条件下的100个片段，提供LiDAR点云、多视角相机图像、RTK-GNSS/IMU数据及3D标注。


<details>
  <summary>Details</summary>
Motivation: 现有数据集主要关注普通交通场景，无法满足V2V协同感知在复杂恶劣条件下的需求，限制了协同感知的效益。

Method: 使用两辆硬件时间同步的车辆采集数据，覆盖10种天气光照条件，提供多模态传感器数据，并提出基于目标的时序对齐方法确保跨模态精确对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现代VQA系统在医疗诊断和自动驾驶等高风险领域应用广泛，但其置信度估计往往过于自信，可靠性不足，需要更好的校准方法

Method: 使用基于辩论的多智能体框架，让采用不同提示策略的专门化VLM生成答案，通过两阶段交互（通用智能体批判、精炼和聚合）来获得更准确的置信度估计，并引入可微分的校准感知损失函数aligncal

Result: 在多个基准VQA数据集上的实验结果表明，该方法显著减少了校准差异，更校准的专门化智能体产生了更好对齐的置信度

Conclusion: AlignVQA框架通过多智能体辩论和校准感知训练，有效提升了VQA系统的置信度校准性能，使其置信度估计更真实地反映预测准确性
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中多视角视频常因相机触发延迟或独立录制设置导致时序不同步，这会降低4D高斯泼溅等动态场景重建方法的质量。

Method: 采用粗到精的对齐模块，首先确定帧级粗偏移，然后细化到亚帧精度，可作为即插即用模块集成到现有4DGS框架中。

Result: 实验表明该方法能有效处理时序错位视频，显著提升基线方法的性能。

Conclusion: 所提出的时序对齐策略增强了4DGS框架在处理异步数据时的鲁棒性，实现了高质量的非同步多视角视频重建。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在资源受限场景（如机器人操作、个人助手、智能摄像头）中计算成本高的问题，以及小模型在细粒度视觉区域捕捉方面的不足。

Method: 使用液态状态空间动态替代Transformer交叉注意力机制，提出Token-Grid相关模块计算文本token和图像patch之间的轻量级相关性，通过FiLM条件调节状态空间动态。
...
TL;DR: 提出了一个高效框架，用于印度次大陆及周边地区PM1、PM2.5和PM10的6小时临近预报，使用轻量级参数专用架构在有限空间域上实现准确预测。


<details>
  <summary>Details</summary>
Motivation: 需要为印度次大陆及周边地区提供准确的颗粒物浓度短期预报，以改善空气质量监测和预警能力。

Method: 利用CAMS全球大气成分预报的分析场数据，开发三个轻量级参数专用架构，在256x256输入区域上训练，预测中心128x128区域。

Result: 在2024年独立测试数据上，使用RMSE、MAE、Bias和SSIM指标评估，相比Aurora基础模型取得了显著性能提升。

Conclusion: 紧凑且专业化的深度学习模型在有限空间域上进行短程预报具有显著效果，证明了轻量级架构的优越性。
...
TL;DR: 基于ConvGRU的迁移学习框架用于短期降雨预测，使用单一SEVIRI红外通道作为输入，采用两阶段训练策略生成未来4小时的降雨估计。


<details>
  <summary>Details</summary>
Motivation: 开发一个有效的短期降雨预测模型，用于Weather4Cast 2025竞赛，旨在提高降雨预报的准确性。

Method: 使用ConvGRU模型，采用两阶段训练：第一阶段预测SEVIRI亮度温度，第二阶段通过非线性变换将预测场映射到OPERA兼容的降雨率。对于事件预测任务，使用3D事件检测和时空特征提取。
...
<details>
  <summary>Details</summary>
Motivation: 现有VQA模型能够扩展解释遥感数据，但缺乏处理复杂地理空间查询所需的结构化推理能力，特别是在灾害监测、基础设施风险评估、城市韧性规划和政策支持等气候应用中。

Method: 集成思维链推理与直接偏好优化，通过生成中间推理过程来处理检测、分类、空间关系和比较分析等任务。

Result: 实验表明，思维链监督比直接基线准确率提高34.9%，而直接偏好优化进一步提升了准确率和推理质量。

Conclusion: 该系统通过实现更丰富的地理空间推理和更有效的气候用例，推动了多光谱地球观测的VQA发展。
...
TL;DR: 该论文首次系统研究了视觉语言模型对良性视觉和文本扰动的鲁棒性，发现即使是最先进的模型也对微小扰动高度敏感，且稳定性与正确性高度相关。
...
Method: 使用大规模系统研究，评估多种VLMs对像素级偏移、几何变换、缩放填充、改写和多语言重写等不改变语义的扰动的敏感性。

Result: 现代VLMs对微小扰动高度敏感，即使在几像素偏移或无害改写下也会频繁失败；样本稳定性与正确性高度相关，稳定样本更可能被正确回答。
...
Result: 在多个数据集和模型架构上系统验证了该结论，实现了高攻击成功率（高达100%），同时保持对干净数据的准确性。
...
TL;DR: MAFM^3是一个模块化框架，让单一基础模型通过轻量级模块组件扩展到医学影像的多个领域、任务和模态，实现高效的多任务多模态适应。


<details>
  <summary>Details</summary>
Motivation: 医学影像领域数据稀缺，为每个领域、模态或任务预训练模型具有挑战性。需要一种方法让单一基础模型能够灵活适应多种需求。
...
TL;DR: 提出GSD框架，利用视频扩散模型的先验知识来提升稀疏视图下3D高斯溅射的性能，通过深度扭曲和语义特征引导解决生成方向与真实相机位姿的对齐问题。


<details>
  <summary>Details</summary>
Motivation: 3D高斯溅射在稀疏训练视图下容易过拟合，缺乏中间视图监督，需要利用预训练视频扩散模型中的多视图一致性先验知识。

Method: 提出Guidance Score Distillation (GSD)框架，基于Score Distillation Sampling (SDS)，通过深度扭曲引导和语义特征引导来修正视频扩散模型的噪声预测结果，确保生成方向与正确相机位姿和准确几何对齐。

Result: 在多个数据集上的实验结果表明，该方法优于现有方法。

Conclusion: GSD框架有效利用视频扩散模型的先验知识，解决了稀疏视图下3D高斯溅射的过拟合问题，提升了多视图一致性。
...
TL;DR: 该研究探讨了多模态表示模型中的位置偏见问题，发现在图像-文本检索任务中，文本编码器偏向输入开头，图像编码器则同时偏向开头和结尾。


<details>
  <summary>Details</summary>
Motivation: 位置偏见在文本生成模型中已被广泛研究，但在多模态表示模型中的存在和影响尚未充分探索，特别是在多模态模型中。

Method: 区分上下文重要性和位置偏见，评估不同模型和数据集中的位置偏见程度，分析其产生原因。

Result: 多模态模型中普遍存在位置偏见，文本编码器偏向输入开头，图像编码器同时偏向开头和结尾。偏见源于位置编码方案、训练损失、上下文重要性等多因素组合。

Conclusion: 多模态表示模型中存在显著的位置偏见，且在不同模态中表现不同，这种偏见由多种训练因素共同导致。
...
TL;DR: DiT-Gaze是一个基于扩散变换器的高保真视线重定向框架，通过弱监督和正交约束损失提升视线重定向质量，在感知质量和重定向精度上都达到了新的最优水平。


<details>
  <summary>Details</summary>
Motivation: 现有的3D高斯溅射模型在渲染细微连续视线变化时存在困难，需要提升视线重定向的保真度以生成更好的增强数据来改进视线估计器的泛化能力。

Method: 结合扩散变换器(DiT)、跨视线角度的弱监督策略和正交约束损失。DiT实现更高保真度的图像合成，弱监督使用合成的中间视线角度在训练中提供平滑的视线方向流形，正交约束损失数学上强制视线、头部姿态和表情的内部表示解耦。

Result: DiT-Gaze在感知质量和重定向精度上都达到了新的最优水平，将最先进的视线误差降低了4.1%至6.353度，为创建合成训练数据提供了更优越的方法。
...
TL;DR: DoReMi是一个混合专家框架，通过联合建模领域感知专家分支和统一表示分支，解决多源点云异构性问题，在多个3D理解基准上取得优异性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D深度学习在多领域泛化受限，主要由于数据集规模有限和多源点云的高度异构性。不同传感器采集的点云在密度和噪声分布上存在显著差异，导致多领域融合时出现负迁移。现有方法通常只关注领域感知或领域通用特征，忽略了二者协同的潜力。

Method: 提出DoReMi框架，包含领域感知专家分支和统一表示分支。通过领域引导空间路由动态激活专家分支，使用熵控制动态分配实现稳定高效的专家利用。统一表示分支通过鲁棒多属性自监督学习预训练并冻结，保持跨领域几何和结构先验。

Result: 在多个3D理解基准上评估，DoReMi在ScanNet Val上达到80.1% mIoU，在S3DIS上达到77.2% mIoU，相比现有方法表现出竞争性或更优性能。

Conclusion: DoReMi展示了作为未来3D理解研究基础框架的强大潜力，能够有效处理多源点云的异构性，实现领域感知与通用表示的协同学习。
...
TL;DR: 提出了一种通过分组caption tokens来获取细粒度语言表示的方法，将语言表示与图像中对象的级别对齐，从而提升视觉语言模型的细粒度理解能力。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型主要关注图像块与语言token的对齐，但图像块对人类没有意义，单个token也不一定携带可接地的图像信息。需要捕获描述场景不同方面的token组来获得细粒度知识。

Method: 提出一种模型，在架构中分组caption tokens以捕获细粒度的语言表示，并将这些表示与经过训练发现对象的图像编码器输出对齐。

Result: 通过学习token分组，视觉语言模型对视觉和语言有了更好的细粒度理解。模型发现的token组与文本中的可接地短语高度相似，定性和定量评估都证明了这一点。

Conclusion: 通过分组caption tokens来学习细粒度表示的方法有效提升了视觉语言模型的细粒度理解能力，发现的token组与真实可接地短语高度一致。
...
TL;DR: 提出了一种新颖的少样本风格编辑框架，通过混合专家LoRA（MoE LoRA）机制，使用少量配对数据有效微调通用图像编辑模型以适应新风格。
...
Method: 构建包含五种不同风格的基准数据集；提出参数高效的MoE LoRA，包含风格特定和风格共享路由机制；通过度量引导方法自动确定各层最优秩；探索在DiT模型中插入LoRA的最佳位置；集成对抗学习和流匹配指导扩散训练过程。
...
TL;DR: 本文提出了一种通过控制合成数据生成来解决视觉语言模型微调中偏见和分布不平衡问题的方法，该方法能显著提升在真实数据上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 传统的视觉语言模型微调过程容易受到数据偏见、标注错误和分布不平衡的影响，导致过拟合和不平衡的性能表现。现有方法在控制分布偏见和标注质量方面存在不足。

Method: 重新设计微调过程：1）控制数据和标注的生成，确保无偏见、分布平衡且无标注错误；2）通过全面采样对象属性（颜色、形状、大小、位置）自动构建数据集；3）使用该数据集微调最先进的视觉语言模型，并在绝对位置任务上评估向真实数据的迁移性能。

Result: 实验表明：1）在平衡合成数据上微调能在整个视觉场景中产生均匀性能并缓解常见偏见；2）在合成刺激上微调显著提升了在真实数据（COCO）上的性能，优于在匹配设置下微调的模型。

Conclusion: 通过控制合成数据生成可以有效地解决微调过程中的偏见和分布不平衡问题，并能显著提升模型在真实世界数据上的性能表现。
...
### [78] [Beyond Flatlands: Unlocking Spatial Intelligence by Decoupling 3D Reasoning from Numerical Regression](https://arxiv.org/abs/2511.11239)
...
Method: 引入GEODE架构，包含两个即插即用模块：解耦推理模块（DRM）作为空间协处理器，通过交叉注意力对齐3D数据和2D特征；直接回归头（DRH）采用"嵌入即值"范式，通过轻量MLP精确回归标量和3D边界框。

Result: 1.5B参数模型作为高级语义调度器，实现了最先进的3D空间推理性能，可与7B+模型相媲美。

Conclusion: GEODE成功解决了视觉语言模型在3D空间理解中的双重瓶颈，证明了通过解耦架构设计可以在小参数模型上实现强大的3D推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 传统的Mamba选择性扫描操作在每个块重新初始化状态空间动态，丢弃了前一个块的终端状态空间表示，这限制了跨块信息的传递和模型性能。
...
Conclusion: Arcee通过跨块状态重用提供了一种有效的建模视角，将终端状态空间表示视为因果传递的方向性先验，显著提升了视觉Mamba模型的生成质量。
...
TL;DR: 提出了一种用于自闭症儿童注视目标检测的新框架SACF，通过社交感知的从粗到细方法解决数据集类别不平衡问题，在AGT数据集上实现了最先进性能。


<details>
  <summary>Details</summary>
Motivation: 通过AI自动检测自闭症儿童的注视目标，为缺乏专业人员的自闭症儿童改善生活质量，特别是测量联合注意力这一自闭症核心挑战。

Method: 提出社交感知从粗到细(SACF)框架，采用双路径架构，分别处理社交和非社交注视，通过上下文感知门模块指导，克服自闭症数据集中常见的类别不平衡问题。

Result: 在首个自闭症注视目标(AGT)数据集上，SACF框架实现了最先进的性能，显著优于现有方法，特别是在关键的少数类别——面部注视上表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型虽然能生成逼真连贯的图像，但往往无法准确遵循文本中的数值指令，这揭示了语言和视觉表示之间的差距。研究发现模型其实隐含地知道自己的计数准确性，其内部信号会根据输出是否满足指定数量而一致变化。

Method: 基于模型已编码数值正确性的潜在认知这一发现，提出了CountSteer方法。该方法在推理过程中引导模型的交叉注意力隐藏状态，无需额外训练即可提高对象数量生成的准确性。

Result: 实验表明，CountSteer将对象计数准确率提高了约4%，同时不损害视觉质量。

Conclusion: CountSteer展示了通过利用模型内部信号实现更可控和语义可靠的文本到图像生成的简单而有效的方法。
...
Method: 将不同抽象层次和格式的场景图序列化，通过结构化提示模板将其整合到模型中，分析关系监督何时以及如何最有效。
...
TL;DR: Φeat是一个基于物理的视觉骨干网络，通过自监督预训练策略学习对材料身份敏感的特征表示，分离高级语义与低级物理因素。
...
TL;DR: 提出了CORAL方法，通过协调学习局部和全局解剖结构，解决体积医学图像分割中解剖关系建模不足的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用硬阈值定义正负样本，丢弃了连续解剖相似性信息，且忽略了全局解剖进展的方向一致性，导致特征空间扭曲。

Method: 使用对比排序目标利用连续解剖相似性，确保切片间特征距离与解剖位置差异成比例；加入序数目标强制全局方向一致性，使学习特征分布与跨患者的标准解剖进展对齐。

Result: 在有限标注设置下，在基准数据集上达到最先进性能，学习到具有有意义解剖结构的表示。

Conclusion: CORAL通过协调学习框架成功捕获体积图像的局部和全局结构，为下游分割任务提供解剖信息丰富的表示。
...
TL;DR: D-GAP是一种数据集无关的梯度引导增强方法，通过在幅度空间和像素空间引入针对性增强来提高模型在领域外数据的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决现实计算机视觉应用中领域外鲁棒性挑战，传统增强方法在图像背景、风格和采集设备变化时效果不一致，且神经网络对领域特定频率分量存在学习偏差。

Method: 在幅度空间中计算任务梯度的敏感度图，反映模型对不同频率分量的响应强度，并自适应地在源样本和目标样本间插值幅度；同时在像素空间进行互补的混合过程以恢复空间细节。

Result: 在四个真实世界数据集和三个领域适应基准测试中，D-GAP始终优于通用和数据集特定的增强方法，在真实世界数据集上平均OOD性能提升+5.3%，在基准数据集上提升+1.8%。

Conclusion: D-GAP通过频率和像素空间的协同增强，有效减轻了模型对领域特定频率分量的学习偏差，显著提升了领域外鲁棒性。
...
TL;DR: RTGaze是一种实时高质量的视线重定向方法，通过可控制的视线表示和神经渲染实现3D一致的视线重定向，比现有3D感知方法快800倍
...
Conclusion: RTGaze实现了实时、3D感知的高质量视线重定向，显著提升了处理效率
...
<details>
  <summary>Details</summary>
Motivation: 当前基于深度学习的水印方法依赖手工制作的噪声模拟层进行训练，这限制了它们对未预见失真的泛化能力。需要一种更鲁棒的水印方法来应对传统信号处理和新型语义编辑攻击。
...
Conclusion: SimuFreeMark通过消除训练中的噪声模拟需求，提供了一种更通用和鲁棒的图像水印解决方案，特别适用于AIGC时代的需求。
...
TL;DR: AUVIC是一个用于多模态大语言模型的视觉概念遗忘框架，通过对抗性扰动实现精确的目标概念移除，同时保持相关实体的性能。
...
Method: 提出AUVIC框架，应用对抗性扰动实现精确遗忘，有效隔离目标概念并避免对相似实体的意外影响。构建VCUBench基准评估视觉概念遗忘效果。

Result: 实验结果显示AUVIC实现了最先进的目标遗忘率，同时对非目标概念的性能退化最小。

Conclusion: AUVIC为MLLMs中的视觉概念遗忘提供了有效的解决方案，能够精确移除目标概念而不显著影响模型整体性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决先进经济体水果采摘面临的高成本和季节性劳动力短缺问题，通过合成数据训练姿态估计算法来推进农业自动化。

Method: 采用YOLOX-6D-Pose单次检测算法，通过程序化Blender管道生成逼真的合成草莓数据用于训练。

Result: 在RTX 3090和Jetson Orin Nano上均达到可比的ADD-S精度，RTX 3090处理速度更快，Jetson Orin Nano适合资源受限环境。模型能准确估计成熟和部分成熟草莓姿态，但对未成熟草莓检测存在挑战。

Conclusion: 该方法可轻松适配其他水果如苹果、桃子、李子等，在农业自动化领域具有广泛适用性和扩展潜力。
...
TL;DR: DocSLM是一个高效的小型视觉语言模型，专为资源受限的边缘设备上的长文档理解设计，通过分层多模态压缩器和流式弃权机制，在显著减少内存消耗的同时保持性能。
...
Method: 使用分层多模态压缩器联合编码每页的视觉、文本和布局信息为固定长度序列，并引入基于熵的流式弃权机制来过滤低置信度响应。

Result: 在多个长多模态文档基准测试中，DocSLM匹配或超越最先进方法，同时减少82%视觉token、75%参数和71%延迟。

Conclusion: DocSLM能够在轻量级边缘设备上提供可靠的多模态文档理解，解决了大型模型部署的实用性限制。
...
<details>
  <summary>Details</summary>
Motivation: 解决事件相机视觉领域缺乏全面合成数据资源的问题，为6DoF物体姿态估计研究提供标准化数据集。

Method: 使用基于物理渲染(PBR)场景和BOP方法合成事件数据，采用模拟线性相机运动确保完整场景覆盖，系统评估不同事件表示方法对CNN推理的影响。

Result: 发现具有线性衰减的时间表面和双通道极性编码在姿态估计性能上表现最优，显著优于指数衰减和单通道替代方案，极性信息对性能提升贡献最大。

Conclusion: YCB-Ev SD数据集填补了事件相机视觉合成数据的空白，提供了结构化格式的原始事件流和预计算最优表示，便于研究和可重复基准测试。
...
TL;DR: Free3D是一个无需3D运动标注的3D人体运动生成框架，通过2D运动序列映射到3D一致潜在空间，并引入多种3D无关正则化目标，在仅使用2D数据训练的情况下实现与3D监督方法相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有3D人体运动生成模型虽然重建精度高，但难以泛化到训练分布之外，这源于精确3D监督使模型过度拟合固定坐标模式，而非学习本质的3D结构和运动语义线索。

Method: 提出Motion-Lifting Residual Quantized VAE (ML-RQ)将2D运动序列映射到3D一致潜在空间，并设计视图一致性、方向连贯性和物理合理性等3D无关正则化目标。

Result: 仅使用2D运动数据训练的Free3D能生成多样化、时间连贯且语义对齐的3D运动，性能与完全3D监督方法相当甚至更优。
...
<details>
  <summary>Details</summary>
Motivation: 从图像中提取数字材料表示是定量分析材料特性的必要前提。现有分割方法在精度或速度上存在不足，监督CNN需要大量标注数据，而无监督方法分割时间长且精度较差。

Method: 集成HMRF理论和CNN分割，利用HMRF的无监督学习特性和CNN的快速分割优势，研究不同邻域项和组件对无监督HMRF损失的影响。

Result: HMRF-UNet在PU泡沫结构的μCT图像数据集上实现了无需真实标签的高分割精度，并提出了一种预训练策略显著减少训练分割模型所需的标注数据量。

Conclusion: 该方法成功结合了HMRF和CNN的优势，实现了无监督高精度快速分割，并通过预训练策略降低了数据标注需求。
...
TL;DR: 提出LSEF框架，通过低秩稀疏原理分层解耦情感动态，包含三个模块分别捕获情感基、瞬态波动和一致性整合，显著提升视频情感计算的鲁棒性和动态判别能力。
...
Method: 提出LSEF框架，基于低秩稀疏原理，包含三个模块：稳定性编码模块（SEM）捕获低秩情感基，动态解耦模块（DDM）分离稀疏瞬态信号，一致性整合模块（CIM）重建多尺度稳定性与反应性一致性。

Result: 在多个数据集上的实验证实，LSEF显著增强了鲁棒性和动态判别能力。
...
TL;DR: 提出了MicroVQA++，一个三阶段构建的大规模高质量显微镜视觉问答数据集，通过专家验证、图结构过滤和人工筛选确保数据质量，使4B参数的多模态大模型在显微镜推理任务上达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 解决生物医学成像中多模态大语言模型在显微镜推理任务上训练数据稀缺且质量不高的问题。

Method: 三阶段构建方法：第一阶段从同行评议文章中获取专家验证的图-标题对；第二阶段使用HiCQA-Graph（融合NLI文本蕴含、CLIP视觉语言对齐和智能体信号的异构图）过滤不一致样本；第三阶段使用多模态大语言模型生成选择题并通过人工筛选。

Result: 构建了包含大规模训练集和人工检查测试集的数据集，其Bloom认知水平难度样本分布超过MicroVQA基准，使4B规模的多模态大模型达到与GPT-5竞争的性能，并在开源模型中达到最先进水平。

Conclusion: 通过精心构建的数据集，即使是较小规模的多模态大语言模型也能在显微镜推理任务上取得优异表现，证明了高质量数据对模型性能的重要性。
...
Result: 评估显示MLLMs具备初步的DIQA能力，但存在评分不一致、失真误识别和严重程度误判等关键限制，思维链提示能显著提升性能。
...
TL;DR: BOFA是一个用于类增量学习的框架，通过限制在CLIP的跨模态桥接层进行适配，无需额外参数，利用正交低秩融合防止遗忘，并结合跨模态混合原型提升分类性能。


<details>
  <summary>Details</summary>
Motivation: 解决类增量学习中应用CLIP模型的两个主要挑战：(1)适配下游任务需要额外可学习模块，增加模型复杂度和遗忘风险；(2)现有方法未能充分利用多模态表示在视觉和文本模态整合方面的潜力。

Method: BOFA框架将所有模型适配限制在CLIP现有的跨模态桥接层，采用正交低秩融合机制将参数更新约束在低秩安全子空间，该子空间与过去任务特征正交，并结合跨模态混合原型。
...
TL;DR: 本文提出了一个评估框架，用于区分文本到图像扩散模型中的文化引用识别与实现，重点关注多模态图标性现象，即模型如何平衡文化参考的识别与再现。


<details>
  <summary>Details</summary>
Motivation: 解决文本到图像扩散模型中泛化与记忆之间的模糊界限，特别关注多模态图标性——图像和文本唤起文化共享关联的情况，如标题唤起熟悉的艺术品或电影场景。

Method: 引入评估框架分离识别（模型是否识别引用）与实现（如何通过复制或重新诠释来描绘），量化这两个维度；评估五个扩散模型在767个维基数据文化引用上的表现；进行提示扰动实验测试语言敏感性。
...
TL;DR: 论文提出自适应教学范式来解决视觉-脑电信号对齐中的不对称性问题，通过让视觉模态调整其知识结构来匹配脑电模态的容量，在零样本脑-图像检索任务中达到60.2%的top-1准确率。


<details>
  <summary>Details</summary>
Motivation: 视觉与脑电信号之间存在根本的不对称性，包括保真度差距（EEG噪声和信号衰减vs视觉高保真特征）和语义差距（EEG浅层概念表示vs视觉丰富语义深度）。现有方法忽视这种不对称性，导致泛化能力差。

Method: 提出自适应教学范式，让"教师"模态（视觉）在任务指导下动态收缩和调整知识结构，使其语义密集特征匹配"学生"模态（EEG）的容量。实现为ShrinkAdapter模块，具有无残差设计和瓶颈结构。

Result: 在零样本脑-图像检索任务中达到60.2%的top-1准确率，比之前最先进方法提升9.8%。
...
### [100] [Benchmarking Visual LLMs Resilience to Unanswerable Questions on Visually Rich Documents](https://arxiv.org/abs/2511.11468)
*Davide Napolitano,Luca Cagliero,Fabrizio Battiloro*

Main category: cs.CV

TL;DR: 该研究提出了VRD-UQA基准，用于评估视觉大语言模型在视觉丰富文档中检测不可回答问题的能力，通过生成看似合理但实际无法回答的问题来测试模型的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 视觉大语言模型在视觉丰富文档的视觉问答任务中表现出色，但其检测不可回答问题的能力尚未得到充分研究。研究旨在评估模型对看似合理但实际无法回答的问题的鲁棒性。

Method: 通过替换原始自然语言实体生成不可回答问题，使用VLLM作为评判者验证问题的不可回答性，并在12个模型上进行多维度实验分析。

Result: 实验揭示了VLLMs在检测不可回答问题方面的局限性，并展示了不同知识注入策略的有效性差异。

Conclusion: VRD-UQA可作为开发鲁棒文档VQA系统的评估框架，帮助提升模型对不可回答问题的识别能力。
...
### [101] [Comprehension of Multilingual Expressions Referring to Target Objects in Visual Inputs](https://arxiv.org/abs/2511.11427)
*Francisco Nogueira,Alexandre Bernardino,Bruno Martins*

Main category: cs.CV

TL;DR: 本文构建了一个包含10种语言的多语言REC数据集，并提出了基于注意力锚定的神经网络架构，在RefCOCO上达到86.9%的多语言准确率。


<details>
  <summary>Details</summary>
Motivation: 解决当前REC研究主要集中于英语的问题，满足全球化部署需求，推动多语言视觉定位系统的发展。

Method: 通过机器翻译和上下文增强扩展12个英语REC基准，构建统一多语言数据集；提出基于注意力锚定的神经网络架构，使用多语言SigLIP2编码器生成空间锚点并通过残差学习进行精炼。

Result: 构建了包含约800万条多语言指代表达、177,620张图像和336,882个标注对象的数据集；在RefCOCO多语言评估中达到86.9%的IoU@50准确率，与英语单语言结果91.3%相比具有竞争力。

Conclusion: 多语言评估显示模型在不同语言间具有一致的能力，证实了多语言视觉定位系统的实际可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在文本描述模糊时存在随机性和不一致性问题，现有方法如提示重写和自优化需要额外模块且效率低下，阻碍了测试时扩展。

Method: ImAgent采用基于策略控制器的统一多模态代理框架，多个生成动作动态交互和自组织，无需外部模型即可增强图像保真度和语义对齐。
...
<details>
  <summary>Details</summary>
Motivation: 现有的统一多模态模型数据集主要关注单轮交互，无法捕捉真实世界图像创建和编辑的多轮、上下文依赖特性。

Method: 构建了WEAVE-100k数据集（10万个交错样本，覆盖37万对话轮次和50万图像）和WEAVEBench基准（100个任务，480张图像），采用基于参考图像和原始图像与编辑指令组合的混合VLM评判器评估框架。

Result: 实验表明，在WEAVE-100k上训练能够提升视觉理解、图像编辑和理解-生成协作能力，并促进UMMs发展出新兴的视觉记忆能力。

Conclusion: WEAVE为研究上下文交错理解和生成提供了视角和基础，同时揭示了当前方法在多轮、上下文感知图像生成和编辑方面的持续局限性和挑战。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型存在物体幻觉问题，研究发现模型在产生幻觉预测时往往忽略图像而依赖先前生成的标记来推断新物体。

Method: 引入Prelim Attention Score (PAS)，基于注意力权重计算信号，无需额外前向传播，可在推理过程中实时计算。

Result: PAS在多个模型和数据集上实现了最先进的物体幻觉检测性能，支持实时过滤和干预。

Conclusion: PAS是一种轻量级、无需训练的方法，能有效检测大型视觉语言模型中的物体幻觉问题。
...
TL;DR: Hi-DREAM是一种受大脑启发的条件扩散框架，通过将fMRI信号按视觉皮层层次结构分组，构建多尺度皮层金字塔，在图像重建中实现更好的语义理解和功能贡献分析。


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的解码器直接使用fMRI特征，忽略了大脑的层次处理结构和不同视觉区域的功能分工，导致视觉信息组织不清晰。

Method: 使用ROI适配器将fMRI分为早期/中期/晚期流，构建与U-Net深度对齐的多尺度皮层金字塔；通过轻量级ControlNet在去噪过程中注入尺度特定提示。

Result: 在NSD数据集上，Hi-DREAM在高层次语义指标上达到最先进性能，同时保持竞争力的低层次保真度。

Conclusion: 基于皮层层次结构的条件构建是纯数据驱动嵌入的强大替代方案，为研究视觉皮层提供了有用视角。
...
TL;DR: VP-Bench是一个评估多模态大语言模型理解和使用视觉提示能力的基准，包含感知测试和下游任务测试两个阶段，评估了28个模型并分析了影响VP理解的关键因素。
...
Method: 采用两阶段评估框架：第一阶段测试模型在自然场景中感知视觉提示的能力，使用3万个可视化提示，涵盖8种形状和355种属性组合；第二阶段评估视觉提示对下游任务的影响。

Result: 评估了28个MLLMs（包括GPT-4o、InternVL3、Qwen2.5-VL等），提供了关于VP属性变化、问题安排和模型规模等因素对VP理解影响的全面分析。
...
Result: 在未见数据集上实现了最先进的零样本性能，表现出强大的跨模态迁移能力、对语言变体和临床术语的鲁棒性，以及从真实世界文本进行实例特定分割的准确性。

Conclusion: VoxTell展示了视觉语言模型在3D医学图像分割中的有效性，为临床应用中基于自然语言描述的自动化分割提供了有前景的解决方案。
...
TL;DR: 提出了MaMOL框架，通过双路由机制将模态缺失问题重构为多任务学习问题，实现了参数高效的自适应，在多个遥感基准测试中表现出优越的鲁棒性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 遥感多模态分类常因环境干扰、传感器故障或大气效应导致模态缺失，严重降低分类性能。现有两阶段自适应方法计算成本高且假设训练时具有完整多模态数据，限制了其在真实世界不完整性下的泛化能力。

Method: 提出Missing-aware Mixture-of-Loras (MaMOL)框架，引入双路由机制：任务导向的动态路由器自适应激活不同缺失模式的专家，以及模态特定-共享的静态路由器维持稳定的跨模态知识共享。通过轻量级专家更新和共享专家重用实现参数高效自适应。
...
<details>
  <summary>Details</summary>
Motivation: 现有3D城市生成方法面临两个主要挑战：需要昂贵的大规模3D城市资产进行监督训练，以及依赖语义或高度图导致生成的城市缺乏真实感。

Method: 提出三个关键技术：基于OSM的空间先验策略实现可解释的几何生成；外观引导的可控建模机制实现精细外观控制；MLLM驱动的语义引导生成管道连接语义解释与几何重建。

Result: 大量定量和定性实验表明，Sat2RealCity在结构一致性和外观真实感方面显著优于现有基线方法。

Conclusion: 该框架为真实世界对齐的3D城市内容创建奠定了坚实基础，代码即将发布。
...
Method: 基于nnUNet-v2构建贝叶斯分割框架，使用多模态后验采样在循环训练中采样多样化模型检查点来近似后验分布

Result: 在肺鳞状细胞癌数据集上，平均Dice分数0.805，平均IoU 0.709，性能与现有基线方法相当，同时提供像素级不确定性图谱

Conclusion: 基于H&E的不确定性感知PD-L1预测是向临床工作流程中可扩展、可解释生物标志物评估迈出的有前景一步
...
TL;DR: OpenUS是首个基于大规模公共数据的可复现、开源超声基础模型，采用视觉Mamba骨干网络，结合对比学习和掩码图像建模的自适应掩码框架，在超过30.8万张超声图像上预训练，支持标签高效的微调。


<details>
  <summary>Details</summary>
Motivation: 超声图像解释高度依赖操作者，且因解剖区域、采集协议和设备类型而异，加上斑点噪声、低对比度和标准化标注有限等挑战，阻碍了泛化性强、标签效率高的超声AI模型发展。

Method: 使用视觉Mamba骨干网络捕获图像局部和全局长程依赖；提出自适应掩码框架，结合对比学习和掩码图像建模，集成教师注意力图和学生重建损失；采用动态学习进度表调整预训练难度；在42个公共数据集共30.8万张图像上预训练。

Result: 构建了迄今为止最大的公共超声数据集，覆盖多种解剖区域、机构、成像设备和疾病类型；开发了可复现的开源超声基础模型OpenUS。

Conclusion: OpenUS可作为骨干网络轻松适应特定下游任务，支持标签高效的微调，为超声AI模型开发提供了重要基础。
...
TL;DR: CVChess是一个深度学习框架，可将物理棋盘图像转换为FEN符号，然后输入在线象棋引擎提供最佳下一步走法


<details>
  <summary>Details</summary>
Motivation: 疫情期间在线象棋观众激增，但物理象棋游戏缺乏类似辅助工具，导致模拟与数字象棋体验之间存在鸿沟

Method: 使用带残差层的卷积神经网络进行棋子识别，包括霍夫线变换边缘检测、投影变换获得俯视视角、分割为64个方格、13类棋子分类

Result: 在包含10,800张智能手机图像的Chess Recognition Dataset上进行训练和评估

Conclusion: 该系统能够将物理棋盘转换为FEN符号，为象棋引擎提供输入以生成最优走法
...
<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型要么在编码器早期融合特征，要么在后期比较池化嵌入，但两种模态的隐藏状态已经包含了丰富的模态特定结构信息，直接对齐这些状态是更自然的融合方式。
...
Conclusion: BRIDGE提供了一种有效的跨模态融合方法，在保持编码器非因果性和理解能力的同时，通过轻量级模块实现更好的模态对齐。
...
### [114] [Spectral Neuro-Symbolic Reasoning II: Semantic Node Merging, Entailment Filtering, and Knowledge Graph Alignment](https://arxiv.org/abs/2511.10655)
...
<details>
  <summary>Details</summary>
Motivation: 提升谱神经符号推理框架的图质量，减少冗余节点和噪声边，增强推理的鲁棒性和可解释性。

Method: 1) 使用Sentence-BERT、SimCSE等上下文嵌入进行节点合并；2) 使用RoBERTa、DeBERTa等预训练NLI分类器进行蕴涵验证；3) 与ConceptNet、Wikidata等知识图谱对齐补充缺失上下文。

Result: 在ProofWriter、EntailmentBank和CLUTRR基准测试中，准确率提升最高达3.8%，对抗性案例泛化能力增强，推理噪声减少。

Conclusion: 通过在谱推理阶段之前进行语义和符号精炼，实现了高效、可解释且可扩展的推理系统，适用于开放领域和真实世界场景。
...
### [115] [Preference Orchestrator: Prompt-Aware Multi-Objective Alignment for Large Language Models](https://arxiv.org/abs/2511.10656)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖手动指定偏好权重，不仅增加用户负担，还会因探索无关偏好组合导致训练效率低下

Method: 使用轻量级偏好适配器，通过在多奖励模型的首选响应上训练，自动学习每个提示的适当偏好权重

Result: 理论分析证明该方法优于固定偏好权重，多任务实验验证了方法的有效性

Conclusion: PRO框架能够自动推断提示特定的偏好权重，在多目标对齐场景中取得优越性能
...
### [116] [Patent Representation Learning via Self-supervision](https://arxiv.org/abs/2511.10657)
*You Zuo,Kim Gerdes,Eric Villemonte de La Clergerie,Benoît Sagot*

Main category: cs.CL

TL;DR: 提出了一种基于专利文档内部多视图的对比学习框架，通过利用专利不同章节（摘要、权利要求、背景等）作为互补视图来学习专利嵌入，解决了SimCSE风格dropout增强在专利领域导致嵌入过于均匀的问题。


<details>
  <summary>Details</summary>
Motivation: 发现SimCSE风格的dropout增强在专利领域存在特定失败模式：会产生过于均匀的嵌入，丢失语义连贯性。需要利用专利文档固有的多章节结构来提供自然的语义和结构多样性。

Method: 提出基于章节的增强方法，将专利的不同章节（如摘要、权利要求、背景）作为互补视图进行对比学习，引入自然语义和结构多样性，缓解过度分散问题。

Result: 在大规模基准测试中，这种完全自监督的方法在先有技术检索和分类任务上匹配或超越了基于引用和IPC监督的基线方法，同时避免了依赖脆弱或不完整的标注。分析显示不同章节专门化于不同任务。
...
### [117] [Evaluating Open-Weight Large Language Models for Structured Data Extraction from Narrative Medical Reports Across Multiple Use Cases and Languages](https://arxiv.org/abs/2511.10658)
...
TL;DR: 评估15个开源LLM在病理学和放射学报告中的结构化信息提取能力，涵盖6种疾病、3个国家，比较不同提示策略。发现中小型通用模型表现接近大型模型，提示图和小样本提示可提升性能约13%。
...
Method: 使用15个开源LLM（包括通用和医学专用模型），在6种疾病（结直肠肝转移、肝脏肿瘤、神经退行性疾病等）的病理和放射报告中，比较6种提示策略：零样本、单样本、小样本、思维链、自一致性和提示图。

Result: 表现最佳的模型在各任务中的宏观平均得分接近评分者间一致性。中小型通用模型表现与大型模型相当，而微型和专用模型表现较差。提示图和小样本提示使性能提升约13%。任务特定因素比模型大小或提示策略影响更大。
...
### [118] [Information Extraction From Fiscal Documents Using LLMs](https://arxiv.org/abs/2511.10659)
*Vikram Aggarwal,Jay Kulkarni,Aditi Mascarenhas,Aakriti Narang,Siddarth Raman,Ajay Shah,Susan Thomas*

Main category: cs.CL

TL;DR: 使用LLM从多页政府财政文件中提取结构化数据的新方法，通过多阶段流水线处理复杂表格数据，利用层次关系进行验证，在印度卡纳塔克邦年度财政文件中实现高精度提取。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在文本理解方面表现出色，但处理复杂层次表格数据的能力尚未充分探索。传统OCR方法难以验证数字提取的准确性，特别是在财政数据中。

Method: 采用基于LLM的多阶段流水线，结合领域知识、顺序上下文和算法验证，利用财政表格固有的层次结构（每级都有总计）进行多级验证检查。

Result: 在200多页的卡纳塔克邦年度财政文件中实现高精度数据提取，证明LLM不仅能读取表格，还能处理文档特定的结构层次。
...
### [119] [Test-Time Steering for Lossless Text Compression via Weighted Product of Experts](https://arxiv.org/abs/2511.10660)
...
<details>
  <summary>Details</summary>
Motivation: 传统通用压缩器（如gzip）计算开销低、速度快、适用范围广，但压缩率不如现代神经压缩器；而神经压缩器虽然能更好地建模数据分布，但在未见数据上泛化能力差。需要结合两者的优势。

Method: 使用加权专家乘积（wPoE）框架，在推理时自适应地结合通用压缩模型和预训练神经语言模型，确保压缩率不低于最佳单个模型。

Result: 大量实验表明，该方法无需微调即可提高文本压缩性能，并能与任何自回归语言模型无缝集成。

Conclusion: 该框架为增强跨不同数据分布的文本压缩提供了一个实用解决方案，成功结合了传统压缩器和神经压缩器的优势。
...
### [120] [Bayesian Evaluation of Large Language Model Behavior](https://arxiv.org/abs/2511.10661)
*Rachel Longjohn,Shang Wu,Saatvik Kher,Catarina Belém,Padhraic Smyth*

Main category: cs.CL

TL;DR: 本文提出了一种贝叶斯方法来量化大型语言模型评估中二元指标的统计不确定性，重点关注由概率文本生成策略引起的不确定性。


<details>
  <summary>Details</summary>
Motivation: 当前基于LLM的文本生成系统评估往往忽视统计不确定性量化，特别是在使用二元指标（如有害/无害）评估模型行为时。

Method: 采用贝叶斯方法量化二元评估指标的不确定性，特别关注LLM概率文本生成策略引起的不确定性。通过两个案例研究进行验证。

Result: 贝叶斯方法能够为LLM系统行为提供有用的不确定性量化，在拒绝率评估和成对偏好比较中表现出有效性。

Conclusion: 贝叶斯不确定性量化方法能够改进LLM文本生成系统的评估，为模型行为提供更可靠的统计推断。
...
### [121] [Evaluating Modern Large Language Models on Low-Resource and Morphologically Rich Languages:A Cross-Lingual Benchmark Across Cantonese, Japanese, and Turkish](https://arxiv.org/abs/2511.10664)
*Chengxuan Xia,Qianye Wu,Hongbin Guan,Sixuan Tian,Yilun Hao,Xiaoyu Wu*

Main category: cs.CL

TL;DR: 评估7个前沿大语言模型在粤语、日语和土耳其语上的表现，结合人工评估和自动指标，发现大型专有模型表现最佳但仍有文化理解和形态学泛化差距，开源模型表现较差。
...
Method: 创建跨语言基准测试，涵盖四个任务：开放域问答、文档摘要、英译X和文化对话，结合人工评估（流畅度、事实准确性、文化适宜性）和自动指标（BLEU、ROUGE）。

Result: 大型专有模型（GPT-4o、GPT-4、Claude 3.5）在多数任务中领先，但在文化细微理解和形态学泛化方面仍有显著差距；开源模型在流畅度和准确性上大幅落后。

Conclusion: 需要开发更具文化意识和语言泛化能力的大语言模型，发布了基准测试数据以促进可重复性和进一步研究。
...
### [122] [Guarding the Meaning: Self-Supervised Training for Semantic Robustness in Guard Models](https://arxiv.org/abs/2511.10665)
...
<details>
  <summary>Details</summary>
Motivation: 防护模型对表面语言变化的敏感性是其关键脆弱点，即使意义保持不变的改写也会导致安全评分大幅波动，揭示缺乏语义基础

Method: 利用改写集通过新颖的偏斜感知聚合策略强制预测一致性，用于鲁棒目标计算

Result: 在六个开源防护模型上，该方法将语义变异性降低约58%，基准准确率平均提高约2.5%，并能泛化到未见过的风格变化

Conclusion: 将语义一致性作为首要训练目标具有重要价值，为构建更可靠的防护模型提供了可扩展方案，揭示了模型校准与一致性之间的双向关系
...
### [123] [Evaluating LLM Understanding via Structured Tabular Decision Simulations](https://arxiv.org/abs/2511.10667)
*Sichao Li,Xinyue Xu,Xiaomeng Li*

Main category: cs.CL

TL;DR: 提出了STaDS评估框架，通过结构化决策模拟测试LLMs的真实理解能力，发现大多数前沿模型在跨领域决策中表现不一致，且预测准确性与决策因素依赖之间存在不匹配。


<details>
  <summary>Details</summary>
Motivation: LLMs虽然预测准确率高，但准确率并不等同于真正的理解能力。需要评估模型是否能像人类专家一样，在多个实例和不同领域中做出一致且基于正确决策因素的决策。

Method: 引入STaDS框架，模拟专家决策考试，通过三个维度评估理解能力：(i)问题和指令理解，(ii)基于知识的预测，(iii)对相关决策因素的依赖。分析了9个前沿LLM在15个不同决策场景中的表现。

Result: 发现大多数模型难以在不同领域保持一致的强准确性；模型可能准确但在全局上不忠实，预测驱动因素与陈述理由之间存在频繁不匹配。

Conclusion: 需要超越准确率的全局理解评估协议，开发能增强LLMs理解能力的新框架。
...
### [124] [Forecasting Spoken Language Development in Children with Cochlear Implants Using Preimplantation MRI](https://arxiv.org/abs/2511.10669)
...
<details>
  <summary>Details</summary>
Motivation: 人工耳蜗植入显著改善重度至极重度感音神经性听力损失儿童的口语能力，但结果变异性较大，且无法通过植入年龄或残余听力可靠预测个体儿童的语言发展结果。
...
### [125] [Towards Fine-Grained Code-Switch Speech Translation with Semantic Space Alignment](https://arxiv.org/abs/2511.10670)
*Yan Gao,Yazheng Yang,Zhibin Lan,Yidong Chen,Min Zhang,Daimeng Wei,Hui Huang,Jinsong Su*

Main category: cs.CL

TL;DR: 提出了一种基于专家混合(MoE)语音投影器的LLM增强方法，用于解决代码切换语音翻译中的语义建模复杂性和数据稀缺问题。
...
Method: 使用MoE语音投影器增强LLM，每个专家专门处理特定语言的语义子空间；采用多阶段训练范式，利用单语ASR和ST数据；结合语言特定损失和组内负载均衡损失来指导MoE分配；使用过渡损失平滑不同训练阶段的数据转换。

Result: 在广泛使用的数据集上的大量实验证明了该方法的有效性和通用性。

Conclusion: 提出的方法能够有效解决代码切换语音翻译中的语义建模和数据稀缺问题，具有很好的实际应用价值。
...
### [126] [Grounded Visual Factualization: Factual Anchor-Based Finetuning for Enhancing MLLM Factual Consistency](https://arxiv.org/abs/2511.10671)
*Filippo Morbiato,Luca Romano,Alessandro Persona*

Main category: cs.CL

TL;DR: 提出了GVF微调方法，通过事实锚点数据增强、事实感知指令调整和事实一致性损失函数三个核心机制，系统性地提升多模态大语言模型的视觉事实一致性。
...
Method: GVF微调整合三个核心机制：事实锚点数据增强（添加结构化事实锚点和反事实提示）、事实感知指令调整（将事实线索嵌入显式指令）、事实一致性损失函数（专门惩罚事实不准确）。

Result: 在LLaVA-1.5-13B模型上，GVF微调在VHTest基准测试中显著优于标准微调，在开放式问题和是/否问题上都表现更好，同时在MME和POPE等多模态基准上保持或略微提升性能。
...
### [127] [Large language models in materials science and the need for open-source approaches](https://arxiv.org/abs/2511.10673)
*Fengxu Yang,Weitong Chen,Jack D. Evans*

Main category: cs.CL

TL;DR: LLMs在材料科学中应用于文献挖掘、预测建模和多代理实验系统，开源模型性能可媲美闭源模型且更具优势。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs在材料科学发现流程中的潜力，特别是开源模型在透明度、可复现性、成本效益和数据隐私方面的优势。

Method: 综述LLMs在三个关键领域的应用：从文本中提取合成条件等有价值信息，学习结构-性能关系，以及协调集成计算工具和实验室自动化的多代理系统。

Result: 基准测试结果显示开源模型性能可与闭源商业模型相媲美，同时提供更好的透明度、可复现性、成本效益和数据隐私。

Conclusion: 随着开源模型持续改进，应更广泛采用以构建可访问、灵活和社区驱动的科学发现AI平台。
...
### [128] [Continual Learning of Domain Knowledge from Human Feedback in Text-to-SQL](https://arxiv.org/abs/2511.10674)
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型在从自然语言生成SQL查询时，难以处理数据库特定模式和隐含的领域知识，需要一种能够从人类反馈中持续学习的方法。

Method: 设计了多种学习代理架构变体，通过接收自然语言反馈来精炼查询，并将揭示的知识提炼存储到结构化记忆中供未来任务重用。

Result: 在BIRD基准测试开发集上的实验表明，特别是程序代理，通过利用人在环路的反馈实现了显著的准确性提升和错误减少。

Conclusion: 将隐含的人类专业知识转化为可重用知识对于构建更适应性强、领域感知的文本到SQL系统至关重要，这些系统能够从人在环路中持续学习。
...
### [129] [Learn to Select: Exploring Label Distribution Divergence for In-Context Demonstration Selection in Text Classification](https://arxiv.org/abs/2511.10675)
...
<details>
  <summary>Details</summary>
Motivation: 现有演示选择方法主要关注测试输入与演示之间的语义相似性，但忽视了标签分布对齐的重要性，这会影响大语言模型的性能。

Method: 使用微调的BERT类小语言模型生成标签分布并计算测试输入与候选演示之间的标签分布差异，结合语义相似性进行两阶段演示选择。

Result: 在七个文本分类基准测试中，该方法始终优于之前的演示选择策略。

Conclusion: 大语言模型性能与用于标签分布估计的小语言模型准确性呈正相关，标签分布对齐对上下文学习效果至关重要。
...
### [130] [Pre-Attention Expert Prediction and Prefetching for Mixture-of-Experts Large Language Models](https://arxiv.org/abs/2511.10676)
*Shien Zhu,Samuel Bohl,Robin Oester,Gustavo Alonso*

Main category: cs.CL

TL;DR: 提出预注意力专家预测方法，使用同一层注意力块前的激活和简单线性函数，实现准确轻量的专家预取，在多个MoE模型上达到93-97%的准确率。
...
Method: 利用LLM中排名保持函数的特性，使用同一层注意力块前的激活，通过2个线性函数和排名感知损失进行专家预测。

Result: 在DeepSeek V2 Lite、Qwen3-30B和Phi-mini-MoE上分别达到93.03%、94.69%和97.62%的准确率，比现有方法提升约15%。

Conclusion: 预注意力专家预测方法实现了准确且轻量的专家预取，支持第一层预取，显著提升了MoE模型的推理效率。
...
### [131] [SpiderGen: Towards Procedure Generation For Carbon Life Cycle Assessments with Generative AI](https://arxiv.org/abs/2511.10684)
*Anupama Sitaraman,Bharathan Balaji,Yuvraj Agarwal*

Main category: cs.CL

TL;DR: SpiderGen是一个基于LLM的工作流，结合传统生命周期评估(LCA)的分类和方法论，利用LLM的推理能力和世界知识来生成用于LCA的过程信息，显著降低了LCA的成本和时间。


<details>
  <summary>Details</summary>
Motivation: 气候变化和全球变暖主要由温室气体排放引起，而消费品在生产、使用和处置过程中是主要排放源。因此需要开发工具来评估消费品的环境影响，其中生命周期评估(LCA)是关键组成部分。

Method: 开发了SpiderGen工作流，将传统LCA的分类和方法论与LLM的推理能力和世界知识相结合，生成LCA所需的过程信息。使用真实LCA文档作为基准进行评估。

Result: SpiderGen能够提供准确或仅有轻微错误的LCA过程信息，在10个样本数据点上达到62%的F1分数。与基线方法相比表现更好，能够以低于1美元的成本在10分钟内生成LCA过程信息，而传统LCA成本超过25000美元，需要21人天。

Conclusion: SpiderGen具有显著降低LCA人力和成本投入的潜力，能够高效生成准确的LCA过程信息，为环境影响评估提供了经济高效的解决方案。
...
### [132] [A methodological analysis of prompt perturbations and their effect on attack success rates](https://arxiv.org/abs/2511.10686)
...
TL;DR: 本文研究了不同LLM对齐方法对提示攻击响应的影响，发现即使是小的提示修改也能显著改变攻击成功率，现有攻击基准测试可能不足以揭示所有漏洞。


<details>
  <summary>Details</summary>
Motivation: 调查不同大型语言模型对齐方法如何影响模型对提示攻击的响应，评估现有攻击基准测试的充分性。

Method: 选择基于SFT、DPO和RLHF对齐方法的开源模型，使用统计方法系统分析攻击成功率对提示变化的敏感性。

Result: 小的提示修改能显著改变攻击成功率，不同对齐方法对提示变化的敏感性不同，现有攻击基准测试可能无法揭示所有漏洞。

Conclusion: 需要进行基于统计的系统性分析来评估模型对齐方法，仅依靠现有攻击基准测试不足以全面评估模型安全性。
...
### [133] [Modeling and Predicting Multi-Turn Answer Instability in Large Language Models](https://arxiv.org/abs/2511.10688)
*Jiahang He,Rishi Ramachandran,Neel Ramachandran,Aryan Katakam,Kevin Zhu,Sunishchal Dev,Ashwinee Panda,Aryan Shrivastava*

Main category: cs.CL

TL;DR: 研究发现大型语言模型在重复提问下存在显著脆弱性，简单的"再想想"提示就能导致准确率下降约10%，通过马尔可夫链建模可预测模型准确率动态变化。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型应用范围扩大，用户与模型交互频率和规模增加，需要评估模型在真实世界任务中的鲁棒性。

Method: 使用多轮跟进提示评估模型回答变化，通过马尔可夫链建模准确率动态，并检验线性探针是否能预测这些变化。

Result: Gemini 1.5 Flash在9轮"再想想"提示下准确率下降约10%；Claude 3.5 Haiku在结合重述问题时准确率下降7.5%；模型准确率可通过马尔可夫链有效建模，稳态准确率比首轮低约8%。

Conclusion: 建立了稳态准确率作为交互场景下的鲁棒性度量标准，揭示了重复提问下模型的不稳定性，这对高风险交互场景部署至关重要。
...
### [134] [Equilibrium Dynamics and Mitigation of Gender Bias in Synthetically Generated Data](https://arxiv.org/abs/2511.10689)
*Ashish Kattamuri,Arpita Vats,Harshwardhan Fartale,Rahul Raja,Akshata Kishore Moharir,Ishita Prasad*

Main category: cs.CL

TL;DR: 研究递归提示下的大语言模型性别偏见动态，发现存在均衡而非单调放大现象，并提出对比增强方法能有效减少下游任务偏见。


<details>
  <summary>Details</summary>
Motivation: 研究递归提示生成合成数据时的偏见放大风险，特别是性别偏见的动态变化。

Method: 使用三种评估框架（规则匹配、语义相似度、下游任务性能），在三种初始偏见水平和四种缓解策略下进行三代递归文本生成实验。

Result: 低初始偏见会放大至模型固有水平（+36%），高初始偏见会衰减至该水平（-26%）。对比增强方法在下游任务中显著减少偏见（平均91%），尽管语义相似度指标显示更高偏见。
...
### [135] [Saying the Unsaid: Revealing the Hidden Language of Multimodal Systems Through Telephone Games](https://arxiv.org/abs/2511.10690)
*Juntu Zhao,Jialing Zhang,Chongxuan Li,Dequan Wang*

Main category: cs.CL

TL;DR: 通过多轮"传话游戏"利用多模态系统的偏好偏差，研究其隐藏语言和概念连接强度，开发了Telescope数据集和可扩展的分析框架。


<details>
  <summary>Details</summary>
Motivation: 当前闭源多模态系统虽然取得进展，但其黑盒架构使得理解世界的隐藏语言不透明，需要新方法来研究其内部概念连接机制。

Method: 利用多轮传话游戏策略性地利用系统偏好偏差，通过观察概念共现频率定量研究多模态系统的概念连接强度，构建Telescope数据集作为分析框架基础。
...
### [136] [Evaluating from Benign to Dynamic Adversarial: A Squid Game for Large Language Models](https://arxiv.org/abs/2511.10691)
*Zijian Chen,Wenjun Zhang,Guangtao Zhai*

Main category: cs.CL

TL;DR: Squid Game是一个动态对抗性评估环境，通过资源受限和不对称信息设置来评估大语言模型在交互式游戏中的表现，包含六个淘汰赛级别，测试多方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试难以跟上大语言模型的发展，存在数据污染问题，且主要假设良性、资源丰富的环境，缺乏对模型在压力下行为的探索。

Method: 设计了包含六个淘汰赛级别的动态对抗性评估环境，让LLM与其他LLM对手进行交互式游戏，测试指令遵循、代码、推理、规划和安全对齐等多方面能力。

Result: 评估了50多个LLM，观察到同一模型谱系中明显的代际性能转变，发现一些模型采用投机性捷径来赢得游戏，表明静态基准测试可能存在更高级别的评估范式污染。
...
### [137] [Do AI Voices Learn Social Nuances? A Case of Politeness and Speech Rate](https://arxiv.org/abs/2511.10693)
...
<details>
  <summary>Details</summary>
Motivation: 研究AI语音系统是否能够学习人类社交中非显性、非编程的礼貌表达特征，特别是语速变化这一微妙韵律标记

Method: 使用两个主流AI平台（AI Studio和OpenAI）的22个合成语音，在"礼貌正式"和"随意非正式"两种提示下朗读固定脚本，测量语音时长

Result: 两个AI平台的礼貌提示都产生了比随意提示更慢的语速，效应规模很大，AI Studio所有语音和OpenAI大部分语音都显示出统计显著性

Conclusion: AI能够隐式学习和复制人类沟通中的心理细微差别，表明其作为能够强化人类社会规范的社会行为者的新兴角色
...
### [138] [Where does an LLM begin computing an instruction?](https://arxiv.org/abs/2511.10694)
*Aditya Pola,Vineeth N. Balasubramanian*

Main category: cs.CL

TL;DR: 论文通过激活修补实验发现指令遵循在模型层堆栈中存在一个起始点（onset），在此点之前干预能改变预测结果，之后则基本无效。


<details>
  <summary>Details</summary>
Motivation: 研究指令遵循在神经网络层堆栈中的起始位置，即从阅读转向执行的转换点。

Method: 使用三个简单数据集和它们的双跳组合，通过激活修补技术测量层级的翻转率来定位指令遵循的起始点。

Result: 在Llama系列模型中观察到一个拐点（onset），多跳任务也显示相似的起始位置。

Conclusion: 提供了一种简单可复制的方法来定位指令遵循的起始位置，并比较不同任务和模型大小下的差异。
...
### [139] ["As Eastern Powers, I will veto." : An Investigation of Nation-level Bias of Large Language Models in International Relations](https://arxiv.org/abs/2511.10695)
...
<details>
  <summary>Details</summary>
Motivation: 随着大型语言模型在国际关系领域的应用日益增多，评估其国家层面偏见变得至关重要。现有研究缺乏对LLMs在国际关系中系统性偏见的全面分析，特别是在联合国安理会常任理事国等关键国家上的偏见表现。

Method: 基于联合国安理会历史记录构建了包含三个测试的偏见评估框架，重点评估五个常任理事国。提出了结合检索增强生成和反思技术的去偏见框架，通过增强事实推理能力来减少偏见。

Result: 实验结果显示，LLMs普遍存在对西方国家的偏好和对俄罗斯的负面偏见，但偏见程度因模型和任务而异。推理能力更强的模型表现出更低的偏见和更好的性能。提出的去偏见框架在GPT-4o-mini和LLama-3.3-70B上有效减少了国家层面偏见并提升了性能。

Conclusion: 在国际关系领域应用LLMs时，需要同时评估国家层面偏见和性能表现。模型偏见具有多维特性，随着模型和任务的变化而变化。增强推理能力是减少偏见的关键途径。
...
### [140] [$π$-Attention: Periodic Sparse Transformers for Efficient Long-Context Modeling](https://arxiv.org/abs/2511.10696)
*Dong Liu,Yanxuan Yu*

Main category: cs.CL

TL;DR: ΠAttention是一种周期性稀疏Transformer，通过环形局部注意力、确定性π步长跳跃和自适应融合门，在保持线性复杂度的同时扩展感受野，在长上下文建模中优于密集注意力和RingAttention。
...
Method: 将注意力分解为环形局部邻域、确定性π步长跳跃和自适应融合门，周期性结构提供对远距离token的可预测覆盖，稀疏足迹保持每层复杂度线性。

Result: 在语言建模、检索和视觉语言任务中，ΠAttention匹配或超越密集注意力质量，困惑度比RingAttention低8.3%，相同上下文长度下GPU使用量减少50%。

Conclusion: 周期性跳跃、自适应融合和头部级稀疏协调对高效长上下文建模至关重要，ΠAttention在保持线性复杂度的同时显著扩展了感受野。
...
### [141] [Faithful Summarization of Consumer Health Queries: A Cross-Lingual Framework with LLMs](https://arxiv.org/abs/2511.10768)
*Ajwad Abrar,Nafisa Tabassum Oeshy,Prianka Maheru,Farzana Tabassum,Tareque Mohmud Chowdhury*

Main category: cs.CL

TL;DR: 提出了结合TextRank句子提取和医疗实体识别的框架，通过微调LLaMA-2-7B模型在英文和孟加拉语医疗问答数据集上，显著提升了医疗文本摘要的忠实度和质量。


<details>
  <summary>Details</summary>
Motivation: 医疗文本摘要中不忠实的摘要可能误传医疗细节，带来严重风险，因此需要提高摘要的忠实度以确保医疗信息准确传达。

Method: 结合TextRank句子提取和医疗命名实体识别，使用LLM进行医疗文本摘要，在MeQSum和BanglaCHQ-Summ数据集上微调LLaMA-2-7B模型。

Result: 在质量和忠实度指标（ROUGE、BERTScore、可读性、SummaC、AlignScore）上取得一致改进，超过零样本基线和先前系统，人工评估显示超过80%的生成摘要保留了关键医疗信息。

Conclusion: 忠实度是可靠医疗摘要的关键维度，该方法展示了在医疗环境中更安全部署LLM的潜力。
...
### [142] [TEDxTN: A Three-way Speech Translation Corpus for Code-Switched Tunisian Arabic - English](https://arxiv.org/abs/2511.10780)
*Fethi Bougares,Salima Mdhaffar,Haroun Elleuch,Yannick Estève*

Main category: cs.CL

TL;DR: TEDxTN是首个公开可用的突尼斯阿拉伯语到英语语音翻译数据集，包含108个TEDx演讲，25小时语音，涵盖11个突尼斯地区的不同口音，支持代码转换研究。
...
Result: 创建了首个开源、公开可用的代码转换突尼斯方言语音翻译语料库，并报告了强基线系统的性能结果。

Conclusion: TEDxTN是有价值的资源，将激励和促进突尼斯方言自然语言处理的进一步研究。
...
### [143] [Sabiá: Um Chatbot de Inteligência Artificial Generativa para Suporte no Dia a Dia do Ensino Superior](https://arxiv.org/abs/2511.10787)
...
Conclusion: GenAI和RAG技术可以有效简化学生获取大学日常信息的流程，Gemini 2.0 Flash和Gemma 3n是合适的模型选择。
...
### [144] [LLM-as-a-Grader: Practical Insights from Large Language Model for Short-Answer and Report Evaluation](https://arxiv.org/abs/2511.10819)
*Grace Byun,Swati Rajwal,Jinho D. Choi*

Main category: cs.CL

TL;DR: 本研究探讨了使用GPT-4o在计算语言学课程中评估学生答题和项目报告的可行性，结果显示与人工评分有强相关性，但也存在技术性开放答案评分的不稳定性。
...
Method: 收集约50名学生的五次测验答案和14个团队的项目报告，使用GPT-4o进行评分，并与助教独立的人工评分进行比较。

Result: GPT-4o与人工评分者达到强相关性（最高0.98），55%的测验案例得分完全一致；项目报告总体对齐良好，但技术性开放答案评分存在变异性。

Conclusion: LLM评分系统在教育评估中具有潜力但也存在局限性，为现实学术环境中自动化评分的发展做出了贡献。
...
### [145] [Tracing Multilingual Representations in LLMs with Cross-Layer Transcoders](https://arxiv.org/abs/2511.10840)
*Abir Harrasse,Florent Draye,Zhijing Jin,Bernhard Schölkopf*

Main category: cs.CL

TL;DR: 该研究发现多语言大语言模型采用枢轴语言表示机制：模型在不同语言间使用几乎相同的内部表示，而语言特定的解码出现在较深层。通过干预这些语言特征，可以抑制一种语言并用另一种语言替换模型输出。


<details>
  <summary>Details</summary>
Motivation: 研究多语言大语言模型如何内部表示不同语言，以及为什么性能仍然偏向主导训练语言，旨在理解多语言对齐机制。
...
### [146] [Reinforcing Stereotypes of Anger: Emotion AI on African American Vernacular English](https://arxiv.org/abs/2511.10846)
*Rebecca Dorn,Christina Chance,Casandra Rusti,Charles Bickham,Kai-Wei Chang,Fred Morstatter,Kristina Lerman*

Main category: cs.CL

TL;DR: 研究发现情感识别模型在非洲裔美国白话英语(AAVE)上存在显著偏见，与通用美式英语(GAE)相比，AAVE文本被错误预测为愤怒的概率高出两倍以上，模型表现与种族人口统计数据相关，揭示了情感AI强化种族刻板印象的安全问题。


<details>
  <summary>Details</summary>
Motivation: 现有情感识别模型通常基于主流文化规范的标注数据训练，可能无法准确识别被排除在训练数据分布之外的方言（如AAVE）中的情感表达，这在高风险应用领域（如心理健康和招聘）中可能带来严重问题。

Method: 分析270万条洛杉矶地理标记推文，使用计算方法评估AAVE方言特征强度，在875条高/低AAVE密度推文上收集情感存在和强度标注，采用社区知情"银标"方法（由AAVE流利的非裔美国人标注），比较GPT、BERT和SpanEmo等模型表现。

Result: 在AAVE文本上，GPT和BERT模型的愤怒误报率比GAE高出两倍以上；SpanEmo模型的愤怒误报率从GAE的25%增加到AAVE的60%；模型和非内部群体标注与基于脏话的AAVE特征显著相关；非裔美国人比例较高的社区与更高的愤怒预测(r=0.27)和更低的快乐预测(r=-0.10)相关。

Conclusion: 情感AI存在强化种族刻板印象的紧急安全问题，需要通过文化和方言知情的情感计算系统来解决模型偏见问题。
...
### [147] [Leveraging Parameter Space Symmetries for Reasoning Skill Transfer in LLMs](https://arxiv.org/abs/2511.10850)
...
<details>
  <summary>Details</summary>
Motivation: 任务算术在大型语言模型间转移技能时存在负干扰问题，特别是当模型在训练过程中出现分歧时。本文旨在通过参数空间对齐来解决这一限制。

Method: 首先利用Transformer架构固有的置换、旋转和缩放对称性来对齐模型的参数空间，针对现代GQA和SwiGLU层适配参数空间对齐方法，探索基于权重和基于激活的两种方法。

Result: 实验表明，该方法在具有挑战性的推理基准测试中持续优于标准任务算术，成功将高级推理技能转移到非推理模型中。

Conclusion: 这项工作为在演进的LLM家族间合并和转移专门技能提供了有效方法，减少了冗余微调并增强了模型适应性。
...
### [148] [Unsupervised Cycle Detection in Agentic Applications](https://arxiv.org/abs/2511.10650)
...
<details>
  <summary>Details</summary>
Motivation: 传统可观测性平台无法检测LLM智能应用中非确定性行为形成的隐藏执行循环，这些循环会默默消耗资源而不触发错误，造成成本浪费。

Method: 采用混合方法：首先进行高效的时间调用栈分析识别显式循环，然后利用语义相似性分析发现由冗余内容生成形成的微妙循环。

Result: 在LangGraph股票市场应用的1575个轨迹上测试，混合方法F1得分为0.72（精确率0.62，召回率0.86），显著优于单独的结构方法（F1:0.08）和语义方法（F1:0.28）。
...
### [149] [From Fact to Judgment: Investigating the Impact of Task Framing on LLM Conviction in Dialogue Systems](https://arxiv.org/abs/2511.10871)
*Parisa Rabbani,Nimet Beyza Bozdag,Dilek Hakkani-Tür*

Main category: cs.CL

TL;DR: LLM作为评判者在社交对话任务中的判断会受到对话框架的显著影响，即使是最小对话语境也会改变模型判断，平均性能变化达9.24%。


<details>
  <summary>Details</summary>
Motivation: 研究LLM作为评判者在需要社交或对话判断的任务中是否可靠，特别关注对话框架如何影响模型判断。

Method: 通过对比模型在直接事实查询和对话判断任务中的表现，并在两种条件下施加简单的反驳压力，测量模型在对话压力下维持立场的坚定程度。

Result: 不同模型表现出不同倾向：GPT-4o-mini在社交框架下显示谄媚倾向，而Llama-8B-Instruct变得过度批判性。对话语境显著改变模型判断。
...
### [150] [Data Analysis and Performance Evaluation of Simulation Deduction Based on LLMs](https://arxiv.org/abs/2511.10651)
*Shansi Zhang,Min Li*

Main category: cs.CL

TL;DR: 提出了一种基于大语言模型的军事模拟推演数据分析方法，通过任务分解、多轮交互和自定义工具生成高质量结构化分析报告


<details>
  <summary>Details</summary>
Motivation: 传统的人工分析方法耗时且容易出错，需要利用大语言模型的强大分析推理能力来提升军事模拟推演数据分析的效率和准确性

Method: 将复杂任务分解为子任务，设计有效的系统提示和用户提示，通过多轮交互结合自检和反思机制，使用自定义工具生成图表和计算指标，并设计多种报告模板

Result: 评估结果显示，该方法生成的报告质量更高，相比基线方法获得更高评分

Conclusion: 该方法能够有效生成高质量的结构化军事模拟推演分析报告，在多种场景下都具有良好的适应性
...
### [151] [ICX360: In-Context eXplainability 360 Toolkit](https://arxiv.org/abs/2511.10879)
...
Method: ICX360工具包实现了三种解释方法：通过扰动（黑盒方法）和梯度（白盒方法）来解释LLMs。

Result: 开发了ICX360工具包，包含快速入门指南和详细教程，涵盖检索增强生成、自然语言生成和越狱等用例。
...
TL;DR: 提出了一种通过离线数据增强和并行检索的结构化情景记忆系统，解决了历史人物对话系统中深度与延迟的权衡问题，在保持高效的同时实现深度对话。


<details>
  <summary>Details</summary>
Motivation: 现有历史人物对话系统面临关键权衡：简单的检索增强生成产生浅层响应，而多阶段反思虽然深度但延迟过高，需要一种既能保持深度又高效的解决方案。

Method: 将传记数据转化为1774个增强的第一人称记忆，带有情感语义元数据，采用两阶段检索架构实现0.52秒的提示生成，通过离线数据增强和并行检索优化效率。

Result: 在GPT-4上与传统RAG表现相当，但在较小模型（GPT-3.5、GPT-3）上显著优于传统RAG，特别适合资源受限的部署。系统还支持时空热图、情感轨迹分析等可视化工具。

Conclusion: 该架构可推广到任何有大量文本记录的历史人物，为教育、博物馆和研究应用提供了既准确又高效的实用框架，既是对话界面也是传记分析研究工具。
...
### [153] [A Multifaceted Analysis of Negative Bias in Large Language Models through the Lens of Parametric Knowledge](https://arxiv.org/abs/2511.10881)
*Jongyoon Song,Sangwon Yu,Sungroh Yoon*

Main category: cs.CL

TL;DR: 本文研究发现大语言模型在二元决策任务中存在格式层面的负向偏见，即提示格式比负向回答的语义更能影响模型响应。模型在缺乏足够知识回答是非问题时倾向于生成负向响应，导致负向偏见。提供相关上下文和"我不知道"选项可减少偏见，而思维链提示会放大偏见。


<details>
  <summary>Details</summary>
Motivation: 先前研究主要关注检测和处理引发负向偏见的负向注意力头，但影响负向偏见的详细底层因素仍未充分探索。本文旨在深入探究负向偏见的影响因素。

Method: 引入构建评估集的流程，系统地将数据集基于模型参数知识分为三个子集：正确、错误和知识不足。分析不同提示场景下负向偏见的变化，包括提供相关上下文、"我不知道"选项和思维链提示。

Result: 发现模型在缺乏足够知识时倾向于生成负向响应，导致负向偏见。提供相关上下文和"我不知道"选项可减少负向偏见，而思维链提示会放大偏见。负向偏见程度因提示类型而异，影响响应方向。
...
### [154] [Hybrid Quantum Transformer for Language Generation](https://arxiv.org/abs/2511.10653)
*Desheng Kong,Xiangshuo Cui,Jiaying Jin,Jing Xu,Donglin Wang*

Main category: cs.CL

TL;DR: 提出了首个用于自然语言生成的混合量子-经典大语言模型HyQuT，在Transformer框架中集成了变分量子电路，在150M参数模型中用10个量子比特和80个量子门替换了约10%的经典参数，实现了可比较的收敛稳定性和生成质量。
...
Method: 在Transformer框架中集成变分量子电路(VQCs)，构建了8M和150M参数规模的混合量子-经典大语言模型HyQuT。

Result: 实验结果表明，在150M参数模型中，仅用10个量子比特和80个量子门就能替换约10%的经典参数，同时实现可比较的收敛稳定性和生成质量。
...
### [155] [MedPath: Multi-Domain Cross-Vocabulary Hierarchical Paths for Biomedical Entity Linking](https://arxiv.org/abs/2511.10887)
*Nishant Mishra,Wilker Aziz,Iacer Calixto*

Main category: cs.CL

TL;DR: MedPath是一个大规模多领域的生物医学实体链接数据集，整合了9个现有专家标注数据集，通过UMLS标准化实体并添加62个生物医学词汇表映射和完整本体路径，旨在解决当前生物医学NER和EL领域的数据碎片化、可解释模型资源缺乏等问题。


<details>
  <summary>Details</summary>
Motivation: 当前生物医学命名实体识别和实体链接领域面临数据碎片化、缺乏构建可解释模型的资源以及语义盲评估指标的局限性等挑战，阻碍了该领域的进展。

Method: 基于9个现有专家标注的实体链接数据集构建MedPath数据集，将所有实体使用最新版UMLS进行标准化，增强到62个其他生物医学词汇表的映射，并关键性地在最多11个生物医学词汇表中丰富了完整的本体路径。

Result: 创建了一个大规模多领域的生物医学实体链接数据集MedPath，为生物医学NLP研究提供了新的前沿，支持训练和评估语义丰富且可解释的实体链接系统。

Conclusion: MedPath直接促进了生物医学NLP的新研究前沿，有助于训练和评估语义丰富且可解释的实体链接系统，并推动下一代可互操作和可解释的临床NLP模型的开发。
...
TL;DR: 研究发现大型语言模型在处理时间约束方面存在系统性风险，包括双峰性能分布、极端提示脆弱性和系统性行动偏见，无法通过自然语言的下一个词预测可靠学习时间约束满足能力。
...
Result: 发现双峰性能分布（模型准确率要么95%要么50%）、极端提示脆弱性（仅格式变化导致30-60个百分点波动）和系统性行动偏见（失败模型100%假阳性率），参数数量与能力无相关性，微调仅能部分改善性能。
...
### [157] [From Proof to Program: Characterizing Tool-Induced Reasoning Hallucinations in Large Language Models](https://arxiv.org/abs/2511.10899)
*Farima Fatahi Bayat,Pouya Pezeshkpour,Estevam Hruschka*

Main category: cs.CL

TL;DR: 工具增强语言模型在使用代码解释器工具时会出现"工具诱导近视"现象，即虽然提高了最终答案准确率，但推理质量下降，模型将工具输出作为推理的替代品而非辅助证据。


<details>
  <summary>Details</summary>
Motivation: 研究工具增强语言模型在使用外部工具时的推理可信度问题，特别关注代码解释器工具是否真正提升了模型的推理能力。

Method: 开发PYMATH基准测试（1,679个竞赛级数学问题），建立多维度评估套件量化推理退化，并提出基于偏好优化的框架来重新对齐模型使用工具的方式。

Result: 工具增强模型最终答案准确率提升19.3个百分点，但推理质量显著下降（非工具模型在推理过程比较中胜出41.5%），工具使用越频繁，推理连贯性越差，约55%高风险案例存在工具诱导近视。

Conclusion: 工具使用虽然提高答案准确率，但损害推理质量，需要通过偏好优化等方法重新对齐模型，使工具作为辅助证据而非推理替代品。
...
### [158] [Expert-Guided Prompting and Retrieval-Augmented Generation for Emergency Medical Service Question Answering](https://arxiv.org/abs/2511.10900)
*Xueren Ge,Sahil Murtaza,Anthony Cortez,Homa Alemzadeh*

Main category: cs.CL

TL;DR: 提出了EMSQA数据集和Expert-CoT、ExpertRAG方法，通过结合临床专业领域和认证级别信息，显著提升了LLM在医疗问答中的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在医疗问答中忽视了专业领域知识（如临床科目、认证级别），限制了在高风险环境下的性能。

Method: 构建EMSQA数据集，开发Expert-CoT提示策略和ExpertRAG检索增强生成管道，将推理和检索与特定临床领域和认证级别对齐。

Result: Expert-CoT比普通CoT提示提升2.05%，结合ExpertRAG比标准RAG基线提升4.59%准确率，32B模型通过所有EMS认证模拟考试。

Conclusion: 结合领域专业知识能显著提升LLM在医疗问答中的性能，特别是在高风险认证环境中。
...
### [159] [Multimodal Peer Review Simulation with Actionable To-Do Recommendations for Community-Aware Manuscript Revisions](https://arxiv.org/abs/2511.10902)
*Mengze Hong,Di Jiang,Weiwei Zhao,Yawen Li,Yihang Wang,Xinyuan Luo,Yanjie Sun,Chen Jason Zhang*

Main category: cs.CL

TL;DR: 提出了一个基于多模态大语言模型的交互式同行评审系统，通过整合文本和视觉信息、基于OpenReview数据的检索增强生成，以及将评审意见转化为可执行任务列表，为学术论文提交前提供有效的修订指导。


<details>
  <summary>Details</summary>
Motivation: 现有学术同行评审系统受限于纯文本输入、有限的上下文基础以及缺乏可操作性反馈，无法充分利用大语言模型在自动化学术工作流程中的潜力。

Method: 开发了基于Web的交互式系统，整合多模态LLM处理文本和视觉信息，使用检索增强生成技术基于OpenReview数据提升评审质量，并采用Action:Objective[#]格式将评审意见转化为结构化、可追踪的任务列表。

Result: 实验结果表明，该系统能生成更全面、有用的评审意见，符合专家标准，优于消融基线模型，推进了透明、以人为本的学术辅助。

Conclusion: 该系统通过多模态交互、社区感知的同行评审模拟，为学术论文提交前修订提供了有效的解决方案，显著提升了评审质量和可操作性。
...
### [160] [Automated Analysis of Learning Outcomes and Exam Questions Based on Bloom's Taxonomy](https://arxiv.org/abs/2511.10903)
*Ramya Kumar,Dhruv Gulwani,Sonit Singh*

Main category: cs.CL

TL;DR: 本文研究了基于布鲁姆分类法的考试题目和学习成果自动分类，比较了传统机器学习、RNN、Transformer和大型语言模型的表现。支持向量机(SVM)结合数据增强取得了94%的最佳性能，而复杂深度学习模型在有限数据上容易过拟合。


<details>
  <summary>Details</summary>
Motivation: 探索如何自动将考试题目和学习成果按照布鲁姆分类法的六个认知层次进行分类，以帮助教育工作者更有效地评估和设计教学材料。

Method: 使用600个标注句子的小数据集，测试了传统ML模型(朴素贝叶斯、逻辑回归、SVM)、RNN架构(LSTM、BiLSTM、GRU、BiGRU)、Transformer模型(BERT、RoBERTa)和大型语言模型(OpenAI、Gemini等)，并采用不同预处理和数据增强策略。

Result: SVM结合数据增强达到94%的准确率、召回率和F1分数，过拟合最小；RNN和BERT严重过拟合；RoBERTa初期克服过拟合但训练后期出现迹象；大型语言模型的零样本评估中OpenAI和Gemini表现最佳，准确率约0.72-0.73。

Conclusion: 在有限数据上训练复杂深度学习模型面临挑战，精心设计的数据增强和简单算法(如增强SVM)在布鲁姆分类法分类中具有重要价值。
...
### [161] [Evaluating Large Language Models on Rare Disease Diagnosis: A Case Study using House M.D](https://arxiv.org/abs/2511.10912)
*Arsh Gupta,Ajay Narayanan Sridhar,Bonam Mingole,Amulya Yadav*

Main category: cs.CL

TL;DR: 评估四种先进大语言模型在罕见疾病诊断任务上的表现，使用从《豪斯医生》剧集提取的176个症状-诊断对作为基准数据集，结果显示准确率在16.48%到38.64%之间，新一代模型性能提升2.3倍。


<details>
  <summary>Details</summary>
Motivation: 探索大语言模型在罕见疾病诊断方面的能力，目前这一领域研究不足，特别是基于叙述性医学病例的诊断推理。

Method: 使用从《豪斯医生》剧集提取的176个症状-诊断对作为基准数据集，评估GPT 4o mini、GPT 5 mini、Gemini 2.5 Flash和Gemini 2.5 Pro四种模型在叙述性诊断推理任务上的表现。

Result: 模型表现存在显著差异，准确率范围16.48%-38.64%，新一代模型相比前代有2.3倍的性能提升，但所有模型在罕见疾病诊断方面仍面临重大挑战。

Conclusion: 虽然模型在罕见疾病诊断上仍有困难，但跨架构的改进趋势为未来发展提供了有希望的方向，该教育验证基准为AI辅助诊断研究建立了基线性能指标和公开评估框架。
...
### [162] [CardioEmbed: Domain-Specialized Text Embeddings for Clinical Cardiology](https://arxiv.org/abs/2511.10930)
*Richard J. Young,Alice M. Matthews*

Main category: cs.CL

TL;DR: CardioEmbed是基于Qwen3-Embedding-8B训练的专门用于心脏病学领域的嵌入模型，通过对比学习在7本心脏病学教科书上训练，在心脏病特定语义检索任务中达到99.60%的检索准确率，比当前最佳医学嵌入模型MedTE提升了15.94个百分点。
...
Conclusion: 基于综合临床教科书的领域专业化训练能够在心脏病学检索中实现近乎完美的性能，显著优于现有医学嵌入模型。
...
### [163] [DiscoX: Benchmarking Discourse-Level Translation task in Expert Domains](https://arxiv.org/abs/2511.10984)
...
TL;DR: DiscoX是一个用于评估专业领域中文-英文篇章级翻译的新基准，包含7个领域的200篇专业文本。同时开发了无参考评估系统Metric-S，实验显示即使最先进的LLM在专业翻译任务上仍显著落后于人类专家。


<details>
  <summary>Details</summary>
Motivation: 当前翻译评估方法主要关注句子级别的准确性和流畅性，无法充分评估专业领域翻译所需的篇章连贯性和术语精确性。

Method: 构建DiscoX基准数据集（200篇专业文本，平均长度超过1700词），开发无参考评估系统Metric-S进行细粒度自动评估。

Result: Metric-S与人工评估具有强一致性，显著优于现有指标；实验显示最先进的LLM在专业翻译任务上仍明显落后于人类专家。

Conclusion: DiscoX基准和Metric-S评估系统为LLM翻译提供了更严格的评估框架，揭示了实现专业级机器翻译仍面临重大挑战。
...
### [164] [When Data is the Algorithm: A Systematic Study and Curation of Preference Optimization Datasets](https://arxiv.org/abs/2511.10985)
...
<details>
  <summary>Details</summary>
Motivation: 现有DPO数据集缺乏系统性比较，主要由于计算成本高和缺乏详细质量标注，难以理解偏好选择方式、任务类型覆盖范围以及人类判断的反映程度。

Method: 使用Magpie框架对每个样本进行任务类别、输入质量和偏好奖励的标注，实现可扩展的细粒度偏好质量检查，并基于分析结果从五个语料库中有选择地构建新的DPO混合数据集UltraMix。

Result: 发现不同数据集在奖励边界上存在结构和质量差异，构建的UltraMix数据集比最佳单个数据集小30%但在关键基准测试中表现更优。

Conclusion: 通过数据中心的偏好优化方法，可以构建更高效的DPO数据集，为未来研究提供有价值的资源和见解。
...
### [165] [Automata-Based Steering of Large Language Models for Diverse Structured Generation](https://arxiv.org/abs/2511.11018)
*Xiaokun Luan,Zeming Wei,Yihao Zhang,Meng Sun*

Main category: cs.CL

TL;DR: 提出一种新方法来增强基于自动机的结构化生成中的多样性，通过利用自动机遍历历史引导LLM生成新颖的结构模式，显著提升结构多样性和内容多样性。
...
Method: 利用自动机遍历历史来引导LLM朝着新颖的结构模式发展，增强结构化生成中的多样性。

Result: 评估显示该方法显著提高了结构和内容多样性，同时保持了相当的生成效率。案例研究展示了该方法在生成多样化测试用例方面的有效性。

Conclusion: 该方法成功解决了结构化生成中缺乏多样性的问题，为测试开源库等应用场景提供了有效的多样化生成解决方案。
...
TL;DR: 提出Renormalization方法，通过去除文本嵌入模型输出中的一致性偏置μ，显著提升模型在MMTEB基准上的性能表现。


<details>
  <summary>Details</summary>
Motivation: 发现当前文本嵌入模型输出存在一致性偏置，即所有句子的嵌入向量都包含几乎相同的偏置分量μ，这影响了模型性能。

Method: 提出无需训练的轻量级解决方案Renormalization，包含两种变体：直接减去偏置μ或减去e在μ上的投影。

Result: 在38个模型上，Renormalization显著提升性能：检索任务提升9.7σ，分类任务提升3.1σ，其他任务提升0.8σ。理论预测投影方法更优，实验证实此预测。

Conclusion: Renormalization是有效且通用的方法，能一致且显著地提升现有文本嵌入模型的性能。
...
TL;DR: 本文研究LLMs能否检测自身幻觉，提出检测框架和基于Chain-of-Thought的分类方法，实验表明GPT-3.5 Turbo能检测58.2%的自身幻觉。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型能生成流畅回答但有时会产生事实幻觉，需要研究模型能否检测自身幻觉。

Method: 将幻觉检测制定为句子分类任务，提出检测框架和使用Chain-of-Thought从参数中提取知识的分类方法。

Result: 实验结果显示GPT-3.5 Turbo配合CoT能检测58.2%的自身幻觉。

Conclusion: 如果参数中包含足够知识，带有CoT的LLMs能够检测幻觉。
...
TL;DR: 提出一个分析美国总统辩论中人身攻击的框架，通过手动标注2016、2020和2024年选举周期的辩论记录，结合统计和语言模型分析，探索微调transformer模型和通用LLM检测正式政治演讲中人身攻击的潜力。


<details>
  <summary>Details</summary>
Motivation: 人身攻击已成为美国总统辩论的显著特征，对选举期间公众认知产生重要影响。检测此类攻击可以提高政治话语透明度，为记者、分析师和公众提供见解。深度学习和transformer模型的发展为有害语言的自动检测创造了新机会。

Method: 手动标注2016、2020和2024年选举周期的辩论记录，进行统计分析，并研究微调transformer模型与通用LLM在检测正式政治演讲中人身攻击的能力。

Result: 展示了现代语言模型的任务特定适应如何有助于更深入地理解政治沟通，证明了基于transformer的模型在检测政治辩论中人身攻击方面的有效性。

Conclusion: 特定任务的语言模型适应可以为政治沟通研究做出贡献，为自动检测政治辩论中的有害语言提供了可行方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有对话模型在嘈杂多说话者环境中表现不佳，常产生不相关响应和尴尬的话轮转换，需要更鲁棒的多模态解决方案。

Method: 结合声学标记化和多任务多阶段训练，在单声道、合成和真实音视频对话数据集上进行训练，实现流式转录、语义基础的话轮边界检测和准确响应。

Result: 在干扰条件下优于纯音频模型，减少转录错误，改进话轮转换预测，提升人类评分的对话质量。

Conclusion: 视听结合为说话者感知交互提供了强大能力，为在真实世界嘈杂环境中鲁棒运行的语音对话代理铺平了道路。
...
TL;DR: 提出了MemoDetector框架，通过四步文本增强模块和双阶段模态融合策略，显著提升了表情包情感理解的性能，在两个数据集上分别实现了4.3%和3.4%的F1分数提升。
...
Method: 1. 四步文本增强模块：利用多模态大语言模型逐步推理和提取表情包的隐含和上下文信息；2. 双阶段模态融合：第一阶段对原始图像和文本进行浅层融合，第二阶段深度整合增强的视觉和文本特征。

Result: 在MET-MEME和MOOD数据集上，MemoDetector的F1分数分别提升了4.3%和3.4%，优于现有最先进方法。消融研究和深入分析验证了方法的有效性和鲁棒性。

Conclusion: MemoDetector通过文本增强和分层融合策略，有效提升了表情包情感理解的性能，具有推进该领域发展的强大潜力。
...
TL;DR: SAP²方法是一种两阶段动态剪枝和整合相关上下文关键词的框架，通过语音驱动的注意力池化机制压缩上下文嵌入，在SlideSpeech和LibriSpeech数据集上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 自动语音识别系统在常见条件下表现优异，但在需要领域特定知识的语境化场景（如会议演示）中难以有效利用长上下文信息，主要受限于模型上下文窗口和大量上下文噪声中相关信息的稀疏性。

Method: 提出SAP²方法，采用两阶段动态剪枝和整合相关上下文关键词的框架，每个阶段使用语音驱动的注意力池化机制，有效压缩上下文嵌入同时保留语音显著信息。

Result: 在SlideSpeech和LibriSpeech数据集上分别达到7.71%和1.12%的词错误率，在SlideSpeech上相比非上下文基线显著降低偏置关键词错误率41.1%，并在广泛上下文输入条件下保持稳健性能。

Conclusion: SAP²方法通过动态上下文剪枝和语音驱动的注意力池化，有效解决了长上下文语音识别中的信息稀疏问题，实现了优异的性能和可扩展性。
...
TL;DR: 提出了PRSM指标来量化CLIP模型对改写查询的敏感性，使用Social Counterfactuals数据集评估CLIP在改写变化下的稳定性，发现稳健性随改写策略变化且在性别相关查询中存在差异。


<details>
  <summary>Details</summary>
Motivation: CLIP模型虽然在多模态任务中表现良好，但其对语言变体（特别是改写）的稳健性研究不足。改写稳健性对于可靠部署至关重要，尤其是在社会敏感场景中，不一致的表征可能放大人口统计偏差。

Method: 引入Paraphrase Ranking Stability Metric (PRSM)指标，使用Social Counterfactuals数据集评估CLIP在改写查询下的稳定性，分析改写稳健性与性别的交互作用。

Result: 稳健性随改写策略不同而变化，在男性和女性相关查询中观察到细微但一致的差异。

Conclusion: CLIP的改写稳健性存在变化，且与性别因素相关，这对多模态系统的公平性和公平部署具有重要影响。
...
TL;DR: 本文提出了一个基于语言学的副词超义类型学，扩展了WordNet对副词的语义分类覆盖，并通过实证验证了该分类系统的可靠性。


<details>
  <summary>Details</summary>
Motivation: WordNet为名词和动词提供了丰富的超义层次结构，但副词的语义分类系统仍不完善，缺乏系统的语义分类框架。

Method: 引入基于语言学的副词超义类型学，通过注释研究进行实证验证，涵盖方式、时间、频率、程度、领域、说话者导向和主语导向等主要语义域。

Result: 试点注释研究表明，这些类别能够广泛覆盖自然文本中的副词，并且可以由人工注释者可靠地分配。
...
TL;DR: LANE是一种对抗性训练策略，通过选择性标记训练集中的替代词生成挑战性负例，迫使模型在相同句子但不同标记词之间产生更大区分度，从而提升细粒度词义分辨能力。
...
Method: 提出LANE对抗训练策略，通过选择性标记替代词生成负训练样本，强制模型学习目标词的区分性表示。

Result: 在词汇语义变化检测和词义消歧基准测试中，该方法比标准对比学习基线表现更好，产生更具区分性的词表示。

Conclusion: LANE方法能有效捕捉细微意义差异，是模型无关的，可集成到现有表示学习框架中。
...
TL;DR: 提出了一种从知识图谱生成问答对的可扩展确定性流水线，结合规则模板和LLM精炼来提升语言质量和事实一致性。


<details>
  <summary>Details</summary>
Motivation: 现有方法在可扩展性、语言质量和事实一致性方面存在不足，需要开发更高效可靠的问答生成方法。

Method: 首先基于关系聚类KG三元组，创建可重用模板；然后使用LLM精炼模板；最后通过从KG选择干扰项来实例化答案选项。

Result: 实验证明该混合方法能高效生成高质量问答对，兼具可扩展性和语言流畅性。

Conclusion: 提出的混合方法成功结合了规则模板的可扩展性和LLM的语言精炼能力，为KG到QA的转换提供了有效解决方案。
...
TL;DR: iMAD是一个智能多智能体辩论框架，通过选择性触发多智能体辩论来显著降低计算成本并提高答案准确性。
...
Method: iMAD从单智能体的结构化自我批判响应中提取41个可解释的语言和语义特征，使用轻量级辩论决策分类器（采用FocusCal损失训练）来决定是否触发多智能体辩论。

Result: 在六个（视觉）问答数据集上的实验表明，iMAD显著减少了token使用量（最高达92%），同时提高了最终答案准确率（最高达13.5%）。

Conclusion: iMAD通过智能选择性地触发多智能体辩论，在保持准确性的同时大幅降低了计算成本，实现了效率与性能的平衡。
...
<details>
  <summary>Details</summary>
Motivation: 近年来机器学习模型被发现存在多种脆弱性，可能危及模型及其应用系统的安全。

Method: 分析现有对抗攻击方法，开发新的攻击配方，通过机器学习方法生成最大困惑度的对抗样本，特别关注孟加拉语的对抗攻击。

Result: 创建了能够使最先进机器学习模型陷入困惑状态的模糊对抗样本。

Conclusion: 这项工作为未来模型鲁棒性发展提供了路径，同时严格遵循效用降低和效率原则。
...
TL;DR: 提出了LAET（层自适应集成调优）方法，通过选择性微调预训练LLM中最有效的层来降低计算成本，在金融NLP任务中表现优异，甚至超越GPT-4等大型模型。


<details>
  <summary>Details</summary>
Motivation: 现有金融LLM（如BloombergGPT、FinMA）计算需求高，限制了组织可访问性，需要开发更高效的模型部署方案。

Method: LAET策略通过分析隐藏状态表示，选择性微调预训练LLM中最有效的层，同时冻结不太关键的层。

Result: LAET显著降低了计算开销，同时提升了任务特定性能，在金融NLP任务中表现优于现有基准和最先进的LLM（如GPT-4），即使使用较小的LLM（约30亿参数）。

Conclusion: 这项工作弥合了尖端金融NLP研究与实际部署之间的差距，为金融应用提供了高效且可扩展的模型。
...
TL;DR: NOVA是一个智能代理框架，可将科学查询转化为可执行的分析流水线，通过迭代生成和运行Python代码，集成49个领域特定工具，并在SlideQuest基准测试中优于其他编码代理基线。
...
Method: NOVA框架通过迭代生成和运行Python代码，将科学查询转化为可执行分析流水线。集成了49个领域特定工具（如细胞核分割、全玻片编码），并能临时创建新工具。

Result: 在SlideQuest基准测试（包含90个经过病理学家验证的问题）中，NOVA在数据处理、定量分析和假设检验方面优于其他编码代理基线。病理学家验证的案例研究展示了其将形态学与预后相关PAM50亚型关联的能力。

Conclusion: NOVA展示了可扩展的发现潜力，能够处理需要多步推理、迭代编码和计算问题解决能力的复杂生物医学分析任务。
...
TL;DR: LaoBench是首个针对老挝语的大规模、高质量、多维度基准数据集，包含17,000多个样本，涵盖知识应用、K12基础教育和多语言翻译三个核心维度，用于评估LLM在老挝语上的综合语言理解和推理能力。
...
Method: 构建了包含17,000多个样本的数据集，涵盖三个核心维度：知识应用、K12基础教育和老挝语-中文-英语双语翻译。采用专家人工策划与自动化智能体辅助验证相结合的数据构建流程。
...
TL;DR: M-DAIGT共享任务专注于在多领域（特别是新闻文章和学术写作）检测AI生成文本，包含新闻文章检测和学术写作检测两个子任务，并发布了包含3万样本的大规模基准数据集。
...
Method: 设计了包含新闻文章检测和学术写作检测两个二元分类子任务的共享任务，构建了平衡的人类撰写和AI生成文本数据集，使用了多种现代LLM和提示策略生成AI内容。

Result: 46个团队注册参与，4个团队提交了最终结果，所有团队都参与了两个子任务，描述了参与团队使用的方法。

Conclusion: M-DAIGT为AI生成文本检测提供了重要基准，并讨论了该任务的未来发展方向。
...
TL;DR: 语言模型和婴儿都能轻松学习自然语言，但语言模型缺乏人类特有的归纳偏置，导致它们对某些不可能语言的学习能力与人类不同。
...
Method: 回顾现有文献，分析语言模型对自然语言和不可能语言的学习表现，并与人类婴儿的语言习得能力进行比较。

Result: 语言模型通常能同等学好自然语言和许多不可能语言，而难以学习的不可能语言往往更复杂或更随机。语言模型缺乏支持人类语言习得的归纳偏置。

Conclusion: 语言模型在语言学习方面与人类婴儿存在根本差异，主要在于缺乏人类特有的归纳偏置，这限制了它们对某些语言结构的敏感性。
...
<details>
  <summary>Details</summary>
Motivation: 传统语料库存在偏见，且缺乏机器可读性。该项目旨在利用影子图书馆的丰富资源，为计算社会科学和文化分析提供高质量、机器可读的数据集。

Method: 将影子图书馆（如Library Genesis和Z-Library）的元数据与Goodreads的结构化书目数据链接，优先选择原生数字EPUB文件，并包含法语、德语和西班牙语的辅助数据集。

Result: 创建了包含53.9万本英文书籍的语料库，涵盖三个世纪，包含首次出版日期、流派、评分和评论等流行度指标，并评估了链接策略的准确性。

Conclusion: MajinBook提供了一个高质量、机器可读的书籍语料库，支持文本和数据挖掘研究，并在欧盟和美国法律框架下讨论了项目的法律可行性。
...
TL;DR: 提出了一种主动式听力辅助系统，能够自动识别和分离佩戴者的对话伙伴，无需明确提示，利用自我语音作为锚点，通过对话动态实时提取对话伙伴音频。


<details>
  <summary>Details</summary>
Motivation: 开发能够主动适应对话动态的听力辅助设备，解决传统听力设备需要明确提示的问题，让听力辅助更加自然和智能化。

Method: 采用双模型架构：轻量级流式模型每12.5毫秒运行一次用于低延迟提取对话伙伴，较慢模型较少运行以捕获长程对话动态，基于自我语音作为锚点，利用对话轮换行为。

Result: 在真实世界的2人和3人对话测试集上（11名参与者共6.8小时数据）表现出良好的泛化能力，能够在多对话环境中有效识别和隔离对话伙伴。

Conclusion: 这项工作标志着向能够主动适应对话动态和参与度的听力辅助设备迈出了重要一步，为更自然的听力辅助体验奠定了基础。
...
Result: 在情感生成、摘要和指令跟随任务中持续优于基线，将Llama3-8B在摘要任务上的性能从1.89提升到2.19，相对改进15.9%。

Conclusion: W2S-AlignTree提供了一种高效、可扩展的推理时对齐解决方案，实现了细粒度控制而不需要修改模型参数。
...
TL;DR: PRBench是一个面向金融和法律领域的专业推理基准测试，包含1,100个专家编写的任务和19,356个专家制定的评分标准，是这两个领域最大的公开基准测试。


<details>
  <summary>Details</summary>
Motivation: 现有学术基准测试对现实世界专业环境中的表现评估有限，特别是在法律和金融等高风险领域，需要评估开放式、具有经济影响的任务。

Method: 招募182名持有JD、CFA证书或6年以上经验的专业人士，基于实际工作流程编写任务，并通过独立专家验证的质量管道验证评分标准。

Result: 评估20个领先模型显示，在困难子集上最高得分仅为0.39（金融）和0.37（法律），表明模型在这些专业领域还有很大改进空间。

Conclusion: 模型在专业推理方面存在显著差距，常见失败模式包括判断不准确、缺乏过程透明度和推理不完整，阻碍了其在专业领域的可靠应用。
...
### [187] [Co-EPG: A Framework for Co-Evolution of Planning and Grounding in Autonomous GUI Agents](https://arxiv.org/abs/2511.10705)
*Yuan Zhao,Hualei Zhu,Tingyu Jiang,Shen Li,Xiaohang Xu,Hao Henry Wang*

Main category: cs.AI

TL;DR: Co-EPG是一个自迭代训练框架，通过规划和接地模型的协同进化实现GUI任务自动化，无需外部数据即可在基准测试中超越现有最优方法。


<details>
  <summary>Details</summary>
Motivation: 当前GUI代理方法存在两个基本限制：(1)跨模型协同利用不足，(2)过度依赖合成数据生成但利用不足。需要解决这些挑战以实现更有效的GUI自动化。

Method: 提出Co-EPG自迭代训练框架，建立规划和接地模型之间的正反馈循环。规划模型通过GRPO在接地奖励指导下探索策略，生成多样化数据优化接地模型；优化后的接地模型为后续规划训练提供更有效奖励，实现持续改进。

Result: 在Multimodal-Mind2Web和AndroidControl基准测试中，仅经过三次迭代就超越了现有最优方法，无需外部数据。代理在每次迭代中持续改进，展现出强大的自我增强能力。
...
### [188] [Picking a Representative Set of Solutions in Multiobjective Optimization: Axioms, Algorithms, and Experiments](https://arxiv.org/abs/2511.10716)
*Niclas Boehmer,Maximilian T. Wittmann*

Main category: cs.AI

TL;DR: 本文重新构建帕累托剪枝为多赢家投票问题，分析了现有质量度量方法的不足，提出了新的定向覆盖度量，并研究了不同质量度量的计算复杂性边界。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的多目标决策问题需要从帕累托最优解集中选择最具代表性的子集，以减轻决策者的认知负担。现有质量度量方法存在反直觉行为，需要改进。

Method: 将帕累托剪枝问题重新构建为多赢家投票问题，进行公理分析；提出新的定向覆盖度量；分析各种质量度量的计算复杂性边界；进行实验评估。

Result: 发现现有质量度量存在反直觉行为；提出了新的定向覆盖度量；识别了可处理和难处理案例的边界；实验表明质量度量选择对解集特征有决定性影响，新度量在多种设置下表现竞争力或更优。

Conclusion: 质量度量选择在多目标决策中至关重要，新提出的定向覆盖度量在多种场景下表现良好，为帕累托剪枝问题提供了更好的解决方案。
...
### [189] [Structure-Aware Encodings of Argumentation Properties for Clique-width](https://arxiv.org/abs/2511.10767)
*Yasir Mahmood,Markus Hecher,Johanna Groven,Johannes K. Fichte*

Main category: cs.AI

TL;DR: 本文研究了图参数clique-width在(Q)SAT编码中的应用，特别关注抽象论证框架。设计了保持clique-width线性的新型归约方法，为所有论证语义（包括计数）建立了新结果。


<details>
  <summary>Details</summary>
Motivation: 树宽度等图结构参数在计算复杂性中很重要，但clique-width作为更一般的参数（对稠密图也有效）在编码方面的研究较少。本文旨在探索clique-width在编码中的能力，选择抽象论证作为研究案例。

Method: 设计了从论证问题到(Q)SAT的新型归约方法，这些归约线性保持clique-width，称为有向分解引导(DDG)归约。

Result: 为所有论证语义（包括计数）建立了新结果，证明DDG归约的开销在合理假设下无法显著改进。

Conclusion: 本文开启了理解clique-width编码能力的研究，通过抽象论证框架展示了clique-width在(Q)SAT编码中的应用潜力，并为未来研究奠定了基础。
...
### [190] [Potential Outcome Rankings for Counterfactual Decision Making](https://arxiv.org/abs/2511.10776)
*Yuta Kawakami,Jin Tian*

Main category: cs.AI

TL;DR: 本文提出了两种新的反事实决策指标：潜在结果排序概率（PoR）和获得最佳潜在结果概率（PoB），用于在不确定性下进行反事实决策，并提供了识别定理、边界估计方法和实证验证。


<details>
  <summary>Details</summary>
Motivation: 在不确定性下进行反事实决策时，决策者通常通过比较候选行动的期望潜在结果来做出选择。本文旨在开发新的决策规则，通过引入PoR和PoB指标来更全面地评估行动偏好。

Method: 提出了PoR和PoB两个新指标，建立了识别定理和边界估计方法，开发了相应的估计方法，并通过数值实验和真实数据集应用进行验证。

Result: 建立了PoR和PoB的识别定理和边界，开发了有效的估计方法，数值实验显示了估计器的有限样本性质，并在真实数据集中成功应用。

Conclusion: PoR和PoB为反事实决策提供了新的有效工具，能够揭示个体最可能的潜在结果排序和获得最佳结果的概率，具有实际应用价值。
...
### [191] [From Efficiency to Adaptivity: A Deeper Look at Adaptive Reasoning in Large Language Models](https://arxiv.org/abs/2511.10788)
*Chao Wu,Baoheng Li,Mingchen Gao,Zhenyi Wang*

Main category: cs.AI

TL;DR: 这篇论文重新定义大语言模型中的推理能力，提出以"适应性"为核心的新视角，即根据任务难度和不确定性动态分配推理资源的能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM对简单和复杂任务采用统一的推理策略，导致对简单问题过度推理而对困难问题推理不足，需要引入适应性推理机制来解决这一根本挑战。

Method: 将适应性推理形式化为控制增强的策略优化问题，提出系统分类法将现有方法分为基于训练的方法（强化学习、监督微调、学习控制器）和免训练方法（提示条件化、反馈驱动停止、模块化组合）。

Result: 建立了连接经典认知范式与算法实现的统一框架，澄清了不同机制如何在实际中实现适应性推理，并支持跨策略的系统比较。

Conclusion: 识别了自我评估、元推理和人类对齐推理控制等开放挑战，为未来研究提供了明确方向。
...
### [192] [HARNESS: Human-Agent Risk Navigation and Event Safety System for Proactive Hazard Forecasting in High-Risk DOE Environments](https://arxiv.org/abs/2511.10810)
...
TL;DR: HARNESS是一个模块化AI框架，用于预测美国能源部环境中的危险事件和操作风险，结合LLM、结构化工作数据、历史事件检索和风险分析，并通过人机协作机制让专家优化预测。


<details>
  <summary>Details</summary>
Motivation: 任务关键工作场所的操作安全至关重要，需要应对复杂危险任务。现有系统需要更主动的风险预测和专家协作机制。

Method: 集成大型语言模型与结构化工作数据、历史事件检索和风险分析，采用人在回路机制让领域专家优化预测，形成自适应学习循环。

Result: 初步部署显示有希望的结果，系统通过专家协作和迭代推理提高了预测安全系统的可靠性和效率。

Conclusion: HARNESS框架通过AI与专家协作提升了操作安全预测能力，未来工作将关注准确性量化评估、专家一致性验证和决策延迟减少。
...
### [193] [HyperComplEx: Adaptive Multi-Space Knowledge Graph Embeddings](https://arxiv.org/abs/2511.10842)
*Jugal Gajjar,Kaustik Ranaware,Kamalasankari Subramaniakuppusamy,Vaibhav Gandhi*

Main category: cs.AI

TL;DR: HyperComplEx是一个混合知识图谱嵌入框架，通过注意力机制自适应结合双曲空间、复数空间和欧几里得空间，解决了现有嵌入方法在处理多样化关系类型时的局限性。
...
Result: 在从1K到10M论文的知识图谱上评估，相比TransE、RotatE、DistMult等基线模型取得一致改进。在10M论文数据集上达到0.612 MRR，相对最佳基线提升4.8%，推理效率为85ms/三元组。

Conclusion: HyperComplEx通过自适应组合多种几何空间，显著提升了知识图谱嵌入的性能和可扩展性，模型大小随图规模近线性增长。
...
### [194] [Advanced Tool for Traffic Crash Analysis: An AI-Driven Multi-Agent Approach to Pre-Crash Reconstruction](https://arxiv.org/abs/2511.10853)
*Gerui Xu,Boyou Chen,Huizhong Guo,Dave LeBlanc,Ananna Ahmed,Zhaonan Sun,Shan Bao*

Main category: cs.AI

TL;DR: 开发了一个多智能体AI框架，用于从碎片化的碰撞数据中重建事故前场景和推断车辆行为，在复杂事故案例中实现了100%的准确率，超越了人类专家的92%准确率。


<details>
  <summary>Details</summary>
Motivation: 传统交通事故重建依赖人工经验，在处理不完整多模态数据时结果不一致。需要开发能够处理碎片化碰撞数据的AI系统来提高重建准确性和一致性。

Method: 采用两阶段协作框架：第一阶段从多模态输入生成自然语言事故重建；第二阶段结合时间性事件数据记录器进行深度事故推理。处理277起追尾前车减速事故，整合文本报告、结构化数据和视觉场景图。

Result: 在39个复杂事故案例评估中，框架实现了100%准确率，成功识别最相关EDR事件并正确区分撞击与被撞车辆，超越了人类研究人员92%的准确率。即使在处理不完整数据时也保持稳健性能。
...
### [195] [Enhancing Demand-Oriented Regionalization with Agentic AI and Local Heterogeneous Data for Adaptation Planning](https://arxiv.org/abs/2511.10857)
*Seyedeh Mobina Noorani,Shangde Gao,Changjie Chen,Karla Saldana Ochoa*

Main category: cs.AI

TL;DR: 开发了一个基于智能AI的规划支持系统，用于创建面向需求的动态规划单元，通过人机协作实现灾害规划的透明性和适应性。


<details>
  <summary>Details</summary>
Motivation: 传统的规划单元（如人口普查区、邮政编码区等）无法准确捕捉当地社区的具体需求，缺乏灵活性来实施有效的灾害预防或响应策略。

Method: 基于代表性初始化空间约束自组织映射（RepSC-SOM），扩展传统SOM方法，结合自适应地理过滤和区域增长细化，AI智能体能够推理、规划和指导整个过程。
...
### [196] [LLM enhanced graph inference for long-term disease progression modelling](https://arxiv.org/abs/2511.10890)
*Tiantian He,An Zhao,Elinor Thompson,Anna Schroder,Ahmed Abdulaal,Frederik Barkhof,Daniel C. Alexander*

Main category: cs.AI

TL;DR: 提出一个使用大型语言模型作为专家指导的新框架，从不规则采样的纵向患者数据中学习神经退行性疾病进展，同时优化长期疾病轨迹构建和生物约束的图结构。


<details>
  <summary>Details</summary>
Motivation: 当前方法过度简化脑连接性，假设单一模态连接组作为疾病传播基质，导致病理传播预测不准确。纯数据驱动方法因缺乏适当约束而面临可识别性问题。

Method: 利用LLM作为区域变量相互作用的专家指导，结合多模态关系和多样化疾病驱动机制，同时优化个体水平观察的长期疾病轨迹构建和生物约束的图结构学习。

Result: 在阿尔茨海默病队列的tau-PET成像数据上验证，新框架相比传统方法显示出更优的预测准确性和可解释性，并揭示了超出传统连接性测量的额外疾病驱动因素。

Conclusion: LLM增强的框架能够更准确地学习疾病进展，提供更好的可识别性和生物学解释性，为神经退行性疾病传播机制研究提供了新途径。
...
### [197] [Multi-Agent Legal Verifier Systems for Data Transfer Planning](https://arxiv.org/abs/2511.10925)
*Ha-Thanh Nguyen,Wachara Fungwacharakorn,Ken Satoh*

Main category: cs.AI

TL;DR: 提出多代理法律验证器，将合规检查分解为法规解释、业务背景评估和风险评估等专业代理，通过结构化合成协议协调，在APPl合规验证中准确率达72%，比单代理基线提高21个百分点。


<details>
  <summary>Details</summary>
Motivation: 在严格的隐私法规（如日本个人信息保护法APPl）下，AI驱动的数据传输规划中的法律合规性变得越来越重要，需要可扩展且符合法规的自动化合规验证框架。

Method: 多代理法律验证器，将合规检查分解为法规解释、业务背景评估和风险评估等专业代理，通过结构化合成协议协调工作。

Result: 在200个APPl第16条修正案案例数据集上，系统准确率达72%，比单代理基线提高21个百分点；在明确合规案例上准确率达90%（基线仅16%），同时保持对明确违规的完美检测。

Conclusion: 领域专业化和协调推理能够显著提高法律AI性能，为可信赖和可解释的自动化合规验证提供了可扩展且符合法规的框架，但在模糊场景中仍存在挑战。
...
### [198] [Requirements for Aligned, Dynamic Resolution of Conflicts in Operational Constraints](https://arxiv.org/abs/2511.10952)
*Steven J. Jones,Robert E. Wray,John E. Laird*

Main category: cs.AI

TL;DR: 该论文研究了自主AI系统在遇到训练数据之外的新场景时，如何构建、评估和证明候选行动方案，以满足人类期望和价值观。


<details>
  <summary>Details</summary>
Motivation: 自主AI系统在部署后必然会遇到训练数据未覆盖的新场景，需要超越训练策略来构建和评估行动方案，以符合人类期望和价值观。

Method: 通过理论分析和实证案例研究，探讨了智能体如何整合规范性、实用性和情境性理解来选择更符合人类期望的行动方案。

Result: 识别了智能体在复杂现实环境中做出稳健决策所需的知识类型，包括规范性、实用性和情境性知识。

Conclusion: 为实现与人类期望和价值观一致的决策，自主AI系统需要整合多种知识类型来构建、评估和证明候选行动方案。
...
### [199] [AI Agent-Driven Framework for Automated Product Knowledge Graph Construction in E-Commerce](https://arxiv.org/abs/2511.11017)
...
<details>
  <summary>Details</summary>
Motivation: 电商平台产生大量非结构化产品数据，知识图谱能提供结构化表示，但构建产品专用知识图谱仍复杂且依赖人工

Method: 使用大语言模型驱动的三阶段代理框架：本体创建与扩展、本体精炼、知识图谱填充，确保语义一致性和可扩展性
...
### [200] [Faster Symmetry Breaking Constraints for Abstract Structures](https://arxiv.org/abs/2511.11029)
...
<details>
  <summary>Details</summary>
Motivation: 在约束编程中，抽象结构需要通过转换才能在求解器中使用。对称性打破是重要的优化技术，但应用于抽象变量时会产生大量复杂约束，导致性能不佳。

Method: 开发了一种新的不完全对称性打破方法，通过更好地利用抽象结构的表示来处理对称性，特别针对不可区分对象产生的对称性。

Result: 该方法比(Akgün et al. 2025)中提出的先前方法更快。

Conclusion: 新方法通过改进抽象结构的表示利用，有效解决了对称性打破在抽象变量上的性能问题，为约束编程中的对称性处理提供了更高效的解决方案。
...
### [201] [Key Decision-Makers in Multi-Agent Debates: Who Holds the Power?](https://arxiv.org/abs/2511.11040)
*Qian Zhang,Yan Zheng,Jinyi Liu,Hebin Liang,Lanjun Wang*

Main category: cs.AI

TL;DR: 研究发现角色分配策略对多智能体辩论性能有显著影响，提出"Truth Last"策略可提升推理任务表现22%，并开发MADC策略解决实际应用中真理未知的问题。


<details>
  <summary>Details</summary>
Motivation: 多智能体辩论在增强推理能力方面有潜力，但角色分配策略的关键方面尚未充分探索，特别是在实际应用中真理未知的情况。

Method: 提出"Truth Last"角色分配策略，并开发多智能体辩论一致性(MADC)策略，通过路径一致性评估独立角色间的一致性，模拟一致性得分最高的角色作为真理。

Result: 在9个LLM模型（包括DeepSeek-R1蒸馏模型）的挑战性推理任务中验证，MADC持续表现出先进性能，有效克服MAD的性能瓶颈。

Conclusion: MADC为LLM智能体扩展提供了关键改进路径，系统性地模拟和优化了多智能体辩论的核心机制。
...
Method: 使用可微分模拟器Waymax作为状态预测器和评估器，利用其硬编码动力学实现准确状态预测，通过可微分性在动作序列上进行有效搜索，使用梯度下降优化未来轨迹中的动作。

Result: 实验表明，DSS（规划梯度和随机搜索的组合）相比序列预测、模仿学习、无模型RL和其他规划方法，显著提高了跟踪和路径规划精度。

Conclusion: DSS框架通过结合规划梯度和随机搜索，在自动驾驶规划任务中表现出优越性能，为安全可靠的自动驾驶系统提供了有效解决方案。
...
TL;DR: ARCTraj是一个用于建模人类抽象推理的数据集和方法框架，通过记录时间有序的对象级动作来揭示推理过程，包含约10,000条轨迹和400个训练任务。


<details>
  <summary>Details</summary>
Motivation: 现有的ARC方法主要依赖静态输入-输出监督，无法洞察推理的时间展开过程，ARCTraj旨在填补这一空白，揭示传统数据集忽略的中间推理步骤。

Method: 通过O2ARC网络界面收集数据，定义统一的推理管道，包括数据收集、动作抽象、MDP制定和下游学习，可与强化学习、生成建模和序列建模方法集成。

Result: 收集了约10,000条带时间戳和成功标签的轨迹，分析了空间选择、颜色归因和策略收敛等人类推理的结构和多样性。
...
Method: 对每个训练问题，按顺序计算每个目标原子的最优规划，进行目标回归，并将输出提升为一阶条件-动作规则集合。
...
TL;DR: GGBench是一个专门评估几何生成推理能力的基准测试，旨在填补现有基准在评估统一多模态模型集成认知过程方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要评估判别性理解或无约束图像生成，无法衡量生成推理的集成认知过程，几何构造作为需要语言理解和精确视觉生成的融合任务，提供了理想的测试平台。

Method: 提出GGBench基准测试，通过几何构造任务系统性地诊断模型的理解、推理和主动构建解决方案的能力。

Result: GGBench为下一代智能系统设定了更严格的标准，能够全面评估模型的几何生成推理能力。

Conclusion: 几何构造是评估统一多模态模型生成推理能力的理想测试平台，GGBench基准填补了现有评估方法的空白，推动了更智能系统的发展。
...
TL;DR: 提出MUG协议，通过多模态反事实测试检测幻觉代理，改进多智能体辩论框架，提升多模态推理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 解决LLM推理中的幻觉问题，现有MAD方法假设所有辩论者都是理性的，但实际代理本身可能存在幻觉。

Method: 受"谁是卧底"游戏启发，通过修改参考图像引入反事实证据，检测无法识别这些变化的"卧底"代理（幻觉代理）。
...
TL;DR: STaR框架通过慢思考机制增强LLMs的表格推理能力，使用两阶段难度感知强化学习和不确定性量化，提升推理深度和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有LLMs表格推理方法存在两个关键局限：(i) 推理过程缺乏人类认知的深度和迭代细化；(ii) 推理过程不稳定，影响下游应用可靠性。

Method: STaR框架通过显式建模逐步思考和不确定性感知推理，为LLMs配备慢思考能力。训练阶段采用两阶段难度感知强化学习，从简单到复杂查询逐步学习；推理阶段通过整合token级置信度和答案一致性进行轨迹级不确定性量化。

Result: 在基准测试上的广泛实验表明，STaR实现了优越性能和增强的推理稳定性。在域外数据集上的强泛化能力进一步证明了其作为可靠表格推理解决方案的潜力。

Conclusion: STaR是一个可靠且受认知启发的表格推理解决方案，通过慢思考机制显著提升了LLMs在结构化数据分析中的推理深度和稳定性。
...
### [208] [UAVBench: An Open Benchmark Dataset for Autonomous and Agentic AI UAV Systems via LLM-Generated Flight Scenarios](https://arxiv.org/abs/2511.11252)
...
Method: 通过分类指导的LLM提示和多阶段安全验证生成5万个验证过的无人机飞行场景，并创建包含10种推理风格的5万个多选题基准。
...
### [209] [AIonopedia: an LLM agent orchestrating multimodal learning for ionic liquid discovery](https://arxiv.org/abs/2511.11257)
*Yuqi Yin,Yibo Fu,Siyuan Wang,Peng Sun,Hongyu Wang,Xiaohui Wang,Lei Zheng,Zhiyong Li,Zhirong Liu,Jianji Wang,Zhaoxi Sun*

Main category: cs.AI

TL;DR: AIonopedia是首个基于大语言模型的离子液体发现智能体，通过多模态领域基础模型实现准确性质预测和分层搜索架构，在真实湿实验验证中表现出优异泛化能力


<details>
  <summary>Details</summary>
Motivation: 离子液体发现面临数据有限、模型精度差和工作流程碎片化等挑战，需要开发更有效的发现方法

Method: 构建LLM增强的多模态离子液体领域基础模型，采用分层搜索架构进行分子筛选和设计，使用新构建的全面离子液体数据集进行训练和评估

Result: 模型性能优异，在文献报道系统上能有效进行离子液体修饰，真实湿实验验证显示在分布外任务上具有卓越泛化能力

Conclusion: AIonopedia能够加速真实世界的离子液体发现，为IL研究提供了有效的AI驱动解决方案
...
### [210] [A Workflow for Full Traceability of AI Decisions](https://arxiv.org/abs/2511.11275)
*Julius Wenzel,Syeda Umaima Alam,Andreas Schmidt,Hanwei Zhang,Holger Hermanns*

Main category: cs.AI

TL;DR: 本文提出了一个可验证的AI决策追踪系统，通过记录训练和推理过程中的所有组件，为高风险AI决策提供防篡改的完整追溯记录。


<details>
  <summary>Details</summary>
Motivation: 随着AI系统在高风险决策中的广泛应用，存在决策过程缺乏适当文档记录的问题，这阻碍了责任链的追溯，特别是在AI决策违反法律时难以在法庭上确定责任。

Method: 扩展DBOM概念，利用机密计算技术构建运行工作流，强制记录训练和推理过程中的每个组件，生成防篡改、可验证的完整AI决策痕迹。

Result: 开发了一个能够区分有毒和可食用蘑菇的应用作为示例，展示了工作流的实际运行机制，证明了该方法的可行性。

Conclusion: 该方法为解决AI决策责任追溯问题提供了实用且彻底的解决方案，为AI系统在法律环境下的问责制奠定了基础。
...
### [211] [Can You Tell the Difference? Contrastive Explanations for ABox Entailments](https://arxiv.org/abs/2511.11281)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法只能单独解释正蕴含或缺失蕴含，无法同时考虑两者来揭示个体间的关键差异，需要一种能够对比分析的解释方法。

Method: 为描述逻辑本体的ABox推理开发了对比解释的适当概念，分析了不同变体在不同最优性标准下的计算复杂度，涵盖轻量级和更表达性的描述逻辑。

Result: 实现了计算对比解释变体的首个方法，并在现实知识库的生成问题上进行了评估。

Conclusion: 对比ABox解释能够有效回答个体分类差异的原因，通过同时考虑正反例来提供更有针对性的解释。
...
### [212] [EcoAlign: An Economically Rational Framework for Efficient LVLM Alignment](https://arxiv.org/abs/2511.11301)
*Ruoxi Cheng,Haoxuan Ma,Teng Ma,Hongyi Zhang*

Main category: cs.AI

TL;DR: EcoAlign是一个推理时框架，通过将大型视觉语言模型视为有限理性智能体，重新构建对齐问题为经济理性搜索，在降低计算成本的同时实现更好的安全性和实用性。


<details>
  <summary>Details</summary>
Motivation: 当前对齐方法在安全性、实用性和运营成本之间存在权衡困难，仅关注最终输出的过程盲目性会浪费大量计算预算在不安全的推理上，有害推理可能通过良性理由伪装来规避安全检测。

Method: 提出EcoAlign框架，增量扩展思维图，使用前瞻性函数（类似净现值）对行动评分，动态权衡预期安全性、实用性和成本与剩余预算，并通过最弱环节原则强制执行路径安全性。
...
### [213] [RLSLM: A Hybrid Reinforcement Learning Framework Aligning Rule-Based Social Locomotion Model with Human Social Norms](https://arxiv.org/abs/2511.11323)
*Yitian Kou,Yihe Gu,Chen Zhou,DanDan Zhu,Shuguang Kuai*

Main category: cs.AI

TL;DR: RLSLM是一个结合强化学习和基于规则的社会运动模型的混合框架，用于实现社会意识导航，通过量化人类舒适度来优化机械能和社会舒适度。
...
Method: 提出RLSLM框架，将基于经验行为实验的社会运动模型集成到强化学习的奖励函数中，生成方向敏感的社会舒适场。

Result: 在沉浸式VR实验中，RLSLM在用户体验方面优于最先进的基于规则模型，且比传统数据驱动方法具有更好的可解释性。
...
### [214] [KarmaTS: A Universal Simulation Platform for Multivariate Time Series with Functional Causal Dynamics](https://arxiv.org/abs/2511.11357)
*Haixin Li,Yanke Li,Diego Paez-Granados*

Main category: cs.AI

TL;DR: KarmaTS是一个交互式框架，用于构建滞后索引、可执行的时空因果图模型，用于多变量时间序列模拟。该系统通过混合主动、人在回路的工作流程，结合专家知识和算法建议来构建离散时间结构因果过程。
...
### [215] [MarsRL: Advancing Multi-Agent Reasoning System via Reinforcement Learning with Agentic Pipeline Parallelism](https://arxiv.org/abs/2511.11373)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在单次推理过程中的输出长度限制了推理深度，而多代理推理系统在开源模型中表现不佳，主要由于批评和修正能力不足。

Method: 提出MarsRL框架，采用代理特定奖励机制减少奖励噪声，并使用管道式训练来处理长轨迹，联合优化求解器、验证器和修正器等所有代理。

Result: 在Qwen3-30B-A3B-Thinking-2507模型上，MarsRL将AIME2025准确率从86.5%提升至93.3%，BeyondAIME从64.9%提升至73.8%，甚至超越了Qwen3-235B-A22B-Thinking-2507。

Conclusion: MarsRL展示了在多代理推理系统中推进强化学习的潜力，能够显著提升开源模型在复杂推理任务上的表现，具有广泛的适用性。
...
### [216] [Robust and Efficient Communication in Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2511.11393)
*Zejiao Liu,Yi Li,Jiali Wang,Junqi Tu,Yitian Hong,Fangfei Li,Yang Liu,Toshiharu Sugawara,Yang Tang*

Main category: cs.AI

TL;DR: 本调查系统回顾了在多智能体强化学习(MARL)中，针对现实约束条件(如消息扰动、传输延迟和有限带宽)下的鲁棒高效通信策略的最新进展，重点关注三个应用领域，并提出了统一设计方法以弥合理论与实践的差距。
...
Method: 系统调查和综述方法，分析在消息扰动、传输延迟和有限带宽等现实约束下的MARL通信策略，并聚焦于三个具体应用领域。

Result: 识别了当前MARL通信策略的研究进展，特别是在应对现实通信约束方面的创新方法，并明确了理论与实践之间的差距。
...
### [217] [CURENet: Combining Unified Representations for Efficient Chronic Disease Prediction](https://arxiv.org/abs/2511.11423)
...
Method: 使用大型语言模型处理临床文本和文本实验室测试，以及transformer编码器处理纵向序列访问，整合多模态EHR数据。
...
### [218] [Experience-Guided Adaptation of Inference-Time Reasoning Strategies](https://arxiv.org/abs/2511.11519)
...
Method: 使用基于LLM的元策略来生成策略，包含两个组件：Guide根据当前问题和结构化记忆生成候选策略，Consolidator整合执行反馈来改进未来策略生成。

Result: 在五个挑战性基准测试中，EGuR相比最强基线准确率提升最高14%，计算成本降低最高111倍，且随着经验积累性能持续提升。

Conclusion: EGuR证明了在推理时动态生成完整策略的可行性，实现了AI系统在部署后的持续自适应改进，同时显著提升性能和效率。
...
### [219] [Aligning Machiavellian Agents: Behavior Steering via Test-Time Policy Shaping](https://arxiv.org/abs/2511.11551)
...
<details>
  <summary>Details</summary>
Motivation: AI智能体在复杂动态环境中部署时面临与人类价值观或指导方针保持一致的挑战，预训练智能体确保对齐尤其困难，因为重新训练成本高昂且缓慢，且伦理价值观属性多样且可能冲突。

Method: 使用模型引导策略塑造方法，在测试时通过场景-行动属性分类器应用策略塑造，确保决策与伦理属性对齐。在MACHIAVELLI基准上进行评估，该基准包含134个基于文本的游戏环境和数千个涉及伦理决策的标注场景。

Result: 测试时策略塑造提供了有效且可扩展的解决方案，能够减轻不同环境和对齐属性中的不道德行为，优于先前的训练时方法和通用智能体。

Conclusion: 测试时策略塑造是确保AI智能体在复杂环境中保持伦理对齐的有效方法，无需重新训练即可实现精确的行为控制和原则性权衡。
