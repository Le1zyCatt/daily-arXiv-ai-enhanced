...
TL;DR: 提出Pygmalion Effect框架，通过图像到黏土转换抑制镜面反射线索，实现反射物体的鲁棒3D重建


<details>
  <summary>Details</summary>
Motivation: 解决3D重建中由于视角依赖反射导致的外观和几何纠缠问题

Method: 双分支网络：基于BRDF的反射分支+黏土引导分支，使用合成的黏土样图像进行联合训练

Result: 在合成和真实数据集上显著提高了法线精度和网格完整性
...
<details>
  <summary>Details</summary>
Motivation: 传统仅使用几何线索（如深度、边缘）的方法不足以约束外观、材质和光照，限制了物理上有意义的编辑（如重新照明或材质交换），并经常导致时间漂移。需要更丰富的图形模态来提供互补约束。

Method: 提出CtrlVDiff模型，采用混合模态控制策略（HMCS）来路由和融合深度、法线、分割、边缘和图形内在属性（反照率、粗糙度、金属度）的特征，并构建了MMVideo混合数据集进行训练。

Result: 在理解和生成基准测试中，CtrlVDiff提供了卓越的可控性和保真度，支持分层编辑（重新照明、材质调整、对象插入），超越了现有最先进方法，并在某些模态不可用时保持鲁棒性。

Conclusion: 通过融合多种图形模态，CtrlVDiff成功解决了视频理解和可控生成的双重挑战，实现了精确、可预测的控制，同时保持了时间一致性。
...
<details>
  <summary>Details</summary>
Motivation: 随着多模态模型的广泛应用，其训练数据隐私泄露风险日益突出。现有研究主要关注单模态系统的隐私攻击，而多模态模型的隐私脆弱性研究不足，特别是神经启发的多模态模型对隐私攻击的抵抗力尚未探索。

Method: 提出系统性的神经科学启发拓扑正则化框架(tau)，在三种视觉语言模型(BLIP、PaliGemma 2、ViT-GPT2)上应用该正则化，其中tau>0的配置定义为神经启发变体，并在三个基准数据集(COCO、CC3M、NoCaps)上进行评估。

Result: 在BLIP模型和COCO数据集上的实验显示，神经启发VLM的成员推理攻击成功率平均ROC-AUC下降了24%，同时保持了相似的模型效用(MPNet和ROUGE-2指标)，在其他模型和数据集上的评估也验证了结果的稳定性。

Conclusion: 神经启发的多模态视觉语言模型相比基线模型对隐私攻击具有更强的抵抗力，且不会显著损害模型性能，这为理解多模态模型的隐私风险提供了重要见解。
...
Result: Inferix专门针对世界模拟优化，区别于高并发系统（如vLLM或SGLang）和经典视频扩散模型（如xDiTs），提供交互式视频流和性能分析功能，支持实时交互和真实模拟。

Conclusion: Inferix作为专门的世界模型推理引擎，通过优化的半自回归解码和集成评估基准，推动世界模型的探索和发展，希望社区共同推进这一方向。
...
TL;DR: 提出LTED-Ada算法，通过深度强化学习自适应选择本地跟踪或边缘检测，在单设备和多设备场景下优化视频对象识别性能，考虑帧率、精度和延迟要求。


<details>
  <summary>Details</summary>
Motivation: 资源受限设备（如交通摄像头）上的视频对象识别面临挑战，混合方法（边缘检测+本地跟踪）需要解决何时执行边缘检测与本地跟踪的决策问题。

Method: 提出LTED-Ada算法，基于深度强化学习自适应选择策略；在多设备场景下使用联邦学习进行协作策略训练；通过硬件在环实验验证。

Result: 实验结果表明LTED-Ada在多个树莓派4B设备和PC边缘服务器上表现出优越性能。

Conclusion: LTED-Ada算法能够有效解决视频对象识别中的边缘检测与本地跟踪决策问题，在单设备和多设备场景下均表现优异。
...
TL;DR: DeeAD是一个无需训练的早期退出框架，通过评估中间轨迹的物理可行性来加速视觉语言动作模型的推理，在保持规划质量的同时减少28%的Transformer层使用和29%的延迟。
...
TL;DR: 提出了Foundation Model Distillation (FMD)新范式，将大型自监督学习模型压缩成紧凑高效的代理模型，保持其通用表征能力。Foundry是该范式的首个3D点云实现。


<details>
  <summary>Details</summary>
Motivation: 基础模型虽然功能强大，但尺寸巨大、计算成本高，难以部署在边缘设备上。现有压缩技术会牺牲模型的通用性，而FMD旨在解决这一矛盾。

Method: 训练学生模型学习压缩的SuperTokens集合，重构教师的token级表征，捕获其潜在空间的紧凑基。Foundry是该方法的3D点云实现。

Result: 单个蒸馏模型在分类、部分分割和少样本场景等多样化下游任务中保持强迁移性，接近完整基础模型性能，同时显著减少token数量和FLOPs。

Conclusion: FMD范式成功压缩了大型SSL模型，使其更适合在资源受限的硬件上部署，同时保持了通用表征能力。
...
TL;DR: DinoLizer是基于DINOv2的生成式修复图像篡改区域定位模型，通过线性分类头和滑动窗口策略实现高精度检测，在多个数据集上超越现有方法。


<details>
  <summary>Details</summary>
Motivation: 针对生成式修复图像中语义篡改区域的定位问题，利用DINOv2强大的表征能力来检测被篡改的区域。

Method: 在预训练的DINOv2模型上添加线性分类头，使用滑动窗口策略处理大尺寸图像，通过后处理优化二进制篡改掩码。

Result: 在多个修复数据集上超越现有最优方法，平均IoU提高12%，对常见后处理操作保持鲁棒性。

Conclusion: DinoLizer证明了Vision Transformers在篡改定位任务中的强大表征能力，并通过消融研究验证了其优越性。
...
TL;DR: CANVAS是一个评估视觉语言模型在基于工具的用户界面设计方面性能的基准，包含598个基于工具的设计任务，涵盖设计复制和设计修改两种任务类型。


<details>
  <summary>Details</summary>
Motivation: 目前没有现有的基准来评估基于工具的设计性能，因此无法了解视觉语言模型在传统设计软件中进行UI设计迭代的能力。

Method: 创建包含598个基于工具设计任务的CANVAS基准，从3.3K个移动UI设计中采样，涵盖30个功能类别，包含设计复制和设计修改两种任务类型。

Result: 领先的模型表现出更策略性的工具调用，提高了设计质量，同时识别了模型常见的错误模式。

Conclusion: CANVAS基准为评估和提升基于工具的设计能力提供了指导，展示了视觉语言模型在设计软件中协作的潜力。
...
<details>
  <summary>Details</summary>
Motivation: 传统视觉语言模型中的图像编码器通常与语言模型独立预训练，导致编码器处理图像时忽略下游任务和文本查询的具体需求。

Method: 开发文本引导的语义图像编码器(TIE)，根据输入文本查询生成条件化的图像表示，使编码器能够捕获与查询相关的关键视觉特征。

Result: 在1B和3B规模的九个图像到文本基准测试中平均提升1.5和1.3个点，DocVQA和InfoVQA任务上提升高达6个点，同时仅使用一半图像tokens实现更高效推理。

Conclusion: TIE通过文本条件化训练有效优化编码器，使其能够关注查询相关区域，提升性能、效率和可解释性。
...
TL;DR: SMARC是一个从最小视觉输入进行表面材料重建和分类的统一模型，仅需10%的图像补丁即可重建完整RGB表面并分类材料类别，在稀疏观测下实现最先进的性能。
...
Method: 结合部分卷积U-Net和分类头的架构，在极端观测稀疏度下同时实现空间修复和语义理解。
...
TL;DR: LongVT是一个端到端的多模态代理框架，通过全局到局部的推理循环解决长视频理解中的幻觉问题，利用LMMs的时间定位能力作为视频裁剪工具进行细粒度推理。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模多模态模型在处理长视频时容易产生幻觉，因为证据稀疏且时间分散。受人类理解长视频的方式启发（先全局浏览再查看相关片段），需要开发能够进行细粒度推理的框架。

Method: 提出LongVT框架，利用LMMs的时间定位能力作为原生视频裁剪工具，通过多模态工具链思维进行全局到局部的推理循环，直到答案基于检索到的视觉证据。采用三阶段训练策略：工具集成冷启动监督微调、代理强化学习和代理强化微调。

Result: 在四个具有挑战性的长视频理解和推理基准测试中，LongVT始终优于现有强基线模型。发布了包含247.9K训练样本和1,280个QA对评估基准的数据集VideoSIAH。

Conclusion: LongVT通过代理式多模态推理框架有效解决了长视频理解中的幻觉问题，证明了全局到局部推理策略的有效性，为长视频推理任务提供了新的解决方案。
...
TL;DR: 轻量级复现KRISP模型，参数大幅减少但性能保持75%，揭示了原始设计缺陷，在资源受限条件下验证知识增强VQA架构的可扩展性。


<details>
  <summary>Details</summary>
Motivation: 原始KRISP模型虽然有效但计算需求大、与大型主干网络紧密耦合，不适合资源受限环境。本研究旨在重新审视该架构，开发轻量级版本。

Method: 通过系统消融研究进行轻量级复现，包括在合成VQA数据上的概念验证和在DAQUAR数据集上的评估，使用低参数配置并限制外部知识图谱领域。

Result: 复现模型参数显著减少，性能达到原版的75%，能够防止AI幻觉并仅在限定知识领域内生成输出，可在智能手机和AR-VR等边缘设备上运行。

Conclusion: 轻量级KRISP复现揭示了原始设计中的缺陷，证明了知识增强VQA架构在资源受限环境下的可行性，为离线视觉推理提供了实用解决方案。
...
TL;DR: 本文提出了一个名为“warping”的统一算子来概括现有的动态采样方法，分析了其统计特性，发现了前向和后向传播的不对称性，并研究了确保动态采样网络稳定训练的条件。


<details>
  <summary>Details</summary>
Motivation: 深度学习架构中的动态采样机制在计算机视觉模型中显示出实用性，但这些结构的理论分析尚未统一。本文旨在连接各种动态采样方法，提供一个可分析的最小实现。

Method: 开发并分析了一个称为“warping”的新算子，该算子概括了现有方法，包括可变形卷积、主动卷积单元和空间变换网络。使用形式化方法对算子进行统计分析，建模输入为IID变量和齐次随机场。

Result: 发现了模型训练中前向和后向传播的独特不对称性，证明这些机制代表了与传统平移不变卷积算子完全不同的正交算子类别。找到了确保动态采样网络稳定训练的条件，并研究了离散化效应的统计分析。

Conclusion: 通过理论分析和实证研究，确定了动态采样网络稳定训练的必要条件，并引入了一种利用梯度更新信息的新型损失景观可视化方法，以更好地理解学习行为。
...
TL;DR: Δ-NeRF是一种模块化残差框架，用于神经辐射场的增量式优化，无需访问历史数据即可实现持续改进，特别适用于卫星地形分析等顺序数据场景。


<details>
  <summary>Details</summary>
Motivation: 现有NeRF框架在引入新视角时需要完全重新训练，限制了其在顺序数据场景（如卫星地形分析）中的应用。增量式优化方法存在灾难性遗忘问题，且该领域研究不足。

Method: 提出Δ-NeRF框架，包含：残差控制器向冻结的基础NeRF注入逐层修正；不确定性感知门控机制防止过度修正；视图选择策略减少47%训练数据；知识蒸馏将增强模型压缩至原大小20%。

Result: 在卫星图像实验中，Δ-NeRF性能与联合训练相当，训练时间减少30-42%，PSNR比简单微调提升43.5%，在某些指标上甚至超越联合训练。

Conclusion: Δ-NeRF成功解决了NeRF增量优化的挑战，在保持性能的同时显著提升效率，为顺序数据场景下的3D重建提供了有效解决方案。
...
TL;DR: 提出Split-then-Merge (StM)框架，通过将未标注视频分割为前景和背景层并进行自组合，解决生成视频合成的控制问题和数据稀缺问题。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖标注数据集或手工规则，难以处理动态主体与多样化场景的复杂交互，且面临数据稀缺问题。

Method: 将未标注视频分割为动态前景和背景层，通过自组合学习主体与场景的交互；采用变换感知训练管道，包括多层融合增强和身份保持损失。

Result: 在定量基准测试和人类/VLLM定性评估中均优于最先进方法。

Conclusion: StM框架有效解决了生成视频合成的控制问题，通过自监督学习实现了逼真的视频生成。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型视觉语言模型在视觉推理任务上表现不佳，需要更精确的评估方法和训练策略来提升其多模态推理能力。

Method: 开发Sphinx合成环境，程序化生成25种任务类型的视觉推理谜题，并使用强化学习与可验证奖励（RLVR）来训练模型。

Result: 评估显示最先进的GPT-5模型准确率仅为51.1%，远低于人类水平；RLVR训练显著提升了模型在这些任务上的准确率，并在外部视觉推理基准上获得改进。

Conclusion: Sphinx为视觉推理提供了精确的评估基准，RLVR训练方法展示了提升多模态推理能力的潜力。
...
TL;DR: 本文提出了优化视觉反演(OVI)方法，无需训练先验网络即可替代传统扩散模型中的文本到图像先验网络，通过优化潜在视觉表示来匹配文本嵌入，并引入两种约束损失来提高图像质量。


<details>
  <summary>Details</summary>
Motivation: 传统扩散模型依赖计算昂贵的训练先验网络将文本嵌入转换为视觉流形，这些先验需要大量数据和训练。本文质疑这种先验的必要性，寻求无需训练和数据的替代方案。

Method: 使用优化视觉反演(OVI)方法，从随机伪标记初始化潜在视觉表示，通过迭代优化最大化与文本提示嵌入的余弦相似度。提出两种约束：基于马哈拉诺比斯距离的损失和最近邻损失，以正则化优化过程朝向真实图像分布。

Result: 在Kandinsky 2.2上的实验表明OVI可作为传统先验的替代方案。分析揭示了当前评估基准T2I-CompBench++的缺陷，仅使用文本嵌入作为先验就能获得高分但感知质量较低。约束OVI方法改善了视觉保真度，最近邻方法尤其有效，达到了与最先进数据高效先验相当或更高的定量分数。

Conclusion: OVI方法展示了无需训练先验网络的可行性，为文本到图像生成提供了有前景的替代方案，值得进一步研究。
...
TL;DR: RefTr是一个用于血管树中心线生成的3D图像到图模型，通过循环精炼汇合轨迹来生成具有正确树形拓扑的中心线图，在保持高召回率的同时减少参数并提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 在临床诊断和治疗规划中，准确检测血管树中心线并保持正确的树形拓扑至关重要。高召回率尤为重要，因为遗漏小分支可能导致致命错误。

Method: 采用Producer-Refiner架构，基于Transformer解码器。Producer提出初始汇合轨迹，Refiner循环精炼这些轨迹形成最终中心线图。汇合轨迹表示确保有效树形拓扑，循环精炼方案重用相同Refiner块减少参数。
...
TL;DR: 提出了首个高分辨率（5472×3648px）立体DSLR数据集，包含18000张图像，系统性地变化焦距和光圈，覆盖复杂真实场景，旨在弥合合成训练数据与真实相机光学之间的真实感差距。


<details>
  <summary>Details</summary>
Motivation: 解决深度估计在真实光学条件下的可靠性问题，当前研究受限于缺乏大规模、高保真度的真实立体DSLR数据集，限制了模型在真实世界中的泛化能力和评估。

Method: 使用两个相同的相机组件，在9个不同场景复杂度、光照和背景的场景中，以10个焦距（28-70mm）和5个光圈（f/2.8-f/22）拍摄，每个场景2000张图像，共50种光学配置。

Result: 创建了包含挑战性视觉元素（如多尺度光学错觉、反射表面、镜子、透明玻璃墙等）的数据集，支持几何和光学效应的受控分析。

Conclusion: 该数据集揭示了当前最先进的单目、立体深度和景深方法面临的挑战，并发布了数据集、校准文件和评估代码以支持可重复研究。
...
TL;DR: 提出了首个大规模无监督视觉记忆性数据集，包含82,000多个视频及其描述性回忆数据，用于建模视觉记忆信号，在回忆生成和舌尖现象检索任务上表现优异。
...
Method: 利用Reddit等在线平台的舌尖现象检索查询构建数据集，采用对比训练策略创建多模态舌尖现象检索模型，并微调大型视觉语言模型。

Result: 基于该数据集微调的视觉语言模型在生成视觉内容的开放式记忆描述方面优于GPT-4o等最先进模型，并创建了首个能够执行多模态舌尖现象检索的模型。
...
TL;DR: 提出一种从立体雾天图像序列中同时估计所有雾模型参数的优化方法，避免传统顺序估计的错误传播问题，并能处理真实世界中全局不均匀的雾。


<details>
  <summary>Details</summary>
Motivation: 现有方法顺序估计雾参数容易导致误差传播，且无法有效处理真实世界中全局不均匀的雾。需要一种能同时估计所有参数并适应真实雾条件的方法。
...
Conclusion: 该方法能有效估计雾参数，适应真实雾条件，并发布了SDIRF数据集和代码以推动雾中视觉感知研究。
...
<details>
  <summary>Details</summary>
Motivation: 跨视角物体对应任务（如ego-exo物体对应）由于视角和外观的剧烈变化，使得现有分割模型（如SAM2）难以直接应用，需要专门的方法来处理这种跨视角的对应关系。
...
Result: 在Ego-Exo4D（ego-exo物体对应）、DAVIS-2017（视频物体跟踪）和HANDAL-X（机器人就绪跨视角对应）上取得了新的最先进性能。

Conclusion: V^2-SAM通过统一的跨视角对应框架有效解决了视角和外观变化带来的挑战，在两个互补提示生成器和自适应专家选择机制的支持下，在多个跨视角对应任务中表现出色。
...
TL;DR: 提出Null-Text Test-Time Alignment (Null-TTA)方法，通过优化分类器自由引导中的无条件嵌入来对齐扩散模型，在语义空间进行优化，防止奖励黑客攻击，实现目标测试时对齐并保持跨奖励泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有测试时对齐方法存在欠优化或过优化（奖励黑客）问题，需要一种在语义相干流形上对齐的方法来防止利用非语义噪声模式提高奖励。

Method: 优化分类器自由引导中的无条件嵌入，而不是操纵潜在变量或噪声变量，在结构化语义的文本嵌入空间中进行对齐。
...
TL;DR: GaINeR是一个结合可训练高斯分布和神经网络的2D图像隐式表示框架，支持连续图像表示、可解释几何结构和灵活局部编辑。
...
Result: 实现了连续图像表示、可解释几何结构和灵活局部编辑能力，为物理感知和交互式图像操作提供基础。
...
TL;DR: 本研究开发了一种深度学习模型，用于降低CPA区域增强T1加权MRI的造影剂剂量，在仅使用10%-30%标准剂量的情况下仍能获得良好的图像质量和诊断性能。


<details>
  <summary>Details</summary>
Motivation: 降低MRI检查中造影剂的使用剂量，减少患者暴露于造影剂的风险，同时保持诊断图像质量。

Method: 使用前庭神经鞘瘤患者的T1和增强T1加权MRI数据，模拟不同剂量减少的低剂量图像，训练深度学习模型从低剂量图像恢复标准剂量图像质量。

Result: 在10%输入剂量下，DL恢复的图像将分割Dice系数从0.673提升至0.734，图像质量指标显著改善。10%和30%剂量的恢复图像均表现优秀，后者信息更丰富。

Conclusion: 深度学习模型显著改善了CPA区域低剂量MRI的图像质量，使得仅使用10%-30%标准剂量即可实现病变检测和诊断特征分析。
...
TL;DR: 提出一种平滑正则化技术，通过高斯随机游走建模视频帧间嵌入变化，增强轻量级视频识别模型的时序一致性，在Kinetics-600数据集上提升准确率3.8%-6.4%。


<details>
  <summary>Details</summary>
Motivation: 轻量级视频识别模型难以有效捕捉复杂的时序动态，需要更强的时序归纳偏置来提升性能。

Method: 使用高斯随机游走建模连续帧中间层嵌入的变化，惩罚表示突变，促进低加速度的平滑解决方案。

Result: MoViNets模型家族在各自FLOP约束下提升SOTA 3.8%-6.1%，MobileNetV3和MoViNets-Stream在可比内存占用下提升4.9%-6.4%。

Conclusion: 平滑正则化技术能有效增强轻量级视频模型的时序建模能力，显著提升识别准确率。
...
TL;DR: 提出了一个开放词汇组合解释框架，利用开放词汇语义分割生成掩码来计算神经元激活与任意概念之间的组合解释，突破了对人工标注数据的依赖。


<details>
  <summary>Details</summary>
Motivation: 现有组合解释方法依赖人工标注数据集，限制了其应用范围和概念灵活性。本文旨在开发一个能够探测神经元对任意概念和数据集响应的框架。

Method: 框架包含三个步骤：指定任意概念、使用开放词汇模型生成语义分割掩码、从这些掩码中推导组合解释。

Result: 与先前方法相比，该框架在定量指标和人类可解释性方面表现良好，展示了从人工标注数据转向模型标注数据时解释的差异，并提供了更大的解释灵活性。

Conclusion: 提出的开放词汇组合解释框架显著扩展了神经元解释的适用范围，使其能够处理任意概念和数据集，为理解神经元信息编码提供了更强大的工具。
...
<details>
  <summary>Details</summary>
Motivation: 解决木材横截面数据稀缺问题，传统手动测量年轮生长耗时且不精确，需要自动化算法和数据集来提高准确性和自动化程度。

Method: 引入UruDendro4数据集，包含沿树干多个高度提取的样本，支持年轮体积建模。使用最先进方法（如DeepCS-TRD）提供自动年轮检测性能基准，并进行消融实验验证参数配置。

Result: DeepCS-TRD方法表现最佳，平均精度0.838，平均召回率0.782，适应兰德误差0.084。包含该数据集的训练提高了模型在年轮检测任务中的泛化能力。

Conclusion: UruDendro4数据集填补了木材横截面数据空白，支持年轮体积建模，为自动年轮检测提供了有价值的基准，有助于改进林木培育实践。
...
Method: 构建基于结构化描述符的报告，使用多头Swin编码器学习描述符感知的视觉表示，通过双级目标对齐视觉和文本标记，结合标记级交叉熵和余弦相似度对齐损失。
...
TL;DR: 提出了表达性图像合成任务，通过StickerNet框架从真实用户编辑行为中学习，专注于艺术性和社交参与而非视觉真实感


<details>
  <summary>Details</summary>
Motivation: 传统图像合成研究过度关注视觉真实性和语义合理性，但实际在线平台用户更倾向于创作艺术性、趣味性和社交参与性的内容，需要新的任务定义来反映真实编辑行为

Method: 提出两阶段框架StickerNet：先确定合成类型，再预测透明度、遮罩、位置和缩放等放置参数；基于180万条真实在线编辑平台数据构建数据集

Result: 用户研究和定量评估表明StickerNet优于常见基线方法，与人类放置行为高度匹配，证明了从真实编辑模式学习的有效性

Conclusion: 这项工作为视觉理解引入了新方向，强调表达性和用户意图而非真实感，为创造性图像编辑提供了新范式
...
TL;DR: TrafficLens是一个针对多摄像头交通路口的优化算法，通过顺序处理摄像头重叠区域、迭代使用VLM和对象级相似性检测，将视频转文本时间减少4倍。


<details>
  <summary>Details</summary>
Motivation: 多摄像头交通视频分析面临海量数据处理挑战，传统VLM转换视频到文本耗时较长，无法及时生成交通洞察和事故调查。

Method: 采用顺序处理策略，利用摄像头重叠覆盖区域，迭代应用不同token限制的VLM，并使用对象级相似性检测器智能跳过冗余VLM调用。

Result: 在真实数据集上的实验表明，TrafficLens将视频到文本转换时间减少高达4倍，同时保持信息准确性。

Conclusion: TrafficLens通过优化多摄像头视频分析流程，显著提高了交通视频处理的效率，为智能交通系统提供了实用的解决方案。
...
TL;DR: 提出了一种结合Vision Transformers和同态加密的隐私保护联邦学习框架，用于安全的跨机构病理学分类，显著降低了通信开销并防止模型反演攻击。
...
Method: 使用Vision Transformers的CLS令牌作为紧凑特征表示，采用CKKS同态加密对CLS令牌进行加密后再传输到服务器进行安全聚合。

Result: CLS令牌加密比梯度加密减少30倍通信开销，防止模型反演攻击，在未加密域达到96.12%分类准确率，加密域达到90.02%。
...
TL;DR: 提出了一种基于双整流流的无反转风格迁移框架，仅需前向传播即可实现内容与风格图像的融合，避免了传统方法依赖计算密集型反转过程的问题。


<details>
  <summary>Details</summary>
Motivation: 主流基于扩散模型的免训练风格迁移方法依赖计算密集的反转过程，影响效率且当反转不准确时会导致视觉失真。

Method: 使用双整流流并行预测内容和风格轨迹，通过动态中点插值融合两者，结合注意力注入指导风格整合。

Result: 实验表明该方法能泛化到多样风格和内容，提供高效有效的风格迁移流程。
...
TL;DR: 提出了一种无需测试时参考图像的伪装目标检测框架，通过将参考图像蒸馏到类别原型记忆中，在推理时通过查询条件化的原型混合生成参考向量
...
Method: 在训练期间将参考图像蒸馏到类别原型记忆中，维护每个类别的EMA更新原型，从查询预测混合权重生成引导向量；提出双向注意力对齐模块来弥合参考统计和伪装查询特征之间的表示差距

Result: 在R2C7K基准测试上进行了广泛实验，证明了与现有最先进方法相比具有竞争力或更优越的性能
...
TL;DR: 提出WavePCNet网络，通过三相位波前复杂传播重投影和动量记忆机制，解决非视域成像中因多重散射和噪声导致的物体定位分割不准确问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于实值建模或局部卷积的方法无法准确捕捉相干光传播物理特性，在低信噪比条件下易收敛到非物理解，影响观测稳定性。

Method: WavePCNet集成TriWCP模块引入复振幅传输算子约束相干传播行为，结合动量记忆机制抑制扰动积累，并采用高频跨层补偿增强构建频率选择性通路。

Result: 在四个物理采集数据集上的实验表明，WavePCNet在准确性和鲁棒性上均优于现有最优方法。

Conclusion: WavePCNet通过物理驱动的波前传播补偿，有效提升了非视域成像中对遮挡物体的感知能力，具有更好的鲁棒性和可解释性。
...
TL;DR: GuardTrace-VL是一个视觉感知的安全审计器，通过联合图像文本分析监控完整的问答推理流程，检测推理过程中出现的不安全内容，在检测不安全推理任务上达到93.1%的F1分数。
...
Method: 提出了GuardTrace-VL安全审计器，通过联合图像文本分析监控完整的问答推理流程；构建了GuardTrace数据集，通过多样化提示策略生成，并经过MLRM和人工投票验证流程精炼；提出了三阶段渐进训练方案结合数据精炼过程。

Result: 在提出的测试集上，GuardTrace-VL模型在不安全推理检测任务上达到93.1%的F1分数，比之前最强的多模态安全防御方法提高了13.5%的F1分数。

Conclusion: GuardTrace-VL能够有效检测推理过程中出现的不安全内容，填补了现有安全防护忽略中间推理过程的空白，显著提升了多模态推理模型的安全性。
...
TL;DR: 提出了一种基于扩散模型的图像分层分解方法，通过轻量级微调将修复模型适配到分层分解任务，并引入多模态上下文融合模块来保持细节，在合成数据集上训练，在目标移除和遮挡恢复方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 图像的分层表示（前景对象叠加在背景上）能够实现元素的独立编辑，为内容创作提供更大灵活性。尽管生成模型取得进展，但单图像分层分解仍因方法和数据有限而具有挑战性。

Method: 观察到分层分解与修复任务之间的强关联，提出通过轻量级微调将基于扩散的修复模型适配到分层分解。引入具有线性注意力复杂度的多模态上下文融合模块来保持潜在空间细节。模型完全在开源资源构建的合成数据集上训练。

Result: 在目标移除和遮挡恢复方面实现了优越性能，为下游编辑和创意应用开辟了新可能性。

Conclusion: 该方法成功地将修复模型适配到分层分解任务，通过多模态上下文融合模块保持细节，在合成数据集上训练取得了优异的分层分解效果。
...
TL;DR: MERGE是一个多模态实体感知检索增强生成框架，通过构建实体中心的多模态知识库来解决新闻图像描述中的信息覆盖不全、跨模态对齐弱和视觉实体定位差的问题。


<details>
  <summary>Details</summary>
Motivation: 解决现有新闻图像描述方法面临的三个关键挑战：信息覆盖不完整、跨模态对齐弱、视觉实体定位不理想。
...
<details>
  <summary>Details</summary>
Motivation: 现有的MTE指标选择方法通常是临时性的或仅基于指标的历史平均性能，但研究发现不同MTE指标在不同目标任务上的效果差异很大，没有单一指标在所有数据集上都表现最优。

Method: 将指标选择建模为学习排序问题，使用预训练语言模型编码数据集和MTE指标的文本描述，将它们嵌入共享语义空间，然后训练元预测器学习数据集特征与指标机制之间的复杂关系。

Result: 在11个预训练模型和11个目标数据集上的广泛实验表明，该方法具有强有效性。
...
TL;DR: 提出了一种新的可信多视图分类框架，通过原型表示各视图的邻居结构，简化了视图内邻居关系学习，并实现了视图内和视图间结构的动态对齐，从而更高效一致地发现跨视图共识。


<details>
  <summary>Details</summary>
Motivation: 现有可信多视图分类方法主要依赖全局密集邻居关系建模视图内依赖，导致计算成本高且无法直接保证视图间关系一致性。同时，这些方法通常通过手动分配权重聚合不同视图的证据，缺乏保证学习到的多视图邻居结构在类别空间内一致性的机制，从而影响分类结果的可信度。

Method: 引入原型来表示每个视图的邻居结构，通过简化视图内邻居关系学习，并实现视图内和视图间结构的动态对齐，促进跨视图共识的更高效和一致发现。

Result: 在多个公开多视图数据集上的广泛实验表明，该方法在性能下游和鲁棒性方面与主流可信多视图分类方法相比具有竞争力。

Conclusion: 所提出的基于原型的可信多视图分类框架能够有效解决现有方法的局限性，实现更高效和一致的多视图分类，提升分类结果的可信度。
...
TL;DR: CameraMaster是一个统一的相机感知图像修饰框架，通过显式解耦相机指令并整合指令表示和参数嵌入，实现精确的相机参数控制和物理一致的图像编辑。


<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖模糊的文本提示难以精确控制相机参数，要么需要为不同参数训练单独的模型头/权重，这限制了可扩展性、多参数组合和对细微变化的敏感性。

Method: CameraMaster使用相机参数嵌入来调制相机指令和内容语义，通过交叉注意力将调制后的指令注入内容特征，同时在去噪过程中将指令和相机嵌入作为条件和门控信号注入时间嵌入。

Result: 实验表明CameraMaster对参数变化产生单调且近似线性的响应，支持无缝的多参数组合，并显著优于现有方法。

Conclusion: CameraMaster通过统一的相机感知框架解决了图像修饰中精确参数控制的挑战，实现了物理一致的图像编辑。
...
Method: 构建可扩展的领域相关基准，涵盖自然、文档、电商和具身AI4个领域，创建33,027个需要视觉信息回答的多选题，通过LLM仅使用描述来回答问题，直接测量描述是否保留图像级效用。

Result: 评估显示图像与其描述效用之间存在显著差距，在传统图像QA基准上表现相近的模型，在描述效用上下降高达32%。

Conclusion: CaptionQA基准有效揭示了当前MLLM在生成具有实际下游任务效用的描述方面的不足，为改进描述生成模型提供了重要指导。
...
Method: 结合MeanFlow与物理一致性约束实现高质量动作生成；采用BiMamba骨干网络和通道级跨模态融合的简单高效架构；支持非自回归生成和交互式动作编辑。

Result: 在AIST++和FineDance数据集上的实验表明，FlowerDance在动作质量和生成效率方面均达到最先进水平。

Conclusion: FlowerDance不仅生成具有物理合理性和艺术表现力的精细动作，还在推理速度和内存利用方面实现了显著的生成效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于多模态大语言模型的肺部CT分析方法在准确描述结节形态和融入医学专业知识方面存在挑战，影响临床可靠性。协作多智能体系统在医学应用中平衡通用性和精确性具有潜力，但在病理学中尚未充分探索。

Method: 系统包含三个主要模块：Nodule Spotter协调临床检测模型识别结节；Radiologist整合局部图像描述技术生成全面CT报告；Doctor Agent System使用图像和CT报告进行恶性推理，支持病理知识库和多智能体框架。
...
TL;DR: 提出了一种新颖的图像去模糊框架，通过将强大的生成先验与显式的物理约束相结合，解决了现有方法在物理准确性和感知质量之间的权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有的基于学习的图像去模糊方法存在两种范式：基于模型的深度展开方法会产生过度平滑、带有伪影的纹理；生成模型虽然感知质量更好但会因物理约束弱而产生幻觉细节。需要一种能够调和这两种范式的方法。
...
Conclusion: 该框架通过将生成先验与密集物理约束相结合，有效解决了空间变化图像去模糊这一病态问题，在保持物理准确性的同时实现了优越的感知质量。
...
TL;DR: MUSE是首个统一的图像情感合成框架，能够同时进行情感生成和编辑，通过测试时缩放策略避免额外训练需求，在情感准确性和语义多样性方面表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前图像情感合成方法将生成和编辑任务人为分离，导致效率低下，限制了在治疗干预、故事讲述等自然交织应用场景中的使用。

Method: 采用测试时缩放策略，利用现成情感分类器进行梯度优化，通过语义相似度确定最佳引导时机，使用多情感损失减少固有和相似情感的干扰。

Result: 实验结果显示MUSE在生成和编辑任务中均优于所有方法，提高了情感准确性和语义多样性，在内容保持、文本提示遵循和情感表达之间保持最佳平衡。

Conclusion: MUSE为情感合成建立了新范式，实现了情感生成和编辑的统一框架，无需额外训练扩散模型或专用数据集。
...
TL;DR: 提出T-NIG模型，通过将时间参数整合到正态逆伽马分布中，处理阿尔茨海默病预测中不规则时间间隔的脑图像序列，生成长期预测图像并保持疾病相关特征。


<details>
  <summary>Details</summary>
Motivation: 解决长期阿尔茨海默病预测中，面对不规则时间间隔的序列数据时，图像生成难以维持疾病相关特征的问题。
...
Conclusion: T-NIG模型能够有效处理不规则时间分布的脑成像数据，在长期阿尔茨海默病预测中保持疾病特征，展示了优越的性能。
...
Conclusion: MIRA通过模拟多轮人机交互过程，有效解决了复杂指令理解问题，为指令引导的图像编辑提供了更准确和直观的解决方案。
...
TL;DR: 提出了一种新颖的虹膜认证匹配流程，通过3D-CNN学习丰富的时空特征表示，采用课程学习方式训练，能够有效处理旋转、尺度、反射和模糊等挑战。


<details>
  <summary>Details</summary>
Motivation: 现有虹膜认证算法在旋转、尺度变化、镜面反射和离焦模糊等情况下鲁棒性不足，且大多采用简单的点对点比较方法，未能有效利用虹膜模式的时空结构信息。
...
TL;DR: RadarFM：通过结构化空间语言监督学习统一场景级表示的雷达基础模型，解决现有雷达方法碎片化、任务特定化的问题
...
Result: 开发了RadarFM雷达基础模型，能够学习统一场景级表示，支持细粒度空间推理
...
TL;DR: 提出EM-KD方法，通过知识蒸馏增强高效多模态大语言模型，解决了视觉令牌不平衡导致的理解能力下降问题


<details>
  <summary>Details</summary>
Motivation: 现有高效MLLMs压缩视觉令牌会损失视觉信息，降低理解能力；现有知识蒸馏方法忽视了学生模型与教师模型之间视觉令牌不平衡导致的细粒度视觉理解差异

Method: 1) 使用曼哈顿距离和匈牙利匹配算法对齐师生视觉令牌；2) 提出视觉-语言亲和力蒸馏(VLAD)和视觉语义蒸馏(VSD)两种蒸馏策略

Result: 在多个基准测试中，EM-KD训练模型在准确性和效率上都显著优于现有高效MLLMs，相比其他蒸馏方法也表现更好

Conclusion: EM-KD通过有效的视觉令牌对齐和双重蒸馏策略，成功提升了高效MLLMs的视觉理解能力，在保持效率的同时显著提高性能
...
TL;DR: FaithFusion是一个基于像素级期望信息增益的3DGS-扩散模型融合框架，用于可控驾驶场景重建和3D场景生成，解决了几何保真度和视觉逼真度之间的平衡问题。
...
Method: 提出FaithFusion框架，使用像素级期望信息增益作为统一策略进行时空一致性合成：EIG作为空间先验指导扩散模型细化高不确定性区域，同时通过像素级加权将编辑结果蒸馏回3DGS。这是一个即插即用的系统，无需额外先验条件和结构修改。

Result: 在Waymo数据集上的广泛实验表明，该方法在NTA-IoU、NTL-IoU和FID指标上达到最先进性能，即使在6米车道偏移时仍能保持107.47的FID分数。

Conclusion: FaithFusion通过像素级期望信息增益有效解决了3DGS和扩散模型融合中的几何保真度和视觉逼真度平衡问题，为可控驾驶场景重建提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于CLIP的提示学习方法使用静态锚点（固定值和位置），缺乏跨任务和阶段自适应的灵活性，限制了模型泛化能力。

Method: 提出AnchorOPT框架：1）动态学习任务特定的锚点值而非手工设计；2）通过可学习位置矩阵自适应优化锚点与软标记的位置关系；3）两阶段训练：先学习锚点标记，然后冻结并优化软标记和位置矩阵。

Result: 实验表明，仅使用简单的可学习锚点和位置矩阵就能达到或超过一些包含额外可学习模块或正则化技术的方法性能。
...
TL;DR: 提出了一种名为DATGN的新方法，用于自动学习脑部图像中与疾病进展相关的形态变化，以实现阿尔茨海默病的早期预测。该方法能够处理MRI时间序列中的缺失数据，并生成符合疾病进展的未来MRI图像。


<details>
  <summary>Details</summary>
Motivation: 阿尔茨海默病是一种退行性脑部疾病，早期预测有助于延缓其进展。当前预测方法主要依赖手动特征提取来分析脑部图像的形态变化，存在自动化程度低的问题。

Method: DATGN方法首先对不完整的时间序列进行插值，然后通过双向时间变形感知模块指导网络生成符合疾病进展的未来MRI图像，从而实现阿尔茨海默病的早期预测。

Result: 在ADNI数据集上的实验结果显示，DATGN在PSNR和MMSE图像质量指标上表现优异。当将DATGN生成的合成数据集成到SVM、CNN和3DCNN分类方法中时，AD vs NC分类准确率提高了6.21%到16%，AD vs MCI vs NC分类准确率提高了7.34%到21.25%。
...
<details>
  <summary>Details</summary>
Motivation: 先前研究表明语言模型中的文本干扰项会导致推理变长但效果变差，本文旨在探究多模态设置中是否出现类似现象。

Method: 引入Idis数据集，系统地在语义、数值和空间维度上变化干扰项，并通过分析推理轨迹中的属性计数来研究干扰项、推理长度和准确性的相互作用。

Result: 视觉干扰项会降低准确性但不增加推理长度，与文本干扰项不同。这些趋势也适用于已建立的视觉偏置基准测试。

Conclusion: 提出了简单的提示策略来减轻推理模型中的偏置驱动预测，揭示了视觉干扰项对视觉语言模型性能的独特影响模式。
...
TL;DR: EntPruner是一个基于熵指导的自动渐进式剪枝框架，专门针对扩散模型和流模型，通过条件熵偏差(CED)指标来评估模块重要性，实现参数冗余减少和推理加速。


<details>
  <summary>Details</summary>
Motivation: 大规模视觉生成模型在迁移到下游任务时存在显著的参数冗余问题，需要一种专门针对生成模型的剪枝方法，既要保持输出分布的多样性，又要保证条件保真度。

Method: 提出熵指导剪枝策略，使用数据依赖的条件熵偏差(CED)来量化移除模块后分布与学习条件数据分布的差异；采用零样本自适应剪枝框架，在训练过程中动态决定何时以及剪枝多少。
...
TL;DR: 提出Fourier Kernel Estimator (FKE)，在傅里叶空间中学习模糊核，将空间域的卷积问题转换为傅里叶空间的乘法问题，实现核级别的模糊过程学习，显著提升图像去模糊性能。


<details>
  <summary>Details</summary>
Motivation: 现有深度网络仍处于像素级学习阶段，无法让去模糊模型理解模糊的本质。模糊本质上是清晰图像与模糊核的卷积过程，因此让网络在核级别学习模糊过程能显著提升性能。

Method: 提出Fourier Kernel Estimator (FKE)，在傅里叶空间进行激活操作；将卷积对象从图像改为网络提取的特征；设计解耦多尺度架构，使用多个分层子Unet和可逆策略实现低内存的多尺度编码解码。

Result: 在运动去模糊任务上达到最先进的结果，并显示出处理其他核相关问题的潜力。分析表明核估计器能够学习具有物理意义的核。

Conclusion: FKE方法通过核级别的模糊过程学习，实现了更有效的图像去模糊，为处理核相关问题提供了新的思路。
...
TL;DR: Ent-Prog：一种用于人类视频生成的扩散模型高效训练框架，通过条件熵膨胀评估模型组件重要性，结合自适应渐进式调度，实现2.2倍训练加速和2.4倍GPU内存减少。


<details>
  <summary>Details</summary>
Motivation: 扩散模型在人类视频生成中计算成本高、内存消耗大，需要更高效的训练方法。

Method: 提出条件熵膨胀(CEI)评估模型组件重要性，优先训练关键组件；引入自适应渐进式调度，根据收敛效率动态增加计算复杂度。

Result: 在三个数据集上的实验表明，Ent-Prog实现了2.2倍训练加速和2.4倍GPU内存减少，同时保持生成性能。

Conclusion: Ent-Prog是一种有效的人类视频生成扩散模型训练框架，显著提升了训练效率。
...
TL;DR: ProxyFormer是一个新的RVOS架构，通过引入代理查询来整合视觉和文本语义，解决现有方法中跨模态对齐不足和文本约束延迟的问题，在四个基准测试中表现出色。
...
Method: 提出ProxyFormer架构，引入代理查询集来整合视觉和文本语义，通过多阶段视频特征编码器逐步更新和传播代理查询，确保视频特征聚焦于目标对象。同时解耦跨模态交互到时间和空间维度以降低计算成本，并设计联合语义一致性训练策略。

Result: 在四个广泛使用的RVOS基准测试上的综合实验表明，ProxyFormer优于最先进的方法。

Conclusion: ProxyFormer通过代理查询机制有效解决了RVOS中的跨模态对齐问题，建立了帧间依赖关系，提高了目标跟踪的准确性和连贯性。
...
TL;DR: TEAR是一个针对文本到视频模型安全性的自动红队测试框架，通过利用时间动态性来发现模型的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有的安全评估方法主要关注静态图像和文本生成，无法捕捉视频生成中的复杂时间动态特性，因此需要专门针对T2V模型的安全评估框架。

Method: 采用时间感知测试生成器，通过两阶段优化（初始生成器训练和时间感知在线偏好学习）来创建看似无害但能利用时间动态性引发违规视频输出的提示词，并使用精炼模型循环改进提示词的隐蔽性和对抗效果。

Result: 在开源和商业T2V系统上的广泛实验表明，TEAR实现了超过80%的攻击成功率，相比之前最佳结果57%有显著提升。

Conclusion: TEAR框架有效揭示了T2V模型中与时间动态性相关的安全风险，证明了现有安全评估方法的不足，并为T2V模型的安全评估提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型倾向于使用全局原生分辨率视觉编码，虽然提升了整体能力但带来了巨大的计算开销，需要找到平衡性能与效率的解决方案。
...
Conclusion: PVC方法能够将预训练的ViT重新配置为高效架构，在保持泛化能力的同时显著提升多模态大语言模型的推理效率。
...
Method: GridAR采用网格分区渐进生成方案，在同一画布位置生成多个部分候选，早期修剪不可行候选，固定可行候选作为锚点指导后续解码；同时使用布局指定提示重构策略，通过检查部分视图推断可行布局。

Result: 在N=4时，GridAR在T2I-CompBench++上比Best-of-N（N=8）性能提升14.4%，同时成本降低25.6%；在图像编辑任务上，PIE-Bench显示编辑质量相当且语义保持提升13.9%。
...
TL;DR: NDTokenizer3D是一个通用的3D视觉语言模型，通过多尺度NDT表示和渐进式特征融合，实现3D场景理解和推理，在多项3D任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 当前3D视觉语言模型在场景理解和推理方面潜力巨大，但如何有效将3D场景标记化为整体场景标记，并在多样化3D理解任务中利用这些标记仍具挑战性。

Method: 提出三阶段场景标记化流程：1)从原始点云构建多尺度NDT表示；2)通过多尺度NDT解码器渐进融合跨尺度特征生成场景标记；3)将解码器重新用作人机交互提示和分割掩码解码的通用接口。

Result: NDTokenizer3D在3D指代分割、3D视觉问答和3D密集描述等任务中取得了显著改进，实现了细粒度的通用3D视觉语言建模。

Conclusion: NDTokenizer3D通过紧凑统一的设计，实现了语言级推理与3D空间理解的桥梁，为3D场景理解提供了通用解决方案。
...
TL;DR: 提出了UPA-RFAS框架，通过鲁棒特征、注意力和语义学习通用对抗补丁，实现跨VLA模型、任务和视角的可迁移攻击。


<details>
  <summary>Details</summary>
Motivation: 当前VLA模型容易受到对抗攻击，但大多数补丁攻击方法在单一模型上过拟合，无法在黑盒设置中实现跨模型迁移，需要研究通用且可迁移的对抗补丁。

Method: 结合特征空间目标（ℓ1偏差先验和排斥性InfoNCE损失）、鲁棒增强的两阶段min-max过程（内环学习不可见样本扰动，外环优化通用补丁），以及VLA特定损失（补丁注意力主导和补丁语义错配）。

Result: 实验表明UPA-RFAS在各种VLA模型、操作套件和物理执行中都能实现跨模型、任务和视角的一致迁移，暴露了基于补丁的实用攻击面。

Conclusion: UPA-RFAS为VLA模型建立了强大的通用对抗补丁攻击基准，为未来防御研究提供了重要参考。
...
<details>
  <summary>Details</summary>
Motivation: 现有深度聚类方法存在全局和局部特征结构不一致的问题：局部结构在类内样本间具有强一致性和紧凑性，而全局特征则存在边界交织和聚类分离度差的问题。

Method: 首先通过自适应k近邻一致性过滤识别高置信度样本作为可靠锚点，然后利用这些样本计算判别性损失，促进类内紧凑性和类间分离性来指导网络优化。

Result: 在多个基准数据集上的实验表明，DCBoost显著提升了各种现有深度聚类模型的性能，将当前最先进基线方法（如ProPos）的性能提高了3%以上，并将轮廓系数放大了7倍以上。
...
Method: 使用对比学习框架将高分辨率航空影像与植物样方数据对齐，采用正则化策略缓解灾难性遗忘问题，生成可迁移的表示用于下游预测器。
...
TL;DR: 提出ART框架，通过查询-响应机制发现和跟踪局部细节动态，有效区分相似动作。使用文本约束语义作为查询捕获动作相关区域，构建动作轨迹，并通过多级对比约束优化。


<details>
  <summary>Details</summary>
Motivation: 现有细粒度动作识别方法难以捕捉局部区域随时间演变的细微细节，导致相似动作区分困难。

Method: ART框架：1）区域特定语义激活模块使用文本约束语义作为查询；2）构建动作轨迹链接跨帧响应；3）多级轨迹对比约束优化；4）任务特定微调机制优化语义表示。

Result: 在广泛使用的动作识别基准测试中表现优于现有最先进基线方法。

Conclusion: ART框架通过跟踪局部细节动态有效解决了细粒度动作识别中的细微差异捕捉问题，在多个基准测试中取得优越性能。
...
TL;DR: ConFu是一个多模态对比学习框架，通过融合模态对比目标，在统一表示空间中同时嵌入单个模态及其融合组合，能够捕捉高阶依赖关系并保持强成对对应性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要处理两模态对齐，但忽略了多模态间的高阶交互，且往往无法充分保持成对关系，限制了在单模态任务上的有效性。

Method: ConFu扩展了传统的成对对比目标，增加了融合模态对比项，鼓励模态对与第三模态的联合嵌入，从而捕捉仅靠成对对齐无法恢复的高阶依赖关系。

Result: 在合成和真实多模态基准测试中，ConFu在检索和分类任务上表现出竞争力，支持统一的一对一和一对多检索。

Conclusion: ConFu能够有效捕捉多模态间的高阶依赖关系，同时保持强成对对应性，为多模态表示学习提供了统一的对比学习框架。
...
TL;DR: 比较三种生成模型：DDPM、CFM和MeanFlow。CFM在50步采样时FID为24.15，显著优于DDPM的402.98。MeanFlow单步采样FID为29.15，推理时间减少50倍。CFM扩展至图像修复，在多种掩码类型下PSNR提升73%，SSIM提升45%。


<details>
  <summary>Details</summary>
Motivation: 系统比较不同生成建模范式的性能，特别是探索直接一步生成的可能性，并扩展条件流匹配到图像修复任务。

Method: 使用统一的TinyUNet架构（<150万参数）在CIFAR-10上实现三种方法：DDPM、CFM和MeanFlow。CFM采用迭代采样，MeanFlow通过建模时间间隔内的平均速度实现直接一步生成。扩展CFM到图像修复，实现四种掩码类型的掩码引导采样。

Result: CFM在50步采样时FID为24.15，DDPM为402.98。MeanFlow单步采样FID为29.15，推理时间减少50倍。图像修复模型在中心掩码上PSNR从4.95提升至8.57 dB（+73%），SSIM从0.289提升至0.418（+45%）。

Conclusion: CFM在生成质量上显著优于DDPM，MeanFlow在保持合理质量的同时实现了高效的单步生成。CFM扩展至图像修复任务效果显著，修复感知训练能大幅提升性能。
...
TL;DR: SIFT-SNN框架：一种低延迟神经形态信号处理管道，用于实时检测交通基础设施中的结构异常，结合SIFT特征提取和SNN分类，在奥克兰海港大桥数据集上达到92.3%准确率和9.5毫秒推理时间。
...
Result: 在包含6000个标记帧的数据集上达到92.3%分类准确率，每帧推理时间9.5毫秒，脉冲活动稀疏度为8.1%，支持实时低功耗边缘部署。

Conclusion: SIFT-SNN框架在保持空间特征基础的同时实现了高效实时处理，为可移动混凝土护栏等交通流控制基础设施的结构安全监测提供了可推广的案例研究。
...
### [72] [3-Tracer: A Tri-level Temporal-Aware Framework for Audio Forgery Detection and Localization](https://arxiv.org/abs/2511.21237)
*Shuhan Xia,Xuannan Liu,Xing Cui,Peipei Li*

Main category: cs.CV

TL;DR: 提出了T3-Tracer框架，通过联合分析帧级、段级和音频级三个层次来检测部分音频伪造，包含帧-音频特征聚合模块和段级多尺度差异感知模块，在三个数据集上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 部分音频伪造通过选择性修改关键帧而保持整体感知真实性，现有方法仅独立检测单帧伪造，缺乏捕捉不同时间层次上瞬时和持续异常的层次结构。

Method: T3-Tracer框架包含两个核心模块：FA-FAM模块结合帧级和音频级时序信息检测帧内伪造线索和全局语义不一致；SMDAM模块采用双分支架构建模多尺度时间窗口内的帧特征和帧间差异，识别伪造边界上的突变异常。

Result: 在三个具有挑战性的数据集上进行广泛实验，证明该方法达到了最先进的性能。

Conclusion: 通过联合分析音频的帧、段和音频三个层次，T3-Tracer能够全面检测伪造痕迹，有效解决了部分音频伪造检测的挑战。
...
Result: 广泛的基线实验表明，在SurgMLLMBench上训练的单一模型在不同领域实现了持续性能，并能有效泛化到未见过的数据集。
...
### [74] [FIELDS: Face reconstruction with accurate Inference of Expression using Learning with Direct Supervision](https://arxiv.org/abs/2511.21245)
*Chen Ling,Henglin Shi,Hedvig Kjellström*

Main category: cs.CV

TL;DR: FIELDS是一个3D人脸重建方法，通过直接3D表情参数监督和情感识别分支，从单张图像生成保留微妙情感细节的高保真3D人脸模型。


<details>
  <summary>Details</summary>
Motivation: 现有3D人脸重建方法依赖2D监督且缺乏3D真实数据，导致丢失微妙的情感细节。

Method: 扩展自监督2D图像一致性线索，加入直接3D表情参数监督和辅助情感识别分支，使用来自自发4D面部扫描的真实表情参数指导编码器，并通过强度感知情感损失鼓励3D表情参数捕捉真实情感内容。

Result: 产生具有高度真实表情的情感丰富人脸模型，显著提升野外面部表情识别性能，同时保持自然度。

Conclusion: 双重监督策略弥合了2D/3D领域差距并减轻了表情强度偏差，能够生成保留微妙情感线索的高保真3D重建结果。
...
### [76] [Shift-Equivariant Complex-Valued Convolutional Neural Networks](https://arxiv.org/abs/2511.21250)
*Quentin Gabot,Teck-Yian Lim,Jérémy Fix,Joana Frontera-Pons,Chengfang Ren,Jean-Philippe Ovarlez*

Main category: cs.CV

TL;DR: 本文扩展了可学习多相采样(LPS)方法到复数神经网络，通过理论分析和引入新的投影层，在复数域实现了平移等变性和不变性，并在极化合成孔径雷达图像的分类、重建和语义分割任务中进行了验证。


<details>
  <summary>Details</summary>
Motivation: 传统卷积神经网络缺乏平移等变性和不变性，虽然数据增强可以经验性地学习这些性质，但需要系统性的方法来理论保证这些性质。

Method: 将可学习多相采样(LPS)扩展到复数神经网络，引入了从复数到实数的投影层，并在Gumbel Softmax之前应用。

Result: 在极化合成孔径雷达图像的多个计算机视觉任务中验证了方法的有效性，包括分类任务的不变性、重建和语义分割任务的等变性。

Conclusion: 成功将LPS方法扩展到复数神经网络，为复数域中的平移等变性和不变性提供了理论保证和实用实现。
...
### [77] [AVFakeBench: A Comprehensive Audio-Video Forgery Detection Benchmark for AV-LMMs](https://arxiv.org/abs/2511.21251)
...
<details>
  <summary>Details</summary>
Motivation: 现有的音视频伪造检测基准仅限于DeepFake伪造和单粒度注释，无法捕捉现实世界伪造场景的多样性和复杂性。
...
Conclusion: AVFakeBench填补了现有基准的空白，为音视频伪造检测研究提供了更全面和现实的评估标准，揭示了当前方法的局限性。
...
### [78] [LaGen: Towards Autoregressive LiDAR Scene Generation](https://arxiv.org/abs/2511.21256)
...
Result: 在基于nuScenes构建的评估协议上，LaGen全面优于最先进的LiDAR生成和预测模型，特别是在后续帧上的表现更佳。
...
<details>
  <summary>Details</summary>
Motivation: 现有遥感变化描述方法通常使用CNN/Transformer提取视觉特征或引入辅助任务，但存在区域感知弱和时间对齐有限的问题。本文旨在通过SAM基础模型提取区域级表示，增强对变化区域的关注。
...
Conclusion: 提出的基于SAM的方法有效提升了遥感变化描述的性能，解决了区域感知和时间对齐的挑战。
...
### [80] [Unlocking Zero-shot Potential of Semi-dense Image Matching via Gaussian Splatting](https://arxiv.org/abs/2511.21265)
...
<details>
  <summary>Details</summary>
Motivation: 基于学习的图像匹配需要大规模、多样化且几何准确的训练数据，但现有3D高斯泼溅技术存在几何不准确和深度渲染偏差问题，限制了其在对应关系标注中的应用。

Method: 1) 几何忠实的数据生成流程：修正3DGS几何以产生精确对应标签；2) 2D-3D表示对齐策略：将3DGS的显式3D知识注入2D匹配器，引导学习视角不变的3D表示。
...
<details>
  <summary>Details</summary>
Motivation: 利用大量未标记的工业演示数据来预训练视觉-语言-动作模型，解决制造业中具身AI集成需要结构化训练数据的挑战。

Method: 首先训练轻量级运动标记器编码运动动态，然后使用基于新型"潜在动作能量"指标的无监督动作分割器来发现和分割语义连贯的动作基元。

Result: 在公共基准和专有电机装配数据集上的评估显示能有效分割工作台中人类执行的关键任务，通过视觉语言模型的聚类和定量评估确认了发现动作基元的语义连贯性。

Conclusion: 这是首个从非结构化工业视频中提取和组织VLA预训练数据的全自动端到端系统，为制造业中的具身AI集成提供了可扩展的解决方案。
...
### [82] [Co-Training Vision Language Models for Remote Sensing Multi-task Learning](https://arxiv.org/abs/2511.21272)
...
<details>
  <summary>Details</summary>
Motivation: 随着Transformer在单个遥感任务上表现出色，现在需要实现一个能在多任务学习中表现出色的统一模型。多任务学习方法相比单任务方法具有更好的泛化性、可扩展性和实际应用性。

Method: 1. 创建数据引擎处理复杂遥感数据环境；2. 提出统一动态分辨率策略处理不同尺度的遥感图像；3. 针对超高分辨率图像引入Zoom-in Chain机制和相应数据集；4. 增强模型的目标检测能力并设计新的评估协议。

Result: RSCoVLM在多样化任务上实现了最先进的性能，超越了现有的遥感视觉语言模型，甚至可与专门的专家模型相媲美。

Conclusion: RSCoVLM作为一个基线模型，将促进通用遥感模型的进一步发展，所有训练和评估工具、模型权重和数据集均已开源以支持可复现性。
...
TL;DR: 提出了一种基于超图的时空事件流补全机制，通过超图连接不同时间和空间位置的事件标记，利用上下文信息传递来补全稀疏事件，并能灵活融合RGB多模态信息。


<details>
  <summary>Details</summary>
Motivation: 传统事件表示学习方法使用事件帧、体素或张量作为输入，但难以解决由空间稀疏性引起的欠采样问题。

Method: 超图引导的时空事件流补全机制，通过超图连接事件标记并利用上下文信息传递完成稀疏事件补全，可融合RGB多模态信息，并通过自注意力聚合不同时间步的超图节点信息。
...
### [84] [PathMamba: A Hybrid Mamba-Transformer for Topologically Coherent Road Segmentation in Satellite Imagery](https://arxiv.org/abs/2511.21298)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于Vision Transformer的方法虽然能捕获全局上下文，但二次复杂度限制了在资源受限平台上的部署效率，而Mamba模型具有线性时间效率，特别适合建模长连续结构

Method: 提出PathMamba混合架构，策略性地使用Mamba块追踪道路网络的连续特性，同时集成Transformer块利用全局上下文精炼特征

Result: 在DeepGlobe Road Extraction和Massachusetts Roads数据集上达到新的SOTA，显著提升了APLS指标衡量的拓扑连续性，同时保持计算竞争力

Conclusion: PathMamba证明了混合架构在道路分割任务中的有效性，能够在不牺牲效率的情况下获得拓扑优越的分割结果
...
### [85] [CaliTex: Geometry-Calibrated Attention for View-Coherent 3D Texture Generation](https://arxiv.org/abs/2511.21309)
...
<details>
  <summary>Details</summary>
Motivation: 当前基于扩散模型的3D纹理生成系统存在跨视图不一致问题，即从一个视角看起来合理的纹理在其他视角下无法对齐。这源于注意力模糊问题，即无结构的全注意力被不加区分地应用于所有token和模态，导致几何混淆和不稳定的外观-结构耦合。

Method: 引入CaliTex框架，包含两个模块：1) 部分对齐注意力，强制跨语义匹配部分的空间对齐；2) 条件路由注意力，通过几何条件路径路由外观信息以保持空间保真度。结合两阶段扩散变换器，使几何一致性成为网络的固有行为。

Result: CaliTex能够生成无缝且视图一致的纹理，在实证评估中优于开源和商业基线方法。

Conclusion: 通过几何校准注意力机制，CaliTex有效解决了3D纹理生成中的跨视图不一致问题，使几何一致性成为模型的固有特性，而非优化的副产品。
...
TL;DR: 提出一种频率感知的token缩减策略，通过将token分为高频和低频两类，选择性保留高频token并将低频token聚合为紧凑的DC token，在提高计算效率的同时缓解rank collapsing问题。
...
Result: 通过大量实验和分析证明，该方法在减少计算开销的同时显著提高准确性，并缓解rank collapsing和过平滑问题。
...
### [87] [HTTM: Head-wise Temporal Token Merging for Faster VGGT](https://arxiv.org/abs/2511.21317)
*Weitian Wang,Lukas Meiner,Rai Shubham,Cecilia De La Parra,Akash Kumar*

Main category: cs.CV

TL;DR: 提出HTTM方法，一种无需训练的3D token合并技术，用于加速VGGT模型在3D场景重建中的推理过程，通过多头粒度的token合并实现7倍加速且性能损失可忽略。


<details>
  <summary>Details</summary>
Motivation: VGGT模型在重建大场景时，由于需要全局注意力层对所有视图的token进行全对全注意力计算，导致显著的延迟瓶颈。

Method: 提出头级时序合并(HTTM)方法，在多头粒度上合并token，保持特征token在头拼接后的独特性，并利用头级别的空间局部性和时序对应关系实现更高的合并比。

Result: 在GPU推理中实现了高达7倍的加速，且性能下降可忽略不计。

Conclusion: HTTM是一种有效的训练free加速方法，能够显著提升VGGT在大场景3D重建中的推理效率。
...
TL;DR: 提出Merge-and-Bound (M&B)方法用于类增量学习，通过在参数空间直接操作模型权重进行优化，包含任务间和任务内权重合并，以及有界更新技术来减少灾难性遗忘。
...
Method: 使用两种权重合并：任务间权重合并（平均所有先前阶段模型的权重）和任务内权重合并（组合当前阶段的模型参数）。结合有界更新技术，以最小累积更新优化目标模型。
...
### [89] [Multimodal Robust Prompt Distillation for 3D Point Cloud Models](https://arxiv.org/abs/2511.21574)
...
Method: 使用多模态教师（视觉模型、高性能3D模型、文本编码器）通过置信度门控机制指导学生模型学习轻量级提示，对齐特征表示
...
### [90] [PFF-Net: Patch Feature Fitting for Point Cloud Normal Estimation](https://arxiv.org/abs/2511.21365)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法难以准确高效地预测各种点云的法向量，主要困难在于为不同数据或几何结构选择合适的邻域尺寸

Method: 使用多尺度特征融合策略，包含特征聚合模块和特征补偿模块，通过渐进式特征聚合和跨尺度特征补偿来逼近最优几何描述

Result: 在合成和真实数据集上达到最先进性能，同时具有更少的网络参数和运行时间

Conclusion: 基于多尺度特征聚合的近似策略能够实现不同局部块的自适应尺度调整，提供最优特征描述
...
### [91] [Endo-G$^{2}$T: Geometry-Guided & Temporally Aware Time-Embedded 4DGS For Endoscopic Scenes](https://arxiv.org/abs/2511.21367)
*Yangle Liu,Fengze Li,Kan Liu,Jieming Ma*

Main category: cs.CV

TL;DR: Endo-G²T是一个用于动态内窥镜场景的几何引导和时间感知训练方案，通过几何先验蒸馏、时间嵌入高斯场和关键帧约束流式处理，在4D高斯泼溅中实现早期几何锚定和时间一致性。
...
Conclusion: 该方法成功解决了内窥镜场景中的几何漂移问题，实现了高质量的时间一致性重建。
...
TL;DR: 提出了STVG-o1框架，通过边界框思维链机制和多维度强化奖励函数，使现有多模态大语言模型无需架构修改即可在时空视频定位任务上达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 解决多模态大语言模型在时空视频定位任务中表现不佳的问题，原因是训练目标不匹配和标准视觉编码器中细粒度区域-词语对齐能力弱。

Method: 引入边界框思维链机制，在最终预测前显式推理时空位置；设计包含格式、一致性、时间、空间和思考奖励的多维度强化奖励函数，通过强化微调提供几何感知监督。

Result: 在HCSTVG-v1/v2和VidSTG数据集上取得最先进结果，在HCSTVG-v1上比最佳任务特定方法提升7.3% m_tIoU，在VidSTG上与专用模型相当，大幅超越现有所有基于MLLM的方法。

Conclusion: 确立了多模态大语言模型作为精确时空定位任务的可行且强大的骨干网络，并展现出强大的跨数据集开放词汇泛化能力。
...
TL;DR: DiverseVAR框架通过文本嵌入噪声注入和尺度旅行精炼技术，在不重新训练的情况下增强视觉自回归模型的多样性，同时保持图像质量。


<details>
  <summary>Details</summary>
Motivation: 现有的视觉自回归模型在文本条件图像生成中存在多样性不足的问题，即使对于简单提示也会产生几乎相同的图像，而当前研究主要关注图像质量而非多样性。

Method: 采用两阶段方法：1）在文本嵌入中注入噪声以增强多样性；2）提出尺度旅行精炼技术，使用多尺度自编码器提取粗尺度标记，在中间阶段恢复生成以保持图像质量。

Result: 实验表明，文本嵌入噪声注入与尺度旅行精炼相结合，在最小化图像质量损失的同时显著增强了多样性，在多样性-质量权衡中达到了新的帕累托边界。

Conclusion: DiverseVAR框架有效解决了视觉自回归模型的多样性问题，无需重新训练或大量计算开销，实现了多样性和图像质量的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 传统基于几何特征的3D重组方法在面对小碎片、侵蚀碎片或对称碎片时表现不佳，且缺乏防止重叠组装的物理约束。
...
TL;DR: MobileI2V是一个270M参数的轻量级扩散模型，用于在移动设备上实现实时图像到视频生成，通过线性混合架构、时间步蒸馏和移动优化技术，将720p视频生成速度提升到每帧小于100ms。
...
Method: 1) 分析线性注意力和softmax注意力模块性能，提出平衡效率与质量的线性混合架构去噪器；2) 设计时间步蒸馏策略，将采样步骤从20+压缩到仅2步；3) 应用移动设备特定的注意力优化技术。

Result: 首次在移动设备上实现快速720p图像到视频生成，质量与现有模型相当，单步条件下每帧生成时间小于100ms，生成速度提升10倍。
...
<details>
  <summary>Details</summary>
Motivation: 传统自注意力蒸馏方法独立对齐师生特征图，无法充分捕捉像素间关系。需要一种能让学生特征动态参考教师全局特征的方法来改善特征表示学习。

Method: 使用交叉注意力机制，让学生特征图的每个像素都能关注教师特征图的所有像素，仅添加额外损失函数实现非局部知识转移。
...
Conclusion: 通过隔离单个因素的影响，旨在为未来深度伪造检测系统的设计和开发建立稳健的、架构无关的最佳实践
...
TL;DR: 提出了一种用于抗核抗体(ANA)检测的新型多实例多标签学习框架，通过实例采样器、概率伪标签分发器和自定步调学习来处理临床显微镜图像中的复杂模式识别问题。


<details>
  <summary>Details</summary>
Motivation: 传统ANA检测方法耗时费力且需要专业训练，而现有自动化方法难以处理真实临床环境中多实例多标签的复杂情况。

Method: 基于人类标注逻辑，使用实例采样器抑制低置信度实例，概率伪标签分发器自适应分配标签，自定步调学习系数根据经验观察调整训练。

Result: 在ANA数据集上比最佳现有方法提升7.0% F1-Macro和12.6% mAP，在公开数据集上关键指标排名前2，汉明损失和单错误率分别降低18.2%和26.9%。
...
TL;DR: 提出了一种用于构建遥感基础模型的高效集成专家框架，通过分解训练过程为轻量级任务特定专家，解决了当前大模型方法在计算资源和可持续性方面的挑战。


<details>
  <summary>Details</summary>
Motivation: 当前遥感基础模型主要依赖扩大模型规模和数据集体积，需要大量计算资源和数据，限制了可访问性，且与可持续AI原则相悖。需要一种更高效、环保的替代方案。
...
Conclusion: 该方法通过定量指标和年龄缩放因子有效提高了阿尔茨海默病长期预测的准确性，为疾病进展监测提供了可靠工具。
...
Method: 利用预训练视频生成模型在噪声潜在空间进行奖励建模，提出PRFL框架在潜在空间进行偏好优化，无需VAE解码即可在整个去噪链中进行梯度反向传播
...
TL;DR: UAVLight是一个用于光照鲁棒3D重建的基准数据集，通过在不同固定时间点沿可重复飞行路径捕获场景，提供自然光照变化下的真实户外环境数据。


<details>
  <summary>Details</summary>
Motivation: 多视图3D重建中的光照不一致性是一个基本挑战，现有数据集要么缺乏有意义的光照多样性，要么因几何和语义变化而无法单独研究光照鲁棒性。

Method: 在每个场景中沿可重复、地理参考的飞行路径在多个固定时间点进行捕获，在一致的几何、校准和视点下产生自然光照变化。

Result: 创建了UAVLight基准数据集，提供了在真实户外环境中标准化评估协议，支持开发光照一致、忠实和可重光照的重建方法。

Conclusion: UAVLight为开发在真实户外环境中具有一致性、忠实性和可重光照性的重建方法提供了可靠基础。
...
<details>
  <summary>Details</summary>
Motivation: 当前骨盆透视地标检测方法大多假设固定的前后位视图，但实际术中患者姿态经常偏离标准视图，导致检测精度下降

Method: 将2D/3D地标配准集成到U-Net地标预测模型的训练中，比较基线U-Net、使用姿态估计损失训练的U-Net以及在姿态估计损失下微调的U-Net的性能

Result: 在患者姿态可变的真实术中条件下评估地标检测精度

Conclusion: 提出的框架能够提高在可变患者姿态下的地标检测性能
...
Method: 提出Cross-Task Synergy训练范式缓解漂移问题；设计Global-Local Decoupled Interaction Module实现高效时序对齐；开发Synchronization-Enhanced CFG在推理时显式增强对齐信号。

Result: 大量实验表明Harmony在生成保真度和细粒度音频-视频同步方面显著优于现有方法，建立了新的最先进水平。

Conclusion: Harmony框架通过机制性地强制音频-视频同步，成功解决了联合扩散过程中的关键挑战，为同步音频-视觉内容生成提供了有效解决方案。
...
Result: 实验结果显示模型达到83.33%准确率、89.12%精确率和77.31%召回率，优于现有最先进方法，特别是在少数类分类方面表现突出。

Conclusion: 该框架展示了过采样和增强策略在少数类分类中的有效性，是迈向可信赖的口腔癌早期计算机辅助诊断系统的有前景的第一步。
...
TL;DR: 本文主张重新定义LLM效率，从追求大规模复杂技术转向关注资源有限环境下的稳健简单性，提出新的研究方向以促进LLM部署的民主化。


<details>
  <summary>Details</summary>
Motivation: 当前主流效率方法（MoE、推测解码、复杂RAG）仅适用于拥有庞大基础设施的科技巨头，导致医院、学校、政府等机构无法受益，加剧了技术不平等和碳浪费问题。

Method: 提出新的研究议程：无需重新训练即可改造预训练模型架构、轻量级微调保持对齐、长思维链的经济推理、无需重型RAG管道的动态知识管理，并采用开销感知效率作为标准基准。

Result: 通过重新定义效率以包含采用成本、可持续性和公平性，可以促进LLM部署的民主化。

Conclusion: 下一代效率研究应关注稳健简单性，确保优化减少不平等和碳浪费，而非加剧这些问题。
...
TL;DR: HTP是一种无需训练的可逆确定性文本嵌入框架，通过将Unicode字符编码为谐波轨迹来生成嵌入向量，在STS-B基准测试中达到0.68的Spearman相关性，并在多语言场景下保持稳定性能。


<details>
  <summary>Details</summary>
Motivation: 传统神经嵌入依赖统计共现和优化过程，缺乏可解释性和确定性。HTP旨在提供一种透明、高效且无需训练的替代方案，直接从几何结构推导语义关系。

Method: 基于Unicode整数值，将每个token编码为谐波轨迹，建立离散符号与连续向量空间的双射映射。通过相位一致的投影保持结构和可逆性，从纯几何对齐估计语义相似度。

Result: 在STS-B及其多语言扩展测试中，HTP在英语上达到Spearman相关性ρ=0.68，在10种语言中保持稳定性能，计算成本极低，每句对延迟低于毫秒级。

Conclusion: 有意义的语义关系可以从确定性几何中涌现，HTP为数据驱动嵌入提供了透明高效的替代方案，证明了无需训练即可获得有效语义表示的可能性。
...
TL;DR: 提出基于双嵌入质心的分类框架，在IT服务管理系统中实现层次化文本分类，性能与SVM相当但训练和更新速度显著提升
...
Result: 在8,968个ITSM工单和123个类别上评估，性能与SVM相当（层次F1：0.731 vs 0.727），训练速度快5.9倍，增量更新快达152倍
...
TL;DR: PIRA提出了一种新的奖励模型训练范式，通过重构问答对为偏好指令、聚合多样化偏好任务奖励、以及使用dropout平均来稳定奖励，解决了传统奖励模型数据效率低和奖励过优化的问题。
...
Method: PIRA采用三种策略：(1)将问答对重构为基于偏好的指令，(2)聚合多样化偏好任务的奖励以减少偏见，(3)在不同dropout率下平均价值头输出以稳定奖励。
...
TL;DR: 该论文研究了如何通过重组法律文档结构、定义修辞角色和模拟法庭推理步骤来提升大型语言模型在法律任务中的表现，在零样本设置下实现了1.5%-4.36%的F1分数提升。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在通用推理方面表现出色，但在法律等专业领域表现不佳，主要因为缺乏领域特定的预训练，且法律文档通常冗长复杂，模型难以有效处理全文。

Method: 通过三个实验：(i)基于修辞角色重组文档以评估结构化信息对长文本处理的影响；(ii)定义修辞角色使模型熟悉法律术语；(iii)模拟法庭关于修辞角色的逐步推理过程来增强模型推理能力。

Result: 在三个印度法律判决预测数据集上的零样本实验显示，重组数据或解释关键法律术语显著提升了模型性能，F1分数相比基线最低提升约1.5%，最高提升4.36%。

Conclusion: 通过结构化信息处理和领域特定知识的引入，可以有效提升大型语言模型在法律领域的表现，特别是在处理复杂法律文档时。
...
TL;DR: 提出了一个新的心理健康分析基准数据集MindSET，包含1300万条来自Reddit的标注帖子，覆盖7种心理健康状况，通过严格预处理确保数据质量，在诊断检测任务上表现优于现有基准。
...
Result: MindSET数据集规模是之前基准的两倍多，在诊断检测任务上相比现有基准实现了高达18个F1分数的提升，特别是在自闭症检测方面表现突出。
...
TL;DR: FlexCode是一个基于流行度感知的生成式推荐框架，通过动态分配CF代码本和语义代码本的token预算，平衡协同过滤精度和语义泛化能力，解决了传统方法对热门和长尾项目统一编码的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有生成式推荐方法使用单一代码本编码所有项目，忽视了热门项目（富含协同信号）和长尾项目（依赖语义理解）之间的不平衡性，这种统一处理限制了表示效率并阻碍了泛化能力。

Method: 引入FlexCode框架，自适应地在协同过滤代码本和语义代码本之间分配固定token预算；使用轻量级MoE动态平衡CF精度和语义泛化；通过对齐和平滑目标保持流行度谱系的一致性。
...
TL;DR: 本文介绍了Ar-SParC，这是第一个阿拉伯语跨领域上下文相关的text-to-SQL数据集，包含3,450个相关问题和10,225个问题-SQL对。作者使用GPT模型进行了40个实验，并开发了GAT corrector方法，显著提升了性能。


<details>
  <summary>Details</summary>
Motivation: 目前大多数text-to-SQL研究和数据集都集中在英语和中文上，阿拉伯语领域尚未有相关研究。作者希望填补这一空白，为阿拉伯语用户提供与数据库自然语言交互的能力。

Method: 构建了Ar-SParC数据集，使用GPT-3.5-turbo和GPT-4.5-turbo模型，应用10种提示工程技术（4种问题表示方法和6种上下文学习技术），并开发了GAT corrector方法来提升性能。

Result: GAT corrector在所有40个实验中均提升了性能：在零样本设置下平均提升1.9%的执行准确率和1.9%的交互准确率；在上下文学习设置下平均提升1.72%的执行准确率和0.92%的交互准确率。

Conclusion: Ar-SParC是首个阿拉伯语跨领域上下文相关text-to-SQL数据集，GAT corrector方法有效提升了阿拉伯语text-to-SQL任务的性能，为阿拉伯语自然语言数据库交互提供了重要基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大型语言模型在临床基准测试中表现良好，但它们可能通过错误的推理得出正确结论，这种失败模式对肿瘤学决策支持具有安全影响，且无法通过基于准确性的评估来捕捉。

Method: 开发了一个三级分类法，从GPT-4对真实肿瘤学笔记的思维链响应中识别推理错误，并在822个前列腺癌咨询笔记响应上进行验证，模拟提取、分析和临床推荐任务。

Result: 23%的解释中出现推理错误，确认偏误和锚定偏误最常见。推理失败与指南不一致和潜在有害的推荐相关，特别是在晚期疾病管理中。自动评估器能检测错误存在但无法可靠分类子类型。
...
TL;DR: 提出了动态模板选择(DTS)方法，根据查询复杂度自适应选择响应模板，在不影响响应质量的前提下显著降低token成本。
...
Method: 比较了两种路由方法：使用预计算嵌入的简单MLP和微调的RoBERTa变换器。在1,000个MMLU问题上进行综合评估，MLP路由器的准确率达到90.5%，略高于RoBERTa的89.5%，且参数数量少1.25亿。

Result: MLP路由器在测试数据上达到90.5%的路由准确率，在三个主要LLM提供商(OpenAI GPT-4、Google Gemini和Anthropic Claude)上表现出提供商无关的行为。通过9,000个生产API调用验证，路由准确率保持90.5%一致，token减少量在32.6%到33.9%之间。

Conclusion: 该工作提供了几个关键贡献：基于机器学习的理论基础的正式问题表述、四种算法及其复杂度分析，以及在生产系统中的广泛实证验证。动态模板选择方法能够有效降低token成本，同时保持响应质量。
...
TL;DR: 本文探讨了二维材料在能源存储和转换中的应用潜力，以及如何从研究论文中提取材料特性和制备方法等信息。


<details>
  <summary>Details</summary>
Motivation: 二维材料因其独特的物理化学和电子特性在能源领域具有广泛应用前景，但相关信息分散在大量研究论文中，难以系统获取。

Method: 通过分析已发表的研究论文，提取二维材料的特性和制备方法等关键信息。

Result: 研究论文包含了二维材料的宝贵信息，包括其特性和制备方法，但这些信息分散在不同文献中。

Conclusion: 需要系统整理和分析二维材料相关研究论文，以便更好地利用这些材料在能源存储和转换中的应用潜力。
...
TL;DR: 提出多前缀记忆框架，通过衡量序列可通过不同前缀检索的多样性来定义记忆化，有效区分记忆与非记忆内容，用于审计LLM数据泄露。


<details>
  <summary>Details</summary>
Motivation: 现有记忆化定义在捕捉对齐模型中的记忆现象时存在不足，需要更全面框架来评估LLM的训练数据记忆问题。

Method: 引入多前缀记忆化框架，定义序列被记忆的标准是：外部对抗搜索能找到足够多的不同前缀来引出该序列，通过检索路径多样性量化记忆稳健性。

Result: 在开源和对齐聊天模型上的实验表明，多前缀定义能可靠区分记忆与非记忆数据，为审计LLM数据泄露提供实用工具。

Conclusion: 多前缀记忆化框架提供了一种稳健实用的方法来检测和量化LLM中的训练数据记忆，有助于解决隐私和版权风险。
...
TL;DR: SAGE是一个基于智能体的框架，将稀疏自编码器特征解释从被动生成任务转变为主动、解释驱动的过程，通过系统制定多解释、设计实验测试和迭代优化来提高解释质量。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的内部机制不透明，稀疏自编码器虽然能分解表示但特征解释仍具挑战性，需要更系统的方法来理解这些特征。

Method: 提出SAGE框架，系统地为每个特征制定多个解释，设计针对性实验进行测试，并基于激活反馈迭代优化解释。

Result: 在多种语言模型的稀疏自编码器特征上测试，SAGE产生的解释在生成和预测准确性方面显著优于现有基线方法。

Conclusion: SAGE通过主动、实验驱动的特征解释方法，有效提升了稀疏自编码器特征的可解释性，为理解大型语言模型内部机制提供了新途径。
...
TL;DR: 本文提出了DSPy+HELM框架，通过结构化提示方法改进语言模型评估，发现传统HELM基准会低估模型性能并产生误导性排名。


<details>
  <summary>Details</summary>
Motivation: 现有语言模型评估框架（如HELM）使用固定提示，无法准确估计模型性能上限，导致性能低估和误导性部署决策。

Method: 开发了可复现的DSPy+HELM框架，引入四种结构化提示方法（包括思维链推理），在7个基准测试上评估4个前沿语言模型。

Result: 发现：HELM平均低估4%性能；性能估计方差增加2%；3/7基准的排名反转；思维链提示减少对提示设计的敏感性。

Conclusion: 可扩展的性能上限估计能够提供更有决策价值的基准测试，这是首个大规模实证研究语言模型在基准测试和提示方法上的行为特征。
...
<details>
  <summary>Details</summary>
Motivation: 传统分词器如BPE主要基于词频优化，但未充分考虑token长度对效率的影响。本文旨在通过优化平均token长度来提高语言模型的训练和推理效率。

Method: 将长度加权目标最大化建模为图划分问题，开发贪心近似算法获得词汇表，最小化平均字符数。

Result: 在FineWeb和多种领域上，相比BPE减少14-18%的token；GPT-2模型训练步骤减少17-18%，推理延迟降低13-14%，吞吐量提升16%；下游任务LAMBADA困惑度降低11.7%，HellaSwag准确率提升4.3%；词汇覆盖率达99.62%。

Conclusion: 优化平均token长度而非仅频率，是实现高效语言建模的有效方法，在不牺牲性能的情况下显著提升效率，兼容生产系统并减少18%的嵌入和KV缓存内存。
...
TL;DR: 提出了Evo-Memory基准和框架，用于评估LLM代理的自进化记忆能力，强调在连续任务流中记忆的检索、整合和更新。
...
Method: 构建了顺序任务流数据集，统一实现了十多个代表性记忆模块，提出了ExpRAG基线方法和ReMem行动-思考-记忆优化管道。

Result: 在10个多样的多轮目标导向和单轮推理问答数据集上评估了记忆模块的性能。

Conclusion: Evo-Memory填补了LLM代理自进化记忆评估的空白，为持续改进提供了有效框架。
...
TL;DR: 本研究采用跨语言方法进行论辩挖掘，针对低资源语言构建了三种训练场景：零样本迁移、LLM增强训练和跨语言模型，在英语和波斯语上验证效果。


<details>
  <summary>Details</summary>
Motivation: 解决低资源语言在论辩挖掘任务中面临的数据资源短缺问题，探索跨语言迁移的有效方法。

Method: 构建三种训练场景：零样本迁移（仅用英语数据）、LLM生成合成数据增强、跨语言模型（结合英语数据和人工翻译的波斯语句子）。

Result: 零样本迁移模型在英语和波斯语测试集上F1分数分别为50.2%和50.7%；LLM增强模型提升至59.2%和69.3%；跨语言模型在波斯语测试集上达到74.8%的F1分数。

Conclusion: 轻量级的跨语言混合方法能够显著优于资源密集的数据增强流程，为低资源语言的论辩挖掘任务提供了实用的解决方案。
...
TL;DR: 提出了一种结合角色交叉最小对、时间涌现分析和跨模型比较的方法，研究LLMs如何实现语义角色。研究发现LLMs形成紧凑的、因果隔离的抽象语义结构机制，这些机制在不同规模和架构间存在部分迁移。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型显示出语义能力，但其实现抽象语义结构的内在机制仍缺乏充分表征。
...
Conclusion: LLMs形成紧凑的、因果隔离的抽象语义结构机制，这些机制在不同规模和架构间存在部分迁移。
...
TL;DR: Chatty-KG是一个用于知识图谱对话问答的模块化多智能体系统，结合了检索增强生成和结构化查询执行，在单轮和多轮对话中显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有知识图谱问答系统在对话上下文处理、结构保持和延迟方面的局限性，将LLM的自然对话能力与知识图谱的结构化知识相结合。

Method: 采用模块化多智能体架构，通过专门化的LLM智能体协作进行上下文解释、对话跟踪、实体关系链接和查询规划，生成可执行的SPARQL查询。

Result: 在多样化的知识图谱上，Chatty-KG在单轮和多轮设置中显著优于最先进的基线方法，获得更高的F1和P@1分数，且具有低延迟特性。

Conclusion: Chatty-KG统一了对话灵活性和结构化知识图谱基础，为可靠的多轮知识图谱问答提供了可扩展和可扩展的方法，兼容商业和开源LLM。
...
TL;DR: LLMs在定义类问题表现最佳，但在举例类问题表现最差；高频概念上更倾向于释义，低频技术知识上表现较差


<details>
  <summary>Details</summary>
Motivation: 评估LLMs在处理不同类型查询时的性能差异，特别是定义类问题与其他类型问题的表现差距

Method: 使用TrackList分析流水线和RefoMed-EN数据集（6170个医学术语的标注数据），通过句法语义相似度、统计相关性和嵌入等方法评估LLM输出质量

Result: LLMs在定义类问题表现最高，举例类问题表现最低；对于定义类问题，LLMs更倾向于对高频知识进行释义，对低频技术知识表现较差

Conclusion: LLMs在处理不同类型查询时存在显著性能差异，特别是在低频技术知识领域表现不佳，需要改进对多样化查询类型的处理能力
...
Result: 在8个分类任务和8个开源LLM（1-12B参数）中，发现ICL无法学习连贯的反语义分类器：提示对齐增加以牺牲准确性为代价，语义覆盖率在少样本设置中保持为零。

Conclusion: ICL主要调整输入如何投影到预训练期间学习的稳定语义方向，而不是灵活地重新映射标签含义，表明在这些规模上覆盖标签语义需要ICL之外的干预措施。
...
TL;DR: 提出了一种基于检索的上下文增强方法，结合非参数知识检索和自知识检索策略，显著提升了LLM在讽刺检测任务中的性能。


<details>
  <summary>Details</summary>
Motivation: 讽刺检测在NLP中仍然具有挑战性，PLM和LLM在处理复杂讽刺文本时存在困难，特别是在涉及文化特定俚语、引用或未知术语时表现不可靠。

Method: 在PMP方法基础上引入检索感知方法：1）使用基于网络的检索添加非参数知识；2）激发模型内部知识的自知识意识策略。

Result: 在三个数据集上的实验显示：非参数检索在Twitter Indonesia Sarcastic上相比原始PMP方法提升9.87% macro-F1；自知识检索在Semeval和MUStARD上分别提升3.29%和4.08% macro-F1。

Conclusion: 上下文信息对提升LLM在讽刺检测任务中的性能至关重要，特别是在处理文化特定内容时。未来工作将优化相关上下文信息的检索并研究检索质量对性能的影响。
...
TL;DR: 在缅甸语等低资源语言分类任务中，Kolmogorov-Arnold Networks (KANs)作为分类头优于传统MLPs，在准确性和效率方面表现出竞争力。


<details>
  <summary>Details</summary>
Motivation: 传统MLPs在低资源语言分类中固定非线性可能限制表达能力并增加计算成本，需要探索更有效的替代方案。

Method: 评估了三种KAN变体（FourierKAN、EfficientKAN、FasterKAN）作为分类头，使用TF-IDF、fastText和多语言transformer等多种嵌入方法。

Result: EfficientKAN与fastText组合获得最高F1分数（0.928），FasterKAN在速度和准确性之间达到最佳平衡，在transformer嵌入上KANs与MLPs性能相当或略优。
...
TL;DR: 对28种大语言模型配置在字符级约束满足任务上的评估显示，架构差异比参数规模扩展对性能影响更大，模型在拼写异常但约束有效的词汇上存在系统性失败。


<details>
  <summary>Details</summary>
Motivation: 系统评估不同架构大语言模型在受控文本生成中满足硬性拼写约束的能力，现有研究对此缺乏跨架构比较。

Method: 评估三个模型家族（Qwen3、Claude Haiku-4.5、GPT-5-mini）的28种配置在58个需要字符级约束满足的单词谜题上的表现，并使用10,000名人类解谜者的难度评级进行分析。

Result: 架构差异产生的性能差距（2.0-2.2倍）远大于家族内参数扩展（83%增益），模型在拼写异常的常见词汇上存在系统性失败（人类成功率86-95%，模型失败率89-96%）。

Conclusion: 约束满足可能需要专门的架构特征或训练目标，而不仅仅是参数规模或计算预算的扩展，模型过度依赖分布合理性而惩罚拼写异常但约束有效的模式。
...
TL;DR: 该论文研究了针对低资源缅甸语的ASR错误校正，使用序列到序列Transformer模型，结合IPA和语音对齐特征，显著降低了词错误率和提升了字符级准确率。
...
Method: 使用序列到序列Transformer模型，整合IPA（国际音标）和语音对齐特征，评估了五种ASR骨干网络。

Result: 提出的AEC模型将平均WER从51.56降至39.82（增强前）和43.59（增强后），chrF++分数从0.5864提升至0.627。
...
TL;DR: 提出了MortgageLLM，一个专门针对抵押贷款金融领域的双专家大语言模型，通过双轨专业化框架解决单一模型在结构化任务和对话任务之间的性能权衡问题。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在通用领域表现出色，但在专业领域如抵押贷款金融中需要增强领域知识，同时保持指令跟随能力。单一模型在多任务优化中存在性能权衡问题。

Method: 采用双轨专业化框架，从单一基础模型创建两个专家模型：对话问答模型和结构化任务模型。应用指令残差技术恢复指令跟随能力，并使用少量样本分类的任务路由机制。

Result: 在领域特定基准测试中，最终模型在摘要任务得分4.58（vs 3.99）、问答得分4.09（vs 4.0）、分类得分2.6（vs 1.2），在语义相似度指标上也显著优于基线方法。

Conclusion: 双专家架构和指令残差技术有效解决了专业领域大语言模型的知识增强与指令跟随能力保持的双重挑战，在抵押贷款金融领域取得了显著性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 推理模型在复杂推理任务中表现出色，但其对对抗性越狱提示的安全性仍面临挑战，需要自适应安全对齐方法来增强防御能力。

Method: SGASA框架包含两个阶段：数据预合成（生成安全指南和增强提示）和对齐微调（使用SFT和DPO将指南嵌入模型）。

Result: 在多个数据集上的广泛实验表明，SGASA显著提高了模型安全性。

Conclusion: SGASA框架具有自适应和可扩展的有效性，能够增强模型对有害对抗提示的鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 探讨是否通过在少量人类调查数据（如试点研究数据）上微调LLM，能够缓解LLM在模拟人类行为时存在的多样性不足、少数群体系统性不对齐、组内方差不足以及信念与行为不一致等问题。

Method: 使用信息披露行为实验，从分布差异、子群体对齐、信念-行为一致性和回归系数恢复等多个维度比较人类和LLM生成的响应。

Result: 在少量人类样本上微调显著改善了基础模型的异质性、对齐度和信念-行为一致性，但即使最佳微调模型也无法复现原始研究的回归系数。

Conclusion: LLM生成的数据仍不适合替代人类参与者进行正式的推断分析，尽管微调能在某些方面改善模拟效果。
...
TL;DR: 开发了首个伊桑语开放对话语音数据集，包含自然语音而非朗读语音，解决了伊桑语缺乏标准化正字法的挑战，建立了实用的转录协议。


<details>
  <summary>Details</summary>
Motivation: 伊桑语是泰国最广泛使用的地区方言，但缺乏自然对话语音数据集。现有语料库主要是朗读或脚本语音，无法捕捉真实语言现象如口语化表达、自发韵律、不流利和与泰语的频繁语码转换。

Method: 建立实用的转录协议，在表征准确性和计算处理需求之间取得平衡，解决了伊桑语缺乏标准化正字法的问题。

Result: 成功创建了首个伊桑语开放对话语音数据集，包含自然语音，捕捉了真实的语言现象。

Conclusion: 通过发布这一开放资源，旨在促进包容性AI发展，支持对代表性不足语言的研究，并为解决建模对话语音的语言和技术挑战提供基础。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在许多任务上达到最先进性能，但其大规模导致高计算和环境成本，限制了可访问性。现有的PEFT方法评估有限且难以复现。

Method: 开发PEFT-Bench基准测试，在27个NLP数据集和6种PEFT方法上进行评估，并引入PSCP指标综合考虑可训练参数、推理速度和训练内存使用。

Result: 建立了一个统一的评估框架，能够系统比较不同PEFT方法的性能表现和效率权衡。
...
TL;DR: 首次系统研究神经网络语言模型训练过程中生成的文本是否遵循Martin定律（词频与多义性关系），发现该规律在训练中期出现峰值后下降，呈现非单调发展轨迹。


<details>
  <summary>Details</summary>
Motivation: 探究神经网络语言模型在训练过程中是否以及如何发展出符合人类语言规律的特征，特别是Martin定律这种词汇语义规律。

Method: 使用DBSCAN聚类分析上下文嵌入来操作化词义，分析四个Pythia模型（70M-1B参数）在30个训练检查点上的表现。

Result: Martin定律在检查点100左右出现，在检查点104达到峰值相关性（r>0.6），然后在检查点105下降。小模型在后期检查点经历灾难性语义崩溃，而大模型显示优雅退化。

Conclusion: 语言模型对语言规律的遵循不是单调递增的，而是遵循具有最优语义窗口的平衡轨迹，这为评估神经网络语言模型中的涌现语言结构建立了新方法。
...
TL;DR: 通过微调训练，成功让7B参数模型从几乎无法检测注入的"思想"（0.4%准确率）转变为可靠检测（85%准确率），满足Lindsey提出的三个标准中的准确性、基础性和内部性。


<details>
  <summary>Details</summary>
Motivation: 解决Lindsey研究中提出的开放性问题：是否可以通过训练来消除跨模型在自省能力上的差异，探索直接训练自省能力的可能性。

Method: 通过对瞬态单令牌注入进行微调，训练7B参数模型检测和识别注入的激活模式，并保留该信息在后续生成步骤中报告语义内容。

Result: 模型从0.4%准确率提升到85%准确率，假阳性率为0%，能够检测单令牌位置注入的短暂"思想"，并对未见过的概念向量表现出7.5个百分点的泛化能力差距。

Conclusion: 研究表明至少部分自省行为可以通过直接训练诱导，为构建内置AI透明度提供了一条可行路径，但并未在Lindsey意义上建立元认知表征。
...
TL;DR: 该论文创建了捷克语和斯洛伐克语的细粒度证据提取数据集，评估了大型语言模型在证据提取任务中的表现，发现LLMs经常无法从源文本中逐字复制证据，导致无效输出。
...
Method: 创建了由付费标注者标注的双向细粒度证据数据集，评估了大型语言模型在该数据集上的表现，以评估它们与人类标注的一致性。

Result: 结果显示LLMs经常无法从源文本中逐字复制证据，导致无效输出。llama3.1:8b模型尽管规模较小但正确输出比例高，而gpt-oss-120b模型尽管参数更多但表现不佳。qwen3:14b、deepseek-r1:32b和gpt-oss:20b模型在模型大小与人类标注一致性之间达到了有效平衡。

Conclusion: 大型语言模型在细粒度证据提取任务中表现参差不齐，模型大小与性能之间没有必然的正相关关系，某些中等规模模型在准确性和效率方面表现更佳。
...
Method: DSR-SQL将Text-to-SQL建模为自适应上下文状态和渐进生成状态之间的交互。上下文状态通过精炼大型模式并选择相关结构来构建紧凑的语义环境；生成状态将SQL合成形式化为反馈引导的状态转换，使模型能够自我纠正并与用户意图对齐。
...
Method: 提出Odin架构，在特定Transformer层通过定向双模块机制注入图结构，不依赖多跳扩散，而是在不同层次集成多跳结构。还提出了轻量级变体Light Odin以提高效率。
...
TL;DR: 对六种最先进的模型融合方法在大型语言模型上进行大规模系统评估，发现最简单的方法Task Arithmetic是唯一能可靠提升性能的方法，其他方法通常导致性能显著下降。


<details>
  <summary>Details</summary>
Motivation: 模型融合能够在不额外训练的情况下将多个微调检查点合并为单一模型，但现有研究主要针对小模型和分类器，不清楚这些优势是否能推广到LLMs。

Method: 对六种最先进的融合方法（包括子空间方法）进行大规模评估，涵盖四个开源LLM、每个基础模型十二个微调检查点以及十六个标准LLM基准测试。

Result: Task Arithmetic是唯一能可靠提升LLM性能的方法，其他干扰感知和子空间融合方法通常导致显著性能下降。当前融合技术不能直接迁移到现代LLMs。
...
TL;DR: 提出了一种双向可读性评估机制，通过捕捉上下文信息识别文本中语义丰富区域，预测句子级可读性，并引入成对排序算法建模可读性级别间的序数关系。
...
Method: 使用双向可读性评估机制捕获上下文信息，识别语义丰富区域预测句子级可读性；引入成对排序算法通过标签减法建模可读性级别的序数关系。
...
TL;DR: 本文研究了语音翻译模型中性别分配的机制，发现模型不仅复制训练数据中的性别关联，还学习更广泛的男性主导模式，并揭示了一种通过第一人称代词将性别化术语与说话者关联的新机制。


<details>
  <summary>Details</summary>
Motivation: 语音翻译中，说话者的声音特征可能影响性别化术语的分配，存在误判性别的风险，但目前对语音翻译模型如何做出这些决策的理解不足。

Method: 研究三个语言对（英-西/法/意）的语音翻译模型，分析训练数据模式、内部语言模型偏见和声学信息的交互作用，使用对比性特征归因分析频谱图。

Result: 模型能够基于声学输入覆盖内部语言模型的男性偏见偏好，性别准确性更高的模型通过第一人称代词将性别化术语与说话者关联，利用分布在频率谱而非集中在音高的性别信息。

Conclusion: 语音翻译模型学习更广泛的男性主导模式而非简单的术语特定性别关联，并发展出通过第一人称代词访问分布在整个频率谱的性别信息的新机制。
...
TL;DR: 提出了孟加拉手语翻译数据集IsharaKhobor及其两个子集，解决了孟加拉手语资源匮乏的问题，并提供了基准测试结果。


<details>
  <summary>Details</summary>
Motivation: 孟加拉手语翻译研究严重受限，因为该语言资源匮乏。为开发面向孟加拉语社区聋人和听力障碍人士的AI辅助工具，创建标准句子级数据集至关重要。

Method: 开发了IsharaKhobor数据集及其两个子集IsharaKhobor_small和IsharaKhobor_canonical_small，使用基于地标的原始嵌入和RQE嵌入进行基准测试，并对词汇限制和规范化进行了消融研究。

Result: 成功创建了公开可用的孟加拉手语翻译数据集，并提供了基准测试结果，为后续研究奠定了基础。

Conclusion: IsharaKhobor数据集填补了孟加拉手语翻译研究的空白，为开发AI辅助工具提供了重要资源，并展示了词汇限制和规范化对数据集性能的影响。
...
TL;DR: RoParQ是一个评估LLM在转述问题中一致性的基准，通过XParaCon指标和针对性微调策略提升模型鲁棒性。
...
Result: 针对性对齐显著增强了鲁棒性，轻量级微调模型达到了与更大预训练模型相当的稳定性水平。
...
### [146] [Minimizing Hyperbolic Embedding Distortion with LLM-Guided Hierarchy Restructuring](https://arxiv.org/abs/2511.20679)
...
<details>
  <summary>Details</summary>
Motivation: 双曲嵌入的质量与输入层次结构密切相关，而最优双曲嵌入需要高分支因子和单继承特性。为了帮助知识工程师重新组织层次知识，研究LLM是否能够自动重构层次结构以满足这些标准。
...
### [147] [AssurAI: Experience with Constructing Korean Socio-cultural Datasets to Discover Potential Risks of Generative AI](https://arxiv.org/abs/2511.20686)
...
TL;DR: AssurAI是一个针对韩语多模态生成AI安全评估的质量控制数据集，包含11,480个文本、图像、视频和音频实例，涵盖35种AI风险因素，特别关注韩国社会文化背景。
...
Method: 定义35种AI风险因素分类法，通过多学科专家组构建数据集，采用两阶段构建（专家引导种子和众包扩展）、三重独立标注和迭代专家红队测试的质量控制流程。
...
### [148] [$A^2Flow:$ Automating Agentic Workflow Generation via Self-Adaptive Abstraction Operators](https://arxiv.org/abs/2511.20693)
...
Result: 在通用和具身基准测试中，A²Flow实现了2.4%和19.3%的平均性能提升，资源使用减少37%，优于最先进的基线方法。
...
### [149] [Reasoning With a Star: A Heliophysics Dataset and Benchmark for Agentic Scientific Reasoning](https://arxiv.org/abs/2511.20694)
...
<details>
  <summary>Details</summary>
Motivation: 解决日球物理学中科学推理的挑战，包括整合物理假设、保持单位一致性和提供清晰科学格式的需求。

Method: 构建来自NASA和UCAR"Living With a Star"暑期学校问题集的数据集，采用程序化评分器检查预测结果，并比较单次提示和四种多智能体模式。

Result: 通过系统工程原则分解工作流程的多智能体模式在需要演绎推理的问题上表现优于直接提示方法。

Conclusion: 多智能体推理方法在复杂科学推理任务中具有优势，特别是在需要演绎推理而非纯粹归纳回忆的问题上。
...
### [150] [A Brief History of Digital Twin Technology](https://arxiv.org/abs/2511.20695)
...
<details>
  <summary>Details</summary>
Motivation: 将数字孪生技术从工业和航天领域引入医疗健康，旨在实现从被动治疗向预测性、预防性和个性化医疗的转变，提高医疗决策的精确性和效率。

Method: 整合医学影像、生物传感器和计算模型，创建动态、数据驱动的患者虚拟副本，通过实时数据流进行持续更新，并支持双向交互。
...
### [151] [Paraconsistent-Lib: an intuitive PAL2v algorithm Python Library](https://arxiv.org/abs/2511.20700)
...
TL;DR: Paraconsistent-Lib是一个开源的Python库，用于构建PAL2v算法，支持12种经典格区域的分析、分析节点输出和决策输出，简化了多种PAL2v算法的实现。


<details>
  <summary>Details</summary>
Motivation: 为了提供一个易于使用的通用PAL2v计算库，减少算法实现的复杂性、代码量和错误，并响应用户需求进行持续开发。

Method: 设计为通用PAL2v标准计算库，支持三种结果类型：12种经典格区域的矛盾分析、矛盾分析节点输出和决策输出，可以独立或网络形式实现多种PAL2v算法。

Result: 成功开发了Paraconsistent-Lib库，能够实现Para-analyzer、ParaExtrCTX、PAL2v Filter、PANnet和PNN等算法，并通过两个示例展示了其有效性。

Conclusion: Paraconsistent-Lib是一个稳定且持续发展的开源库，能够简化PAL2v算法的构建过程，并通过GitHub接收用户反馈进行功能增强。
...
### [152] [Cross Domain Evaluation of Multimodal Chain-of-Thought Reasoning of different datasets into the Amazon CoT Framework](https://arxiv.org/abs/2511.20701)
...
<details>
  <summary>Details</summary>
Motivation: 探索多模态思维链方法在科学问答之外的多样化领域（如常识推理、图表理解）中的泛化能力，因为现有方法主要针对科学推理任务。

Method: 采用Zhang等人提出的两阶段框架：先分离生成推理依据，再进行答案推断，通过门控融合机制整合视觉特征与基于T5的语言模型。

Result: 视觉特征整合显著减少了推理依据生成中的幻觉现象，但思维链推理的有效性在不同问题类型间差异很大，常识推理尤其具有挑战性。

Conclusion: 为多模态推理系统的实现提供了实用见解，并指出了跨领域泛化能力改进的关键方向。
...
### [153] [Learning Multi-Access Point Coordination in Agentic AI Wi-Fi with Large Language Models](https://arxiv.org/abs/2511.20719)
*Yifan Fan,Le Liang,Peng Liu,Xiao Li,Ziyang Guo,Qiao Lan,Shi Jin,Wen Tong*

Main category: cs.AI

TL;DR: 提出了一个基于大型语言模型代理的智能Wi-Fi框架，让接入点能够通过自然语言对话实时协商自适应协调策略，在动态网络环境中显著提升性能。
...
Method: 将每个接入点建模为自主的大型语言模型代理，通过认知工作流程进行自然语言对话，利用集成记忆、反思和工具使用来基于过往经验和环境反馈做出决策。

Result: 仿真结果表明，该代理框架成功适应了多样化和动态的网络环境，显著优于最先进的空间复用基准方法。
...
### [154] [OpenApps: Simulating Environment Variations to Measure UI-Agent Reliability](https://arxiv.org/abs/2511.20766)
...
TL;DR: OpenApps是一个轻量级开源生态系统，包含6个可配置外观和内容的应用程序，用于评估多模态UI代理在应用变体中的可靠性。研究发现，在固定应用中的标准可靠性相对稳定，但在应用变体中可靠性差异巨大，某些代理的任务成功率波动超过50%。


<details>
  <summary>Details</summary>
Motivation: 当前自主UI代理的评估依赖固定环境，无法衡量代理在不同应用设计和内容变体中的可靠性，而实际部署时会遇到各种应用变体影响任务完成能力。

Method: 开发OpenApps生态系统，包含6个可配置应用，使用单个CPU即可运行并生成数千个应用版本。进行了超过10,000次独立评估，研究7个领先多模态代理在应用变体中的可靠性。

Result: 代理在固定应用中的可靠性相对稳定，但在应用变体中可靠性差异巨大。Kimi-VL-3B在所有任务中的平均成功率从63%波动到仅4%。代理行为如循环或幻觉操作也因环境配置而异。

Conclusion: 测量代理在应用变体维度上的可靠性至关重要，OpenApps为此提供了评估工具，揭示了当前代理在面对应用变化时的脆弱性。
...
TL;DR: RILKE是一种在表示空间进行干预的知识控制方法，通过低维子空间更新和查询自适应路由，实现LLMs知识的终身更新而无需重新训练。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型经常产生错误或过时的内容，如何在终身学习设置中高效准确地更新其知识而不需要昂贵的重新训练是一个主要挑战。

Method: RILKE在模型的表示空间进行干预，学习抗释义和编辑局部化的模块，将每个更新限制在低维子空间以减少交叉编辑干扰，并通过查询自适应路由选择适当的模块。

Result: 在LLaMA和Qwen模型的知识编辑基准测试中，RILKE可扩展到大规模数据集，表现出高编辑成功率、强释义泛化能力，并以适度的内存开销保持通用效用。

Conclusion: RILKE是LLMs终身知识控制的有效且可扩展的解决方案。
...
### [156] [Guaranteed Optimal Compositional Explanations for Neurons](https://arxiv.org/abs/2511.20934)
*Biagio La Rosa,Leilani H. Gilpin*

Main category: cs.AI

TL;DR: 提出了首个保证最优的组合解释计算框架，包含分解方法、启发式估计和高效算法，发现在计算机视觉领域10-40%的beam search解释是次优的。


<details>
  <summary>Details</summary>
Motivation: 现有组合解释方法使用beam search但无法提供最优性保证，不清楚当前解释与真正最优解的接近程度。

Method: 提出分解框架识别影响空间对齐的因素，设计启发式估计对齐度，开发首个能在可行时间内计算最优组合解释的算法。

Result: 在计算机视觉和卷积神经网络中，当涉及重叠概念时，10-40%的beam search解释是次优的；提出的beam search变体在运行时间上匹配或优于先前方法。

Conclusion: 该框架首次提供了保证最优的组合解释，揭示了现有方法的局限性，并展示了改进的beam search方法在保持效率的同时提供更大灵活性。
...
### [157] [ENACT: Evaluating Embodied Cognition with World Modeling of Egocentric Interaction](https://arxiv.org/abs/2511.20937)
...
TL;DR: ENACT是一个评估具身认知的基准，通过视觉问答形式测试模型从自我中心交互中进行世界建模的能力，包括前向世界建模和逆向世界建模两个任务。


<details>
  <summary>Details</summary>
Motivation: 研究现代视觉语言模型是否表现出具身认知的迹象，因为具身认知理论认为智能源于感觉运动交互而非被动观察。

Method: 将具身认知评估构建为部分可观察马尔可夫决策过程，使用BEHAVIOR机器人模拟生成8,972个QA对，包含前向世界建模和逆向世界建模两个序列重排任务。

Result: 前沿视觉语言模型与人类存在性能差距，且随着交互视野扩大而加剧；模型在逆向任务上表现优于前向任务，并表现出人类中心偏见。

Conclusion: 现代视觉语言模型确实显示出一些具身认知的迹象，但仍与人类能力存在差距，特别是在长期交互和视角变化方面。
...
### [158] [Improving Procedural Skill Explanations via Constrained Generation: A Symbolic-LLM Hybrid Architecture](https://arxiv.org/abs/2511.20942)
*Rahul Dass,Thomas Bowlin,Zebing Li,Xiao Jin,Ashok Goel*

Main category: cs.AI

TL;DR: Ivy系统结合符号TMK模型与LLM生成层，通过结构化约束提升教学解释的质量，在程序技能学习中提供因果、目标和组合逻辑的深度解释。


<details>
  <summary>Details</summary>
Motivation: 解决LLM生成的教学解释虽然流畅但缺乏深度结构的问题，需要在程序技能学习中传达步骤背后的因果、目标和组合逻辑。

Method: 开发Ivy系统，将符号Task-Method-Knowledge (TMK)模型与生成式LLM解释层结合，TMK编码因果转换、目标层次和问题分解，约束LLM在明确结构边界内构建解释。

Result: 与GPT和检索增强GPT基线相比，Ivy在专家和独立标注的三个推理维度上，符号约束持续提升了"如何"和"为什么"问题解释的结构质量。

Conclusion: 研究证明了一种可扩展的AI教育方法，通过符号约束增强AI生成解释在教学辅导系统中的教育价值。
...
### [159] [ICPO: Intrinsic Confidence-Driven Group Relative Preference Optimization for Efficient Reinforcement Learning](https://arxiv.org/abs/2511.21005)
*Jinpeng Wang,Chao Li,Ting Ye,Mengyuan Zhang,Wei Liu,Jian Luan*

Main category: cs.AI

TL;DR: 提出了ICPO方法，通过利用LLM生成不同响应的概率来反映其对推理过程的自我评估，结合可验证奖励来增强推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的RLVR方法存在奖励粒度粗、奖励噪声和探索效率低等问题，导致训练不稳定和熵崩溃，需要更好的方法来提升LLM的推理能力。

Method: ICPO通过计算相同输入提示下多个响应的相对生成概率来获得偏好优势分数，并将该分数与可验证奖励结合来指导探索过程。

Result: 在四个通用领域基准和三个数学基准上的综合实验表明，ICPO相比GRPO能够稳定提升推理能力。

Conclusion: ICPO方法有效解决了现有RLVR方法的局限性，通过偏好优势分数缓解了奖励粒度和噪声问题，同时抑制了过度自信错误，促进了更彻底的探索。
...
### [160] [Towards Trustworthy Legal AI through LLM Agents and Formal Reasoning](https://arxiv.org/abs/2511.21033)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的系统擅长表层文本分析，但缺乏原则性法理学所需的保证。法律理性表现为实质理性（结果公平性）和形式理性（遵循明确规则），需要结合两者的方法。

Method: 三阶段流程：1) 法规形式化：将法律条款转换为逻辑公式；2) 双重事实和法规提取：检察官和辩护方对齐的LLM独立映射案件叙述；3) 求解器中心裁决：将双方论证编译为逻辑约束，通过不满足核心触发迭代自我批评，最终由法官LLM生成透明裁决。

Result: 在公共基准测试中，该系统超越了GPT-4-mini、DeepSeek-V3和Claude 4等先进LLM以及最先进的法律AI基线，同时提供严谨且可解释的符号论证。

Conclusion: L4M成功地将自然语言的灵活性与符号验证的严谨性相结合，为法律决策提供了既灵活又可靠的方法，实现了实质理性和形式理性的统一。
...
### [161] [OVOD-Agent: A Markov-Bandit Framework for Proactive Visual Reasoning and Self-Evolving Detection](https://arxiv.org/abs/2511.21064)
*Chujie Wang,Jianyu Lu,Zhiyuan Luo,Xi Chen,Chu He*

Main category: cs.AI

TL;DR: OVOD-Agent将开放词汇目标检测从被动类别匹配转变为主动视觉推理和自我进化检测，通过视觉思维链和弱马尔可夫决策过程建模视觉上下文转换，在罕见类别上表现优异。
...
Method: 提出OVOD-Agent框架，将文本优化过程扩展为可解释的视觉思维链，通过弱马尔可夫决策过程建模八个状态空间的视觉上下文转换，使用Bandit模块生成探索信号，并结合马尔可夫转移矩阵与Bandit轨迹进行自监督奖励模型优化。

Result: 在COCO和LVIS数据集上的实验表明，OVOD-Agent在各种OVOD骨干网络上均能提供一致改进，特别是在罕见类别上表现突出。
...
### [162] [Causality Without Causal Models](https://arxiv.org/abs/2511.21260)
*Joseph Y. Halpern,Rafael Pass*

Main category: cs.AI

TL;DR: 本文提出了Halpern-Pearl因果关系的抽象定义，使其能应用于更广泛的模型，包括处理复杂逻辑公式和嵌套反事实的情况。
...
Method: 通过抽象化Halpern-Pearl定义的关键特征，构建一个可在任何定义了反事实的模型中应用的因果定义框架。

Result: 新定义不仅扩展了应用范围（支持回溯等特性），还能处理复杂逻辑公式，并进一步扩展到解释的抽象定义。

Conclusion: 抽象化方法提供了更通用的因果定义框架，深化了对因果定义特征的理解，并扩展了其在各类模型中的应用能力。
...
### [163] [Prune4Web: DOM Tree Pruning Programming for Web Agent](https://arxiv.org/abs/2511.21398)
*Jiayuan Zhang,Kaiquan Chen,Zhihao Lu,Enshen Zhou,Qian Yu,Jing Zhang*

Main category: cs.AI

TL;DR: Prune4Web提出了一种新的DOM处理范式，通过程序化剪枝而非LLM直接读取来高效处理大规模DOM结构，在网页自动化任务中实现25-50倍的候选元素减少，显著提升动作定位精度。


<details>
  <summary>Details</summary>
Motivation: 现有基于LLM的网页代理在处理复杂真实网页时面临DOM结构过大的挑战（通常1万-10万token），现有策略要么依赖粗糙的DOM截断（可能丢失关键信息），要么使用低效启发式方法，无法在精度和可扩展性之间达到平衡。

Method: 提出DOM树剪枝编程方法，让LLM生成可执行的Python评分脚本来动态筛选DOM元素，基于分解子任务的语义线索。该方法将DOM遍历和评分委托给轻量级可解释程序，避免LLM直接处理原始大规模DOM。

Result: 在低级别定位任务中，准确率从46.8%大幅提升至88.28%，实现了25-50倍的候选元素减少，有效缓解注意力稀释问题，在真实网页自动化中表现出最先进的性能。

Conclusion: Prune4Web通过将DOM处理从资源密集的LLM读取转向高效程序化剪枝，成功解决了大规模DOM处理的挑战，为网页自动化提供了更精确和可扩展的解决方案。
...
### [164] [New Hybrid Heuristics for Pseudo-Boolean Propagation](https://arxiv.org/abs/2511.21417)
*Mia Müßig,Jan Johannsen*

Main category: cs.AI

TL;DR: 提出新的启发式方法来改进伪布尔求解中的混合单元传播策略，显著提升RoundingSAT求解器的性能


<details>
  <summary>Details</summary>
Motivation: 当前伪布尔求解中最成功的单元传播策略是观察文字方案与计数方法的混合模式，但现有混合决策方法仍有改进空间

Method: 为混合决策引入新的启发式方法，优化观察文字方案与计数方法之间的选择

Result: 新启发式方法能够显著优于当前方法，在RoundingSAT求解器中实现大幅性能提升

Conclusion: 新的混合决策启发式方法能够有效提升伪布尔求解器的单元传播效率
...
### [165] [Conversational no-code and multi-agentic disease module identification and drug repurposing prediction with ChatDRex](https://arxiv.org/abs/2511.21438)
...
<details>
  <summary>Details</summary>
Motivation: 传统药物重定位预测需要多领域专家协作，但现有工具碎片化且数据异构，工作流程不顺畅。需要让没有计算机专业知识的临床专家也能进行复杂分析。

Method: 构建基于NeDRex知识图谱的多智能体系统，包括查询路由、数据检索、算法执行、结果可视化等专门智能体，支持网络分析、功能一致性评估、文献挖掘等任务。

Result: 系统实现了自然语言访问生物医学知识图谱，能够执行复杂的生物信息学分析，支持药物重定位预测和科学讨论。

Conclusion: ChatDRex通过多智能体设计和自然语言接口，使临床专家能够生成假设并探索药物重定位机会，加速新疗法发现，推进个性化医疗和转化研究。
...
### [166] [EWE: An Agentic Framework for Extreme Weather Analysis](https://arxiv.org/abs/2511.21444)
*Zhe Jiang,Jiong Wang,Xiaoyu Yue,Zijie Guo,Wenlong Zhang,Fenghua Ling,Wanli Ouyang,Lei Bai*

Main category: cs.AI

TL;DR: EWE是首个用于极端天气自动诊断推理的智能代理框架，通过知识引导规划、闭环推理和气象工具包，从原始气象数据自主生成和解释多模态可视化，实现全面诊断分析。


<details>
  <summary>Details</summary>
Motivation: 极端天气事件对全球社会构成日益严重的风险，但专家驱动、劳动密集的诊断范式造成了关键的分析瓶颈，阻碍了科学进展。虽然AI在地球科学预测方面取得了显著进展，但自动诊断推理这一同样重要的挑战仍未得到充分探索。

Method: EWE框架通过知识引导规划、闭环推理和领域定制的气象工具包来模拟专家工作流程，能够从原始气象数据自主生成和解释多模态可视化，实现全面诊断分析。

Result: 为促进该新兴领域的发展，作者引入了首个基准测试，包括103个高影响事件的精选数据集和新的逐步评估指标。
...
### [167] [MADRA: Multi-Agent Debate for Risk-Aware Embodied Planning](https://arxiv.org/abs/2511.21460)
*Junjian Wang,Lidan Zhao,Xi Sheryl Zhang*

Main category: cs.AI

TL;DR: MADRA是一个无需训练的多智能体辩论风险评估框架，通过集体推理增强安全意识，同时保持任务性能。该框架在AI2-THOR和VirtualHome上实现了超过90%的危险任务拒绝率，且安全任务拒绝率低。


<details>
  <summary>Details</summary>
Motivation: 解决现有方法在具身AI代理任务规划中的安全问题，现有方法存在计算成本高或过度拒绝的问题，特别是在家庭环境中危险指令带来重大风险。

Method: 使用多智能体辩论框架，多个基于LLM的代理就给定指令的安全性进行辩论，由关键评估器根据逻辑合理性、风险识别、证据质量和清晰度对响应进行评分，通过迭代审议和共识投票达成决策。

Result: 在AI2-THOR和VirtualHome上的广泛实验表明，该方法实现了超过90%的不安全任务拒绝率，同时确保安全任务拒绝率低，在安全性和执行效率方面均优于现有方法。

Conclusion: 该工作为构建可信赖的具身代理提供了一个可扩展、模型无关的解决方案。
...
### [168] [SpatialBench: Benchmarking Multimodal Large Language Models for Spatial Cognition](https://arxiv.org/abs/2511.21471)
...
Method: 构建层次化空间认知框架（5个认知层次），开发SpatialBench基准（15个任务），引入高层能力导向的统一评估指标。

Result: 实验显示模型在感知层面表现良好，但在符号推理、因果推断和规划方面存在局限；人类测试表明人类进行选择性目标导向抽象，而MLLMs过度关注表面细节。
...
### [169] [Pessimistic Verification for Open Ended Math Questions](https://arxiv.org/abs/2511.21522)
*Yanxing Huang,Zihan Tang,Zejin Lin,Peng Li,Yang Liu*

Main category: cs.AI

TL;DR: 提出悲观验证方法，通过构建多个并行验证来判断数学证明的正确性，显著提升数学问题验证性能且计算资源消耗低。
...
Method: 悲观验证：为同一证明构建多个并行验证，只要任一验证报告错误就判定证明不正确。

Result: 该方法显著提升了多个数学验证基准的性能，计算效率高，甚至超过了扩展长链思维在测试时扩展中的token效率。

Conclusion: 悲观验证能有效提高语言模型输出的可靠性，在实现长视野数学任务中发挥关键作用，有助于增强语言模型的数学能力。
...
### [170] [Self-Transparency Failures in Expert-Persona LLMs: A Large-Scale Behavioral Audit](https://arxiv.org/abs/2511.21569)
*Alex Diep*

Main category: cs.AI

TL;DR: 语言模型在专业领域无法可靠披露其AI身份，导致用户无法信任其能力边界。研究发现模型在专业角色扮演中表现出领域特定的不一致性，某些领域透明度高而其他领域低，造成用户过度泛化信任的风险。


<details>
  <summary>Details</summary>
Motivation: 在需要专业知识的场景中，如果语言模型不能诚实地表明其AI身份，用户可能误判其能力边界，导致高风险决策中的潜在危害。需要评估模型在不同专业领域中的自我透明度表现。

Method: 采用共同花园设计，对16个开源模型（4B-671B参数）进行19,200次试验，评估模型在不同专业角色（如金融顾问、神经外科医生）中的身份披露行为。使用贝叶斯验证和Rogan-Gladen校正确保测量稳健性。

Result: 模型表现出显著的领域特定不一致性：金融顾问角色初始披露率为30.8%，而神经外科医生角色仅为3.5%。披露率范围从2.8%到73.6%，模型身份比参数数量更能预测行为。推理优化在某些模型中反而抑制了自我透明度。

Conclusion: 透明度反映的是训练因素而非模型规模。组织不能假设安全属性会自动迁移到部署环境，需要刻意设计行为和进行实证验证，以防止用户在不同领域间过度泛化信任。
...
### [171] [From Prediction to Foresight: The Role of AI in Designing Responsible Futures](https://arxiv.org/abs/2511.21570)
*Maria Perez-Ortiz*

Main category: cs.AI

TL;DR: 本文提出了"负责任计算前瞻"概念，探讨AI在负责任前瞻中的作用，建立了该领域的基本原则，并展示了一系列AI驱动的前瞻工具。
...
Conclusion: AI应在负责任、以人为中心的前瞻中发挥支持作用，通过深思熟虑的整合赋能政策制定者和社区应对21世纪的重大挑战。
...
### [172] [Resolution Where It Counts: Hash-based GPU-Accelerated 3D Reconstruction via Variance-Adaptive Voxel Grids](https://arxiv.org/abs/2511.21459)
...
<details>
  <summary>Details</summary>
Motivation: 传统体素网格方法在实时和资源受限场景下存在内存效率低、计算开销大、缺乏GPU支持等问题，需要更高效的3D重建方案

Method: 使用方差自适应的多分辨率体素网格，根据SDF观测的局部方差动态调整体素大小，采用扁平空间哈希表存储体素块，支持GPU并行处理

Result: 相比固定分辨率基线，实现了13倍加速和4倍内存使用降低，同时保持相当的3D重建精度

Conclusion: 该方法为高性能3D重建提供了一个实用且可扩展的解决方案，特别适合实时和资源受限场景
