...
Motivation: 探索多模态大语言模型在视觉任务中的潜力，特别是在有限数据条件下如何有效利用LLM进行精确视觉理解，以弥补传统CNN方法在数据效率方面的不足。

Method: 对传统CNN、零样本预训练多模态LLM和微调多模态LLM进行综合比较，特别关注LLM在少于1000张图像数据上的微调效果。

Result: LLM在少量数据微调后实现了高达36%的准确率提升，能够匹配或超越需要大量数据的CNN基线方法。

Conclusion: 多模态LLM方法在数据效率和适应性方面具有显著优势，为低资源视觉环境中的对象检测任务提供了有效解决方案，展示了跨模态学习策略的潜力。
...
Method: 在公开MRI数据集上使用U-Net架构，采用焦点损失函数并进行参数调优，评估三种数据增强技术：水平翻转、旋转和缩放。

Result: 使用焦点损失的U-Net模型达到了90%的精确度，与最先进结果相当，所有代码和结果均公开可用。

Conclusion: 本研究为脑肿瘤分割中的增强策略和损失函数设计建立了透明、可重复的基准，可指导未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型经常记忆和复制训练数据，引发隐私和版权担忧。现有方法延迟应用分类器自由引导(CFG)会导致生成的图像与输入提示对齐不佳，需要促进更早逃离记忆化吸引盆地。

Method: 基于初始噪声样本对逃离时间的影响，提出两种缓解策略：集体调整和个体调整初始噪声，以找到能促进更早逃离吸引盆地的初始样本。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注AI生成可视化内容的创建过程和质量，而忽视了表征偏见问题。随着AI图像工具日益普及，解决和减轻有害的性别偏见对于确保媒体和专业场景中的多样化表征至关重要。

Method: 研究提示生成了750多张职业相关的AI生成图像，使用DALL-E 3和Ideogram两个工具，并进行了主题分析来评估性别表征偏见。

Result: 研究发现DALL-E 3和Ideogram都在AI生成图像中强化了传统的性别刻板印象，尽管程度有所不同。AI可视化工具存在强化狭隘表征的风险。

Conclusion: 研究强调了解决AI生成图像中性别偏见的重要性，并为从业者、个人和研究人员提出了增加可见性别图像生成中多样性的建议。
...
Conclusion: 提出的动态MoE路由器有效解决了VAR的计算冗余问题，实现了计算效率与图像质量的良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法使用多视角扩散模型生成3D人体时面临视角不一致的问题，这阻碍了高质量的3D人体生成。

Method: Human-VDM包含三个模块：视角一致的人体视频扩散模块、视频增强模块和高斯溅射模块。首先从单张图像生成连贯的人体视频，然后通过超分辨率和视频插值增强纹理和几何平滑度，最后在高分辨率、视角一致的图像指导下学习逼真的3D人体。

Result: 实验表明Human-VDM能够从单张图像生成高质量的3D人体，在生成质量和数量上都优于现有最先进方法。

Conclusion: Human-VDM通过视频扩散模型解决了视角一致性问题，实现了从单张RGB图像生成逼真3D人体的目标。
...
<details>
  <summary>Details</summary>
Motivation: 现有无监督OOD检测方法通常依赖预测熵，但这种方法混淆了认知不确定性和随机不确定性，导致将分布内模糊区域误分类为OOD。需要一种能够更好分离不确定性的方法。

Method: 使用分层贝叶斯建模对深度神经网络特征空间中的高斯混合模型参数进行建模，从而提取纯粹的认知不确定性，无需辅助数据或额外训练阶段。

Result: 在SemanticKITTI数据集上，相比预测熵方法，AUROC提升18%，AUPRC提升22%，FPR95从76%降低到40%（减少36%）。

Conclusion: 该方法有效解决了认知和随机不确定性的混淆问题，在无监督OOD检测任务中表现出色，无需额外数据或训练即可显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 人类活动识别中存在训练数据中活动范围与真实生活中活动多样性之间的巨大差距，且活动类别之间存在层次关系，需要处理未知活动和类别间相似性问题。

Method: 将活动类别组织成结构化层次，开发Hi-OSCAR层次化开放集分类器，能够识别已知活动并拒绝未知活动，同时将未知类别定位到最近的内部节点。

Result: 创建了新的公开数据集NFI_FARED，包含19个不同情境下的活动数据，支持开放集HAR研究。

Conclusion: Hi-OSCAR不仅实现了开放集分类，还提供了层次化的未知活动定位能力，显著提升了HAR分类器的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 高频振荡是识别致痫区的新生物标志物，但视觉识别HFOs耗时费力且主观，需要开发自动化检测方法用于研究和临床。

Method: 使用时频域S变换提取事件，采用无监督聚类技术对事件进行分类，区分HFOs与尖波、背景活动和伪影。

Result: 在控制数据集上敏感度97.67%、精确度98.57%、F分数97.78%；在癫痫患者中，切除与非切除接触点的HFOs比率与手术结果相关性更强(0.73)。

Conclusion: HFOs是癫痫患者致痫性的有前景生物标志物，切除HFOs（特别是快波纹）可导致无癫痫发作，而残留HFOs则导致癫痫复发。
...
<details>
  <summary>Details</summary>
Motivation: 理解DINOv2模型感知的本质，探索其内部表示的组织方式，超越传统的线性表示假说。

Method: 采用稀疏自编码器(SAE)构建32,000个单元的字典，分析下游任务如何利用这些概念，研究表示几何和统计特性。

Result: 发现功能专业化现象：分类使用"他处"概念实现否定逻辑，分割依赖边界检测器，深度估计使用三种单眼深度线索。表示部分密集而非严格稀疏，具有局部连接的低维结构。

Conclusion: 提出Minkowski表示假说，认为标记是通过凸混合原型组合形成的，这种结构基于概念空间理论，为理解视觉Transformer表示提供了新视角。
...
Conclusion: PhyDAE通过显式建模退化物理和自适应专家机制，有效解决了遥感图像中的异构退化问题，在保持高性能的同时大幅提升了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 解决真实世界临床决策中整合多种数据模态（医学文本、2D/3D图像、视频）的挑战，克服通用视觉语言模型在医学领域开发中的不透明流程、数据稀缺和架构不灵活等问题。

Method: 基于统一的分块视觉编码器和LLM解码器构建，通过渐进式训练在1670万个样本上从2D扩展到3D和视频理解，采用医学感知的token减少技术实现高效训练。
...
<details>
  <summary>Details</summary>
Motivation: 解决相机中心的理解和生成任务通常被孤立研究的问题，需要开发一个统一的模型来整合这两种能力。
...
Conclusion: Puffin通过统一的相机中心方法推进了多模态空间智能研究，将发布代码、模型、数据集和基准测试以促进该领域发展。
...
<details>
  <summary>Details</summary>
Motivation: 传统迁移学习冻结预训练网络权重并添加任务特定层，这种方法计算效率高但限制了模型适应领域特定特征的能力，在数据极少时仍可能导致过拟合。

Method: 提出SOR框架，冻结网络内部结构（如卷积滤波器），同时使用组套索和L1惩罚的组合，以最少的额外参数使模型适应特定数据。

Result: 在三个少样本医疗图像分类任务上评估SOR，使用DenseNet121和EfficientNetB4基础网络，相比现有基准取得了竞争性结果。
...
Conclusion: BEAR基准填补了MLLM具身能力评估的空白，BEAR-Agent能有效提升MLLM的具身能力，且这种改进对模拟环境中的具身任务也有益。
...
<details>
  <summary>Details</summary>
Motivation: 传统防御方法依赖计算密集型优化，而人类视觉系统通过进化机制天然具备对抗扰动的鲁棒性，假设注意力引导的非均匀稀疏采样和预测编码是关键因素。

Method: 结合三种生物机制：中央-外周处理、扫视眼动、皮层填充。使用强化学习引导扫视选择性捕获多个中央-外周区域，整合重构图像后分类。

Result: 在ImageNet数据集上实验表明，该方法能提高系统对各种分类器和攻击类型的鲁棒性，同时显著降低训练开销。
...
<details>
  <summary>Details</summary>
Motivation: 开发能够在各种环境条件下快速准确检测泄漏的安全关键系统，利用热成像技术在速度和鲁棒性方面的优势。

Method: 使用平衡的二元数据集（4000张图像），采用VGG19和NasNetMobile等轻量级模型，结合RGB和热成像进行泄漏分类。

Result: 热成像模型在推理速度、准确性和模型大小方面表现更优，VGG19热成像模型达到最佳性能，模型大小小于350MB，推理时间低至44ms。

Conclusion: 基于热成像的VGG19模型在泄漏检测中表现最佳，系统在消费级硬件上具有高部署性，适用于安全关键场景。
...
<details>
  <summary>Details</summary>
Motivation: 生成式图像超分辨率模型依赖自注意力机制，但其二次复杂度造成计算瓶颈。线性注意力提供线性复杂度解决方案，但在超真实感超分辨率中面临训练不稳定、感知-失真权衡等未解决挑战。

Method: 1. 基于"拐点"的早期停止引导微调策略解决训练不稳定问题；2. 基于SNR的专家混合架构缓解感知-失真权衡；3. 轻量级TAG指导范式实现"精度优先于数量"原则。

Result: LinearSR在核心扩散前向传播中达到最先进的速度，整体多步推理时间保持高度竞争力，同时提供最先进的感知质量。

Conclusion: 该工作为在超真实感超分辨率领域应用线性注意力提供了首个稳健方法，为高效生成式超分辨率的未来研究建立了基础范式。
...
Conclusion: 这种基于人工智能和计算机视觉的非侵入式方法为野生动物监测提供了有价值的替代方案，有助于生态学和保护生物学的新方法发展。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频质量评估模型存在泛化能力差、可解释性有限、难以扩展到新用例或内容类型的问题，需要一种更灵活、鲁棒的解决方案。

Method: 构建多层级路由系统，集成多种专家模型，使用视觉语言模型作为实时路由器，根据输入视频语义动态推理并组合最合适的专家模型。

Result: Q-Router在多个基准测试中达到或超越最先进的VQA模型，显著提高了泛化性和可解释性，在Q-Bench-Video基准上表现优异，并能定位时空伪影。

Conclusion: Q-Router展示了作为下一代VQA系统基础的潜力，并有望作为视频生成模型后训练阶段的奖励函数。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散的视频超分辨率方法计算成本高且速度慢，限制了其在高分辨率和长视频中的应用。研究发现视频中许多区域本身细节较少，从细化中获益有限，但现有方法对所有像素进行统一处理。

Method: 提出SkipSR框架，直接从低分辨率输入中识别低细节区域，完全跳过这些区域的计算，仅对需要细化的区域进行超分辨率处理。

Result: 在标准SR基准测试中，该方法在720p视频上比现有模型实现高达60%的端到端延迟减少，且没有可感知的质量损失。

Conclusion: SkipSR是一种简单有效的策略，可在保持感知质量的同时显著减少视频超分辨率的计算量，适用于标准和单步扩散SR模型。
...
Method: 设计教师模型提取高质量实例嵌入，通过标签引导蒸馏算法将开放词汇知识蒸馏到3D学生模型中。教师模型生成包含可见性和视角多样性的2D CLIP嵌入，学生模型直接生成3D嵌入。

Result: 在ScanNet200数据集上达到35.7的AP50分数，在Replica数据集上表现优异，推理速度比先前方法快6.0倍到152.2倍。
...
<details>
  <summary>Details</summary>
Motivation: 蔓越莓种植中的变化监测通常需要人工完成，耗时且效率低。深度学习虽有潜力，但面临高维特征难以解释和需要大量手动标注的问题。

Method: 使用自监督方法微调视觉变换器，通过时间回归和类别预测的双重预训练任务，学习植物和果实外观随时间变化的潜在空间表示。

Result: 生成的2D时间轨迹提供了可解释的作物生长时间序列模型，能够预测生长趋势并区分不同蔓越莓品种的时间差异。

Conclusion: 该方法通用性强，可应用于其他作物和场景，并提供了一个包含8个蔓越莓品种、52次观测的新时间序列数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有的视觉语言模型难以同时表达概念族内的层次结构（如狗⊆哺乳动物⊆动物）和跨概念族的组合性（如"车里的狗"⊆狗、车），双曲空间虽然能有效捕捉树状层次结构，但对组合性的表示能力尚不明确。
...
Method: 将输入样本划分为多个局部区域，重新映射语义信息生成增强样本，用这些增强样本替代原始样本进行扰动优化，仅保留局部语义信息而非全局信息。
...
<details>
  <summary>Details</summary>
Motivation: 知识蒸馏攻击对深度模型知识产权构成严重威胁。虽然图像分类领域已有成功的防御方法，但将这些方法扩展到图像恢复任务很困难，因为恢复任务是生成式任务，具有连续高维输出，依赖空间一致性和细节，微小扰动往往不足以阻止学生模型学习。

Method: 提出自适应奇异值扰动(ASVP)方法，在运行时对教师模型的内部特征图进行奇异值分解，放大前k个奇异值来注入结构化高频扰动，破坏蒸馏所需的对齐关系，同时保持教师模型的输出质量。

Result: 在五个图像恢复任务（超分辨率、低光增强、水下增强、去雾、去雨）上的实验表明，ASVP可将学生模型的PSNR降低高达4dB，SSIM降低60-75%，而对教师模型的性能影响可以忽略不计。

Conclusion: ASVP为保护开源图像恢复模型免受未经授权的知识蒸馏攻击提供了一个实用且有效的解决方案，相比现有方法提供更强更一致的防御效果。
...
Method: 创建Ro-Bench基准测试，通过编辑视频的风格、对象、背景及其组合来生成高质量、多样化和时间相关的反事实视频数据。

Result: 评估了8个最新的视频MLLMs，发现当前模型在反事实视频内容上性能显著下降。通过反事实数据微调，Ro-Bench性能提升21.73%，MVBench数据集20个任务性能提升12.78%。
...
<details>
  <summary>Details</summary>
Motivation: 现代农业监测系统依赖多种数据源，但无人机动物健康监测面临数据稀缺问题，包括动物体型小、遮挡、部分可见等场景特定问题。迁移学习方法因缺乏反映特定农场条件的大数据集而效果有限。

Method: 开发了物体聚焦数据增强框架，分割动物与背景，通过变换和基于扩散的合成技术创建真实多样的场景，专门针对动物健康监测的数据约束问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有推理时缩放技术主要提升LVLMs的推理能力，但对视觉感知的影响尚不明确。当前LVLMs采用快速感知范式，将视觉理解视为一次性输出，缺乏对底层感知过程的建模。

Method: 提出Perception-Time Scaling (PTS)范式，鼓励丰富token感知，将复杂感知问题分解为中间可处理子问题；结合强化学习技术实现感知与推理时缩放的协同。

Result: PTS显著提升感知精度，在DisTANCE基准上高精度性能从8.0%提升至64.7%；即使使用纯合成数据，结合数学推理数据也能在推理和真实世界感知基准上获得一致增益。

Conclusion: PTS通过引入更多感知相关token和增加对图像token的关注，使感知能够与推理时缩放对齐并从中受益，为LVLMs的视觉感知能力提供了有效解决方案。
...
Method: 引入mmJoints框架，为黑盒毫米波3D姿态估计器输出增强额外的关节描述符，显式估计关节被感知的可能性和位置预测可靠性。

Result: 在13种姿态估计设置中超过115,000个信号帧的评估显示，mmJoints描述符估计错误率低于4.2%，关节位置精度提升达12.5%，活动识别性能提升达16%。

Conclusion: mmJoints通过显式建模姿态估计中的偏差，有效提高了姿态估计的准确性和下游任务的性能。
...
<details>
  <summary>Details</summary>
Motivation: 传统检索方法精度有限，现有多向量检索方法存在图像对象对齐不佳和细粒度图像片段冗余的问题

Method: 采用分层范式使用多个中间粒度处理不同图像对象以增强对齐，利用跨层次相似性一致性和层次稀疏性减少冗余匹配计算，自动为每个数据集配置参数

Result: HiMIR在精度上有显著提升，同时计算量比现有MVR系统减少最多3.5倍

Conclusion: HiMIR框架在提高检索精度的同时大幅提升了效率，适用于多样化场景
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到图像模型在复杂局部区域（特别是人手）生成准确细节方面仍存在困难，生成的手部经常出现结构扭曲和不真实纹理，但手部区域质量评估一直被忽视，限制了人中心生成质量优化和AIGC检测等下游任务性能。

Method: 1) 构建HandPair数据集，包含48k张高质量和低质量手部配对图像，无需人工标注；2) 开发HandEval模型，利用多模态大语言模型的视觉理解能力，并结合手部关键点先验知识；3) 构建人工标注测试集验证模型性能。

Result: HandEval在质量评估方面比现有最先进方法更符合人类判断。将其集成到图像生成和AIGC检测流程中，分别显著提升了生成手部的真实性和检测准确率，证实了在下游应用中的普适有效性。

Conclusion: 该研究填补了生成图像手部质量评估的空白，提出的HandEval模型和HandPair数据集为提升文本到图像模型的手部生成质量和相关下游任务性能提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: AI着色技术存在版权侵权风险，如未经授权对黑白漫画和电影进行着色并转售，但目前缺乏有效的防御方法。

Method: 提出PAChroma方法，通过拉普拉斯滤波器优化不可察觉的扰动以保持感知质量，并在优化过程中应用多样化输入变换来增强跨模型迁移性和对后处理的鲁棒性。

Result: 在ImageNet和Danbooru数据集上的实验表明，PAChroma能有效降低着色质量同时保持视觉外观。

Conclusion: 这是保护视觉内容免受非法AI着色的首个防御方法，为生成媒体中的版权感知防御开辟了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 自回归文本到图像模型由于顺序token解码过程导致推理速度慢，需要数千次前向传播生成单张图像，存在效率问题

Method: 引入next-clean-token预测范式，通过低成本微调使预训练模型能接受噪声扰动token嵌入并预测下一个干净token；在推理时用高斯噪声初始化token序列，在嵌入空间进行迭代预测，使用概率标准并行验证和接受多个token

Result: 实验表明该方法能通过减少模型前向传递次数来加速生成，同时保持生成图像的视觉质量

Conclusion: SJD2框架成功解决了自回归模型推理效率低的问题，实现了并行token生成，在保持质量的同时显著提升生成速度
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常显式组合多模态信息，本文旨在将多模态知识嵌入到单模态网络中，提升单模态网络的性能

Method: 为每个模态分配单独网络，通过时空语义对齐损失(SSA)和焦点正则化参数来强制网络协作学习共同语义和更好表示

Result: 实验结果表明该方法提高了单模态网络的测试时识别准确率，并在多个动态手势识别数据集上达到最先进性能

Conclusion: 提出的框架能够有效利用多模态知识提升单模态网络的性能，避免负知识传递，在动态手势识别任务中表现优异
...
Conclusion: 通过修改视觉编码器来识别和掩码不确定视觉token，可以有效缓解大型视觉语言模型中的物体幻觉问题。
...
<details>
  <summary>Details</summary>
Motivation: 改进传统稀疏表示分类方法，利用深度学习学习更鲁棒的特征表示，提高分类性能

Method: 构建包含卷积自编码器和中间全连接层的网络架构，自编码器学习深度特征，全连接层寻找稀疏表示，最终基于稀疏码进行分类

Result: 在三个不同数据集上的实验表明，该方法产生的稀疏表示比现有最先进的SRC方法获得更好的分类结果

Conclusion: 提出的深度稀疏表示分类网络能够学习更有效的稀疏表示，显著提升分类性能
...
Method: 1) 基于token分布空间熵的动态温度控制，提升内容多样性、对齐精度和结构一致性；2) 推测解码中的熵感知接受规则，实现接近无损的生成。

Result: 在多个基准测试中验证了方法的有效性，能够在约85%的推理成本下实现接近无损的生成质量提升。

Conclusion: 该方法能够显著提升自回归图像生成的质效，具有很好的通用性和实用性。
...
<details>
  <summary>Details</summary>
Motivation: LiDAR标注通常存在噪声，这些噪声在域偏移下会进一步放大，威胁系统可靠性。现有噪声标签学习方法在3D LiDAR分割中的域泛化应用尚未充分探索。

Method: 提出了DuNe双视图框架，包含强分支和弱分支，强制特征级一致性，并基于置信度感知过滤应用交叉熵损失。

Result: 在10%对称标签噪声下，在SemanticKITTI上达到56.86% mIoU，nuScenes上42.28%，SemanticPOSS上52.58%，算术平均49.57%，调和平均48.50%。

Conclusion: DuNe框架在DGLSS-NL任务中展示了强大的域泛化能力，为噪声标签下的LiDAR语义分割提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 潜在扩散模型在医学图像转换中效率高但可能损失关键像素级细节，特别是对诊断至关重要的病灶区域，这会影响临床决策的可靠性。

Method: 在预训练的潜在扩散模型基础上，加入病灶感知的医学像素空间目标进行后训练，专注于提升病灶区域的精确重建。

Result: 在817名急性缺血性卒中患者的脑CT到MRI转换任务中，该方法在合成DWI和ADC图像时显著提升了整体图像质量和病灶描绘精度，优于现有图像转换模型。

Conclusion: 该后训练策略易于适配预训练模型，在医学图像转换任务中具有广泛应用潜力，能有效提升临床诊断的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 柔性微电极植入到大脑皮层具有挑战性，因为FME探针的可变形纤维状结构与关键生物组织的相互作用。为确保可靠性和安全性，需要对植入过程进行仔细监控。

Method: 利用机器人FME植入系统的显微相机，在四个检查点使用统一的异常检测框架。基于现有目标定位结果提取对齐的感兴趣区域，输入预训练的ViT模型。提出渐进粒度补丁特征采样方法解决不同位置的敏感度-容错度权衡问题，并选择信噪比更高的特征通道为特定场景提供更好的描述符。

Result: 所提方法在植入系统收集的图像数据集上验证了有效性。

Conclusion: 提出的基于ViT的图像异常检测框架能够有效监控柔性微电极植入过程，确保植入的安全性和可靠性。
...
Method: 提出注意力驱动的层次特征融合(AHFF)方案自适应融合多尺度点云补丁特征，增强局部邻域的几何上下文学习；在此基础上提出补丁级状态空间模型(PSSM)，将点云补丁建模为隐式超曲面，通过状态动态实现细粒度几何理解。

Result: 在基准数据集上的大量实验表明，该方法在精度、鲁棒性和灵活性方面优于现有方法。消融研究进一步验证了所提出组件的贡献。

Conclusion: MambaH-Fit通过创新的特征融合和状态空间建模方法，成功解决了点云法向量估计中细粒度几何结构建模的挑战，取得了显著的性能提升。
...
<details>
  <summary>Details</summary>
Motivation: 无人机在军事侦察、环境监测等领域的广泛应用对多目标跟踪技术提出了迫切需求，但复杂背景、小尺度目标、频繁遮挡等问题仍挑战现有方法的检测精度和轨迹连续性。
...
Conclusion: 该方法为无人机检测和跟踪技术的发展提供了有力支持，有效解决了复杂场景下的多目标跟踪挑战。
...
### [45] [A Novel Multi-branch ConvNeXt Architecture for Identifying Subtle Pathological Features in CT Scans](https://arxiv.org/abs/2510.09107)
*Irash Perera,Uthayasanker Thayasivam*

Main category: cs.CV

TL;DR: 提出了一种用于医学图像分析的多分支ConvNeXt架构，在COVID-19 CT扫描诊断中取得优异性能，ROC-AUC达0.9937，验证准确率97.57%，F1分数0.9825。


<details>
  <summary>Details</summary>
Motivation: 医学影像智能分析在临床诊断中至关重要，特别是识别细微病理特征。针对医学图像分析的复杂挑战，需要开发专门的有效方法。

Method: 采用多分支ConvNeXt架构，集成全局平均池化、全局最大池化和新的注意力加权池化三个并行分支。包含端到端流程：数据预处理、增强和两阶段训练策略，有效利用迁移学习。
...
### [46] [SOS: Synthetic Object Segments Improve Detection, Segmentation, and Grounding](https://arxiv.org/abs/2510.09110)
*Weikai Huang,Jieyu Zhang,Taoyang Jia,Chenhao Zheng,Ziqi Gao,Jae Sung Park,Ranjay Krishna*

Main category: cs.CV

TL;DR: SOS是一个基于物体中心组合策略的简单可扩展数据合成流水线，通过将高质量合成物体片段粘贴到新图像中，生成准确多样的掩码、边界框和指代表达式，在检测和定位任务上超越大型真实图像数据集。


<details>
  <summary>Details</summary>
Motivation: 视觉分组任务需要大量标注数据，但真实数据标注成本高、覆盖偏差大且难以扩展。合成数据虽然前景广阔，但通常缺乏灵活性、准确性和组合多样性。

Method: 使用物体中心组合策略，基于结构化布局先验和生成式重光照，将高质量合成物体片段粘贴到新图像中。

Result: 在100,000张合成图像上训练的模型在检测和定位任务上优于在更大真实图像数据集（如GRIT和V3Det）上训练的模型，在LVIS检测上提升+10.9 AP，在gRefCOCO定位上提升+8.4 N_Acc。

Conclusion: SOS支持可控数据集构建，在低数据和封闭词汇设置下都能改善泛化能力，增强真实数据集可获得跨数据尺度的强大性能，在极有限真实数据下表现更佳。
...
### [47] [MSDM: Generating Task-Specific Pathology Images with a Multimodal Conditioned Diffusion Model for Cell and Nuclei Segmentation](https://arxiv.org/abs/2510.09121)
*Dominik Winter,Mai Bui,Monica Azqueta Gavaldon,Nicolas Triltsch,Marco Rosati,Nicolas Brieu*

Main category: cs.CV

TL;DR: 提出多模态语义扩散模型MSDM，通过整合细胞核形态、颜色特征和元数据来生成逼真的图像-掩码对，用于增强细胞和细胞核分割数据集的多样性，显著提升分割模型的准确性。
...
Method: 使用多模态语义扩散模型MSDM，通过水平/垂直形态图、RGB颜色特征和BERT编码的元数据作为条件输入，利用多头交叉注意力机制整合多模态信息，生成像素级精确的图像-掩码对。

Result: 合成图像与真实数据高度匹配，在匹配生物条件下生成图像与真实图像的嵌入向量Wasserstein距离较低。合成样本显著提高了分割模型在柱状细胞上的准确性。

Conclusion: 多模态扩散增强策略能有效提升细胞和细胞核分割模型的鲁棒性和泛化能力，为生成模型在计算病理学中的广泛应用铺平了道路。
...
### [48] [Polar Separable Transform for Efficient Orthogonal Rotation-Invariant Image Representation](https://arxiv.org/abs/2510.09125)
*Satya P. Singh,Rashmi Chaudhry,Anand Srivastava,Jagath C. Rajapakse*

Main category: cs.CV

TL;DR: 提出了PSepT（极坐标可分离变换），一种可分离的正交变换，解决了极坐标中的不可分离性问题，显著降低了计算复杂度和内存需求，提高了数值稳定性。
...
Conclusion: PSepT的可分离框架使得之前使用经典方法无法实现的高阶矩分析成为可能，为鲁棒的图像分析应用开辟了新的可能性。
...
### [49] [Training Feature Attribution for Vision Models](https://arxiv.org/abs/2510.09135)
*Aziz Bacha,Thomas George*

Main category: cs.CV

TL;DR: 提出训练特征归因方法，将测试预测与特定训练图像的特定区域关联，提供对深度模型内部工作的新见解


<details>
  <summary>Details</summary>
Motivation: 深度神经网络通常被视为不透明系统，需要可解释性方法来提高信任和问责。现有方法通常将测试时预测归因于输入特征或影响训练样本，我们认为这两种视角应该联合研究

Method: 探索训练特征归因方法，将测试预测与特定训练图像的特定区域关联起来

Result: 在视觉数据集上的实验表明，训练特征归因产生细粒度的、测试特定的解释：识别导致错误分类的有害示例，并揭示传统归因方法未能暴露的虚假相关性（如基于补丁的捷径）

Conclusion: 训练特征归因方法能够提供比传统方法更深入的模型解释，揭示模型决策的底层机制
...
### [50] [Online Topological Localization for Navigation Assistance in Bronchoscopy](https://arxiv.org/abs/2510.09144)
*Clara Tomasini,Luis Riazuelo,Ana C. Murillo*

Main category: cs.CV

TL;DR: 提出了一种基于图像的支气管镜拓扑定位方法，无需患者CT扫描，仅在模拟数据上训练即可为手术提供导航辅助。


<details>
  <summary>Details</summary>
Motivation: 传统支气管镜导航方法依赖患者CT扫描和额外传感器，需要额外设置、扫描和培训。准确度量定位并非总是必要，拓扑定位通常足以辅助外科医生导航。

Method: 开发了基于图像的支气管镜拓扑定位流程，仅使用模拟数据进行训练，无需真实患者CT扫描，降低了数据标注成本。

Result: 该方法在真实数据测试序列上表现优异，超越了现有方法，具有良好的泛化能力。

Conclusion: 该图像支气管镜拓扑定位方法提供了一种无需CT扫描的有效导航辅助方案，在模拟数据训练下具有良好的实际应用效果。
...
### [51] [Instance-Level Generation for Representation Learning](https://arxiv.org/abs/2510.09171)
*Yankun Wu,Zakaria Laskar,Giorgos Kordopatis-Zilos,Noa Garcia,Giorgos Tolias*

Main category: cs.CV

TL;DR: 提出了一种无需真实图像即可合成多样化对象实例的新方法，通过生成大规模训练数据来改进实例级识别性能。


<details>
  <summary>Details</summary>
Motivation: 实例级识别需要精细标注，但大规模标注数据集难以获取，限制了其实际应用。

Method: 从多个领域合成不同条件和背景下的对象实例，构建大规模训练集，并在基础视觉模型上进行微调。

Result: 在七个跨领域的ILR基准测试中显著提升了检索性能。

Conclusion: 该方法为实例级识别提供了高效的数据生成替代方案，仅需目标领域名称即可应用，具有广泛的实际应用前景。
...
### [52] [TARO: Toward Semantically Rich Open-World Object Detection](https://arxiv.org/abs/2510.09173)
*Yuchen Zhang,Yao Lu,Johannes Betz*

Main category: cs.CV

TL;DR: TARO是一个新颖的开放集检测框架，不仅能识别未知物体，还能将其分类到语义层次结构中的粗粒度父类别，相比传统方法将未知物体统一标记为'Unknown'提供更有用的信息。


<details>
  <summary>Details</summary>
Motivation: 现代物体检测器局限于'封闭世界'假设，遇到未知物体时存在风险。传统开放集检测方法将所有未知物体标记为单一'Unknown'类别不够实用，在安全关键场景中需要更细粒度的分类信息。
...
Conclusion: TARO框架成功解决了开放集检测中未知物体分类粒度不足的问题，为安全关键应用提供了更实用的检测能力。
...
### [53] [Online Video Depth Anything: Temporally-Consistent Depth Prediction with Low Memory Consumption](https://arxiv.org/abs/2510.09182)
*Johann-Friedrich Feiden,Tim Küchler,Denis Zavadski,Bogdan Savchynskyy,Carsten Rother*

Main category: cs.CV

TL;DR: 提出在线视频深度估计方法oVDA，通过借鉴LLM的缓存和掩码技术，实现实时处理并降低VRAM使用


<details>
  <summary>Details</summary>
Motivation: 现有Video Depth Anything方法依赖批处理，无法用于在线场景，需要开发能实时处理的视频深度估计方法

Method: 采用大语言模型技术：推理时缓存潜在特征，训练时使用帧掩码技术

Result: oVDA在准确率和VRAM使用上均优于所有竞争方法，在A100上达42FPS，Jetson边缘设备上达20FPS

Conclusion: oVDA成功实现了在线视频深度估计，特别适合边缘设备部署，将发布代码和编译脚本
...
### [54] [Modern Deep Learning Approaches for Cricket Shot Classification: A Comprehensive Baseline Study](https://arxiv.org/abs/2510.09187)
*Sungwoo Kang*

Main category: cs.CV

TL;DR: 本文首次对板球击球动作分类进行了全面的基准研究，比较了7种深度学习方法在4个研究范式下的表现，发现学术文献中的性能声明与实际实现结果存在显著差距。
...
Result: 重新实现先前文献中的方法（Balaji LRCN、IJERCSE、Sensors）分别仅获得46.0%、55.6%和57.7%的准确率，远低于文献声称的96%、99.2%和93%。现代SOTA方法（EfficientNet-B0 + GRU）达到92.25%准确率。
...
### [55] [Towards Safer and Understandable Driver Intention Prediction](https://arxiv.org/abs/2510.09200)
...
Result: transformer模型比传统CNN模型具有更好的可解释性，并开发了多标签t-SNE可视化技术展示解释之间的解缠和因果关联。

Conclusion: VCBM框架和DAAD-X数据集为自动驾驶中的可解释驾驶员意图预测提供了有效解决方案，transformer架构在可解释性方面优于CNN。
...
### [56] [Cattle-CLIP: A Multimodal Framework for Cattle Behaviour Recognition](https://arxiv.org/abs/2510.09203)
...
<details>
  <summary>Details</summary>
Motivation: 牛只行为是动物健康和生产力的重要指标，现有视频监测方法在数据稀缺的行为识别任务中存在性能不足的问题，需要开发能够有效利用语义信息的多模态方法。

Method: 基于CLIP大规模图像-语言模型，添加时间整合模块，采用定制化数据增强策略和专门设计的文本提示，以弥合预训练模型与真实牛只监控视频之间的领域差距。

Result: 在CattleBehaviours6数据集上，监督学习场景下达到96.1%的整体准确率，其中进食、饮水和站立反刍行为的召回率接近100%，在少样本场景中表现出稳健的泛化性能。
...
### [57] [3D Reconstruction from Transient Measurements with Time-Resolved Transformer](https://arxiv.org/abs/2510.09205)
...
Method: TRT架构包含两个精心设计的注意力机制：时空自注意力编码器通过将输入特征分割或下采样到不同尺度来探索瞬态数据中的局部和全局相关性；时空交叉注意力解码器在令牌空间中整合局部和全局特征，产生具有高表示能力的深度特征。

Result: 在合成数据和不同成像系统捕获的真实数据上，TRT-LOS和TRT-NLOS两种实现都显著优于现有方法。

Conclusion: TRT架构有效提升了光子效率成像中的3D重建性能，同时贡献了大规模合成LOS数据集和真实NLOS测量数据，增强了该领域的数据多样性。
...
### [58] [Stable Video Infinity: Infinite-Length Video Generation with Error Recycling](https://arxiv.org/abs/2510.09212)
*Wuyang Li,Wentao Pan,Po-Chien Luan,Yang Gao,Alexandre Alahi*

Main category: cs.CV

TL;DR: 提出Stable Video Infinity (SVI)方法，能够生成无限长度视频，具有高时间一致性、合理的场景转换和可控的流式故事情节。通过错误回收微调技术，让模型主动识别和纠正自身错误。


<details>
  <summary>Details</summary>
Motivation: 现有长视频生成方法局限于单提示外推，产生同质化场景和重复运动。根本挑战在于训练假设（看到干净数据）与测试时自回归现实（基于自生成、易出错输出）之间的关键差异。

Method: 采用错误回收微调技术，通过闭环回收将Diffusion Transformer自生成的错误转化为监督提示。具体包括：注入历史错误干预干净输入、用一步双向积分高效近似预测、动态将错误存入重放记忆库。

Result: SVI能够将视频从秒级扩展到无限时长，无需额外推理成本，同时兼容多种条件（音频、骨架、文本流）。在三个基准测试中验证了其多功能性和最先进性能。

Conclusion: SVI通过错误回收微调成功解决了长视频生成中的假设差距问题，实现了无限时长视频生成，在一致性、创造性和条件设置方面均表现出色。
...
### [59] [Tag-Enriched Multi-Attention with Large Language Models for Cross-Domain Sequential Recommendation](https://arxiv.org/abs/2510.09224)
...
<details>
  <summary>Details</summary>
Motivation: 跨域序列推荐在消费电子和电商平台中至关重要，需要准确捕捉域内和跨域行为模式来提供个性化体验。

Method: 使用LLM生成领域感知的语义标签，将标签嵌入与物品标识符、文本和视觉特征融合构建增强的物品表示，引入标签增强的多注意力机制联合建模域内和跨域用户偏好。

Result: 在四个大规模电商数据集上的实验表明，TEMA-LLM持续优于最先进的基线方法。

Conclusion: LLM驱动的语义标签和多注意力集成对面向消费者的推荐系统有益，展示了LLM在推进智能用户中心服务方面的潜力。
...
### [60] [Clear Roads, Clear Vision: Advancements in Multi-Weather Restoration for Smart Transportation](https://arxiv.org/abs/2510.09228)
...
TL;DR: 本文对图像和视频修复技术进行了全面综述，重点针对雾霾、雨雪等恶劣天气条件下的视觉退化问题，涵盖了传统方法和现代数据驱动模型，并讨论了智能交通系统中的实际应用挑战。


<details>
  <summary>Details</summary>
Motivation: 恶劣天气条件会显著降低图像和视频质量，对依赖视觉输入的智能交通系统（如自动驾驶、交通监控等）造成严重影响，需要有效的修复技术来应对这些视觉退化问题。

Method: 将现有方法分为传统基于先验的方法和现代数据驱动模型（包括CNN、Transformer、扩散模型和视觉语言模型），并根据处理范围分为单任务模型、多任务/多天气系统和全能框架。

Result: 提供了天气诱导视觉损伤修复技术的系统分类和综述，涵盖了日间和夜间修复挑战、基准数据集和评估协议，为开发天气弹性视觉系统提供了参考。

Conclusion: 当前研究存在局限性，未来方向包括混合/复合退化修复、实时部署和代理AI框架，该调查旨在推动智能交通环境中天气弹性视觉系统的发展。
...
### [61] [Diagnosing Shoulder Disorders Using Multimodal Large Language Models and Consumer-Grade Cameras](https://arxiv.org/abs/2510.09230)
...
TL;DR: 提出HMVDx框架，使用多模态大语言模型进行肩部疾病的初步诊断，通过分离动作理解和疾病诊断任务，显著提升诊断准确率79.6%。
...
Method: 提出混合运动视频诊断框架(HMVDx)，使用两个MLLM分别完成动作理解和疾病诊断任务，并引入基于医疗决策逻辑的可用性指数新指标。

Result: HMVDx在肩关节损伤诊断中的准确率相比直接视频诊断提高了79.6%。

Conclusion: 该研究展示了低成本MLLM在医疗应用中的潜力，为未来MLLM在医学视频理解领域的研究提供了重要技术贡献。
...
### [62] [D-CoDe: Scaling Image-Pretrained VLMs to Video via Dynamic Compression and Question Decomposition](https://arxiv.org/abs/2510.08818)
...
<details>
  <summary>Details</summary>
Motivation: 将图像预训练的视觉语言模型适配到视频领域面临挑战，主要问题是处理密集且时间延长的视觉输入超出了图像模型的能力，存在感知瓶颈和令牌过载问题。

Method: 提出D-CoDe框架，包含动态压缩（自适应选择代表性帧和内容感知的空间令牌聚合）和问题分解（将原始查询重构为子问题），两者都无需训练。

Result: 实验表明D-CoDe在各种基准测试中有效提升了视频理解能力，在具有挑战性的长视频基准上表现出色。

Conclusion: D-CoDe框架通过动态压缩和问题分解成功解决了视频适配中的关键挑战，展示了处理复杂视频语言任务的潜力。
...
TL;DR: 本文建立了图像隐私分类的零样本基准，评估了开源视觉语言模型(VLMs)与专用模型的性能对比，发现尽管VLMs资源密集，但在隐私预测准确性上落后于更小的专用模型，但具有更高的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 当前文献倾向于采用通用任务的视觉语言模型(VLMs)，但缺乏系统评估，可能忽略了专用模型的性能上限。

Method: 建立零样本基准，使用任务对齐提示评估前3名开源VLMs，并与专用视觉和多模态方法对比性能、效率和鲁棒性。

Result: VLMs在隐私预测准确性上落后于专用小模型，尽管参数更多、推理更慢；但VLMs对图像扰动表现出更高的鲁棒性。

Conclusion: 视觉语言模型目前在图像隐私预测任务上尚未超越专用模型，但具有更好的鲁棒性优势。
...
TL;DR: 本研究使用离散语义熵(DSE)来检测和过滤可能产生幻觉的问题，显著提高了黑盒视觉语言模型在放射学图像视觉问答中的准确性。
...
Method: 使用两个公开数据集评估DSE方法，通过多次采样生成回答并计算语义熵，过滤高熵值问题后重新计算准确性。

Result: 过滤高熵问题(DSE>0.3)后，GPT-4o的准确率从51.7%提升至76.3%，GPT-4.1从54.8%提升至63.8%，结果具有统计学显著性。

Conclusion: DSE方法能够可靠地检测黑盒视觉语言模型中的幻觉，显著提高诊断答案的准确性，为临床VLM应用提供了有效的过滤策略。
...
TL;DR: 提出了一个统一框架，联合优化时序语句定位（TSG）和参考视频目标分割（RefVOS），通过专用[FIND]令牌进行关键时刻识别，并设计了时刻中心采样策略和双向锚点更新传播机制。


<details>
  <summary>Details</summary>
Motivation: 现有的LLM方法采样策略要么依赖手工启发式方法忽略时序线索，要么使用外部关键帧模型增加系统复杂性，需要更有效的解决方案。

Method: 使用专用[FIND]令牌通过时序令牌相似性匹配识别关键时刻，设计时刻中心采样策略密集采样信息时刻，稀疏采样非必要帧，并采用双向锚点更新传播机制提升跟踪稳定性。

Result: 该方法在多个数据集上实现了最先进的性能，有效平衡了运动细节和全局上下文，减少了累积误差。

Conclusion: 提出的统一框架成功整合了时序语句定位和参考视频目标分割，通过创新的采样和传播机制显著提升了分割质量和跟踪稳定性。
...
TL;DR: 本文提出VPPO算法，通过token感知视角优化多模态RLVR，仅关注视觉依赖度高的关键token，在8个基准测试中显著提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有多模态推理方法在RLVR优化过程中忽视了视觉感知的关键作用，需要从token感知角度重新审视视觉依赖度对推理的影响。

Method: 提出VPPO算法：1）基于轨迹整体视觉依赖度重新加权优势函数；2）仅对感知关键token进行策略更新。

Result: 在8个感知和推理基准测试中，VPPO显著优于领先的开源RL调优模型，在7B和32B模型规模上均验证了有效性。

Conclusion: 建立了分析多模态RLVR的token级感知视角，提出了有效优化策略显著提升LVLMs的多模态推理能力。
...
TL;DR: 人类视觉注视在扫描图像时遵循类似动物觅食的莱维行走模式，这是一种在稀疏资源环境中优化的随机轨迹。研究发现人类眼睛以最优效率搜寻视觉信息，并且可以通过卷积神经网络从图像结构预测注视热图。
...
Method: 进行了大规模人类受试者实验，40名参与者观看50张不同图像，使用高速眼动仪记录超过400万个注视点。分析数据并训练卷积神经网络从图像输入预测注视热图。

Result: 分析显示人类眼睛的注视轨迹遵循类似动物觅食的莱维行走模式。训练的CNN模型能够准确重现新颖图像中的显著注视区域，表明注视行为的关键成分可以从视觉结构中学习。
...
<details>
  <summary>Details</summary>
Motivation: 现有先进多模态大语言模型在几何推理方面表现不佳，瓶颈在于理解几何图形而非推理本身。由于几何图形可以用简洁文本描述，将视觉内容转换为标题是可行方向。

Method: 引入CapGeo框架，通过辅助标题连接视觉和文本模态；创建CapGeo-Bench数据集（4,641个图形-标题对）和基于关键点的评估指标。

Result: 配备标题后模型性能显著提升：Qwen2.5-VL-72B从8.6%提升至59.0%，Claude-Opus-4从44.8%提升至73.0%。评估指标与下游性能强相关。

Conclusion: CapGeo框架和基准为推进多模态大语言模型的几何推理能力提供了新途径。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的无线电地图生成方法存在模型大、迭代去噪慢、推理延迟高等问题，阻碍了实际部署。
...
TL;DR: 提出了Hybrid-depth框架，通过整合CLIP和DINO基础模型来增强自监督单目深度估计，采用从粗到细的渐进学习策略，显著提升了深度估计性能。


<details>
  <summary>Details</summary>
Motivation: 当前自监督单目深度估计方法由于语义-空间知识提取不足而面临性能限制，需要更有效的上下文信息获取方式。

Method: 1) 从CLIP和DINO聚合多粒度特征，在对比语言指导下设计近距离-远距离图像块比较的代理任务；2) 在粗特征基础上整合相机姿态信息和像素级语言对齐来细化深度预测，可作为即插即用模块与现有MDE管道集成。
...
TL;DR: 提出IRCR-Net网络，通过实例感知一致性正则化机制改进半监督细胞核实例分割，在多个数据集上优于现有方法。
...
Method: 提出IRCR-Net网络，包含匹配驱动实例感知一致性(MIAC)和先验驱动实例感知一致性(PIAC)机制，利用细胞核形态先验知识评估伪标签质量，丢弃低质量伪标签，增强高质量预测。

Result: 实验结果表明，该方法在多个公共数据集上显著提升半监督细胞核实例分割性能，在某些场景下甚至超越全监督方法。

Conclusion: IRCR-Net通过实例感知一致性正则化和先验知识利用，有效解决了半监督细胞核分割中的伪标签噪声问题，取得了优越性能。
...
TL;DR: 提出了一种用于热红外图像增强的渐进提示融合网络(PPFN)，通过建立基于热成像过程的提示对来调制模型特征，能够有效处理单一或复合的图像退化问题。
...
Result: 该方法在特定退化条件下提供有前景的视觉结果，在复杂退化场景中性能显著提升8.76%，同时建立了高质量多场景红外基准数据集。

Conclusion: PPFN能够有效去除相机噪声、保留关键结构细节并增强热图像整体对比度，为热红外图像增强提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态方法在处理缺失和未见场景时存在显著局限性，且现有基准测试因训练测试重叠而高估模型能力。需要更有效的方法来提升多模态关键词预测的性能。
...
Conclusion: 提出的基于视觉语言模型的方法，特别是动态CoT策略，能够有效解决多模态关键词预测中的挑战，显著提升模型性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有的推理基准主要评估基于语言的推理，往往将视觉输入视为可替换的上下文。为了填补这一空白，需要开发一个基于挑战性感知任务的视觉中心推理基准。

Method: BLINK-Twice整合了三个核心组件：七种视觉挑战类型测试视觉推理、自然对抗图像对强制依赖视觉内容、带注释的推理链用于细粒度评估推理过程。

Result: 评估了20个领先的MLLMs，包括12个基础模型和8个推理增强模型。BLINK-Twice对当前模型构成显著挑战，现有语言空间的推理策略（如思维链或自我批评）虽然能提高性能，但往往导致不稳定和冗余的推理。

Conclusion: 重复图像观察能提高模型性能，像o3这样的模型展示的主动视觉交互突显了视觉推理新范式的需求。
...
<details>
  <summary>Details</summary>
Motivation: 传统3D高斯泼溅方法严重依赖初始点云质量，在城市等无边界动态环境中，由于观察视锥体重叠不足，导致点云覆盖不均匀不完整，训练时会产生失真和伪影。现有密集化策略无法重建缺失结构。

Method: 1）通过体素可见性推理识别不可靠几何结构；2）通过多样性感知视图选择信息丰富的支持视图；3）通过基于块匹配的多视图立体重建恢复缺失结构；4）在缺乏初始点的区域生成新的高斯基元。

Result: 在Waymo和nuScenes数据集上的大量实验表明，VAD-GS优于最先进的3DGS方法，显著提高了静态和动态对象重建几何的质量。

Conclusion: VAD-GS能够生成由可靠几何先验引导的新高斯基元，即使在缺乏初始点的区域也能有效恢复缺失结构，为挑战性城市场景的几何重建提供了有效解决方案。
...
TL;DR: 提出Minkowski-MambaNet框架，直接从原始LiDAR数据估计森林体积和地上生物量，无需数字地形模型，在丹麦国家森林清查数据上表现优于现有方法。
...
Result: 在丹麦国家森林清查LiDAR数据上的评估显示，Minkowski-MambaNet显著优于最先进方法，提供更准确和稳健的估计。
...
TL;DR: 提出了两种无需重新训练即可利用DETR中MLP层稀疏性的方法：静态指示器稀疏化(SIBS)和微门控稀疏化(MGS)，MGS通过轻量级门控机制实现85-95%的激活稀疏度，在COCO数据集上保持或提升性能的同时显著减少计算量。


<details>
  <summary>Details</summary>
Motivation: 基于transformer的模型在视觉任务中推理效率仍然是一个挑战，特别是在目标检测等任务中。研究发现DETR的MLP层存在固有的稀疏性，但现有方法需要重新训练模型。

Method: 1. SIBS：基于固定激活模式的启发式方法预测神经元不活跃性；2. MGS：在预训练DETR上训练的轻量级门控机制，使用小型线性层预测动态稀疏性。

Result: 在COCO数据集上的实验表明，MGS能够实现85-95%的激活稀疏度，同时保持或甚至提升模型性能，显著减少计算量。
...
TL;DR: Mono4DEditor是一个用于从单目视频重建的4D场景中进行文本驱动编辑的新框架，通过语言嵌入的动态表示和两阶段定位策略实现精确的语义编辑。


<details>
  <summary>Details</summary>
Motivation: 解决从单目视频重建的4D场景中基于文本提示进行语义精确编辑的挑战，特别是在复杂动态场景的局部区域进行编辑，同时保持未编辑内容的完整性。

Method: 使用量化CLIP特征增强3D高斯，形成语言嵌入的动态表示；提出两阶段点级定位策略（CLIP相似度候选选择+空间范围精炼）；使用基于扩散的视频编辑模型进行目标编辑，通过流和涂鸦引导确保空间保真度和时间一致性。

Result: 在多样场景和对象类型上实现了高质量的文本驱动编辑，保持了未编辑区域的外观和几何特征，在灵活性和视觉保真度方面超越了先前方法。

Conclusion: Mono4DEditor框架能够有效解决4D场景编辑中的语义精确性和局部化挑战，为内容创作和虚拟环境提供了实用的编辑工具。
...
TL;DR: DWTA-Net是一个两阶段低光视频增强框架，通过结合短期和长期时间信息有效抑制噪声和伪影，提升视觉质量。
...
Conclusion: DWTA-Net通过有效利用短期和长期时间线索，在低光视频增强任务中取得了显著性能提升。
...
TL;DR: 提出了SilvaScenes数据集，用于从林下图像进行树种实例分割，包含24个物种的1476棵树，展示了树种分类的挑战性。


<details>
  <summary>Details</summary>
Motivation: 森林管理机器人面临复杂自然环境下的感知挑战，现有数据集不足以开发先进的感知系统，特别是针对树种精细分类。

Method: 在加拿大魁北克五个生物气候区域收集数据，由林业专家标注，包含1476棵树和24个物种，并基准测试现代深度学习实例分割方法。

Result: 树木分割相对容易（mAP 67.65%），但树种分类极具挑战性（mAP仅35.69%）。

Conclusion: SilvaScenes数据集填补了森林环境中树种实例分割数据集的空白，为开发更先进的森林感知系统提供了重要资源。
...
TL;DR: 本文提出了一种维度熵最大化方法，通过正则化文本特征分布来缓解模态间隙问题，提高视觉语言模型在测试时提示调优中的校准性能。
...
Result: 该方法有效缓解了测试时提示调优中校准性能的退化问题，提高了视觉语言模型在现实部署场景中的可靠性。

Conclusion: 维度熵最大化为视觉语言模型的测试时适应提供了一个简单而有效的解决方案，能够增强模型在领域偏移下的可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 音频视觉行业正在整合AI技术，需要从少量人类设计的参考角色生成无限新角色，同时保持艺术风格和共享视觉特征，以扩展动画、游戏等领域的创作可能性。

Method: 基于DreamBooth微调文本到图像扩散模型，采用多令牌策略（通过聚类为个体角色和集体风格分配独立令牌），结合LoRA参数高效微调，移除类别特定正则化集，在生成时引入随机令牌和嵌入。

Result: 在五个小型专业数据集上评估，与相关基线比较，定量指标和人工评估均显示该方法能生成高质量、多样化角色，同时保持参考角色的独特美学特征。

Conclusion: 该方法有效解决了从少量参考生成无限新角色并保持风格一致性的问题，人工评估进一步验证了其有效性，展现了在创意领域的应用潜力。
...
TL;DR: 提出了一种临床基础的方法论来定义交互式医学图像分割的评估任务和指标，并构建了标准化评估框架，评估了最先进算法在不同任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 交互式分割是构建稳健、可泛化的医学图像分割算法的有前景策略，但不一致且临床不现实的评估阻碍了公平比较并歪曲了真实世界性能。

Method: 提出了临床基础的方法论来定义评估任务和指标，构建了软件框架用于构建标准化评估流程，评估了多种最先进算法在异质复杂任务中的表现。

Result: 发现：(i)最小化处理用户交互时的信息损失对模型稳健性至关重要；(ii)自适应缩放机制提升稳健性和收敛速度；(iii)验证提示行为/预算与训练不同时性能下降；(iv)2D方法在板状图像和粗目标上表现良好，但3D上下文有助于处理大或不规则形状目标；(v)非医学领域模型在对比度差和复杂形状下性能下降。

Conclusion: 标准化评估框架对于公平比较交互式分割算法至关重要，并揭示了影响算法性能的关键因素，为未来算法开发提供了重要指导。
...
TL;DR: PhysToolBench是首个专门评估多模态大语言模型对物理工具理解能力的基准，包含1000多个图像-文本对，测试工具识别、理解和创造三个难度级别。评估32个MLLM模型发现其在工具理解方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 工具使用、理解和创造是人类智能的标志，但现代MLLM对物理工具的真正理解程度尚未量化。需要建立基准来评估MLLM在物理工具理解方面的能力。

Method: 构建PhysToolBench基准，作为视觉问答数据集，包含三个难度级别：工具识别（识别工具主要功能）、工具理解（理解工具操作原理）、工具创造（用周围物体创造新工具）。评估了32个MLLM模型。

Result: 对32个MLLM模型的全面评估显示，在工具理解方面存在显著缺陷。提供了深入分析并提出了初步解决方案。

Conclusion: 当前MLLM在物理工具理解方面存在明显不足，PhysToolBench为评估和改进这一能力提供了重要基准。代码和数据集已公开。
...
TL;DR: 三星智能手机图像中的对角线伪影会影响PRNU相机源验证，但PRO模式的原始图像可避免此问题，这些伪影在取证中也有应用价值。


<details>
  <summary>Details</summary>
Motivation: 研究三星智能手机图像中的对角线伪影对PRNU相机源验证的影响，以及这些伪影在取证中的潜在应用。

Method: 分析多个三星手机型号的图像伪影模式，比较PRO模式原始图像与普通图像的差异，探索伪影在HDR和肖像模式图像中的应用。

Result: 发现某些Galaxy S和A系列存在共同的伪影模式导致指纹碰撞，PRO模式原始图像可可靠验证PRNU，伪影可用于减少HDR误检和定位肖像模式合成虚化区域。

Conclusion: 对角线伪影影响PRNU验证但原始图像可解决此问题，这些伪影在图像取证中具有实际应用价值。
...
TL;DR: PRNet是一个实时检测框架，通过保留和有效利用原始浅层空间特征来增强小目标表示，在保持计算效率的同时提升了小目标检测精度。


<details>
  <summary>Details</summary>
Motivation: 解决航拍图像中小目标检测因像素表示有限导致的信息退化问题，现有FPN方法通过后处理增强但重建细节常偏离原始图像信息，阻碍与语义内容的融合。

Method: 提出PRNet框架，包含两个模块：渐进细化颈部(PRN)通过骨干网络重用和迭代细化实现空间-语义对齐；增强切片采样(ESSamp)通过优化重排和卷积在降采样过程中保留浅层信息。

Result: 在VisDrone、AI-TOD和UAVDT数据集上的广泛实验表明，PRNet在可比较的计算约束下优于最先进方法，实现了优异的精度-效率权衡。

Conclusion: PRNet通过优先保留和有效利用原始浅层空间特征，成功解决了航拍图像中小目标检测的信息退化问题，在实时性和准确性方面都表现出色。
...
TL;DR: TC-LoRA提出了一种动态权重调节方法，通过超网络实时生成LoRA适配器，在扩散模型的每个生成步骤中根据时间和条件动态调整权重，相比静态激活调节方法显著提升了生成质量和条件遵循能力。


<details>
  <summary>Details</summary>
Motivation: 现有可控扩散模型使用固定的架构和静态条件策略来修改中间激活，这种静态方法无法适应多阶段去噪过程中从粗到细的动态变化需求，限制了模型根据生成阶段调整响应策略的能力。

Method: TC-LoRA框架使用超网络基于时间和用户条件实时生成LoRA适配器，直接调节冻结骨干网络的权重，实现动态、上下文感知的控制。
...
Result: 在卵细胞、血细胞和疟疾检测任务上的广泛实验表明，FSP-DETR在少样本和开放集场景下显著优于现有方法，特别是在低样本情况下表现优异。

Conclusion: FSP-DETR提供了一个统一的检测框架，能够在推理时灵活支持未见类别识别、背景拒绝和跨任务适应，无需重新训练，为生物医学目标检测提供了有效的解决方案。
...
TL;DR: 对2023-2025年CVPR、ICLR和NeurIPS的26,104篇论文进行透明可重复的研究趋势测量，识别出三大宏观转变：多模态视觉-语言-LLM工作的急剧增长、生成方法的稳步扩展、以及3D和视频活动的韧性发展。


<details>
  <summary>Details</summary>
Motivation: 量化计算机视觉和机器学习领域的研究趋势，通过系统化分析大规模会议论文来识别宏观转变和微观模式，为领域发展提供数据支持。

Method: 对论文标题和摘要进行标准化、短语保护和匹配，使用手工构建的词典分配最多35个主题标签，挖掘任务、架构、训练机制、目标、数据集和共提及模态的细粒度线索。

Result: 发现三大宏观转变：多模态视觉-语言-LLM工作急剧增长，将经典感知重新构建为指令跟随和多步推理；生成方法稳步扩展，扩散研究围绕可控性、蒸馏和速度整合；3D和视频活动保持韧性，组合从NeRFs转向高斯泼溅，人类和智能体中心理解日益重要。

Conclusion: 研究揭示了计算机视觉和机器学习领域的显著转变趋势，多模态和生成方法成为主导，同时发布了词典和方法论以支持审计和扩展，尽管存在词典召回率和仅限摘要范围的局限性。
...
TL;DR: 本文提出了SpaceVista框架，通过构建大规模空间问答数据集SpaceVista-1M和开发空间推理模型SpaceVista-7B，解决了全尺度空间推理中的数据集构建和模型泛化问题。


<details>
  <summary>Details</summary>
Motivation: 当前空间推理研究主要依赖室内3D扫描和人工标注，缺乏全尺度场景建模能力，导致模型容易过拟合特定场景。本文旨在扩展MLLMs的全尺度空间智能，适用于机器人、自动驾驶等多样化应用场景。

Method: 1) 构建包含38K视频场景、约1M空间问答对的SpaceVista-1M数据集；2) 开发SpaceVista-7B模型，采用尺度感知专家和渐进奖励机制；3) 建立精确标注的全尺度基准测试。

Result: 在5个基准测试（包括SpaceVista-Bench）上的广泛评估显示，该方法在所有尺度和场景下都具有竞争性的性能和强大的泛化能力。

Conclusion: SpaceVista框架成功解决了全尺度空间推理的关键挑战，为多样化应用场景提供了有效的解决方案，相关资源将在指定网站发布。
...
TL;DR: 提出了一种基于知识蒸馏的简单有效框架，通过将预训练小动作模型的知识迁移到视觉语言模型中，使VLM具备动作执行能力，显著降低训练成本并提升性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言动作(VLA)模型通过利用预训练视觉语言模型的感知能力显著推进了机器人操作，但从头训练这些模型成本高昂。

Method: 采用两阶段训练策略：首先通过轻量级对齐将VLM隐藏状态映射到小动作模型的动作空间，然后选择性微调语言模型、状态编码器和动作模块，集成多模态输入与精确动作生成。

Result: 在LIBERO上达到97.3%平均成功率(提升11.8%)，在LIBERO-LONG上达到93.5%(提升24.5%)；在五个真实世界操作任务中达到82.0%成功率(提升17%)。

Conclusion: 动作蒸馏有效使VLM能够生成精确动作，同时大幅降低训练成本，在仿真和真实世界任务中都优于先前方法。
...
TL;DR: StreamingVLM是一个专为实时处理无限视频流设计的视觉语言模型，通过统一的训练-推理对齐框架和紧凑的KV缓存机制，在保持低延迟的同时实现长视频的稳定理解。
...
Method: 采用统一的训练-推理对齐框架，在推理时维护包含注意力汇聚点、近期视觉令牌和文本令牌的紧凑KV缓存，通过简单的监督微调策略在短重叠视频块上应用完整注意力。

Result: 在Inf-Streams-Eval基准测试中，StreamingVLM以66.18%的胜率击败GPT-4O mini，在单张NVIDIA H100上保持高达8 FPS的稳定实时性能，同时在多个基准测试上提升了通用VQA能力。

Conclusion: StreamingVLM成功解决了长视频流处理的挑战，实现了实时、稳定的视觉理解，其微调策略还能提升模型的通用能力。
...
TL;DR: 评估GLiNER-BioMed模型在BioASQ数据集上的表现，引入基于词典的后处理策略来改善常见误分类，但该方法在开发集上有效而在盲测集上未能泛化。


<details>
  <summary>Details</summary>
Motivation: 生物医学命名实体识别(BioNER)在区分相似实体类型(如基因和化学物质)方面面临挑战，需要改进模型性能。

Method: 使用GLiNER-BioMed模型，并引入基于词典的后处理策略来处理常见误分类问题。

Result: 后处理策略在开发集上将微F1分数从0.79提升到0.83，但在盲测集上反而从0.79降至0.77，未能实现泛化。

Conclusion: 基于词典的细化方法对预训练BioNER模型有潜力，但面临开发数据过拟合的挑战，需要确保稳健的泛化能力以实现实际应用。
...
TL;DR: 研究发现测试时缩放(TTS)方法存在一个未被识别的失败模式：当候选答案多样性受限时，TTS更容易产生不安全输出。


<details>
  <summary>Details</summary>
Motivation: 揭示TTS方法中关于候选答案多样性假设的潜在风险，证明多样性受限会导致安全漏洞。

Method: 提出RefDiv协议作为诊断攻击来压力测试TTS流程，在多个开源和闭源模型上进行广泛实验。

Result: 限制多样性显著提高了TTS产生不安全结果的概率，且现有安全防护措施无法有效检测这种攻击。

Conclusion: TTS方法存在普遍的安全漏洞，需要设计既能保持有效性又能抵御多样性针对性攻击的鲁棒TTS策略。
...
TL;DR: 提出HAREN-CTC架构，通过多任务学习和CTC损失整合多层SSL语音特征，用于语音抑郁检测，在DAIC-WOZ和MODMA数据集上达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有语音抑郁检测方法主要依赖SSL模型的单层特征，难以捕捉稀疏、异质的抑郁线索，且容易过拟合特定数据集。

Method: 使用分层自适应聚类模块重组SSL特征，通过跨模态融合模块建模层间依赖关系，结合CTC损失处理稀疏时间监督。

Result: 在DAIC-WOZ和MODMA数据集上分别达到0.81和0.82的宏F1分数，优于现有方法。

Conclusion: HAREN-CTC通过有效利用SSL模型的多层特征和CTC对齐训练，显著提升了语音抑郁检测的准确性和泛化能力。
...
TL;DR: 提出了一个评估数学推理能力的新框架，通过分析GPT-3.5在GSM8K数据集上的推理步骤，识别特定失败点，揭示模型在组合推理方面的脆弱性。
...
Method: 首先让GPT-3.5在GSM8K数据集上生成结构化逐步推理，然后使用更强大的GPT-4o-mini模型对错误进行分类，并通过无监督聚类识别出现的"推理模式"。

Result: 分析揭示了一个非人类化的脆弱认知特征：模型在顺序计算等程序性模式上表现近乎完美，但在需要限制条件下的组合推理模式上性能急剧下降。
...
Method: 从模型输出概率分布中推导出置信度分数(CS)，在99个创意提示上进行实验比较。

Result: CS在19%的情况下偏好新颖回复，而流畅性指标为0%，差异显著。CS能有效区分不同难度任务。

Conclusion: 置信度分数减轻了传统指标的创造力偏见，同时保留了核心评估优势，为现代LLM提供更平衡的评估。
...
TL;DR: Recover-LoRA是一种轻量级、数据集无关的方法，通过合成数据和logit蒸馏学习选择性层的LoRA适配器，恢复因量化、剪枝等优化操作导致性能下降的语言模型精度。


<details>
  <summary>Details</summary>
Motivation: 推理优化（如量化、剪枝、格式转换等）会导致语言模型任务性能下降，现有研究主要关注鲁棒量化技术，而本文关注从任何导致模型权重退化的源中恢复模型精度。

Method: 使用合成数据和logit蒸馏在选择性层上学习LoRA适配器，将退化模型与其全精度模型对齐。

Result: 在多种小型语言模型（包括MHA和GQA架构）上，Recover-LoRA能够恢复5-17%的模型精度。

Conclusion: Recover-LoRA是一种有效的模型精度恢复方法，适用于不同类型的注意力架构和评估数据集。
...
TL;DR: Mnemosyne是一种受人类记忆启发的无监督长期记忆架构，专为边缘设备上的LLM设计，采用图结构存储、过滤机制和概率召回，在医疗对话中表现优于传统RAG方法。


<details>
  <summary>Details</summary>
Motivation: 解决当前LLM内存系统在边缘设备上的局限性，传统方法要么依赖暴力上下文扩展，要么使用静态检索管道，无法处理语义相似但时间不同的对话。

Method: 使用图结构存储、模块化内容和冗余过滤器、内存提交和修剪机制，以及基于人类记忆模型的概率召回（包含时间衰减和刷新过程），并引入浓缩的"核心摘要"来捕捉用户长期特征。

Result: 在纵向医疗对话实验中，Mnemosyne在盲测人类评估中获得65.8%的胜率（基准RAG为31.1%），在LoCoMo基准测试中取得当前最高的时间推理和单跳检索分数，总体得分54.6%排名第二。

Conclusion: Mnemosyne证明通过边缘兼容且易于迁移的无监督内存架构，可以实现改进的事实召回、增强的时间推理和更自然的用户响应。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法将AI文本检测视为二分类问题，但人类文本并不构成统一分布，导致模型记忆OOD特征而非学习'非ID'行为本质，限制了泛化能力。
...
Method: 构建包含28个子领域和153万段落的病理学向量数据库，采用双通道混合检索（BGE-M3密集检索+词汇引导稀疏检索）和基于LLM的支持性证据判断模块。

Result: 在YpathR基准测试中Recall@5达到98.64%，比基线提升23个百分点；在YpathQA-M的300个最具挑战性问题中，将通用和医学LLM的准确率平均提升9.0%，最高提升15.6%。

Conclusion: YpathRAG提供了可扩展的构建范例和可解释的评估方法，显著改善了病理学导向RAG的检索质量和事实可靠性。
...
TL;DR: LatentBreak是一种新型的越狱攻击方法，通过用语义等效词替换输入提示中的词汇来生成低困惑度的自然对抗提示，从而规避基于困惑度的防御机制。


<details>
  <summary>Details</summary>
Motivation: 现有的越狱攻击通常通过添加高困惑度的对抗后缀或长模板来解锁模型响应，但这些攻击容易被基于困惑度的过滤检测到。

Method: LatentBreak采用白盒攻击方式，在潜在空间中最小化对抗提示与无害请求表示之间的距离，选择语义等效词替换输入提示中的词汇，保持原始意图同时降低困惑度。

Result: 评估显示LatentBreak生成的提示更短且困惑度更低，在多个安全对齐模型上对基于困惑度的过滤器表现优于其他越狱算法。

Conclusion: LatentBreak提供了一种有效的低困惑度越狱攻击方法，能够规避现有的基于困惑度的防御机制。
...
TL;DR: 本文研究了数字平台上错误信息的传播问题，提出了一个多语言、多代理的大型语言模型框架，通过检索增强生成技术来检测错误信息，并可作为网络插件部署到在线平台。
...
Method: 开发了一个多语言、多代理的大型语言模型框架，采用检索增强生成技术，能够部署为网络插件。研究了跨英语、法语、西班牙语、阿拉伯语、印地语和中文的语言切换及翻译，查询长度膨胀前的摘要，以及结构重格式化为多项选择题。

Result: 展示了基于插件部署的可行性，用于现实世界网络应用中的错误信息检测。

Conclusion: 强调了AI驱动的错误信息检测在保护在线事实完整性免受多样化攻击方面的重要性，同时展示了插件部署在现实世界网络应用中的可行性。
...
TL;DR: 提出了一种基于情感热点的对话情感识别方法，通过检测多模态中的情感热点，使用热点门控融合和路由对齐器来解决证据稀疏和模态不对齐问题。
...
Method: 检测文本、音频和视频中的情感热点，通过热点门控融合将热点与全局特征结合，使用路由混合对齐器进行模态对齐，并构建跨模态图编码对话结构。

Result: 在标准ERC基准测试中表现优于强基线模型，消融实验验证了热点门控融合和混合对齐器的有效性。

Conclusion: 情感热点为中心的方法为多模态学习提供了新视角，能够指导未来的多模态融合研究。
...
TL;DR: MMA-ASIA是一个评估大语言模型在亚洲文化背景下文化意识的多模态基准框架，包含27,000个多语言多模态问题，79%需要基于文化背景的多步推理。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型在多模态理解和推理方面在非西方、低资源环境下表现不佳，需要专门评估其在亚洲文化背景下的文化意识。

Method: 构建人类策划的多语言多模态对齐的多选题基准，涵盖8个亚洲国家和10种语言；提出五维评估协议和文化意识基础验证模块；使用比较模型分析、注意力追踪和视觉消融前缀重放方法。
...
TL;DR: GraphGhost是一个统一框架，将神经元激活和信号传播表示为图结构，用于分析LLMs的推理机制。通过图算法揭示模型推理行为，并通过结构干预验证关键神经元节点的重要性。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型展现出卓越的推理能力，但其背后的结构机制仍未被充分探索。研究者希望理解LLMs如何从序列输入中捕获结构语义，并通过一致的结构机制生成输出。

Method: 提出GraphGhost框架，将神经元激活和信号传播建模为图结构，应用PageRank等图算法分析模型特性，并通过结构干预（编辑关键神经元节点）来验证推理机制。

Result: 揭示了不同模型在多样化数据集上共享和特定的推理行为，发现对关键神经元节点的编辑会触发推理崩溃，改变逻辑流程和语义理解。
...
TL;DR: 研究发现大型语言模型在医疗诊断中对患者性别相关性的判断存在不一致性，特别是在性别相关性评估方面，这可能影响AI辅助临床护理的可靠性。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在医疗领域的应用日益增多，但其对性别偏见的敏感性尚未充分研究。性别长期影响医生行为和患者结果，因此需要评估LLMs在扮演类似人类角色时是否会复制或放大性别相关偏见。

Method: 使用《新英格兰医学杂志》挑战赛的案例研究，为多个开源和专有LLMs分配性别（女性、男性或未指定），评估它们在LLM性别分配下的响应一致性，包括基于LLM的诊断以及模型对患者性别临床相关性或必要性的判断。
...
TL;DR: 提出一个利用LLM自动生成数学应用题中干扰条件的迭代框架，保持原问题解不变，减少人工标注成本。


<details>
  <summary>Details</summary>
Motivation: 现有数学应用题数据集大多只包含必要信息，缺乏有意义的干扰条件，且现有含干扰条件的数据集难度低、质量差，影响模型评估的可信度。

Method: 设计迭代框架，使用LLM从多角度和认知层面修订数学应用题，生成有意义的干扰条件，同时确保原问题解不变。

Result: 该框架能高效生成高质量的含干扰条件数学应用题，大幅减少人工工作量。

Conclusion: 该自动生成框架为创建更可靠的数学推理基准提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型的基准分数可能因记忆测试项或其近似重复项而虚高，需要评估模型在改写问题上的泛化能力。

Method: 使用Mistral-7B-Instruct和Qwen2.5-7B-Instruct模型，在ARC-Easy和ARC-Challenge数据集上测量原始问题和改写问题之间的准确率差距，包含解码控制、多选输出格式强制和改写清理步骤。

Result: 改写会导致非平凡的准确率下降（原始vs改写），这与先前关于污染和脆弱的表面形式捷径的担忧一致。

Conclusion: 基准测试可能因记忆效应而夸大模型能力，需要更严格的泛化评估方法。
...
TL;DR: JAI-1是一个750亿参数的泰语中心语言模型，采用上扩增策略从较小的英语模型扩展参数空间，系统性地集成泰语知识，在泰语基准测试中表现优于Typhoon2-70B。
...
Method: 从高性能英语开源LLM出发，扩展参数空间，利用新增容量系统集成泰语知识。预训练使用1.5万亿token（含3000亿泰语token），随后进行监督微调和对齐调优。
...
TL;DR: 本文研究基于用户画像（年龄、性别、职业）的销售对话代理，发现职业对对话意图影响最大，提出轻量级职业条件策略来优化对话效果。
...
Method: 引入轻量级的职业条件策略，引导代理优先选择与用户偏好一致的对话意图。

Result: 该策略实现了更短且更成功的对话，职业因素对对话意图的影响最为显著。

Conclusion: 丰富的用户画像和基于职业的简单策略可以显著提升销售导向对话系统的有效性。
...
<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型可以自动从自然语言输入生成软件需求，但评估这些需求是否真实反映利益相关者需求仍然主要依赖人工。需要自动化方法来量化需求与原始访谈内容的对齐程度。

Method: 将访谈记录分段为文本块，将对齐问题建模为文本块与用户故事之间的匹配问题。使用基于LLM的匹配器，结合嵌入模型进行有效阻塞。

Result: 在四个数据集上的实验表明，基于LLM的匹配器在保留注释上达到0.86的宏F1分数，而仅使用嵌入模型的性能较低但可用于有效阻塞。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法直接将LLM应用于提取任务，但将JSON模式视为静态合约，导致提取性能不佳、幻觉频繁和代理行为不可靠。

Method: 开发了PARSE系统，包含ARCHITECT组件（自动优化JSON模式）和SCOPE组件（基于反射的提取方法，结合静态和LLM防护机制）。

Result: 在三个数据集上评估，SWDE提取准确率提升64.7%，整体框架改进达10%，首次重试错误减少92%，同时保持实用延迟。

Conclusion: PARSE通过协同优化模式设计和提取过程，显著提高了结构化信息提取的准确性和可靠性。
...
TL;DR: 研究发现评估导向的提示语会夸大LLM性能，增加推理长度但未必提升准确率，建议采用中性措辞和双重框架检查来确保基准测试反映真实部署能力。


<details>
  <summary>Details</summary>
Motivation: 现有LLM基准测试使用评估导向的提示语，但实际部署需要简洁、契约约束的回答，这种"评估气味"可能夸大性能测量而不带来相应能力提升。

Method: 使用GPT-OSS-20B模型进行六组A/B测试，固定任务内容和解码，仅改变框架（评估导向vs真实世界）和推理深度，通过确定性验证器计算准确率、合规性等指标。

Result: 评估框架显著增加推理长度（数百到1000+字符），降低简洁回答合规性，准确率提升有限或不一致。激励措辞改变错误构成，多语言标题重现这些特征。

Conclusion: 需要采用中性措辞、双重框架检查、契约感知评分等方法来确保基准测试增益反映可部署能力，并提供可复现的A/B框架和实用指南。
...
TL;DR: PULSE框架使用小型语言模型生成推理依据作为监督信号，结合交互历史共同建模用户行为及其语义驱动因素，在保持高效的同时提升推荐性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型推理能力强但推理成本高，小型语言模型效率高但推理能力未被充分探索。现有系统未能充分利用自然语言推理依据作为学习信号。

Method: 提出PULSE框架，将SLM生成的推理依据作为直接学习信号，与交互历史共同监督，构建跨域的用户和物品理解空间（Thought Space）。

Result: PULSE在多个基准数据集上优于主流ID、协同过滤和基于LLM的序列推荐模型，在跨域推荐中表现出更好的可迁移性，在下游推理问答任务中也有强性能。

Conclusion: 将SLM生成的推理依据作为一等信号处理，能产生更鲁棒和可泛化的嵌入表示，为高效且有效的推荐系统提供了新方向。
...
TL;DR: ExPO-HM提出了一种新的仇恨表情包检测方法，通过结合SFT预热、GRPO课程学习和条件决策熵奖励，实现了从简单二元检测到解释驱动检测的转变，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有仇恨表情包检测方法多为简单的二元预测，缺乏上下文和解释，无法满足实际审核需求。现有的解释-检测方法性能不佳，主要问题是模型无法生成政策相关的解释线索，且二元奖励信号不足以指导推理。
...
TL;DR: HDLM是一种新的离散扩散语言模型，通过分层词汇表实现从粗粒度到细粒度的语义生成过程。
...
Method: 使用分层词汇表，前向过程将细粒度token扰动为粗粒度token，反向过程逐步预测更详细的语义。

Result: 实验表明HDLM在验证困惑度和生成困惑度上均优于基线模型。

Conclusion: HDLM提供了一种通用的时变语义尺度预测框架，在文本生成任务中表现出色。
...
TL;DR: UCoT是一种高效的推理框架，通过前期思想嵌入实现CoT压缩，使用小模型作为压缩器和大模型作为执行器的协作工作流，在保持强大推理能力的同时显著减少推理长度。


<details>
  <summary>Details</summary>
Motivation: 长链式思维推理在大型语言模型中计算成本高、延迟大，现有CoT压缩方法要么需要繁琐的手工提示设计，要么构建压缩数据集时牺牲关键推理细节。

Method: 两阶段协作框架：第一阶段训练压缩器生成富含推理信息的前期思想嵌入；第二阶段优化执行器利用这些嵌入进行简短推理，采用奖励机制。

Result: 在GSM8K数据集上，UCoT将Qwen2.5-7B-Instruct模型的token使用量减少50%，性能比SOTA方法高出3.08%。

Conclusion: UCoT框架有效解决了长CoT推理的效率问题，在保持推理质量的同时显著降低计算成本，为高效推理提供了新思路。
...
TL;DR: 提出了一个分析个人叙事风格的形式化框架，将风格定义为作者表达主观体验时的语言选择模式，结合功能语言学、计算机科学和心理学，通过语言模型自动提取语言特征并分析序列模式。
...
Method: 整合功能语言学、计算机科学和心理学，使用语言模型自动提取过程、参与者和环境等语言特征，分析序列模式，应用于数百个梦境叙事案例研究。

Result: 在战争创伤后应激障碍退伍军人的案例中，分析发现其叙事中言语过程主导心理过程的独特模式，揭示了语言选择与心理状态的关系。

Conclusion: 该框架成功形式化了个人叙事中的风格分析，展示了语言选择模式与心理状态之间的系统性联系，为理解主观体验的表达提供了新方法。
...
TL;DR: 该研究利用LLM技术将定性文本数据与传统评分量表相结合，开发增强型心理测试，在抑郁评估案例中显著提高了测量精度和准确性。


<details>
  <summary>Details</summary>
Motivation: 传统心理评估主要依赖结构化评分量表，无法充分利用受访者自然语言的丰富信息。本研究旨在利用LLM技术挖掘定性数据价值，增强传统心理测量工具。

Method: 结合LLM评分的文本数据和传统评分量表项目，创建增强型测试框架。在真实世界高中生样本(n=693)和合成数据集(n=3,000)上开发评估，基于项目信息量计算经验性选择最具信息量的LLM评分指令。

Result: 在保留测试集上，增强型测试在测量精度和准确性方面实现了统计显著提升。LLM项目带来的信息增益相当于为原始19项测试额外增加6.3(真实数据)至16.0(合成数据)个项目。

Conclusion: 该框架为利用日益增长的转录文本数据增强传统心理测量工具提供了可扩展方法，在临床健康等领域具有潜在应用价值，代表了绕过传统自动化评分瓶颈的概念性转变。
...
TL;DR: dInfer是一个高效可扩展的扩散式大语言模型推理框架，通过模块化设计和系统优化，在保持输出质量的同时实现了显著的效率提升。


<details>
  <summary>Details</summary>
Motivation: 扩散式大语言模型(dLLMs)作为自回归模型的有前景替代方案，但由于缺乏标准化和高效的推理框架，其广泛应用受到限制。

Method: 将推理流程分解为四个模块化组件：模型、扩散迭代管理器、解码策略和KV缓存管理器，并为每个组件集成新算法和系统级优化。

Result: 在8×H800 GPU上，批处理大小为1时，HumanEval达到1100+ tokens/秒，六个基准测试平均超过800 tokens/秒；相比Fast-dLLM实现10倍加速，相比优化后的自回归模型QWen2.5-3B仍有2-3倍加速。

Conclusion: dInfer通过算法创新和系统优化的结合，为扩散式大语言模型提供了高效推理解决方案，显著提升了推理效率而不影响输出质量。
...
TL;DR: 该研究首次对代码LLM的缩放规律进行大规模实证分析，发现代码比自然语言更数据饥渴，需要更高的数据参数比，且Farseer缩放定律比Chinchilla定律更准确。


<details>
  <summary>Details</summary>
Motivation: 现有缩放定律主要基于自然语言分析，但代码与自然语言存在根本差异（如严格语法），不清楚这些定律是否直接适用于代码LLM训练。

Method: 进行了117次实验，模型规模从0.2B到3.8B，训练token从2B到128B，拟合了Chinchilla和Farseer两种缩放定律。

Result: 代码LLM能有效随模型规模扩展，但代码比自然语言更数据饥渴，需要更高的数据参数比；Farseer定律比Chinchilla定律更准确；代码-自然语言混合实验中，自然语言在资源受限时有帮助，但在高计算预算时反而有害。

Conclusion: 代码LLM的缩放规律与自然语言不同，需要专门的分析和优化策略，特别是在数据需求和模型规模平衡方面。
...
TL;DR: 本文提出了一个三阶段推理框架来分析LLM在法律案例推理中的能力，发现在表面推理任务上表现良好，但在层次推理和综合分析任务上性能显著下降，且错误回答消耗更多计算资源。


<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型在复杂、细致的法律案例推理中的能力，特别是类比和区分判例的能力，这是美国法律实践的基础。

Method: 提出一个正式框架，将案例区分过程分解为三阶段推理任务：使用事实谓词（因素）建模案例，组织成法律知识层次结构，定义可验证的规则来识别区分、分析论证支持并评估其重要性。

Result: 现代推理LLM在表面推理（任务1）上准确率高，但在层次推理（任务2：64.82%-92.09%）和综合分析（任务3：11.46%-33.99%）上性能显著下降。模型在错误回答上比正确回答消耗更多计算资源。
...
TL;DR: 提出了一个框架来构建紧凑、多样且抗分数膨胀的测试用例基准TC-Bench，通过选择最大多样性的错误代码来评估LLM生成的测试用例。


<details>
  <summary>Details</summary>
Motivation: 现有基准存在计算成本高、分数膨胀和偏向简单错误的问题，需要找到最小错误代码集来代表整个错误空间。
...
Conclusion: 该框架能够构建紧凑且多样化的基准，有效评估测试用例生成方法，揭示了现有方法在检测关键错误方面的不足。
...
TL;DR: 研究发现微基准测试在模型排名一致性方面存在局限性，无法可靠地区分性能相近的模型，需要250个样本才能获得可靠结果，而随机采样与现有方法效果相当。


<details>
  <summary>Details</summary>
Motivation: 解决语言模型开发中时间和成本过高的问题，评估微基准测试能否像完整基准测试一样一致地排名模型，以及是否优于随机数据点选择。

Method: 引入元评估方法，分析微基准测试在不同性能差异下对模型排名的可靠性，比较不同样本大小下的效果。

Result: 微基准测试无法一致地区分MMLU-Pro上3.5分或BIG-bench Hard上4分差异的模型对，需要250个样本才能获得可靠排名，随机采样与现有方法效果相当。
...
TL;DR: 本文提出了一种基于LLM的复合位置引用地理编码方法，通过评估LLM的地理空间知识与推理能力，开发出比大型现成模型性能更好且更小的微调LLM。
...
Method: 基于LLM处理地理空间数据的能力评估，提出了一种基于LLM的复合位置引用地理编码策略，并进行了模型微调。

Result: 该方法提高了地理编码任务的性能，相对较小的微调LLM能够达到与大型现成模型相当的性能。

Conclusion: LLM在地理编码任务中表现出色，通过适当的策略和微调，较小的模型也能实现与大型模型相当的性能。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM在不同国家和语言中的广泛使用，需要理解和保护其多语言响应，确保在不同语言环境中的准确性和一致性。

Method: 使用五点评分标准和法官LLM，评估模型在五个维度上的响应，涵盖低资源和高资源语言。

Result: GPT-5在各项类别中平均表现最佳，尤其在同意与自主权和伤害预防与安全类别中得分最高（3.56和4.73），而Gemini 2.5 Pro得分最低（1.39和1.98）。
...
TL;DR: 提出Adaptive Retention方法，通过概率性、分层token选择机制，在严格全局预算M下学习保留哪些表示，解决Transformer注意力O(n²)复杂度问题，实现长上下文效率提升。


<details>
  <summary>Details</summary>
Motivation: Transformer注意力机制随序列长度呈二次方增长(O(n²))，限制了长上下文应用。需要一种方法在保持性能的同时减少计算和内存开销。
...
<details>
  <summary>Details</summary>
Motivation: 现有对大型语言模型的评估在通用中文语境下研究不足，需要专门的中文基准来评估模型整合中文特定事实知识与多步逻辑推理的能力。

Method: 首先从现有QA数据集中构建领域平衡的种子集，然后开发基于LLM的流水线生成基于事实单元链的多跳问题，并采用人机协同验证系统确保数据质量。

Result: 评估显示最先进的LLMs在处理长尾知识和执行知识密集型推理方面存在持续局限性，但检索增强生成能显著缓解知识差距并带来性能提升。

Conclusion: CCMOR基准揭示了LLMs在中文多跳推理任务中的知识处理局限，检索增强是有效的改进方向，为中文语言模型评估提供了重要工具。
...
<details>
  <summary>Details</summary>
Motivation: 科学编码任务需要严谨的算法、深度领域知识和特定推理，且通常涉及一系列子问题的链式求解，而现有方法在处理这类复杂科学任务时存在局限性。

Method: 采用训练免费的多智能体框架，设计专门智能体进行自我反思、创建推理、编码和调试，使用学生-教师范式，结合整合上下文窗口(CCW)来减少LLM幻觉。

Result: 在科学编码基准测试中，MOSAIC在准确性、鲁棒性和可解释性方面优于现有方法。

Conclusion: MOSAIC框架通过多智能体协作和专门设计的学生-教师范式，有效解决了科学代码生成中的挑战，为复杂科学任务提供了更好的解决方案。
...
TL;DR: 本文研究了不同语言结构对LLMs隐私泄露的影响，发现意大利语泄露最严重，英语在成员推断方面更易区分，而法语和西班牙语因形态复杂性更具韧性。
...
Method: 量化六种语言指标，评估三种攻击向量：提取攻击、反事实记忆和成员推断攻击，基于英语、西班牙语、法语和意大利语医学语料库训练的LLMs。

Result: 隐私漏洞与语言冗余和分词粒度相关：意大利语泄露最强，英语成员可区分性更高，法语和西班牙语因形态复杂性更具韧性。
...
TL;DR: 提出Search-on-Graph框架，通过让LLM在知识图谱上执行迭代的知情图导航来解决多跳问答问题，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在知识密集型多跳问答中存在局限性：LLMs会遗漏长尾事实、产生幻觉且知识更新滞后；传统KGQA方法在查询编译、子图检索和搜索空间控制方面面临根本性权衡。

Method: 采用"观察-然后导航"原则，设计单一的Search函数让LLM在每一步检查当前实体的可用关系后再决定下一跳，自适应处理不同图谱模式和高度数节点。

Result: 在六个KGQA基准测试中达到最先进性能，特别是在Wikidata基准上比之前最佳方法提升16%，在Freebase基准上也表现一致提升。

Conclusion: SoG框架通过简单的迭代图导航方法有效解决了复杂KGQA问题，无需微调即可适应不同知识图谱模式。
...
TL;DR: 提出了PE-CoA框架，包含五种对话模式来构建有效的多轮越狱攻击，在12个LLM和10种危害类别上实现了最先进的性能，揭示了模式特定的漏洞和LLM行为特征。


<details>
  <summary>Details</summary>
Motivation: 现有多轮越狱方法依赖启发式或临时探索策略，对模型底层弱点提供有限洞察，对话模式与跨危害类别模型漏洞之间的关系理解不足。

Method: PE-CoA框架包含五种对话模式，通过自然对话构建有效的多轮越狱攻击。

Result: 在12个LLM和10种危害类别上实现最先进性能，发现模型表现出不同的弱点分布，对一种对话模式的鲁棒性不能推广到其他模式，模型家族共享相似的失败模式。
...
TL;DR: 本文评估了质量估计重排在文档级机器翻译中的应用，发现使用SLIDE和GEMBA-DA等质量评估指标可以显著提升翻译质量，特别是在使用更多候选翻译时效果更明显。


<details>
  <summary>Details</summary>
Motivation: 质量估计重排虽然在句子级机器翻译中已被证明有效，但在日益重要的文档级翻译领域应用不足。本文旨在探索质量估计重排在文档级翻译中的性能表现。

Method: 使用各种学习型和基于大语言模型的质量评估指标（包括SLIDE和GEMBA-DA），在文档级翻译任务中对生成的翻译候选进行评分和选择。

Result: 使用最佳学习型指标SLIDE时，BLEURT-20分数在仅2个候选翻译时提升+2.00，在32个候选翻译时提升+5.09。使用最佳LLM指标GEMBA-DA时，相应提升为+1.63和+4.30。即使在较长输入下，32个候选翻译仍能带来+2.34（SLIDE）和+1.40（GEMBA-DA）的提升。

Conclusion: 文档级质量估计重排具有实际应用价值，在合适的翻译模型和硬件条件下，仅需最小的运行时开销即可显著提升翻译质量。
...
Method: 从真实的US-GAAP合规XBRL文件中构建FinAuditing基准，定义三个互补的子任务（FinSM、FinRE、FinMR），并提出统一的评估框架，整合检索、分类和推理指标。

Result: 对13个最先进的LLMs进行零样本实验，发现当前模型在语义、关系和数学维度上表现不一致，当推理分层多文档结构时，准确率下降60-90%。

Conclusion: 现代LLMs在基于分类法的财务推理中存在系统性限制，FinAuditing为开发可信赖、结构感知和法规对齐的财务智能系统奠定了基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通常通过限制更新间接鼓励探索，但未在token生成阶段显式促进探索行为。研究发现LLM中的token在推理任务中扮演不同角色，分为高熵推理token和低熵知识token。

Method: 引入互补方法，在采样过程中显式促进探索：对推理token应用较高温度以鼓励探索，对知识token保持较低温度以维持事实正确性。系统研究多种多温度调度策略及其在强化学习中的影响。

Result: 在多个推理基准测试上的实证评估表明，该方法显著提升了LLMs的推理性能。

Conclusion: 通过为不同token类型应用差异化温度设置的方法，能够有效增强语言模型的推理能力，在保持事实准确性的同时促进探索性推理。
...
TL;DR: 提出基于大语言模型的统一生物医学命名实体识别框架，通过符号化标注策略处理嵌套实体，采用双语联合微调提升跨语言泛化能力，并引入对比学习实体选择器过滤错误预测。
...
Method: 将BioNER重新定义为文本生成任务，设计符号化标注策略处理扁平与嵌套实体；进行中英文数据集的双语联合微调；引入基于对比学习的实体选择器过滤错误预测。
...
TL;DR: 提出Semantic-Anchor Compression (SAC)方法，通过直接选择锚点token并聚合上下文信息到其KV表示中，避免自编码训练，提升上下文压缩性能。


<details>
  <summary>Details</summary>
Motivation: 现有上下文压缩方法依赖自编码任务训练压缩token，但自编码优化的重建目标与下游任务存在不匹配，削弱了实际使用中有用的特征。

Method: SAC直接从原始上下文中选择锚点token，通过锚点嵌入和双向注意力修改两个关键设计，将上下文信息聚合到锚点token的KV表示中，无需自编码训练。

Result: SAC在各种压缩比下均优于现有方法，在MRQA的分布外评估中，5倍压缩时比强基线提升1 EM，压缩比越高优势越明显。

Conclusion: SAC通过直接利用锚点token的架构设计，避免了自编码训练的不匹配问题，在上下文压缩任务中表现出色且具有可扩展性。
...
TL;DR: 该论文研究了LLM内部表征中类似人类印象和刻板印象的人工印象模式，使用线性探针预测刻板印象内容模型的两个维度，并分析了这些印象与模型行为的关系。


<details>
  <summary>Details</summary>
Motivation: 研究LLM内部表征中是否存在类似人类印象和刻板印象的模式，以及这些模式如何影响模型的下游行为。

Method: 使用线性探针在生成的提示上预测刻板印象内容模型的两个维度，分析隐藏表征中印象的可解码性及其与模型响应的关系。

Result: 发现LLM在提示下不一致地报告印象，但隐藏表征中的印象更一致地可线性解码；人工印象能预测模型响应的质量和模糊语使用；提示中的内容、风格和方言特征会影响LLM印象。
...
TL;DR: 提出了SOP-Maze基准，包含397个基于真实业务数据的任务，用于评估LLM在复杂标准操作流程中的表现，发现现有模型在流程遵循、对话处理和计算推理方面存在显著困难。


<details>
  <summary>Details</summary>
Motivation: 现有基准未能充分评估LLM在复杂业务标准操作流程中的能力，需要专门基准来填补这一空白。

Method: 从真实业务数据构建SOP-Maze基准，包含397个任务，分为横向根系统（宽选项选择）和核心根系统（深度逻辑推理）两类。

Result: 实验显示几乎所有最先进模型在SOP-Maze上表现不佳，识别出三种主要错误类型：流程盲区、对话脆弱性和计算错误。

Conclusion: SOP-Maze为评估和改进LLM在复杂业务流程中的能力提供了新视角，相关资源已开源。
...
TL;DR: 该研究分析了开源社区治理文档的变化模式，通过解析710个项目的治理文档，发现项目随时间推移定义了更多角色和行动，且分布更均衡，但规则组成保持稳定。


<details>
  <summary>Details</summary>
Motivation: 研究开源社区如何通过版本控制的治理文档描述参与和控制机制，为评估未来AI中介工作流是否会集中或重新分配权力提供可复现的基线。

Method: 使用710个项目的配对快照，将文本解析为参与者、规则、行动和对象，通过熵值测量均匀性、丰富度测量多样性、Jensen-Shannon散度测量漂移来量化变化。

Result: 项目随时间定义了更多角色和行动，这些元素分布更加均衡，但规则组成保持稳定，表明治理通过扩展和平衡参与类别而增长，没有重大规范性转变。

Conclusion: 治理增长是通过扩展和平衡参与类别实现的，而非规范性力量的重大转变，这为评估AI工作流是否集中或重新分配权力提供了分析框架。
...
TL;DR: CAPC-CG是中国首个开放的政策指令数据集，包含1949-2023年中国中央政府的法律、行政法规和部门规章，采用五色分类法标注清晰和模糊语言，包含330万个段落单元。


<details>
  <summary>Details</summary>
Motivation: 建立首个中文政策指令数据集，支持政策沟通研究和多语言NLP研究，基于Ang的自适应政策沟通理论。

Method: 构建包含国家法律、行政法规和部门规章的语料库，采用两轮标注框架，由专家和训练有素的标注员进行标注，使用五色分类法。
...
TL;DR: MASA是一个基于大型语言模型的多智能体自动形式化框架，通过协作智能体将自然语言语句转换为形式化表示，强调模块化、灵活性和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 自动形式化在连接自然语言和形式推理中起着关键作用，需要高效可靠的方法来处理真实世界的数学定义和形式数学数据集。

Method: 采用多智能体系统架构，利用LLM和定理证明器的交互协作，将自然语言转换为形式化表示，支持新智能体和工具的集成。

Result: 在真实世界数学定义和形式数学数据集上的实验展示了MASA的有效性，证明了其在自动形式化任务中的潜力。

Conclusion: MASA框架展示了多智能体系统在提升自动形式化效率和可靠性方面的潜力，为研究人员和从业者提供了有价值的见解和支持。
...
<details>
  <summary>Details</summary>
Motivation: 现有GRPO方法及其变体使用静态或过于简化的权重方案，无法适应模型能力的变化，导致训练在不同难度级别上不平衡，阻碍整体性能提升。
...
Conclusion: DARO通过动态难度感知重加权机制有效解决了现有RLVR方法的局限性，显著提升了推理能力的训练效果。
...
<details>
  <summary>Details</summary>
Motivation: Current safety alignment methods require expensive searches for optimal data proportions to balance safety and general performance, with limited gains.

Method: 使用基于LoRA的拒绝训练方法，仅使用安全数据进行训练，将安全特性解耦到与模型固有转换空间正交的低秩子空间中。

Result: LoRA能够有效将安全特性解耦到低秩子空间，确保安全增强不会干扰模型的固有能力，实现了性能保持的安全对齐。

Conclusion: LoRA作为成本高效、性能保持且即插即用的安全补丁，为构建可信人工智能提供了新的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有基于大型语言模型的Text-to-SQL方法对专有模型的依赖，这些方法存在部署可行性和数据隐私问题。

Method: 包含两个组件：使用预计算模式嵌入向量数据库的模式检索器，以及经过监督微调和执行引导强化学习两阶段微调的SQL生成器。

Result: 在BIRD数据集上达到72.10%执行准确率，在Spider 1.0上达到88.45%，性能与LLM方法相当或更优，但参数数量减少2-30倍。

Conclusion: 高质量Text-to-SQL生成可以通过轻量级模型实现，为隐私敏感和资源受限场景提供了实用解决方案。
...
TL;DR: 该研究提出通过迭代优化评分标准来提升大语言模型在自动作文评分中的表现，通过模型自我反思评分理由和与人类评分差异来改进评分标准，在TOEFL11和ASAP数据集上显著提高了评分一致性。


<details>
  <summary>Details</summary>
Motivation: 大语言模型的性能对提示词高度敏感，受提示优化领域启发，本研究探索通过优化评分标准来提升自动作文评分系统的表现。

Method: 让模型基于自身评分理由和与人类评分的差异，迭代优化评分标准，在TOEFL11和ASAP数据集上使用GPT-4.1、Gemini-2.5-Pro和Qwen-3-Next-80B-A3B-Instruct进行实验。

Result: 在TOEFL11和ASAP数据集上分别实现了高达0.19和0.47的二次加权Kappa系数提升，即使使用简单初始评分标准，也能达到或超过使用详细人工编写评分标准的性能。

Conclusion: 迭代优化评分标准对于基于大语言模型的自动作文评分系统至关重要，能够显著提升与人类评价的一致性。
...
TL;DR: 本文针对资源匮乏的查克玛语，构建了孟加拉文转写的查克玛语语料库，并通过微调多语言Transformer模型，在MLM任务上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 查克玛语作为印度-雅利安语系中数据有限的语种，在语言模型中代表性不足，需要开发有效的迁移学习方法。

Method: 从查克玛文学中构建上下文连贯的孟加拉文转写查克玛语料库，经母语者验证后，对6种编码器型多语言Transformer模型进行MLM任务微调。

Result: 微调后的多语言模型在查克玛语上表现优于预训练模型，最高达到73.54%的token准确率和2.90的低困惑度。

Conclusion: 孟加拉文转写的查克玛语对查克玛语的迁移学习非常有效，同时揭示了OCR管道在形态丰富的印度文字中的局限性。
...
TL;DR: LLMs内部状态无法可靠区分事实与幻觉，因为基于主题知识的幻觉与正确回答使用相同的内部回忆过程，导致隐藏状态几何重叠且不可区分。


<details>
  <summary>Details</summary>
Motivation: 探索LLMs内部计算是否能可靠区分事实性输出和幻觉输出，因为现有研究表明LLMs在内部表征中编码了事实性信号，但也会产生基于捷径或虚假关联的事实错误。

Method: 对LLMs处理事实查询进行机制分析，比较两种基于主题信息依赖的幻觉类型，分析隐藏状态几何特征。

Result: 当幻觉与主题知识相关时，LLMs使用与正确回答相同的内部回忆过程，隐藏状态几何重叠且不可区分；而与主题知识无关的幻觉产生独特的聚类表示，使其可检测。
...
TL;DR: 提出了Alif-1.0-8B-Instruct模型，这是一个针对低资源乌尔都语的多语言模型，通过改进的自指导技术创建高质量合成数据集，在100美元预算内显著提升了乌尔都语任务性能。
...
Conclusion: 证明使用改进的自指导方法可以高效开发高性能、低资源语言的LLM，并实现文化对齐，训练成本低于100美元。
...
TL;DR: ReFIne训练框架通过监督微调和GRPO优化推理模型的可信度，包括可解释性、忠实性和可靠性，在数学基准测试中显著提升这些维度。


<details>
  <summary>Details</summary>
Motivation: 现有长链思维推理研究过于关注答案准确性和token效率，而忽视了可信度这一关键方面。作者认为实用的推理系统必须具备可解释性、忠实性和可靠性。

Method: 提出ReFIne训练框架，结合监督微调和GRPO，鼓励模型：(i)生成结构化、基于标签的推理轨迹；(ii)明确披露指导每个解决方案的关键信息；(iii)提供推导过程和最终答案置信度的自我评估。

Result: 在Qwen3模型(1.7B/4B/8B)上的实验表明，ReFIne模型生成更清晰的结构化推理轨迹(可解释性+44.0%)，更忠实地暴露决策过程(忠实性+18.8%)，并提供信息丰富的置信度估计(可靠性+42.4%)。

Conclusion: 推理模型不仅应优化准确性，还应优化可信度的更广泛维度。ReFIne框架为构建可信赖的推理系统提供了重要方向。
...
TL;DR: 提出基于因果语言模型(CLM)的语义框架归纳新方法FrameEOL，通过提示工程获取框架嵌入，结合上下文学习和深度度量学习，在英语和日语FrameNet数据集上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 因果语言模型(如GPT、Llama系列)在多种语言理解任务中表现出色，但尚未应用于语义框架归纳任务。现有方法主要使用掩码语言模型(MLM)，而CLM在对话中表现出对框架的理解能力。

Method: 提出FrameEOL方法：1) 使用提示工程获取框架嵌入，输出代表给定情境的框架名称标签；2) 结合上下文学习(ICL)和深度度量学习(DML)获得更适合框架归纳的嵌入；3) 对嵌入进行聚类完成框架归纳。

Result: 在英语和日语FrameNet数据集上的实验表明，该方法优于现有框架归纳方法。特别是对于缺乏丰富框架资源的日语，仅使用5个ICL示例的CLM方法就达到了与使用DML微调的MLM方法相当的性能。

Conclusion: 证明了CLM在语义框架归纳任务中的有效性，为资源匮乏语言提供了有效的框架归纳解决方案。
...
TL;DR: 本文对检索增强生成(RAG)进行了全面综述，分析了RAG的目标、组件、挑战和应用场景，指出在LLMs能力不断增强的背景下需要重新思考RAG的角色。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型(LLMs)能力的不断提升，传统RAG框架的相对优势变得不那么明显，需要重新评估RAG在增强LLMs处理动态信息和领域特定查询方面的价值。

Method: 采用综述研究方法，首先分析RAG的总体目标和核心组件，然后识别RAG面临的关键挑战和局限性，最后展示RAG与LLMs结合的应用案例。

Result: 识别了RAG的关键弱点，展示了在LLMs单独表现不佳但RAG能显著增强其效果的多个应用场景，为下一代RAG系统的发展提供了方向。

Conclusion: RAG在与LLMs结合时仍能显著提升处理动态和领域特定信息的能力，需要重新思考RAG的角色并开发新一代RAG系统。
...
<details>
  <summary>Details</summary>
Motivation: 现有机器翻译评估主要依赖表面指标，无法准确评估网络小说翻译的特有特征，需要专门针对该领域的评估框架。

Method: 构建包含18K专家标注中英句对的DITING评估框架，提出基于推理的多智能体评估方法AgentEval，开发MetricAlign元评估数据集。

Result: 评估14个模型发现，中文训练的LLM优于更大的国外模型，DeepSeek-V3在忠实度和风格一致性方面表现最佳。

Conclusion: 建立了LLM网络小说翻译的新范式，为未来研究提供了公开资源。
...
TL;DR: 通过添加思考式话语(TAU)来增强对话数据，使LLM能更好地模拟说话者的个性特征，特别是在大五人格中的宜人性和神经质维度上表现更好。


<details>
  <summary>Details</summary>
Motivation: 通过思考式话语(TAU)来增强对话数据，期望训练出的个性化LLM能更好地模拟说话者的人格特质。

Method: 在文本对话数据中增加思考式话语(TAU)，即说话者在表达话语前的思维过程，然后训练个性化LLM。

Result: 使用TAU增强数据训练的LLM在大五人格的宜人性和神经质维度上更接近说话者的真实人格特质，且TAU增强的质量影响模型性能。

Conclusion: 思考式话语增强能有效提升LLM对个体人格特质的建模能力，特别是在宜人性和神经质维度上效果显著。
...
TL;DR: 本文提出了两种增强文本去标识化攻击的策略：通过多排序聚合预测和使用推理模型来提升重新识别性能。
...
Result: 多排序聚合和推理模型都能显著提升重新识别性能，特别是在攻击者拥有丰富背景知识的情况下。

Conclusion: 这两种策略可以构建更强的重新识别攻击，为评估文本去标识化方法的鲁棒性提供了有效工具。
...
<details>
  <summary>Details</summary>
Motivation: 通用大语言模型擅长推理，但专门优化的翻译模型在推理任务上表现不佳。需要一种方法既能提升翻译能力，又不损害原有的推理能力。

Method: 从指令模型开始，仅在平行数据上应用层选择性调优，训练Qwen3-XPlus模型。

Result: 在高低资源语言翻译上均有显著提升，低资源语言如斯瓦希里语达到15+ spBLEU和40+ xComet。仅用少量平行数据，在7个多语言任务上平均提升1+分，同时在15个推理数据集上保持与Qwen3指令模型相当的水平。
...
TL;DR: DICE是一个轻量级框架，通过小语言模型(SLMs)来修正大语言模型(LLMs)的输出，使其符合结构化格式要求，同时保持LLMs的知识和推理能力。


<details>
  <summary>Details</summary>
Motivation: LLMs在执行推理任务时往往优先考虑推理过程而忽视详细的输出格式要求，直接微调LLMs成本高昂且参数访问受限。

Method: DICE框架首先让LLMs生成自然语言响应，然后使用经过训练的SLMs通过思维链(CoT)修正来分析和精炼这些输出，使其符合结构化规范。采用两阶段方法构建结构化CoT适应数据集，并应用双重调优策略来微调SLMs。

Result: 实验表明DICE将LLM输出的格式准确性和内容正确性分别提高了35.4%和29.4%，达到了最先进的性能。

Conclusion: DICE框架有效解决了LLMs在遵循结构化输出格式方面的问题，在保持LLMs优势的同时显著提升了输出质量。
...
TL;DR: IRIS是一个新颖的实时因果发现框架，通过结合统计算法和LLM方法，能够从初始变量集自动发现已知和新颖的因果关系，并识别缺失变量来扩展因果图。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法面临数据收集成本高、对已知关系重复计算、假设不现实等挑战，而现有LLM方法只能发现常见因果关系，无法发现新颖关系。

Method: IRIS框架从初始变量集自动收集相关文档、提取变量、发现因果关系，采用混合方法结合统计算法和LLM方法，并包含缺失变量提议组件来扩展因果图。

Result: 该方法实现了仅从初始变量集进行实时因果发现，无需预存数据集，能够发现已知和新颖的因果关系。

Conclusion: IRIS解决了传统因果发现方法的局限性，提供了一个无需预存数据集的实时因果发现解决方案，能够同时处理已知和新颖的因果关系。
...
TL;DR: 本文提出了CrisiText，这是第一个用于在13种不同危机场景下生成警告信息的大规模数据集，包含超过40万条警告信息，旨在帮助平民在危机期间和之后获得及时指导。


<details>
  <summary>Details</summary>
Motivation: 在自然灾害或暴力袭击等危机情况下，有效识别威胁并减轻其潜在损害至关重要。虽然AI已被用于协助人类应对紧急情况，但NLP技术的应用仍然有限，主要集中在分类任务上，而利用NLG架构生成及时警告信息的巨大潜力在很大程度上被忽视了。

Method: 从现有的危机描述出发，创建与场景相关的事件链，每个事件都与警告信息配对。生成过程遵循专家编写的指南，确保术语正确性和建议的事实性。此外，每条信息都附带三种次优警告类型，以研究不同的NLG方法。

Result: 创建了包含超过40万条警告信息的CrisiText数据集，涵盖近18,000个危机情况。通过一系列实验比较了监督微调设置与偏好对齐、零样本和少样本方法，并评估了模型在分布外场景下的性能以及自动后编辑器的有效性。

Conclusion: CrisiText数据集填补了危机情况下NLG警告信息生成的空白，为研究不同NLG方法提供了基础，并展示了在危机响应中生成及时、准确警告信息的可行性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法要么依赖提示激发模型内在能力，要么在复杂交互任务中应用RL时存在性能瓶颈和崩溃问题，未能充分发挥模型的代理潜力

Method: DSPO（动态过滤序列级策略优化）算法，通过序列级优化和动态样本过滤进行鲁棒代理训练，纯RL训练实现多轮搜索与推理交错

Result: 在多个QA基准测试中，7B模型比同类先前工作提升34.1%，在HotpotQA等复杂多跳QA任务中甚至超过先前工作的14B模型近9%，保持出色的训练稳定性

Conclusion: DSPO算法能有效提升LLM在复杂任务中的外部知识搜索能力，无需监督演示数据，训练稳定且性能优越
...
TL;DR: 本文提出了Self-Critique方法，这是首个针对RL后训练阶段数据污染检测的系统性研究，通过探测模型策略崩溃来检测污染数据，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 随着RL后训练在提升LLM推理能力中变得日益重要，但缺乏专门针对该阶段的数据污染检测方法，这构成了关键漏洞。现有方法主要关注预训练和SFT阶段，而RL阶段的污染检测研究存在空白。

Method: 提出Self-Critique方法，基于关键观察：RL阶段后LLM的输出熵分布会崩溃为高度特定和稀疏的模式。该方法通过探测底层策略崩溃（模型收敛到狭窄推理路径）来检测污染。还构建了RL-MIA基准来模拟特定污染场景。

Result: 广泛实验表明，Self-Critique在多个模型和污染任务中显著优于基线方法，AUC提升高达30%。而现有方法在RL阶段污染检测上接近随机猜测，本方法使检测成为可能。

Conclusion: Self-Critique有效解决了RL后训练阶段的数据污染检测问题，填补了该领域的研究空白，为可靠评估LLM性能提供了重要工具。
...
TL;DR: 提出了CFVBench基准测试，评估多模态检索增强生成模型在视频理解中的表现，发现现有模型在捕捉细粒度多模态细节方面存在瓶颈，并提出了自适应视觉优化框架来提升性能。
...
Method: 构建了包含599个公开视频、5360个开放式问答对的大规模手动验证基准CFVBench，并提出了自适应视觉优化框架，通过自适应增加帧采样密度和选择性调用外部工具来提升细粒度理解。

Result: 系统评估了7种检索方法和14个常用MLLM，发现即使GPT5或Gemini等先进模型也难以捕捉瞬态但关键的细粒度多模态细节。AVR框架在所有评估的MLLM中都能持续提升细粒度多模态理解和性能。

Conclusion: 当前多模态大语言模型在细粒度多模态细节理解方面存在显著瓶颈，提出的自适应视觉优化框架能有效缓解这一问题，提升模型在复杂视频内容理解上的表现。
...
TL;DR: DyReMe是一个动态医疗诊断基准，通过生成类似真实咨询的病例来评估大语言模型，比静态医学考试基准更能反映真实临床实践。


<details>
  <summary>Details</summary>
Motivation: 当前基于静态医学考试的大语言模型评估与真实临床实践存在根本性偏差，无法反映现实世界中模糊多变的医疗条件。

Method: 提出DyReMe动态基准，生成包含干扰因素（如鉴别诊断、常见误诊因素）的新鲜咨询式病例，并评估准确性、真实性、帮助性和一致性四个临床相关维度。

Result: 实验表明动态评估方法更具挑战性和真实性，揭示了最先进大语言模型性能与真实临床实践之间的显著偏差。

Conclusion: 迫切需要能够更好反映可信医疗诊断需求的评估框架，DyReMe为这一目标提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在数据稀缺领域训练专家LLM很困难，通常依赖多项选择题。但标准的基于结果的RL存在风险，可能提高准确性但损害推理质量（如逻辑一致性）。现有监督推理的解决方案（如大规模过程奖励模型）成本过高。

Method: CLARity框架包含：一致性感知奖励机制、两阶段精炼-监控训练流程（增强推理一致性）、动态数据重构策略（更好利用有限数据），仅使用小型通用LLM。

Result: 实验显示CLARity相比基线提高了16.5%的响应一致性和7.5%的准确性。人工评估进一步证实了连贯性和专业性的整体改进。
...
<details>
  <summary>Details</summary>
Motivation: LLM预训练评估存在显著不稳定性，掩盖了真实的学习动态，阻碍了模型发展

Method: MaP框架结合检查点合并（平滑参数空间）和Pass@k指标（提供低方差统计估计）

Result: 实验表明MaP能产生更平滑的性能曲线，减少运行间方差，确保更一致的模型排名

Conclusion: MaP为观察LLM训练动态提供了更可靠和忠实的视角，为LLM研究奠定了关键实证基础
...
TL;DR: ShiZhi是首个专门用于法院观点生成的大语言模型，在中文法院观点生成数据集CCVG上取得了58.5 BLEU-1分数和86.1%的罪名预测准确率。
...
Conclusion: 实验结果表明，即使是一个小型大语言模型，在高质量领域特定数据上训练后也能生成合理且法律连贯的法院观点。
...
TL;DR: MaskKV是一种专为扩散大语言模型设计的训练无关缓存淘汰框架，通过掩码查询引导的评分机制和自适应缓存预算策略，显著减少KV缓存内存占用，在仅保留5%令牌的情况下保持94%性能，实现31倍加速。


<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型的双向注意力缓存机制需要大量内存，限制了在资源受限环境下处理长上下文的能力。现有的缓存淘汰策略专为自回归模型设计，忽略了扩散模型的独特特性，导致性能不佳。

Method: 提出MaskKV框架，包含两个关键创新：1）掩码查询引导的评分机制，利用注意力权重识别和淘汰每个注意力头中不太重要的提示令牌；2）自适应缓存预算策略，减少中间层分配，将资源集中在偏好提示的注意力头上。

Result: 在LLaDA模型上，将KV缓存压缩到仅256对（少于5%的令牌），在LongBench基准上保留了94%的全缓存性能，在32k提示长度下实现了高达31倍的加速。

Conclusion: MaskKV有效解决了扩散大语言模型的缓存内存问题，通过专门设计的缓存淘汰策略显著提升了模型在长上下文处理中的效率和性能。
...
TL;DR: 提出了一种基于电路推理验证的白盒方法，通过分析正确与错误推理步骤在计算图结构上的差异来验证推理过程，相比黑盒和灰盒方法能提供更深入的错误原因分析。


<details>
  <summary>Details</summary>
Motivation: 现有的CoT验证方法基于输出或激活状态，对计算失败原因提供有限洞察，需要一种能深入理解推理过程失败原因的白盒方法。

Method: 使用基于电路推理验证的白盒方法，通过训练分类器分析正确和错误推理步骤在计算图结构上的特征差异，识别推理错误的特定结构指纹。

Result: 发现错误的结构特征具有高度预测性，在不同推理任务中表现出特定计算模式，且通过针对性干预能成功修正模型的错误推理。

Conclusion: 通过深入分析模型的计算过程，可以从简单的错误检测转向对LLM推理的因果理解，为推理验证提供了新的科学视角。
...
TL;DR: 提出了细粒度低秩压缩器（FLRC），通过为每层分配最优秩并采用渐进式低秩解码，在保持文本生成质量的同时显著提升LLM压缩性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型参数量庞大，难以在资源受限硬件上部署。现有低秩压缩方法采用统一压缩率会导致性能显著下降，且在解码阶段表现不佳。

Method: FLRC方法：1）高效确定每层的最优秩分配；2）结合渐进式低秩解码来维持文本生成质量

Result: 在多样化基准测试中，FLRC在摘要任务上的ROUGE-L指标比最先进的低秩压缩方法提升高达17%，建立了更鲁棒高效的LLM推理框架。

Conclusion: FLRC通过细粒度秩分配和渐进式解码策略，为LLM低秩压缩提供了更有效的解决方案，显著提升了压缩后的模型性能。
...
TL;DR: LLP是首个基于大语言模型的二手产品定价生成框架，通过检索相似产品、两阶段优化和置信度过滤，显著提升定价准确率和采纳率。
...
Method: 使用LLM框架，先检索相似产品，然后通过双向推理构建的数据集进行监督微调和组相对策略优化的两阶段优化，并采用置信度过滤机制。

Result: 在闲鱼平台上部署后，在30%产品覆盖率下，静态采纳率从40%提升到72%，在90%召回率下仍保持47%的采纳率。

Conclusion: LLP框架显著优于现有方法，能很好地泛化到未见过的类别，在实际部署中取得了显著效果。
...
TL;DR: ReTraceQA是一个新的基准测试，通过过程级评估来评估常识推理任务，发现SLMs在14-24%的情况下虽然给出正确答案但推理过程有缺陷，表明仅关注最终答案的评估方法高估了SLMs的能力。


<details>
  <summary>Details</summary>
Motivation: 当前对小语言模型(SLMs)的评估主要依赖最终答案的准确性，忽视了推理过程的有效性，这可能导致对模型能力的错误估计。

Method: 引入ReTraceQA基准测试，使用专家标注的数据集进行过程级评估，并采用大型语言模型(LLMs)作为自动评判器进行推理感知评估。

Result: 研究发现SLMs在14-24%的情况下虽然给出正确答案但推理过程有缺陷；当使用LLMs进行推理感知评估时，SLMs在所有模型和数据集上的性能显著下降，得分最多降低25%。

Conclusion: 仅关注最终答案的评估方法会高估SLMs的能力，过程级评估对于准确评估语言模型的推理能力至关重要。
...
TL;DR: ThinkLogit是一种无需训练的解码时方法，使用小型推理模型引导大型非推理模型进行长链推理，通过logit算术实现性能提升。


<details>
  <summary>Details</summary>
Motivation: 研究大型推理模型的长链推理能力是否可以在无需额外训练的情况下被激发，避免昂贵的后训练成本。

Method: 提出ThinkLogit方法，利用logit算术将小型推理模型作为引导器来调优大型目标模型；进一步开发ThinkLogit-DPO，通过偏好优化训练引导器模型。

Result: 在五个推理基准测试中，ThinkLogit和ThinkLogit-DPO分别实现了24.5%和29.1%的相对准确率提升，使用21倍小的引导模型。

Conclusion: ThinkLogit方法有效且实用，可在不同模型家族间工作，与小型模型的后训练方法正交，为解锁大型模型的长推理能力提供了低成本路径。
...
### [176] [Identifying & Interactively Refining Ambiguous User Goals for Data Visualization Code Generation](https://arxiv.org/abs/2510.09390)
*Mert İnan,Anthony Sicilia,Alex Xie,Saujas Vaduguru,Daniel Fried,Malihe Alikhani*

Main category: cs.CL

TL;DR: 本文研究人机通信中的目标共享问题，特别关注数据可视化领域中的自然语言歧义如何影响代码生成。作者开发了歧义分类法和量化指标，并通过多轮对话策略减少歧义，提高代码准确性。
...
Method: 开发歧义分类法和量化指标，使用DS-1000数据集中的Matplotlib问题进行评估。探索三种语用模型（Gricean合作原则、话语表征理论、问题讨论）来指导多轮对话策略，通过模拟用户研究评估效果。

Result: 提出的歧义指标比不确定性基线更好地与人工标注相关。多轮对话策略能够有效减少歧义，提高代码准确性，使生成的代码更好地匹配用户目标。

Conclusion: 多轮对话在代码生成中具有重要价值，能够通过减少歧义来改善人机通信效果。语用模型为设计有效的对话策略提供了理论基础。
...
TL;DR: NL2GenSym框架将大语言模型与SOAR认知架构结合，通过执行引导的生成器-评判器机制，从自然语言自动生成符号规则，解决了SOAR手动编码规则的瓶颈。


<details>
  <summary>Details</summary>
Motivation: SOAR认知架构需要手动编码规则，阻碍了实际应用。大语言模型具有生成规则的潜力，但缺乏实验验证。

Method: 提出执行引导的生成器-评判器机制：LLM生成器基于检索增强生成访问自演进知识库提出规则，在SOAR环境中执行验证，LLM评判器基于执行反馈迭代优化规则。

Result: 在Water Jug Problem数据集上，成功率达到86%以上，生成新颖启发式规则，将平均决策周期降至最优解的1.98倍，是基线方法的1/1000。小参数模型性能优于大模型。

Conclusion: NL2GenSym框架有效解决了SOAR规则生成问题，展示了LLM与符号AI结合的潜力，为构建通用智能体提供了新途径。
...
TL;DR: 提出了一个多尺度图思维链提示框架MSGCOT，通过整合多尺度结构信息来增强图提示调优的语义多样性，在少样本场景下显著优于现有单粒度方法。
...
Method: 设计轻量级低秩粗化网络捕捉多尺度结构特征作为层次基向量，模仿人类从粗到细的认知过程，在每一步推理中动态整合多尺度信息形成渐进式提示链。

Result: 在八个基准数据集上的实验表明，MSGCOT优于最先进的单粒度图提示调优方法，特别是在少样本场景下表现优异。
...
### [179] [Understanding the Effects of Domain Finetuning on LLMs](https://arxiv.org/abs/2510.09359)
*Eshaan Tanwar,Deepak Nathani,William Yang Wang,Tanmoy Chakraborty*

Main category: cs.CL

TL;DR: 本文首次系统研究医学领域大语言模型的领域特定微调机制，发现微调仅改变表示子空间的小部分，提出tuning vectors框架来捕获参数方向性变化，这些向量对提升指令遵循和生成质量至关重要。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注自回归或通用指令模型，领域专业化大语言模型的微调机制尚未得到充分理解，特别是微调如何重塑参数空间的内在机制不明确。

Method: 提出tuning vectors框架，受task vectors启发，显式捕获微调引起的参数方向性偏移，分析这些向量在不同模型层的分布和影响。

Result: 发现微调主要修改表示子空间的小部分，tuning vectors能提升指令遵循和生成质量，跨领域组合可改善泛化能力，MLP层写入新方向信息，注意力头放大现有方向。

Conclusion: 研究为大语言模型适应提供了新见解，并提供了一个通用、可解释的分析专业化大语言模型的框架。
...
TL;DR: 该论文提出了实体提及重构框架，通过任务向量方法研究大语言模型如何内部表示和操作实体，特别是多标记实体。


<details>
  <summary>Details</summary>
Motivation: 尽管命名实体是文本知识的基础构件，但大语言模型如何内部表示实体仍不清楚。现有研究主要关注显式关系，对实体表示本身知之甚少。

Method: 引入实体提及重构框架，利用任务向量从LLM隐藏状态的各种实体表示中一致生成多标记提及，扩展logit-lens为Entity Lens来预测多标记提及。

Result: 研究结果表明LLM开发了实体特定机制来表示和操作任何多标记实体，包括训练期间未见过的实体。

Conclusion: 该工作为理解LLM内部实体表示提供了新证据，表明模型能够发展出实体特定的表示机制。
...
TL;DR: TEPO是一种新颖的token级框架，通过马尔可夫似然将组级奖励与token连接，解决了GRPO等熵正则化方法在思维链稀疏token奖励中的挑战，显著提升了数学推理性能和训练稳定性。
...
Result: TEPO在关键指标（包括@k和准确率）上一致优于现有基线，在数学推理任务上创下新纪录，并显著提升训练稳定性。

Conclusion: TEPO通过token级马尔可夫似然框架有效解决了稀疏奖励问题，为LLM推理能力提升提供了更稳定高效的优化方法。
...
TL;DR: 该论文比较了端到端口语对话状态跟踪中不同的上下文管理策略，发现提供完整口语对话历史作为输入能获得最佳性能，而基于注意力池化的压缩方法能在保持竞争力的同时减少上下文大小。
...
Method: 系统评估了三种上下文管理方法：传统多模态上下文（结合文本历史和当前口语轮次）、完整口语历史、以及压缩口语历史方法，并在SpokenWOZ语料库上进行实验。

Result: 完整口语对话作为输入在相似规模模型中表现最佳，显著超越先前方法；基于注意力池化的压缩方法在保持竞争力的准确率的同时减少了上下文大小。

Conclusion: 改进主要源于更有效的上下文利用，完整口语历史输入策略在口语对话状态跟踪中表现最优，而压缩方法提供了良好的性能与效率平衡。
...
<details>
  <summary>Details</summary>
Motivation: 现代对话系统依赖预定义策略，但随着系统扩展，这些策略变得复杂冗长，导致遵循困难且计算成本高。多模态策略管理研究不足，现有方法主要关注文本压缩或安全规则。

Method: 提出TriMPI三阶段训练框架：1)通过持续预训练注入策略知识；2)监督微调；3)PolicyRollout强化学习，在rollout中增强策略感知响应以实现有基础的探索。
...
<details>
  <summary>Details</summary>
Motivation: 现有的大语言模型评估和基准测试方法依赖完全标注的数据集，标注成本高昂。需要一种能够用有限标注有效识别最佳LLM的方法。

Method: LLM SELECTOR框架自适应选择对任务最有信息量的少量查询进行标注，并利用基于评判者的标注模型进一步降低标注成本。

Result: 在6个基准测试和151个LLM上的实验表明，LLM SELECTOR在选择最佳和接近最佳LLM时，标注成本降低了高达59.62%。

Conclusion: LLM SELECTOR提供了一种高效的LLM选择方法，显著减少了标注需求，为实际应用中的模型选择提供了实用解决方案。
...
TL;DR: 提出GRSP方法，通过段级惩罚机制来减少大型推理模型在强化学习中的过度思考问题，平衡计算效率和模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于验证器奖励的强化学习方法虽然能减少token消耗，但往往损害模型性能，这是因为token级监督过于简单。需要更细粒度的监督来平衡效率和准确性。

Method: 提出Group Relative Segment Penalization (GRSP)，这是一种段级正则化方法。通过分析发现推理段与token消耗和模型性能强相关，设计了跨段簇的长度感知加权机制。

Result: 实验表明GRSP在保持较高准确率的同时实现了优越的token效率，特别是在处理更难问题时优势更明显。此外，GRSP稳定了RL训练并在不同模型规模上有效扩展。

Conclusion: GRSP通过段级监督有效解决了大型推理模型在强化学习中的过度思考问题，在计算效率和模型性能之间取得了良好平衡。
...
<details>
  <summary>Details</summary>
Motivation: 扩散大语言模型(dLLMs)由于能够并行解码多个token而成为自回归模型的高效替代方案，但因其不可处理的对数似然而难以通过强化学习与人类偏好或任务特定奖励对齐。现有方法使用ELBO等单边近似会引入显著的策略梯度偏差。

Method: 提出了Sandwiched Policy Gradient (SPG)方法，利用真实对数似然的上界和下界来减少策略梯度偏差，相比仅使用ELBO或一步估计的方法更加准确。

Result: SPG在多个基准测试中显著优于基于ELBO或一步估计的基线方法：在GSM8K上准确率提升3.6%，在MATH500上提升2.6%，在Countdown上提升18.4%，在Sudoku上提升27.0%。

Conclusion: SPG方法通过同时利用对数似然的上界和下界，有效解决了dLLMs强化学习中的策略梯度偏差问题，在各种推理任务中取得了显著的性能提升。
...
TL;DR: KORMo-10B是首个基于合成数据训练的大规模韩英双语开源大语言模型，证明了合成数据在大规模预训练中的可行性。


<details>
  <summary>Details</summary>
Motivation: 针对韩语等非英语语言资源匮乏的问题，探索使用合成数据构建完全开源的双语大语言模型。

Method: 从头训练10.8B参数的韩英双语模型，其中68.74%的韩语数据为合成数据，采用平衡语言覆盖和多样化指令风格的精心策划方法。

Result: 模型在推理、知识和指令遵循等广泛基准测试中表现与当代开源多语言基线相当，未出现模型崩溃或不稳定性。

Conclusion: 合成数据可可靠支持长期预训练，双语指令调优能实现接近母语的韩语推理和语篇连贯性，为低资源语言开发提供了透明框架。
...
TL;DR: Dyna-Mind是一个两阶段训练框架，通过教导AI代理进行心理模拟来提升在复杂交互环境中的表现。第一阶段ReSim训练代理从真实经验生成结构化推理轨迹，第二阶段Dyna-GRPO使用强化学习进一步强化模拟和决策能力。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理在数学和编程方面表现出色，但在长视野交互任务中表现不佳。受人类认知启发，需要让AI具备"替代性试错"能力，即行动前心理模拟替代未来，以提升复杂环境中的理解和表现。

Method: 两阶段框架：1) ReSim：从环境交互收集的真实经验构建扩展搜索树，训练代理生成结构化推理轨迹；2) Dyna-GRPO：在线强化学习方法，使用结果奖励和中间状态作为反馈来加强模拟和决策能力。

Result: 在Sokoban、ALFWorld和AndroidWorld三个基准测试中验证：1) ReSim有效向AI代理注入模拟能力；2) Dyna-GRPO利用结果和交互级信号学习更好的长视野规划密集型任务策略。

Conclusion: 模拟在使AI代理在日益挑战的环境中更有效推理、规划和行动方面发挥着核心作用。
...
### [189] [Domain-Adapted Pre-trained Language Models for Implicit Information Extraction in Crash Narratives](https://arxiv.org/abs/2510.09434)
*Xixi Wang,Jordanka Kovaceva,Miguel Costa,Shuai Wang,Francisco Camara Pereira,Robert Thomson*

Main category: cs.CL

TL;DR: 本文研究了使用紧凑型开源预训练语言模型从交通事故叙述中进行推理密集型信息提取，包括碰撞方式和车辆事故类型识别，通过微调技术超越GPT-4o等闭源大模型性能。


<details>
  <summary>Details</summary>
Motivation: 现有工具难以批量处理非标准化的交通事故叙述文本，闭源大模型在推理密集型任务上表现不佳且存在隐私问题，缺乏领域知识导致性能受限。

Method: 使用LoRA和BERT微调技术向紧凑型开源预训练语言模型注入任务特定知识，在权威真实数据集CISS上进行实验。

Result: 微调后的紧凑模型在碰撞方式和事故类型识别任务上超越了GPT-4o等强闭源大模型，且仅需极少的训练资源，还能捕获更丰富的叙述细节并纠正数据集中的错误标注。

Conclusion: 紧凑型开源预训练语言模型通过适当微调可以有效支持交通事故叙述中的推理密集型信息提取，在性能、隐私和资源效率方面均优于闭源大模型。
...
### [190] [Prompting Test-Time Scaling Is A Strong LLM Reasoning Data Augmentation](https://arxiv.org/abs/2510.09599)
...
<details>
  <summary>Details</summary>
Motivation: 传统方法需要收集大量推理数据集，这既费时又资源密集。P-TTS旨在通过少量样本和推理时数据增强来低成本地提升LLM的推理能力。

Method: 使用仅90个手动选择的推理实例，通过系统化改变提示强度来合成多样化的推理轨迹上下文，然后对各种规模的Qwen-2.5模型进行微调。

Result: 在多个数学推理基准测试中，P-TTS-7B和32B模型显著优于基线模型，在AIME'24和AIME'25上分别获得+26.66%到+30.00%的绝对准确率提升，并在其他基准测试中表现相当或更好。
...
TL;DR: 本文提出了一个用于Apertus LLM训练数据的全文索引流水线，使用Elasticsearch在arm64超级计算集群上成功索引了8.6T tokens，创建了LLM安全工具和离线网络搜索引擎。


<details>
  <summary>Details</summary>
Motivation: 尽管开源LLM模型增多，但其训练数据仍难以访问和分析，特别是包含从互联网抓取的关键数据，这对LLM安全性和透明度构成挑战。

Method: 利用Elasticsearch并行索引和Alps基础设施（高效arm64超级计算集群），构建了大规模LLM训练数据的全文索引流水线。

Result: 成功索引了Apertus LLM家族训练数据中的8.6T tokens（共15.2T），证明了在arm64架构上进行大规模全文索引的可行性，并实现了jailbreak无关的LLM安全机制。

Conclusion: 该研究证明了在下一代arm64基础设施上进行大规模数据索引的可行性，为LLM安全提供了新工具，并推动了绿色计算的转型。
...
TL;DR: LLMs在逻辑推理中表现出递归能力但缺乏组合性，为此提出结合符号推理与神经计算的混合架构来提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 区分神经模型的两种泛化能力：组合性（抽象原子逻辑规则）和递归性（迭代应用推理规则），研究LLMs在逻辑推理中的局限性。

Method: 使用三段论片段作为基准测试LLMs的逻辑泛化能力，提出集成符号推理与神经计算的混合架构。

Result: LLMs在递归性方面表现良好，但在组合性方面存在显著困难。混合架构在保持高效率的同时实现了稳健推理。

Conclusion: 混合模型能有效解决神经推理系统中的关键泛化障碍，为可靠逻辑证明器提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注数学和逻辑推理，而统计学作为一个独特且综合的学科在基准测试中尚未得到充分探索。

Method: 设计了一个可扩展的多智能体流水线，结合人工验证，自动化大规模问题提取、重写和质量控制。

Result: 实验结果显示，闭源模型如GPT5-mini在研究级问题上准确率低于57%，开源模型表现更差。

Conclusion: StatEval揭示了统计推理的独特挑战和当前LLMs的局限性，有望成为推进大语言模型统计智能的严格基准。
...
TL;DR: 本文分析了无标签情况下模型性能估计的可靠性，发现基于大语言模型的错误预测器比基于数据漂移或零样本基线方法能产生更可靠和一致的性能排名。


<details>
  <summary>Details</summary>
Motivation: 理解NLP模型如何泛化时，无需标签估计模型性能是一个重要目标。现有方法基于数据集相似性或预测正确性，但它们在跨域时产生可靠性能排名的条件尚不明确。

Method: 采用两步评估设置，使用四个基础分类器和多个大语言模型作为错误预测器，在包含15个域的GeoOLID和Amazon Reviews数据集上进行实验。

Result: 实验表明，基于大语言模型的错误预测器比漂移基线和零样本基线产生更强、更一致的与真实准确率的排名相关性。当跨域性能差异较大且错误模型预测与基础模型真实失败模式一致时，排名更可靠。

Conclusion: 研究结果阐明了性能估计方法何时可信，为跨域模型评估中的使用提供了指导。
...
Method: 训练基于频谱图的口音分类器，识别关键口音特征区域并进行掩码，使用掩码后的频谱图进行数据增强。

Result: 在英语和波斯语实验中，使用Whisper模型验证了该方法能显著降低词错误率，并发布了首个波斯语多地区口音数据集。

Conclusion: 该方法有效提升了多语言ASR系统对口音和方言多样性的鲁棒性，为低资源语言多样性语言研究奠定了基础。
...
TL;DR: 提出了MulTypo多语言拼写错误生成算法，评估了18个开源LLM在5个下游任务中的抗错能力，发现拼写错误会显著降低性能，特别是在生成性和推理任务中，且不同语言间的抗错能力存在差异。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试大多假设输入是干净的，而真实应用中用户输入自然包含拼写错误，LLM在多语言环境下对拼写错误的鲁棒性研究不足。

Method: 开发了基于语言特定键盘布局和打字行为的MulTypo多语言拼写错误生成算法，评估了18个开源LLM在语言推理、多选问答、数学推理和机器翻译等5个任务上的表现。

Result: 拼写错误持续降低性能，特别是在生成任务和需要推理的任务中；指令微调提高了干净输入的性能但可能增加对噪声的脆弱性；高资源语言比低资源语言更鲁棒；从英语翻译比翻译成英语更鲁棒。

Conclusion: 研究强调了需要噪声感知训练和多语言鲁棒性评估，代码和数据已公开。
...
TL;DR: 扩散大语言模型存在并行-顺序矛盾，限制了其在复杂推理任务中的并行解码能力，导致效率下降和推理深度受限。


<details>
  <summary>Details</summary>
Motivation: 研究扩散大语言模型在并行解码与因果推理顺序之间的内在矛盾，分析其对推理效率和能力的限制。

Method: 通过行为分析识别并行-顺序矛盾，引入三个扩展维度（并行、扩散、顺序）进行实证研究，并提出缓解策略。

Result: 发现扩散大语言模型仅在直接可判定输出时展现真正并行性，任务难度增加时回归自回归行为，并行扩展带来持续改进但扩散和顺序扩展受限于并行-顺序矛盾。

Conclusion: 提出并行导向提示、扩散早停和并行扩展等实用缓解措施，以减少并行-顺序矛盾引起的无效和低效问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有系统要么将长视频压缩为粗糙嵌入，要么进行细粒度匹配时成本过高，需要解决多语言视频语料检索中的效率和精度问题。

Method: 将视频字幕分为语义连贯的块，用知识图谱事实丰富内容，构建分层树结构，使用语言无关的多语言编码器生成节点嵌入，采用粗到细的树搜索和轻量级LLM重排序。
...
TL;DR: 本文首次全面研究多语言思维链推理，评估了三个关键维度：性能、一致性和忠实度，揭示了LRMs在不同语言中的推理差异。


<details>
  <summary>Details</summary>
Motivation: 虽然大型推理模型越来越多地使用思维链推理来提高任务性能，但在多语言环境中，导致最终答案的中间思维轨迹本身仍未得到充分探索。

Method: 通过测量语言遵从性、答案准确性和一致性来评估LRMs；通过跨语言交换思维轨迹来评估一致性；采用截断和错误注入等扰动技术来探测思维轨迹的忠实度。

Result: 发现LRMs在不同语言中表现出强烈的语言偏好和差异性能；思维轨迹的质量和有效性因提示语言而异；模型在不同语言中对思维轨迹的依赖程度不同。

Conclusion: 多语言思维链推理存在显著的语言差异，需要更深入的研究来理解不同语言环境下的推理过程。
...
TL;DR: 该论文提出了WUGNECTIVES数据集，研究话语连接词是否能帮助语言模型进行世界知识推理，发现经过推理调优的模型表现更好，但所有模型在让步连接词上都表现不佳。


<details>
  <summary>Details</summary>
Motivation: 传统研究关注世界知识如何帮助预测话语连接词，本文反过来研究话语连接词是否能帮助语言模型理解世界知识。

Method: 构建包含8,880个刺激项的WUGNECTIVES数据集，评估17个不同规模和训练方式的语言模型在新实体推理任务上的表现。

Result: 经过推理调优的语言模型在大多数连接词类型上表现显著提升，但所有模型在表达让步含义的连接词上都表现不佳。

Conclusion: 话语连接词可以作为语言模型理解世界知识的线索，但模型对不同类型的连接词理解能力存在差异，为更细致研究语言线索功能提供了基础。
...
TL;DR: 提出了AutoPR任务，将研究论文转化为准确、吸引人且及时的宣传内容，并开发了PRAgent多智能体框架和PRBench基准测试，显著提升了学术传播效果。


<details>
  <summary>Details</summary>
Motivation: 随着同行评审研究数量激增，学者们依赖社交平台发现研究，作者需要投入大量精力推广工作以确保可见性和引用。为简化这一过程并减少人力依赖，需要自动化解决方案。

Method: 引入AutoPR任务，开发PRAgent多智能体框架，包含三个阶段：多模态内容提取、协作合成生成精炼输出、平台特定适配优化规范和标签。同时发布PRBench多模态基准测试，从保真度、参与度和对齐度三个维度评估系统。
...
<details>
  <summary>Details</summary>
Motivation: 解决实时口语语言模型在利用链式思维推理时因顺序生成整个思维过程而产生的高延迟问题，实现类似人类边说边思考的能力。

Method: 采用受大脑启发的双脑方法："构思脑"负责高层推理来引导和协调"表达脑"的流畅语音生成，实现分工协作。

Result: 在数学推理任务Spoken-MQA上达到92.8%准确率，在语音对话任务URO-Bench上获得82.5分，显著优于现有边说边思考方法，推理性能接近预计算完整CoT的模型。

Conclusion: MPS框架有效弥合了高质量推理与实时交互之间的差距，实现了高保真实时推理。
...
### [203] [Generating Sizing Fields for Mesh Generation via GCN-based Simplification of Adaptive Background Grids](https://arxiv.org/abs/2510.08645)
*Xunyang Zhu,Hongfei Ye,Yifei Wang,Taoran Liu,Jianjun Chen*

Main category: cs.GR

TL;DR: 提出基于图卷积网络的自适应背景网格简化框架，将网格简化任务转化为边评分回归问题，显著减少网格元素数量并提升尺寸场查询效率


<details>
  <summary>Details</summary>
Motivation: 创建几何贴合、计算轻量且无伪影的最优背景网格是控制非结构化网格生成质量和效率的关键挑战

Method: 使用图卷积网络将网格简化重构为边评分回归问题，通过定制损失函数综合考虑几何保真度和尺寸场精度，替代昂贵的程序化评估

Result: 简化后的背景网格元素减少74%-94%，尺寸场查询时间降低35%-88%，在多样复杂工程模型中均表现有效

Conclusion: 该数据驱动方法能高效生成优化的背景网格，显著提升非结构化网格生成的效率和质量
...
### [204] [A 3D Generation Framework from Cross Modality to Parameterized Primitive](https://arxiv.org/abs/2510.08656)
*Yiming Liang,Huan Yu,Zili Wang,Shuyou Zhang,Guodong Yi,Jin Wang,Jianrong Tan*

Main category: cs.GR

TL;DR: 提出了一种基于参数化基元的多阶段3D模型生成框架，能够在保持高质量表面的同时显著减少存储开销。


<details>
  <summary>Details</summary>
Motivation: 当前AI驱动的3D模型生成方法在跨模态方面取得进展，但生成平滑表面模型和最小化存储开销仍然是挑战。

Method: 采用多阶段框架，基于参数化基元的模型生成算法识别模型组成元素的形状特征，并用高质量表面的参数化基元替换元素，同时提出相应的模型存储方法。

Result: 在虚拟场景和真实场景数据集上的实验显示，Chamfer距离为0.003092，VIoU为0.545，F1-Score为0.9139，NC为0.8369，基元参数文件大小约为6KB。
...
### [205] [MCMC: Bridging Rendering, Optimization and Generative AI](https://arxiv.org/abs/2510.09078)
*Gurprit Singh,Wenzel Jakob*

Main category: cs.GR

TL;DR: 该论文探讨了马尔可夫链蒙特卡洛（MCMC）方法在生成式AI和基于物理渲染中的重要作用，提出将MCMC作为连接这两个研究领域的桥梁。


<details>
  <summary>Details</summary>
Motivation: 生成式AI在视觉语言模型方面取得了前所未有的进展，但将物理真实感引入基于扩散的生成模型的研究缺乏统一框架。MCMC方法在从复杂高维分布中采样方面特别有效，可以成为连接生成模型和基于物理渲染的桥梁。

Method: 通过理解MCMC方法在生成模型和基于物理渲染中的核心作用，探索MCMC如何作为统一框架连接这两个领域。课程提供理论工具和实践演示（Jupyter笔记本）。

Result: 提出了一个初步框架，展示了MCMC在生成式AI和基于物理渲染中的潜在桥梁作用，为研究人员提供了必要的理论和实践工具。

Conclusion: MCMC方法有潜力成为连接生成式AI和基于物理渲染的统一框架，为实现生成基于物理的渲染这一共同目标提供了理论基础和实践指导。
...
### [206] [Real-Time Rendering of Dynamic Line Sets using Voxel Ray Tracing](https://arxiv.org/abs/2510.09081)
*Bram Kraaijeveld,Andrei C. Jalba,Anna Vilanova,Maxime Chamberland*

Main category: cs.GR

TL;DR: 提出了一个高效的基于体素的射线追踪框架，用于渲染大规模动态线集，支持环境光遮蔽和真实透明度，在质量和性能上优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 动态线集的实时渲染在可视化任务中很重要，但高质量的全局光照和透明度在交互速率下难以实现。

Method: 开发了支持体素锥追踪和射线追踪加速结构的高效体素化算法，以及基于体素的剔除方法限制加速结构构建到相机可见体素。

Result: 方法在渲染（半）不透明动态线集时，在质量和性能上优于现有技术。

Conclusion: 提出的框架实现了大规模动态线集的高质量实时渲染，具有物理准确的透明度。
...
### [207] [Two-Stage Gaussian Splatting Optimization for Outdoor Scene Reconstruction](https://arxiv.org/abs/2510.09489)
*Deborah Pintani,Ariel Caputo,Noah Lewis,Marc Stamminger,Fabio Pellacini,Andrea Giachetti*

Main category: cs.GR

TL;DR: 提出两阶段高斯泼溅框架，分别优化前景和背景区域，提高室外场景重建质量


<details>
  <summary>Details</summary>
Motivation: 室外场景重建面临前景纹理丰富与背景细节稀疏、光照不均的对比问题

Method: 第一阶段在球形壳内初始化背景基元并优化，第二阶段初始化前景高斯并精炼，背景固定但参与最终渲染

Result: 在多样化室外数据集上减少背景伪影，提升感知质量，优于现有基线方法

Conclusion: 显式背景分离支持自动环境贴图估计，为真实感室外渲染和混合现实应用开辟新可能
...
### [208] [Hypothesis Hunting with Evolving Networks of Autonomous Scientific Agents](https://arxiv.org/abs/2510.08619)
*Tennison Liu,Silas Ruhrberg Estévez,David L. Bentley,Mihaela van der Schaar*

Main category: cs.AI

TL;DR: AScience框架通过LLM研究代理的自组织网络实现大规模科学假设探索，在癌症队列实验中能够持续产生多样化、高质量的新发现


<details>
  <summary>Details</summary>
Motivation: 大规模科学数据集为无约束的探索性发现提供了机会，但传统方法难以有效处理这种假设空间的累积搜索过程

Method: 提出AScience框架，将发现建模为代理、网络和评估规范的交互，实现为ASCollab分布式系统，包含具有异质行为的LLM研究代理，这些代理在共享评估标准下自组织成演化网络，持续产生和同行评审发现

Result: 实验显示这种社会动力学能够沿着多样性-质量-新颖性前沿积累专家评级结果，包括重新发现已建立的生物标志物、扩展已知通路和提出新的治疗靶点

Conclusion: 虽然湿实验室验证仍然不可或缺，但社会结构化、代理网络可以大规模持续进行探索性假设搜索
...
### [209] [Optimizing delivery for quick commerce factoring qualitative assessment of generated routes](https://arxiv.org/abs/2510.08671)
*Milon Bhattacharya,Milan Kumar*

Main category: cs.AI

TL;DR: 该研究提出使用大语言模型来评估车辆路径规划生成的配送路线，在印度电商物流场景下，开源LLM识别路线问题的准确率达79%，专有推理模型可达86%。
...
Method: 提出一个框架，使用LLM根据政策标准对VRP生成的路线进行批判性评估，让物流运营商能够评估和优先选择更高效的配送计划。生成了400个案例进行标注和评估。

Result: 开源LLM识别路线问题的准确率为79%，专有推理模型最高可达86%。LLM评估能够超越传统的距离和时间指标。

Conclusion: 基于LLM的VRP路线评估是一个有效且可扩展的评估层，对提高成本效率、配送可靠性和可持续性具有重要意义，特别适用于印度等发展中国家。
...
### [210] [Unified World Models: Memory-Augmented Planning and Foresight for Visual Navigation](https://arxiv.org/abs/2510.08713)
*Yifei Dong,Fengyi Wu,Guangyu Chen,Zhi-Qi Cheng,Qiyu Hu,Yuxuan Zhou,Jingdong Sun,Jun-Yan He,Qi Dai,Alexander G Hauptmann*

Main category: cs.AI

TL;DR: 提出了UniWM，一个统一、记忆增强的世界模型，将自我中心视觉前瞻和规划集成在单一多模态自回归主干中，显著提升了具身导航性能。


<details>
  <summary>Details</summary>
Motivation: 当前最先进的具身导航方法采用模块化架构，将导航规划与视觉世界建模分离，导致状态-动作不对齐，在新型或动态场景中适应性有限。

Method: UniWM通过单一多模态自回归主干统一整合视觉前瞻和规划，采用分层记忆机制结合短期感知线索和长期轨迹上下文，确保预测与控制紧密对齐。

Result: 在四个挑战性基准测试中，UniWM将导航成功率提升高达30%，显著减少轨迹误差，并在未见过的TartanDrive数据集上表现出优异的零样本泛化能力。

Conclusion: UniWM是实现统一、想象力驱动的具身导航的原则性步骤，证明了统一世界模型在提升导航性能和泛化能力方面的有效性。
...
### [211] [Robust Heuristic Algorithm Design with LLMs](https://arxiv.org/abs/2510.08755)
...
Method: 采用三个简单策略：(1)向LLM展示启发式算法表现不佳的实例；(2)解释这些表现不佳的原因；(3)针对输入空间特定区域进行专门化设计。

Result: 生成的启发式算法相比FunSearch在worst-case性能上提升约28倍，平均性能也有所改善，同时保持运行时间不变。
...
### [212] [COMPASS: Enhancing Agent Long-Horizon Reasoning with Evolving Context](https://arxiv.org/abs/2510.08790)
*Guangya Wan,Mingyang Ling,Xiaoqi Ren,Rujun Han,Sheng Li,Zizhao Zhang*

Main category: cs.AI

TL;DR: COMPASS是一个轻量级分层框架，通过将战术执行、战略监督和上下文管理分离为三个专门组件，解决了LLM代理在长时程任务中的上下文管理瓶颈问题，显著提升了任务准确性。


<details>
  <summary>Details</summary>
Motivation: 长时程任务需要持续推理和多次工具交互，这对LLM代理具有挑战性：小错误会在步骤间累积，即使最先进的模型也经常产生幻觉或失去连贯性。上下文管理被识别为核心瓶颈，过长的历史记录导致代理忽视关键证据或被无关信息分散注意力。

Method: 提出COMPASS框架，包含三个专门组件：(1)主代理执行推理和工具使用，(2)元思考者监控进度并发出战略干预，(3)上下文管理器为不同推理阶段维护简洁相关的进度简报。

Result: 在三个具有挑战性的基准测试（GAIA、BrowseComp和Humanity's Last Exam）中，COMPASS相比单代理和多代理基线将准确性提高了高达20%。

Conclusion: COMPASS通过专门的上下文管理有效解决了长时程任务中的推理挑战，并通过测试时扩展和训练后流水线进一步提升了性能和效率。
...
### [213] [Everyone prefers human writers, including AI](https://arxiv.org/abs/2510.08831)
*Wouter Haverals,Meredith Martin*

Main category: cs.AI

TL;DR: 研究发现人类和AI在评估文学风格时都存在系统性偏见，AI比人类表现出2.5倍更强的偏见，会系统性地贬低标记为AI生成的内容。
...
Method: 使用Raymond Queneau的《风格练习》进行对照实验，研究1比较人类参与者和AI模型在三种条件下（盲测、准确标记、反事实标记）的评估；研究2测试偏见在14×14矩阵的AI评估者和创作者中的普遍性。

Result: 人类显示+13.7个百分点的偏见，AI模型显示+34.3个百分点的偏见（2.5倍更强）。AI系统会系统性地贬低标记为AI生成的创意内容，归因标签导致评估者颠倒评估标准。
...
### [214] [What Is Your Agent's GPA? A Framework for Evaluating Agent Goal-Plan-Action Alignment](https://arxiv.org/abs/2510.08847)
*Allison Sihan Jia,Daniel Huang,Nikhil Vytla,Nirvika Choudhury,John C Mitchell,Anupam Datta*

Main category: cs.AI

TL;DR: 提出了Agent GPA框架，这是一个基于智能体目标-计划-行动操作循环的评估范式，包含五个评估指标，能够系统性地覆盖广泛的智能体失败情况。


<details>
  <summary>Details</summary>
Motivation: 需要一种系统性的方法来评估智能体的性能，特别是能够全面覆盖各种失败模式并支持针对性改进的评估框架。

Method: 开发了基于目标-计划-行动循环的Agent GPA评估框架，包含五个核心指标：目标达成度、逻辑一致性、执行效率、计划质量和计划遵循度。

Result: 在两个基准数据集上的实验表明，该框架能够覆盖TRAIL/GAIA基准数据集中的所有智能体错误，支持与人工标注高度一致的LLM评判器（80%-95%错误覆盖），并能以86%的一致性定位错误。

Conclusion: Agent GPA框架为智能体评估提供了系统化的方法，能够全面识别失败模式并支持针对性性能改进。
...
### [215] [ReviewerToo: Should AI Join The Program Committee? A Look At The Future of Peer Review](https://arxiv.org/abs/2510.08867)
...
<details>
  <summary>Details</summary>
Motivation: 解决同行评审中存在的不一致性、评审员主观性和可扩展性挑战，通过AI系统提供更系统和一致的评估。

Method: 开发了模块化框架ReviewerToo，支持专门的评审员角色和结构化评估标准，可部分或完全集成到实际会议工作流程中。

Result: 在ICLR 2025的1,963篇论文上验证，AI评审在事实核查、文献覆盖方面表现出色，但在评估方法新颖性和理论贡献方面仍有不足。AI生成的评审质量高于人类平均水平。

Conclusion: 提出了AI集成到同行评审流程的指南，强调AI可以增强一致性、覆盖范围和公平性，但复杂的评估判断仍需领域专家。为混合同行评审系统提供了基础。
...
### [216] [GTAlign: Game-Theoretic Alignment of LLM Assistants for Mutual Welfare](https://arxiv.org/abs/2510.08872)
*Siqi Zhu,David Zhang,Pedro Cisneros-Velarde,Jiaxuan You*

Main category: cs.AI

TL;DR: 提出GTAlign框架，将博弈论决策整合到LLM的推理和训练中，通过构建收益矩阵来选择对用户和模型双方都有利的行动，解决传统对齐方法中模型奖励最大化不等于用户福利最大化的问题。


<details>
  <summary>Details</summary>
Motivation: 传统对齐方法假设最大化模型奖励就能最大化用户福利，但实际上LLM可能产生过度澄清或冗长推理等次优行为，这类似于囚徒困境，需要一种能同时惠及LLM和用户的决策机制。

Method: 在推理阶段将用户-LLM交互视为策略博弈，构建收益矩阵评估双方福利；在训练阶段引入相互福利奖励来强化合作响应；还提出基于博弈论推理的动态适应技术来应对LLM服务定价政策变化。

Result: 大量实验表明，GTAlign相比基线方法显著提高了推理效率、答案质量和相互福利，在多样化任务中表现优异。

Conclusion: GTAlign通过博弈论方法有效解决了LLM对齐中的社会效率问题，实现了用户和模型的双赢，代码已开源。
...
### [217] [LM Fight Arena: Benchmarking Large Multimodal Models via Game Competition](https://arxiv.org/abs/2510.08928)
*Yushuo Zheng,Zicheng Zhang,Xiongkuo Min,Huiyu Duan,Guangtao Zhai*

Main category: cs.AI

TL;DR: 提出了LM Fight Arena框架，通过在格斗游戏《真人快打II》中让大型多模态模型相互对战，评估它们在实时对抗环境中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有的大型多模态模型基准测试往往无法捕捉它们在实时对抗环境中的真实表现，需要一种动态评估方法。

Method: 构建了一个自动化对战框架，让6个领先的开源和闭源模型在《真人快打II》游戏中控制相同角色进行对战，通过解析游戏画面和状态数据来选择行动。

Result: 提供了一个完全自动化、可重复且客观的评估方法，能够测试LMM在动态环境中的战略推理能力。

Conclusion: LM Fight Arena是一个具有挑战性和吸引力的基准测试，弥合了AI评估与互动娱乐之间的差距。
...
### [218] [RADAR: Mechanistic Pathways for Detecting Data Contamination in LLM Evaluation](https://arxiv.org/abs/2510.08931)
...
Method: RADAR提取37个特征，涵盖表面级置信度轨迹和深层机制特性（包括注意力专业化、电路动态和激活流模式），并使用集成分类器。

Result: 在多样化评估集上达到93%准确率，在清晰案例上表现完美，在具有挑战性的模糊案例上达到76.7%准确率。
...
### [219] [FATHOMS-RAG: A Framework for the Assessment of Thinking and Observation in Multimodal Systems that use Retrieval Augmented Generation](https://arxiv.org/abs/2510.08945)
*Samuel Hildebrand,Curtis Taylor,Sean Oesch,James M Ghawaly Jr,Amir Sadovnik,Ryan Shivers,Brandon Schreiber,Kevin Kurian*

Main category: cs.AI

TL;DR: 提出了一个评估检索增强生成（RAG）管道的基准，包括多模态数据处理能力评估、短语级召回指标、幻觉检测方法，以及开源与闭源管道的性能比较。


<details>
  <summary>Details</summary>
Motivation: 现有基准主要关注检索等特定方面，缺乏对RAG管道整体能力的评估，特别是在处理多模态信息和跨文档推理方面的能力。

Method: 创建了93个问题的数据集评估文本、表格、图像等多模态数据处理；提出短语级召回指标；使用最近邻嵌入分类器检测幻觉；比较2个开源和4个闭源管道的性能。

Result: 闭源管道在正确性和幻觉检测方面显著优于开源管道，特别是在多模态和跨文档问题上差距更大；人工评估显示指标与人类判断高度一致（4.62/5和4.53/5）。

Conclusion: 该基准有效评估RAG管道的整体性能，闭源模型在多模态处理方面表现更优，提出的评估指标与人类判断具有良好一致性。
...
### [220] [EcphoryRAG: Re-Imagining Knowledge-Graph RAG via Human Associative Memory](https://arxiv.org/abs/2510.08958)
*Zirui Liao*

Main category: cs.AI

TL;DR: EcphoryRAG是一个基于实体知识图谱的RAG框架，通过提取核心实体和元数据实现轻量级索引，相比其他结构化RAG系统减少94%的token消耗。它通过提取查询中的线索实体，在知识图谱中进行可扩展的多跳关联搜索，并动态推断实体间的隐式关系来填充上下文，在复杂问答任务上达到最先进水平。


<details>
  <summary>Details</summary>
Motivation: 受人类认知神经科学启发，人类利用线索激活实体中心记忆痕迹进行复杂多跳回忆。作者希望开发一个类似的实体中心知识图谱RAG框架，实现高效的多跳推理。

Method: 1. 索引阶段：仅提取和存储核心实体及对应元数据，实现轻量级索引；2. 检索阶段：从查询中提取线索实体，在知识图谱中进行可扩展的多跳关联搜索；3. 动态推断实体间的隐式关系来填充上下文，无需预先枚举所有关系。

Result: 在2WikiMultiHop、HotpotQA和MuSiQue基准测试上的广泛评估表明，EcphoryRAG达到了新的最先进水平，将平均精确匹配(EM)分数从0.392提高到0.474，优于HippoRAG等强基线KG-RAG方法。
...
### [221] [DualResearch: Entropy-Gated Dual-Graph Retrieval for Answer Reconstruction](https://arxiv.org/abs/2510.08959)
...
<details>
  <summary>Details</summary>
Motivation: 现有的深度研究框架虽然能协调外部工具进行复杂的多步科学推理，但仍存在上下文污染、证据支持薄弱和执行路径脆弱等问题，需要更稳健的推理方法。

Method: 提出DualResearch框架，联合建模两个互补图：广度语义图编码稳定背景知识，深度因果图捕捉执行来源。每个图都有层原生相关函数，广度使用种子锚定的语义扩散，深度使用因果语义路径匹配和可靠性加权。通过熵门控规则在log空间融合答案分布。

Result: 在科学推理基准HLE和GPQA上表现优异，使用开源系统InternAgent的日志文件后，在HLE上准确率提升7.7%，在GPQA上提升6.06%。

Conclusion: DualResearch作为深度研究系统的补充，能够将冗长的多工具执行日志压缩为简洁的推理图，稳定有效地重建答案，在科学推理任务中具有竞争力。
...
### [222] [Semantic-Condition Tuning: Fusing Graph Context with Large Language Models for Knowledge Graph Completion](https://arxiv.org/abs/2510.08966)
...
<details>
  <summary>Details</summary>
Motivation: 现有前缀调优方法简单拼接知识嵌入与文本输入，忽视了知识图谱中的丰富关系语义，给LLM带来了隐式推理负担。

Method: 包含两个关键模块：语义图模块使用GNN从局部图邻域提取上下文感知语义条件；条件自适应融合模块通过参数化投影器自适应调制文本嵌入。
...
### [223] [Tiny-R1V: Lightweight Multimodal Unified Reasoning Model via Model Merging](https://arxiv.org/abs/2510.08987)
*Qixiang Yin,Huanjin Yao,Jianghao Chen,Jiaxing Huang,Zhicheng Zhao,Fei Su*

Main category: cs.AI

TL;DR: 提出了Tiny-R1V，一个轻量级的3B多模态大语言模型，通过两阶段优化实现更快的推理速度和更高的准确率，统一了多任务的多模态推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有的多模态大语言模型在推理效率方面面临模型尺寸大、过度思考和轻量级场景下准确率下降等挑战，而轻量级MLLMs的推理能力研究相对缺乏。

Method: 采用两阶段优化：第一阶段引入LIPO强化学习方法，动态调整组内响应优势，鼓励生成更短更准确的响应；第二阶段提出AMM训练无关的模型融合方法，通过自适应调整任务向量权重和梯度投影正则化损失函数，将多个专家模型融合为统一架构。

Result: 在十个广泛使用的推理基准测试（涵盖数学、结构化数据、OCR和通用能力）上展现出优越性能，使轻量级模型在多样化多模态推理任务中表现出色。
...
TL;DR: 该论文评估了SOTA LLMs在数学证明评分方面的能力，发现模型能可靠识别错误证明但存在评分校准问题，提出了基于智能体工作流的多步骤评分方法来解决部分分数分配不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在解决奥林匹克数学问题方面取得显著进展，需要评估它们在证明评分方面的能力，包括错误检测、严重性判断和公平分数分配，而不仅仅是二元正确性判断。

Method: 使用90个Gemini 2.5 Pro生成的解决方案和MathArena数据集，引入智能体工作流来提取和分析参考解决方案，自动推导问题特定的评分标准，实现多步骤评分过程。

Result: 提出的工作流在人类评分一致性方面表现更好，在部分分数处理上更加一致，在注释语料库和MathArena数据集上都取得了更高的评分一致性。

Conclusion: 智能体工作流能够有效解决LLMs在证明评分中的校准问题，提供更一致的部分分数处理，为未来研究提供了代码、数据和提示/日志资源。
...
TL;DR: 提出LRR混合框架，结合符号分析和LLM生成，解决正则表达式修复中的精度与泛化能力权衡问题


<details>
  <summary>Details</summary>
Motivation: 现有正则表达式修复方法存在权衡：符号方法精确但无法处理复杂模式，LLM方法泛化能力强但可靠性不足

Method: 使用符号模块定位漏洞子模式，然后调用LLM生成语义等效的修复方案

Result: 修复率比现有最优方法提高15.4个百分点，成功解决规则方法无法处理的复杂修复案例

Conclusion: LRR框架为自动化修复问题提供了验证有效的方法论，平衡了泛化能力和可靠性
...
TL;DR: 提出了一种连续记忆机制，将GUI轨迹编码为固定长度的连续嵌入，显著减少上下文成本并保留细粒度视觉信息，在长任务和分布偏移下提升GUI代理性能。


<details>
  <summary>Details</summary>
Motivation: 现有GUI代理将历史轨迹压缩为文本token，导致上下文长度膨胀且丢失关键视觉线索（如控件尺寸和位置），需要可扩展的记忆机制来泛化到不熟悉界面和长任务。

Method: 使用VLM作为编码器将GUI轨迹编码为固定长度的连续嵌入，直接插入主干网络输入层；引入自动扩展数据飞轮，通过搜索发现新环境、合成任务、执行轨迹和验证成功来低成本扩展记忆。

Result: 随着记忆大小和检索深度增加，性能单调提升；在真实GUI基准测试中，记忆增强代理在长任务和分布偏移下持续提升成功率；Qwen-2.5-VL-7B+连续记忆达到与GPT-4o、Claude-4等闭源模型相当的性能。

Conclusion: 连续记忆机制有效解决了GUI代理的上下文膨胀和视觉信息丢失问题，通过低成本数据飞轮实现记忆扩展，显著提升了代理在复杂GUI任务中的性能。
...
TL;DR: 本研究提出了一种整合精神分析和MBTI人格理论的新方法，构建了包含自我意识、潜意识和前意识的人工意识模块，以及16种不同MBTI人格类型的角色，通过多种评估方法验证了模型能够模拟人类意识。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型虽然在各领域取得显著进展，但由于存在幻觉问题，难以真正模仿人类意识。本研究旨在通过整合精神分析和人格理论来构建更接近人类意识的人工智能系统。

Method: 基于精神分析原理开发了三种人工意识（自我意识、潜意识、前意识），设计了16种MBTI人格类型的角色，创建了10种不同情境来测试模型，并通过问卷调查、ChatGPT三级分类和定性评估三种方法进行评估。

Result: 定量和定性分析表明模型能够很好地模拟意识，尽管不同角色和意识之间的响应差异不够显著。这表明整合精神分析和人格理论的模型可以构建更直观、适应性更强的人工智能系统。

Conclusion: 本研究为在复杂认知情境中改进AI交互开辟了新途径，证明了整合精神分析和人格理论的方法在构建类人意识AI系统方面的潜力。
...
TL;DR: 提出了MEC$^3$O多专家共识系统，通过将LLM分配到不同复杂度类别并让它们进行结构化辩论，使用加权共识机制整合预测结果，在代码复杂度预测任务上显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 现有LLM在代码复杂度预测任务中表现不均衡，没有单一模型在所有复杂度类别上都表现优异，需要一种能够整合不同模型优势的方法。

Method: MEC$^3$O系统基于LLM在不同复杂度类别上的性能进行专家分配，为每个专家提供类别特定的指令，让专家进行结构化辩论，最后通过加权共识机制整合预测结果。

Result: 在CodeComplex数据集上，MEC$^3$O比开源基线方法准确率和macro-F1分数至少提高10%，在macro-F1分数上平均超过GPT-4o-mini，与GPT-4o和GPT-o4-mini的F1分数相当。
...
<details>
  <summary>Details</summary>
Motivation: 基于流的文本到图像模型遵循确定性轨迹，用户需要重复采样才能发现不同的模式，这是一个成本高昂且效率低下的过程。

Method: 通过特征空间目标鼓励轨迹间的横向扩散，并通过时间调度的随机扰动重新引入不确定性。关键的是，该扰动被投影为与生成流正交，这种几何约束使其能够在提升多样性的同时不降低图像细节或提示保真度。

Result: 在固定采样预算下，该方法在多个文本到图像设置中持续改进了多样性指标（如Vendi Score和Brisque），同时保持了图像质量和对齐度。

Conclusion: 该方法无需重新训练或修改基础采样器，与常见的流匹配求解器兼容，理论上显示能单调增加体积代理，同时由于几何约束近似保持边缘分布，这为生成质量稳健保持提供了原理性解释。
...
### [230] [Physics-Informed High-order Graph Dynamics Identification Learning for Predicting Complex Networks Long-term Dynamics](https://arxiv.org/abs/2510.09082)
*Bicheng Wang,Jinping Wang,Yibo Sue*

Main category: cs.AI

TL;DR: 提出了一种高阶网络动力学识别方法，用于复杂网络的长期动态预测，结合动态超图学习和物理数据双驱动预测模块，解决了传统方法只能处理成对关系且缺乏可解释性的问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法使用简单图描述复杂网络关系，只能捕捉成对关系，无法处理丰富的非成对结构化关系；同时理论预测模型精度不足，数据驱动模型缺乏可解释性。

Method: 引入动态超图学习捕捉高阶非成对关系；提出物理数据双驱动动态预测模块，结合Koopman算子理论将非线性动力学微分方程转化为线性系统求解，并利用物理信息神经微分方程确保演化符合物理规律。

Result: 在公共数据集和自建产业链网络数据集上的实验验证表明，该方法具有良好的预测精度和长期预测性能。

Conclusion: 该方法通过动态超图学习和双驱动预测模块，在保持可解释性的同时提高了复杂网络动态预测的准确性，特别适用于长期预测任务。
...
### [231] [Leading the Follower: Learning Persuasive Agents in Social Deduction Games](https://arxiv.org/abs/2510.09087)
*Zhang Zheng,Deheng Ye,Peilin Zhao,Hao Wang*

Main category: cs.AI

TL;DR: 本文提出了一种基于Stackelberg竞争模型的强化学习框架，用于训练LLM智能体在社交推理游戏中进行更有说服力的对话，显著提升了智能体的表现。


<details>
  <summary>Details</summary>
Motivation: 现有LLM智能体在社交推理游戏中主要关注信息处理和策略选择，忽视了说服性沟通对影响其他玩家信念和反应的重要性。游戏成功不仅需要正确推理，还需要说服他人按照自己的意图行动。

Method: 将回合制对话形式化为Stackelberg竞争，当前玩家作为领导者战略性地影响跟随者的反应。基于此理论框架，提出了一个强化学习框架来训练智能体优化话语的说服效果。

Result: 在三个不同的社交推理游戏中进行全面实验，证明所提出的智能体显著优于基线方法。

Conclusion: 这项工作代表了开发具有战略性社会影响力AI智能体的重要一步，其影响延伸到需要说服性沟通的各种场景。
...
### [232] [PAC Reasoning: Controlling the Performance Loss for Efficient Reasoning](https://arxiv.org/abs/2510.09133)
...
<details>
  <summary>Details</summary>
Motivation: 大型推理模型在复杂问题解决中表现出色，但部署时计算成本高昂。现有动态切换方法存在额外推理错误且缺乏性能损失统计保证，这对高风险应用至关重要

Method: 构建性能损失的单调置信上界函数，基于不确定性分数确定切换阈值，在分布无关方式下确保有界性能损失

Result: 在推理基准测试中，该方法能节省计算预算并控制用户指定的性能损失

Conclusion: PAC推理方法为大型推理模型提供了统计保证的效率优化方案，在高风险应用中具有重要价值
...
### [233] [Dr. Bias: Social Disparities in AI-Powered Medical Guidance](https://arxiv.org/abs/2510.09162)
*Emma Kondrup,Anne Imouza*

Main category: cs.AI

TL;DR: 研究发现大型语言模型在生成医疗建议时，会根据患者的性别、年龄和种族产生系统性差异，特别是对原住民和双性人群体的建议可读性较差且更复杂。
...
Method: 通过模拟不同患者档案（基于性别、年龄、种族）向LLMs提出一系列临床问题，比较生成回答的自然语言特征。

Result: LLMs生成的医疗建议在不同社会群体间存在系统性差异，原住民和双性人群体获得的建议可读性更差、更复杂，交叉群体中这种趋势更加明显。

Conclusion: 鉴于人们对AI模型的信任度增加，需要提高AI素养，并呼吁AI开发者紧急调查和缓解这些系统性差异，确保公平的患者支持。
...
### [234] [Comparing Knowledge Source Integration Methods for Optimizing Healthcare Knowledge Fusion in Rescue Operation](https://arxiv.org/abs/2510.09223)
...
<details>
  <summary>Details</summary>
Motivation: 医疗领域需要统一的方法来收集、分析和利用现有医疗知识，以支持准确的以患者为导向的决策制定。医疗知识的复杂性和多样性使得知识融合成为关键需求。

Method: 提出多个基于知识图谱结构的概念模型，评估如何实现知识融合，并展示如何将各种知识源整合到知识图谱中用于救援操作。

Result: 开发了支持医疗知识融合的概念框架，使医疗专业人员能够从多个上下文对齐的知识源中进行选择，为关键决策提供支持。

Conclusion: 知识图谱结构为医疗知识融合提供了可行的技术路径，能够有效整合分散的医疗知识资源，提升医疗决策的准确性和效率。
...
### [235] [RegexPSPACE: A Benchmark for Evaluating LLM Reasoning on PSPACE-complete Regex Problems](https://arxiv.org/abs/2510.09227)
*Hyundong Jin,Joonghyuk Hahn,Yo-Sub Han*

Main category: cs.AI

TL;DR: 本文提出了基于PSPACE完全正则表达式问题的新基准，用于评估大语言模型和大推理模型的空间计算限制，发现了常见失败模式如冗长和重复。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型在自然语言处理、数学推理和编程方面表现强劲，但其在有限上下文窗口下的空间计算限制仍未被充分理解。现有研究多关注NP复杂度问题，而PSPACE完全问题能更严格地评估计算能力。

Method: 通过双指数空间探索构建了包含超过100万个正则表达式实例的标注数据集，建立了基于两个PSPACE完全正则表达式问题（等价性决策和最小化）的基准，并对6个LLM和5个LRM进行了广泛评估。

Result: 评估揭示了常见失败模式，包括冗长和重复。这是首次对LLM和LRM空间计算限制的实证研究，为评估其高级推理能力提供了新框架。

Conclusion: 该工作通过PSPACE完全正则表达式问题建立了评估大模型计算能力的新基准，揭示了其在空间复杂度方面的限制，为理解大模型的推理能力边界提供了重要见解。
...
### [236] [Fundamentals of Building Autonomous LLM Agents](https://arxiv.org/abs/2510.09244)
*Victor de Lamo Castrillo,Habtom Kahsay Gidey,Alexander Lenz,Alois Knoll*

Main category: cs.AI

TL;DR: 本文综述了基于大语言模型的智能体架构与实现方法，探索如何开发具备自主任务执行能力的"代理性"LLM，通过感知、推理、记忆和执行四大系统集成，构建能够模拟人类认知过程、实现自主智能行为的软件机器人。


<details>
  <summary>Details</summary>
Motivation: 传统大语言模型在现实世界任务中存在局限性，研究旨在探索开发能够自动化复杂任务、缩小与人类能力差距的"代理性"LLM的模式。

Method: 构建包含四个关键组件的智能体系统：感知系统将环境感知转换为有意义表示；推理系统通过思维链、思维树等技术制定计划、适应反馈和评估行动；记忆系统通过短期和长期机制保留知识；执行系统将内部决策转化为具体行动。

Result: 研究表明，集成这些系统能够产生更具能力和泛化性的软件机器人，这些机器人能够模拟人类认知过程，实现自主和智能行为。

Conclusion: 通过整合感知、推理、记忆和执行系统，可以开发出能够自动化复杂任务、模拟人类认知过程的智能代理，这为解决传统LLM在现实世界任务中的局限性提供了有效途径。
...
### [237] [Localist LLMs -- A Mathematical Framework for Dynamic Locality Control](https://arxiv.org/abs/2510.09338)
*Joachim Diederich*
...
Method: 使用组稀疏惩罚、信息论锚点设计和动态规则注入，通过可调节的局部性参数控制注意力机制的集中程度，实现从局部化到分布式表示的连续调整。
...
### [238] [Toward Mechanistic Explanation of Deductive Reasoning in Language Models](https://arxiv.org/abs/2510.09340)
*Davide Maltoni,Matteo Ferrara*

Main category: cs.AI

TL;DR: 小型语言模型可以通过学习底层规则（而非统计学习）解决演绎推理任务，研究发现归纳头在规则补全和规则链式推理中发挥核心作用。


<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型展现出逻辑推理能力，但其内部机制仍不明确。本文旨在探索语言模型如何实现逻辑推理，特别是小模型是否能够学习推理规则而非统计模式。

Method: 使用小型语言模型解决演绎推理任务，通过分析其内部表示和计算电路，揭示模型实现逻辑推理的机制。

Result: 研究发现模型确实能够学习推理规则，并且归纳头在规则补全和规则链式推理步骤中起核心作用。

Conclusion: 语言模型可以通过学习底层推理规则来实现逻辑推理，归纳头是实现这种推理能力的关键组件。
...
### [239] [Sequence Variables: A Constraint Programming Computational Domain for Routing and Sequencing](https://arxiv.org/abs/2510.09373)
*Augustin Delecluse,Pierre Schaus,Pascal Van Hentenryck*

Main category: cs.AI

TL;DR: 本文在约束规划中形式化了序列变量，以解决传统后继变量模型在处理可选访问和插入启发式时的局限性，为车辆路径问题提供了更直观的建模框架。
...
Method: 形式化定义了序列变量的计算域、更新操作和一致性级别，设计了专门的数据结构和全局约束，并实现了基于插入的大邻域搜索。

Result: 序列变量简化了问题建模，在Dial-a-Ride问题上实现了有竞争力的计算性能。

Conclusion: 序列变量为车辆路径问题的约束规划建模提供了更强大和灵活的工具，能够有效处理可选访问和插入启发式。
...
### [240] [Agentic Systems in Radiology: Design, Applications, Evaluation, and Challenges](https://arxiv.org/abs/2510.09404)
...
TL;DR: 这篇论文综述了基于大语言模型的智能体系统在放射学中的应用，探讨了如何通过外部工具和反馈机制增强LLM能力，实现从半自动化工作流到自适应智能体的不同自主程度。


<details>
  <summary>Details</summary>
Motivation: 放射学具有多模态数据流和协调工作流程的特点，非常适合使用能够适应上下文并自动化复杂重复任务的智能体系统。虽然LLM在单个任务上已表现出色，但孤立使用无法充分发挥其在复杂多步骤工作流中的潜力。

Method: 通过为LLM配备外部工具和反馈机制，使其能够驱动展现不同自主程度的系统，从半自动化工作流到能够管理复杂过程的自适应智能体。

Result: LLM驱动的智能体系统在放射学中显示出潜力，能够处理信息提取、报告总结等任务，并在复杂多步骤工作流中发挥作用。

Conclusion: LLM驱动的智能体系统为放射学提供了新的可能性，但仍面临错误级联、工具使用效率和健康IT集成等挑战，需要进一步研究和开发。
...
### [241] [Titans Revisited: A Lightweight Reimplementation and Critical Analysis of a Test-Time Memory Model](https://arxiv.org/abs/2510.09551)
...
### [242] [Safe, Untrusted, "Proof-Carrying" AI Agents: toward the agentic lakehouse](https://arxiv.org/abs/2510.09567)
*Jacopo Tagliabue,Ciro Greco*

Main category: cs.AI

TL;DR: 本文提出使用API优先、可编程的数据湖仓来构建安全的设计，以支持AI驱动的自动化工作流，通过数据分支和声明式环境实现可复现性和可观察性。
...
Method: 使用Bauplan作为案例研究，展示数据分支和声明式环境如何自然地扩展到智能体，通过受证明携带代码启发的正确性检查来修复数据管道。

Result: 原型演示表明，不受信任的AI智能体可以在生产数据上安全操作。

Conclusion: 为实现完全智能体化的数据湖仓铺平了道路。
...
### [243] [GraphMERT: Efficient and Scalable Distillation of Reliable Knowledge Graphs from Unstructured Data](https://arxiv.org/abs/2510.09580)
...
<details>
  <summary>Details</summary>
Motivation: 神经符号AI框架因难以扩展且神经方法的隐式表示和近似推理限制了可解释性和可信度，而知识图谱作为显式语义知识的黄金标准表示可以解决符号方面的问题。

Method: 引入GraphMERT，一个微小的图形编码器模型，从非结构化文本语料库及其内部表示中提取高质量知识图谱，形成模块化的神经符号堆栈。

Result: 在糖尿病PubMed论文文本上，80M参数的GraphMERT生成的知识图谱FActScore达到69.8%，而32B参数的基线LLM仅为40.2%；GraphMERT的ValidityScore为68.8%，基线为43.0%。

Conclusion: GraphMERT+KG是首个高效且可扩展的神经符号模型，在基准精度和符号表示方面均达到最先进水平，优于大型语言模型基线。
...
### [244] [LiveOIBench: Can Large Language Models Outperform Human Contestants in Informatics Olympiads?](https://arxiv.org/abs/2510.09595)
...
Method: 从72个官方信息学奥林匹克竞赛中收集403个专家策划的问题，每个问题平均60个测试用例，建立包含精英选手性能数据的自包含评估系统。
