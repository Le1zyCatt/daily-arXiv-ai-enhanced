...
TL;DR: 提出一种自监督符号自动编码器，通过符号分层过程奖励建模，将图表编码为结构化基元并在潜在空间中表示其关系，然后通过可执行引擎解码重建图表。
...
Method: 提出自监督符号自动编码器：1）将图表编码为结构化基元和关系；2）通过可执行引擎解码重建；3）核心是符号分层过程奖励建模，强制执行点在线、线在形状、形状在关系的一致性；4）引入稳定化机制平衡探索与利用；5）在下游任务上微调符号编码器，构建神经符号系统。

Result: 在多个任务上表现优异：几何图表重建MSE降低98.2%；图表重建任务上7B模型超越GPT-4o 0.6%；MathGlance感知基准提升13%；MathVerse和GeoQA推理基准提升3%。

Conclusion: 该方法成功整合了神经网络的推理能力和符号模型的可解释性，通过推理基础的视觉奖励实现了有效的符号视觉学习，在重建、感知和推理任务上均表现出色。
...
TL;DR: 提出"排水节点"统一框架，通过重新分配概率质量到不确定性，处理噪声标签、类别模糊和异常样本，在CIFAR-10/100等数据集上实现高达9%的准确率提升。


<details>
  <summary>Details</summary>
Motivation: 现代深度学习面临噪声标签、类别模糊以及需要鲁棒拒绝分布外或损坏样本的挑战。现有方法在处理实例依赖和不对称标签噪声时存在局限。

Method: 在神经网络输出层添加"排水节点"，重新分配概率质量到不确定性，同时保持端到端训练和可微性。该机制为高度模糊、异常或噪声样本提供自然逃逸路径。

Result: 在CIFAR-10/100添加实例依赖噪声或不对称噪声的实验中，高噪声情况下比现有方法准确率提升高达9%。在mini-WebVision、mini-ImageNet和Clothing-1M等真实数据集上达到或超越SOTA。

Conclusion: 排水节点框架能有效吸收损坏、误标记或异常数据，产生更稳定的决策边界，且应用范围远超分类任务，可用于网络规模半监督数据清洗和开放集应用。
...
TL;DR: AI驱动的头部姿态校正和图像修复技术对生物特征人脸识别准确性的影响研究，发现选择性应用CFR-GAN和CodeFormer组合能有效提升识别精度


<details>
  <summary>Details</summary>
Motivation: 现实世界中的人脸图像通常存在质量差、非正面姿态和遮挡等问题，导致生物特征人脸识别模型的准确性显著下降。研究旨在探索针对性的AI驱动头部姿态校正和图像修复技术是否能改善识别准确性。

Method: 使用模型无关的大规模法证评估流程，评估三种修复方法：3D重建（NextFace）、2D正面化（CFR-GAN）和特征增强（CodeFormer）。研究比较了这些技术的单独应用和组合应用效果。

Result: 简单应用这些技术会显著降低人脸识别准确性。然而，选择性应用CFR-GAN与CodeFormer的组合能够产生有意义的改进。

Conclusion: 虽然直接的修复技术应用会损害识别性能，但通过精心选择和组合特定的修复方法（特别是CFR-GAN和CodeFormer），可以有效提升在现实世界复杂条件下的人脸识别准确性。
...
### [4] [Deep-BrownConrady: Prediction of Camera Calibration and Distortion Parameters Using Deep Learning and Synthetic Data](https://arxiv.org/abs/2501.14510)
*Faiz Muhammad Chaudhry,Jarno Ralli,Jerome Leudet,Fahad Sohrab,Farhad Pakdaman,Pierre Corbani,Moncef Gabbouj*

Main category: cs.CV

TL;DR: 使用深度学习从单张图像预测相机标定和畸变参数，通过合成数据集训练ResNet模型，在真实图像上表现良好。


<details>
  <summary>Details</summary>
Motivation: 传统相机标定方法需要多张不同角度的标定板图像，这在公开数据集中往往不可得。本研究旨在通过深度学习从单张图像直接预测相机参数，解决实际应用中的标定难题。

Method: 使用AILiveSim仿真平台创建包含焦距和镜头畸变参数变化的合成数据集。基于ResNet架构构建深度学习网络，将其适配为回归任务，预测Brown-Conrady镜头模型参数。训练主要依赖合成图像，辅以少量真实图像。

Result: 深度学习模型能够从单张图像准确预测相机和镜头参数。在合成数据上训练的模型在真实世界图像上表现出良好的标定性能，验证了合成数据训练的有效性。

Conclusion: 该方法为相机标定提供了实用解决方案，特别适用于自动驾驶、机器人和增强现实等应用，其中传统标定方法难以实施。合成数据集为模型训练提供了可靠基础。
...
Method: Flux4D采用完全无监督方法，仅使用光度损失并强制执行"尽可能静态"的正则化，直接预测3D高斯及其运动动态来重建传感器观测。通过在多个场景上训练，直接从原始数据中分解动态元素，无需预训练监督模型或基础先验。

Result: 在室外驾驶数据集上的实验表明，Flux4D在可扩展性、泛化能力和重建质量方面显著优于现有方法。能够在几秒钟内高效重建动态场景，有效扩展到大型数据集，并能很好地泛化到未见过的环境，包括罕见和未知物体。

Conclusion: Flux4D提供了一个简单且可扩展的框架，用于大规模动态场景的4D重建，通过完全无监督的方法实现了高效、可扩展和泛化能力强的动态场景重建。
...
Method: 两阶段LLM方法：第一阶段预测物体语义类别，第二阶段基于推断的语义为每个几何段分配合理材质，均为零样本方式
...
TL;DR: 利用GPT-4o和GPT-5等多模态大语言模型进行零样本物体计数，仅使用文本提示无需视觉示例，在FSC-147和CARPK数据集上达到与现有零样本方法相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 现有零样本物体计数方法通常需要大量标注数据和视觉示例指导，而大语言模型具有强大的推理和数据理解能力，探索能否利用多模态LLM仅通过文本提示完成计数任务而无需监督。

Method: 利用GPT-4o和GPT-5两种多模态大语言模型，仅使用文本提示进行零样本物体计数，无需任何视觉示例或监督训练，在FSC-147和CARPK数据集上进行评估。

Result: 在FSC-147数据集上，模型性能与最先进的零样本方法相当，在某些情况下甚至超越了现有方法；在CARPK数据集上也进行了评估和比较分析。

Conclusion: 多模态大语言模型仅通过文本提示就能实现有效的零样本物体计数，展示了LLM在视觉计数任务中的潜力，为无需监督的计数方法提供了新思路。
...
Method: 使用泊松分布建模信号相关噪声，并引入傅里叶域频谱采样算法精确建模信号无关噪声。该方法仅需每个ISO设置下的单张噪声图像和单张暗帧。

Result: 该方法能生成保持真实传感器噪声空间和统计特性的多样化噪声实现，在多个低光去噪基准测试中取得了最先进的性能。
...
TL;DR: PixPerfect是一个像素级细化框架，通过可微分的判别性像素空间、全面的伪影模拟管道和直接的像素空间细化方案，解决了潜在扩散模型在图像修复和局部编辑中产生的像素级不一致问题。
...
Method: PixPerfect采用三个关键技术：(1)可微分的判别性像素空间，放大和抑制细微的颜色和纹理差异；(2)全面的伪影模拟管道，在训练期间将细化器暴露于真实的局部编辑伪影；(3)直接的像素空间细化方案，确保在不同潜在表示和任务中的广泛适用性。
...
TL;DR: 论文提出PyroFocus两阶段深度学习管道，用于实时机载/星载野火检测，结合火灾分类和辐射功率估计，在计算资源受限条件下实现速度与精度的平衡。


<details>
  <summary>Details</summary>
Motivation: 随着野火频率和严重性增加，需要低延迟、计算高效的机载实时检测方法。多光谱和高光谱热成像仪提供丰富光谱信息，但高维数据和有限机载资源使实时处理具有挑战性。

Method: 提出PyroFocus两阶段管道：第一阶段进行火灾分类（无火、活跃火、后火），第二阶段进行火灾辐射功率回归或分割。系统评估多种深度学习架构，包括自定义CNN和Transformer模型，使用NASA MASTER数据。

Result: 实验结果表明，两阶段管道在速度和精度之间实现了良好的权衡，显著降低了推理时间和计算成本，适合未来野火监测任务的实时边缘部署。

Conclusion: PyroFocus管道在机载/星载野火检测中展现出实时部署的潜力，为未来野火监测任务提供了计算高效的解决方案。
...
TL;DR: 提出了H²U3D数据集和SpatialReasoner框架，用于解决大尺度3D环境下的空间推理问题，通过主动感知和分层探索实现高效场景理解。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型主要局限于房间尺度场景，缺乏处理大尺度多楼层3D环境的能力，需要专门的数据集和方法来解决房屋尺度的空间推理问题。

Method: 1) 创建H²U3D数据集：包含多楼层环境（最多3层，10-20个房间），通过自动化标注流程构建分层粗到细的视觉表示和带思维链标注的问答对；2) 提出SpatialReasoner框架：主动感知框架，基于文本查询自主调用空间工具探索3D场景，采用两阶段训练策略：监督式冷启动+带自适应探索奖励的强化学习。

Result: SpatialReasoner在H²U3D数据集上达到最先进性能，优于GPT-4o和Gemini-2.5-Pro等强基线。特别地，该方法平均仅需3-4张图像，而基线方法需要16+张图像，证明了粗到细主动探索范式的有效性。

Conclusion: H²U3D数据集填补了大尺度3D场景理解的数据空白，SpatialReasoner框架通过主动感知和分层探索实现了高效的空间推理，为房屋尺度的3D视觉问答提供了有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 自动驾驶需要准确的环境表示，传统HD地图需要预先构建但无法适应实时变化，而广泛可用的SD地图分辨率不足但可作为先验指导在线地图构建

Method: 提出NavMapFusion扩散框架，通过迭代去噪过程融合高保真传感器数据和低保真导航地图，将先验地图与在线感知的差异对应为扩散过程中的噪声

Result: 在nuScenes基准测试中，基于OpenStreetMap道路线数据，100米范围内相对提升21.4%，更大感知范围提升更显著，同时保持实时能力
...
TL;DR: 提出SLEDGE框架，将设计生成建模为遵循指令序列的逐步分层过程，解决现有方法将设计合成视为单步生成的问题


<details>
  <summary>Details</summary>
Motivation: 设计本质上是逐步细化的过程，但现有方法主要将设计合成视为单步生成问题，低估了创意过程的复杂性，需要更符合实际设计流程的方法

Method: 提出SLEDGE框架，利用多模态大语言模型将设计更新建模为原子化的分层变化，每个变化都基于前一个状态并受指令约束

Result: 实验分析表明该方法优于为这一新设置定制的先进方法，并引入了包含数据集和基准的新评估套件

Conclusion: 该工作提出了逐步分层设计生成的新问题设置，展示了SLEDGE方法的有效性，希望吸引对这一实用但未充分探索的研究领域的关注
...
TL;DR: ProtoEFNet：一种基于视频原型学习的连续射血分数回归模型，通过学习动态时空原型来捕获临床相关的心脏运动模式，提供可解释性预测。


<details>
  <summary>Details</summary>
Motivation: 传统射血分数评估需要手动追踪和专业知识，耗时且存在观察者间差异。现有深度学习模型多为黑盒，缺乏透明度，临床信任度低。后验可解释性方法无法指导模型内部推理，临床应用可靠性有限。

Method: 提出ProtoEFNet视频原型学习模型，学习动态时空原型捕获心脏运动模式；引入原型角度分离损失，在连续射血分数谱上强制区分性表示。

Result: 在EchonetDynamic数据集上，ProtoEFNet达到与非可解释模型相当的准确度，同时提供临床相关见解。消融研究表明，提出的损失将F1分数从77.67±2.68提升至79.64±2.10。

Conclusion: ProtoEFNet在保持准确性的同时提供临床可解释性，原型角度分离损失有效提升性能，为心脏功能评估提供了透明可靠的深度学习解决方案。
...
TL;DR: 提出HalluGen框架，通过扩散模型生成可控的幻觉图像，创建首个大规模幻觉数据集，用于安全关键图像恢复任务的幻觉评估与检测。


<details>
  <summary>Details</summary>
Motivation: 生成模型在图像恢复中容易产生幻觉（看似合理但实际错误的伪结构），这在医疗影像、工业检测等安全关键领域尤为危险。现有研究受限于评估幻觉需要标注数据，而标注成本高且主观。

Method: 提出HalluGen框架，基于扩散模型合成具有可控类型、位置和严重程度的幻觉图像。使用该框架构建包含4,350张标注图像的大规模幻觉数据集，并开发SHAFE指标（基于特征评估的语义幻觉评估）和参考无关的幻觉检测器。

Result: 生成的幻觉图像在感知上真实但语义错误（分割IoU从0.86降至0.36）。SHAFE指标在幻觉敏感性上优于传统指标，训练的幻觉检测器能泛化到真实恢复失败场景。

Conclusion: HalluGen及其开放数据集为安全关键图像恢复中的幻觉评估建立了首个可扩展的基础，有助于提高生成模型在关键应用中的可靠性和可信度。
...
TL;DR: 该研究比较了16种深度学习架构用于3D OCT图像中早期角膜疾病的检测，发现分层注意力模型在稀疏异常检测方面优于2D/3D CNN和ViT，具有更高的敏感性和特异性。


<details>
  <summary>Details</summary>
Motivation: 医学影像中微弱、空间分散的异常检测面临挑战，现有架构存在局限性：2D/3D CNN过于局部化，而ViT的全局注意力过于扩散，缺乏适合稀疏体积模式识别的最优归纳偏置。

Method: 系统比较了16种现代深度学习架构，包括2D/3D卷积网络、混合模型和体积Transformer家族，用于从3D前段OCT体积中检测亚临床圆锥角膜（SKC）。

Result: 分层注意力模型在稀疏异常检测中表现最佳，比2D/3D CNN和ViT的敏感性和特异性高出21-23%，且参数效率更高。机制分析显示其优势源于空间尺度对齐：分层窗口化产生的有效感受野与亚临床异常的中等多切片范围匹配。

Conclusion: 分层注意力为3D医学影像中的早期病理变化分析提供了原则性和有效的方法，为未来体积异常检测系统设计提供了指导，特别是在稀疏信号检测方面具有优势。
...
<details>
  <summary>Details</summary>
Motivation: 大多数视觉理解、预测和生成直接在2D观测上进行，导致性能不佳。图像和视频是4D世界（3D空间+时间）的离散2D投影，需要学习连续4D动态来生成未见视觉内容

Method: 1) 从稀疏单目2D帧重建4D世界（2D→4D）；2) 在低秩表示和物理约束下学习连续4D动态（离散4D→连续4D）；3) 在时间上向前推进世界，在采样时间和视点重新投影回2D，基于时空上下文意识生成未见区域（4D→2D）

Result: 通过建模4D动态，SeeU实现了连续且物理一致的新颖视觉生成，在未见时间生成、未见空间生成和视频编辑等多个任务中展现出强大潜力

Conclusion: SeeU的2D→4D→2D框架通过建模连续4D动态，能够生成物理一致的未见视觉内容，为视觉理解和生成任务提供了新思路
...
TL;DR: 该研究提出基于深度学习的肺癌自动分类系统，使用DenseNet169和SVM模型在IQOTHNCCD数据集上达到98%准确率，并集成Grad-CAM和SHAP增强模型可解释性。
...
Method: 使用IQOTHNCCD公开CT数据集（正常、良性、恶性三类），采用DenseNet169（含Squeeze-and-Excitation注意力模块）和Focal Loss处理类别不平衡，结合特征金字塔网络进行多尺度特征融合。同时开发SVM模型，使用MobileNetV2进行特征提取。集成Grad-CAM可视化决策区域，SHAP解释SVM特征贡献。

Result: DenseNet169和SVM模型均达到98%的准确率，表现出强大的分类性能。模型在准确率、透明度和鲁棒性方面均有显著提升。

Conclusion: 深度学习方法能够显著提高肺癌诊断的准确性、透明度和鲁棒性，为实际医疗应用提供了有前景的解决方案，具有临床实践潜力。
...
Method: 1) 构建FireSentry数据集：使用同步无人机平台采集亚米级空间分辨率和亚秒级时间分辨率的可见光/红外视频流、现场环境测量和人工验证的火掩码；2) 提出FiReDiff框架：先在红外模态预测未来视频序列，然后在掩码模态基于生成的动态精确分割火掩码
...
TL;DR: ShelfGaussian是一个开放词汇的多模态高斯3D场景理解框架，利用现成的视觉基础模型进行监督，在零样本语义占用预测和真实世界UGV应用中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有高斯方法存在局限性：要么使用带标注的3D标签监督封闭集语义高斯，忽略了渲染能力；要么通过纯2D自监督学习开放集高斯表示，导致几何质量下降且仅限于相机设置。需要充分发挥高斯方法的潜力。

Method: 提出多模态高斯变换器，使高斯能从多种传感器模态查询特征；提出货架监督学习范式，在2D图像和3D场景层面联合优化高斯与VFM特征。

Result: 在Occ3D-nuScenes上实现了最先进的零样本语义占用预测性能；在无人地面车辆(UGV)上评估了其在多样化城市场景中的野外性能。

Conclusion: ShelfGaussian通过结合多模态特征查询和货架监督学习，成功构建了一个开放词汇的多模态3D场景理解框架，在各种感知和规划任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 光学与合成孔径雷达(SAR)图像之间存在显著的模态差异，这给跨模态船舶重识别带来了巨大挑战，而当前该任务尚未得到充分探索
...
Result: 在HOSS ReID数据集上的实验表明，MOS在所有评估协议下显著超越现有方法，在ALL to ALL、Optical to SAR和SAR to Optical设置下分别实现了+3.0%、+6.2%和+16.4%的R1准确率提升
...
TL;DR: 提出了视频差异描述任务ViDiC及其数据集ViDiC-1K，用于评估多模态大语言模型在视频对比较描述和差异感知方面的能力。


<details>
  <summary>Details</summary>
Motivation: 现有图像差异描述方法无法捕捉动态场景中的运动连续性、事件演变或编辑一致性，视觉语言系统在动态场景的对比感知能力方面研究不足。

Method: 1. 引入ViDiC任务和ViDiC-1K数据集（包含1000个视频对，标注了7个类别超过4000个比较检查项）；2. 提出基于LLM-as-a-Judge协议的双检查表框架，分别测量相似性和差异的准确性。

Result: 在19个代表性多模态模型上的实验显示出它们在比较描述和差异感知能力方面存在显著性能差距。

Conclusion: ViDiC-1K可作为具有挑战性的基准，为推进视频理解、编辑感知和多模态智能中的比较推理奠定坚实基础。
...
<details>
  <summary>Details</summary>
Motivation: 现有的可负担性检测方法要么只关注"如何使用"物体而忽略"是什么"和"在哪里"，要么将物体检测和可负担性学习作为独立任务处理，缺乏有效交互和实时能力。

Method: 提出YOLOA模型，使用轻量级检测器包含物体检测和可负担性学习分支，通过LLM适配器进行精炼。LLM适配器与物体和可负担性初步预测交互，生成更准确的类别先验、边界框偏移和可负担性门控。
...
<details>
  <summary>Details</summary>
Motivation: 状态空间模型（SSMs）在长序列建模方面潜力巨大，但其对输入顺序的依赖与点云的不规则性存在冲突。现有方法依赖预定义的序列化策略，无法根据不同的几何结构进行调整。

Method: DM3D引入偏移引导的高斯序列化机制，包含：1) 高斯KNN重采样（GKR）增强结构感知；2) 高斯可微分重排序（GDR）实现端到端序列化优化；3) 三路径频率融合模块增强特征互补性并减少混叠。

Result: 在基准数据集上的广泛实验表明，DM3D在分类、少样本学习和部件分割任务上达到了最先进的性能。

Conclusion: 自适应序列化有效释放了SSMs在点云理解中的潜力，DM3D通过结构自适应序列化机制成功解决了点云不规则性与SSMs顺序依赖之间的冲突。
...
TL;DR: 首次系统评估8种最先进立体深度估计方法在林业场景的零样本泛化能力，发现不同方法在不同场景表现差异显著，DEFOM在植被密集环境中表现最佳。
...
Method: 在Scene Flow数据集上训练8种立体方法（RAFT-Stereo、IGEV、IGEV++、BridgeDepth、StereoAnywhere、DEFOM及基线方法ACVNet、PSMNet、TCstereo），然后在4个标准基准（ETH3D、KITTI 2012/2015、Middlebury）和新的5,313对Canterbury林业数据集上进行零样本评估。

Result: 方法表现呈现场景依赖性：基础模型在结构化场景表现优异（BridgeDepth在ETH3D: 0.23px，KITTI: 0.83-1.07px），迭代方法保持跨域鲁棒性（IGEV++: 0.36-6.77px）。RAFT-Stereo在ETH3D上出现灾难性失败（26.23px EPE，98%错误率），但在KITTI上正常（0.90-1.11px）。DEFOM在林业场景表现最佳，具有优越的深度平滑性、遮挡处理和跨域一致性。

Conclusion: DEFOM被确定为植被深度估计的最佳基准方法，尽管IGEV++能保留更精细的细节，但DEFOM在林业环境中的整体性能更优，为自主无人机林业操作提供了可靠的深度估计解决方案。
...
TL;DR: 提出基于冻结扩散模型预训练特征的标签高效高光谱图像分类框架，通过提取早期去噪时间步的空间特征并与光谱信息融合，在稀疏标注下实现优越性能。


<details>
  <summary>Details</summary>
Motivation: 高光谱图像分类面临空间分辨率低和标注稀疏的挑战，需要开发标签高效的方法。传统方法难以在有限标注下有效利用空间和光谱信息。

Method: 利用在自然图像上预训练的冻结扩散模型，提取早期去噪时间步的高分辨率解码器层特征作为空间表示；设计轻量级FiLM融合模块，使用光谱线索自适应调制冻结的空间特征，实现多模态学习。

Result: 在两个最新高光谱数据集上的实验表明，该方法仅使用稀疏训练标签即可超越现有最优方法。消融研究验证了扩散模型特征和光谱感知融合的有效性。

Conclusion: 预训练的扩散模型能够支持领域无关、标签高效的表征学习，适用于遥感及更广泛的科学成像任务，为稀疏标注场景提供了有效解决方案。
...
TL;DR: 提出整合多智能体数据生成系统(MAGEN)和基于本体的多维度知识增强预训练(O-MAKE)的医学视觉语言预训练框架，解决网络数据噪声和长文本复杂性，在皮肤病学领域实现SOTA零样本性能。


<details>
  <summary>Details</summary>
Motivation: 现有医学视觉语言预训练方法面临网络收集数据的噪声问题和非结构化长医学文本的复杂性挑战，需要提升数据质量和文本理解能力。

Method: 1. MAGEN系统：通过基础模型辅助的标注和检索验证流程合成知识丰富的描述，提升数据质量；2. O-MAKE：将长文本分解为不同知识维度，实现全局和局部细粒度对齐，并通过本体引导机制显式建模医学概念关系。

Result: 在皮肤病学领域验证，在8个数据集上实现疾病分类和跨模态检索任务的SOTA零样本性能，并发布包含40万皮肤图像-文本对的增强数据集Derm1M-AgentAug。

Conclusion: 提出的MAGEN和O-MAKE框架有效解决了医学VLP中的数据噪声和长文本复杂性挑战，为医学图像分析提供了强大的预训练范式。
...
TL;DR: LM-CartSeg是一个全自动的膝关节MRI分割和放射组学分析流程，通过几何规则改进分割质量，提供稳定的内外侧分区，并生成具有诊断价值的放射组学特征。


<details>
  <summary>Details</summary>
Motivation: 现有膝关节MRI放射组学研究依赖手动ROI且缺乏质量控制，需要自动化的、解剖学意义明确的ROI来同时捕捉软骨和软骨下骨信息。

Method: 使用两个3D nnU-Net模型分别在两个数据集上训练，测试时融合零样本预测并通过几何规则精化：连通分量清洗、构建10mm软骨下骨带、基于PCA和k-means的数据驱动胫骨内外侧分割。

Result: 后处理显著改善分割质量（ASSD从2.63降至0.36mm，HD95从25.2降至3.35mm，DSC 0.91）；几何内外侧规则产生跨数据集稳定分区；放射组学特征中仅6-12%与体积或厚度强相关。

Conclusion: LM-CartSeg提供自动化的质量控制ROI和放射组学特征，这些特征携带超出简单形态测量的判别信息，为多中心膝骨关节炎放射组学研究提供实用基础。
...
TL;DR: 提出一种无监督学习3D关键点的方法，用于条件生成扩散模型，在点云数据上实现空间结构化表示
...
Result: 学习的关键点在不同物体实例间具有可重复的空间结构，支持关键点空间的平滑插值，表明捕捉了几何变化；在多个物体类别上表现优异，关键点一致性比先前方法提高6个百分点

Conclusion: 提出的框架成功弥合了无监督关键点学习与3D生成模型之间的差距，为3D生成提供了结构化、可解释的表示方法
...
TL;DR: GalaxyDiT是一种无需训练的视频生成加速方法，通过指导对齐和系统代理选择实现计算重用，在保持高质量的同时获得显著加速效果。


<details>
  <summary>Details</summary>
Motivation: 尽管基于Transformer的扩散模型（DiTs）和分类器自由指导（CFG）在视频生成中表现出色，但它们需要大量计算资源。每个视频生成需要数十次迭代步骤，而CFG会使计算量加倍，这种低效率阻碍了在下游应用中的广泛采用。

Method: GalaxyDiT通过指导对齐和系统代理选择实现计算重用。使用秩次相关分析为每个视频模型（跨模型家族和参数规模）识别最佳代理，确保最优的计算重用。

Result: 在Wan2.1-1.3B和Wan2.1-14B上分别实现了1.87倍和2.37倍的加速，VBench-2.0基准测试仅下降0.97%和0.72%。在高加速率下，该方法保持了对基础模型的优异保真度，在峰值信噪比（PSNR）上比先前最先进方法高出5到10 dB。

Conclusion: GalaxyDiT提供了一种无需训练的高效视频生成加速方法，通过智能计算重用显著减少计算需求，同时保持生成质量，有助于扩散模型在下游应用中的更广泛采用。
...
TL;DR: ThinkDeeper是一个用于自动驾驶视觉定位的框架，通过空间感知世界模型预测未来空间状态来解决指令歧义问题，在多个基准测试中取得最优性能。


<details>
  <summary>Details</summary>
Motivation: 现有自动驾驶视觉定位方法难以处理模糊、依赖上下文的指令，因为它们缺乏对3D空间关系和场景演化的推理能力。需要一种能够预测未来空间状态以消除歧义的解决方案。

Method: 提出ThinkDeeper框架，包含：1) 空间感知世界模型(SA-WM)，将当前场景蒸馏为指令感知潜在状态并展开未来潜在状态序列；2) 超图引导解码器，分层融合这些状态与多模态输入，捕获高阶空间依赖关系。同时创建DrivePilot数据集，使用RAG和CoT提示的LLM管道生成语义标注。

Result: 在六个基准测试中表现优异：在Talk2Car排行榜排名第一，在DrivePilot、MoCAD和RefCOCO/+/g基准上超越现有最佳方法。在挑战性场景（长文本、多智能体、歧义）中表现出强鲁棒性和高效性，即使使用50%数据训练仍保持优越性能。

Conclusion: ThinkDeeper通过引入世界模型原理，使自动驾驶系统能够推理未来空间状态，有效解决视觉定位中的指令歧义问题，为自动驾驶中的自然语言交互提供了更可靠的解决方案。
...
Method: 在潜在扩散模型中增加每帧深度预测，并提出多视角几何损失，在共享3D坐标系中对齐跨帧的预测深度图，以增强结构一致性。

Result: 在多个数据集上的实验表明，该方法相比现有基线能产生显著更稳定和几何一致的结果，改善了时空连贯性、形状一致性和物理合理性。

Conclusion: 通过深度预测引入几何正则化损失，成功弥合了外观生成与3D结构建模之间的差距，提升了视频生成的几何一致性。
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLM需要大量图像-文本对进行任务特定微调，成本高昂。文本数据广泛可得且易于编辑，但纯文本训练因模态差距效果有限

Method: 提出文本打印图像(TPI)方法：将文本描述直接渲染到纯白画布上生成合成图像，将文本投影到图像模态，可低成本集成到现有LVLM训练流程

Result: 在4个模型和7个基准测试中，TPI比扩散模型生成的合成图像更有效。作为低成本数据增强策略也显示出实用价值

Conclusion: 文本中心训练具有显著潜力，TPI为LVLM的全自动数据生成开辟了新路径，提供了一种可扩展、低成本的训练范式
...
TL;DR: NAS-LoRA：一种结合神经架构搜索的参数高效微调方法，用于增强SAM在特定领域的适应能力，通过动态优化先验知识并减少训练成本。


<details>
  <summary>Details</summary>
Motivation: SAM作为视觉基础模型在图像分割中表现出色，但在特定下游任务（如医疗和农业成像）的适应仍面临挑战。现有LoRA方法虽然广泛使用，但缺乏对模型引入归纳偏置的能力，特别是SAM的Transformer编码器缺乏图像块的空间先验，可能阻碍高级语义信息的获取。

Method: 提出NAS-LoRA方法：1）在LoRA的编码器和解码器之间加入轻量级神经架构搜索（NAS）块，动态优化集成到权重更新中的先验知识；2）提出分阶段优化策略，帮助ViT编码器平衡权重更新和架构调整，逐步学习高级语义信息。

Result: 实验表明NAS-LoRA改进了现有PEFT方法，同时在不增加推理成本的情况下减少了24.14%的训练成本，证明了NAS在增强视觉基础模型PEFT方面的潜力。
...
TL;DR: PRIS提出了一种在推理时自适应修改提示词的框架，通过分析生成视觉内容中的重复失败模式来重新设计提示词，从而提高文本到视觉生成的对齐质量。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要通过增加采样步数或种子数来扩展视觉生成过程，但这种方法很快会遇到质量瓶颈，因为提示词在生成过程中保持不变。用户意图与生成视觉内容之间的精确对齐仍然是一个核心挑战。

Method: PRIS框架在推理时自适应地修改提示词：1）审查生成的视觉内容，2）识别跨视觉内容的重复失败模式，3）相应地重新设计提示词，4）使用修订后的提示词重新生成视觉内容。还引入了元素级事实校正验证器，在细粒度层面评估提示属性与生成视觉内容之间的对齐。

Result: 在文本到图像和文本到视频基准测试上的广泛实验证明了该方法的有效性，包括在VBench 2.0上获得15%的性能提升。结果表明，联合扩展提示词和视觉内容是充分利推理时扩展定律的关键。

Conclusion: PRIS框架通过自适应修改提示词来解决文本到视觉生成中的对齐问题，相比固定提示词的方法能更有效地利用推理时扩展，显著提高了生成质量。
...
TL;DR: 提出基于基分解的模块BDM，并扩展为SD²M、SD³M和TD²M，构建SD²Net用于单帧红外小目标检测，STD²Net用于多帧检测，在SISTD和MISTD任务上取得SOTA性能。
...
Method: 提出基分解模块BDM，将复杂特征分解为多个基特征，增强有用信息并消除冗余。基于BDM扩展出SD²M、SD³M和TD²M模块。SD²Net采用U型架构集成SD²M和SD³M，STD²Net通过TD²M引入运动信息扩展为多帧检测网络。

Result: 在SISTD任务上，SD²Net表现优于大多数现有网络。在MISTD数据集上，STD²Net达到87.68% mIoU，显著优于SD²Net的64.97% mIoU，实现SOTA性能。

Conclusion: 提出的基分解模块及其扩展模块有效解决了红外小目标检测中的背景干扰问题，构建的网络在单帧和多帧检测任务上都取得了优异的性能。
...
Conclusion: CookAnything框架在指令媒体和程序化内容创作方面具有广泛应用潜力，能够生成连贯、语义分明的图像序列来可视化任意长度的烹饪指令。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要分析动作如何执行，但忽略了动作产生的效果（action effect）。许多错误并不体现在执行过程中，而是体现在结果状态上，如意外的物体状态或错误的空间排列。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在视觉语言任务中表现出色，但存在幻觉问题，生成与输入视觉内容不一致的输出，这在精度敏感领域严重影响可靠性。现有方法通常通过干预注意力分数或输出logits来缓解幻觉，但只关注"如何干预"而忽略了"何时干预"，导致"过度干预"问题，反而引入新的幻觉和不必要的计算开销。

Method: 提出V-ITI框架：1）视觉忽视检测器：通过头级判别探针识别视觉忽视现象；2）视觉回忆干预器：仅在检测到视觉忽视时，用预存的视觉激活信息调制激活。该方法首先研究视觉忽视机制，发现可以通过MLLMs中的头级激活模式准确检测。

Result: 在八个基准测试和不同MLLM家族上的广泛实验表明，V-ITI能够持续缓解与视觉相关的幻觉，同时保持一般任务性能。

Conclusion: V-ITI通过智能检测视觉忽视并仅在需要时进行干预，有效解决了现有方法中的"过度干预"问题，为缓解MLLMs的幻觉问题提供了一种轻量级且高效的解决方案。
...
TL;DR: 提出Fair-LoRA方法，通过可微分的MaxAccGap损失函数优化医学视觉语言模型的公平性，在仅需0.24%可训练参数的情况下，显著减少不同人口群体间的诊断准确率差异。


<details>
  <summary>Details</summary>
Motivation: 医学视觉语言模型在医疗影像任务上表现出专家级性能，但在不同人口群体间存在显著的诊断准确率差异，这引发了公平性问题。需要开发参数高效且能显式优化公平性的方法。

Method: 提出公平感知的低秩适应方法，核心是引入可微分的MaxAccGap损失函数，实现跨人口群体准确率平等的端到端优化。具体提出三种方法：FR-LoRA将MaxAccGap正则化整合到训练目标中；GR-LoRA应用逆频率加权平衡梯度贡献；Hybrid-LoRA结合两种机制。

Result: 在10,000张青光眼眼底图像上评估，GR-LoRA将诊断准确率差异减少69%，同时保持53.15%的整体准确率。消融研究显示强正则化强度能以最小准确率代价实现最优公平性，种族特定优化可实现60%的差异减少。方法仅需0.24%可训练参数。

Conclusion: 该方法实现了参数高效的公平性优化，能够在资源受限的医疗环境中实际部署公平的医学AI系统，为解决医疗AI中的公平性问题提供了实用解决方案。
...
TL;DR: 论文提出了一种混合内容审核框架，结合监督分类和基于参考的相似性匹配，用于大规模用户生成视频平台（特别是直播环境）的多模态内容审核。


<details>
  <summary>Details</summary>
Motivation: 大规模用户生成视频平台的内容审核面临挑战，特别是在直播环境中需要及时、多模态且能应对不断演化的不良内容。传统方法难以处理新颖或微妙的违规案例。

Method: 采用混合审核框架：监督分类管道处理已知违规，基于参考的相似性匹配管道处理新颖或微妙案例。多模态输入（文本、音频、视觉）通过两个管道处理，使用多模态大语言模型（MLLM）将知识蒸馏到每个管道中以提高准确性，同时保持推理轻量级。

Result: 分类管道在80%精确率下达到67%召回率，相似性管道在80%精确率下达到76%召回率。大规模A/B测试显示用户观看不良直播的观看量减少6-8%。

Conclusion: 该框架展示了一种可扩展且适应性强的多模态内容治理方法，能够同时处理明确违规和新兴对抗行为，为大规模视频平台提供了有效的解决方案。
...
TL;DR: 提出Object-IVQA基准测试，用于评估视频中基于对象状态演变的推理能力，并开发了一个结合规划、感知和分析工具的智能体框架来提升性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于动作的方法难以处理真实过程中步骤顺序随对象状态变化的灵活性，需要转向以对象为中心的方法，将动作视为驱动状态转换的机制。

Method: 提出Object-IVQA基准测试（包含107个长视频和514个开放式问答对），并设计了一个智能体框架，整合对象中心规划、感知、分析和生成工具，支持显式证据检索和跨片段多跳推理。

Result: 实验显示现有大型视觉语言模型在对象级识别和推理方面表现不佳，而提出的框架实现了显著改进。

Conclusion: 对象中心范式能更好地理解程序性活动，提出的基准测试和框架为未来辅助AI在复杂现实任务推理方面提供了重要进展。
...
### [43] [CartoMapQA: A Fundamental Benchmark Dataset Evaluating Vision-Language Models on Cartographic Map Understanding](https://arxiv.org/abs/2512.03558)
...
TL;DR: CartoMapQA是一个专门评估视觉语言模型理解地图能力的基准数据集，包含2000多个地图问答样本，测试低中高三个层次的地图解读技能，发现现有模型在地图语义理解、空间推理和OCR方面存在显著缺陷。


<details>
  <summary>Details</summary>
Motivation: 尽管视觉语言模型在视觉-文本融合方面取得了进展，但它们在地图理解方面的能力尚未得到充分探索。地图解读对于导航、地理搜索、城市规划等实际应用至关重要，需要专门的评估工具来推动模型改进。

Method: 创建了CartoMapQA基准数据集，包含2000多个样本，每个样本由地图图像、问题（开放式或多选题）和正确答案组成。任务涵盖符号识别、信息提取、比例尺解读和路线推理等低中高三个层次的地图解读技能。对开源和专有视觉语言模型进行了评估。

Result: 评估发现现有模型存在持续挑战：经常难以理解地图特定语义，空间推理能力有限，容易产生OCR相关错误。这些弱点表明当前视觉语言模型在地图理解方面仍有显著不足。

Conclusion: CartoMapQA通过揭示视觉语言模型在地图理解方面的具体弱点，为未来模型架构改进提供了有价值的指导工具。该基准支持开发更适用于依赖可靠地图理解的实际应用（如导航、地理搜索、城市规划）的模型。数据集和代码已开源。
...
Method: EEA通过语义引导的层次树搜索过程实现探索-利用平衡：1）自主发现并动态更新任务相关的语义查询，收集与这些查询匹配的视频帧作为语义锚点；2）在树搜索过程中，优先探索语义相关帧，同时确保未知段的充分覆盖；3）通过显式建模不确定性，自适应结合视觉语言模型的内在奖励与语义先验，实现稳定精确的视频段评估。

Result: 在各种长视频基准测试上的实验验证了所提方法的优越性能和计算效率。

Conclusion: EEA框架通过语义引导的层次树搜索有效解决了长视频理解中的探索-利用平衡问题，在保持计算效率的同时显著提升了性能。
...
<details>
  <summary>Details</summary>
Motivation: 遥感图像中小目标检测面临两大挑战：1) 传统CNN深层网络会丢失小目标的关键信息；2) 遥感图像的空间冗余和复杂背景细节容易掩盖小目标。需要新的网络架构来解决这些问题。

Method: 提出MKSNet网络架构，包含两个核心组件：1) 多核选择机制(MKS)，使用大卷积核捕获广泛上下文信息，自适应选择核大小以动态处理空间细节；2) 双注意力机制，结合空间注意力模块（自适应调整特征图空间权重）和通道注意力模块（优化通道信息选择）。

Result: 在DOTA-v1.0和HRSC2016基准测试中，MKSNet显著超越现有最先进模型，证明其在处理多尺度和高分辨率遥感图像数据方面的优越能力。

Conclusion: MKSNet通过创新的多核选择机制和双注意力机制，有效解决了遥感图像中小目标检测的挑战，在复杂场景下展现出卓越的检测性能和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: DeepSeek-OCR展示了从少量视觉token中高保真重建渲染文本的能力，这引发了关于视觉上下文压缩的兴奋。但现有评估仅停留在重建层面，未测试这些表示是否真正有助于语言建模。

Method: 比较DeepSeek-OCR的视觉编码器与两种简单替代方法：参数无关的平均池化和学习的分层编码器，在相同压缩比下评估文本重建和语言建模性能

Result: 简单方法在匹配压缩比下达到或超过视觉编码器的重建性能，且在语言建模方面表现更优；视觉压缩在语言建模上甚至无法超越简单的截断方法

Conclusion: 视觉上下文压缩的兴奋超过了实际证据支持，视觉压缩在文本重建和语言建模方面并不比简单替代方法更有优势
...
### [47] [Exploiting Domain Properties in Language-Driven Domain Generalization for Semantic Segmentation](https://arxiv.org/abs/2512.03508)
*Seogkyu Jeon,Kibeom Hong,Hyeran Byun*

Main category: cs.CV

TL;DR: DPMFormer：一种用于语义分割的领域泛化框架，通过领域感知提示学习和对比学习解决视觉-文本语义错配问题，在多个DGSS基准上达到SOTA。


<details>
  <summary>Details</summary>
Motivation: 现有基于视觉语言模型的知识蒸馏方法忽略了视觉和文本上下文之间的语义错配问题，这种错配源于在单一源域上学习的固定上下文提示的僵化性。

Method: 提出DPMFormer框架：1）领域感知提示学习促进视觉和文本线索的语义对齐；2）领域感知对比学习结合纹理扰动多样化可观察域；3）领域鲁棒一致性学习最小化原始图像与增强图像预测的差异。

Result: 在多个领域泛化语义分割基准测试中取得了最先进的性能。

Conclusion: DPMFormer通过解决视觉-文本语义错配问题，建立了一个对多样化环境变化具有鲁棒性的框架，显著提升了领域泛化语义分割的性能。
...
TL;DR: 提出了首个面向任务的时空视频定位基准ToG-Bench，针对具身智能中的任务导向对象定位，包含任务导向定位、显隐双重定位和一对多定位三个关键特征。


<details>
  <summary>Details</summary>
Motivation: 现有时空视频定位研究主要局限于对象中心和描述性指令，缺乏任务导向推理能力，而这对具身智能体完成目标导向交互至关重要。

Method: 基于ScanNet视频构建包含100个标注片段和2,704个任务导向定位指令的基准，采用基础模型标注和人工精化的半自动化流程，并设计了针对多对象和显隐对象定位的任务级评估指标。

Result: 系统评估了7个最先进的多模态大语言模型，实验揭示了任务导向时空视频定位的内在挑战，以及在显隐定位和多对象定位方面存在的显著性能差距。

Conclusion: ToG-Bench凸显了在具身场景中连接感知与交互的困难，为任务导向时空视频定位研究提供了重要基准，揭示了现有模型的局限性。
...
TL;DR: CodeVision：一个基于代码作为通用工具的框架，通过两阶段训练方法显著提升多模态大语言模型在图像处理中的鲁棒性和工具组合能力


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在图像处理工具使用上存在局限性：工具集狭窄、缺乏实际必要性和可扩展性，且模型对简单方向变化或自然损坏的图像表现出惊人的脆弱性，性能显著下降

Method: 提出CodeVision框架，将代码作为通用接口来调用任何图像操作；采用两阶段训练方法：1）在有监督微调数据集上进行SFT，该数据集针对复杂多轮工具组合和错误恢复而构建；2）使用新颖密集的过程奖励函数进行强化学习，鼓励策略性和高效的工具使用

Result: 在Qwen2.5-VL和Qwen3-VL系列上的实验表明，该方法显著提升了模型性能，并促进了新兴能力：灵活的工具组合、高效的链式执行、以及从运行时反馈中实现鲁棒的错误恢复

Conclusion: CodeVision通过代码作为通用工具的框架和两阶段训练方法，有效解决了多模态大语言模型在图像处理中的鲁棒性问题，为工具组合和错误恢复提供了可扩展的解决方案
...
### [50] [AfroBeats Dance Movement Analysis Using Computer Vision: A Proof-of-Concept Framework Combining YOLO and Segment Anything Model](https://arxiv.org/abs/2512.03509)
*Kwaku Opoku-Ware,Gideon Opoku*

Main category: cs.CV

TL;DR: 提出一个结合YOLOv8/v11和SAM的舞蹈动作分析框架，通过计算机视觉技术实现无标记的舞蹈检测、分割和量化分析


<details>
  <summary>Details</summary>
Motivation: 探索利用现代计算机视觉技术实现自动化舞蹈动作分析的可行性，为舞蹈量化评估提供无需专业设备或标记的解决方案

Method: 集成YOLOv8/v11进行舞者检测，结合Segment Anything Model (SAM)进行精确分割，实现舞蹈动作的跟踪和量化分析

Result: 在49秒加纳AfroBeats舞蹈视频测试中，检测精度约94%，召回率89%，分割IoU约83%；主要舞者比次要舞者多执行23%舞步，运动强度高37%，使用空间多42%

Conclusion: 该框架证明了技术可行性，但存在单视频验证、缺乏系统标注和姿态估计对比等限制，为未来系统验证研究奠定了基础
...
TL;DR: BlackCAtt：一种基于因果像素的黑盒对抗攻击方法，针对目标检测器生成可解释、不可感知的对抗样本


<details>
  <summary>Details</summary>
Motivation: 现有对抗攻击方法多为白盒且架构特定，难以理解其工作原理。需要一种能够解释攻击机制、帮助开发者分析和防御的黑盒攻击方法。

Method: 提出BlackCAtt算法，使用最小因果像素集结合目标检测器的边界框，生成可解释、不可感知、可复现、架构无关的黑盒对抗攻击，导致边界框的消失、修改或新增。

Result: 在COCO测试集上，相比基线方法：移除检测效果提升2.7倍，修改检测提升3.86倍，触发虚假检测提升5.75倍。攻击生成的图像与原始图像几乎无法区分。

Conclusion: 因果像素识别能够产生更精确、更不可感知的对抗攻击，BlackCAtt展示了因果像素在理解目标检测器漏洞方面的强大能力，为模型分析和防御提供了新视角。
...
TL;DR: AdaptVision：一种通过粗到细的自适应视觉标记获取机制实现高效视觉语言建模的新范式，使用强化学习框架平衡准确性和效率
...
Method: 提出AdaptVision范式，采用粗到细方法：先处理低分辨率图像的压缩视觉标记，必要时调用边界框工具裁剪关键区域获取额外视觉信息。使用强化学习框架训练，核心是解耦回合策略优化(DTPO)，将学习目标分解为工具学习和准确性改进两个组件。

Result: 在多个VQA基准测试中，AdaptVision在消耗显著更少视觉标记的同时，实现了优于现有高效VLM方法的性能。

Conclusion: AdaptVision通过自适应视觉标记获取机制，为高效视觉语言建模提供了新范式，在保持准确性的同时显著降低了计算开销。
...
### [53] [CSMapping: Scalable Crowdsourced Semantic Mapping and Topology Inference for Autonomous Driving](https://arxiv.org/abs/2512.03510)
*Zhijian Qiao,Zehuan Yu,Tong Li,Chih-Chung Chou,Wenchao Ding,Shaojie Shen*

Main category: cs.CV

TL;DR: CSMapping系统通过潜在扩散模型学习地图结构先验，结合约束优化和聚类方法，实现高质量语义地图和拓扑路网中心线构建，质量随众包数据增加而提升。


<details>
  <summary>Details</summary>
Motivation: 众包数据可用于大规模自动驾驶地图构建，但低成本传感器噪声限制了数据量增加带来的质量提升。需要一种方法能够利用众包数据生成准确语义地图和拓扑路网中心线，且质量随数据量增加而持续改善。

Method: 1) 语义地图：训练潜在扩散模型学习真实世界地图结构先验，通过约束MAP优化在潜在空间进行，结合鲁棒向量化映射、扩散反演、高斯基重参数化、投影梯度下降和多起点优化；2) 拓扑地图：应用置信度加权k-medoids聚类和运动学细化处理轨迹，生成平滑、类人的中心线。

Result: 在nuScenes、Argoverse 2和大型专有数据集上实现了最先进的语义和拓扑地图性能，进行了全面的消融实验和可扩展性研究，证明质量随数据量增加而持续提升。

Conclusion: CSMapping系统能够从嘈杂的众包数据中生成高质量的语义地图和拓扑路网中心线，通过结合生成先验和优化方法，实现了对严重噪声的鲁棒性和未观测区域的合理补全，为大规模自动驾驶地图构建提供了有效解决方案。
...
TL;DR: 提出基于改进ResNet34的脑肿瘤分类模型，通过多尺度特征提取和通道注意力机制，在减少参数的同时将分类准确率提升至98.8%
...
Method: 以ResNet34残差网络为骨干网络，加入多尺度特征提取：使用多尺度输入模块作为第一层，Inception v2模块作为残差下采样层，并引入通道注意力机制模块为不同通道分配权重

Result: 五折交叉实验显示改进网络模型平均分类准确率约98.8%，比原始ResNet34提高1%，参数数量仅为原模型的80%，实现了更少参数、更高准确率的分类效果

Conclusion: 改进的ResNet34网络模型不仅提高了脑肿瘤图像分类的准确性，还减少了模型复杂度，实现了参数更少、准确率更高的分类效果
...
TL;DR: 提出Stable Signer模型，将手语生成重新定义为包含文本理解和Pose2Vid的端到端分层生成任务，通过SLUL链接器和SLP-MoE手势渲染模块，性能比当前SOTA提升48.6%


<details>
  <summary>Details</summary>
Motivation: 传统手语生成方法存在文本转换不准确、姿态生成误差、姿态渲染到真实视频的累积误差等问题，导致进展缓慢，需要简化优化任务目标

Method: 1. 重新定义SLP为分层端到端任务（文本理解+ Pose2Vid）；2. 提出SLUL（Sign Language Understanding Linker）进行文本理解，使用SAGM Loss训练；3. 设计SLP-MoE手势渲染专家模块生成高质量多风格手语视频

Result: 性能比当前最先进生成方法提升48.6%，能够端到端生成高质量、多风格的手语视频

Conclusion: 通过简化传统冗余结构、优化任务目标，提出的Stable Signer模型有效解决了手语生成中的累积误差问题，显著提升了生成质量
...
### [56] [FloodDiffusion: Tailored Diffusion Forcing for Streaming Motion Generation](https://arxiv.org/abs/2512.03520)
*Yiyi Cai,Yuhan Wu,Kunhang Li,You Zhou,Bo Zheng,Haiyang Liu*

Main category: cs.CV

TL;DR: FloodDiffusion是一个用于文本驱动的流式人体运动生成的新框架，通过改进的扩散强制方法实现实时生成，在HumanML3D基准上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖分块或自回归模型，难以实现文本对齐、无缝的运动序列生成。需要一种能够处理时变文本提示并保证实时延迟的流式运动生成框架

Method: 采用扩散强制框架，针对运动生成任务进行三项关键改进：1) 使用双向注意力而非因果注意力；2) 采用下三角时间调度器而非随机调度器；3) 使用连续时变方式引入文本条件

Result: 在HumanML3D基准测试中达到FID 0.057的SOTA性能，首次证明扩散强制框架在流式运动生成任务上的优越性

Conclusion: FloodDiffusion通过改进的扩散强制框架成功解决了流式人体运动生成问题，实现了文本对齐、无缝的运动序列生成，为实时运动生成提供了有效解决方案
...
TL;DR: OpenTrack3D：一种通用的开放词汇3D实例分割框架，通过视觉-空间跟踪器在线构建跨视图一致的对象提议，并使用多模态大语言模型增强组合推理能力，在多种基准测试中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 现有开放词汇3D实例分割方法存在两个关键限制：1）提议生成依赖于数据集特定的提议网络或基于网格的超点，在无网格场景中不适用且泛化能力有限；2）CLIP分类器的文本推理能力较弱，难以处理组合式和功能性的用户查询。

Method: 提出OpenTrack3D框架：1）使用2D开放词汇分割器生成掩码，通过深度信息提升到3D点云；2）利用DINO特征图提取掩码引导的实例特征；3）设计视觉-空间跟踪器融合视觉和空间线索保持实例一致性；4）核心流程完全无网格，但提供可选超点细化模块；5）用多模态大语言模型替代CLIP增强组合推理能力。

Result: 在ScanNet200、Replica、ScanNet++和SceneFun3D等多个基准测试上进行广泛实验，展示了最先进的性能和强大的泛化能力。

Conclusion: OpenTrack3D解决了现有开放词汇3D实例分割方法的局限性，通过在线跟踪和MLLM增强的组合推理，实现了在多样化、非结构化、无网格环境中的通用且准确的分割能力。
...
TL;DR: PULSE是一个统一的心脏图像分析多任务视觉语言框架，能在单一架构中同时处理解剖分割、疾病分类和临床报告生成，并具有跨模态和数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 目前心脏图像分析任务（解剖分割、疾病分类、临床报告生成）是碎片化的，由不同网络单独处理，缺乏统一的框架。现有方法无法在单一架构中统一这些目标，同时保持跨成像模态和数据集的泛化能力。

Method: 基于自监督表示构建多任务视觉语言框架，采用复合监督策略平衡区域重叠学习、像素级分类保真度和边界感知IoU细化。多尺度令牌重建解码器实现解剖分割，共享全局表示支持疾病分类和临床文本输出。

Result: PULSE能够学习任务不变的心脏先验知识，在数据集间具有鲁棒泛化能力，并能以最小监督适应新的成像模态，推动领域向可扩展的基础心脏分析框架发展。

Conclusion: PULSE首次在单一架构中统一了心脏图像分析的核心任务，实现了从像素到结构再到临床推理的端到端处理，为可扩展的基础心脏分析框架奠定了基础。
...
TL;DR: BlurDM：一种将模糊形成过程融入扩散模型的去模糊方法，通过双重扩散方案同时去噪和去模糊，显著提升现有去模糊方法性能。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在动态场景去模糊中未能充分利用模糊形成过程的本质特性，限制了其潜力。需要将模糊形成过程自然地整合到扩散模型中。

Method: 提出BlurDM，通过双重扩散前向方案隐式建模模糊形成过程（同时扩散噪声和模糊到清晰图像），在反向生成过程中推导出双重去噪和去模糊公式，并在潜在空间中执行以提高效率。

Result: 在四个基准数据集上的大量实验表明，BlurDM显著且一致地提升了现有去模糊方法的性能。

Conclusion: BlurDM成功地将模糊形成过程整合到扩散模型中，通过同时去噪和去模糊有效恢复清晰图像，为去模糊任务提供了灵活的生成先验网络。
...
TL;DR: DIQ-H是首个评估视觉语言模型在动态视觉退化时序序列中鲁棒性的基准，通过物理模拟的视觉退化、多轮问答任务和不确定性引导的迭代精炼方法，揭示了现有模型在真实世界部署中的严重鲁棒性缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有视觉语言模型基准主要关注静态高质量图像，忽略了真实世界应用中存在的时序退化和错误传播问题。在自动驾驶等安全关键应用中，瞬态视觉退化会引发幻觉并持续影响后续帧，这是关键的失效模式，但现有基准未能有效评估。

Method: DIQ-H基准应用基于物理的视觉退化（运动模糊、传感器噪声、压缩伪影），通过多轮问答任务评估幻觉持续性、错误恢复和时序一致性。提出不确定性引导的迭代精炼方法，利用轻量级VLM进行不确定性过滤，生成可靠的伪真值标注。

Result: 在16个最先进的VLM上实验显示显著的鲁棒性差距：即使是GPT-4o也只有78.5%的恢复率，开源模型在时序一致性上表现更差（低于60%）。UIR方法实现了15.3%的准确率提升，为大规模标注提供了有效解决方案。

Conclusion: DIQ-H为评估VLM在真实世界部署中的可靠性提供了全面平台，揭示了现有模型在动态视觉退化下的严重鲁棒性缺陷，强调了时序一致性和错误恢复能力在安全关键应用中的重要性。
...
Method: 三阶段框架：1) 微调点云生成模型从文本生成粗略物体表示；2) 设计基于超图的学习方法细化表示，将物体部件表示为图顶点；3) 使用扩散模型基于物体部件生成铰接关节（表示为图边）。
...
TL;DR: 本文提出了一种新的测试时缩放方法，通过文本嵌入扰动增强扩散模型的生成多样性和质量，该方法可与现有空间噪声互补，在频域分析基础上实现高效改进。


<details>
  <summary>Details</summary>
Motivation: 现有测试时缩放方法主要关注搜索策略和奖励模型，但忽视了T2I扩散模型中噪声随机性对方法性能的影响。本文旨在探索新的随机性形式——文本嵌入扰动，以提升生成多样性和质量。

Method: 1. 引入基于步骤的文本嵌入扰动，结合频率引导的噪声调度与空间噪声扰动；2. 根据频率特定贡献和扰动容忍度自适应调整扰动强度。

Result: 该方法可无缝集成到现有TTS方法中，在多个基准测试上显著提升性能，且几乎不增加额外计算开销。

Conclusion: 文本嵌入扰动是一种有效的随机性形式，可与空间噪声互补，在频域上分别增强低频和高频成分，从而提升扩散模型的生成质量。
...
<details>
  <summary>Details</summary>
Motivation: 现有场景文本编辑方法存在两个主要挑战：1）不一致性 - 编辑后的局部区域与周围区域不协调；2）长度不敏感 - 难以处理编辑前后文本长度的显著差异。需要一种能同时保持局部文本样式一致性和全局协调性的方法。

Method: 提出GLASTE框架，包含：1）全局-局部组合结构，同时整合高层全局上下文信息和精细局部特征；2）联合全局和局部损失函数；3）增强的文本图像特征表示；4）将文本样式表示为与图像大小无关的向量，可转移到不同大小的目标文本图像；5）使用仿射融合将目标文本图像填充到编辑区域，同时保持其纵横比不变。

Result: 在真实世界数据集上的大量实验验证表明，GLASTE模型在定量指标和定性结果上都优于先前方法，有效缓解了不一致性和长度不敏感两个挑战。

Conclusion: GLASTE通过全局-局部感知机制成功解决了场景文本编辑中的关键挑战，实现了更一致、更灵活的文本编辑效果，为实际应用提供了有效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 大型多模态模型处理长视频时面临上下文长度限制和高计算成本，现有查询感知帧选择方法计算开销大。本文挑战了复杂搜索机制普遍必要的假设，提出根据查询类型自适应选择策略。
...
Conclusion: 复杂帧选择机制并非普遍必要，应根据查询类型自适应选择策略。DIG框架通过区分全局和局部查询，实现了高效且有效的长视频理解。
...
Method: 引入信息独特性概念衡量token内在冗余度，设计三个模块：帧组融合、token分配和空间动态压缩，实现语义帧分组、自适应资源分配和细粒度空间压缩

Result: 大量实验表明UniComp在有限计算预算下持续优于现有压缩方法，能更好地保留关键视觉token，证明信息独特性在token压缩中的关键作用

Conclusion: 信息独特性驱动的压缩框架UniComp为视频压缩提供了新视角，在计算约束下有效提升信息保真度，具有重要应用价值
...
TL;DR: 研究探讨草图作为序列处理的合理性，发现绝对坐标优于相对坐标，非自回归解码器优于自回归解码器，时序重要性取决于具体顺序和任务。


<details>
  <summary>Details</summary>
Motivation: 尽管草图表示学习领域已有显著进展，但时序方面对表示质量的实际相关性仍不清楚。本研究旨在探究将草图视为序列是否合理，以及哪些内部顺序更具相关性。

Method: 通过比较不同顺序处理方式（绝对坐标vs相对坐标）和解码器类型（自回归vs非自回归），评估时序对草图表示的影响。

Result: 结果表明：1) 使用传统位置编码对草图序列建模是有效的；2) 绝对坐标始终优于相对坐标；3) 非自回归解码器优于自回归解码器；4) 时序重要性取决于具体顺序和评估任务。

Conclusion: 草图可以合理视为序列，但时序重要性与具体顺序和任务相关，绝对坐标和非自回归方法在草图表示学习中表现更优。
...
TL;DR: 提出Cross-Stain Contrastive Learning (CSCL)框架，通过多染色对比学习提升WSI表示质量，在癌症亚型分类、生物标志物状态分类和生存预测任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 计算病理学需要通用、可迁移的WSI表示。多染色（如IHC）能丰富H&E特征，但现有数据集对齐不佳，染色间错位导致特征不一致，限制了多染色学习的进展。
...
Conclusion: CSCL框架通过多染色对比学习有效提升了WSI表示的质量和可迁移性，为计算病理学提供了强大的基础表示学习方法。
...
Method: 提出金字塔稀疏注意力（PSA），采用多级池化的KV表示而非二值掩码。每个查询块动态分配较低池化级别给关键KV块，较高级别给次要KV块，在完全保留和完全剪枝之间创建信息插值。采用解耦块-瓦片设计的硬件友好内核确保高效执行。

Result: 在视频理解和生成基准测试中，PSA能保持上下文信息和视觉保真度，在效率-质量权衡方面优于或与现有稀疏注意力基线相当。
...
TL;DR: DOT-BI是一种利用人类运动感知区分人类与机器人的光学测试方法，通过在动态背景中隐藏数字，人类可感知而算法难以识别


<details>
  <summary>Details</summary>
Motivation: 需要一种快速简便的方法来区分在线调查和流程中的人类受访者与自动化系统，防止机器人干扰

Method: 使用与背景相同纹理的隐藏数字，通过数字与背景之间的运动和尺度差异使人类可感知，而逐帧算法处理无法获得有效信号

Result: 1) GPT-5-Thinking和Gemini 2.5 Pro等先进多模态模型无法正确提取数值；2) 在线调查中99.5%参与者成功完成任务，平均耗时10.7秒；实验室研究显示不影响易用性或完成时间

Conclusion: DOT-BI是一种有效的人类-机器人区分方法，已发布代码和100+预渲染变体以便在调查和在线流程中采用
...
TL;DR: BBF是一个上下文感知的视频帧插值框架，通过多模态条件引导（文本、音频、图像、视频）实现高质量插值，特别擅长处理音频-视觉同步的精细运动任务。


<details>
  <summary>Details</summary>
Motivation: 传统光流方法和现有基于扩散的方法在处理快速、复杂、高度非线性运动模式时存在困难，特别是在音频-视觉同步等细粒度运动任务中难以生成清晰、时间一致的结果。

Method: 1. 增强输入设计以灵活处理多模态条件；2. 提出解耦的多模态融合机制，将不同条件信号顺序注入DiT骨干网络；3. 采用渐进多阶段训练范式，使用起始-结束帧差异嵌入动态调整数据采样和损失权重。

Result: BBF在通用插值和音频-视觉同步插值任务上均优于专门的SOTA方法，建立了在协调多通道条件下视频帧插值的统一框架。

Conclusion: BBF通过上下文感知和多模态引导，成功解决了复杂运动模式下的视频帧插值问题，特别是在音频-视觉同步等细粒度任务中表现出色。
...
TL;DR: 该论文在生成模型效率和实际应用两方面做出贡献：第一部分通过6项创新改进归一化流架构，第二部分将生成模型应用于5个计算机视觉实际问题。


<details>
  <summary>Details</summary>
Motivation: 提升生成模型（特别是归一化流）的计算效率，并将这些高效模型应用于解决现实世界中的计算机视觉挑战，包括农业质量评估、地质测绘、隐私保护和艺术修复等领域。

Method: 第一部分：提出6项归一化流架构改进：1) 可逆3x3卷积层的数学条件证明；2) 更高效的Quad-coupling层；3) kxk卷积层的并行反演算法；4) 卷积逆运算的快速反向传播算法；5) Inverse-Flow中使用卷积逆进行前向传播；6) Affine-StableSR超分辨率模型。第二部分：1) 使用条件GAN的农业产品质量评估系统；2) 堆叠自编码器的无监督地质测绘框架；3) 基于人脸检测和图像修复的自动驾驶数据隐私保护方法；4) Stable Diffusion图像修复替换人脸和车牌；5) 适应多种退化类型的艺术修复扩散模型。

Result: 实现了归一化流架构的显著效率提升，开发了紧凑高效的Affine-StableSR超分辨率模型。在农业种子纯度测试中获得了良好的准确率，地质测绘中展示了优于传统方法的特征提取能力，建立了有效的隐私保护框架，以及能够处理多种退化类型的艺术修复模型。

Conclusion: 该研究通过理论创新和实际应用相结合，不仅推进了归一化流等生成模型的技术发展，还成功将这些技术应用于多个重要领域，展示了生成模型在解决现实世界问题中的巨大潜力。
...
Method: 提出HyperRNA框架，包含三个主要组件：预处理、编码和解码。预处理阶段基于3-bead粗粒度表示提取RNA骨架原子坐标构建图结构；编码阶段使用注意力嵌入模块和超图编码器处理这些图，捕捉高阶依赖和复杂生物分子相互作用；解码阶段以自回归方式生成RNA序列。

Result: 在PDBBind和RNAsolo数据集上进行了定量和定性实验，评估RNA序列生成和RNA-蛋白质复合物序列生成的逆折叠任务。实验结果表明HyperRNA不仅优于现有RNA设计方法，还突显了利用超图在RNA工程中的潜力。

Conclusion: HyperRNA是一个有效的RNA逆折叠框架，通过超图编码器-解码器架构成功解决了RNA序列设计问题，展示了超图在捕捉复杂生物分子相互作用方面的优势。
...
TL;DR: CloseUpAvatar是一种新颖的关节人体化身表示方法，通过双纹理系统（低频和高频细节）根据相机距离自动切换纹理，在保持近距离渲染质量的同时支持更广泛的相机运动范围。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理广泛的相机运动时难以保持近距离渲染质量，特别是当相机靠近化身表面时，需要更精细的细节表示。

Method: 使用带纹理的平面集表示化身，包含两组可学习纹理（低频和高频细节）。根据相机距离自动切换：近距离使用高频纹理，远距离逐渐降低其影响。

Result: 在ActorsHQ高分辨率数据集上实验显示，CloseUpAvatar在广泛的相机位置下，渲染质量在定性和定量上都优于现有方法，同时通过限制基元数量保持高FPS。

Conclusion: CloseUpAvatar通过距离自适应的纹理切换机制，实现了在更广泛相机运动范围内保持高质量渲染，为关节人体化身表示提供了更通用的解决方案。
...
TL;DR: HBFormer：一种新型混合桥接Transformer架构，通过多尺度特征融合解码器解决局部注意力机制难以有效融合局部细节与全局上下文的问题，在微肿瘤和微小器官分割任务中达到SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 当前基于移位窗口自注意力的Vision Transformers在医学图像分割中存在局限性：局部注意力机制难以有效融合局部细节与全局上下文，这对于微肿瘤和微小器官分割等挑战性任务尤为不利，因为这类任务需要同时具备细粒度边界定义和广泛的上下文理解能力。

Method: 提出HBFormer（混合桥接Transformer），采用经典U型编码器-解码器框架与强大的Swin Transformer骨干网络相结合的混合设计。核心创新是桥接机制，通过新颖的多尺度特征融合（MFF）解码器实现，该解码器使用通道和空间注意力模块（由一系列扩张卷积和深度卷积构建）来融合编码器的多尺度特征与全局上下文信息。

Result: 在多器官、肝肿瘤和膀胱肿瘤等具有挑战性的医学图像分割数据集上的综合实验表明，HBFormer取得了最先进的结果，在微肿瘤和微小器官分割方面展现出卓越能力。

Conclusion: HBFormer通过其混合桥接架构有效解决了局部注意力机制在融合局部细节与全局上下文方面的不足，为医学图像分割特别是微肿瘤和微小器官分割任务提供了强大的解决方案。
...
TL;DR: 提出基于检索增强的牙齿点云补全框架，通过可学习原型记忆库提供结构先验，稳定缺失区域推理并提升细节恢复能力


<details>
  <summary>Details</summary>
Motivation: 牙齿点云通常存在因遮挡和有限扫描视角导致的大面积缺失区域，这会偏置编码器全局特征并迫使解码器产生幻觉结构，需要更准确的补全方法

Method: 提出检索增强框架，将原型记忆库集成到标准编码器-解码器流程中：编码部分输入为全局描述符，从可学习记忆库检索最近邻流形原型，通过置信度门控加权与查询特征融合后解码；记忆库端到端优化，无需牙齿位置标签即可自组织为可重用牙齿形状原型

Result: 在自处理的Teeth3DS基准测试中，Chamfer距离持续改善，可视化显示更锐利的牙尖、脊和邻接过渡区域；模块即插即用，与常见补全主干网络兼容

Conclusion: 该方法通过利用跨样本规律性，为牙齿点云补全提供了简单有效的解决方案，能产生更准确和忠实的补全结果
...
TL;DR: Motion4D：将2D基础模型先验集成到4D高斯溅射表示中，解决动态场景3D一致性问题的新框架


<details>
  <summary>Details</summary>
Motivation: 现有2D视觉基础模型在单目视频动态场景分析中缺乏3D一致性，导致复杂3D环境中的空间错位和时间闪烁问题，需要结合几何理解能力

Method: 采用两阶段迭代优化框架：1）顺序优化更新运动和语义场保持局部一致性；2）全局优化联合细化所有属性实现长期一致性。引入3D置信度图动态调整运动先验，自适应重采样基于RGB和语义误差在欠表示区域插入新高斯点，迭代细化语义一致性

Result: 在点跟踪、视频对象分割和新视角合成等多种场景理解任务中，Motion4D显著优于2D基础模型和现有3D方法

Conclusion: Motion4D成功将2D先验集成到统一的4D表示中，有效解决了动态场景的3D一致性问题，为复杂3D环境理解提供了新解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法在运动控制方面接口有限，难以通过自然语言精确控制物体动态和摄像机轨迹，限制了复杂电影场景的创作

Method: 1. 定义运动领域特定语言(DSL)，基于电影摄影惯例；2. 利用LLM的程序合成能力将自然语言转换为结构化运动程序；3. 将运动程序确定性映射到3D轨迹；4. 构建大规模程序化数据集

Result: LAMP在运动可控性和用户意图对齐方面优于现有方法，建立了首个直接从自然语言生成物体和摄像机运动的框架

Conclusion: LAMP通过LLM驱动的运动规划，实现了从自然语言到3D轨迹的有效转换，为视频生成提供了更精确的运动控制能力
...
<details>
  <summary>Details</summary>
Motivation: 现有修复方法无法恢复复杂伪影，LiDAR方法依赖稀疏不完整的线索，需要一种能实现精确相机控制且具有结构一致性的视频生成方法。

Method: 采用两阶段训练：第一阶段使用相机位姿进行粗控制，第二阶段结合3DGS渲染进行细粒度视角和几何引导；提出基于3DGS的跨轨迹数据策略消除训练测试差异；构建ParaDrive数据集包含11万+并行轨迹视频对。

Result: ReCamDriving在相机可控性和结构一致性方面达到最先进水平，能够生成精确相机控制的新轨迹视频。

Conclusion: ReCamDriving通过密集3DGS渲染的几何引导和创新的训练策略，成功实现了纯视觉的精确相机可控视频生成，为自动驾驶等应用提供了有效解决方案。
...
TL;DR: FeatureLens是一个轻量级对抗样本检测框架，通过分析图像特征异常，使用浅层分类器实现高精度检测，具有优秀的泛化能力和可解释性。
...
Method: 提出FeatureLens框架，包含图像特征提取器(IFE)和浅层分类器(SVM、MLP或XGBoost)，模型参数量仅1,000到30,000个，使用51维特征进行对抗样本检测。
...
TL;DR: MSVP是一种多尺度视觉提示方法，通过全局、中尺度和局部提示图与输入图像融合，在小型图像数据集上显著提升各种骨干网络的性能，参数增加不到0.02%。


<details>
  <summary>Details</summary>
Motivation: 现有视觉提示方法主要针对大型Vision Transformers和高分辨率数据集（如ImageNet），而小型图像基准（如MNIST、Fashion-MNIST、CIFAR-10）在教育、原型设计和研究中广泛使用，但在提示方面却很少受到关注。

Method: 提出多尺度视觉提示（MSVP），学习一组全局、中尺度和局部提示图，通过轻量级1×1卷积与输入图像融合。该方法与骨干网络无关，参数增加极少。

Result: 在MNIST、Fashion-MNIST和CIFAR-10上使用简单CNN、ResNet-18和小型Vision Transformer进行统一基准测试，MSVP在可忽略的计算开销下带来一致的性能提升。

Conclusion: 多尺度提示即使在低分辨率图像上也能提供有效的归纳偏置，通过提示可视化和Grad-CAM的定性分析进一步验证了方法的有效性。
...
TL;DR: Colon-X是一个推进结肠镜多模态智能的开源项目，包含ColonVQA（最大结肠镜多模态数据集）和ColonReason（临床推理数据集），并开发了ColonR1模型，在数据稀缺条件下显著提升临床推理性能。


<details>
  <summary>Details</summary>
Motivation: 当前结肠镜多模态大语言模型在临床推理方面存在可靠性不足的问题，需要从多模态理解向临床推理演进，建立更稳健可信的结肠镜智能分析系统。

Method: 1. 构建ColonVQA数据集（110万+视觉问答条目，覆盖76个临床发现和18个多模态任务）；2. 评估22个多模态大语言模型的泛化性和抗干扰能力；3. 通过多专家辩论流程标注ColonReason临床推理数据集；4. 开发ColonR1模型，采用任务自适应奖励和梯度稳定优化技术。

Result: 1. 现有MLLMs在临床输出方面远未达到稳健可信；2. ColonR1在数据稀缺条件下达到56.61%总体准确率，比监督微调提升25.22%，为多模态结肠镜分析建立了新的推理基准。

Conclusion: Colon-X项目成功建立了从多模态理解到临床推理的桥梁，通过开源数据集和模型为结肠镜智能分析社区提供了重要基础，ColonR1模型在临床推理方面表现出显著优势。
...
TL;DR: 提出ConvRot方法，通过分组旋转量化技术实现扩散Transformer的W4A4推理，在保持图像质量的同时获得2.26倍加速和4.05倍内存减少


<details>
  <summary>Details</summary>
Motivation: 扩散Transformer在生成高质量图像方面表现出色，但随着模型规模增大，内存占用和推理延迟显著增加，给实际部署带来挑战。现有的旋转量化方法存在较大开销且难以处理扩散Transformer中的行级异常值。

Method: 提出ConvRot方法，采用分组旋转量化技术，利用正则Hadamard变换抑制行级和列级异常值，将复杂度从二次降低到线性。在此基础上设计ConvLinear4bit模块，集成旋转、量化、GEMM和反量化操作，实现无需重新训练的W4A4推理。

Result: 在FLUX.1-dev上的实验表明，该方法实现了2.26倍的加速和4.05倍的内存减少，同时保持了图像保真度。这是首次将旋转量化技术应用于扩散Transformer的即插即用W4A4推理。

Conclusion: ConvRot方法有效解决了扩散Transformer部署中的内存和延迟问题，通过创新的分组旋转量化技术实现了高效的W4A4推理，为扩散模型的实用化部署提供了可行方案。
...
TL;DR: GaussianBlender：首个前馈式文本驱动3D风格化框架，无需逐资产优化，实现即时推理编辑，通过结构化解耦潜在空间和潜在扩散模型实现高质量、多视角一致的3D风格化。
...
Result: GaussianBlender不仅提供即时、高保真、几何保持、多视角一致的风格化，而且超越了需要逐实例测试时优化的方法，实现了可扩展的实用化3D风格化。

Conclusion: 该框架解锁了实用化、民主化的大规模3D风格化，为游戏开发、虚拟现实和数字艺术提供了高效的解决方案。
...
TL;DR: 本文探讨主动视觉感知的概念、应用、机遇与挑战，提供该领域的全面综述


<details>
  <summary>Details</summary>
Motivation: 被动视觉系统在复杂环境中信息获取有限，主动视觉感知通过动态交互能获取更丰富信息，在机器人、自动驾驶等领域有重要应用价值，但目前面临实时处理、动态决策等挑战

Method: 本文采用综述研究方法，系统分析主动视觉感知的概念框架、应用场景、技术挑战和研究现状

Result: 明确了主动视觉感知的核心特征（动态环境交互、目标导向行为），识别了在机器人、自动驾驶、人机交互等领域的应用潜力，并指出了实时处理、动态决策、多模态融合等关键技术挑战

Conclusion: 主动视觉感知是应对复杂环境感知需求的重要范式，虽然面临诸多技术挑战，但通过持续研究和技术突破，有望在多个应用领域实现更广泛的采用
...
TL;DR: 提出SUSS（结构化不确定性相似度评分），通过结构化多元正态分布建模图像感知组件，在像素空间学习线性变换，实现可解释的感知相似度评估。


<details>
  <summary>Details</summary>
Motivation: 现有感知相似度方法存在局限性：深度感知损失（如LPIPS）依赖复杂非线性特征且不可解释，手工设计方法（如SSIM）可解释但缺乏关键感知特性。需要一种既与人类感知对齐又具有可解释性的方法。

Method: SUSS将图像建模为一组感知组件，每个组件用结构化多元正态分布表示。通过生成式自监督训练，使模型为人眼不可察觉的增强赋予高似然度。最终评分是组件对数概率的加权和，权重从人类感知数据集中学习。在像素空间学习图像特定的线性残差变换，实现可解释性。

Result: SUSS与人类感知判断高度一致，在不同失真类型上表现出强大的感知校准能力，提供局部化、可解释的相似度评估解释。作为感知损失在下游成像任务中展现稳定优化行为和竞争性性能。

Conclusion: SUSS成功结合了深度感知方法的高对齐性和手工方法的可解释性，通过结构化不确定性建模在像素空间实现透明检查，为感知相似度评估提供了新的可解释框架。
...
Result: 在Kaggle Image Matching Challenge 2025比赛中，平均准确率(mAA)获得显著提升，排名943支队伍中的第47位，获得银奖。
...
<details>
  <summary>Details</summary>
Motivation: 当前VLA模型在具身任务中经常产生冗余和不稳定的动作，限制了其在时间敏感场景中的应用。作者认为这是由于现有VLA模型的空间均匀感知场导致其容易被目标无关物体分散注意力。

Method: 提出PosA-VLA框架，通过姿态条件监督锚定视觉注意力，引导模型感知任务相关区域。该框架采用轻量级架构，无需辅助感知模块（如分割或接地网络）。
...
TL;DR: 提出DMDL框架，通过模型级和优化级的双重去偏学习解决无监督可见光-红外行人重识别中的模态偏差问题


<details>
  <summary>Details</summary>
Motivation: 现有两阶段学习管道在单模态训练中学习的模态特定线索会传播到跨模态学习中，导致模态偏差，损害身份识别和泛化能力

Method: 提出双重模态去偏学习框架：模型级使用因果启发的调整干预模块进行因果建模；优化级采用协作无偏训练策略，通过模态特定增强、标签细化和特征对齐中断偏差传播

Result: 在基准数据集上的广泛实验表明，DMDL能够实现模态不变特征学习和更泛化的模型

Conclusion: DMDL框架有效解决了无监督可见光-红外行人重识别中的模态偏差问题，实现了更好的模态不变特征学习和模型泛化
...
TL;DR: SelfDebias是一种完全无监督的测试时去偏方法，适用于任何使用UNet作为噪声预测器的扩散模型，通过识别图像编码器嵌入空间中的语义聚类来引导推理过程，减少输出分布与均匀分布之间的KL散度。


<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型在大型互联网数据集上训练，这些数据包含众多偏见，导致模型学习并再现这些偏见，产生刻板印象的输出。需要一种无需人工标注数据或外部分类器的去偏方法。

Method: SelfDebias在推理过程中识别图像编码器嵌入空间中的语义聚类，并使用这些聚类引导扩散过程，最小化输出分布与均匀分布之间的KL散度。该方法完全无监督，不需要人类标注数据集或为每个生成概念训练的外部分类器。

Result: 实验表明SelfDebias能够泛化到不同提示和扩散模型架构，包括条件模型和无条件模型。它不仅能有效去除关键人口统计学维度上的偏见，同时保持生成图像的视觉保真度，还能处理更抽象概念的偏见识别。

Conclusion: SelfDebias是一种有效的无监督测试时去偏方法，能够自动识别语义模式并减少扩散模型中的偏见，适用于各种模型架构和概念类型。
...
<details>
  <summary>Details</summary>
Motivation: VAR模型在并行处理同一尺度的多个token时可能导致结构错误，影响生成图像质量，需要一种高效的推理时优化方法

Method: 提出潜在尺度拒绝采样(LSRS)，使用轻量级评分模型评估每个尺度采样的多个候选token图，选择高质量图指导后续尺度生成，优先处理对结构一致性关键的前期尺度

Result: LSRS显著提升VAR生成质量，仅增加1%推理时间可将VAR-d30的FID从1.95降至1.78；增加15%推理时间可进一步降至1.66

Conclusion: LSRS为VAR模型提供了一种高效的测试时扩展解决方案，有效减轻自回归误差累积，保持计算效率的同时提升生成质量
...
TL;DR: 提出了一种基于深度学习的古埃及象形文字自动识别与翻译方法，通过分割、映射和翻译三阶段流程，实现了从图像到英文的翻译，BLEU得分达到42.2。


<details>
  <summary>Details</summary>
Motivation: 古埃及象形文字完全由图画组成，单个符号可能具有多重含义，翻译成英文面临诸多挑战。深度学习翻译应用发展迅速，能够产生显著影响生活的重要成果。

Method: 采用三阶段方法：1）使用Contour和Detectron2进行分割；2）将符号映射到Gardiner代码；3）使用CNN模型进行翻译。使用Morris Franken数据集和EgyptianTranslation数据集进行分类和翻译。

Result: 模型取得了42.2的BLEU分数，与先前研究相比是显著的结果。

Conclusion: 该方法成功实现了古埃及象形文字的自动识别和翻译，深度学习技术在古文字翻译领域具有重要应用价值。
...
Method: 结合数学变换的方法，硬件需求最小化，能抵抗受试者运动引起的失真，相对地面真实值的偏差小于5%。

Result: 在14名志愿者超过2.5小时的视频测试中，平均绝对误差为0.57次呼吸/分钟，明显优于先前工作，对受试者行为限制极少。

Conclusion: 该方法提供了一种鲁棒的远程呼吸率测量方案，能有效应对实际场景中的运动干扰，具有实际应用价值。
...
TL;DR: 提出LUnet架构，采用扁平层次结构和恒定通道数，大幅减少参数（30倍以上）的同时保持与标准Unet相当的性能


<details>
  <summary>Details</summary>
Motivation: 传统Unet架构在空间分辨率下采样时加倍通道数，导致内存占用大、训练批量小、推理延迟高。现有通道剪枝方法需要冗长优化且泛化性差。研究发现最终结构比通道选择策略更重要。

Method: 提出LUnet架构：1）紧凑扁平层次结构；2）分辨率减半时通道数不翻倍；3）恒定通道数跨层；4）利用跳跃连接大幅减少瓶颈通道；5）固定架构而非数据自适应剪枝。

Result: 在公共MRI数据集和两个内部CT数据集上评估，LUnet参数减少30倍以上，性能与传统Unet相当，且优于相同参数量的标准Unet。随机剪枝与SOTA剪枝方法效果相似。

Conclusion: 最终网络结构比通道选择策略更重要。LUnet通过扁平层次和恒定通道数实现高效分割，参数大幅减少且性能相当，特别适合医学图像分割应用。
...
TL;DR: 提出HP-Net网络，通过热图池化提取人体丰富、鲁棒且简洁的特征，结合多模态数据实现更鲁棒的动作识别
...
Result: 在NTU RGB+D 60/120、Toyota-Smarthome和UAV-Human等基准测试中优于现有方法，验证了HP-Net的有效性

Conclusion: HP-Net通过热图池化提取高质量人体特征，结合多模态学习实现了更鲁棒的动作识别，代码已开源
...
TL;DR: CoDA提出一种无需目标数据集预训练扩散模型的数据集蒸馏框架，通过识别目标数据集的核心分布并引导生成过程对齐该分布，在ImageNet-1K等基准上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有数据集蒸馏方法存在两个根本限制：1）需要目标数据集预训练的扩散模型，违背DD初衷且训练成本高昂；2）使用通用文本到图像模型时存在分布不匹配问题，无法准确捕捉目标特定语义

Method: 提出核心分布对齐框架，首先通过鲁棒的基于密度的发现机制识别目标数据集的"内在核心分布"，然后引导生成过程使生成样本与该核心分布对齐，从而弥合通用生成先验与目标语义之间的差距

Result: 在不依赖目标数据集特定训练生成模型的情况下，CoDA在所有基准测试中达到或超越先前依赖此类模型的方法性能，在ImageNet-1K的50 IPC设置下创下60.4%的新SOTA准确率

Conclusion: CoDA成功解决了数据集蒸馏中依赖目标数据集预训练模型和分布不匹配的问题，通过核心分布对齐机制实现了高效的数据集蒸馏，为实际应用提供了更可行的解决方案
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注空间域建模而忽略频域先验，Mamba架构在频域特征提取方面的潜力尚未被探索，特别是在恶劣天气条件下的交通图像修复中

Method: 提出FAMamba框架：1) 双分支特征提取块(DFEB)通过双向2D频率自适应扫描增强局部-全局交互；2) 先验引导块(PGB)通过基于小波的高频残差学习细化纹理细节；3) 为Mamba设计自适应频率扫描机制(AFSM)

Result: 大量实验证明了FAMamba的效率和有效性

Conclusion: FAMamba成功将频率引导与序列建模相结合，为恶劣天气条件下的交通图像修复提供了高效解决方案
...
TL;DR: 发布来自伊拉克埃尔比勒的339张前列腺穿刺活检全切片图像数据集，包含三位病理学家独立评估的Gleason评分和ISUP分级，支持AI模型在多样化人群中的开发和验证。


<details>
  <summary>Details</summary>
Motivation: 当前公开的病理学数据集稀缺且主要代表西方人群，AI模型在中东等数字化程度较低地区的泛化能力未知，因此需要发布代表多样化人群的数据集。

Method: 收集伊拉克埃尔比勒185名患者的前列腺穿刺活检样本，获得339张全切片图像，由三位病理学家独立进行Gleason评分和ISUP分级，使用三种不同扫描仪（Leica、Hamamatsu、Grundium）扫描，数据去标识化后以原始格式提供。

Result: 创建了包含339张前列腺活检全切片图像的数据集，包含三位病理学家的独立评估结果，支持分级一致性分析、颜色归一化和跨扫描仪鲁棒性评估，将在Bioimage Archive发布并采用CC BY 4.0许可。

Conclusion: 该数据集填补了中东地区病理学数据的空白，有助于开发更具全球代表性的AI病理学模型，促进AI在多样化人群中的泛化能力研究。
...
TL;DR: 小规模视觉Transformer（5M参数）的实验表明，预训练和微调有帮助但收益递减，中间微调可能因任务机制差异而对下游性能产生负面影响，建议针对性预训练和谨慎数据选择。
...
Method: 使用三个不同的预训练、中间微调和下游数据集及训练目标进行实验，分析它们对5M参数视觉Transformer的边际影响。通过系统比较不同训练阶段的组合效果，评估各阶段的贡献。

Result: 发现预训练和微调总是有帮助但收益递减；中间微调可能因任务机制差异而对下游性能产生有害影响；小规模ViT从针对性预训练和谨慎数据选择中获益最大，而无差别堆叠中间任务会浪费计算资源甚至降低性能。

Conclusion: 对于小规模视觉Transformer，最有效的策略是进行针对性预训练和精心选择数据，而非盲目堆叠多个中间任务，因为后者可能浪费计算资源并损害最终性能。
...
TL;DR: CaravelMetrics是一个用于脑血管自动分析的框架，通过骨架化图表示建模血管形态，计算15个形态、拓扑、分形和几何特征，支持全局和区域分析，在570个3D TOF-MRA扫描中验证了其可重复性和对年龄、性别、教育相关变化的敏感性。


<details>
  <summary>Details</summary>
Motivation: 需要一种可扩展、全自动的定量脑血管特征提取方法，以支持血管健康和衰老的规范建模和群体水平研究。现有方法可能缺乏系统性、自动化程度不足或难以处理大规模数据。

Method: 基于骨架化的图表示建模血管形态，整合基于图谱的区域分割、中心线提取和图构建，计算15个形态计量、拓扑、分形和几何特征，支持全局和动脉区域的多尺度分析。

Result: 在IXI数据集的570个3D TOF-MRA扫描（年龄20-86岁）中，CaravelMetrics产生了可重复的血管图，捕捉到年龄和性别相关的变化，以及教育相关的血管复杂性增加，与文献报道一致。

Conclusion: 该框架为定量脑血管特征提取提供了一个可扩展、全自动的方法，支持血管健康和衰老的规范建模和群体水平研究，能够检测与年龄、性别和教育相关的脑血管组织变化。
...
TL;DR: 开发了一种名为SSDCA的Siamese Swin Transformer模型，结合纵向内窥镜图像来区分直肠癌患者的完全临床缓解和局部复发，在62名患者的测试集上取得了81.76%的平衡准确率。


<details>
  <summary>Details</summary>
Motivation: 对于接受新辅助治疗后达到临床完全缓解的直肠癌患者，采用观察等待策略需要准确的方法来早期检测局部复发，以管理护理并预防远处转移。

Method: 开发了Siamese Swin Transformer with Dual Cross-Attention (SSDCA)模型，利用预训练的Swin transformer提取领域无关特征，通过双重交叉注意力机制结合重新分期和随访时的纵向内窥镜图像，无需图像空间对齐即可预测治疗反应。

Result: 在62名患者的测试集上，SSDCA取得了最佳性能：平衡准确率81.76%±0.04，敏感性90.07%±0.08，特异性72.86%±0.05。模型对血液、粪便、毛细血管扩张和图像质量差等伪影具有鲁棒性。特征提取显示最大化的簇间分离和最小化的簇内离散。

Conclusion: SSDCA能够有效结合纵向内窥镜图像来区分直肠癌患者的完全临床缓解和局部复发，为观察等待策略提供了客观准确的监测工具，有助于早期发现复发并改善患者管理。
...
<details>
  <summary>Details</summary>
Motivation: 现有的零样本视频编辑方法主要关注帧间对应关系，但仅通过注意力机制的软约束来识别有效特征是不够的，这会导致时间不一致性问题。

Method: FRESCO整合了帧内对应关系和帧间对应关系，形成更鲁棒的时空约束，确保语义相似内容在帧间的一致性转换，并显式优化特征而不仅仅是注意力引导。

Result: 在视频到视频转换和文本引导视频编辑两个零样本任务上验证了有效性，实验表明FRESCO能生成高质量、连贯的视频，显著优于当前零样本方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注以另一种模态为条件生成单一模态，或将它们与文本、音频等其他模态整合。由于2D视频和3D运动在结构和分布上存在显著差异，统一这两种模态进行同时优化和生成仍未被充分探索，具有重要挑战性。

Method: 1) 将视频和3D运动建模为统一的token序列，使用独立的嵌入层缓解分布差异；2) 设计序列建模策略，在单一框架中整合两种不同任务；3) 设计新颖的3D运动tokenizer，采用时间扩展策略，使用单个VQ-VAE生成量化运动token，配备多个专家解码器处理身体形状、平移、全局方向和身体姿态。

Result: 大量实验表明，该方法能同时生成对应的视频和运动，同时执行准确的动作捕捉。模型展示了统一建模的有效性。

Conclusion: 这项工作挖掘了LLMs融合多种数据类型的能力，为将人体中心信息整合到现有模型中铺平了道路，可能实现人体、物体和场景的多模态可控联合建模。
...
TL;DR: 提出一种通过超分辨率增强现有ground truth图像的新框架，使用自适应频率掩码融合原始和超分辨率图像的频率成分，生成感知增强的监督信号来提升真实世界图像恢复性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像恢复任务中，由于数据采集的实际限制，ground truth图像的质量限制了模型性能。现有数据集中的ground truth图像质量不足，需要更高质量的监督信号来提升恢复效果。

Method: 1. 使用条件频率掩码生成器学习自适应频率掩码；2. 将原始ground truth图像与其超分辨率变体的频率成分进行最优融合；3. 生成感知增强的ground truth图像，保持语义一致性同时丰富感知细节；4. 用增强的ground truth训练轻量级输出细化网络，可与现有恢复模型无缝集成。

Result: 大量实验表明该方法能持续提升恢复图像质量。用户研究验证了监督增强和输出细化的有效性。代码已开源。

Conclusion: 通过频率域混合生成增强的ground truth图像，为真实世界图像恢复提供了更高质量的监督信号，有效提升恢复性能，同时避免产生幻觉伪影。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于状态的循环神经网络在静态3D重建上表现良好，但在动态场景中容易受到运动伪影的影响。研究发现预训练transformer中已经编码了隐含的运动线索，但从未被显式利用。

Method: 通过分析transformer内部状态和图像token更新机制，发现跨层聚合自注意力图可以揭示动态区域被自然降权的模式。MUT3R利用这种注意力衍生的运动线索，在推理早期层中抑制动态内容，通过注意力级门控模块防止动态区域伪影在特征层次中传播。

Result: 该方法在多个动态基准测试中提升了时间一致性和相机姿态鲁棒性，无需重新训练或微调模型，让预训练transformer自我诊断运动线索并自我修正。

Conclusion: MUT3R提供了一种简单且无需训练的途径，通过早期调节稳定流式场景中的几何推理，实现了运动感知的流式重建。
...
TL;DR: TempR1是一个用于增强多模态大语言模型时间理解能力的多任务强化学习框架，通过设计针对不同时间任务类型的定位奖励，在多个基准测试中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 当前用于改进多模态大语言模型时间推理的强化学习方法通常局限于有限的任务类型和数据，限制了它们在不同时间理解场景中的泛化能力。需要一种系统性的方法来增强模型对时间结构的理解。

Method: 提出TempR1框架：1）构建多任务语料库，让模型接触多样化的时间结构和语义；2）基于GRPO算法实现稳定有效的跨任务优化；3）将时间任务分为三种预测区间与真实实例对应类型，并为每种类型设计定制定位奖励，以捕捉细粒度时间依赖关系。

Result: TempR1在多个基准测试中达到最先进性能。联合优化互补任务产生强大的协同效应，既增强了泛化能力，又提升了单任务性能。

Conclusion: TempR1为多模态大语言模型中的时间推理建立了一个可扩展且有原则的范式，通过系统性的多任务强化学习框架有效增强了模型的时间理解能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有无需调优的人脸个性化方法分为文本嵌入和适配器两种范式，但都无法同时实现高身份保真度和灵活文本控制性。需要一种统一的框架来解决这一挑战。

Method: UniID通过原则性的训练-推理策略整合两种范式：训练时采用身份聚焦学习方案，引导两个分支专门捕获身份特征；推理时引入归一化重缩放机制，恢复基础扩散模型的文本控制性，同时让互补的身份信号相互增强。

Result: 与六种最先进方法相比，UniID在身份保持和文本控制性方面都表现出优越性能，实现了高保真度的人脸个性化。

Conclusion: UniID通过统一两种范式并采用原则性的训练-推理策略，成功解决了无需调优人脸个性化中身份保真度和文本控制性的权衡问题，为相关领域提供了新的解决方案。
...
TL;DR: DirectDrag是一个无需手动掩码和文本提示的基于拖拽的图像编辑框架，通过自动软掩码生成和读出引导的特征对齐机制，实现高保真度的精确图像操控。
...
Method: 提出DirectDrag框架，包含两个关键创新：1) 自动软掩码生成模块，从点位移智能推断可编辑区域，自动定位沿运动路径的变形；2) 读出引导的特征对齐机制，利用中间扩散激活来保持基于点编辑时的结构一致性。

Result: 尽管无需手动掩码或提示，DirectDrag在DragBench和真实场景实验中实现了优于现有方法的图像质量，同时保持了有竞争力的拖拽精度。

Conclusion: DirectDrag为高质量、交互式图像操作提供了一个有效且实用的解决方案，通过最小化用户输入实现了精确高效的操控，同时保持了高图像保真度和准确的点对齐。
...
TL;DR: 研究发现现有前馈3D重建模型（如VGGT）即使没有显式离群点拒绝机制或噪声感知训练，也能内在区分干扰图像，通过特定层的内部表征实现有效的噪声过滤。


<details>
  <summary>Details</summary>
Motivation: 解决野外图像集合中"噪声"图像（与其他图像重叠度低或不相关的输入）对3D重建的负面影响。传统SfM流程通过几何验证和离群点拒绝处理这些问题，但前馈3D重建模型缺乏这些显式机制，导致在野外条件下性能下降。

Method: 通过在不同比例合成干扰图像下的深入分析，识别出VGGT模型中自然表现出离群点抑制行为的特定层。进一步探究发现该层编码了具有判别性的内部表征，能够实现有效的噪声过滤能力，无需额外微调或监督即可用于前馈3D重建中的离群视图拒绝。

Result: 在受控和野外数据集上的大量实验表明，这种隐式过滤机制具有一致性，并能很好地泛化到多样化场景中。

Conclusion: 现有前馈3D重建模型具有内在的离群点识别能力，通过利用特定层的内部表征可以实现有效的噪声过滤，为处理野外图像集合中的干扰图像提供了一种无需额外训练的新方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在高维数据空间应用群理论先验或建模变换时，难以解耦变换下的子空间；潜在空间方法虽然更灵活，但仍需要手动划分潜在变量为等变和不变子空间，限制了在表示空间中鲁棒学习和操作群作用的能力

Method: 使用可学习的二元掩码配合直通估计，动态划分潜在表示为变换敏感和不变组件；在统一优化框架中联合学习潜在解耦和群变换映射；可无缝集成到任何标准编码器-解码器架构

Result: 在五个2D/3D图像数据集上验证了方法，证明能够自动学习群作用的解耦潜在因子；下游分类任务确认了学习表示的有效性

Conclusion: 该框架首次实现了在潜在图像流形上学习群作用，自动发现变换相关结构，为群作用在表示空间中的学习提供了新方法
...
TL;DR: NVRC-Lite：轻量级神经视频表示压缩框架，通过多尺度特征网格和八叉树上下文模型，在保持低计算复杂度的同时提升压缩性能并加速编码速度。


<details>
  <summary>Details</summary>
Motivation: 现有基于隐式神经表示（INR）的视频压缩方法中，虽然NVRC取得了最佳性能，但计算复杂度较高。最近一些轻量级INR在低复杂度下表现不错，但现有方法通常使用自回归模型进行熵编码，编码速度过慢，限制了实际应用。

Method: 1. 将多尺度特征网格集成到轻量级神经表示中，使用更高分辨率网格提升INR在低复杂度下的性能；2. 提出基于八叉树的上下文模型用于高维特征网格的熵编码，替代缓慢的自回归模型，加速熵编码模块。

Result: NVRC-Lite在PSNR和MS-SSIM指标上分别比最佳轻量级INR视频编码器C3节省高达21.03%和23.06%的BD-rate，同时实现8.4倍编码加速和2.5倍解码加速。

Conclusion: NVRC-Lite成功将NVRC扩展到轻量级表示，通过多尺度特征网格和八叉树上下文模型，在保持低计算复杂度的同时显著提升了压缩性能并大幅加速了编码速度，为实际应用提供了可行的解决方案。
...
TL;DR: C3G：一种前馈框架，通过可学习token引导生成紧凑的3D高斯分布，实现高效场景重建与理解


<details>
  <summary>Details</summary>
Motivation: 现有方法使用逐像素3D高斯泼溅进行重建，但会产生大量冗余高斯分布，导致内存开销大、多视图特征聚合效果差，影响新视角合成和场景理解性能

Method: 提出C3G框架，仅在关键空间位置估计紧凑的3D高斯分布；引入可学习token通过自注意力聚合多视图特征来引导高斯生成；利用学习到的注意力模式进行高斯解码以高效提升特征

Result: 在无姿态新视角合成、3D开放词汇分割和视图不变特征聚合等任务上表现优异，相比现有方法具有更好的内存效率和特征保真度

Conclusion: 紧凑但几何意义明确的表示足以实现高质量场景重建和理解，证明了高效表示在3D计算机视觉中的重要性
...
TL;DR: RELIC是一个统一的交互式世界建模框架，通过压缩的历史潜在token和相机感知内存结构，实现实时长时探索、空间记忆保持和精确用户控制。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常只解决交互式世界建模的三个关键要素之一（实时长时流、一致空间记忆、精确用户控制），而无法同时满足所有要求。长时记忆机制往往会降低实时性能，因此需要一种统一框架来同时解决这三个挑战。

Method: 基于自回归视频扩散蒸馏技术，使用压缩的历史潜在token（包含相对动作和绝对相机位姿）作为KV缓存中的长时记忆。采用双向教师视频模型微调以超越原始5秒训练范围，并通过内存高效的自强制范式将其转换为因果学生生成器，实现长时教师和长学生自展开的全上下文蒸馏。

Result: RELIC作为140亿参数模型，在精心策划的Unreal Engine渲染数据集上训练，实现了16FPS的实时生成，相比先前工作表现出更准确的动作跟随、更稳定的长时流和更鲁棒的空间记忆检索。

Conclusion: RELIC通过统一框架同时解决了交互式世界建模的三个关键挑战，为下一代交互式世界建模奠定了坚实基础。
...
TL;DR: 提出了Double Interactive Reinforcement Learning (DIRL)框架，让视觉语言模型通过两阶段训练学习协调多种视觉工具，提升空间推理能力，在多个基准测试中取得SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在定性视觉理解方面表现良好，但在需要精确空间推理的具身应用中存在困难。虽然可以通过工具增强能力，但现有方法依赖手工提示或固定工具流水线，限制了模型发现最优工具使用模式的能力。

Method: 提出DIRL两阶段训练框架：1) 教学阶段：结合单工具专家的交互RL演示和前沿模型使用所有工具的轨迹；2) 探索阶段：通过持续RL进一步优化多工具协调。模型SpaceTools具备工具增强的空间推理能力。

Result: 在空间理解基准测试(RoboSpatial-Home, BLINK, BOP-ASK)上取得SOTA性能，比普通SFT基线提升12%，比RL基线提升16%。在7-DOF机器人上展示了可靠的现实世界操作能力。

Conclusion: DIRL框架有效解决了VLMs在多工具协调中的挑战，通过交互式探索和反馈学习，显著提升了空间推理能力，为具身智能应用提供了有前景的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于大型多模态模型的自动化图形设计方法存在几何布局不准确、缺乏专业工作流程所需的迭代分层编辑能力的问题，无法满足专业设计需求。

Method: 提出三阶段训练策略：1) 扰动监督微调；2) 视觉-现实对齐的强化学习；3) 美学反馈的强化学习。同时开发完整工作流程，将训练好的设计模型与生成模型结合，支持分层可控的迭代编辑。

Result: 实验表明PosterCopilot能够生成几何准确且美学优越的布局，为专业迭代设计提供前所未有的可控性。

Conclusion: PosterCopilot框架通过创新的训练策略和工作流程，解决了现有自动化图形设计方法的局限性，实现了专业级的布局推理和可控编辑能力。
...
TL;DR: 提出SimFlow方法，通过固定VAE编码器的方差为常数，简化归一化流训练流程，无需额外噪声添加和去噪步骤，同时提升重建和生成质量。


<details>
  <summary>Details</summary>
Motivation: 现有归一化流方法存在两个主要问题：1）需要添加随机噪声作为数据增强，导致复杂的噪声添加和去噪流程；2）使用预训练且冻结的VAE编码器，导致次优的重建和生成质量。

Method: 提出SimFlow方法，将VAE编码器的方差固定为常数（如0.5），而不是让编码器预测方差。这样可以让编码器输出更广泛的token分布，解码器学习从增强的token分布中重建干净图像，避免了额外的噪声设计。同时固定方差简化了VAE的证据下界，使得归一化流与VAE可以稳定地联合训练。

Result: 在ImageNet 256×256生成任务中，SimFlow获得gFID 2.15，优于当前最佳方法STARFlow（gFID 2.40）。当与REPA-E方法结合时，gFID进一步提升至1.91，在归一化流方法中达到新的最佳性能。

Conclusion: 通过简单的方差固定策略，SimFlow解决了现有归一化流方法的两个关键问题，实现了更简单的训练流程和更好的生成质量，为归一化流与VAE的联合训练提供了有效解决方案。
...
TL;DR: 单一生学习范式：通过单个人的自我中心视频训练视觉模型，利用多视角自监督学习，发现不同个体模型具有高度对齐的几何理解，并能有效迁移到深度估计等下游任务。


<details>
  <summary>Details</summary>
Motivation: 探索单个人生活视频能否提供足够的视觉学习信号，验证世界共享结构是否能让不同个体训练的模型产生一致的几何理解。

Method: 采用"单一生"学习范式，在单个人的自我中心视频上进行自监督视觉编码器训练，利用视频中的多视角信息，并开发了基于交叉注意力的新指标来量化不同模型内部表征的功能对齐。

Result: 1) 不同个体独立训练的模型发展出高度对齐的几何理解；2) 单一生模型学习到的几何表征能有效迁移到未见环境的深度估计任务；3) 一周30小时的单人数据训练效果与30小时多样化网络数据相当。

Conclusion: 世界共享结构既导致不同个体训练的模型具有一致性，又为视觉表征学习提供了强大信号，单一生学习范式是有效的视觉表示学习方法。
...
TL;DR: 论文提出了一种动态评估大语言模型安全性的方法，通过定义伦理熵概念来量化模型价值漂移，并开发监控系统实时检测对齐退化。


<details>
  <summary>Details</summary>
Motivation: 传统静态基准测试无法捕捉动态安全失效问题，如分布漂移下的价值漂移、越狱攻击和部署中对齐缓慢退化。需要建立动态监控框架来应对这些现实挑战。

Method: 基于智能第二定律，将伦理熵作为状态变量；定义五维行为分类法；训练分类器从模型转录中估计伦理熵S(t)；测量四个前沿模型在压力测试下的熵动态；计算有效对齐工作率gamma_eff；将S(t)和gamma_eff嵌入监控管道。

Result: 基础模型显示持续熵增长，而调优变体抑制漂移并将伦理熵降低约80%；从轨迹中估计出有效对齐工作率；监控系统能在熵漂移超过稳定阈值时发出警报，实现运行时价值漂移监督。

Conclusion: 伦理熵框架为动态监控大语言模型安全提供了可操作的方法，能够实时检测对齐退化，比静态基准更有效地应对实际部署中的安全挑战。
...
<details>
  <summary>Details</summary>
Motivation: 随着企业提供Embeddings-as-a-Service，模型知识产权保护变得重要。现有水印技术存在漏洞，需要更强大的防御机制来对抗模仿攻击。

Method: 1. 揭示现有EaaS水印的漏洞：通过文本改写攻击可移除水印
2. 提出WET水印技术：使用线性变换嵌入向量，通过反向变换和相似度比较进行验证

Result: 1. 文本改写攻击能有效绕过当前最先进的EaaS水印
2. WET水印对改写攻击具有近乎完美的可验证性，表现出强鲁棒性

Conclusion: 现有EaaS水印存在安全漏洞，WET水印技术能有效防御文本改写攻击，为EaaS提供商提供更可靠的模型知识产权保护方案。
...
TL;DR: 提出Reasoning Dependency Generation (RDG)框架，通过生成平衡的推理QA对来减轻大语言模型中的选择支持性偏见，在实验中显著改善模型表现。


<details>
  <summary>Details</summary>
Motivation: 大语言模型存在选择支持性偏见，会系统性偏向自己选择的选项，这可能影响AI辅助决策的客观性。现有去偏见方法主要针对人口统计和社会偏见，而认知偏见的处理方法尚未充分探索。

Method: 提出Reasoning Dependency Generation (RDG)框架，自动构建平衡的推理QA对，明确建模选择、证据和理由之间的依赖关系。该方法生成跨领域的大规模数据集，包含上下文依赖数据和依赖解耦数据。

Result: 使用RDG生成数据微调的大语言模型在基于记忆的实验中改善了81.5%，在基于评估的实验中改善了94.3%，同时在标准BBQ基准测试中保持相似性能。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在改进劳动力市场信息的分类，通过将招聘文本链接到欧洲技能、能力、资格和职业（ESCO）分类框架和欧洲资格框架（EQF），以超越表面技能提取，深入分析职业和资质的表示。

Method: 研究比较了两种主要方法：句子链接和实体链接，并开发了开源工具整合这两种方法。同时创建了两个标注数据集用于评估职业和资质的表示，并探索了使用生成式大语言模型的不同方式。

Result: 研究开发了开源工具和标注数据集，为劳动力分类和就业话语研究提供了计算基础设施。该工具能够将招聘文本链接到ESCO和EQF框架，推动了工作实体提取的技术进步。

Conclusion: 该研究通过开发开源工具和创建标注数据集，为数字中介经济中的工作、技能和劳动力市场叙事分析提供了先进的计算基础设施，推动了工作实体提取技术的发展。
...
TL;DR: InvertiTune框架通过受控数据生成管道和监督微调，实现单次推理的知识图谱构建，在计算效率和性能上优于现有方法。
...
Method: 提出InvertiTune框架：1）从大型知识库系统提取子图并过滤噪声；2）利用LLM生成对应的自然文本描述；3）使用生成的数据集对轻量模型进行监督微调，实现单次推理的KG构建。
...
TL;DR: 提出一个用于检测和分析政治文本中解释的框架，通过训练轻量级因果语言模型提取因果对，实现大规模研究解释
...
Result: 方法具有适度的标注需求、良好的泛化能力和相对于人工编码的准确性，能够大规模研究因果解释

Conclusion: 提出的框架为政治文本中的解释提供了系统分析方法，填补了现有研究空白，支持大规模因果解释研究
...
TL;DR: 提出随机掩码微调（RMFT）技术，在保持性能的同时显著降低LLM对个人身份信息（PII）的记忆，相比基线减少80%以上的提取率，仅增加5.73%的困惑度。


<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在训练过程中容易记忆个人身份信息，带来严重的安全和隐私风险，需要开发既能保护隐私又不过度影响模型性能的微调方法。

Method: 提出随机掩码微调（RMFT）技术，在微调过程中随机掩码部分输入，减少模型对敏感信息的记忆。同时提出MaxTER评估框架，使用AURC指标评估隐私-效用权衡。
...
TL;DR: LLM辅助的注释管道用于分析西班牙语-英语和西班牙语-瓜拉尼语双语话语的社会语言学和主题模式，揭示了人口统计学因素与话语功能之间的系统性联系


<details>
  <summary>Details</summary>
Motivation: 传统上，社会语言学模式需要通过手动注释才能获取，这在大规模语料库分析中效率低下。本研究旨在利用大语言模型自动恢复可解释的社会语言学模式，推进跨语言和低资源双语研究的计算方法。

Method: 使用大语言模型自动标注了3,691个语码转换句子的主题、体裁和话语-语用功能，整合了迈阿密双语语料库的人口统计元数据，并为西班牙语-瓜拉尼语数据集添加了新的主题注释。

Result: 在迈阿密数据中发现了性别、语言优势和话语功能之间的系统性联系；在巴拉圭文本中观察到正式的瓜拉尼语和非正式的西班牙语之间的明显双语分工。这些发现用语料库规模的定量证据复制并扩展了早期的互动和社会语言学观察。

Conclusion: 大语言模型能够可靠地恢复传统上只能通过手动注释获取的可解释社会语言学模式，这为跨语言和低资源双语研究提供了有效的计算方法。
...
TL;DR: PERCS数据集：包含针对四种用户角色（普通人、医学生、非医学研究者、医学专家）定制的生物医学摘要总结，支持个性化可控摘要生成研究
...
Method: 构建PERCS数据集，包含生物医学摘要及其针对四种角色的定制总结：普通人、医学生、非医学研究者、医学专家；所有总结由医生审核事实准确性和角色一致性；使用详细错误分类法评估

Result: 技术验证显示不同角色总结在可读性、词汇和内容深度上存在明显差异；基准测试评估了四个大语言模型在自动评估指标上的表现，建立了未来研究的基线结果

Conclusion: PERCS数据集支持角色特定沟通和可控生物医学摘要研究，公开提供数据集、标注指南和评估材料，推动个性化医学信息传播的发展
...
TL;DR: 提出Idea-Gated Transformer架构，通过分离语义规划和语法生成来解决自回归语言模型中的"主题漂移"问题，使用概念向量门控机制实时抑制语义无关的词汇。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在下一个词预测训练中容易产生"主题漂移"问题，生成内容会偏离初始提示，主要依赖局部关联而非全局规划。虽然增大模型规模可以缓解此问题，但NTP目标的根本短视性仍然存在。

Method: 引入Idea-Gated Transformer架构，分离语义规划和语法生成。添加辅助的"Idea Head"来预测未来上下文窗口的词袋分布，创建潜在"概念向量"，在生成过程中主动门控主要词汇表。提出可微分门控机制，实时抑制语义无关的词汇，有效修剪搜索空间。

Result: 在WikiText-103上的实验表明，Idea-Gated模型在验证困惑度上与标准GPT-2基线相当，但在领域保持方面显著更优。定性和定量分析显示，门控机制成功将生成锁定在特定语义簇（如金融、科学）中，并抵抗关联漂移。

Conclusion: Idea-Gated Transformer为更可控的语言建模提供了参数高效的路径，通过分离规划和生成，有效解决了主题漂移问题，实现了更好的语义一致性。
...
TL;DR: 提出HBLR框架，结合置信度感知的符号翻译与假设驱动的逆向推理，提升逻辑推理的准确性和效率


<details>
  <summary>Details</summary>
Motivation: 当前LLM的前向推理方法存在冗余推理路径、幻觉步骤和语义漂移问题，导致推理效率低且不可靠。需要更接近人类演绎思维的推理范式。

Method: HBLR框架包含两个核心模块：1) 置信度感知符号翻译：仅将高置信度文本转换为FOL等逻辑形式，不确定内容保留自然语言，并通过翻译反思模块确保语义保真度；2) 假设驱动逆向推理：模拟人类演绎思维，假设结论为真并递归验证前提，通过推理反思模块识别纠正错误推理步骤。

Result: 在五个推理基准测试上的广泛实验表明，HBLR在准确性和效率方面均持续优于强基线方法。

Conclusion: HBLR通过结合符号翻译和逆向推理，提供了一种更高效可靠的逻辑推理框架，模拟人类演绎思维过程，显著提升了LLM的逻辑推理能力。
...
TL;DR: 提出高阶注意力网络(Hon)，通过递归框架增强Transformer的表示能力，打破标准注意力的线性瓶颈，在多个基准测试中优于标准Transformer。
...
Method: 设计递归框架的高阶注意力网络，通过嵌套自注意力机制动态精炼Query和Key表示，在最终注意力计算前让token聚合全局上下文并建模高阶相关性，采用参数高效的权重共享策略。

Result: 理论分析表明该方法打破了标准注意力的线性瓶颈，实证结果显示在多个基准测试中优于标准Transformer。

Conclusion: 高阶注意力网络通过递归框架有效增强了Transformer的表示能力，解决了标准注意力的低秩瓶颈问题，同时保持了参数效率。
...
TL;DR: 收集了Portal 2合作模式中的11.5小时语音对话语料库，包含24.5K话语，分析了复杂空间指代、澄清修复等独特语言现象，并公开了多模态数据。
...
Method: 收集Portal 2合作模式中的玩家对话，构建包含11.5小时语音、24.5K话语的语料库，分析玩家语言和行为，识别独特语言现象。

Result: 识别出复杂空间指代、澄清修复、临时约定形成等独特语言现象，这些在现有对话语料库中罕见。语料库包含玩家视频、音频、转录、游戏状态数据及人工/自动标注。

Conclusion: Portal对话语料库为分析复杂、情境化、协作问题解决场景中的语言使用提供了宝贵资源，已公开多模态数据支持未来研究。
...
TL;DR: Dual LoRA通过将低秩矩阵分为幅度组和方向组，并分别应用ReLU和符号函数，改进了传统LoRA的性能，在多种NLP任务上优于LoRA及其变体。


<details>
  <summary>Details</summary>
Motivation: 传统LoRA方法基于低秩假设，在适应下游任务时性能往往不理想。需要一种方法来更好地模拟基于梯度优化的全参数微调过程，以提高模型性能。

Method: 提出Dual LoRA方法，将低秩矩阵分为两组：幅度组控制参数是否更新及更新幅度（使用ReLU函数），方向组决定参数更新方向（使用符号函数）。这种方法更好地模拟了全参数微调中的梯度优化过程。

Result: 在广泛的NLP任务（自然语言生成、理解、常识推理）上，使用GPT-2、RoBERTa、DeBERTa、LLaMA-1/2/3等基线模型进行实验，Dual LoRA在相同可训练参数数量下，始终优于LoRA及其最先进的变体。

Conclusion: Dual LoRA通过引入归纳偏置，将参数更新过程分解为幅度和方向两个维度，有效提升了LoRA的性能，为参数高效微调提供了更优的解决方案。
...
Method: 提出PretrainZero强化主动学习框架：1) 主动预训练：学习统一推理策略，主动识别语料中的合理信息内容并进行预测；2) 自监督学习：无需可验证标签或监督微调，直接在通用语料上使用RL预训练；3) 验证缩放：通过处理逐渐增加的掩码跨度来增强推理能力
...
TL;DR: Top-k注意力机制在解码和训练阶段的有效性研究，验证了在解码阶段仅保留与查询最相似的关键词能达到与完整注意力相当或更好的性能，并通过训练-推理一致性进一步提升模型表现。
...
Result: 实验表明：1）解码阶段仅保留与查询最相似的关键词能在HELMET和LongBench v2等下游任务上达到与完整注意力相当或更好的性能；2）训练-推理一致性进一步释放Top-k解码潜力；3）下游任务性能与近似精度正相关；4）Top-k注意力SFT导致下游任务熵降低，验证了低熵状态更适合Top-k解码的假设。

Conclusion: Top-k注意力机制是解决LLM推理计算成本瓶颈的有效方法，通过精确和近似的Top-k操作，结合训练-推理一致性策略，能在保持性能的同时显著降低计算复杂度，为长上下文建模提供高效解决方案。
...
TL;DR: 大型语言模型在摘要生成中的推理能力研究：推理策略对摘要质量和事实忠实度存在权衡，并非万能解决方案


<details>
  <summary>Details</summary>
Motivation: 虽然大型语言模型在数学和代码生成等分析任务中表现出色，但其在抽象摘要生成中的效用被广泛假设但未经验证。本研究旨在填补这一空白，验证推理策略在摘要任务中的实际效果。

Method: 首先将通用推理策略定制到摘要领域，然后对8种推理策略和3个大型推理模型在8个不同数据集上进行系统化、大规模的比较研究，评估摘要质量和忠实度。

Result: 研究发现推理并非通用解决方案，其效果高度依赖于具体策略和上下文。观察到摘要质量与事实忠实度之间的权衡：显式推理策略倾向于提高流畅性但牺牲事实基础，而大型推理模型中的隐式推理则呈现相反模式。增加模型的内部推理预算不会改善事实一致性，甚至可能损害它。
...
<details>
  <summary>Details</summary>
Motivation: 叙事是宣传的认知和情感支架，将孤立的说服技巧组织成连贯的故事。需要开发细粒度叙事分类方法来分析偏见新闻文章中的意识形态叙事框架

Method: 1) 创建INDI-PROP数据集：包含1,266篇关于CAA和农民抗议的文章，进行三层标注：意识形态偏见、事件特定细粒度叙事框架、说服技巧；2) 提出FANTA和TPTC两个GPT-4o-mini引导的多跳提示推理框架

Result: FANTA和TPTC框架在偏见、叙事和说服技巧分类任务上相比基线方法都有显著改进

Conclusion: 该研究为分析宣传叙事提供了首个意识形态基础的细粒度叙事数据集和有效的多跳推理框架，有助于理解印度新闻媒体中的意识形态叙事结构
...
TL;DR: 提出一个可解释的事实一致性评估框架，通过分解文本为原子事实并使用加权指标，解决LLM幻觉问题，特别针对临床等高风险领域。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在自然语言处理任务中取得显著进展，但容易生成错误或误导性但看似合理的论点（幻觉问题）。这个问题在高风险领域如临床应用中尤其令人担忧，因为事实不准确可能带来严重后果。现有评估指标未能充分评估事实一致性且缺乏可解释性，使得诊断和缓解错误变得困难。

Method: 提出一个可解释的事实一致性评估框架，将文本分解为原子事实，并引入灵活、无模式的方法。与之前使用绝对指标的方法不同，我们采用加权指标来增强事实评估。此外，提出一种机制来控制复杂领域中的评估复杂度。

Result: 在流行的通用和临床数据集上对方法进行了基准测试，并发布了代码以支持未来研究中事实感知模型的训练。

Conclusion: 该框架为解决LLM幻觉问题提供了可解释的事实一致性评估方法，特别适用于高风险领域，通过加权指标和复杂度控制机制提高了评估的准确性和实用性。
...
TL;DR: 意大利首次全面调查显示：生成式AI被广泛用于工作和个人任务，正取代其他技术成为主要信息来源，但用户数字素养低且存在显著性别鸿沟，女性采用率仅为男性一半。


<details>
  <summary>Details</summary>
Motivation: 研究生成式AI聊天机器人的采用情况，关注其可能因采用不均衡和用户对其局限性认识不足而加剧数字鸿沟的风险。意大利缺乏相关实证数据，需要了解采用模式、使用习惯和数字素养现状。

Method: 基于新收集的1,906名意大利语成年人的调查数据，进行首次全面的实证映射，分析生成式AI的采用率、使用模式和数字素养。

Result: 1) 生成式AI在工作和个人用途中被广泛采用，包括情感支持和医疗建议等敏感任务；2) 正取代其他技术成为主要信息来源；3) 用户数字素养低，难以识别错误或虚假信息；4) 存在显著性别鸿沟，女性采用率仅为男性一半，在年长群体中尤为明显；5) 数字素养是采用的关键预测因素，但只能部分解释性别差异。

Conclusion: 生成式AI的多用途使用需要有针对性的教育计划，并进一步研究数字素养无法完全解释的公平参与障碍。需要解决用户素养不足和性别鸿沟问题，确保AI技术的公平采用。
...
TL;DR: 提出了Hydro-SE Bench基准测试，包含4000道选择题，评估大语言模型在水科学与工程领域的知识与应用能力，发现商业模型准确率0.74-0.80，小参数模型0.41-0.68，模型在自然物理科学相关领域表现良好，但在行业标准和水利结构等专业领域存在不足。
...
Method: 提出Hydro-SE Bench评估基准，包含4000道选择题，涵盖9个子领域，从基本概念知识、工程应用能力、推理计算能力三个维度评估大语言模型。评估了商业模型和小参数模型的表现。

Result: 商业大语言模型准确率在0.74-0.80之间，小参数模型在0.41-0.68之间。模型在与自然物理科学密切相关的子领域表现良好，但在行业标准、水利结构等专业领域知识上存在困难。模型扩展主要提升推理计算能力，但在实际工程应用问题处理上仍有很大提升空间。

Conclusion: 研究揭示了LLMs在Hydro-SE任务中的优势和不足，为模型开发者提供了明确的训练目标，为Hydro-SE研究人员提供了应用LLMs的实践指导，强调了模型在专业领域知识方面的改进需求。
...
TL;DR: LLMs中的语法知识表征研究：通过功能定位方法发现不同句法现象（特别是语法一致性）在模型中共享神经单元，构成有意义的句法功能类别。


<details>
  <summary>Details</summary>
Motivation: 虽然大语言模型能够可靠地区分语法正确和不正确的句子，但语法知识如何在模型内部表征仍然是一个开放性问题。研究者希望探究不同句法现象是否在LLMs中共享或使用不同的组件。

Method: 采用受认知神经科学启发的功能定位方法，识别七个开源模型中67种英语句法现象最敏感的单元。这些单元在包含相应现象的句子中一致被激活，并因果性地支持模型的句法表现。研究还扩展到俄语、中文，并在57种不同语言中进行跨语言分析。

Result: 不同类型的语法一致性（如主谓一致、照应一致、限定词-名词一致）招募重叠的单元集合，表明一致性构成了LLMs中有意义的功能类别。这一模式在英语、俄语和中文中都成立，并且在跨语言分析中，结构更相似的语言在主谓一致上共享更多单元。

Conclusion: 句法一致性——作为句法依赖关系的关键标记——在LLMs的表征空间中构成了有意义的类别，揭示了LLMs内部语法知识的组织方式。
...
TL;DR: AITutor-EvalKit是一个使用语言技术评估AI导师教学质量的工具，提供演示评估、模型检查和数据可视化功能，服务于教育利益相关者和ACL社区。


<details>
  <summary>Details</summary>
Motivation: 当前AI导师系统缺乏标准化的教学质量评估工具，教育利益相关者和ACL社区需要能够评估AI导师教学效果、收集用户反馈和标注的工具。

Method: 开发了一个集成语言技术的应用程序，包含演示评估、模型检查、数据可视化等功能模块，支持用户反馈和标注收集。

Result: 创建了AITutor-EvalKit工具，能够评估AI导师的教学质量，为教育利益相关者提供实用的评估软件，同时支持ACL社区的研究和学习需求。

Conclusion: AITutor-EvalKit是一个有价值的工具，不仅能够评估AI导师的教学质量，还能促进教育技术研究和用户反馈收集，对教育利益相关者和ACL社区都有重要意义。
...
TL;DR: DZ-TDPO提出了一种非破坏性对齐框架，通过冲突感知的动态KL约束和可学习的时间注意力偏置，解决长对话系统中的状态惯性问题，在保持模型通用能力的同时实现SOTA性能。


<details>
  <summary>Details</summary>
Motivation: 长对话系统存在"状态惯性"问题，即静态约束导致模型无法解决用户意图演变与历史上下文之间的冲突，这限制了对话系统的适应性和响应准确性。

Method: 提出DZ-TDPO框架：1）冲突感知的动态KL约束，根据上下文冲突程度自适应调整对齐强度；2）可学习的时间注意力偏置，调节模型对历史信息的关注程度。该方法避免破坏性权重更新，保持模型原有能力。

Result: 在Multi-Session Chat数据集上，DZ-TDPO达到SOTA胜率（Phi-3.5模型86.2%），Qwen2.5-7B模型达到99.4%胜率且困惑度开销可忽略。发现"容量-稳定性权衡"现象：小模型需要付出"对齐税"（困惑度激增）来克服历史惯性，而大模型能近乎完美对齐且保持通用能力（MMLU）。

Conclusion: 状态惯性问题可以通过精确的注意力调节而非破坏性权重更新来缓解，DZ-TDPO框架在保持模型通用能力的同时有效解决长对话中的冲突问题，为对话系统对齐提供了新思路。
...
TL;DR: AR-Med是一个用于医疗搜索的自动相关性评估框架，通过检索增强和知识蒸馏技术，在在线医疗平台实现高精度、低成本的LLM部署。


<details>
  <summary>Details</summary>
Motivation: 在线医疗平台的搜索准确性对用户安全和服务效果至关重要，但传统方法难以理解复杂查询。虽然LLM有潜力，但在医疗领域部署面临幻觉、专业知识缺失和高成本等挑战。

Method: 1. 检索增强方法：将LLM推理基于已验证的医学知识；2. 知识蒸馏方案：将大型教师模型压缩为紧凑的学生模型；3. 创建LocalQSMed基准：多专家标注的数据集，用于模型迭代和离线-在线性能对齐。

Result: 离线准确率超过93%，比原在线系统提升24%绝对改进；在线相关性和用户满意度显著提升；成功在在线医疗交付平台大规模部署。

Conclusion: AR-Med为医疗领域开发可信赖、可扩展的LLM系统提供了实用蓝图，解决了LLM在医疗搜索中的关键挑战，实现了高精度和高效部署。
...
TL;DR: 提出了ESPO方法，通过序列级优化解决扩散大语言模型中的强化学习挑战，使用ELBO作为似然代理，在数学推理、编程和规划任务上显著优于token级基线方法。


<details>
  <summary>Details</summary>
Motivation: 强化学习在自回归语言模型中很有效，但难以应用于扩散大语言模型。核心困难在于似然近似：自回归模型天然提供token级条件概率，而扩散模型通过迭代去噪步骤生成序列，缺乏这种分解。
...
Conclusion: ESPO建立了序列级优化作为扩散大语言模型中强化学习的原理性和经验有效的范式，解决了自回归和扩散模型之间的根本性不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 揭示当前大语言模型安全对齐策略的不足，发现模型在表示层面的脆弱性，即通过简单的上下文替换就能劫持内部语义表示，绕过安全防护。

Method: 提出"上下文表示劫持"攻击方法：在多个上下文示例中系统性地将有害关键词（如"炸弹"）替换为良性标记（如"胡萝卜"），配合有害请求的前缀。这种替换导致良性标记的内部表示向有害标记收敛，将有害语义嵌入到委婉语中。

Result: 攻击无需优化，可跨模型家族广泛转移，在闭源和开源系统上都取得高成功率。在Llama-3.3-70B-Instruct上，仅用单句上下文覆盖就达到74%的攻击成功率。可解释性工具显示语义覆盖逐层出现，早期层的良性含义在后期层收敛为有害语义。

Conclusion: Doublespeak揭示了LLM潜在空间中的新攻击面，表明当前的对齐策略不足，需要在表示层面进行操作。表面无害的提示（如"如何建造胡萝卜？"）在内部被解释为被禁止的指令（如"如何建造炸弹？"），成功绕过模型的安全对齐。
...
TL;DR: 本研究将对比解码方法DoLa应用于T5和FLAN-T5编码器-解码器架构，评估其对指令遵循能力的影响，发现DoLa在某些任务类别中提高文本生成的忠实性，但在其他任务中会降低性能。


<details>
  <summary>Details</summary>
Motivation: 虽然对比解码方法（如DoLa）已被证明能提高大型语言模型的文本生成质量，但现有研究仅限于解码器架构，且主要关注事实性改进。本研究旨在探索DoLa在编码器-解码器架构（T5/FLAN-T5）中的应用，并评估其对指令遵循能力的影响。

Method: 将DoLa方法适配到T5和FLAN-T5模型家族，通过层对层分析logit演化来量化DoLa对令牌输出概率的影响，评估其在各种任务上的指令遵循能力表现。

Result: DoLa在某些任务类别中提高了文本生成的忠实性，但在其他任务中会降低性能。这是首次在编码器-解码器架构中实现对比解码策略的研究。

Conclusion: DoLa在编码器-解码器架构中的应用效果具有任务依赖性，需要进一步研究以理解其在不同任务类别中的表现差异，为改进指令遵循能力提供新方向。
...
TL;DR: 论文提出一个优化LLM提示词的实证框架，通过五种提示策略比较，发现构造定义、任务框架和示例对分类性能影响最大，推荐结合人工和自动方法生成并评估多个提示变体。


<details>
  <summary>Details</summary>
Motivation: LLM在文本分类中表现良好，但输出结果严重依赖提示词的措辞。现有研究很少关注分类任务，特别是在心理学等领域，这些领域的构念具有精确、理论驱动的定义，可能未在预训练数据中充分体现。需要系统优化LLM提示词以提升与专家判断的一致性。

Method: 提出一个实证框架，实验评估五种提示策略：代码本引导的经验提示选择、自动提示工程、角色提示、思维链推理和解释性提示。采用零样本和少样本分类设置，在三个心理学构念和两个模型上进行测试。

Result: 发现角色提示、思维链和解释性提示并不能完全解决提示词措辞不当导致的性能损失。最重要的提示特征是构造定义、任务框架，以及较小程度的示例。与专家判断最一致的分类结果来自结合代码本引导经验提示选择和自动提示工程的少样本提示。

Conclusion: 建议研究人员尽可能生成和评估多个提示变体（人工制作、自动生成或两者结合），基于训练数据集中的实证性能选择提示和示例，并在保留集上验证最终方法。这为需要与专家判断对齐的场景提供了实用、系统且理论驱动的方法。
...
TL;DR: 该论文提出通过微调LLMs学习医学共识指南的逐步推理过程，以提升早期预测的可解释性和准确性，并以Sepsis-3定义为例验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 当前医学早期预测的机器学习方法虽然性能突出，但过度关注预测准确性而忽视了可解释性，导致难以获得医疗从业者的信任。医学领域普遍存在共识指南，但现有方法未能有效利用这些结构化知识来指导模型的推理过程。

Method: 将医学共识指南转化为可执行的推理规则，利用电子健康记录中的实例对LLMs进行微调，使其学习遵循共识指南的逐步推理过程。采用多模态方法，将时间序列预测模型的输出表示与LLM集成，以处理稀疏和不规则采样的临床变量预测问题。

Result: 实验表明，经过微调的小型模型在Sepsis-3共识定义任务上，明显优于使用显式定义进行单次学习的大型LLMs，以及在包含共识定义的医学文本上训练的模型。微调模型在未见患者数据上对规则（及例外）的推导正确性接近完美，主要瓶颈在于对未来稀疏不规则采样变量的预测。

Conclusion: 通过微调LLMs学习医学共识指南的推理过程，可以有效提升早期预测的可解释性和准确性。多模态集成方法能够改善对未来稀疏不规则采样临床变量的预测性能，为医学AI系统获得临床信任提供了可行路径。
...
TL;DR: FusedKV通过跨层融合KV缓存，在保持性能的同时减少50%内存占用，解决了Transformer长序列KV缓存内存瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: Transformer解码器在长序列任务中面临KV缓存内存消耗过大的问题。现有的跨层KV缓存共享方法（如YOCO、CLA）性能通常不如层内方法（如GQA），需要找到更有效的跨层KV缓存优化方案。

Method: 通过分析顶层KV的信息流，发现值主要来自底层，键则同时来自底层和中间层。基于此提出FusedKV，顶层KV缓存是可学习的底层和中间层最有信息量的KV融合。进一步提出FusedKV-Lite，顶层KV缓存直接来自底层值和中间层键，减少I/O开销。

Result: 在332M到4B参数的LLM实验中，FusedKV方法减少50%缓存内存，同时获得比标准Transformer解码器更低的验证困惑度，成为内存高效的高性能架构替代方案。

Conclusion: FusedKV通过智能融合不同层的KV信息，有效解决了Transformer长序列KV缓存内存瓶颈，在保持性能的同时显著减少内存占用，为大规模语言模型部署提供了实用解决方案。
...
TL;DR: 研究显示，结合标准文本和多样化文本（社交媒体、历史资料等）训练的语言模型，在保持标准基准准确性的同时，能显著提升对非标准语言变体的理解能力，促进更具包容性和泛化性的语言模型发展。


<details>
  <summary>Details</summary>
Motivation: 当前语言模型依赖经过质量过滤的大规模文本语料，但这一过程可能无意中排除非标准语言变体（方言、历史、非正式语言等），从而降低模型鲁棒性并强化表征偏见。研究者认为语言模型应捕捉语言变异的完整频谱，而不仅仅依赖标准化文本。

Method: 以巴斯克语（形态丰富、资源稀缺语言）为研究对象，构建结合标准、社交媒体和历史来源的新语料库，预训练BERnaT系列编码器模型（三种配置：标准、多样化、组合）。提出将自然语言理解任务分为标准和多样化子集的评估框架，以评估语言泛化能力。

Result: 在标准数据和多样化数据上训练的模型始终优于仅在标准语料上训练的模型，在所有任务类型上都有性能提升，且不损害标准基准的准确性。多样化训练显著提高了模型对非标准语言变体的理解能力。

Conclusion: 语言多样性对于构建包容性、可泛化的语言模型至关重要。结合标准和非标准文本的训练方法能有效提升模型的语言理解能力，同时保持对标准语言的性能，为低资源语言和多样化语言建模提供了有价值的见解。
...
<details>
  <summary>Details</summary>
Motivation: 尽管大语言模型在偏见检测方面有所改进，但宗教等敏感话题仍面临挑战，因为细微错误可能导致严重误解。多语言模型经常误传宗教信息，在宗教语境中准确性不足。

Method: 引入BRAND数据集，聚焦南亚四大宗教（佛教、基督教、印度教、伊斯兰教），包含2400多个条目，使用英语和孟加拉语三种不同类型的提示来评估模型表现。

Result: 模型在英语中的表现优于孟加拉语，即使回答宗教中立问题时也持续表现出对伊斯兰教的偏见，揭示了多语言模型在不同语言中询问相似问题时的持续偏见问题。

Conclusion: 研究结果突显了多语言模型在宗教话题上的持续偏见问题，并将这些发现与人机交互中宗教和灵性相关的更广泛问题联系起来。
...
TL;DR: 该研究提出了一种两阶段方法，将Qwen2.5-3B模型适配到藏语这一低资源语言，通过持续预训练建立语言基础，再通过监督微调进行任务专业化，显著提升了翻译质量和模型性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在低资源语言（如藏语）上的适配面临数据稀缺和跨语言漂移的挑战。藏语作为形态丰富且代表性不足的语言，需要专门的方法来提升LLM在该语言上的性能。
...
Conclusion: CPT构建了藏语语义流形，SFT以最小的表征干扰优化任务对齐。该研究首次定量探索了藏语适配动态，为将多语言基础模型扩展到低资源环境提供了开放、可复现的框架。
...
TL;DR: 提出两种tokenizer适应方法：持续BPE训练用于词汇扩展，叶基词汇剪枝用于词汇缩减，提升跨领域/语言迁移效率


<details>
  <summary>Details</summary>
Motivation: 现有tokenizer适应方法（在新数据上训练tokenizer并添加非重叠词汇）常导致许多词汇无法访问或从未使用，需要更高效的词汇扩展和剪枝方法

Method: 1. 持续BPE训练：在预训练tokenizer基础上，在新数据上继续BPE合并学习过程；2. 叶基词汇剪枝：移除冗余词汇同时保持模型质量

Result: 多语言和多模型家族实验表明，该方法提高tokenization效率，更好利用新增词汇，同时保持模型质量

Conclusion: 这两种方法为受控词汇修改提供实用工具，已发布为开源包，能有效支持预训练语言模型向新领域或语言的迁移
...
TL;DR: AugServe：针对增强型大语言模型推理服务的两阶段自适应调度框架，通过优化请求顺序和动态调整批处理机制，显著提升有效吞吐量并降低延迟。


<details>
  <summary>Details</summary>
Motivation: 随着增强型大语言模型在Web应用中日益普及，现有推理系统面临两大挑战：1）先到先服务调度导致严重的队头阻塞，使许多请求超出服务级别目标延迟；2）静态批处理令牌限制无法适应负载和硬件条件波动，这两者都降低了有效吞吐量和服务质量。

Method: AugServe采用两阶段自适应请求调度策略：第一阶段结合增强型LLM请求的推理特征优化调度决策顺序；第二阶段利用运行时信息持续优化决策，适应请求特性和系统能力。此外，还基于硬件状态和实时负载动态调整令牌批处理机制。

Result: 实验结果显示，AugServe相比vLLM和InferCept实现了4.7-33.1倍和3.3-13.2倍的有效吞吐量提升，同时将首令牌时间分别降低了96.3%和95.0%。

Conclusion: AugServe通过创新的两阶段自适应调度和动态批处理机制，有效解决了增强型LLM推理服务的延迟和吞吐量问题，显著提升了服务质量和用户体验。
...
Conclusion: Jina-VLM证明了通过精心设计的架构组合，可以在2B参数规模下实现多语言视觉问答的先进性能。
...
<details>
  <summary>Details</summary>
Motivation: 当基础语言模型不具备某些认知技能（如验证答案、回溯、尝试替代方法等）时，如何让模型学会利用这些技能？传统方法依赖从更强模型蒸馏，但SkillFactory探索不依赖蒸馏的方法

Method: SkillFactory在强化学习前进行监督微调，使用模型自身生成的样本重新组织成"银色"SFT轨迹来模拟认知技能。这些轨迹可能不完美，但能为RL阶段学习技能提供基础

Result: (1) SkillFactory SFT初始化帮助模型在RL后泛化到更难的任务变体；(2) 模型确实使用了认知技能；(3) SkillFactory模型在域外任务上比基础RL模型更稳健
...
Method: 提出VoxSet半结构化表示，将3D资产压缩为锚定在粗体素网格上的潜在向量集；采用两阶段流程：首先生成稀疏体素化几何锚点，然后使用整流流变换器生成详细几何。

Result: 方法支持任意分辨率解码、低成本训练和灵活推理方案，在各种方面实现了最先进的性能，为可扩展的高质量3D资产创建迈出了重要一步。

Conclusion: LATTICE通过创新的VoxSet表示和两阶段生成框架，成功弥合了2D和3D生成模型之间的质量和可扩展性差距，为高质量3D资产生成提供了有效的解决方案。
...
### [156] [Exploring Syntropic Frameworks in AI Alignment: A Philosophical Investigation](https://arxiv.org/abs/2512.03048)
*Austin Spizzirri*
...
<details>
  <summary>Details</summary>
Motivation: 传统基于内容的价值规范方法存在结构不稳定性问题，包括"是-应该"鸿沟、价值多元主义和扩展框架问题，这构成了"规范陷阱"

Method: 提出熵减（syntropy）作为多智能体对齐的信息论框架，建立真实与模拟道德能力的功能区分，结合兼容论指导控制理论和具身实验范式

Result: 建立了AI对齐的新哲学框架，提出了可证伪的预测，但实证验证仍在进行中

Conclusion: AI对齐应转向过程性、发展性的多智能体架构，而非固定价值内容编码，这为解决对齐问题提供了新的哲学基础
...
### [157] [Beyond the Black Box: A Cognitive Architecture for Explainable and Aligned AI](https://arxiv.org/abs/2512.03072)
*Hu Keyi*

Main category: cs.AI

TL;DR: 论文提出"权重计算主义"认知架构，通过逻辑原子和基本操作实现可解释的AGI，决策模型为权重=收益×概率，所有值可追溯至初始权重，实现透明推理和可追溯价值对齐。
...
Method: 提出"权重计算主义"认知架构，将认知分解为不可分割的逻辑原子和两个基本操作：指向和比较。通过权重=收益×概率的决策模型，所有值可追溯至可审计的初始权重集。实现基于图算法的计算引擎和全局工作空间工作流。

Result: 架构实现了透明、类人的推理能力，在全新场景中表现出稳健学习能力，为构建可信赖和对齐的AGI建立了实践和理论基础。

Conclusion: 权重计算主义是基于第一原理的可行AGI路径，通过原子分解实现根本可解释性、内在通用性和可追溯价值对齐，为可信赖AGI提供了实用框架。
...
### [158] [When Do Symbolic Solvers Enhance Reasoning in Large Language Models?](https://arxiv.org/abs/2512.03272)
*Zhiyuan He,Dingmin Wang*

Main category: cs.AI

TL;DR: 该研究探讨了符号求解器集成方法何时能增强传统长思维链推理，发现该方法仅在问题需要有限隐式推理但涉及充足搜索空间时有效，特别在需要重复回溯的约束满足问题上表现优异。


<details>
  <summary>Details</summary>
Motivation: 大型推理模型通过生成长思维链在复杂推理任务上表现良好，但这种方法可能导致显著的token开销，甚至因"过度思考"产生冗长推理链而得出错误答案。符号求解器集成方法利用LLM的代码生成能力将推理任务转化为可执行代码，然后用符号求解器解决，但何时传统长思维链能被符号求解器增强仍是一个开放问题。

Method: 研究探索符号求解器集成方法何时能增强传统长思维链推理。通过实验分析不同问题类型的性能表现，比较GPT-4o等最新LLM在演绎推理问题上的表现与符号求解器集成方法在约束满足问题上的表现，并测试在提供声明性示例时CodeLlama-13B在困难Zebra谜题上的性能。

Result: 符号求解器集成方法仅在问题需要有限隐式推理但涉及充足搜索空间时有效。GPT-4o在推理深度较浅的演绎问题上表现更好，而符号求解器集成方法在需要重复回溯的约束满足问题上显著提升LLM性能。当提供声明性示例时，即使是CodeLlama-13B也能在困难的Zebra谜题上超越GPT-4o。

Conclusion: 符号求解器集成方法对传统长思维链推理的增强效果取决于问题类型：对于需要有限隐式推理但搜索空间充足的问题，特别是约束满足问题，该方法能显著提升性能。最新LLM在浅层推理问题上表现良好，而符号求解器集成方法在需要复杂搜索的问题上具有优势，为混合推理系统设计提供了指导。
...
### [159] [Prior preferences in active inference agents: soft, hard, and goal shaping](https://arxiv.org/abs/2512.03293)
...
Method: 考虑了四种定义偏好分布的方式：硬目标vs软目标、涉及目标塑造vs不涉及目标塑造。在网格世界导航任务中比较了使用这四种偏好分布的智能体表现。

Result: 目标塑造能带来最佳整体性能（促进利用），但会牺牲对环境转移动态的学习（阻碍探索）。

Conclusion: 偏好分布的定义方式对主动推理智能体的性能有重要影响，目标塑造在提升性能的同时会限制探索能力，需要在利用和探索之间权衡。
...
### [160] [Evaluating Generalization Capabilities of LLM-Based Agents in Mixed-Motive Scenarios Using Concordia](https://arxiv.org/abs/2512.03318)
...
TL;DR: 论文提出了一种评估LLM智能体在零样本混合动机环境中合作能力的方法，使用Concordia自然语言多智能体模拟环境，通过NeurIPS 2024竞赛发现现有智能体在说服和规范执行等需要稳健泛化的合作场景中存在显著能力差距。
...
Method: 提出使用Concordia自然语言多智能体模拟环境来评估LLM智能体在零样本、混合动机环境中的合作能力。该方法通过测试智能体在不同合作伙伴和情境中识别和利用互利机会的能力，来衡量一般合作智能。

Result: 通过NeurIPS 2024 Concordia竞赛的实证结果，评估了智能体在从谈判到集体行动问题等多种场景中实现互利的能力。结果显示当前智能体能力与可靠合作所需的稳健泛化之间存在显著差距，特别是在需要说服和规范执行的场景中。

Conclusion: 当前LLM智能体在合作能力方面存在重要局限，特别是在需要泛化到新颖社交情境的场景中。研究强调了开发更强大合作智能的必要性，特别是在说服和规范执行等关键领域，为未来LLM智能体发展提供了重要评估框架。
...
### [161] [Multimodal Reinforcement Learning with Agentic Verifier for AI Agents](https://arxiv.org/abs/2512.03438)
...
TL;DR: 提出Argos奖励代理，通过多维度评分函数选择来训练多模态推理模型，解决传统稀疏奖励问题，在多个智能体任务上取得SOTA结果。


<details>
  <summary>Details</summary>
Motivation: 当前多模态强化学习主要依赖基于最终答案的稀疏奖励，缺乏对推理过程的细粒度指导。不同样本需要不同的评分函数，且教师模型可能提供噪声信号，因此需要更智能的奖励机制。

Method: 提出Argos奖励代理，为每个样本从教师模型和规则基础的评分函数池中选择，同时评估：1)最终响应准确性；2)时空定位质量；3)推理过程质量。用于SFT数据筛选和RL训练。

Result: 在空间推理、视觉幻觉、机器人和具身AI等多个智能体任务上取得SOTA结果。证明仅依赖SFT后训练会导致模型崩溃，而在线验证能防止非接地解。还能减少奖励黑客行为。

Conclusion: Argos通过智能奖励选择机制有效提升多模态推理模型的训练效果，防止模型崩溃和奖励黑客问题，理论证明其帕累托最优性，为智能体任务训练提供了新范式。
...
### [162] [Multi-Agent Reinforcement Learning with Communication-Constrained Priors](https://arxiv.org/abs/2512.03528)
*Guang Yang,Tianpei Yang,Jingwen Qiao,Yanqing Wu,Jing Huo,Xingguo Chen,Yang Gao*

Main category: cs.AI

TL;DR: 提出一个通信受限的多智能体强化学习框架，通过区分有损/无损消息并量化其对决策的影响，解决现实场景中通信质量不稳定的问题。


<details>
  <summary>Details</summary>
Motivation: 现实世界多智能体系统中普遍存在有损通信问题，现有通信增强的多智能体强化学习方法在可扩展性和鲁棒性方面存在局限，难以应用于复杂动态的真实环境。

Method: 1) 提出广义通信约束模型统一描述不同场景的通信条件；2) 将其作为学习先验区分有损和无损消息；3) 使用双重互信息估计器解耦有损/无损消息对分布式决策的影响；4) 引入通信约束的多智能体强化学习框架，将通信消息的影响量化为全局奖励。

Result: 在多个通信受限的基准测试中验证了方法的有效性。

Conclusion: 提出的框架能够有效处理现实世界中的有损通信问题，提高了多智能体系统在复杂动态环境中的适应性和性能。
...
TL;DR: PARC是一个用于自主执行长时程计算任务的编码代理，采用分层多智能体架构，具备自我评估和反馈机制，能在材料科学和Kaggle竞赛中独立完成复杂任务。
...
Method: 采用分层多智能体架构，包含任务规划、执行、自我评估和反馈机制。系统能够从独立上下文评估自身行为和结果，检测并纠正高层策略错误。

Result: 在材料科学中成功复现锂离子传导和合金偏析的关键结果，协调数十个并行模拟任务（每个约43小时计算时间）；在Kaggle实验中，从自然语言指令出发，实现数据分析和搜索策略，产生与人工设计基线竞争的结果。

Conclusion: 分层多智能体系统与自我评估、反馈机制的结合，能够实现独立、大规模科学和分析工作的AI系统，展示了在复杂计算任务中自主工作的潜力。
...
### [164] [Reason-Plan-ReAct: A Reasoner-Planner Supervising a ReAct Executor for Complex Enterprise Tasks](https://arxiv.org/abs/2512.03560)
*Gianni Molinari,Fabio Ciravegna*

Main category: cs.AI

TL;DR: RP-ReAct是一种新型多智能体架构，通过将战略规划与低级执行解耦，解决企业级复杂任务中单智能体架构的轨迹不稳定性和上下文窗口限制问题，在ToolQA基准测试中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 当前自主智能体在企业级复杂任务中存在两个主要限制：1) 单智能体架构的单一规划-执行循环导致轨迹不稳定；2) 为数据隐私使用本地开源模型时，较小的上下文窗口难以处理大型工具输出。

Method: 提出RP-ReAct多智能体方法，包含：1) 推理规划智能体(RPA)，负责子步骤规划并使用大型推理模型持续分析执行结果；2) 代理执行智能体(PEA)，使用ReAct方法将子步骤转化为具体工具交互；3) 上下文保存策略，通过外部存储和按需访问管理大型工具输出。

Result: 在ToolQA多领域基准测试中使用六种开源推理模型评估，RP-ReAct在性能和泛化能力上优于现有最先进基线方法，并在不同模型规模下展现出增强的鲁棒性和稳定性。

Conclusion: RP-ReAct通过解耦规划与执行，结合上下文管理策略，为企业级可部署的智能体解决方案提供了有效途径，解决了复杂任务中的稳定性和效率问题。
...
### [165] [EnCompass: Enhancing Agent Programming with Search Over Program Execution Paths](https://arxiv.org/abs/2512.03571)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM智能体开发方法将核心工作流逻辑与推理时策略（如树搜索）耦合在一起，导致难以独立修改和实验不同策略

Method: 提出"概率天使非确定性"（PAN）编程模型，分离工作流与推理策略；实现EnCompass框架，使用Python装饰器将智能体工作流编译为搜索空间

Result: 通过三个案例研究证明，该框架能让开发者快速提升智能体可靠性，轻松切换不同推理策略，仅需少量额外编码

Conclusion: PAN模型和EnCompass框架有效解耦了智能体设计的两个关键方面，简化了LLM智能体的开发和实验过程
...
### [166] [DeepRule: An Integrated Framework for Automated Business Rule Generation via Deep Predictive Modeling and Hybrid Search Optimization](https://arxiv.org/abs/2512.03607)
*Yusen Wu,Xiaotie Deng*

Main category: cs.AI

TL;DR: DeepRule是一个集成框架，用于零售品类和定价优化的自动化业务规则生成，通过LLM解析非结构化文本、博弈论约束优化和可解释决策蒸馏来解决理论与现实经济复杂性之间的差距。


<details>
  <summary>Details</summary>
Motivation: 现有理论模型与真实世界经济复杂性之间存在系统性错配，具体表现为三个关键差距：1) 非结构化文本数据模态不匹配阻碍准确客户画像；2) 动态特征纠缠挑战非线性价格弹性和时变属性建模；3) 多层级业务约束导致操作不可行性。

Method: 采用三层架构：1) 混合知识融合引擎使用LLM深度语义解析非结构化文本，将分销协议和销售评估转化为结构化特征；2) 博弈论约束优化机制通过双边效用函数动态协调供应链利益，将制造商-分销商利润再分配编码为层级约束下的内生目标；3) 可解释决策蒸馏接口利用LLM引导的符号回归优化定价策略和可审计业务规则，将经济先验作为硬约束嵌入数学表达式搜索。
...
### [167] [MemVerse: Multimodal Memory for Lifelong Learning Agents](https://arxiv.org/abs/2512.03627)
...
TL;DR: MemVerse是一个模型无关、即插即用的记忆框架，通过结合快速参数化回忆和分层检索式记忆，解决AI代理的记忆问题，支持多模态智能的扩展和自适应。


<details>
  <summary>Details</summary>
Motivation: 当前AI代理存在根本性限制：无法记忆。没有可靠记忆会导致灾难性遗忘、长时推理困难，以及在多模态或交互环境中无法连贯操作。

Method: MemVerse采用分层记忆架构：短期记忆处理近期上下文，长期记忆将原始多模态经验转化为结构化知识图谱。引入周期性蒸馏机制，将长期记忆中的关键知识压缩到参数化模型中，实现快速可微回忆。

Result: 大量实验表明，MemVerse显著提升了多模态推理和持续学习效率，使代理能够在扩展交互中记忆、适应和连贯推理。

Conclusion: MemVerse通过桥接参数化回忆和分层检索式记忆，为AI代理提供了可扩展、自适应的记忆能力，解决了现有系统的记忆限制问题。
...
### [168] [RoCo: Role-Based LLMs Collaboration for Automatic Heuristic Design](https://arxiv.org/abs/2512.03762)
*Jiawei Xu,Fengfeng Wei,Weineng Chen*

Main category: cs.AI

TL;DR: RoCo是一个基于多智能体角色协作的系统，通过四个专门化的LLM智能体（探索者、利用者、批评者、整合者）协同设计高质量启发式算法，在组合优化问题上超越现有方法。
...
Method: 提出RoCo多智能体角色系统，包含四个专门化LLM智能体：探索者（创造性、多样性驱动）、利用者（保守性、效率导向）、批评者（评估进化步骤并提供反馈）、整合者（平衡创新与利用）。这些智能体通过结构化的多轮过程进行交互，包括反馈、精炼和精英突变。

Result: 在五个不同组合优化问题的白盒和黑盒设置下进行评估，RoCo始终生成具有竞争力的启发式算法，在两种场景下都优于ReEvo和HSEvo等现有方法。

Conclusion: 这种基于角色的协作范式为稳健且高性能的自动启发式设计设立了新标准，证明了多角色协作在增强启发式算法多样性和质量方面的有效性。
...
### [169] [Omni-AutoThink: Adaptive Multimodal Reasoning via Reinforcement Learning](https://arxiv.org/abs/2512.03783)
*Dongchao Yang,Songxiang Liu,Disong Wang,Yuanyuan Wang,Guanglu Wan,Helen Meng*

Main category: cs.AI

TL;DR: Omni-AutoThink：自适应推理框架，根据任务难度动态调整模型推理深度，提升多模态推理性能
...
Result: 实验结果表明，该框架相比现有基线显著提升了自适应推理性能。基准数据和代码将公开。

Conclusion: Omni-AutoThink通过自适应推理机制有效解决了现有Omni模型推理行为僵化的问题，在多模态任务中实现了更智能的推理调整。
...
### [170] [A Hierarchical Tree-based approach for creating Configurable and Static Deep Research Agent (Static-DRA)](https://arxiv.org/abs/2512.03887)
*Saurav Prateek*
...
<details>
  <summary>Details</summary>
Motivation: 现有静态RAG管道在处理复杂多轮研究任务时存在局限，需要更灵活的代理系统来支持深度研究
...
### [171] [Autonomous Agents and Policy Compliance: A Framework for Reasoning About Penalties](https://arxiv.org/abs/2512.03931)
*Vineel Tummala,Daniela Inclezan*

Main category: cs.AI

TL;DR: 本文提出了一种基于逻辑编程的策略感知自主代理框架，能够推理违规行为的潜在惩罚并相应行动。相比以往仅关注合规的方法，该框架考虑为达成高风险目标而偏离策略的必要性，并引入惩罚机制来区分不同违规计划。


<details>
  <summary>Details</summary>
Motivation: 现有研究主要关注确保策略合规性，但实际场景中为达成高风险目标可能需要偏离策略。同时，建模违规行为有助于政策制定者模拟现实人类决策过程。因此需要一种能够权衡合规与目标达成、并考虑违规后果的推理框架。

Method: 扩展Gelfond和Lobo的授权与义务策略语言(AOPL)以纳入惩罚机制，集成答案集编程(ASP)进行推理。开发从扩展AOPL到ASP的自动转换，并改进基于ASP的规划算法以考虑累积惩罚。引入基于惩罚的推理来区分违规计划，优先选择后果最小的方案。

Result: 在两个领域的实验中，该框架生成了更高质量的计划，能够避免有害行为，并在某些情况下提高了计算效率。相比先前方法，确保了策略的良好形式化、考虑了策略优先级，并通过明确识别规则违反及其后果增强了可解释性。

Conclusion: 该框架通过整合惩罚推理，增强了自主决策能力并为策略优化提供了信息支持。在逻辑编程中引入惩罚机制能够生成更实用的计划，平衡合规需求与目标达成，对自主系统和政策制定都有重要价值。
...
### [172] [Benchmark for Planning and Control with Large Language Model Agents: Blocksworld with Model Context Protocol](https://arxiv.org/abs/2512.03955)
*Niklas Jobs,Luis Miguel Vieira da Silva,Jayanth Somashekaraiah,Maximilian Weigand,David Kube,Felix Gehlhoff*

Main category: cs.AI

TL;DR: 提出了一个基于Blocksworld问题的可执行仿真基准测试，用于系统评估LLM智能体在工业自动化中的规划与执行能力，通过标准化工具接口支持不同架构的对比。


<details>
  <summary>Details</summary>
Motivation: 工业自动化需要灵活的自适应控制策略，基于LLM的智能体具有潜力但缺乏标准化基准进行系统比较，因此需要建立统一的评估框架。

Method: 引入包含五个复杂度类别的Blocksworld问题仿真环境，集成模型上下文协议（MCP）作为标准化工具接口，允许不同智能体架构无需修改即可连接和评估。

Result: 通过单智能体实现展示了基准的适用性，建立了用于比较LLM规划与执行方法的定量指标，为系统评估提供了可行框架。

Conclusion: 该基准测试为LLM智能体在工业自动化中的自适应规划与执行能力提供了标准化评估平台，支持不同架构的系统比较和性能分析。
