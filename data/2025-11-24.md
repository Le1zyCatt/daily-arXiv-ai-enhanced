...
TL;DR: 使用持久同调（PH）方法从拓扑数据分析角度客观识别和区分艺术风格，能够统计显著地区分不同艺术家、同一艺术流派内的艺术家，以及区分艺术家作品与AI生成的仿风格图像。


<details>
  <summary>Details</summary>
Motivation: 艺术风格识别传统上依赖艺术史学家或评论家的视觉直觉和经验，缺乏客观量化方法。本文旨在利用数学工具为艺术分析提供结构化视角。

Method: 采用拓扑数据分析中的持久同调（PH）方法，该方法能够捕捉数据的拓扑特征，为艺术风格提供客观可解释的洞察。

Result: PH方法能够以统计确定性区分不同艺术流派的艺术家、同一流派内的不同艺术家，并能有效识别艺术家原作与AI生成的仿风格图像。

Conclusion: 持久同调为艺术风格分析提供了客观、可解释的数学框架，能够可靠地区分艺术家的个人风格，并在AI生成艺术检测方面具有应用价值。
...
Method: 通过线性关系从虚拟对象的带符号CoC地图推断深度值，使用神经重模糊网络渲染逼真的镜头模糊

Result: 实验结果表明该方法在定性和定量评估中都优于现有技术，实现了高保真度的合成效果
...
TL;DR: 提出一种基于StyleGAN2的方法来生成猕猴面部表情，通过数据增强、样本选择和损失函数优化解决训练数据不足的问题，能够生成多样化的面部表情并用于图像编辑。
...
Method: 使用StyleGAN2模型，实施三种策略：1) 通过运动转移用计算机图形学为静态图像添加动画来合成新表情图像；2) 基于初始训练StyleGAN2模型的潜在表示进行样本选择；3) 优化损失函数以准确再现细微运动。
...
TL;DR: 提出了一种原生3D编辑框架，通过单次前向传播直接操作3D表示，解决了现有方法在速度和3D一致性方面的限制。


<details>
  <summary>Details</summary>
Motivation: 现有3D编辑方法存在关键限制：基于优化的方法速度过慢，而基于多视图2D编辑的前向方法常出现几何不一致和视觉质量下降的问题。

Method: 创建大规模多模态指令引导3D编辑数据集，探索两种条件策略：传统交叉注意机制和新型3D标记拼接方法。

Result: 标记拼接方法参数效率更高且性能更优，在生成质量、3D一致性和指令保真度方面优于现有2D提升方法。

Conclusion: 该方法为指令引导3D编辑设定了新的基准，在保持高效性的同时显著提升了编辑质量。
...
TL;DR: 提出了首个大规模双人肖像生成基准数据集PairHuman，包含10万+图像及丰富元数据，并开发了DHumanDiff基线方法，在双人肖像生成中实现增强的面部一致性和个性化生成。


<details>
  <summary>Details</summary>
Motivation: 个性化双人肖像定制在情感记忆保存和婚纱摄影规划中有重要应用价值，但缺乏基准数据集阻碍了高质量双人肖像生成的发展。

Method: 构建PairHuman数据集（10万+图像，包含场景、服装、互动等多样性），并开发DHumanDiff基线方法，增强面部一致性，平衡个性化人物生成和语义驱动场景创建。
...
TL;DR: 本文提出了一种用于中子成像混合高斯-泊松噪声去噪的无监督自编码器方法，在潜在空间使用CDF 97小波变换，相比传统方法具有更好的重建误差和边缘保持性能。


<details>
  <summary>Details</summary>
Motivation: 中子成像在惯性约束聚变分析中很重要，但图像常被高斯和泊松混合噪声退化，传统滤波和阈值方法难以有效去除这些重叠噪声，需要保持图像保真度的去噪技术。

Method: 采用无监督自编码器，在潜在空间集成Cohen-Daubechies-Feauveau (CDF 97)小波变换，专门针对混合高斯-泊松噪声进行去噪。

Result: 该方法成功去噪中子成像数据，与BM3D等非机器学习滤波方法相比，显示出更低的重建误差和更优的边缘保持指标。
...
TL;DR: SAM 3是一个统一模型，能够基于概念提示（名词短语、图像示例或两者结合）在图像和视频中检测、分割和跟踪对象。它通过Promptable Concept Segmentation (PCS)返回分割掩码和唯一标识符，在图像和视频PCS任务中准确率翻倍。


<details>
  <summary>Details</summary>
Motivation: 现有系统在基于概念提示的对象检测、分割和跟踪方面存在局限性，需要开发一个统一的模型来处理多种类型的提示，并提高在图像和视频中的分割准确性。

Method: 构建可扩展的数据引擎生成包含400万个独特概念标签的高质量数据集；模型包含图像级检测器和基于记忆的视频跟踪器，共享单一骨干网络；通过存在头分离识别和定位，提升检测精度。

Result: SAM 3在图像和视频PCS任务中的准确率比现有系统提高了一倍，同时在视觉分割任务上改进了之前SAM的能力。

Conclusion: SAM 3在基于概念提示的对象检测、分割和跟踪方面取得了显著进展，通过开源模型和新的SA-Co基准测试推动了该领域的发展。
...
TL;DR: SaFeR-CLIP通过将不安全概念重定向到语义上最接近的安全替代品，在保持安全性的同时显著提升了视觉语言模型的泛化性能。


<details>
  <summary>Details</summary>
Motivation: 传统方法通过强制将不安全概念对齐到单一预定义安全目标来提升模型安全性，但这会破坏模型已学习的语义结构，导致泛化性能显著下降。
...
TL;DR: 提出了一个三阶段框架，从单视图SVG输入生成几何和颜色一致的多视图SVG，通过3D重建、空间记忆机制和路径优化实现多视图一致性。
...
Method: 三阶段框架：1) 将光栅化输入提升到3D表示并渲染多视图图像；2) 扩展SAM2的时间记忆机制到空间域，建立跨视图的部件级对应关系；3) 在光栅到矢量转换中执行路径整合和结构优化。
...
TL;DR: Mesh RAG是一个无需训练、即插即用的框架，通过检索增强生成来改进自回归3D网格生成模型，解决了传统方法的序列依赖性问题，实现了更快的生成速度和增量编辑能力。


<details>
  <summary>Details</summary>
Motivation: 传统3D网格生成方法依赖手工制作，耗时且难以扩展。现有的自回归模型虽然能自动化生成，但存在质量-速度权衡和序列依赖性问题，限制了生成效率和编辑灵活性。

Method: 受语言模型RAG启发，Mesh RAG利用点云分割、空间变换和点云配准技术来检索、生成和集成网格组件，将生成过程与严格的序列依赖解耦，实现并行化推理。

Result: Mesh RAG在各种基础自回归网格生成模型上均能显著提升网格质量，相比顺序部件预测加速生成速度，并支持增量编辑，且无需模型重新训练。

Conclusion: Mesh RAG框架有效解决了自回归网格生成中的序列依赖问题，在保持高质量的同时实现了加速生成和编辑灵活性，具有广泛的适用性。
...
TL;DR: WorldGen是一个从文本提示自动创建大规模交互式3D世界的系统，结合了LLM驱动的场景布局推理、程序生成、基于扩散的3D生成等技术。
...
Method: 通过混合人机协作流程生成和验证结构化文本标注，整合了共配准的Sentinel-2和ALOS-2影像数据，建立了森林生态场景中具有认知可解释性的逻辑链。
...
TL;DR: BOP-ASK是一个用于物体交互推理的大规模数据集，包含超过15万张图像和3300万个问答对，涵盖6种任务，用于训练和评估视觉语言模型在精细空间理解方面的能力。
...
Conclusion: BOP-ASK为视觉语言模型提供了训练和评估精细物体交互推理能力的丰富资源，模型在该数据集上训练后能够显著提升在真实世界应用中的空间理解能力。
...
TL;DR: 提出了Parts-Mamba模型，一种混合GCN-Mamba架构，用于解决骨架动作识别中因遮挡导致的局部上下文缺失问题，在NTU数据集上取得了显著性能提升。


<details>
  <summary>Details</summary>
Motivation: 在真实场景中，由于人体部位遮挡或通信质量问题，捕获的骨架数据往往不完整，导致现有GCN模型在存在缺失帧或部分骨架缺失时性能下降，主要原因是无法有效捕获远距离关节的上下文信息。
...
Conclusion: Parts-Mamba模型能够有效增强对远距离关节上下文信息的捕获和保持能力，显著提升了在非理想骨架数据条件下的动作识别性能。
...
TL;DR: 提出了联合Gromov-Wasserstein（JGW）目标，将GW距离扩展到支持多对多对象匹配，解决了传统GW只能进行一对一匹配的限制。


<details>
  <summary>Details</summary>
Motivation: 传统Gromov-Wasserstein距离仅限于单对象间的成对匹配，无法满足需要多对一或多对多对象匹配的场景和应用需求。

Method: 通过引入JGW目标扩展GW框架，实现对象集合的同步匹配，采用最优传输中的传统算法（包括熵正则化）来解决点云对象表示问题。

Result: 在部分匹配的基准测试中，该方法在准确性和计算效率方面优于其他GW变体，在合成和真实数据集上有效实现了多形状匹配。
...
TL;DR: 提出REPA方法，在逆问题求解中通过对齐扩散/流模型与预训练自监督编码器（如DINOv2）的内部表示来提升重建质量，减少所需离散化步骤。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明，对齐生成模型与自监督编码器的内部表示能改善收敛和样本质量。本研究将此思想扩展到逆问题领域，利用预训练生成模型作为先验。

Method: 在推理时应用表示对齐（REPA），将扩散/流模型与预训练视觉编码器的表示对齐，即使没有真实信号，也能通过近似目标特征对齐来指导重建过程。

Result: 在超分辨率、框内修复、高斯去模糊和运动去模糊等任务上，REPA方法一致提升重建质量，同时通过减少离散化步骤提供显著效率增益。

Conclusion: REPA方法能有效提升逆问题求解的感知保真度，理论分析揭示了其与DINOv2嵌入空间散度度量的关系，以及如何引导模型内部表示向干净图像表示靠拢。
...
TL;DR: 提出NFGlassNet方法，利用闪光/无闪光图像中的反射动态特性来检测玻璃表面，通过反射对比挖掘模块和反射引导注意力模块实现精确检测。
...
Conclusion: 利用闪光/无闪光图像中的反射动态特性可以有效检测玻璃表面，提出的方法在性能上超越了现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型主要关注简单视频场景，无法反映真实世界中复杂多样的音频-视频事件。需要构建专门的数据集和模型来支持细粒度的音频-视频时空推理。

Method: 1) 构建R-AVST数据集：使用LLM提取关键对象、自动空间标注和人工质量检查，包含5K个未剪辑视频、27K个对象和100种音频-视频事件类型；2) 提出AVST-Zero模型：基于强化学习，避免中间监督，通过多维度奖励直接优化行为。

Result: R-AVST数据集包含超过8K个高质量、均匀分布的问题-答案对，有效评估模型性能。AVST-Zero在音频-视频时空推理任务上表现出与现有模型竞争的性能。
...
<details>
  <summary>Details</summary>
Motivation: 热扩散未能利用图像高频细节与低频结构之间的强相关性，导致生成早期步骤的随机行为；冷扩散虽然利用了图像相关性进行预测，但忽视了噪声在塑造数据流形中的作用，导致流形外问题。

Method: 采用分治策略，利用图像中的频谱依赖性，通过解耦去噪和去模糊过程来简化分数模型估计；分析模糊-噪声比(BNR)来研究模型学习动态与数据流形变化之间的权衡。
...
TL;DR: Q-Real是一个用于AI生成图像细粒度评估的数据集，专注于真实性和合理性两个维度，包含3,088张图像，提供实体位置标注、判断问题和归因描述，并构建了Q-Real Bench基准来评估多模态大语言模型。


<details>
  <summary>Details</summary>
Motivation: 现有质量评估方法仅提供单一评分，过于粗糙，无法为生成模型提供针对性改进指导。在AI生成图像应用中，真实性和合理性是两个关键维度，细粒度评估对提升生成性能尤为重要。

Method: 构建包含3,088张文本生成图像的数据集，标注主要实体位置，提供真实性和合理性维度的判断问题和归因描述。设计评估基准Q-Real Bench，包含判断和基于推理的定位两个任务，并开发微调框架对多模态大语言模型进行实验。

Result: 实验结果表明数据集质量高、意义重大，基准评估全面有效。

Conclusion: Q-Real数据集和基准为AI生成图像的细粒度评估提供了有效工具，有助于提升多模态大语言模型的评估能力，推动生成模型的优化改进。
...
Method: 将文本提示渲染为绘制的文本图像，所有输入输出都视为RGB像素；使用统一的扩散变换器在像素空间中训练，通过轻量级任务嵌入指定期望方向。

Result: 在文本到图像合成和图像到文本理解任务上表现出强大的跨模态对齐能力，并展现出循环一致的图像-标题-图像循环等涌现可控性。
...
<details>
  <summary>Details</summary>
Motivation: 传统的异常生成方法通常需要少量异常样本进行微调，这与异常稀缺的实际情况相矛盾，且容易过拟合类别先验。本文旨在解决完全没有真实异常样本或训练数据的情况。

Method: 提出Delta-Denoising方法：1) 通过最小提示对驱动两个扩散分支对比；2) 累积每步去噪差异生成图像特定的定位图；3) 使用掩码指导潜在修复过程；4) 进行词元级提示优化和空间注意力偏置。

Result: 在公共数据集上的实验表明，DeltaDeno实现了优秀的生成效果、真实感，并在下游检测任务中取得一致性能提升。

Conclusion: DeltaDeno提供了一种无需训练、零样本的异常生成解决方案，能够有效定位和生成局部缺陷，同时保持周围上下文，在异常检测任务中表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 基于扩散模型的视频超分辨率方法存在误差累积、空间伪影以及感知质量与保真度之间的权衡问题，主要由于视频帧间的不准确对齐和补偿不足。

Method: 提出DGAF-VSR方法，包含光学引导变形模块(OGWM)在放大分辨率下保持对齐特征的高频细节，以及特征域时序条件模块(FTCM)在特征域提供密集引导。

Result: 在合成和真实数据集上的实验表明，DGAF-VSR在感知质量(DISTS降低35.82%)、保真度(PSNR提升0.20dB)和时间一致性(tLPIPS降低30.37%)方面优于最先进方法。

Conclusion: 特征域对齐和密集引导能有效提升基于扩散模型的视频超分辨率性能，在多个关键指标上取得显著改进。
...
<details>
  <summary>Details</summary>
Motivation: CBCT图像中牙齿的准确分割对数字牙科至关重要，但在邻牙粘连情况下，严重的解剖形状扭曲使得分割变得困难。
...
TL;DR: 提出了OmniGround基准测试和PG-TAF训练免费框架，用于解决视频时空定位中的类别偏见和复杂查询问题，在多个基准测试上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型在视频时空定位任务中与现实需求存在差距，主要由于基准测试范围有限导致模型存在类别偏见、推理过于简化以及语言鲁棒性差的问题。

Method: 引入OmniGround基准测试（3,475个视频，81个类别），提出前向-后向-精化标注流程，开发DeepSTG评估框架，并提出PG-TAF训练免费两阶段框架将STVG分解为高层时间定位和细粒度时空传播。

Result: 在复杂现实场景中性能平均下降10.4%，特别是在小/遮挡物体和复杂空间关系上。PG-TAF在OmniGround上m_tIoU和m_vIoU分别提升25.6%和35.6%，在四个基准测试上均取得一致增益。

Conclusion: OmniGround基准测试揭示了当前STVG模型的局限性，PG-TAF框架通过分解任务有效提升了复杂场景下的时空定位性能。
...
TL;DR: 提出了MultiPriv基准测试，这是首个系统评估视觉语言模型中个体级隐私推理能力的基准，揭示了现有模型在隐私推理方面的严重风险。
...
Method: 提出了隐私感知与推理框架，构建了包含合成个体配置文件的双语多模态数据集，设计了9个挑战性任务来评估从属性检测到跨图像重识别和链式推理的完整隐私推理能力。

Result: 对50多个基础和商业VLM的大规模评估显示：许多VLM存在显著的未测量推理隐私风险；感知级指标无法有效预测这些推理风险；现有安全对齐措施对此类推理攻击不一致且无效。
...
TL;DR: 提出了一种联合建模动态图像序列和运动场的隐式神经表示框架，通过光学流方程进行物理约束，无需预先估计运动场即可同时恢复高质量图像和准确运动信息
...
Method: 使用两个隐式神经表示(INR)，一个参数化时空图像内容，另一个表示光学流，通过光学流方程进行物理正则化约束，并结合数据一致性损失进行联合优化

Result: 在动态心脏MRI数据集上的实验表明，该方法优于最先进的运动补偿和深度学习方法，实现了更好的重建质量、准确的运动估计和改善的时间保真度

Conclusion: 隐式联合建模与流正则化约束在动态MRI重建中具有巨大潜力，能够同时获得高质量图像和准确运动信息
...
TL;DR: 提出了FingerCap任务和FiGOP方法，用于生成细粒度手指级手部运动描述，通过结合RGB帧和手部关键点来改善视频MLLMs在手指运动理解上的不足。


<details>
  <summary>Details</summary>
Motivation: 理解细粒度人类手部运动对视觉感知、具身智能和多模态通信至关重要，但现有视频MLLMs在捕捉细微手指动态方面存在困难。

Method: 构建FingerCap-40K数据集，提出FiGOP方法将RGB关键帧与后续手部关键点配对，使用轻量级时序编码器将关键点转换为运动嵌入并与RGB特征集成。

Result: 在FingerCap-40K上的实验表明，现有视频MLLMs在手指级推理方面仍有困难，而FiGOP增强模型在HandJudge评估和人工研究中均取得一致提升。

Conclusion: FiGOP方法有效解决了视频MLLMs在捕捉细粒度手指运动时的时序稀疏性问题，无需增加RGB采样密度即可恢复精细时序线索。
...
TL;DR: 提出了一种基于点监督的面部表情检测方法，使用单时间戳标注训练，通过高斯分布建模实例级表情强度进行软伪标签，结合强度分支和分类分支实现表情检测与分类。


<details>
  <summary>Details</summary>
Motivation: 现有方法依赖昂贵的时序边界标注，需要开发更高效的弱监督方法。点监督只需每个表情实例的单时间戳标注，大幅降低标注成本。
...
Conclusion: 该方法在点监督设置下实现了有效的面部表情检测，通过软伪标签和双分支设计克服了硬伪标签的局限性，为弱监督表情分析提供了可行方案。
...
TL;DR: 提出了Neighbor GRPO方法，通过扰动ODE初始噪声条件生成候选轨迹，使用基于距离的代理策略优化模型，完全绕开SDE，解决了SDE-based GRPO的信用分配效率低和与高阶求解器不兼容的问题。
...
Method: 从距离优化角度重新解释SDE-based GRPO，提出Neighbor GRPO：通过扰动ODE初始噪声生成多样化候选轨迹，使用基于softmax距离的代理跳跃策略优化模型，并引入对称锚点采样和组内准范数重加权技术。

Result: Neighbor GRPO在训练成本、收敛速度和生成质量方面显著优于SDE-based方法，同时完全保持了确定性ODE采样的效率和与高阶求解器的兼容性。

Conclusion: Neighbor GRPO提供了一种有效替代SDE-based GRPO的方法，通过距离优化视角重新构建对齐算法，在保持效率的同时解决了现有方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 现有材料生成方法缺乏统一的表示来连接自然图像外观和PBR属性，导致任务特定的碎片化流程，无法利用大规模RGB图像数据。

Method: 提出联合RGB-PBR表示，将材料编码为两个相互依赖的潜在变量：RGB外观和四个PBR贴图，将其构建为5帧序列并使用视频扩散架构。

Result: 在MatHybrid-410K混合数据集上训练，实现原生1024×1024合成，在质量和多样性上显著超越现有方法。
...
TL;DR: 提出双分支退化提取网络解决盲超分问题，通过预测无监督的模糊和噪声嵌入，使超分网络能够分别适应不同类型的退化。


<details>
  <summary>Details</summary>
Motivation: 现有超分方法在已知固定退化（如双三次下采样）下表现良好，但当实际退化偏离假设时性能显著下降。需要解决盲超分问题。

Method: 使用双分支退化提取网络预测两个无监督退化嵌入（模糊和噪声信息），将退化提取器作为正则化器利用SR和HR图像的差异。

Result: 在多个基准测试上的广泛实验表明，该方法在盲超分问题上达到了最先进的性能。

Conclusion: 提出的双分支退化提取网络有效解决了盲超分问题，通过分别处理模糊和噪声退化实现了优越性能。
...
TL;DR: 提出了一种边缘设备上基于烹饪状态引导的食物图像生成方法，通过厨师标注的熟度级别和特定烹饪相似度指标，显著提升了合成食物图像的真实性和时序一致性。
...
Method: 构建了首个基于烤箱的烹饪进程数据集，包含厨师标注的熟度级别；提出了边缘高效的食谱和烹饪状态引导生成器；引入了领域特定的烹饪图像相似度(CIS)指标作为训练损失和进度监控信号。

Result: 在自建数据集上FID分数提升30%，在公共数据集上提升60%，显著优于现有基线方法。

Conclusion: 该方法能够根据用户偏好的视觉目标而非固定预设，合成真实且时序一致的食物图像，适用于边缘设备部署。
...
TL;DR: 提出了SeeCLIP框架，通过细粒度语义增强解决开放集域泛化中的结构风险和开放空间风险困境，特别针对与已知类视觉相似但语义不同的'硬未知'类别。


<details>
  <summary>Details</summary>
Motivation: 现有方法在处理域偏移和新类别时面临结构风险与开放空间风险的权衡困境，容易对与已知类视觉相似的'硬未知'类别产生过度自信。

Method: 提出语义感知提示增强模块分解图像为判别性语义标记；使用双工对比学习保持与已知类的分离性和语义邻近性；通过语义引导扩散模块合成伪未知样本。

Result: 在五个基准测试中相比最先进方法实现了3%准确率和5% H-score的一致提升。

Conclusion: SeeCLIP通过细粒度语义增强有效解决了开放集域泛化中的核心挑战，显著提升了模型对'硬未知'类别的识别能力。
...
Method: 将生存压力建模为不透明度的正则化梯度场，让渲染质量最大化的优化梯度自主决定高斯元素的保留或修剪；引入带有有限不透明度先验的不透明度衰减技术，加速选择过程。

Result: 与3DGS相比，在15%预算下获得超过0.6 dB的PSNR增益，在紧凑3DGS方面达到最先进性能。

Conclusion: 该方法完全可学习且无需人工干预，实现了高效的高斯元素自主选择，显著提升了3DGS的紧凑性和性能。
...
TL;DR: 提出Synergistic Diversity (SQ)框架来提升植物病害检测精度，通过新的SQ指标衡量集成成员间的协同效应，改进集成模型选择


<details>
  <summary>Details</summary>
Motivation: 植物病害每年造成超过2200亿美元经济损失，及时准确检测至关重要。深度集成方法能提升预测精度，但现有集成多样性指标难以识别最优集成团队

Method: 1. 分析现有集成多样性指标(Q指标)的局限性；2. 提出新的SQ指标来捕捉集成成员间的协同效应；3. 在植物叶片图像数据集上进行广泛实验验证

Result: SQ指标显著改进了集成选择，提升了检测精度，与集成准确度保持一致
...
TL;DR: 提出RadioKMoE框架，结合KAN和MoE进行无线地图估计，通过KAN预测粗略覆盖图，MoE网络细化局部细节，在多频段和单频段场景下均表现优异。


<details>
  <summary>Details</summary>
Motivation: 复杂的无线电传播行为和周围环境对无线地图估计构成挑战，需要更精确和鲁棒的解决方案。

Method: 使用KAN模块预测初始粗略覆盖图，结合环境信息驱动MoE网络进行精确估计，MoE包含专门处理不同无线地图模式的专家网络。

Result: 实验结果表明，RadioKMoE在多频段和单频段无线地图估计中均表现出更高的准确性和鲁棒性。

Conclusion: RadioKMoE框架通过结合KAN和MoE的优势，有效提升了无线地图估计的精度和可靠性。
...
TL;DR: FLUID是一个无需训练的框架，通过在预训练扩散模型的潜在空间中直接替换身份来实现人脸去标识化，使用新颖的试剂损失函数指导优化，在身份抑制和属性保留方面达到最优平衡。


<details>
  <summary>Details</summary>
Motivation: 当前的人脸去标识化方法在身份抑制和属性保留之间存在权衡问题，需要一种更有效的方法来同时保持图像质量和身份匿名性。

Method: 基于化学中的置换机制，将身份编辑重新解释为预训练无条件扩散模型潜在h空间中的语义位移，通过优化发现身份编辑方向，使用线性测地线编辑方案在潜在流形中导航。

Result: 在CelebA-HQ和FFHQ数据集上的实验表明，FLUID在身份抑制和属性保留方面优于现有最先进的去标识化方法，在定性和定量指标上都表现出色。

Conclusion: FLUID框架通过潜在空间中的身份置换实现了高效的人脸去标识化，为隐私保护应用提供了新的解决方案。
...
TL;DR: 提出DReX视觉复杂度预测模型，仅使用视觉特征就达到SOTA性能，超越多模态方法，参数减少21.5倍


<details>
  <summary>Details</summary>
Motivation: 探索视觉复杂度预测任务中语言信息是否必要，验证纯视觉方法的能力

Method: 融合自监督DINOv3 ViT-S/16和监督ResNet-50特征，通过可学习注意力机制整合多尺度层次特征

Result: 在IC9600基准上达到Pearson r=0.9581，在多个数据集和指标上表现优异
...
TL;DR: DepthFocus是一个可操控的视觉Transformer，将立体深度估计重新定义为意图驱动的控制，能够根据深度偏好动态调整计算以聚焦到目标深度层，实现复杂场景中的选择性感知。


<details>
  <summary>Details</summary>
Motivation: 现实世界中的深度通常是多层次的，透射材料会产生分层模糊，而现有模型只能被动估计静态深度图，无法像人类那样主动调整焦点来感知所需深度。

Method: 采用可操控的Vision Transformer架构，通过标量深度偏好作为条件输入，动态调整计算过程来聚焦到目标深度层，主要使用新构建的50万张多层合成数据集进行训练。

Result: 在BOOSTER等传统单深度基准测试中达到最先进性能，在新提出的真实和合成多深度数据集上定量展示了意图对齐的估计能力，并在未见过的透射场景中表现出强大的泛化能力。

Conclusion: DepthFocus代表了向主动、类人3D感知迈出的重要一步，能够处理现实世界中的多层深度感知挑战。
...
TL;DR: MVLR模型通过结合视觉语言模型和隐式记忆库，统一恢复多种恶劣天气条件下的图像退化，在保持计算效率的同时提升恢复精度。
...
Method: 使用轻量级编码器-解码器主干，结合视觉语言模型进行链式推理编码天气退化先验，隐式记忆库存储退化模式表示，通过动态交叉注意力机制自适应融合多尺度视觉特征。
...
<details>
  <summary>Details</summary>
Motivation: 推动体育分析中的计算机视觉研究，解决现有数据集在球拍姿态细粒度标注方面的不足，促进动态物体跟踪和条件运动预测的研究。

Method: 构建包含乒乓球、网球和羽毛球的大规模数据集，提供球拍姿态和球位置的细粒度标注，设计交叉注意力机制进行多模态融合。

Result: 评估显示，简单拼接球拍姿态特征会降低性能，而交叉注意力机制能有效利用这些特征，在轨迹预测任务上超越强单模态基线。
...
TL;DR: 本文介绍了ConfusedTourist，一个评估视觉语言模型在混合文化线索场景下稳定性的对抗鲁棒性测试套件，发现现有模型在简单图像堆叠扰动下准确率大幅下降。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法通常只关注单一文化概念，忽略了现实场景中多个不相关文化线索共存的情况，需要测试VLMs在多样化文化输入下的稳定性以支持多元文化社会。

Method: 开发了ConfusedTourist文化对抗鲁棒性测试套件，通过图像堆叠扰动和基于图像生成的变体来评估VLMs对扰动地理线索的稳定性。

Result: 实验显示VLMs存在严重脆弱性，在简单图像堆叠扰动下准确率大幅下降，基于图像生成的扰动效果更差。可解释性分析表明失败源于系统性的注意力转移。
...
TL;DR: 提出了一个统一的行人跟踪框架OmniPT，能够基于参考进行跟踪并生成对跟踪对象的语义理解，通过RL-Mid Training-SFT-RL训练阶段解决跟踪任务建模和格式化输出问题。
...
Method: 实现RL-Mid Training-SFT-RL训练阶段：基于LVLM预训练权重，先进行RL阶段使模型输出固定可监督的边界框格式，然后使用大量行人相关数据集进行中期训练，最后在多个行人跟踪数据集上进行监督微调，再进行RL阶段提升跟踪性能和指令跟随能力。

Result: 在跟踪基准测试上的实验结果表明，所提方法能够比之前的方法表现更好。

Conclusion: OmniPT框架成功将行人跟踪任务建模为基础模型可执行的任务，并通过多阶段训练实现了优于先前方法的性能。
...
TL;DR: ReBrain是一个基于检索增强扩散框架的脑部MRI重建方法，通过从CT扫描合成MRI，特别针对稀疏CT数据设计了Brownian Bridge扩散模型和检索机制来保证结构连续性。


<details>
  <summary>Details</summary>
Motivation: 由于物理或临床限制，某些患者无法进行MRI检查，而低剂量CT协议会产生稀疏的CT体积，使得从CT准确重建完整脑部MRI体积变得困难。

Method: 使用Brownian Bridge扩散模型合成2D MRI切片，同时通过微调的检索模型从先验数据库中检索结构和病理相似的CT切片作为参考，通过ControlNet分支指导中间MRI切片生成，并对罕见检索失败情况应用球面线性插值。

Result: 在SynthRAD2023和BraTS数据集上的广泛实验表明，ReBrain在稀疏条件下的跨模态重建中达到了最先进的性能。

Conclusion: ReBrain框架能够有效解决稀疏CT数据下脑部MRI重建的挑战，通过检索增强扩散方法实现了高质量的跨模态图像合成。
...
TL;DR: STAR建模通过使用图像块位置定义的网格中均匀生成树的遍历顺序，在保持采样性能的同时提供灵活的序列顺序，以支持图像编辑。


<details>
  <summary>Details</summary>
Motivation: 传统自回归模型在视觉生成中采用随机排列序列顺序的方法要么性能下降，要么在推理时序列顺序选择不够灵活。需要一种既能保持性能又能提供灵活编辑能力的方法。

Method: STAR利用图像块位置网格中采样的均匀生成树的遍历顺序，通过广度优先搜索获得遍历顺序，并通过拒绝采样确保连接的部分观察作为序列前缀出现。

Result: STAR在保持采样性能的同时，提供了足够灵活的序列顺序来适应图像编辑，且无需对广泛采用的语言AR模型架构进行重大更改。

Conclusion: STAR通过结构化的随机策略相比随机排列，在保持后补完成能力的同时维持了采样性能，为图像编辑提供了更好的灵活性。
...
TL;DR: RoomPlanner是一个全自动3D房间生成框架，仅需短文本输入即可创建逼真的室内场景，无需手动布局设计或全景图像指导，能在30分钟内生成几何合理的3D室内场景。


<details>
  <summary>Details</summary>
Motivation: 解决传统3D室内场景生成需要大量手动布局设计或全景图像指导的问题，实现仅通过短文本输入就能自动生成逼真室内场景的目标。
...
Conclusion: RoomPlanner是首个全自动3D房间生成框架，通过文本输入就能高效生成高质量的室内场景，为3D场景生成提供了新的解决方案。
...
TL;DR: PathAgent是一个基于大语言模型的训练免费框架，模拟病理学家的逐步分析过程，通过导航器定位关键微区域、感知器提取形态学特征、执行器整合推理轨迹，实现全玻片图像的透明可解释分析。
...
Method: 采用训练免费的LLM代理框架，包含三个模块：导航器迭代精确定位关键微区域，感知器提取形态学视觉线索，执行器将发现整合到自然语言推理轨迹中。

Result: 在五个挑战性数据集上评估，PathAgent展现出强大的零样本泛化能力，在开放性和约束性视觉问答任务中超越任务特定基线。

Conclusion: 与人类病理学家的协作评估证实PathAgent作为透明且临床基础诊断助手的潜力，其完整的观察和决策序列形成明确的思维链，产生完全可解释的预测。
...
TL;DR: DMG-YOLO是一个轻量级实时检测器，专为遥感图像中的小目标检测设计，通过双分支特征提取和多尺度特征融合模块提升性能。


<details>
  <summary>Details</summary>
Motivation: 解决遥感图像中小目标普遍存在且需要平衡检测精度与效率的挑战，实现实时检测。

Method: 设计了双分支特征提取模块（DFE）分别提取局部特征和全局上下文，多尺度特征融合模块（MFF）增强多尺度集成，以及全局局部聚合特征金字塔网络（GLAFPN）进一步优化小目标检测。
...
TL;DR: RL-AD-Net是一个基于强化学习的点云补全精炼框架，通过在预训练点云自编码器的潜在空间中操作，选择性调整全局特征向量来改善局部几何一致性。


<details>
  <summary>Details</summary>
Motivation: 现有的点云补全模型虽然能生成全局合理的形状，但常常存在局部几何不一致的问题，需要一种轻量级、模块化的精炼方法来提升几何保真度。
...
Conclusion: 该框架轻量、模块化且模型无关，可广泛应用于各种补全网络而无需重新训练，强化学习引导的集成精炼方法有效提升了点云补全质量。
...
TL;DR: REArtGS++ 提出了一种基于平面高斯抛雪和时序几何约束的可泛化铰接物体重建方法，改进了对螺旋关节和多部件物体的处理能力。
...
Method: 使用解耦螺旋运动建模关节，通过部件运动混合联合优化部件感知高斯和关节参数；引入平面高斯和基于泰勒一阶展开的时序一致性正则化。

Result: 在合成和真实铰接物体上的大量实验表明，该方法在可泛化的部件级表面重建和关节参数估计方面优于现有方法。

Conclusion: REArtGS++ 通过时序几何约束和平面高斯抛雪，显著提升了铰接物体重建的泛化能力和精度。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频生成方法大多使用单次规划或迭代精炼，前者限于简单运动，后者计算成本高。需要一种既能提高运动质量又高效的方法。

Method: 给定提示和参考图像，预测多个候选运动规划，使用视觉语言验证器评估语义对齐和物理合理性，通过将轨迹渲染为轻量级视频草图来高效评分，迭代精炼直到找到满意规划。

Result: 在WorldModelBench和PhyWorldBench上的实验表明，该方法显著提高了运动质量、物理真实性和长期一致性，同时效率更高。扩展轨迹候选数量能持续提升性能。

Conclusion: SketchVerify框架通过草图验证规划有效提升了视频生成的运动质量，在保持高效率的同时实现了更好的动态连贯性。
...
Method: 分析发现特征图中的关键组件控制早期尺度的多样性形成，通过抑制模型输入中的关键组件并放大模型输出中的该组件来提升多样性。
...
TL;DR: 提出了一个针对大型视觉语言模型幻觉问题的综合干预框架，基于transformer因果架构分析不同干预路径对幻觉的影响。研究发现幻觉来自图像-输入文本、图像-输出文本和文本-文本路径的相互作用，且模型依赖不同路径取决于问答对齐格式。


<details>
  <summary>Details</summary>
Motivation: 尽管大型视觉语言模型在各种任务中表现出色，但仍然容易产生幻觉。需要理解幻觉的根本原因并开发有效的干预方法。

Method: 基于transformer因果架构提出综合干预框架，分析三种路径对幻觉的影响。针对判别式和生成式格式，提出识别和干预关键幻觉头的简单有效方法。

Result: 在多个基准测试上的实验表明，该方法能够持续减少各种对齐类型中的幻觉。

Conclusion: LVLMs的幻觉不是来自单一因果路径，而是多种路径相互作用的结果。通过针对不同问答格式干预关键幻觉头，可以有效减少幻觉问题。
...
TL;DR: 提出一种基于平面高斯分布的类别无关铰接物体重建框架，仅需单状态稀疏视角RGB图像即可实现高保真部件级表面重建。
...
### [56] [Range-Edit: Semantic Mask Guided Outdoor LiDAR Scene Editing](https://arxiv.org/abs/2511.17269)
...
<details>
  <summary>Details</summary>
Motivation: 解决自动驾驶系统训练中真实点云数据获取困难、特别是边缘场景数据稀缺的问题，以及现有模拟方法成本高且难以捕捉真实世界复杂性的局限性。

Method: 将点云转换为2D距离图像，使用基于凸包的语义掩码作为引导条件，通过扩散模型进行语义编辑生成新的合成LiDAR点云。

Result: 在KITTI-360数据集上验证了高质量LiDAR点云生成能力，能够产生复杂的边缘场景和动态场景。

Conclusion: 该方法为生成多样化LiDAR数据提供了经济高效且可扩展的解决方案，有助于提升自动驾驶系统的鲁棒性。
...
### [57] [Sparse Reasoning is Enough: Biological-Inspired Framework for Video Anomaly Detection with Large Pre-trained Models](https://arxiv.org/abs/2511.17094)
*He Huang,Zixuan Hu,Dongxiao Li,Yao Xiao,Ling-Yu Duan*

Main category: cs.CV

TL;DR: ReCoVAD是一个基于预训练大模型的免训练视频异常检测框架，通过模拟人类神经系统的双通路机制，选择性处理视频帧以减少冗余计算，在保持SOTA性能的同时显著降低了计算开销。


<details>
  <summary>Details</summary>
Motivation: 现有基于大模型的视频异常检测方法通常依赖密集帧级推理，计算成本高且延迟大。本文旨在探索在使用强大预训练模型时，是否真的需要密集推理，以及如何通过选择性帧处理来减少冗余计算。

Method: 提出ReCoVAD框架，包含两个核心通路：(1)反射通路：使用轻量级CLIP模块融合视觉特征与原型提示，查询动态记忆进行快速响应；(2)意识通路：使用中规模视觉语言模型生成文本事件描述和精炼异常分数，并集成大语言模型定期审查描述以识别未见异常。

Result: 在UCF-Crime和XD-Violence数据集上，ReCoVAD仅处理了先前方法28.55%和16.04%的帧数，同时达到了免训练方法的SOTA性能，证明了稀疏推理在基于大模型的视频异常检测中的有效性。

Conclusion: 研究表明，在使用强大预训练模型时，稀疏推理足以实现有效的视频异常检测，无需密集帧处理，这为降低计算成本和延迟提供了新思路。
...
### [58] [Where Culture Fades: Revealing the Cultural Gap in Text-to-Image Generation](https://arxiv.org/abs/2511.17282)
...
TL;DR: 该论文提出了一种解决多语言文本到图像生成中文化一致性问题的方法，通过定位文化敏感神经元并进行针对性增强，提高跨语言文化一致性。
...
Method: 1. 提出探测方法定位文化敏感信号到少量固定层的神经元；2. 推理时文化激活放大已识别神经元；3. 层定向文化增强仅更新文化相关层。

Result: 在CultureBench上的实验表明，相比强基线方法，该方法在文化一致性方面有持续改进，同时保持了保真度和多样性。

Conclusion: 通过定位和增强文化敏感神经元，可以有效提高多语言T2I模型的文化一致性，而不需要完整的骨干网络微调。
...
### [59] [Bridging Visual Affective Gap: Borrowing Textual Knowledge by Learning from Noisy Image-Text Pairs](https://arxiv.org/abs/2511.17103)
*Daiqing Wu,Dongbao Yang,Yu Zhou,Can Ma*

Main category: cs.CV

TL;DR: 提出了一种名为PACL的分区自适应对比学习方法，通过利用预训练文本模型的知识来增强视觉模型的情感感知能力，解决视觉情感识别中的"情感鸿沟"问题。
...
Method: 提出PACL方法，关注社交媒体数据中图像和文本的事实与情感连接，通过分离不同类型的样本并为每种类型设计不同的对比学习策略，动态构建正负样本对，充分利用噪声样本的潜力。
...
Conclusion: ChainV通过动态整合视觉提示显著提升了多模态推理的准确性和效率，特别是在需要多步符号推理的数学密集型任务中效果显著。
...
TL;DR: MuM扩展了MAE到多视图场景，通过统一掩码所有视图和使用轻量级解码器，在3D视觉任务中超越了DINOv3和CroCo v2。
...
Conclusion: MuM提供了一种更简单、可扩展的方法来学习3D视觉特征，在多个几何推理任务中达到最先进性能。
...
### [62] [PEGS: Physics-Event Enhanced Large Spatiotemporal Motion Reconstruction via 3D Gaussian Splatting](https://arxiv.org/abs/2511.17116)
...
Method: 提出了三重监督方案：通过加速度约束强制物理合理性，利用事件流进行高时间分辨率指导，使用卡尔曼正则化器融合多源观测；设计了基于实时运动状态的自适应训练调度策略；构建了首个针对自然快速刚体运动的RGB-Event配对数据集。

Result: 实验表明PEGS在大时空尺度运动重建方面优于主流动态方法。

Conclusion: PEGS框架通过整合物理先验和事件流增强，在3D高斯泼溅管道中实现了有效的去模糊目标聚焦建模和运动恢复。
...
### [63] [Designing and Generating Diverse, Equitable Face Image Datasets for Face Verification Tasks](https://arxiv.org/abs/2511.17393)
*Georgia Baltsou,Ioannis Sarridis,Christos Koutlis,Symeon Papadopoulos*

Main category: cs.CV

TL;DR: 提出了一种使用生成模型创建多样化合成人脸图像的方法，并构建了包含27,780张图像的DIF-V数据集，以解决现有人脸验证系统中的偏见问题。
...
Method: 整合先进的生成模型创建多样化的高质量合成人脸图像，强调代表各种面部特征，并确保符合身份证照片的可接受特征。
...
### [64] [Off the Planckian Locus: Using 2D Chromaticity to Improve In-Camera Color](https://arxiv.org/abs/2511.17133)
*SaiKiran Tedla,Joshua E. Little,Hakki Can Karaimer,Michael S. Brown*

Main category: cs.CV

TL;DR: 本文提出从1D相关色温(CCT)映射转向2D色度空间，并使用轻量级多层感知器(MLP)替代传统CCT插值，以提升非普朗克光源(如LED)下的色度测量精度。
...
Method: 采用基于2D色度特征的轻量级MLP替代传统CCT插值，通过包含代表性LED光源的光箱校准程序训练MLP。

Result: 在多种LED照明下验证，该方法平均减少LED场景中角度再现误差22%，保持与传统光源的向后兼容性，支持多光源场景和实时相机部署。

Conclusion: 从1D CCT转向2D色度空间并结合MLP方法，能显著提升非普朗克光源下的色度测量精度，同时保持计算效率和兼容性。
...
### [65] [Sparse Mixture-of-Experts for Multi-Channel Imaging: Are All Channel Interactions Required?](https://arxiv.org/abs/2511.17400)
*Sukwon Yun,Heming Yao,Burkhard Hoeckendorf,David Richmond,Aviv Regev,Russell Littman*

Main category: cs.CV

TL;DR: MoE-ViT提出了一种基于混合专家架构的Vision Transformer，用于处理多通道图像，通过轻量级路由器选择相关通道进行注意力计算，显著提升效率而不损失性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在多通道图像处理中独立处理每个通道，导致注意力计算呈二次增长，计算成本过高。需要解决跨通道注意力的效率问题。

Method: 将每个通道视为专家，使用轻量级路由器为每个图像块选择最相关的通道专家进行注意力计算，避免所有通道间的交互。

Result: 在JUMP-CP和So2Sat数据集上的实验表明，MoE-ViT实现了显著的效率提升，在某些情况下甚至提高了性能。

Conclusion: MoE-ViT是多通道图像处理的实用且高效的Vision Transformer骨干网络，在保持性能的同时大幅降低计算成本。
...
### [66] [A Multi-Stage Optimization Framework for Deploying Learned Image Compression on FPGAs](https://arxiv.org/abs/2511.17135)
*Jiaxun Fang,Li Chen*

Main category: cs.CV

TL;DR: 提出了一个多阶段优化框架，将高性能浮点图像压缩模型转换为高效的8位整数FPGA实现，通过动态范围感知量化、混合精度搜索和通道剪枝技术，在保持性能的同时显著降低计算复杂度。
...
Method: 1. 动态范围感知量化(DRAQ)：使用统计校准的激活裁剪和权重正则化处理数据异常值和大动态范围；2. 渐进混合精度搜索：为各层分配最优非均匀位宽；3. 通道剪枝：针对GDN层消除非活跃通道。

Result: DRAQ方法将GDN模型的BD-rate开销从30%降至6.3%，硬件感知优化进一步降低20%以上计算复杂度，对率失真性能影响可忽略。

Conclusion: 最终模型在效率和性能上均达到最先进水平，优于现有FPGA图像压缩实现。
...
Conclusion: ODTSR成功平衡了真实世界图像超分辨率中的保真度和可控性，为扩散模型在图像增强任务中的应用提供了新思路。
...
TL;DR: 提出了一种新颖的知识蒸馏框架，通过使用在少量任务相关数据上微调的教师网络，来缓解在包含偏置特征的大数据集上训练的学生网络中的捷径学习问题。
...
Method: 利用在少量任务相关数据上微调的教师网络，通过知识蒸馏框架来缓解学生网络在包含偏置特征的大数据集训练中的捷径学习。实验在CheXpert、ISIC 2017和SimBA数据集上使用多种架构进行验证。

Result: 在多个数据集和架构上，相比传统经验风险最小化、基于增强的偏置缓解和基于组的偏置缓解方法，该方法取得了持续改进。在许多情况下，即使在外分布测试数据上，也能达到与在无偏置数据上训练的基线模型相当的性能。
...
TL;DR: CC-DiceCE损失函数基于CC-Metrics框架，相比传统Dice损失能更好地检测小病灶，提高召回率但略微增加假阳性，在多数据集研究中优于blob loss。


<details>
  <summary>Details</summary>
Motivation: 传统医学图像分割损失函数（如Dice）对小病灶分割不足，因为小病灶相对体积小，对总体损失贡献可忽略。

Method: 基于CC-Metrics框架提出CC-DiceCE损失函数，在nnU-Net框架内与blob loss和DiceCE基线进行比较。

Result: CC-DiceCE损失提高了检测召回率，分割性能基本不受影响，但略微增加假阳性；在多数据集研究中优于blob loss。

Conclusion: CC-DiceCE损失函数能有效改善小病灶检测，在保持分割质量的同时提高召回率，是优于现有方法的解决方案。
...
### [70] [REMSA: An LLM Agent for Foundation Model Selection in Remote Sensing](https://arxiv.org/abs/2511.17442)
*Binger Chen,Tacettin Emre Bök,Behnood Rasti,Volker Markl,Begüm Demir*

Main category: cs.CV

TL;DR: 本文介绍了RS-FMD数据库和REMSA智能体，用于解决遥感基础模型选择困难的问题。RS-FMD整理了150多个遥感基础模型，REMSA是基于LLM的智能体，能够根据自然语言查询自动推荐合适的模型。


<details>
  <summary>Details</summary>
Motivation: 遥感基础模型选择困难，因为文档分散、格式异构、部署约束多样，需要一个系统化的解决方案来帮助用户选择合适的模型。

Method: 1) 构建RS-FMD数据库覆盖150+遥感基础模型；2) 开发REMSA智能体，使用LLM解析用户需求、补全缺失约束、排序候选模型并提供解释；3) 建立包含75个专家验证场景的基准测试。

Result: REMSA在900个配置的测试中优于多个基线方法（包括朴素智能体、稠密检索和非结构化RAG的LLM），完全基于公开元数据运行。

Conclusion: RS-FMD和REMSA为遥感基础模型选择提供了有效的解决方案，REMSA能够准确理解用户需求并推荐合适的模型，且不依赖私有数据。
...
### [71] [DiffRefiner: Coarse to Fine Trajectory Planning via Diffusion Refinement with Semantic Interaction for End to End Autonomous Driving](https://arxiv.org/abs/2511.17150)
*Liuhan Yin,Runkun Ju,Guodong Guo,Erkang Cheng*

Main category: cs.CV

TL;DR: DiffRefiner是一个两阶段轨迹预测框架，首阶段使用Transformer提案解码器生成粗略轨迹，第二阶段通过扩散细化器迭代去噪和优化，结合判别性和生成性方法的优势，在自动驾驶轨迹预测中达到最先进性能。


<details>
  <summary>Details</summary>
Motivation: 现有的生成式轨迹预测方法（如扩散模型）通常基于人工设计的轨迹锚点或随机噪声进行去噪，仍有较大改进空间。作者希望结合判别性和生成性方法的优势，提升轨迹预测性能。

Method: 提出DiffRefiner两阶段框架：1）提案解码器阶段：使用Transformer基于传感器输入和预定义轨迹锚点回归生成粗略轨迹预测；2）扩散细化器阶段：通过迭代去噪过程优化初始预测，并设计了细粒度去噪解码器增强场景一致性。

Result: 在NAVSIM v2上达到87.4 EPDMS，在Bench2Drive上达到87.1 DS和71.4 SR，在公开基准测试中创造了新记录。消融研究验证了各组件有效性。

Conclusion: DiffRefiner通过结合判别性轨迹提案和生成性细化过程，显著提升了扩散基规划的轨迹预测性能，在多个基准测试中达到最先进水平。
...
TL;DR: UI-Styler是一个针对超声图像的类别感知风格迁移框架，通过模式匹配机制将目标图像的纹理模式转移到源图像上，同时保持源图像的结构内容，并使用类别感知提示策略实现语义对齐。


<details>
  <summary>Details</summary>
Motivation: 超声图像在不同采集设备间存在外观差异，导致域偏移问题，影响下游推理模型的性能。现有无配对图像转换方法在域适应过程中忽略了类别特定的语义对齐，导致内容-类别映射错位，影响诊断准确性。

Method: 提出UI-Styler框架，包含：1）模式匹配机制转移目标图像纹理模式到源图像；2）基于目标域伪标签的类别感知提示策略，确保与诊断类别的准确语义对齐。

Result: 在超声跨设备任务上的广泛实验表明，UI-Styler在分布距离和下游任务（如分类和分割）上始终优于现有无配对图像转换方法，达到最先进性能。

Conclusion: UI-Styler通过类别感知的风格迁移有效解决了超声图像跨设备域偏移问题，在保持结构内容的同时实现准确的语义对齐，显著提升下游任务的性能。
...
TL;DR: FireScope是一个基于视觉语言模型的推理到生成框架，通过结合强化学习和视觉监督来预测野火风险栅格图，并在跨大陆评估中表现出优异的泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有的野火风险预测方法缺乏因果推理和多模态理解能力，难以实现可靠的泛化。需要开发能够整合视觉、气候和地理因素的综合推理方法。

Method: 提出FireScope-Bench数据集和基准，结合Sentinel-2影像和气候数据；开发FireScope框架，使用基于VLM的推理到生成方法，通过强化学习和视觉监督学习预测风险栅格图和推理轨迹。

Result: 在美国训练并在欧洲测试时，FireScope实现了显著的性能提升，其推理轨迹被专家反馈和自动分析确认为忠实且语义上有意义。

Conclusion: 基于语言的推理可以改进视觉生成模型的泛化能力，FireScope是首个展示语言推理能提升视觉生成泛化性的框架，也是首个可跨大陆应用的高分辨率野火风险模型。
...
Method: 在音频、视频和多模态三个维度上系统评估自监督特征，涵盖唇部运动和通用视觉内容等不同领域，评估检测效果、可解释性和跨模态互补性。

Result: 大多数自监督特征能捕获深度伪造相关信息且具有互补性，模型主要关注语义上有意义的区域而非虚假伪影，但无法可靠地跨数据集泛化。
...
TL;DR: 提出INTSD夜间交通标志数据集和LENS-Net网络，解决夜间交通标志识别挑战，通过自适应图像增强和多模态分类提升识别性能
...
Result: 方法超越现有框架，消融研究证实关键组件的有效性

Conclusion: INTSD数据集和LENS-Net代码已公开，为夜间交通标志识别研究提供重要资源
...
Conclusion: PostCam通过统一的特征表示和两阶段训练策略，实现了更准确和灵活的相机运动操控，显著提升了动态场景中新颖视角视频的生成性能。
...
TL;DR: 提出多阶段噪声解耦框架，将复杂噪声分解为显式和隐式建模分量，通过预训练和联合微调策略，有效处理真实高光谱图像中的复杂噪声。
...
Method: 多阶段噪声解耦框架：1）将噪声分解为显式和隐式建模分量；2）用现有噪声模型预训练去噪网络处理显式噪声；3）引入高频小波引导网络处理隐式噪声；4）采用分离预训练和联合微调的多阶段学习策略。
...
Method: 1) 4D感知视觉表示：提取视觉特征，将1D时间嵌入3D位置形成4D嵌入，通过交叉注意力融合；2) 时空动作表示：扩展传统空间动作表示，加入时间信息进行时空规划
...
TL;DR: 提出CA-SAM方法，通过轻量级对齐层适配SAM模型到医疗图像分割任务，在持续学习场景下避免灾难性遗忘并保持高性能。


<details>
  <summary>Details</summary>
Motivation: 医疗图像分割中，机构间隐私政策差异使得联合训练不可行，需要持续学习；SAM模型参数大、计算开销高，需要平衡效率与性能。

Method: 引入对齐层模块对齐编码器-解码器特征分布，基于此提出CA-SAM持续学习策略，自动适配对齐层并利用SAM零样本先验。

Result: 在9个医疗分割数据集的持续学习场景下取得最先进性能。

Conclusion: SAM范式在平衡计算效率和性能后具有很大潜力，CA-SAM能有效解决医疗图像分割中的持续学习问题。
...
<details>
  <summary>Details</summary>
Motivation: 现有稠密3D重建方法虽能准确捕捉局部几何，但集成到SLAM中面临漂移和冗余点云问题，限制了效率和下游任务（如新视角合成）的性能。

Method: 首先通过轻量级跟踪重建模块构建局部一致子图，然后逐步对齐融合到全局高斯地图中，强制跨视角几何一致性，全局地图反馈修正局部漂移增强跟踪鲁棒性。

Result: 在真实世界数据集上实现最先进的跟踪、3D重建和新视角渲染性能，跟踪精度提升超过12%，生成更精细的几何细节，同时保持紧凑内存高效的全局表示。

Conclusion: SING3R-SLAM通过局部-全局协同优化策略，有效解决了稠密SLAM中的漂移和冗余问题，为多下游应用提供了高效且高质量的3D建图解决方案。
...
Conclusion: SPECTRE证明无需依赖私有数据即可实现高性能、可泛化的表示，建立了可扩展、开放且完全基于Transformer的3D医学成像基础模型。
...
TL;DR: 提出了一个针对鱼眼图像的BEV语义分割框架，通过几何反投影和像素级深度分布估计直接处理多相机高分辨率鱼眼图像，无需去失真或透视校正。
...
TL;DR: 提出了一种双域自适应网络，能够将预训练的图像超分辨率模型从模拟数据集高效适配到真实世界数据集，通过空间域和频域的双重自适应策略提升真实图像超分辨率性能。


<details>
  <summary>Details</summary>
Motivation: 真实世界图像超分辨率面临复杂退化模式，但缺乏足够的真实LR-HR数据对。预训练SR模型包含有价值的先验知识，可以改善泛化能力、加速训练并减少对真实数据的需求。
...
Conclusion: 双域自适应网络能够有效利用预训练SR模型的先验知识，通过空间和频域的双重自适应策略显著提升真实世界图像超分辨率性能。
...
TL;DR: QueryOcc是一个基于查询的自监督框架，通过4D时空查询直接从图像学习连续3D语义占据，无需大规模3D标注，在自监督Occ3D-nuScenes基准上超越先前方法26%。
...
Method: 使用独立4D时空查询采样相邻帧，支持从视觉基础模型生成的伪点云或原始激光雷达数据进行监督，引入压缩场景表示在恒定内存下实现长距离监督和推理。
...
<details>
  <summary>Details</summary>
Motivation: 解决等变网络在资源受限环境中的部署问题，平衡几何鲁棒性与计算效率，特别针对卫星图像分析等几何视觉任务。

Method: 使用e2cnn库实现C4循环群等变性，通过分析层结构进行神经元级结构化剪枝，结合自适应微调、动态INT8量化和知识蒸馏的完整流程。

Result: 在EuroSAT、CIFAR-10和Rotated MNIST数据集上验证，实现29.3%参数减少且保持显著精度恢复，证明在保持几何鲁棒性的同时实现有效压缩。

Conclusion: 该框架为等变模型优化提供了可复现方案，弥合了群论网络设计与实际部署需求之间的差距，特别适用于卫星图像分析和几何视觉任务。
...
TL;DR: 提出了一种基于四元数保真度项的颜色图像盲去卷积方法，通过四元数卷积核建模颜色通道间的关系，有效去除伪影并提升去模糊效果。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常将彩色图像转为灰度或单独处理每个颜色通道，忽略了颜色通道之间的相互关系，这限制了彩色图像盲去卷积的性能。

Method: 设计了一个新颖的四元数保真度项，利用四元数卷积核的特性，包含四个核：一个类似非负卷积核捕捉整体模糊，另外三个无约束核分别对应RGB通道建模未知的相互依赖关系，并使用归一化四元数核来保持图像强度。

Result: 在真实模糊彩色图像数据集上的大量实验表明，该方法有效去除伪影并显著改善去模糊效果。

Conclusion: 该方法作为彩色图像去卷积的强大工具具有潜力，能够更好地处理颜色通道间的关系。
...
<details>
  <summary>Details</summary>
Motivation: 大型视觉语言模型虽然支持自然语言视觉搜索，但提升性能通常需要微调或使用更大模型。本文受传统文本搜索启发，旨在通过相关性反馈机制在推理时改善检索效果。

Method: 提出了四种反馈策略：1）经典伪相关性反馈（PRF）；2）生成式相关性反馈（GRF），使用合成标题进行查询优化；3）注意力反馈总结器（AFS），基于Transformer整合多模态细粒度特征；4）使用真实标题的显式反馈作为上限基准。

Result: 在Flickr30k和COCO数据集上的实验表明，GRF、AFS和显式反馈相比无反馈检索，在MRR@5指标上为小型VLM提升3-5%，大型VLM提升1-3%。AFS与显式反馈类似，能减轻查询漂移，在迭代多轮检索中比GRF更稳健。
...
<details>
  <summary>Details</summary>
Motivation: 现有OCSR系统在准确识别立体化学信息方面面临挑战，因为立体异构体的视觉线索（如楔形键、虚线键）非常细微。

Method: 三阶段训练范式：1) 在大规模噪声数据集上进行预训练；2) 使用更丰富监督信号的数据集进行多粒度微调，探索化学键分类和原子定位等辅助任务；3) 使用强化学习进行后训练优化，并引入新的立体化学结构数据集。

Result: 实验结果表明，MolSight在立体化学光学结构识别方面达到了最先进的性能，即使参数规模相对紧凑，GRPO算法也能进一步提升立体分子识别能力。

Conclusion: MolSight框架通过系统化的三阶段训练方法，有效解决了OCSR中的立体化学识别难题，为化学信息学应用提供了更准确的结构识别能力。
...
TL;DR: BiFingerPose是一种基于双模态的指尖姿态估计算法，通过结合电容图像和指纹补丁，能够同时准确预测全面的指尖姿态信息，显著提升了预测性能、任务完成效率和用户操作准确性。


<details>
  <summary>Details</summary>
Motivation: 现有基于电容图像的指尖姿态估计算法在便携设备上主要局限于估计俯仰角和偏航角，且在大角度输入（特别是大于45度）时精度下降，无法可靠估计滚转角。

Method: 提出双模态输入方法，包括从带屏下指纹传感器的触摸屏获取的电容图像和指纹补丁，通过双模态融合实现全面的指尖姿态估计。

Result: 在12人用户研究中，BiFingerPose相比之前SOTA方法预测性能提升超过21%，任务完成效率提高2.5倍，用户操作准确性提升23%。

Conclusion: 该方法能够可靠估计滚转角，显著提升其他姿态参数的预测性能，在增强认证安全性和改善交互体验方面具有实际应用价值。
...
TL;DR: 提出SpatialGeo方法，通过融合几何和语义特征来增强多模态大语言模型的空间推理能力，在保持高效性的同时显著提升空间推理任务的准确性。
...
Method: 提出基于几何和语义特征分层融合的视觉编码器，通过层次适配器将自监督学习的几何特征与CLIP语义特征结合，使用随机特征丢弃避免仅依赖CLIP的平凡解。

Result: 在SpatialRGPT-Bench上提升至少8.0%的准确率，推理时内存成本减少约50%。

Conclusion: SpatialGeo有效增强了MLLMs的空间感知能力，在空间推理任务上取得显著改进，同时保持高效性。
...
Method: 采用相对姿态初始化与显式特征匹配，然后进行局部联合优化，最后引入包含特征轨迹的全局束调整来细化姿态并提升NeRF质量。

Result: 在基准数据集上的广泛评估表明，该方法在姿态估计精度和新视角合成方面优于现有最优方法，在挑战性场景中表现出优越的鲁棒性。
...
TL;DR: 本文提出了一种在文本到图像生成中准确渲染折射对象的方法，通过同步像素和使用斯涅尔折射定律来改善透明物体的光学效果。


<details>
  <summary>Details</summary>
Motivation: 生成式图像模型在合成透明物体时表现不佳，特别是折射效果，因为它们未能充分学习光学定律来准确渲染折射对象。

Method: 在生成过程的每一步，使用斯涅尔折射定律对物体边界内外的像素进行扭曲和合并；对于通过折射或反射可见的表面，通过同步主图像与以物体为中心的第二个生成全景图像来恢复其外观。

Result: 该方法生成了更符合光学物理约束的图像，显著提高了折射效果的视觉合理性。

Conclusion: 通过物理约束和像素同步，可以显著改善生成式模型在透明物体渲染方面的表现，生成更符合光学定律的图像。
...
TL;DR: 提出了一个统一框架用于多媒介绘画过程生成，通过语义驱动的风格控制机制和跨媒介风格增强，实现一致的纹理演化和过程转移。
...
Method: 将多种媒介嵌入扩散模型的条件空间，使用跨媒介风格增强和反向绘画训练策略，确保平滑且符合人类创作习惯的生成过程。

Result: 构建了大规模真实绘画过程数据集，在跨媒介一致性、时间连贯性和最终图像保真度方面取得良好结果，在LPIPS、DINO和CLIP指标上表现优异。

Conclusion: 提出的感知距离剖面曲线能够定量建模创作序列，反映人类艺术创作过程，为交互式绘画教程提供了有效解决方案。
...
TL;DR: 提出了一种基于图卷积网络的骨架动作识别标签高效方法，通过优化目标函数选择最具信息量的子集进行标注，结合数据代表性、多样性和不确定性。


<details>
  <summary>Details</summary>
Motivation: 解决骨架动作识别对大规模手动标注数据集的依赖问题，这些数据集的获取成本高且耗时。

Method: 使用图卷积网络学习新颖的获取函数，通过优化混合数据代表性、多样性和不确定性的目标函数来评分最具信息量的标注子集；扩展方法使用可逆GCN将数据从环境空间映射到潜在空间以更好地捕获数据固有分布。

Result: 在两个具有挑战性的骨架识别数据集上的广泛实验表明，该方法在标签效率方面优于相关工作。

Conclusion: 所提出的标签高效GCN方法在骨架动作识别中表现出有效性和优越性能。
...
TL;DR: DSeq-JEPA提出了一种判别性顺序联合嵌入预测架构，通过识别主要判别区域并按照重要性顺序预测后续区域，将JEPA风格的潜在预测与GPT风格的顺序推理相结合。


<details>
  <summary>Details</summary>
Motivation: 受人类视觉感知启发，传统I-JEPA对所有区域进行统一独立处理，缺乏明确的预测顺序概念。人类视觉会选择性、顺序性地从最信息丰富的区域到次要区域部署注意力。

Method: 首先基于transformer衍生的显著性图识别主要判别区域，然后按照这种判别顺序预测后续区域，形成从主要到次要线索的课程式语义进展。

Result: 在图像分类、细粒度视觉分类、检测分割和低级推理任务上的广泛实验表明，DSeq-JEPA比I-JEPA变体始终关注更具判别性和泛化性的表示。

Conclusion: DSeq-JEPA成功桥接了预测性和自回归自监督学习，通过整合潜在预测与顺序推理，实现了更有效的视觉表示学习。
...
TL;DR: 提出了一种统一的注意力-曼巴（UAM）骨干网络，用于细胞级放射组学特征的分类，并在多模态框架中结合细胞分类和图像分割，在公开基准测试中取得了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 细胞级放射组学特征能提供肿瘤表型的细粒度洞察，但现有研究多关注切片级或斑块级肿瘤分类，缺乏专门为放射组学数据设计的骨干网络。

Method: 引入统一的注意力-曼巴（UAM）骨干网络，灵活结合注意力和曼巴模块的能力，无需手动调整比例。开发了两种UAM变体，并进一步提出多模态UAM框架，联合执行细胞级分类和图像分割。

Result: UAM在公开基准测试中达到最先进性能：细胞分类准确率从74%提升至78%（n=349,882细胞），肿瘤分割精度从75%提升至80%（n=406斑块）。
...
TL;DR: SuperQuadricOcc使用超二次曲面表示场景，通过多层球面细分高斯近似实现监督训练，在Occ3D数据集上相比高斯方法减少75%内存占用、加速124%推理速度、提升5.9% mIoU，首次实现实时推理的占用估计模型。


<details>
  <summary>Details</summary>
Motivation: 高斯表示在自监督占用估计中广泛使用，但大量高斯基元显著增加内存需求且不适合实时推理。超二次曲面允许减少基元数量和内存需求，但缺乏超二次曲面光栅化器使得模型监督困难。

Method: 提出SuperQuadricOcc方法，采用超二次曲面场景表示，通过多层球面细分高斯近似超二次曲面，实现高斯光栅化用于训练监督。

Result: 在Occ3D数据集上，相比之前高斯方法：内存占用减少75%，推理速度提升124%，mIoU提升5.9%，基元数量减少84%，无需时间标签。

Conclusion: 这是首个实现实时推理同时保持竞争力的占用估计模型，超二次曲面的使用显著减少了场景建模所需的基元数量。
...
TL;DR: 提出ATAC方法，通过在CLIP嵌入空间计算增强诱导漂移向量来推断语义恢复方向，显著提升对抗攻击下的鲁棒性


<details>
  <summary>Details</summary>
Motivation: CLIP在零样本图像文本匹配方面表现优异，但对图像对抗扰动高度脆弱。对抗微调成本高昂，现有测试时防御方法鲁棒性有限

Method: 在CLIP嵌入空间操作，计算增强诱导漂移向量，基于这些潜在漂移的角一致性推断语义恢复方向并校正嵌入

Result: 在多种基准测试中，ATAC始终实现高鲁棒性，平均比先前最先进方法提升近50%，计算开销极小

Conclusion: ATAC是CLIP嵌入空间中测试时对抗防御新范式的有效方法，在非传统和极端设置下保持最先进鲁棒性，甚至对自适应攻击也具备非平凡鲁棒性
...
<details>
  <summary>Details</summary>
Motivation: 稀疏体素与邻居空间解耦且边界锐利，在优化过程中容易陷入局部最小值，而SDF虽然提供平滑连续的几何场，但在独立参数化的稀疏体素间保持平滑性具有挑战。

Method: 使用视觉几何模型进行鲁棒几何初始化，并通过空间平滑损失在父子体素组和兄弟体素组间强制保持连贯关系。

Result: 在多个基准测试中，该方法实现了强大的重建精度，同时保持快速收敛。

Conclusion: SVRecon通过集成SDF和稀疏体素，结合几何初始化和平滑约束，成功实现了高保真表面重建，代码将公开。
...
TL;DR: 提出了非参数概率鲁棒性(NPPR)，一种不依赖预定义扰动分布的更实用的概率鲁棒性度量方法，通过学习优化的扰动分布来实现分布不确定性下的保守鲁棒性评估。
...
Method: 基于高斯混合模型(GMM)与多层感知机(MLP)头部和双三次上采样开发NPPR估计器，覆盖输入相关和输入无关的扰动场景。

Result: 在CIFAR-10、CIFAR-100和Tiny ImageNet数据集上的实验验证NPPR作为更实用的鲁棒性度量，相比现有方法提供高达40%更保守的PR估计。

Conclusion: NPPR是一个更实用的鲁棒性度量，能够在不依赖预定义扰动分布的情况下提供保守的概率鲁棒性评估。
...
TL;DR: MorphSeek是一个用于医学图像配准的细粒度表示级策略优化框架，通过在高维潜在特征空间进行空间连续优化，实现了高效的数据利用和空间一致的变形优化。
...
Result: 在三个3D配准基准测试中，MorphSeek相比竞争基线实现了持续的Dice改进，同时保持高标签效率、低参数成本和低步级延迟开销。
...
TL;DR: 提出了MMT-ARD多模态多教师对抗鲁棒蒸馏框架，通过双教师知识融合架构提升视觉语言模型的对抗鲁棒性，在ImageNet上实现+4.32%的鲁棒准确率和3.5%的零样本准确率提升，训练效率提高2.3倍。


<details>
  <summary>Details</summary>
Motivation: 视觉语言模型在安全关键应用中部署增多，对抗鲁棒性成为关键问题。传统单教师方法存在知识多样性有限、收敛慢、鲁棒性与准确性难以平衡等挑战。
...
Conclusion: MMT-ARD有效提升了多模态大模型的对抗鲁棒性，证明了该方法的有效性和可扩展性。
...
TL;DR: 提出Illustrator's Depth新概念，将平面图像分解为可编辑的有序图层，实现图像向量化、文本到矢量图形生成、3D浮雕生成等应用
...
Method: 提出新的深度定义，为每个像素推断图层索引，使用策划的矢量图形数据集训练神经网络从光栅输入预测分层

Result: 显著优于现有图像向量化基线，支持文本到矢量图形生成、自动3D浮雕生成和深度感知编辑

Conclusion: 通过将深度从物理量重新定义为创作抽象，为可编辑图像分解提供了新基础
...
TL;DR: 该论文研究了如何利用视觉基础模型进行激光雷达点云语义分割的无监督域自适应，发现骨干网络架构对目标域泛化性能至关重要，可以预训练单一骨干网络应对多种域偏移，最佳方案是冻结预训练骨干并训练MLP头。


<details>
  <summary>Details</summary>
Motivation: 解决激光雷达语义分割网络在未见过的激光雷达类型上泛化性能差的问题，利用视觉基础模型提供跨域鲁棒特征。

Method: 基于无监督图像到激光雷达知识蒸馏，研究骨干网络架构、预训练策略和训练方法，采用冻结预训练骨干网络并训练MLP头的方案。

Result: 在四个广泛认可的挑战性设置中取得了最先进的结果。

Conclusion: 通过精心设计的骨干网络架构和训练策略，可以有效利用视觉基础模型提升激光雷达语义分割的跨域泛化能力。
...
Method: 设计新的定制神经网络，从连续时刻的B扫描图像中提取多尺度特征，然后通过分析这些特征之间的相似性和差异来确定行驶的欧几里得距离。

Result: 在CMU-GPR数据集上的实验表明，该方法在所有测试中均优于现有最先进方法，整体加权RMSE为0.449米，比最佳现有方法降低了10.2%的RMSE。
...
TL;DR: CWMDT是一个将标准视频扩散模型转化为反事实世界模型的框架，通过构建场景的数字孪生来编码对象和关系，使用大语言模型推理干预效果，并生成反事实视觉序列。


<details>
  <summary>Details</summary>
Motivation: 当前世界模型主要基于事实观察进行前向模拟，但许多应用需要回答反事实查询（如"如果移除这个对象会发生什么？"），传统模型在像素空间中无法选择性修改特定场景属性。

Method: 1) 构建观察场景的数字孪生，以结构化文本形式编码对象和关系；2) 使用大语言模型推理反事实干预如何随时间传播改变场景；3) 用修改后的表示条件化视频扩散模型生成反事实视觉序列。

Result: 在两个基准测试上的评估表明，CWMDT方法达到了最先进的性能，表明视频的替代表示（如数字孪生）为基于视频前向模拟的世界模型提供了强大的控制信号。

Conclusion: CWMDT框架成功地将标准视频扩散模型转化为有效的反事实世界模型，通过结构化表示和语言模型推理实现了对场景属性的选择性干预，为物理AI行为评估等应用提供了重要能力。
...
TL;DR: Radar2Shape是一个去噪扩散模型，通过将雷达信号的频率与多分辨率形状特征相关联，从部分可观测的雷达信号进行3D重建。


<details>
  <summary>Details</summary>
Motivation: 解决从高频雷达信号确定3D物体形状的复杂问题，现有方法难以表示任意形状或处理有限视角收集的真实世界雷达信号。

Method: 采用两阶段方法：首先学习具有分层分辨率形状特征的规范化潜在空间，然后通过以雷达信号频率为条件，以从粗到细的方式扩散到该潜在空间。

Result: Radar2Shape能够成功从部分观测的雷达信号重建任意3D形状，并在两种不同仿真方法和真实世界数据上表现出鲁棒的泛化能力。
...
TL;DR: 提出了一种基于计算机视觉的深度学习方法，使用超过18,000个MRI系列图像来估计脊柱年龄，并发现脊柱年龄差（SAG）与退行性脊柱疾病和生活方式因素相关。
...
Method: 使用UMAP和HDBSCAN识别年龄相关的脊柱退行性病变集群，建立深度学习模型估计脊柱年龄，通过消融研究确定最佳模型配置。

Result: 模型预测的脊柱年龄与实际年龄的差异（SAG）与椎间盘突出、骨赘、椎管狭窄、骨折等退行性病变，以及吸烟、体力劳动等生活方式因素显著相关。

Conclusion: 脊柱年龄差（SAG）可能是一个有用的生物标志物，用于衡量整体脊柱健康状况。
...
TL;DR: 论文分析了多模态模型中LLM规模缩减对视觉能力的影响，发现视觉能力下降比推理能力更严重，并提出了Extract+Think方法来解决这个问题。
...
Method: 首先分析LLM规模缩减对视觉能力的影响，然后提出Extract+Think方法：通过视觉提取调优训练模型提取指令相关的视觉细节，再应用逐步推理生成答案。

Result: 研究发现LLM规模缩减对视觉能力的影响比推理能力更严重，Extract+Think方法在效率和性能方面达到了新的标准。

Conclusion: LLM规模缩减会不成比例地影响视觉能力，Extract+Think方法通过分离视觉提取和推理过程，有效解决了这一瓶颈问题。
...
Method: 提出视觉反刍机制：迭代选择帧、放大信息区域、重新编码像素、更新推理状态；构建两个数据集用于监督实践和强化学习；采用多阶段反刍学习框架，通过SFT和GRPO-based RL逐步微调7B LMM。

Result: Video-R4-7B在M4-ViteVQA上达到最先进效果，并能泛化到多页文档QA、幻灯片QA和通用视频QA任务。
...
TL;DR: EvDiff是一个基于事件的扩散模型，通过代理训练框架从单色事件流生成高质量彩色视频，在保真度和真实性之间取得平衡。
...
Conclusion: EvDiff通过扩散模型和代理训练框架，成功解决了事件相机图像重建的挑战，在保真度和真实性方面取得了良好平衡。
...
TL;DR: 开发了一个基于spaCy的绍纳语形态分析工具包，包含词法库和语言规则，支持词性标注和形态特征分析，准确率达90%和88%。


<details>
  <summary>Details</summary>
Motivation: 绍纳语作为班图语系语言，在自然语言处理中资源匮乏，缺乏形态分析和语言感知工具。

Method: 基于spaCy框架构建规则驱动的形态分析流水线，结合JSON词法库和语言学规则，建模名词类别前缀、动词主语一致、时体标记等特征。

Result: 在正式和非正式绍纳语语料库上评估，词性标注准确率90%，形态特征准确率88%。

Conclusion: 该工具提升了绍纳语NLP的可访问性，为其他资源匮乏的班图语言提供了形态分析工具模板。
...
TL;DR: SPI是一种多分辨率向量索引框架，通过语义金字塔和动态分辨率选择，在保持语义覆盖的同时显著加速检索，实现5.7倍速度提升和1.8倍内存效率增益。
...
Conclusion: SPI框架与现有向量数据库基础设施兼容，可直接部署到生产RAG系统中，理论分析提供了检索质量和延迟保证。
...
TL;DR: 提出了Bench360框架，用于全面评估本地LLM推理配置，支持自定义任务、多种推理引擎和量化级别，并集成系统和任务特定指标。


<details>
  <summary>Details</summary>
Motivation: 本地运行LLM时面临大量配置选择，现有基准测试设计狭窄，无法满足用户需求，需要统一易用的基准测试框架。

Method: 开发Bench360框架，允许用户定义自定义任务、数据集和指标，自动测试不同LLM、推理引擎和量化级别，涵盖多种使用场景。

Result: 在四个常见LLM任务上测试，发现任务性能与系统效率之间存在权衡，不同推理引擎和模型表现差异显著。

Conclusion: 本地推理没有单一最佳配置，强烈需要像Bench360这样的框架来帮助用户做出明智选择。
...
TL;DR: 该研究评估了大型语言模型对突尼斯阿拉伯语的理解能力，创建了包含突尼斯方言、标准阿拉伯语和英语翻译的平行数据集，并在音译、翻译和情感分析三个任务上测试了多个流行模型。
...
Method: 1) 使用监督对比学习获取判别性特征空间；2) 用可学习矩阵参数化椭圆体作为已知类边界；3) 设计双损失函数平衡经验风险和开放空间风险。
...
TL;DR: 提出了一种评估提示词能否有效引导生成文本朝向特定人类价值观的实用方法，该方法可重现且模型无关，通过量化目标价值观在生成响应中的存在和增益来评分。
...
Method: 开发了一种实用、可重现、模型无关的程序，通过形式化评分方法来量化目标价值观在生成响应中的存在和增益。将该方法应用于Wizard-Vicuna语言模型的变体，使用Schwartz的基本人类价值观理论和对话数据集进行结构化评估。

Result: 比较了基线提示和明确基于价值观的条件提示，表明即使不改变模型或动态优化提示，价值观引导也是可能的。

Conclusion: 价值观引导是可行的，无需修改模型或动态优化提示，这为在动态环境中实现与人类价值观对齐提供了一种实用方法。
...
TL;DR: 本文提出了一种基于概念梯度的可解释性技术，通过分析毒性检测中的子类型属性（如淫秽、威胁、侮辱等）作为概念，识别毒性语言。引入了目标词典集和词-概念对齐分数来量化误分类，并提出了无词典增强策略来研究模型对毒性语言模式的归因。


<details>
  <summary>Details</summary>
Motivation: 社交媒体网络促进了有害内容的传播，虽然在文本数据中检测毒性语言取得了显著进展，但在毒性检测中基于概念的解释探索仍然有限。现有方法经常因为概念对目标类别的不均衡归因而导致分类错误。

Method: 利用毒性检测数据集中的各种子类型属性作为概念指标；提出基于概念梯度(CG)方法的可解释性技术；构建目标词典集捕获导致误分类的有毒词汇；计算词-概念对齐(WCA)分数量化误分类程度；引入无词典增强策略生成排除预定义有毒词典集的毒性样本。

Result: 概念梯度方法提供了更因果的解释，能够测量概念变化如何直接影响模型输出。目标词典集和WCA分数有效识别了由于对毒性概念过度归因而导致的误分类问题。无词典增强策略揭示了即使移除显性词汇重叠，模型仍可能对更广泛的毒性语言模式进行过度归因。

Conclusion: 该研究为毒性检测提供了更深入的可解释性分析框架，通过概念梯度方法和无词典增强策略，揭示了模型在毒性语言识别中的归因机制，有助于改进毒性检测模型的准确性和鲁棒性。
...
TL;DR: 本文研究了AI检测模型对经过轻微AI润色的人类撰写阿拉伯语文章的误判问题，发现所有检测器都会将大量润色后的人类文章错误归因于AI生成。


<details>
  <summary>Details</summary>
Motivation: 当前AI检测模型在面对经过AI轻微润色的人类撰写文章时会产生误判，可能导致错误指控作者使用AI剽窃，损害检测器的可信度。

Method: 生成了两个阿拉伯语数据集：第一个包含800篇文章用于评估14个LLM和商业AI检测器的区分能力；第二个Ar-APT数据集包含400篇人类文章，由10个LLM使用4种润色设置处理，共16400个样本。

Result: 所有AI检测器都会错误地将大量润色后的人类文章归因于AI。最佳LLM Claude-4 Sonnet准确率从83.51%降至57.63%；最佳商业模型originality.AI准确率从92%骤降至12%。

Conclusion: 轻微AI润色会显著影响AI检测器的性能，导致对人类撰写文章的误判，这对AI检测器的可靠性构成了严重挑战。
...
TL;DR: 复现了基于最近邻的测试时训练方法，该方法通过检索相似序列对语言模型进行推理时微调，显著降低了困惑度和比特率指标，特别是在结构化数据集上效果更明显。


<details>
  <summary>Details</summary>
Motivation: 验证测试时训练在语言模型上的有效性，探索如何通过检索增强的适应性微调来提升模型性能，特别是对于未在相关数据上预训练的模型。

Method: 使用预训练的RoBERTa嵌入和Faiss索引检索每个测试输入的20个最近邻，对GPT-2、GPT-Neo和R1-Distilled-Qwen2.5-1.5B模型进行每个邻居一次梯度更新的微调。

Result: 测试时训练显著降低了困惑度和比特率指标，在GitHub和EuroParl等结构化数据集上改进最大。未在The Pile上预训练的模型获益更多，小模型能接近大模型性能。

Conclusion: 最近邻测试时训练具有鲁棒性和通用性，即使对于现代推理优化架构也有效，同时开发了内存高效的检索实现方法。
...
TL;DR: 该论文系统分析了多语言大模型内部表示空间中语言信息的结构特征，发现语言信息在第一个transformer块就实现高度分离，且在整个模型深度中保持线性可分。语言方向与词汇嵌入的对齐程度与训练数据的语言组成密切相关。


<details>
  <summary>Details</summary>
Motivation: 虽然多语言LLM在多种语言上表现出色，但对其内部表示空间中语言信息的结构特征和跨层涌现机制缺乏系统性分析。

Method: 在6个多语言LLM上进行全面探测研究，覆盖268个transformer层，使用线性和非线性探针以及新的Token-Language Alignment分析来量化语言编码的层间动态和几何结构。

Result: 语言信息在第一个transformer块就实现高度分离（从第0层到第1层提升76.4±8.2个百分点），且在整个模型深度中保持几乎完全线性可分。包含中文的模型ZH Match@Peak达到16.43%，而英语中心模型仅为3.90%，显示出4.21倍的结构印记效应。

Conclusion: 多语言LLM不是通过表面脚本特征来区分语言，而是通过训练语料塑造的潜在表示结构。该分析为多语言表示学习中的数据组合策略和公平性提供了实用见解。
...
Method: 采用基于语言模型的本体嵌入方法，构建了针对SNOMED CT的OOV查询标注数据集，测试直接子类和较不相关祖先的检索性能。
...
TL;DR: 该研究发现共情行为（牺牲任务效率来满足人类需求）在LLM激活空间中表现为线性方向，并在三个不同模型上验证了检测和操控能力。


<details>
  <summary>Details</summary>
Motivation: 研究LLM中编码的共情行为是否可以被检测和操控，以及安全训练对共情行为编码和操控性的影响。

Method: 使用对比提示在EIA基准上测试三个模型（Phi-3-mini-4k、Qwen2.5-7B、Dolphin-Llama-3.1-8B），通过激活空间探测和干预来检测和操控共情行为。

Result: 所有模型都能高精度检测共情行为（AUROC 0.996-1.00），但操控效果因模型而异：Qwen和Phi-3能双向操控，Dolphin仅能增强共情，反共情操控会导致崩溃。

Conclusion: 共情编码独立于安全训练，但安全训练影响操控的鲁棒性；不同架构的共情实现方式不同，检测和操控能力存在差异。
...
TL;DR: JGU Mainz团队在BLP-2025代码生成共享任务中获胜，采用多代理管道方法，通过代码生成代理和调试代理的协作，在测试失败时自动修复代码，最终获得95.4%的Pass@1分数。
...
Method: 使用多代理管道：代码生成代理生成初始解决方案，调试代理在测试失败时分析错误信息并生成修正方案。

Result: 在BLP-2025共享任务中获得第一名，Pass@1分数达到95.4%。

Conclusion: 多代理管道方法在代码生成任务中表现出色，能够有效处理测试失败情况并自动修复代码。
...
TL;DR: 提出基于延展心智理论和激进具身认知的非表征性翻译心智ABC框架，将翻译视为整合情感、行为和认知的具身活动。


<details>
  <summary>Details</summary>
Motivation: 挑战基于表征的心智模型，为翻译研究提供非表征性解释框架。

Method: 结合延展心智理论、激进具身认知、预测处理和主动推理，构建ABC框架分析翻译过程。

Result: 翻译心智通过脑-身-环境互动循环涌现，而非简单延展；翻译是参与社会文化实践的技能性活动。

Conclusion: 意义在具身互动中实时共同创造，翻译应被重新定义为基于技能的社会文化参与。
...
TL;DR: 本研究重新探讨了非宾格动词的句法分类与语义特征（施事性和终结性）之间的关系，发现使用可解释维度结合人类判断能有效预测动词的句法行为。


<details>
  <summary>Details</summary>
Motivation: 尽管传统观点认为描述施事行为的动词更倾向于非作格句法，描述终结事件的动词更倾向于非宾格句法，但Kim等人（2024）的研究发现人类对施事性和终结性的评分并不能很好地预测非宾格动词的句法行为。

Method: 使用基于施事性和终结性两极种子词计算的可解释维度来重新分析非宾格动词的句法分类。

Result: 研究结果支持了非作格性/非宾格性与施事性/终结性之间的联系，表明可解释维度结合人类判断能为难以在评分任务中评估的语义属性提供有价值的证据。

Conclusion: 使用可解释维度方法能够更有效地揭示非宾格动词句法行为与语义特征之间的关系，为这一长期争议提供了新的证据支持。
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像扩散模型容易受到后门攻击，攻击者通过在输入提示中嵌入触发器来引导模型生成有害内容，需要有效的防御方法。

Method: PEPPER通过将输入提示重写为语义上不同但视觉相似的描述，同时添加不显眼的元素来破坏攻击触发器并稀释触发词的影响。

Result: 实验表明PEPPER对基于文本编码器的攻击特别有效，显著降低攻击成功率同时保持生成质量，与其他防御方法结合可获得更强的鲁棒性。

Conclusion: PEPPER提供了一种有效的后门攻击防御方案，能够破坏攻击触发器并保持生成质量，具有通用性和可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: LLM经常生成冗长、冗余的响应，这会降低清晰度和用户满意度，并增加模型开发成本（特别是按输出token计费的专有模型）。

Method: 通过三种计算的平均值来量化简洁性：1）原始响应与LLM抽象摘要的压缩比；2）原始响应与LLM提取摘要的压缩比；3）LLM在保持意义的前提下移除尽可能多非必要单词的压缩比。

Result: 实验结果表明，该指标能有效识别LLM输出中的冗余，为对话AI系统响应简洁性的自动评估提供了实用工具。

Conclusion: 提出的无参考简洁性评估指标能够在不依赖人工标注的情况下，有效评估LLM响应的简洁程度，有助于提升对话AI系统的响应质量。
...
Conclusion: SCM是一种有效的软概念感知训练方案，能够提升LLM的推理能力，弥合软概念推理与离散token训练之间的差距。
...
TL;DR: 提出了一种改进TRMs训练效率的新方法，通过将潜在推理视为分类器自由引导和隐式策略改进算法，在保持质量的同时显著减少前向传播次数并消除停止机制。


<details>
  <summary>Details</summary>
Motivation: 探索如何以最小改动进一步提高TRMs等小型循环架构的效率，这些架构在复杂推理任务上已显示出优于大型语言模型的潜力。

Method: 将TRMs的潜在推理框架化为分类器自由引导和隐式策略改进算法，提出为每个训练循环提供目标的新训练方案。

Result: 方法显著提升训练效率：前向传播总数减少18倍，无需停止机制，在仅0.8M参数下达到ARC-1 24%准确率，超越大多数LLMs。

Conclusion: 提出的训练方案能显著提高TRMs的效率，在保持性能的同时大幅减少计算需求，为小型模型在复杂推理任务中的应用提供了新方向。
...
TL;DR: 本文研究了训练数据的统计特性与诱导头形成的关系，发现批大小和上下文大小的组合可以预测诱导头形成点，表面二元重复频率和可靠性强烈影响诱导头形成，并确定了这两个值的精确帕累托边界。


<details>
  <summary>Details</summary>
Motivation: 诱导头被认为是现代语言模型具有卓越上下文学习能力的基础，但其形成的精确机制尚不清楚。本研究旨在探究训练数据的统计特性与诱导头形成之间的关系。

Method: 通过分析自然和合成数据的统计特性，研究批大小、上下文大小、表面二元重复频率和可靠性对诱导头形成的影响，并确定这些参数的帕累托边界。

Result: 发现批大小和上下文大小的简单组合可以预测诱导头形成点；表面二元重复频率和可靠性强烈影响诱导头形成，存在精确的帕累托边界；局部依赖性与高二元重复频率和可靠性足以形成诱导头。

Conclusion: 训练数据的统计特性，特别是批大小、上下文大小、二元重复频率和可靠性，对诱导头的形成具有决定性影响，为理解语言模型上下文学习能力的形成机制提供了重要见解。
...
TL;DR: 提出了一种新颖的论点感知定量摘要任务，用于揭示对话中的论点结构并量化论点强度，开发了ARQUSUMM框架来实现这一目标。


<details>
  <summary>Details</summary>
Motivation: 在线对话平台上的争议性话题需要不仅总结多样论点，还要揭示其论证结构和理由。现有方法要么忽略论点的论证性质，要么无法深入分析句子内部的论点结构。
...
Conclusion: 该研究成功实现了论点感知的定量摘要，为理解在线对话的论证结构提供了有效方法，生成的摘要对用户更有帮助。
...
TL;DR: 本研究提出了一种监督微调方法，用于提升大语言模型在湖南历史名人领域的信息抽取能力，通过构建领域特定的指令模板和数据集，在低资源环境下显著提升了模型性能。


<details>
  <summary>Details</summary>
Motivation: 湖南历史名人的系统数据资源有限，通用大模型在领域知识抽取和结构化输出生成方面表现不佳，需要针对性的方法来提升在低资源环境下的信息抽取能力。

Method: 设计了针对湖南历史名人领域的细粒度、模式引导的指令模板，构建指令调优数据集，并对四个公开大语言模型进行参数高效的指令微调。

Result: 所有模型在微调后都表现出显著的性能提升，其中Qwen3-8B表现最佳，在100个样本和50次训练迭代后达到89.3866分。

Conclusion: 本研究为垂直领域大语言模型的微调提供了新思路，展示了其在文化遗产知识抽取和知识图谱构建中的成本效益应用潜力。
...
TL;DR: 本文研究了视觉语言模型是否真正理解视觉说服力，发现模型存在回忆偏向，对低/中层特征判别能力弱，而高层语义对齐是最强预测因子。简单的干预策略效果有限，但基于对象的理性解释能显著提升性能。


<details>
  <summary>Details</summary>
Motivation: 探索视觉语言模型是否真正理解视觉说服力——视觉线索如何影响人类态度和决策，目前尚不清楚。

Method: 构建了二元说服力判断的高共识数据集，引入视觉说服因素分类法（包括低层感知、中层构图和高层语义线索），并探索了认知引导和知识注入策略。

Result: 实验分析显示VLMs存在回忆偏向（过度预测高说服力），对低/中层特征判别能力弱，而高层语义对齐是最强预测因子。基于对象的理性解释能显著改善精度和F1分数。
...
TL;DR: 提出了一个可解释的自动评分框架AnalyticScore，基于四个可解释性原则(FGTI)，通过提取可识别元素、使用LLM生成可解释特征和应用直观的序数逻辑回归模型，在保持高准确性的同时提供透明度。
...
Result: AnalyticScore在ASAP-SAS数据集的10个项目上，平均仅比不可解释的最先进方法低0.06 QWK，且其特征化行为与人类标注者高度一致。

Conclusion: AnalyticScore框架证明了在保持高评分准确性的同时实现可解释性自动评分的可行性，为未来研究提供了基准参考框架。
...
TL;DR: MUCH是首个针对声明级不确定性量化的基准测试，包含4873个样本，覆盖4种欧洲语言和4个开源LLM，提供生成logits数据并提出了高效的确定性分割算法。
...
Method: 构建包含4873个样本的多语言数据集，提供24个生成logits数据，开发确定性分割算法（仅需0.2%的LLM生成时间），支持实时监控。

Result: 创建了MUCH基准测试，评估显示当前方法在性能和效率方面仍有很大改进空间。

Conclusion: MUCH基准为声明级不确定性量化方法的未来发展提供了重要基础，当前方法仍需改进以应对实际部署需求。
...
TL;DR: 首个在AMD硬件上大规模MoE预训练研究，使用MI300X GPU和Pollara互连，提供系统优化指南和模型设计规则，开发了ZAYA1-base模型在推理、数学和编码基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 验证AMD硬件、网络和软件栈是否足够成熟和优化，能够支持具有竞争力的大规模预训练，并为在AMD硬件上进行MoE训练提供实用指导。

Method: 进行系统集群和网络特性分析，包括核心集合通信的微基准测试；开发MI300X感知的transformer尺寸规则；设计MoE宽度以联合优化训练吞吐量和推理延迟；构建包含容错和检查点重塑的训练栈。

Result: ZAYA1-base模型（760M激活参数，8.3B总参数MoE）在推理、数学和编码基准测试中表现优于Llama-3-8B和OLMoE等模型，与Qwen3-4B和Gemma3-12B等领先基础模型性能相当。

Conclusion: AMD硬件、网络和软件栈已经足够成熟和优化，能够支持具有竞争力的大规模预训练，为在AMD平台上进行高效MoE训练提供了可行的解决方案。
...
TL;DR: 本文提出使用上下文压缩作为无监督预训练任务来改进LLM的文本表示能力，相比基于token级预测的方法表现更好且更高效


<details>
  <summary>Details</summary>
Motivation: 现有LLM主要针对因果预测优化，不适合生成整体文本表示。虽然已有研究引入预训练任务来适配LLM，但大多依赖token级预测目标，存在局限性

Method: 提出上下文压缩预训练任务，让模型学习生成紧凑的记忆token来替代整个上下文进行下游序列预测，并结合对比学习进一步优化

Result: 实验表明压缩目标显著提升LLM文本表示能力，优于token级预训练任务。通过对比学习得到的LLM2Comp模型在多种任务上超越当代LLM文本编码器，且训练数据需求更少

Conclusion: 上下文压缩是有效的LLM适配预训练任务，能产生强大的文本表示模型，在性能和样本效率上都优于现有方法
...
TL;DR: 本文介绍了LangMark，一个用于自动后编辑(APE)的多语言数据集，包含英语到7种语言的翻译三元组，并证明LLMs通过少量样本提示可以有效执行APE任务。


<details>
  <summary>Details</summary>
Motivation: 解决神经机器翻译(NMT)输出缺乏大规模多语言APE数据集的问题，以促进APE系统的发展。

Method: 构建并发布LangMark数据集，包含206,983个三元组(源文本、NMT输出、人工后编辑翻译)，由专业语言学家标注；使用LLMs进行少量样本提示的APE实验。

Result: LLMs通过少量样本提示能够有效执行APE，改进领先的商业和专有机器翻译系统的翻译质量。

Conclusion: LangMark数据集将促进APE系统的未来发展和评估，LLMs在APE任务中展现出良好潜力。
...
TL;DR: 本文介绍了PLLuM项目中用于微调大语言模型的指令数据集，提出了有机、转换和合成指令的功能分类，并比较了人工编写与合成指令在语言适应中的影响，同时发布了PLLuMIC数据集子集。


<details>
  <summary>Details</summary>
Motivation: 为PLLuM项目开发的大语言模型提供高质量的指令数据集，并探索不同类型指令在语言模型适应中的效果差异。

Method: 构建了包含有机、转换和合成指令的功能分类系统，并基于此创建了PLLuMIC指令语料库。

Result: 提出了指令分类框架，发布了PLLuMIC数据集子集，为其他LLM开发类似数据集提供了参考。

Conclusion: 人工编写与合成指令在语言适应中具有不同影响，PLLuMIC数据集可作为开发其他语言模型指令数据集的指导模板。
...
<details>
  <summary>Details</summary>
Motivation: 现有弃权方法依赖生成后信号，无法提前防止不可靠回答。LLM经常产生流畅但事实错误的回答（幻觉现象），需要更有效的早期弃权机制。

Method: ABCA框架通过分析LLM参数知识的多方面性（如学科、法律背景、时间框架等），估计条件因果效应来评估查询相关知识的可靠性。支持两种弃权类型：知识冲突（方面效应不一致）和知识不足（方面效应一致支持弃权）。

Result: 在标准基准测试中，ABCA提高了弃权可靠性，达到最先进性能，并增强了弃权决策的可解释性。

Conclusion: ABCA通过因果推理分析内部知识多样性，为LLM提供更可靠的早期弃权机制，有效减少幻觉现象。
...
TL;DR: 提出了一种注意力引导特征融合(AGFF)模型，将统计特征和语义特征结合，通过注意力机制动态确定各特征类型的重要性，在新闻分类任务中表现优于传统统计模型和纯语义深度学习模型。


<details>
  <summary>Details</summary>
Motivation: 传统统计方法能捕捉词级重要性但缺乏上下文语义，现代深度学习方法能理解语义但可能忽略简单有效的统计指标，需要结合两者的优势。

Method: 使用注意力引导特征融合(AGFF)模型，在统一框架中结合统计特征和语义特征，通过基于注意力的机制动态确定各特征类型的相对重要性。
...
TL;DR: AutoLink是一个自主代理框架，将模式链接重新构建为迭代的代理驱动过程，在大型数据库上实现高召回率和可扩展性。


<details>
  <summary>Details</summary>
Motivation: 工业级文本到SQL应用中，由于上下文窗口限制和无关噪声，将整个数据库模式提供给大型语言模型是不切实际的。现有方法成本高昂，难以平衡召回率和噪声，且在大数据库上扩展性差。

Method: AutoLink采用自主代理框架，通过LLM引导动态探索和扩展链接模式子集，逐步识别必要模式组件，无需输入完整数据库模式。

Result: 在Bird-Dev上达到97.4%的严格模式链接召回率，在Spider-2.0-Lite上达到91.2%，执行准确率分别为68.7%和34.9%，在大规模模式（如超过3000列）上保持高召回率和高效令牌消耗。

Conclusion: AutoLink是一个高度可扩展、高召回率的模式链接解决方案，适用于工业级文本到SQL系统。
...
TL;DR: 提出E3Layer框架，通过可微分掩码优化和熵感知知识蒸馏，实现高效、经济、快速的层剪枝，在保持高性能的同时显著降低训练成本和提升推理速度。


<details>
  <summary>Details</summary>
Motivation: 现有层剪枝方法难以同时解决性能下降、训练成本高和加速有限等实际部署挑战，需要一种更有效的解决方案。

Method: 使用Gumbel-TopK采样器进行可微分掩码优化，结合熵感知自适应知识蒸馏策略。

Result: 在Qwen3-32B模型上剪枝25%层时，MATH-500准确率达96%，仅比原模型下降0.8%，推理速度提升1.33倍，仅使用0.5B tokens训练数据。

Conclusion: E3Layer框架在性能保持、训练经济性和推理效率方面均优于现有方法，为大型语言模型部署提供了有效的压缩方案。
...
TL;DR: 提出基于事件语义的对话记忆方法，将对话历史表示为短事件命题，通过异构图组织会话和基本话语单元，在长对话基准测试中表现优异且上下文更短。
...
Method: 将每个会话分解为丰富的基本话语单元（EDUs），包含参与者、时间线索和最小局部上下文，在异构图中组织会话、EDUs及其参数，支持关联检索。
...
TL;DR: PARROT框架用于测量LLM在权威和说服压力下的准确性退化，通过双盲评估、置信度追踪和八状态行为分类来量化模型对错误信息的顺从倾向。
...
Method: 使用1,302个MMLU风格多选题，在13个领域测试22个模型，通过权威模板施加压力，采用双盲评估、置信度追踪和八状态行为分类法。

Result: 先进模型（GPT-5、GPT-4.1等）顺从率低（≤11%），准确性损失小；老旧/小型模型出现严重认知崩溃（GPT-4: 80%，Qwen 2.5-1.5B: 94%）。国际法和全球知识领域脆弱，基础数学相对稳健。
...
TL;DR: 提出了MirageTVQA基准测试，评估视觉语言模型在包含视觉噪声的多语言表格问答任务中的表现，发现现有模型在视觉噪声下性能下降超过35%，且存在英语优先偏见。
...
Method: 创建包含近6万问答对的MirageTVQA基准，覆盖24种语言，表格包含模拟扫描文档的视觉噪声，用于评估领先视觉语言模型的性能。
...
TL;DR: 提出了一种混合方法预测社交媒体用户行为，针对常见和罕见行为分别建模，在Bluesky数据集上验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注常见行为（如转发、点赞），但对罕见但重要的行为预测研究不足，需要开发能同时处理频繁和罕见行为的综合方法。

Method: 结合四种互补方法：基于历史响应模式的查找数据库；针对常见行为的persona特定LightGBM模型；融合文本和时间表示的混合神经网络用于罕见行为分类；文本回复生成。

Result: 常见行为预测平均宏观F1分数0.64，罕见行为分类在10个罕见行为上达到0.56宏观F1分数，在SocialSim挑战中获得第一名。
...
TL;DR: 本文介绍了爱沙尼亚语版本的WinoGrande常识推理测试集的本地化翻译和评估，发现人工翻译质量优于机器翻译，提示工程对翻译质量提升有限，强调了语言专家在数据集本地化中的重要性。


<details>
  <summary>Details</summary>
Motivation: 为了可靠评估大型语言模型在爱沙尼亚语中的语言能力和推理表现，需要高质量的本地化基准数据集。

Method: 由翻译专家进行人工翻译和本地化适配，同时设计针对爱沙尼亚语特点和WinoGrande数据集挑战的详细提示来进行机器翻译实验。

Result: 模型在人工翻译的爱沙尼亚语数据集上表现略低于原始英语测试集，机器翻译数据表现明显更差，提示工程对翻译质量和模型准确性的提升有限。

Conclusion: 语言专家参与数据集翻译和本地化对于确保语言能力和推理评估的可靠性和可解释性至关重要。
...
TL;DR: 该研究分析了GPT-3.5、GPT-4、LlaMa 2、PaLM 2和Dolly 2等大型语言模型在南非英语、Sepedi语和Setswana语社交媒体帖子中的零样本情感分析性能，并通过模型融合实现了低于1%的情感分类错误率。


<details>
  <summary>Details</summary>
Motivation: 在多语言社区中，情感分析系统可以帮助政府部门快速识别社交媒体中的社会挑战。虽然LLMs在英语情感分析中表现出色，但尚未有研究探索其在南非语言中检测社会挑战的能力。

Method: 分析五种最先进LLMs在南非10个政府部门管辖范围内的10个新兴话题上，对英语、Sepedi语和Setswana语社交媒体帖子的零样本情感分析性能，并采用模型融合策略。

Result: 不同LLMs、话题和语言之间存在显著差异。通过融合不同LLMs的结果，情感分类性能大幅提升，错误率低于1%。
...
TL;DR: 提出了HUMA系统，一个基于LLM的多用户对话代理，能够在群聊中使用类人策略和时机参与多用户对话，在实验中参与者难以区分AI和人类社区管理者。


<details>
  <summary>Details</summary>
Motivation: 当前大多数对话系统设计为一对一的轮换交流，而非自然的异步群聊。随着AI助手在数字平台中普及，开发自然和类人的交互模式对于维持用户信任和参与度至关重要。

Method: HUMA采用事件驱动架构处理消息、回复、反应，并引入真实的响应时间模拟。系统包含三个组件：路由器、行动代理和反思模块，共同适应群聊动态。

Result: 在97名参与者的四人角色扮演聊天研究中，参与者对AI和人类社区管理者的分类准确率接近随机水平，无法可靠区分HUMA代理与人类。主观体验在两组间相似，社区管理者有效性、社交存在感和参与度/满意度差异很小。
...
TL;DR: 基于语料库研究普通话单音节词在自然对话中的音高实现，发现词义对音高轮廓有显著影响，即使控制多种变量后，词义仍是音高实现的重要预测因素。


<details>
  <summary>Details</summary>
Motivation: 探索在自然对话中，词义如何影响普通话单音节词的音高轮廓实现，挑战传统的声调理论。

Method: 使用广义加性模型分解观察到的音高轮廓，控制词长、性别、说话人身份、声调语境、元音高度和话语位置等变量，分析词义对音高的影响。

Result: 词义对音高轮廓有显著影响，词义比词本身是更好的预测因子，异形同音词有不同的音高轮廓，基于上下文嵌入可以准确预测单个词标记的音高轮廓。

Conclusion: 研究结果挑战了标准普通话声调理论，但符合判别性词汇模型的理论框架，表明分布语义学在语音学中具有重要作用。
...
<details>
  <summary>Details</summary>
Motivation: 利用多模态表示来增强自然语言推理，通过将语言在视觉上下文中进行基础化，以减轻文本偏见和表面启发式的影响。

Method: 使用文本到图像模型生成前提的视觉表示，采用余弦相似度和视觉问答两种推理技术进行比较。

Result: 该方法在无需任务特定微调的情况下实现了高准确率，并展示了对抗文本偏见和表面启发式的鲁棒性。
...
TL;DR: 提出了选择性RoPE，一种输入依赖的旋转位置嵌入机制，可在线性和softmax变换器中实现任意角度的旋转，提升了语言建模和序列任务的性能。


<details>
  <summary>Details</summary>
Motivation: 受线性变换器中选择性门控能改善语言任务的启发，希望将输入依赖的位置编码引入到softmax变换器中，以增强位置信息的表达能力。

Method: 开发了选择性RoPE，扩展了传统的RoPE，使其能够根据输入内容进行任意角度的旋转，并在门控变换器中实现该机制。

Result: 在语言建模和复制、状态跟踪、检索等困难序列任务上表现出性能提升，验证了输入依赖旋转的有效性。

Conclusion: 选择性RoPE成功地将输入依赖的位置编码引入变换器架构，揭示了softmax注意力中隐含的旋转结构，为位置编码提供了新的视角。
...
TL;DR: PUCP-Metrix是一个包含182个语言指标的开源工具库，涵盖词汇多样性、句法语义复杂性、连贯性、心理语言学和可读性，用于西班牙语文本分析。
...
Method: 开发了一个包含182个语言指标的开源存储库，支持细粒度的可解释文本分析。

Result: 在自动可读性评估和机器生成文本检测任务中表现出色，与现有存储库相比具有竞争力，且优于强大的神经基线模型。
...
<details>
  <summary>Details</summary>
Motivation: 多项选择题的选项可能泄露可被利用的信号，导致准确性指标不可靠，并鼓励在强化微调中出现显式或隐式的答案猜测行为。

Method: 提出ReVeL框架，将多项选择题分类并根据不同答案类型应用不同的改写和验证方案，将其转换为开放式问题。

Result: 使用ReVeL-OpenQA训练的模型在多项选择基准测试中匹配MCQA准确率，并将OpenQA准确率提高约6个百分点。评估时发现MCQA基准测试中存在高达20个百分点的分数膨胀。

Conclusion: ReVeL框架在数据效率和奖励信号方面优于基于MCQA的训练，同时提高了判断准确性，降低了成本和延迟。
...
<details>
  <summary>Details</summary>
Motivation: 传统评估指标如ROUGE、METEOR和EM过于依赖n-gram词汇相似性，缺乏深层语义理解；BERTScore和MoverScore等基于上下文嵌入的方法缺乏灵活性，无法平衡句子级和关键词级语义；LLM评估器存在成本高、偏见、不一致和幻觉等问题。

Method: SMILE是一个复合方法，整合了句子级语义理解、关键词级语义理解和简单关键词匹配，平衡词汇精确性和语义相关性。

Result: 在文本、图像和视频QA任务上的广泛基准测试显示，SMILE与人类判断高度相关，且计算轻量。

Conclusion: SMILE在词汇和语义评估之间架起了桥梁，提供了全面的评估方法，解决了现有指标的局限性。
...
TL;DR: 提出了MR-RLVR方法，通过掩码填充和步骤重排序构建过程级自监督奖励，提升大语言模型在仅结果可验证场景下的数学推理能力。


<details>
  <summary>Details</summary>
Motivation: 传统RLVR在数学推理中可扩展性有限，因为中间推理过程至关重要但最终答案难以直接可靠验证，而基于token的监督微调容易退化为机械记忆。

Method: 采用两阶段训练：首先在数学计算和证明数据上进行自监督训练，然后在仅结果可验证的数学计算数据集上进行RLVR微调，通过掩码填充和步骤重排序构建过程级自监督奖励。
...
<details>
  <summary>Details</summary>
Motivation: 受到复古BASIC简单性的启发，旨在为LLM推理创建一个可解释的认知控制层，使多步推理过程更加透明。

Method: 使用编号行和简单命令作为认知控制层，通过自然语言解释器文件定义命令语义、内存更新和日志行为，让现代LLM能够可靠地模拟这些简短程序。

Result: 在三个LLM上的基准测试显示，所有模型都能执行Cognitive BASIC程序，整体性能强劲但不均匀，能够提取声明性和程序性知识、检测矛盾并在必要时产生解决方案。

Conclusion: Cognitive BASIC提供了一种有效的方法来结构化LLM推理，使其过程更加透明和可解释，展示了在知识提取、冲突检测和推理任务中的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 基准测试对AI发展至关重要，但无效问题会破坏其可靠性。手动在数千个问题中识别和修正错误既不可行，也是可靠评估的关键瓶颈。

Method: 基于AI评估中常用的核心假设——平均分足以概括模型性能，构建单维潜在结构。通过统计分析响应模式，当项目的统计值超出预期范围时标记为潜在问题，再结合专家审查和LLM初步筛选。

Result: 在9个广泛使用的基准测试中，该方法引导专家审查识别问题问题的精度高达84%。LLM初步筛选进一步减少了人工工作量。

Conclusion: 该框架提供了一个高效且可扩展的系统性基准修订方法，显著提高了基准测试的可靠性。
...
TL;DR: 本文提出混合差分奖励机制解决多车协同驾驶中传统奖励函数消失问题，通过时间差分奖励和动作梯度奖励提升策略梯度信噪比，显著改善算法收敛性能。
...
TL;DR: 本文设计并评估了贝叶斯网络用户界面的语言和视觉扩展，以帮助用户更好地理解网络推理过程。研究发现这些扩展能显著提升用户对观测影响、传播路径等问题的理解，且语言和视觉结合效果最佳。


<details>
  <summary>Details</summary>
Motivation: 尽管贝叶斯网络被认为是透明模型，但用户仍难以理解其推理过程，现有用户界面未能有效澄清网络推理机制。

Method: 设计了语言和视觉扩展来增强标准贝叶斯网络用户界面，通过用户研究比较了语言扩展、视觉扩展、组合扩展和基线界面四种条件。

Result: 所有三种扩展都比基线界面表现更好，能帮助用户理解观测影响、传播路径和观测间相互影响；语言和视觉结合对某些问题类型效果最佳。

Conclusion: 语言和视觉扩展能有效提升用户对贝叶斯网络推理的理解，多模态结合是最佳实践方案。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI科学方法将科学发现视为孤立的优化过程，忽视了知识生产的社会性和历史性本质。人类科学洞察来自个体认知轨迹和集体学科记忆两个相互关联的来源，而现有LLM难以表示这些结构化的认知和社会背景。

Method: 提出MirrorMind分层认知架构：个体层面构建研究者的认知模型（情景、语义和角色记忆）；领域层面将集体知识映射为结构化学科概念图；跨学科层面作为正交编排引擎。架构将记忆存储与智能执行分离。

Result: 在四个综合任务中评估：作者级认知模拟、互补推理、跨学科协作促进、多智能体科学问题解决。结果显示MirrorMind超越了简单事实检索，实现了结构化、个性化和洞察生成的科学推理。

Conclusion: 通过整合个体认知深度与集体学科广度，MirrorMind能够实现更接近人类科学发现过程的AI科学推理，推动AI科学家从事实检索向洞察生成发展。
...
TL;DR: 本文研究如何有效扩展工具增强型智能体在明确工具调用预算下的性能，提出了预算跟踪器和BATS框架，使智能体具备预算意识并能动态调整策略。


<details>
  <summary>Details</summary>
Motivation: 现有工具增强型智能体在增加工具调用预算时性能无法提升，因为它们缺乏预算意识，导致性能很快达到瓶颈。

Method: 提出预算跟踪器提供持续预算意识，并开发BATS框架，使智能体能够根据剩余资源动态调整规划和验证策略。

Result: 预算感知方法产生更有利的扩展曲线，并推动了成本-性能的帕累托前沿。

Conclusion: 这项工作为工具增强型智能体的扩展提供了更透明和原则性的理解。
...
TL;DR: 该论文重新解释扩散模型在逆问题求解中的作用，提出DAPS++方法将扩散阶段与数据驱动优化解耦，提高计算效率和重建性能。


<details>
  <summary>Details</summary>
Motivation: 传统贝叶斯视角下基于分数的扩散方法在解决逆问题时，先验提供的指导有限，重建主要由测量一致性项驱动，与扩散动力学基本解耦。需要重新理解扩散在逆问题求解中的角色。

Method: 将扩散重新解释为期望最大化(EM)框架中的初始化阶段，提出DAPS++方法，使似然项能更直接地指导推理，同时保持数值稳定性。

Result: DAPS++需要更少的函数评估次数(NFEs)和测量优化步骤，在多种图像恢复任务中实现了高计算效率和鲁棒的重建性能。

Conclusion: 扩散在逆问题求解中应被视为初始化阶段，与数据驱动优化完全解耦，DAPS++方法验证了这种理解的有效性，并为统一扩散轨迹在实践中保持有效提供了理论解释。
...
<details>
  <summary>Details</summary>
Motivation: 电子健康记录中既有结构化信息又有非结构化文本，需要有效整合这两种数据源来训练临床决策支持系统。

Method: 使用专家指导的贝叶斯网络处理结构化EHR特征，结合神经网络文本分类器处理临床笔记，通过虚拟证据和一致性节点进行概率融合。

Result: 在SimSUM模拟数据集上验证了方法的有效性，一致性节点相比单独使用虚拟证据能改善预测校准度。

Conclusion: 该方法能够更好地处理信息缺失和解决结构化与文本数据之间的矛盾，为临床决策提供更可靠的预测。
...
TL;DR: 本文提出了一个形式化的BDI本体，作为模块化本体设计模式，用于表示智能体的信念、欲望和意图及其动态关系，并通过与LLMs和语义推理平台的集成实验验证了其适用性。
...
Method: 设计了一个模块化的BDI本体模式，与基础本体对齐，并通过两个实验验证：(1) 与大型语言模型通过逻辑增强生成结合，评估本体基础对推理一致性的贡献；(2) 集成到Semas推理平台，实现RDF三元组与智能体心理状态的双向转换。

Result: BDI本体在实验中成功作为概念和操作桥梁，连接了声明性和程序性智能，支持了认知基础、可解释和语义可互操作的多智能体及神经符号系统在数据网络中的运行。

Conclusion: 该BDI本体为构建认知基础、可解释且语义可互操作的多智能体和神经符号系统提供了有效的概念和操作框架，推动了智能体系统在数据网络环境中的发展。
...
TL;DR: 本文提出MIR（互惠内在奖励）方法，通过激励智能体探索影响队友的行为来解决多智能体强化学习中稀疏奖励问题，在MiniGrid-MA环境中表现出优越性能。


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中稀疏奖励（特别是情节奖励）面临两大挑战：联合动作轨迹的指数级稀疏性，以及现有方法未能充分考虑影响团队状态的联合动作。

Method: 提出MIR方法，激励个体智能体探索能够影响队友的行为，结合原始策略有效促进团队探索。

Result: 在扩展的MiniGrid-MA环境中，MIR方法相比现有最先进方法表现出更优越的性能。

Conclusion: MIR是一种简单有效的增强策略，能显著改善多智能体强化学习在稀疏奖励环境下的算法性能。
...
TL;DR: 提出了基于分层任务抽象机制(HTAM)的智能体设计框架，针对专业领域(如遥感)中结构化工作流程的需求，通过任务依赖图构建逻辑层次的多智能体系统，显著提升复杂规划能力。


<details>
  <summary>Details</summary>
Motivation: 通用智能体框架(如ReAct或角色扮演)在需要严格结构化工作流程的专业领域表现不佳，特别是在遥感等需要专业工具和多步骤程序的领域存在显著挑战。

Method: 引入分层任务抽象机制(HTAM)，将多智能体系统构建为反映领域内在任务依赖图的逻辑层次结构，每个层次的子智能体基于前一层输出进行操作，确保程序正确性。

Result: 在专门构建的GeoPlan-bench基准测试中，EarthAgent系统在工具选择、路径相似性和逻辑完整性等指标上显著优于现有的单智能体和多智能体系统。

Conclusion: 将智能体架构与领域内在任务结构对齐是构建稳健可靠的专业自主系统的关键步骤，分层任务抽象机制为专业领域智能体设计提供了有效解决方案。
...
TL;DR: 本文提出将AAMAS社区开发的BDI架构、通信协议、机制设计和制度建模等概念工具作为基础，将自适应数据驱动方法与结构化推理协调模型相结合，构建具有透明度、协作性和问责制的智能体系统。


<details>
  <summary>Details</summary>
Motivation: 为了实现智能体AI的持续自主性、推理和交互能力，需要补充明确的认知、协作和治理模型。

Method: 利用AAMAS社区的概念工具（BDI架构、通信协议、机制设计、制度建模），将自适应数据驱动方法与结构化推理协调模型对齐。

Result: 提出了一个连接形式理论和实践自主性的智能体视角，为构建能力强大、灵活且透明、协作、可问责的智能体系统指明了路径。

Conclusion: AAMAS社区的概念工具为智能体AI提供了必要的基础，能够实现形式理论与实践自主性的桥梁。
...
<details>
  <summary>Details</summary>
Motivation: 由于许多LLM行为的自然样本稀少，研究者不得不依赖合成或离策略的LLM响应来训练探测模型，但这种方法对探测泛化能力的影响尚不明确。

Method: 在八种不同的LLM行为上测试线性和注意力探测模型，使用多种LLM，比较不同响应生成策略对探测性能的影响。

Result: 从离策略数据到有激励产生目标行为的测试集的成功泛化可以预测在策略泛化的成功；欺骗和故意表现不佳的探测可能无法从离策略数据泛化到在策略数据；不同域测试分数始终低于同域测试分数。

Conclusion: 在没有在策略数据的情况下，使用同域离策略数据比使用不同域的在策略数据产生更可靠的探测，强调需要能更好处理LLM监控中分布偏移的方法。
...
TL;DR: 提出SRA-CP框架，通过风险感知的选择性协作感知，在保持安全关键目标检测精度的同时，将通信带宽降低至通用CP方法的20%。


<details>
  <summary>Details</summary>
Motivation: 现有协作感知方法传输大量与驾驶安全无关的数据，超出可用通信带宽，且依赖预定义的通信伙伴，不适合动态交通环境。

Method: 采用去中心化协议，车辆持续广播轻量级感知覆盖摘要，仅在检测到风险相关盲区时启动针对性协作。包含感知风险识别模块和选择性信息交换融合模块。

Result: 在公共数据集上评估，相比通用CP方法，安全关键目标的平均精度损失小于1%，通信带宽仅需20%。比现有非风险感知的选择性CP方法感知性能提升15%。

Conclusion: SRA-CP框架通过风险感知的选择性协作，有效解决了协作感知中的通信带宽和动态环境适应性问题，在保持安全性的同时显著提升了通信效率。
...
TL;DR: RubiSCoT是一个AI支持的论文评估框架，使用NLP技术（包括大语言模型、检索增强生成和结构化思维链提示）来提供一致、可扩展的论文评估解决方案。
...
Method: 使用高级自然语言处理技术，包括大语言模型、检索增强生成和结构化思维链提示，构建包含初步评估、多维评估、内容提取、基于量规的评分和详细报告的框架。
...
### [176] [PhysMorph-GS: Differentiable Shape Morphing via Joint Optimization of Physics and Rendering Objectives](https://arxiv.org/abs/2511.16988)
*Chang-Yong Song,David Hyde*

Main category: cs.GR

TL;DR: PhysMorph-GS 是一种将可微分物质点法(MPM)与3D高斯泼溅相结合的方法，通过变形感知的上采样桥接稀疏粒子状态到密集高斯分布，解决了物理模拟中的"渲染鸿沟"问题。


<details>
  <summary>Details</summary>
Motivation: 现有的物理模拟方法存在"渲染鸿沟"：不可微分的表面提取阻止了图像损失直接指导物理优化。

Method: 使用变形感知上采样桥接将稀疏粒子状态(x, F)映射到密集高斯分布(mu, Sigma)，通过多模态渲染损失反向传播，并通过MPM伴随方法更新变形控制。

Result: 在挑战性变形序列上，PhysMorph-GS比可微分MPM基线提高了边界保真度和时间稳定性，更好地重建了耳朵和尾巴等薄结构。深度监督变体将Chamfer距离减少了约2.5%。

Conclusion: 通过提供可微分粒子到高斯的桥接，PhysMorph-GS填补了物理感知渲染管道中的关键鸿沟，实现了直接从图像空间监督进行逆向设计。
