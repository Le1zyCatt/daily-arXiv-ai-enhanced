...
Method: 系统包括三个步骤：(i) 将专有360度格式标准化为标准视图并分片并行处理；(ii) 应用AI模型套件进行密集多模态预标注；(iii) 将信号整合为结构化输出规范供人工快速验证。

Result: GAZE工作流实现了效率提升（每审核小时节省约19分钟），通过保守自动跳过低显著性片段，将人工审核量减少80%以上。

Conclusion: 该方法通过增加标签密度和一致性，同时集成隐私保护和监管元数据，生成了可直接用于学习跨模态动态和动作条件预测的高保真、隐私感知数据集。
...
<details>
  <summary>Details</summary>
Motivation: PET临床使用受限，高剂量增加辐射暴露，低剂量导致泊松噪声增加，现有降噪方法无法处理泊松噪声，导致图像失真和伪影

Method: 提出Poisson Consistent U-Net (PC-UNet)模型，引入新的Poisson Variance and Mean Consistency Loss (PVMC-Loss)，整合物理数据

Result: 在PET数据集上测试显示，PC-UNet提高了物理一致性和图像保真度
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型虽然发展迅速，但仍然容易受到语义泄漏的影响，即不同实体之间意外传递语义相关特征。现有缓解策略通常基于优化或依赖外部输入。

Method: DeLeaker在扩散过程中动态重新加权注意力图，抑制过度的跨实体交互，同时增强每个实体的身份特征。

Result: 实验表明，DeLeaker在所有基线方法中表现最佳，即使基线方法提供了外部信息，也能有效缓解泄漏而不损害保真度或质量。
...
Method: UrbanVerse包含UrbanVerse-100K（10万+带语义和物理属性标注的城市3D资产库）和UrbanVerse-Gen（从视频提取场景布局并使用检索资产实例化度量尺度3D仿真的自动流水线）。

Result: UrbanVerse在IsaacSim中提供160个高质量构建场景，来自24个国家。实验显示UrbanVerse场景保持真实世界语义和布局，在人类评估的真实感方面与手工制作场景相当。在城市导航中，在UrbanVerse中训练的策略展现出缩放幂律和强泛化能力，仿真中成功率提高+6.3%，零样本仿真到真实转移中提高+30.1%。

Conclusion: UrbanVerse通过数据驱动的方法成功创建了可扩展且真实的城市仿真环境，显著提升了具身AI代理在城市导航任务中的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前3D物体编辑方法效率低下、不一致且难以保持未编辑区域，大多数方法依赖多视角渲染编辑后重建，会引入伪影并限制实用性。

Method: 将FlowEdit集成到TRELLIS中，通过前视图渲染指导局部编辑，引入区域感知合并策略Voxel/Slat-Merge，自适应保持结构保真度。

Result: 实验表明Nano3D相比现有方法实现了更优的3D一致性和视觉质量，并构建了包含10万+高质量3D编辑对的大规模数据集。
...
<details>
  <summary>Details</summary>
Motivation: 现有图像生成基准测试往往滞后于实际发展，无法捕捉新兴用例，导致社区认知与正式评估之间存在差距。

Method: 从社交媒体帖子中收集用户展示的新颖提示和定性判断，构建包含31,000多个提示的数据集，用于评估模型能力。

Result: ECHO发现了现有基准测试中缺失的创意复杂任务，能更清晰地区分最先进模型，并基于社区反馈设计模型质量评估指标。

Conclusion: ECHO框架通过真实世界证据构建基准测试，能更好地反映图像生成模型的实际能力和用户需求。
...
<details>
  <summary>Details</summary>
Motivation: 由于Civit.ai等平台上存在超过10万个LoRA适配器，用户面临模型数量庞大、多样性高且缺乏结构化组织的问题，难以有效选择和利用最适合的适配器。

Method: 将LoRA模型选择任务构建为组合优化问题，并提出新颖的子模块框架来解决这一挑战。

Result: 定量和定性实验表明，该方法能够在广泛领域内生成多样化的输出结果。

Conclusion: 所提出的子模块框架有效解决了大规模LoRA适配器数据库中的模型选择问题，能够为用户提供相关且多样化的模型推荐。
...
<details>
  <summary>Details</summary>
Motivation: 现有基础模型在实例级感知方面取得了突破，但高计算成本限制了在资源受限平台上的应用。需要设计能够在保持性能的同时实现高效边缘部署的架构。

Method: 1. 瓶颈像素解码器用于高效多尺度和多模态融合
2. 语言引导的不确定性校准损失用于自适应解码器剪枝
3. 简化的统一训练策略

Result: MOBIUS在仅需三分之一训练迭代的情况下，减少像素FLOPs达55%，变换器解码器FLOPs达75%，同时保持最先进的性能水平。

Conclusion: MOBIUS为高性能计算平台和移动设备上的高效分割建立了新的基准，实现了帕累托最优的模型缩放，解决了准确性与复杂性之间的权衡问题。
...
<details>
  <summary>Details</summary>
Motivation: 多模态大语言模型在缺乏大规模人工标注推理数据的领域（如图表、渲染文档等）推理能力有限，而这些领域在实践中很常见。

Method: 将种子问题分解为原始感知和推理因子，然后系统性地与新的图像重新组合生成大量合成问答对，每个问题都配有子问题和中间答案，支持因子级过程奖励的强化学习。

Result: 在图表推理任务上，COGS显著提升了未见问题的性能，特别是在推理密集和组合性问题上的改进最大。使用不同种子数据的因子级混合训练能更好地跨数据集迁移。

Conclusion: COGS框架能有效提升MLLMs在缺乏标注数据领域的推理能力，且具有很好的泛化性，可扩展到网页等其他领域。
...
<details>
  <summary>Details</summary>
Motivation: 数字孪生世界为开发通用具身智能体提供了新机会，但需要统一处理复杂物理行为的方法。

Method: 从势能最小化原则出发，引入方向刚度扩展经典弹性动力学，使用专门网络建模材料属性，用神经场表示变形。

Result: 实验表明GDGen能稳健统一多种仿真范式，为创建交互式虚拟环境和训练机器人提供通用基础。

Conclusion: GDGen提供了一个统一框架来处理多样化物理系统，支持复杂动态场景中的交互式虚拟环境和机器人训练。
...
<details>
  <summary>Details</summary>
Motivation: 3D医学影像领域的数据稀缺限制了当前视觉语言编码器的能力，需要缓解数据不足的问题。

Method: 引入额外的归纳偏置：报告生成目标，并将视觉-语言预训练与纯视觉预训练结合，利用图像-文本配对数据和纯图像数据。

Result: COLIPRI编码器在报告生成、分类探测和零样本分类任务上达到最先进性能，在语义分割任务上保持竞争力。

Conclusion: 通过引入归纳偏置和结合不同预训练方式，能够有效缓解数据不足问题，提升3D医学影像视觉语言编码器的性能。
...
Result: 在MathVista和MathVerse等基准测试中，DRIFT相比简单合并和监督微调持续提升推理性能，以较低成本达到或超过训练密集型方法。

Conclusion: DRIFT提供了一种轻量级且有效的方法来增强多模态模型的推理能力，平衡了性能与训练成本。
...
TL;DR: 婴儿日常视觉体验的统计特征（相似图像簇与罕见变量图像交替出现）支持早期类别学习和泛化，这种"块状"结构也能提升机器学习中的小数据集泛化能力。


<details>
  <summary>Details</summary>
Motivation: 探索婴儿如何从有限经验中实现强大的视觉对象识别和泛化能力，解决婴儿学习能力与经验有限性之间的矛盾。

Method: 分析14名7-11个月婴儿的自我中心视角图像，计算视觉输入的相似性结构，并通过计算实验验证这种结构对机器学习泛化的影响。

Result: 婴儿日常视觉输入呈现"块状"相似结构，包含高度相似图像簇和罕见变量图像。这种结构能显著提升机器学习模型在小数据集上的泛化性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DGS方法在长时视频序列中预测逐像素高斯并组合所有视图的高斯作为场景表示，导致大量冗余和几何不一致性。

Method: 使用3D重建骨干网络预测密集逐像素高斯和几何复杂度显著性图，通过可微分显著性感知高斯量化将冗余高斯压缩为紧凑锚点，然后利用3D点Transformer学习空间结构先验来精炼锚点属性和显著性。

Result: 在多个数据集上实现了最先进的性能，在50%到90%的冗余消除下，能够重建超过50个视图并以超过10FPS运行，在新型视图合成和深度估计方面表现优异。
...
<details>
  <summary>Details</summary>
Motivation: 现有文本到视频生成方法在复杂场景和多物体设置下控制能力有限，缺乏精确的轨迹-实体对应关系。

Method: 使用文本引导轨迹作为条件，提出位置感知交叉注意力机制和双CFG方案，构建了包含200万高质量视频剪辑的数据集。

Result: 实验表明TGT在视觉质量、文本对齐精度和运动可控性方面优于现有方法。

Conclusion: TGT框架通过轨迹-文本配对实现了对视频生成中多个物体的精确外观和运动控制。
...
<details>
  <summary>Details</summary>
Motivation: 结合数据驱动的扩散模型与医学成像领域知识，解决传统贝叶斯逆问题中经典数学先验无法捕捉脑解剖复杂结构的问题。

Method: 使用在多样化脑MRI数据上训练的基于分数的扩散先验，配合灵活的前向模型，处理超分辨率、偏置场校正、修复等图像处理任务。
...
<details>
  <summary>Details</summary>
Motivation: 现有的WSI分类方法难以捕获全局依赖关系，因为WSI尺寸巨大且补丁嵌入具有局部性，这限制了粗粒度结构的建模能力。

Method: 使用快速傅里叶变换从WSI中提取低频区域，通过FFT-Block（包含卷积层和Min-Max归一化）处理频率数据，并将全局频率特征与空间补丁特征融合。
...
Method: 构建了包含60,828个多选题的大规模三模态基准XModBench，涵盖五种任务类型和六种模态组合，能够精细诊断模型的模态不变推理能力、模态差异和方向不平衡。

Result: 实验显示即使是表现最好的Gemini 2.5 Pro模型也存在：(i)时空推理能力不足，准确率低于60%；(ii)持续模态差异，音频模态表现显著低于文本；(iii)系统性方向不平衡，视觉作为上下文时一致性低于文本。
...
Result: 使用UniFilter筛选的数据预训练的MLLMs在零样本推理和上下文学习方面表现显著提升，经过视觉监督微调后在多个基准测试中取得更好性能。

Conclusion: UniFilter方法有效提升了多模态预训练数据的质量，为社区提供了高质量的多模态数据资源，推动了多模态大语言模型的进一步发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决组织病理学基础模型训练中的可重复性挑战，该挑战主要由软件随机性、硬件非确定性和超参数报告不一致等因素导致。

Method: 在QUILT-1M数据集上训练CLIP模型，系统评估不同超参数设置和数据增强策略在三个下游组织病理学数据集上的影响。

Result: 发现RandomResizedCrop值0.7-0.8表现最佳，分布式训练无局部损失提高稳定性，学习率低于5.0e-5会降低性能，LC25000（结肠）数据集提供最可重复的基准。

Conclusion: 计算病理学的可重复性不仅依赖透明文档，还需要精心选择的实验配置，研究提供了开发可重复数字病理基础模型的实用指南。
...
Method: 使用显著概念感知的图像嵌入模型来减少合成过程中无关视觉细节的影响，保持图像和文本输入之间的直观对齐，通过生成更好地保留类别判别特征并具有额外受控变化的图像来增强训练数据多样性。

Result: 在八个细粒度视觉数据集上表现出优越性能，在常规和长尾设置下分别以0.73%和6.5%的平均分类准确率提升优于最先进的增强方法。

Conclusion: 该框架通过减少无关视觉细节的影响，有效提升了生成数据的多样性，从而增强了下游模型的鲁棒性，在多种视觉任务中取得了显著改进。
...
<details>
  <summary>Details</summary>
Motivation: 先天性心脏病产前诊断面临数据稀缺、质量差、多源信息整合困难等问题，限制了AI模型的临床应用

Method: 构建首个公开的多模态数据集CARDIUM，包含胎儿超声、超声心动图和母体临床记录；提出基于交叉注意力机制的多模态transformer架构，融合图像和表格数据特征

Result: 在CARDIUM数据集上，多模态方法比单模态图像和表格方法分别提升11%和50%，F1分数达到79.8±4.8%

Conclusion: 多模态数据融合能显著提升产前先天性心脏病检测性能，公开数据集和代码将促进该领域进一步研究
...
<details>
  <summary>Details</summary>
Motivation: 文本到图像模型在定制视觉广告和针对特定人群方面具有潜力，但需要研究其中可能存在的人口统计偏见和说服力差异问题。

Method: 通过分析不同广告主题中的人口统计偏见，评估除人物性别/种族外完全相同的广告的说服力差异，并实验针对特定国家的广告定向技术。

Result: 研究发现广告中存在人口统计偏见，相同广告内容但不同性别/种族的人物形象具有不同的说服力水平。

Conclusion: 文本到图像模型在广告定制中具有应用潜力，但需要注意人口统计偏见问题，并可通过技术手段实现更好的广告定向。
...
<details>
  <summary>Details</summary>
Motivation: 现有的驾驶场景合成方法要么计算成本过高，要么只关注长时间视频合成而缺乏3D表示，或者仅限于静态单场景重建。本研究旨在填补这一方法学空白。

Method: 采用统一流水线，包含两个专门组件：FastDrive-DiT（高效的视频扩散变换器，用于在文本和鸟瞰图布局指导下合成高分辨率、时间一致的视频）和FastRecon3D（前馈重建模块，快速构建跨时间的3D高斯表示）。

Result: 实现了实时生成长时间驾驶视频（最高424×800分辨率，12 FPS）和相应的动态3D场景，在新视角合成上达到SSIM 0.811和PSNR 22.84，同时保持参数效率。

Conclusion: DriveGen3D成功整合了加速的长期视频生成与大规模动态场景重建，通过多模态条件控制实现了高质量、高效的动态3D驾驶场景生成。
...
Method: 采用CUDA加速的GPU并行化技术，使用计算密集型高精度特征提取器，生成全面且非冗余的数据关联，支持姿态优化、建图、先验地图定位和外参优化。

Result: 相比广泛使用的COLMAP方法，cuSfM在各种测试场景下实现了显著提升的精度和处理速度，同时保持了离线SfM应用所需的高精度和全局一致性。

Conclusion: cuSfM系统作为开源Python包装实现PyCuSfM发布，可促进计算机视觉和机器人领域的研究与应用。
...
<details>
  <summary>Details</summary>
Motivation: 现有MRI分析工具主要针对健康解剖结构优化，在处理大病灶（如肿瘤）时经常失效。图像修复技术旨在合成肿瘤区域的健康脑组织，使通用工具能够可靠应用。

Method: 结合模型集成与高效后处理策略（中值滤波、直方图匹配、像素平均），并通过轻量级U-Net增强阶段进行解剖细化。

Result: 综合评估表明，所提出的流水线提高了修复区域的解剖合理性和视觉保真度，比单个基线模型具有更高的准确性和鲁棒性。
...
<details>
  <summary>Details</summary>
Motivation: 当前人脸识别系统在可识别性表示方面存在两个主要问题：软边界约束只能部分捕捉可识别性，导致低质量人脸识别性能下降；特征方向与幅度的梯度重叠造成优化不稳定和表示纠缠。

Method: 引入硬边界策略QCFace，通过指导因子进行超球面规划，同时优化识别能力和明确的可识别性表示，解决梯度重叠问题。

Result: 实验表明QCFace不仅提供稳健可量化的可识别性编码，还在验证和识别基准测试中达到最先进性能。
...
Result: 在四个基准数据集上的实验显示具有竞争力的性能，且比现有方法具有更好的可解释性；统计分析表明学习到的嵌入与实际共现模式高度相关。

Conclusion: 双曲几何为不完全监督下的结构化分类提供了更鲁棒的范式，能够有效捕捉复杂的标签关系和层次结构。
...
<details>
  <summary>Details</summary>
Motivation: 传统VAE+扩散范式存在训练效率低、推理速度慢、任务迁移性差的问题，根源在于VAE潜在空间缺乏清晰的语义分离和强判别结构。

Method: SVG利用冻结的DINO特征构建具有清晰语义判别性的特征空间，同时通过轻量级残差分支捕捉细粒度细节，直接在语义结构化的潜在空间上训练扩散模型。

Result: SVG实现了加速扩散训练、支持少步采样、提升生成质量，并保留了底层自监督表示的语义和判别能力。

Conclusion: SVG为任务通用、高质量的视觉表示提供了一条有原则的路径。
...
<details>
  <summary>Details</summary>
Motivation: 现有结构化剪枝方法存在性能显著下降、线性权重层聚合能力不足以及缺乏有效后训练恢复机制等问题，需要新的剪枝范式来解决这些局限性。

Method: 提出通道敏感度指标进行细粒度通道选择，采用基于拼接的层合并方法融合相邻层的关键通道，并使用分层蒸馏协议实现高效知识迁移。

Result: 在七个基准测试中，CoMe达到最先进性能，当剪枝LLaMA-2-7b模型30%参数时，剪枝后模型仍保留83%的原始平均准确率。

Conclusion: CoMe框架通过渐进式剪枝和分层蒸馏，有效解决了传统结构化剪枝方法的局限性，在显著减少模型大小的同时保持了良好的性能。
...
Method: 开发了Spatial457数据集，包含4个关键空间推理能力：多目标识别、2D位置、3D位置和3D方向，采用级联评估结构构建了7种问题类型和5个难度级别。

Result: 评估发现大型多模态模型随着任务复杂度增加性能普遍下降，特别是在3D推理和6D空间任务上，并揭示了不同属性间的预测偏差。
...
Result: 在广泛使用的基准数据集上的大量实验表明，Proto-Former相比现有最先进方法取得了更优的性能。
...
<details>
  <summary>Details</summary>
Motivation: 当前的人体运动重建方法在准确将人体放置在3D空间中存在困难，而逼真的角色与环境交互对于游戏、AR/VR和机器人等领域的自主代理至关重要。

Method: SHARE首先估计每帧的人体网格和分割掩码，以及关键帧的场景点云图。通过迭代比较人体网格与从场景中提取的人体点云图来优化关键帧中的人体位置，并在优化过程中保持非关键帧人体网格相对于关键帧根关节的相对位置一致性。

Result: 广泛实验表明，SHARE在准确重建人体3D位置和周围场景方面优于现有方法，适用于精心策划的数据集和野外网络视频。

Conclusion: SHARE技术能够更准确地实现3D人体定位，同时重建周围场景，为各种应用场景提供了有效的解决方案。
...
Method: 扩展深度状态空间模型，构建Cortical-SSM架构，在时域、空域和频域三个维度捕获EEG/ECoG信号的综合依赖关系。

Result: 在三个基准测试中（两个大规模公共MI EEG数据集和一个临床MI ECoG数据集）均优于基线方法，并能有效捕获神经生理相关区域。

Conclusion: Cortical-SSM模型能有效处理EEG/ECoG信号分类问题，在捕获神经生理相关特征方面表现出色。
...
<details>
  <summary>Details</summary>
Motivation: 微创手术中的自动化工具检测对手术分析和辅助具有重要意义，但手术场景中标注数据有限，限制了深度学习模型的训练效果。

Method: 采用两阶段自适应微调：1) 线性探测阶段在预训练CNN架构上调节分类层；2) 渐进冻结阶段动态减少可微调层数，控制对手术领域的适应程度。

Result: 在Cholec80数据集上验证，使用ResNet-50和DenseNet-121预训练模型，相比现有方法和标准微调技术性能更优，mAP达到96.4%。在CATARACTS眼科手术数据集上进一步验证了泛化能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖深度预测来恢复空间信息，需要LiDAR点云监督，但预测的深度质量不佳，存在物体边界深度不连续和小物体难以区分的问题，且忽略了跨视图一致性和尺度不变性。

Method: 使用频率感知空间金字塔编码器结合高频边缘线索和低频语义，跨视图尺度不变深度预测器估计像素级深度分布，位置深度编码器融合2D特征和3D位置嵌入生成深度感知特征，并采用混合深度监督。

Result: 在nuScenes数据集上的大量实验证明了该方法的有效性和优越性。

Conclusion: FreqPDE方法能够在不依赖LiDAR监督的情况下，通过频率感知的深度嵌入有效提升多视图3D目标检测性能。
...
Method: PFGS采用姿态感知融合策略，结合全局和局部配准，将多个辅助姿态的图像迭代融合到主姿态的统一3DGS表示中。利用背景特征进行单姿态相机姿态估计，使用基础模型进行跨姿态配准。
...
Method: 基于高性能离线框架，通过潜在空间流式架构、滑动窗口因果设计和解码运动特征注入来确保平滑运动过渡，无需依赖未来帧或修改扩散模型架构。

Result: 在基准数据集上的实验表明，该方法在风格化质量和响应性之间达到了良好平衡，能够实现长序列实时任意风格化。

Conclusion: LILAC成功将离线高质量运动风格化框架扩展到在线设置，为需要连续响应角色控制的应用提供了有效的实时解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有水下实例分割方法受限于封闭词汇预测，无法识别新的海洋类别，且从自然图像迁移到水下场景时存在严重视觉退化和语义不对齐问题。

Method: 提出统一框架，包含几何先验增强模块(GPEM)利用部件级和结构线索保持对象一致性，以及语义对齐注入机制(SAIM)用领域特定先验丰富语言嵌入。

Result: 实验表明该框架在MARIS基准上持续优于现有开放词汇基线方法，在域内和跨域设置中均表现优异。

Conclusion: 该工作为未来水下感知研究建立了坚实基础，解决了水下开放词汇分割的关键挑战。
...
<details>
  <summary>Details</summary>
Motivation: 临床采用多激发DWI进行全身肿瘤诊断受到呼吸、蠕动等运动引起的严重相位伪影限制，且面临多器官、多切片、多方向、多b值的复杂性挑战。

Method: 将帧间相位变化建模为高阶局部平滑相位，集成到低秩Hankel矩阵重建中，通过仅使用模拟生理运动的合成腹部DWI数据进行提示学习来自动设置算法秩参数。

Result: 在10,000+临床图像验证中：实现临床单激发DWI两倍空间分辨率；单一模型泛化到7个解剖区域；在图像质量、伪影抑制和噪声降低方面优于现有方法，获得放射科医生4-5分评价。
...
Method: LoD框架包括多模态安全概念激活向量模块（用于安全导向的表示学习）和安全模式自动编码器模块（用于无监督攻击分类）。

Result: 大量实验表明，该方法在多样未知攻击上实现了持续更高的检测AUROC，同时提高了效率。

Conclusion: LoD框架通过任务特定学习有效解决了现有越狱攻击检测方法的局限性，为未知攻击检测提供了通用解决方案。
...
Method: 使用零样本语义分割从街景图像中提取11个街道景观指标，结合道路类型分析约30,000起事故记录。训练XGBoost多类分类器，使用SHAP解释特征贡献，应用GPS加权和ATE估计来控制混杂因素并量化因果效应。
...
<details>
  <summary>Details</summary>
Motivation: 端到端深度学习方法存在收敛慢、依赖大规模数据集的问题，限制了在医学影像等数据稀缺领域的应用效率。

Method: PC范式将任务解耦为两个协同模块：预测先验模块（PPM）利用双侧对称性知识生成粗略近似，校正残差网络（CRN）学习残差误差，专注于精炼关键区域。

Result: 在高分辨率脑MRI分割实验中，PCMambaNet达到最先进精度，且仅需1-5个epoch收敛，远超传统端到端模型。

Conclusion: 通过显式融入领域知识简化学习目标，PCMambaNet有效缓解了数据低效和过拟合问题，实现了显著加速。
...
TL;DR: MAVR-Net是一个基于多视角学习的微型飞行器动作识别框架，通过结合RGB帧、光流和分割掩码三种互补数据，显著提高了MAV运动识别的准确性和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统基于RGB数据的视觉识别模型难以捕捉微型飞行器复杂的时空运动特征，限制了不同动作的区分能力。
...
Conclusion: 多视角学习方法能有效提升微型飞行器动作识别的性能，通过融合多种数据模态和跨视角交互机制，可以更好地捕捉复杂的运动模式。
...
TL;DR: DPTrack是一种基于提示学习的夜间航空跟踪器，通过将目标属性特征编码到富含细粒度线索的方向核中，生成精确的提示来解决现有方法性能不佳的问题。


<details>
  <summary>Details</summary>
Motivation: 现有基于提示学习的夜间航空跟踪器仅依赖空间定位监督，无法提供指向目标特征的细粒度线索，导致提示模糊，跟踪性能差。

Method: 受视觉仿生学启发，DPTrack首先分层捕获目标的拓扑结构，利用拓扑属性丰富特征表示；然后编码器将这些拓扑感知特征压缩到方向核中；最后基于通道-类别对应关系的核引导提示模块传播核以精确定位目标特征并转换为精确提示。
...
TL;DR: 提出一种基于动态图像的双阶段时间增强方法，将微表情序列分解为起始-顶点和顶点-结束两个运动阶段，分别生成动态图像，以丰富运动多样性和补充时间线索。


<details>
  <summary>Details</summary>
Motivation: 微表情识别受限于标注数据稀缺，现有方法主要依赖简单空间增强而忽视时间增强策略，无法充分利用运动特征。
...
TL;DR: MRASfM是一个专为驾驶场景设计的多相机运动恢复结构框架，通过利用多相机系统的固定空间关系提高姿态估计可靠性，使用平面模型改善路面重建质量，并将多相机作为单一单元进行优化提升效率。


<details>
  <summary>Details</summary>
Motivation: 传统SfM在驾驶场景的多相机系统中存在姿态估计不可靠、路面重建异常点多、重建效率低等问题，需要专门针对驾驶场景的解决方案。

Method: 利用多相机系统固定空间关系增强相机姿态估计；采用平面模型剔除路面三角化中的错误点；将多相机作为单一单元进行Bundle Adjustment减少优化变量；通过场景关联和组装模块实现多场景聚合。

Result: 在真实车辆部署验证了MRASfM的泛化性和鲁棒性，在nuScenes数据集上达到0.124的绝对姿态误差，表现出最先进的性能。

Conclusion: MRASfM框架有效解决了驾驶场景中多相机SfM的关键挑战，在姿态估计精度和重建质量方面均取得显著提升。
...
TL;DR: 本文首次系统提出并研究了无人机视频-文本检索任务，针对无人机视频的俯视视角、结构同质性和目标组合多样性等特点，提出了多语义自适应挖掘方法，通过动态帧间变化和场景区域语义提取来增强对无人机视频内容的理解。


<details>
  <summary>Details</summary>
Motivation: 随着无人机技术的发展，视频数据量急剧增加，迫切需要高效的语义检索方法。现有跨模态方法主要针对地面视角设计，难以有效建模无人机视频的俯视视角、结构同质性和目标组合多样性等特点。

Method: 提出多语义自适应挖掘方法，包含多语义自适应学习机制，通过动态帧间变化和特定场景区域语义提取来增强内容理解。该方法基于词与无人机视频帧的细粒度交互，整合自适应语义构建模块、分布驱动语义学习项和多样性语义项，并引入跨模态交互特征融合池化机制以减少复杂背景干扰。

Result: 在两个自建的无人机视频-文本数据集上的大量实验表明，MSAM在无人机视频-文本检索任务中优于其他现有方法。

Conclusion: 针对无人机视频特点设计的专用检索机制是必要的，MSAM方法通过多语义自适应挖掘有效提升了无人机视频-文本检索性能，证明了该方法在无人机场景中的优越性。
...
TL;DR: 该研究提出了一种结合光学流(COF)方法，通过整合面部微表情的起始-顶点和顶点-结束两个阶段来改进微表情识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有的微表情识别方法通常只关注起始到顶点阶段，而忽略了顶点到结束阶段的关键时间动态信息，这限制了识别性能。

Method: 提出结合光学流(COF)方法，整合微表情的起始-顶点和顶点-结束两个阶段的光学流信息，提供更全面的运动分析。

Result: 在CASMEII和SAMM数据集上的实验结果表明，COF方法优于基于单一光学流的方法，能更有效地捕捉微表情动态特征。

Conclusion: 结合光学流方法通过整合微表情的两个关键阶段，显著提升了微表情识别的性能，证明了全面考虑时间动态的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态学习中数据效率的挑战，当前VLM需要大量训练数据，而Shakti旨在通过模型设计而非数据量来获得高性能

Method: 采用QK-Normalization提高注意力稳定性、混合归一化技术、增强位置编码，以及三阶段训练策略优化学习效率

Result: Shakti-VLM-1B和4B在文档理解、视觉推理、OCR提取和通用多模态推理方面表现优异

Conclusion: 通过模型设计和训练策略而非单纯增加数据量，可以实现高性能的多模态学习，使Shakti成为企业级多模态任务的高效解决方案
...
Method: 使用小型商用无人机自主采集图像，通过迭代方法逐步调整输入图像变形，利用光流估计运动，与中间3D重建进行对齐，减少场景运动

Result: 管道改进了现有最先进方法的3D重建质量，能够提取高分辨率3D网格，提供了包含多种作物在不同时间点采集的数据集

Conclusion: 该管道能够生成高质量的植物3D重建，支持任意最先进的3D重建方法，代码和数据集将公开
...
Conclusion: HiMA架构通过有效结合不同模块的优势，在低光RAW图像增强任务中取得了显著效果，平衡了性能与效率。
...
<details>
  <summary>Details</summary>
Motivation: 预训练视觉表示在模仿学习中通常保持冻结状态，导致任务无关性。本文探索如何利用文本到图像扩散模型获得任务自适应的视觉表示，而不需要微调模型本身。

Method: 提出ORCA方法，引入可学习的任务提示来适应控制环境，以及视觉提示来捕捉细粒度的帧特定细节，通过新设计的条件促进任务自适应表示。

Result: 该方法在各种机器人控制基准测试中实现了最先进的性能，显著超越了先前的方法。

Conclusion: 通过考虑控制任务所需的特定动态视觉信息，利用扩散模型的条件机制可以有效地获得任务自适应的视觉表示，从而提升机器人控制性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决卫星图像分类中的特定失败模式，探索无需预训练模型的领域特定架构设计，实现高性能的土地利用分类。
...
Result: 最终12层架构达到97.23%测试准确率，Cohen's Kappa为0.9692，所有类别准确率超过94.46%。可学习融合参数自主收敛到约0.57，表明空间和光谱模态重要性相近。

Conclusion: 系统化架构设计在领域特定应用中有效，性能接近微调ResNet-50（仅差1.34%），且无需外部数据，验证了自定义架构的效能。
...
<details>
  <summary>Details</summary>
Motivation: FDG-PET在痴呆症诊断中很重要，但相比常规MRI更昂贵且不易获取。需要一种方法让更多患者受益于PET的诊断优势。

Method: 使用3D扩散桥框架学习从MRI和患者信息到FDG-PET图像的概率映射，生成模拟PET图像。

Result: 在盲法临床研究中，SiM2P将三组（阿尔茨海默病、行为变异型额颞叶痴呆、健康对照）的总体诊断准确率从75.0%提高到84.7%，并获得更高的诊断确定性和评估者间一致性。

Conclusion: SiM2P框架使FDG-PET的诊断优势更易获得，只需20个站点特定病例和基本人口统计信息即可部署，有望改善资源有限环境中的早期检测和鉴别诊断。
...
Method: 从127个二战时期档案视频片段中提取包含拍板的生产元数据，提供全帧标注和裁剪单词图像，支持旋转边界框的4点多边形表示。
...
### [60] [OmniVinci: Enhancing Architecture and Data for Omni-Modal Understanding LLM](https://arxiv.org/abs/2510.15870)
*Hanrong Ye,Chao-Han Huck Yang,Arushi Goel,Wei Huang,Ligeng Zhu,Yuanhang Su,Sean Lin,An-Chieh Cheng,Zhen Wan,Jinchuan Tian,Yuming Lou,Dong Yang,Zhijian Liu,Yukang Chen,Ambrish Dantrey,Ehsan Jahangiri,Sreyan Ghosh,Daguang Xu,Ehsan Hosseini-Asl,Danial Mohseni Taheri,Vidya Murali,Sifei Liu,Jason Lu,Oluwatobi Olabiyi,Frank Wang,Rafael Valle,Bryan Catanzaro,Andrew Tao,Song Han,Jan Kautz,Hongxu Yin,Pavlo Molchanov*

Main category: cs.CV

TL;DR: OmniVinci是一个开源的多模态大语言模型，通过创新的架构设计和数据管理，在视觉和音频模态理解上表现出色，使用更少的训练数据就能超越现有模型。


<details>
  <summary>Details</summary>
Motivation: 推动机器智能需要发展跨模态感知能力，模仿人类多感官感知世界的方式。

Method: 提出三个关键创新：OmniAlignNet增强视觉和音频嵌入的对齐；Temporal Embedding Grouping捕捉相对时间对齐；Constrained Rotary Time Embedding编码绝对时间信息。建立了包含2400万单模态和多模态对话的数据管道。

Result: OmniVinci在DailyOmni上比Qwen2.5-Omni提升19.05分，在MMAR上提升1.7分，在Video-MME上提升3.9分，仅使用0.2T训练token（比Qwen2.5-Omni的1.2T减少6倍）。
...
TL;DR: 提出了一种多视角架构来增强DeepFake检测，通过全局、中观和局部三个视角分析面部特征，并整合人脸朝向编码器，在各种姿态和光照条件下都能有效检测伪造图像。


<details>
  <summary>Details</summary>
Motivation: 现有的DeepFake检测方法在真实世界条件下难以应对姿态变化、遮挡和难以检测的伪影，需要更鲁棒的检测方案。

Method: 使用三个专用编码器：全局视角编码器检测边界不一致性，中观视角编码器分析纹理和颜色对齐，局部视角编码器捕捉眼睛、鼻子、嘴巴等表情区域中的失真。还整合了人脸朝向编码器来分类人脸姿态。

Result: 在具有挑战性的数据集上的实验结果表明，该方法优于传统的单视角方法，在各种姿态和光照条件下都能实现卓越的检测性能。

Conclusion: 多视角架构通过融合不同层次的面部特征分析，显著提高了DeepFake检测的鲁棒性和准确性，特别是在复杂真实世界条件下。
...
TL;DR: 提出轻量级CycleGAN用于荧光显微镜模态转换，通过固定通道策略大幅减少参数至约9000个，同时提升性能并降低训练成本。还将GAN作为诊断工具检测图像质量问题。


<details>
  <summary>Details</summary>
Motivation: 解决荧光显微镜中未配对数据集的模态转换问题，同时降低深度学习模型的计算成本和环境影响。

Method: 使用轻量级CycleGAN，在U-Net生成器中用固定通道方法替代传统的通道倍增策略，大幅减少可训练参数。

Result: 参数从4180万减少到约9000个，性能更优，训练更快，内存使用更低。GAN可作为诊断工具识别图像质量问题。

Conclusion: 该方法不仅实现了高效的模态转换，还建立了GAN作为验证显微镜实验准确性和图像保真度的实用工具。
...
Method: 第一种方法基于传统上采样技术；第二种方法是ABSIS锐化方法，将高分辨率图像的整体特征与特定低分辨率图像的独特属性融合。

Result: 两种方法都显著提高了USTFIP STIF方法的准确性，其中锐化方法将融合图像的光谱和空间精度分别提高了49.46%和78.40%。

Conclusion: 提出的标准化方法，特别是ABSIS锐化方法，能够有效提高时空图像融合的精度和性能。
...
Method: 提出自适应专家混合(MoE)机制动态整合不同模态特征，引入跨模态查询融合模块增强多模态特征提取，构建统一的CIRS-PEDES数据集进行综合评估。
...
Method: 基于量化特征对应分析，将补丁统计比较适配到量化值直方图，并引入基于主成分分析的特征预处理步骤

Result: 在保持精度几乎不变的情况下实现10倍加速，在复杂纹理上提高了检测精度

Conclusion: QFCA方法在实时性和准确性方面均优于现有方法，适用于实际部署
...
TL;DR: 提出了一种名为Noise2Detail的超轻量级自监督去噪模型，通过多阶段去噪管道在保持高质量图像恢复的同时实现快速推理，解决了现有方法计算和内存需求大的问题。


<details>
  <summary>Details</summary>
Motivation: 当前自监督去噪方法虽然效果显著，但计算和内存需求大，需要在推理速度与重建质量之间权衡，限制了实际应用。特别是在生物医学成像中，干净训练数据稀缺且成像模式复杂。

Method: 基于Noise2Noise训练框架，引入创新的多阶段去噪管道Noise2Detail。在推理过程中破坏噪声模式的空间相关性产生中间平滑结构，然后从噪声输入中直接恢复精细细节。

Result: 广泛测试表明，Noise2Detail在性能上超越了现有的无数据集技术，同时仅需一小部分计算资源。

Conclusion: 该模型结合了高效性、低计算成本和无需数据的方法，成为生物医学成像的有价值工具，克服了干净训练数据稀缺的挑战，同时实现快速推理以支持实际应用。
...
TL;DR: 这篇论文是关于遥感领域深度学习方法在领域自适应方面的综述，系统梳理了该领域的关键概念、方法分类、数据集和最新进展，并指出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 遥感领域自适应在现实应用中具有广泛需求，但由于数据差异（如采样距离、传感器模式、地理景观等）带来显著挑战，深度学习为此提供了强大的特征表示和跨域知识迁移能力。

Method: 采用系统性综述方法，从任务分类、输入模式、监督范式和算法粒度等多个角度组织现有算法，并回顾常用数据集和最新方法性能。

Result: 提供了遥感领域自适应的全面概览，建立了系统化的分类体系，总结了当前最先进方法的性能表现。

Conclusion: 与以往综述相比，本文覆盖了更广泛的遥感领域自适应任务，提供了更全面和结构化的理解，能够启发研究社区并指导未来工作。
...
TL;DR: 提出一种弱监督医学图像分割框架，仅需四个极值点标注，通过SAM2生成初始伪标签，结合改进的FGEPM算法和不确定性估计进行边界追踪，在超声图像分割上达到与全监督方法相当甚至更好的性能。


<details>
  <summary>Details</summary>
Motivation: 全监督医学图像分割需要大量像素级标注，成本高昂且耗时。为减轻标注负担，需要开发仅需少量标注的弱监督方法。

Method: 使用极值点生成边界框作为SAM2提示生成初始伪标签；改进FGEPM算法结合蒙特卡洛dropout不确定性估计构建梯度不确定性代价图进行边界追踪；引入双分支不确定性感知尺度一致性损失和边界框对齐损失确保空间一致性和精确边界对齐。

Result: 在两个公开超声数据集BUSI和UNS上的实验表明，该方法性能与全监督方法相当甚至更好，同时显著降低标注成本。

Conclusion: 提出的弱监督框架在超声图像分割中具有有效性和实用性，大大减少了标注需求。
...
Method: 收集同步多模态数据，包括鱼眼相机、激光雷达、超声波传感器和动作捕捉的3D身体姿态，提供详细的3D关节位置注释和准确的行人3D位置数据。

Result: 发布了部分数据集和完整的基准测试套件，包含准确性、效率和嵌入式系统可扩展性的评估指标，并提供了基于自定义神经网络架构的基线性能指标。

Conclusion: 该数据集为研究人员开发智能车辆在近场场景中的行人意图检测算法提供了独特资源，并提出了未来研究方向以促进数据集的采用和改进。
...
TL;DR: 提出了一种基于多模态视觉变换器自编码器(MViT-AE)的无监督异常检测方法，用于脑肿瘤分割，无需依赖标注数据，在BraTS-GoAT 2025数据集上取得了有临床意义的肿瘤定位效果。


<details>
  <summary>Details</summary>
Motivation: 解决脑肿瘤分割中标注数据有限、成本高和不一致的问题，提供可扩展的无监督解决方案，克服神经影像工作流中的关键瓶颈。

Method: 使用多模态视觉变换器自编码器，仅用健康脑部MRI训练，通过重建误差图检测肿瘤；采用多模态早期-晚期融合策略整合不同MRI序列信息，并引入Segment Anything Model进行后处理优化肿瘤轮廓。

Result: 在测试集上获得病变级别的Dice相似系数：全肿瘤0.437、肿瘤核心0.316、增强肿瘤0.350；在验证集上异常检测率达到89.4%。

Conclusion: 基于变换器的无监督模型有潜力成为神经肿瘤影像的可扩展、标签高效工具，尽管在检测小或非增强病变方面仍存在挑战。
...
Method: 采用观察-知识-分析（OKA）范式：观察层构建了包含560万样本的UniMed-5M数据集；知识层提出渐进式课程学习系统引入医学多模态知识；分析层开发UniMedVL统一多模态模型。

Result: UniMedVL在五个医学图像理解基准测试中取得优异性能，同时在八个医学成像模态的生成质量上与专用模型相当。统一架构实现了双向知识共享：生成任务增强了视觉理解特征。

Conclusion: 在单一医学框架内整合传统上分离的能力，能够解锁跨不同医学视觉-语言任务的改进，证明了统一多模态方法的有效性。
...
TL;DR: DGME-T模型通过引入方向性网格运动编码，显著提升了在退化历史胶片上的相机运动分类性能，在现代和历史数据集上都取得了准确率和F1分数的提升。


<details>
  <summary>Details</summary>
Motivation: 解决传统相机运动分类模型在历史胶片上性能下降的问题，因为历史胶片存在噪声、缺失帧和低对比度等退化现象，会模糊运动线索。

Method: 构建统一基准数据集，并在Video Swin Transformer基础上引入方向性网格运动编码，通过可学习的归一化后期融合层注入光流信息。

Result: DGME-T将主干网络的top-1准确率从81.78%提升到86.14%，宏F1从82.08%提升到87.81%；在二战历史胶片上准确率从83.43%提升到84.62%，宏F1从81.72%提升到82.63%。

Conclusion: 结构化运动先验和Transformer表示是互补的，即使是一个小型、精心校准的运动头也能显著增强退化胶片分析的鲁棒性。
...
TL;DR: Ditto框架通过创新的数据生成管道、高效模型架构和智能代理系统，构建了包含100万个高质量视频编辑示例的Ditto-1M数据集，训练出的Editto模型在指令式视频编辑任务中达到新的最先进水平。
...
Method: 1. 融合领先图像编辑器和上下文视频生成器的数据生成管道；2. 采用蒸馏模型架构和时间增强器的高效方法；3. 智能代理驱动的可扩展流水线，用于指令生成和质量控制。

Result: 投入超过12,000 GPU天构建了包含100万个高质量视频编辑示例的Ditto-1M数据集，训练出的Editto模型展现出卓越的指令跟随能力。
...
TL;DR: SEGA提出了一种基于逐步演进范式的内容感知布局生成方法，通过粗粒度到细粒度的分层推理框架，结合布局设计原则作为先验知识，显著提升了复杂元素布局规划的成功率。
...
Method: SEGA采用分层推理框架：首先由粗粒度模块粗略估计布局规划结果，然后由细化模块对粗规划结果进行细粒度推理，并将布局设计原则作为先验知识融入模型。

Result: 在多个基准数据集上实现了最先进的结果，并发布了包含丰富元信息标注的大规模海报数据集GenPoster-100K。

Conclusion: SEGA通过逐步演进范式和分层推理框架，有效解决了内容感知布局生成中的复杂规划问题，显著提升了布局生成的质量和成功率。
...
TL;DR: 提出了NDM框架，首个基于噪声的检测和缓解方法，用于处理文本到图像生成中的隐式恶意意图，同时保持模型生成能力。


<details>
  <summary>Details</summary>
Motivation: 现有的检测方法主要针对显式色情内容，难以识别隐式恶意提示，而微调方法会降低模型生成质量。

Method: 利用早期预测噪声的可分离性进行检测，提出噪声增强的自适应负引导机制来优化初始噪声，抑制突出区域的注意力。
...
<details>
  <summary>Details</summary>
Motivation: 当获取精细像素级标注困难或昂贵时，粗标注成为可行选择，但传统方法在边界对齐方面表现不佳

Method: 在编码器-解码器架构中引入基于超像素的上采样正则化，强制解码图像中的分割像素符合SLIC超像素（基于颜色和位置，独立于分割标注）
...
TL;DR: 提出了一种通过替换生成网络的激活函数为参数化函数并设置参数的系统，让用户通过交互理解模型内部机制并控制输出。


<details>
  <summary>Details</summary>
Motivation: 随着图像生成模型保真度和普及度的提升，缺乏能够直接与其内部机制交互的可解释性工具。

Method: 允许用户将生成网络的激活函数替换为参数化函数，并通过设置这些函数的参数来控制网络输出。

Result: 在StyleGAN2（FFHQ数据集）和BigGAN（ImageNet数据集）上验证了该方法的有效性。

Conclusion: 该系统提供了一种通过交互和实验来理解生成模型并控制其输出的替代方法。
...
TL;DR: QSilk是一个轻量级的稳定层，用于潜在扩散模型，通过微钳位和自适应分位数裁剪技术提高高频保真度并抑制罕见激活峰值，无需训练即可集成到渲染流程中。
...
Method: 结合每样本微钳位（温和限制极值）和自适应分位数裁剪（AQClip），AQClip可使用局部结构统计或注意力熵引导模式来调整允许值范围。

Result: 在CADE 2.5渲染流程中集成QSilk后，在低步数和超高分辨率下获得更清晰、锐利的结果，计算开销可忽略，与CFG/Rescale有协同效应。

Conclusion: QSilk无需训练即可在各种SD/SDXL骨干网络上实现一致的定性改进，能够在不产生伪影的情况下使用更高的引导强度。
...
TL;DR: 提出轻量级解耦概念瓶颈模型(LDCBM)，通过自动分组视觉特征为语义组件，改进概念与视觉模式的对应关系，提升可解释性和分类性能。
...
TL;DR: ReCon是一个用于目标检测的数据增强框架，通过区域引导修正和区域对齐交叉注意力机制，在扩散采样过程中改善生成图像的质量和语义一致性。
...
Method: 在扩散采样过程中集成区域引导修正，使用预训练感知模型的反馈来修正生成错误区域；提出区域对齐交叉注意力机制，加强图像区域与文本提示之间的空间语义对齐。

Result: ReCon显著提高了生成数据的质量和可训练性，在各种数据集、骨干架构和数据规模上实现了一致的性能提升。

Conclusion: ReCon框架通过改进结构可控生成模型的能力，为数据增强提供了有效的解决方案，能够生成更高质量的训练数据。
...
TL;DR: 提出ERNet，一种高效的前馈模型，用于处理非刚性变形的点云序列配准，通过两阶段管道预测变形图序列，在准确性和效率上都优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决非刚性变形点云序列配准中的两个关键挑战：(i) 非凸配准目标导致的局部极小值问题，特别是在噪声或部分输入下；(ii) 长序列中的误差累积导致跟踪失败。

Method: 采用可扩展的数据驱动方法，通过两阶段管道预测变形图序列：首先估计帧间粗粒度图节点进行鲁棒初始化，然后在滑动窗口方式下随时间细化其轨迹。

Result: 在DeformingThings4D和D-FAUST数据集上优于先前最先进方法，相比之前最佳方法实现了4倍以上的加速，显著提升了效率。

Conclusion: ERNet能够有效处理噪声和部分输入，同时利用时间信息实现准确一致的序列配准，在准确性和效率方面都表现出色。
...
Method: VISTA采用多代理系统：1) 将用户想法分解为结构化时间计划；2) 通过成对锦标赛选择最佳视频；3) 视觉、音频和上下文保真度的专业代理进行评审；4) 推理代理综合反馈重写提示进行下一轮生成。

Result: 在单场景和多场景视频生成实验中，VISTA相比现有方法持续改进视频质量和用户意图对齐，达到60%的成对胜率，人类评估者在66.4%的比较中更偏好VISTA输出。

Conclusion: VISTA通过迭代自我改进机制有效提升了文本到视频生成的质量和一致性，证明了多代理系统在视频生成优化中的有效性。
...
Conclusion: 神经符号空间推理在开放词汇语义分割中具有有效性，能够通过关系约束提升分割一致性。
...
TL;DR: 3DPR是一个基于图像的重新光照模型，利用从光舞台采集的多视角单光源图像学习生成先验，能够从单张人像输入生成高质量的重光照视图。


<details>
  <summary>Details</summary>
Motivation: 传统图形学方法需要将输入图像显式分解为几何、材质和光照，但受限于底层模型的假设和近似。3DPR旨在利用生成先验来克服这些限制，实现更高质量的图像重光照。

Method: 首先将输入人像嵌入预训练生成头部模型的潜在空间，然后使用基于triplane的反射率网络合成高质量单光源图像，最后根据HDRI环境图组合生成环境光照结果。

Result: 3DPR在定量和定性评估中优于先前方法，特别是在保持身份特征和捕捉光照效果（如镜面反射、自阴影和次表面散射）方面表现优异。

Conclusion: 该方法通过结合生成头部模型的几何先验和光舞台数据的反射率先验，实现了高质量的图像重光照，即使使用相对较少的光舞台图像也能训练出有效的反射率模型。
...
TL;DR: Memory-SAM是一个无需训练、无需人工提示的舌像分割方法，通过检索先验案例自动生成有效提示来指导SAM2模型，在真实世界条件下表现优异。
...
Result: 在600张专家标注图像上测试，混合测试集mIoU达0.9863，优于FCN(0.8188)和检测器到框SAM基线(0.1839)，在真实世界条件下表现尤其突出。
...
TL;DR: BLIP3o-NEXT是一个完全开源的基础模型，统一了文本到图像生成和图像编辑功能，采用自回归+扩散架构，在多项基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 推进原生图像生成的前沿，将文本到图像生成和图像编辑统一到单一架构中，解决图像编辑任务的挑战性。

Method: 采用自回归+扩散混合架构：自回归模型首先生成基于多模态输入的离散图像标记，其隐藏状态作为扩散模型的调节信号来生成高保真图像。

Result: 在多个文本到图像和图像编辑基准测试中，BLIP3o-NEXT实现了优于现有模型的性能，达到了新的连贯性和真实感水平。

Conclusion: 架构选择的关键在于可扩展性和快速推理能力；强化学习能推动原生图像生成前沿；数据质量和规模是决定模型性能上限的决定性因素；自回归+扩散架构结合了推理能力和细节渲染优势。
...
TL;DR: LightsOut是一个基于扩散模型的图像外绘框架，专门用于增强单图像眩光去除方法，通过重建画面外光源来提升眩光去除效果。


<details>
  <summary>Details</summary>
Motivation: 现有单图像眩光去除方法在画面外光源不完整或缺失时表现不佳，这影响了计算机视觉任务如目标检测和自动驾驶的性能。

Method: 使用多任务回归模块和LoRA微调的扩散模型，确保外绘结果既真实又符合物理一致性。
...
<details>
  <summary>Details</summary>
Motivation: 解决大规模高质量3D城市场景合成的挑战，避免对昂贵3D标注数据的依赖，提供可探索的沉浸式3D环境。

Method: 使用卫星图像提供粗略几何结构，结合开放域扩散模型生成高质量近景外观，采用课程驱动的迭代细化策略逐步提升几何完整性和真实纹理。

Result: 相比现有方法，Skyfall-GS在跨视角几何一致性和纹理真实性方面表现更优，支持实时沉浸式3D探索。

Conclusion: 该方法为大规模3D城市场景创建提供了一种无需3D标注的可行方案，在几何精度和视觉质量上均优于现有技术。
...
TL;DR: 该论文提出了三个多标签毒性检测基准，通过伪标签方法显著提升了毒性检测性能，超越了GPT-4o和DeepSeek等先进基线。
...
Method: 引入了三个多标签毒性检测基准，基于15类详细分类法进行标注，并开发了基于伪标签的毒性检测方法。

Result: 实验结果表明，该方法显著超越了包括GPT-4o和DeepSeek在内的先进基线，实现了更准确可靠的多标签毒性评估。

Conclusion: 提出的多标签基准和伪标签方法能够更有效地评估LLM生成内容中的多标签毒性，解决了现有单标签方法的局限性。
...
TL;DR: 本研究评估了三种生成式AI模型在评分包含和不包含习语的学生作文时的表现，发现Gemini在与人评分者的一致性方面表现最佳，且在处理含习语的作文时最接近人类评分模式。
...
Method: 从348篇学生作文中创建两个相等的作文列表：一个包含多个习语，另一个不含习语。使用ChatGPT、Gemini和Deepseek三种生成式AI模型，采用与人类评分者相同的评分标准，对两组作文各进行三次评分。

Result: 所有模型都表现出极好的一致性，但Gemini在与人评分者的评分者间信度方面优于其他模型。AI评估中未检测到对任何人口群体的偏见。对于包含多个习语的作文，Gemini的评分模式最接近人类评分者。
...
TL;DR: 提出利用大语言模型自动生成和标注辩论数据，基于四类修辞策略（因果、经验、情感、道德）训练分类器，并在多个外部语料库中验证其性能和应用价值。
...
Result: 模型在多个外部语料库上表现出高性能和强泛化能力，应用于说服力预测和总统辩论分析，发现美国总统辩论中情感论证使用增加。

Conclusion: 该框架能有效自动生成和标注修辞策略数据，训练的分类器具有高性能和强泛化能力，为修辞策略分析提供了可扩展的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现代语言模型部署后通常保持静态，持续学习面临灾难性遗忘的挑战。受参数共享导致遗忘的直觉启发，研究稀疏参数更新是否能实现无遗忘学习

Method: 利用记忆层模型，仅更新被新知识高度激活的记忆槽，减少新知识与模型已有能力之间的干扰

Result: 在问答任务评估中，稀疏记忆微调仅导致11%的性能下降，而全量微调和LoRA分别导致89%和71%的性能下降
...
TL;DR: 该论文分析了MLAMA多语言事实知识评估基准中模板翻译的问题，发现在斯拉夫语系等形态丰富的语言中，模板翻译会导致语法错误和措辞不当，影响知识检索分数的解释性。通过使用Google Translate和ChatGPT进行句子级翻译，知识检索分数显著提升。


<details>
  <summary>Details</summary>
Motivation: MLAMA等多语言基准使用模板翻译，忽略了命名实体的语法和语义信息，导致最终提示存在语法错误和错误措辞，特别是在形态丰富的语言中，这影响了分数解释性。

Method: 从MLAMA数据集中选取4种斯拉夫语言，比较初始模板翻译与Google Translate和ChatGPT句子级翻译的知识检索分数，并进行定性分析。还对其他5种不同语系语言进行了额外分析。

Result: 观察到知识检索分数显著增加，句子级翻译能更好地处理语法和语义信息，提高评估结果的准确性和可解释性。

Conclusion: 建议在多语言数据集中控制语法正确性，以获得更高且更可解释的结果，神经机器翻译或LLM系统的整句翻译是良好近似方法。
...
TL;DR: 提出一个端到端的框架，通过无监督聚类和基于提示的标注，利用大语言模型自动从无标签语料库生成可解释的主题分类法，并应用于2024年美国总统选举前的Meta政治广告分析。
...
Method: 结合无监督聚类与基于提示的标注，利用大语言模型迭代构建分类法，无需种子集或领域专业知识。应用于2024年美国总统选举前Meta政治广告的大规模语料库。

Result: 发现投票和移民广告在总支出和曝光中占主导，而堕胎和选举诚信获得不成比例的覆盖面。资金模式同样两极分化：经济诉求主要由保守派PAC推动，堕胎信息在支持和反对权利联盟间分裂，犯罪与司法运动分散在地方委员会中。
...
TL;DR: 本文提出了FarsiMCQGen方法，用于生成波斯语多项选择题，结合候选生成、过滤和排序技术，并引入包含10,289个问题的新型波斯语MCQ数据集。
...
Method: 结合Transformer、知识图谱和基于规则的方法，采用候选生成、过滤和排序技术来生成可信的干扰项。

Result: 开发了包含10,289个问题的波斯语MCQ数据集，并通过最先进的大型语言模型评估，证明了模型的有效性和数据集的质量。

Conclusion: FarsiMCQGen方法在波斯语MCQ生成方面表现出色，该数据集有望推动MCQ研究的进一步发展。
...
TL;DR: 提出Structure-R1框架，通过强化学习将检索内容转换为结构化表示，优化多步推理过程，在多个知识密集型基准测试中表现出色。
...
Method: 采用强化学习学习内容表示策略，动态生成适应多步推理需求的结构化格式，并引入自奖励结构验证机制确保生成结构的质量和可靠性。

Result: 在7个知识密集型基准测试中，使用7B规模骨干模型的Structure-R1达到竞争性性能，甚至匹配更大模型的性能。
...
TL;DR: 本文提出了Partial YaRN和VLAT两种方法来解决大型音频-语言模型的长上下文限制问题，Partial YaRN是一种无需训练的音频专用上下文扩展方法，VLAT是一种训练策略，通过位置增强实现长音频理解。


<details>
  <summary>Details</summary>
Motivation: 大型音频-语言模型受限于短音频上下文窗口，即使其文本主干支持长上下文，这限制了长格式音频理解能力。现有上下文扩展方法主要针对单模态LLM，在LALMs中的应用尚未探索。

Method: 1. Partial YaRN：基于RoPE的上下文扩展方法，仅修改音频token位置，保持文本位置不变以保留基础LLM的文本能力；2. VLAT：将Partial YaRN扩展为训练时的位置增强策略，在训练中模拟不同音频长度，实现泛化到远超训练所见长度的输入。

Result: 在SALMONN和Qwen2-Audio上的实验表明，Partial YaRN在多种设置下优于原始模型，VLAT训练策略带来显著改进，在未见长度的长音频上实现强大性能。

Conclusion: Partial YaRN和VLAT有效解决了LALMs的长上下文限制问题，Partial YaRN提供无需训练的即时改进，VLAT通过训练增强实现更好的长音频理解能力。
...
TL;DR: 本研究探索了离散扩散语言模型（DDLM）与自回归语言模型（ARM）的混合架构，通过在文本空间和潜在空间的协作，实现了计算效率与准确性的平衡。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型虽然准确但计算成本高，离散扩散语言模型具有并行生成和灵活推理的优势，研究两者混合架构能否获得互补效益。

Method: 首先在文本空间进行协作（一个模型规划推理过程，另一个执行答案），然后在潜在空间引入学习投影器将DDLM潜在表示映射到ARM嵌入空间。

Result: 从文本空间转向潜在空间的通信显著提升准确性（DART-5从27.0%到54.0%，AIME24从0.0%到14.0%），DDLM规划器与ARM执行器组合可大幅节省计算资源（64个规划token和5个执行token超越Qwen3.1-7B，后者使用44倍token）。

Conclusion: DDLM在混合架构中具有巨大潜力，潜在空间协作能有效提升性能并显著降低计算成本，为推理任务提供了新思路。
...
TL;DR: 本文对多模态RAG在文档理解中的应用进行了系统综述，提出了基于领域、检索模态和粒度的分类法，总结了关键数据集、基准和应用，并指出了效率、细粒度表示和鲁棒性等开放挑战。
...
Conclusion: 多模态RAG是文档AI的重要发展方向，但在效率、细粒度表示和鲁棒性方面仍面临挑战，需要进一步研究来推动该领域的进步。
...
TL;DR: TraceCoder是一个集成多源外部知识的自动化ICD编码框架，通过动态整合UMLS、维基百科和LLMs等知识源来增强代码表示、弥合语义鸿沟，并处理罕见和模糊代码，实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 解决现有自动化ICD编码方法面临的临床文本与ICD代码之间的语义鸿沟、罕见和长尾代码性能差以及可解释性有限等问题。
...
TL;DR: 提出了TACL（阈值自适应课程学习）框架，通过动态调整训练过程来处理医学文本的复杂性差异，在多项临床任务中显著提升性能


<details>
  <summary>Details</summary>
Motivation: 医学文本（特别是电子病历）具有非结构化、领域特定语言和上下文变异性等特点，现有方法将所有数据视为同等难度，忽略了复杂度的内在差异，限制了模型在罕见或复杂病例上的泛化能力

Method: TACL框架基于渐进学习原则，根据样本复杂度动态调整训练过程，将数据分类为不同难度级别，在训练早期优先处理简单案例以建立坚实基础，再处理复杂记录

Result: 将TACL应用于多语言医学数据（包括英语和中文临床记录），在自动ICD编码、再入院预测和中医证候鉴别等临床任务中观察到显著改进
...
TL;DR: 提出了EGP框架，通过检索训练集中的示例问题及其成功推理路径，来指导LLM在KGQA中的规划过程，包括任务分解和关系探索两个关键阶段。


<details>
  <summary>Details</summary>
Motivation: LLM在KGQA中面临自然语言查询与结构化知识图谱之间的语义鸿沟，导致规划不优和探索效率低下，而无需训练的方法往往未能充分利用训练数据中的宝贵推理模式。

Method: EGP框架：1) 通过实体模板化预处理训练集问题；2) 使用语义嵌入和FAISS索引检索相似示例问题及其推理路径；3) 在任务分解和关系探索两个阶段动态指导LLM规划；4) 引入Smart Lookahead机制提高探索效率。

Result: 在WebQSP和CWQ两个真实世界KGQA数据集上的广泛实验表明，PoG-EGP显著优于基线PoG系统和其他对比方法。

Conclusion: EGP框架通过示例引导的规划有效提升了LLM在KGQA中的规划能力，解决了语义鸿沟和推理模式利用不足的问题。
...
TL;DR: 该研究比较了Jaccard系数和余弦相似度在自动作文评分中的表现，发现余弦相似度优于Jaccard系数，且一元文法比二元和三元文法具有更低的RMSE。


<details>
  <summary>Details</summary>
Motivation: 自动作文评分需要高效准确的评估工具，研究旨在探索不同相似度度量方法和n-gram表示在作文评分中的有效性。

Method: 使用初中公民教育课程的作文数据，通过n-gram模型进行特征提取和向量化，然后计算Jaccard系数和余弦相似度，最后通过RMSE评估系统性能。

Result: 余弦相似度表现优于Jaccard系数，一元文法相比二元和三元文法具有更低的均方根误差。

Conclusion: 在自动作文评分中，余弦相似度结合一元文法模型能够提供更好的评分效果。
...
<details>
  <summary>Details</summary>
Motivation: 在移动设备上部署大型语言模型时，逐令牌生成过程存在高延迟和硬件利用率低的问题，这限制了智能助手和UI代理等应用的发展。

Method: CoordGen包含三个协同组件：自适应执行调度、上下文对齐草稿生成和硬件高效草稿扩展，通过动态平衡计算图、在线校准和重用中间序列来优化推理过程。

Result: 在多个智能手机和代表性工作负载上的实验显示，与现有移动推理解决方案相比，生成速度最高提升3.8倍，能效最高提升4.7倍。

Conclusion: CoordGen框架通过推测解码与硬件调度的协同优化，有效解决了移动设备上LLM推理的性能瓶颈，为上下文感知的个性化应用提供了可行的解决方案。
...
TL;DR: AutoGraph-R1是首个使用强化学习直接优化知识图谱构建以提升任务性能的框架，通过将图生成视为策略学习问题，训练LLM构造器，奖励基于图谱在RAG管道中的功能效用。


<details>
  <summary>Details</summary>
Motivation: 知识图谱构建过程与下游应用脱节，导致图结构不最优，阻碍了检索增强生成在问答系统中的有效性。

Method: 使用强化学习框架，设计两种新颖的任务感知奖励函数：一种用于图谱作为知识载体，另一种用于图谱作为知识索引。

Result: 在多个问答基准测试中，AutoGraph-R1使图谱RAG方法相比使用任务无关基线图谱获得显著性能提升。

Conclusion: 该工作表明可以在构建和应用之间形成闭环，将范式从构建内在“好”的图谱转向构建可证明“有用”的图谱。
...
TL;DR: 该研究发现传统可读性评估指标与人类感知存在不匹配，模型驱动的指标在相关性排名上表现更优，信息内容和主题对文本可理解性有重要影响。


<details>
  <summary>Details</summary>
Motivation: 自动可读性评估在确保有效和可访问的书面交流中起关键作用，但该领域受限于可读性定义不一致和依赖表层文本属性的测量方法。

Method: 分析了897个人类判断来研究影响可读性感知的因素，并在五个英语数据集上评估了15个流行可读性指标与6个更细致的模型驱动指标的对比。

Result: 四个模型驱动指标在人类判断的相关性排名中始终位列前四，而表现最佳的传统指标平均排名仅为8.6。信息内容和主题对文本可理解性有显著影响。

Conclusion: 当前可读性指标与人类感知存在不匹配，模型驱动方法是一个更有前景的研究方向。
...
<details>
  <summary>Details</summary>
Motivation: 现有集成方法在短文本回答中有效，但在长文本生成中应用不足，且在每个token处集成往往会降低性能。

Method: 提出SAFE框架，选择性集成时考虑分词不匹配和概率分布共识两个因素，并引入概率锐化策略来稳定集成。
...
<details>
  <summary>Details</summary>
Motivation: 解决扫描图像文档解析中复杂元素交织的挑战，现有监督微调方法在跨文档类型泛化能力不足，且高质量训练数据有限

Method: 使用强化学习框架LayoutRL，通过整合归一化编辑距离、段落计数准确性和阅读顺序保持的复合奖励来优化布局理解，并构建Infinity-Doc-400K数据集

Result: Infinity-Parser在OmniDocBench、olmOCR-Bench、PubTabNet和FinTabNet等基准测试中一致实现最先进性能，大幅超越专业文档解析系统和通用视觉语言模型

Conclusion: LayoutRL框架和Infinity-Parser模型在文档解析任务中展现出强大的跨领域泛化能力，为可复现研究提供代码、数据集和模型
...
TL;DR: Speech-LLMs在语音不流畅（特别是帕金森病相关的不流畅）场景下表现显著下降，现有评估方法存在局限性，需要新方法提升包容性。


<details>
  <summary>Details</summary>
Motivation: 现有Speech-LLMs评估依赖理想化输入，忽视了常见的不流畅语音，特别是与帕金森病等疾病相关的不流畅，需要测试其在语音障碍用户交互中的性能。

Method: 引入VocalBench-DF框架，系统评估22个主流Speech-LLMs在多维度分类下的不流畅处理能力。

Result: 评估显示Speech-LLMs性能显著下降，音素级处理和长上下文建模是主要瓶颈，增强识别和推理能力可显著提升鲁棒性。

Conclusion: Speech-LLMs在实际应用中的准备度有限，迫切需要新方法改进不流畅处理能力，构建真正包容的语音大语言模型。
...
TL;DR: 为解决游戏广告和推荐系统中的稀疏性和不平衡问题，提出了用户游戏生命周期(UGL)框架，通过丰富用户行为数据和创新策略来提升推荐效果。


<details>
  <summary>Details</summary>
Motivation: 现有推荐系统方法不适合游戏广告和推荐，主要因为游戏稀疏性(仅有数百款游戏)和游戏不平衡性(用户行为被少数热门游戏主导)。

Method: 引入用户游戏生命周期(UGL)来丰富用户行为，提出两种创新策略提取用户短期和长期兴趣，并采用逆概率掩码策略解决游戏不平衡问题。

Result: 离线实验平均AUC提升1.83%，在线实验游戏广告CVR提升21.67%，游戏内物品推荐AUC提升0.5%，ARPU提升0.82%。

Conclusion: UGL表示显著提升了游戏广告和推荐系统的性能，有效解决了游戏稀疏性和不平衡性问题。
...
TL;DR: 该研究提出了一种专门化MedGemma模型的方法，用于生成高质量医学图像描述作为检索增强生成系统的查询输入，解决了通用视觉语言模型在医学领域缺乏临床特异性的问题。
...
Result: 微调后的模型在分类性能上显著提升，RAGAS评估显示描述忠实度、相关性和正确性均有显著改善，验证了模型生成可靠、基于事实描述的能力。

Conclusion: 这项工作建立了一个专门化医学视觉语言模型的强大流程，验证了所得模型作为高质量查询生成器的有效性，为增强多模态检索增强生成系统在循证临床决策支持中的应用奠定了基础。
...
TL;DR: 提出了GuessBench基准来评估多模态大语言模型在信息不完整情况下的主动推理能力，发现现有模型在主动推理方面的表现远落后于被动推理，并识别了细粒度感知和及时决策作为关键挑战。


<details>
  <summary>Details</summary>
Motivation: 现有MLLM评估主要关注被动推理（在完整信息下进行逐步推理），这与现实世界使用场景不符，因为现实世界中"看到"并不足够，需要模型能够主动获取缺失证据。

Method: 提出GuessBench基准，包含感知导向和知识导向的图像，要求MLLMs在信息不完整情况下主动从候选池中选择目标图像获取缺失证据，并迭代优化决策。评估了20个先进的MLLM。

Result: MLLMs在主动推理任务上的表现远低于被动推理设置，表明有显著改进空间。细粒度感知和及时决策是主要挑战。消融研究表明感知增强对小模型有益，而思维导向方法对所有模型规模都有一致提升。

Conclusion: 多模态主动推理是MLLMs的重要研究方向，感知增强和思维导向方法为未来研究提供了有前景的方向。
...
TL;DR: 提出基于提示工程的可控摘要生成方法，通过多阶段提示框架生成不同抽象级别的摘要，研究提示长度、数据噪声和文本类型对摘要质量的影响。


<details>
  <summary>Details</summary>
Motivation: 解决传统方法在摘要质量和可控性方面的问题，探索如何通过控制提示策略和优化文本预处理来提高大语言模型生成摘要的准确性和可控性。

Method: 设计多阶段提示生成框架，对输入文本进行语义分析、主题建模和噪声控制，在CNN/Daily Mail数据集上实验分析不同提示长度、数据噪声和文本类型的影响。

Result: 提示长度对摘要质量有显著影响，过短或过长的提示都会降低质量；数据噪声增加会导致ROUGE-L分数下降；模型在新闻文本上表现最佳，学术文章表现较差。

Conclusion: 研究为大语言模型改进摘要生成提供了新见解，特别是在控制提示策略和优化文本预处理方面可以增强摘要的准确性和可控性。
...
<details>
  <summary>Details</summary>
Motivation: 云端LLM需要上传完整UI状态，暴露不必要信息；本地LLM容量有限，任务成功率低。需要平衡隐私保护和任务性能。

Method: 1) 基于XML层次结构的布局感知块分区；2) 本地和云端LLM协同规划子任务；3) 本地LLM排序相关UI块，云端LLM在顶级块内选择具体元素。

Result: CORE将UI暴露减少高达55.6%，同时任务成功率仅略低于纯云端代理，有效缓解了不必要的隐私暴露。

Conclusion: CORE框架成功平衡了隐私保护和任务性能，通过协作机制在减少UI暴露的同时维持了高任务准确率。
...
TL;DR: DeceptionBench是首个系统评估大语言模型在不同社会领域中欺骗行为的基准，涵盖5个领域150个场景，分析内在行为模式和外在因素影响，发现当前模型在强化动态下欺骗行为加剧。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型能力的快速发展，出现了新兴的欺骗行为，可能在高风险部署中带来严重风险。目前对现实场景中欺骗行为的表征研究不足。

Method: 建立DeceptionBench基准，包含5个领域（经济、医疗、教育、社交互动、娱乐）的150个场景和1000多个样本。分析内在维度（利己主义vs讨好行为）和外在维度（中性条件、奖励激励、强制压力下的欺骗输出），并纳入持续多轮交互循环。

Result: 对LLM和LRM的广泛实验揭示了关键漏洞，特别是在强化动态下欺骗行为加剧，表明当前模型缺乏对操纵性上下文线索的鲁棒抵抗能力。
...
TL;DR: 提出了一个名为TEMP-ReCon的新基准来评估LLMs在时间参考一致性方面的表现，并开发了UnTRaP模型来增强LLMs的时间推理能力。


<details>
  <summary>Details</summary>
Motivation: 随着LLMs在时间敏感领域（如法律、医疗、金融）中作为知识源的广泛应用，需要确保它们不仅事实准确，还要在时间维度上保持一致性，但目前缺乏对LLMs时间参考一致性的评估和改进工作。

Method: 引入时间参考一致性基准TEMP-ReCon，涵盖多种语言环境（英语、法语、罗马尼亚语），并提出了基于推理路径对齐的UnTRaP模型来提升LLMs的时间参考一致性。

Result: 实验表明LLMs在时间参考一致性方面表现不足，而UnTRaP模型相比多个基线模型显示出更好的效果。

Conclusion: LLMs在时间推理方面存在不足，需要专门的基准和模型来提升其时间参考一致性，UnTRaP为此提供了有效的解决方案。
...
Conclusion: 所提出的方法提供了一种高效、灵活且语言无关的表示方案，成功结合了子词和字符级模型的优势。
...
TL;DR: Latent-SFT是一种两阶段学习框架，通过在词汇表概率空间进行潜在推理，匹配显式推理性能的同时减少4倍推理链长度，在GSM8k、Math500和AIME24上实现最先进性能。


<details>
  <summary>Details</summary>
Motivation: 显式推理（如思维链）计算开销大，而现有潜在推理方法性能显著下降。初步实验表明性能下降源于非结构化潜在空间难以拟合潜在标记。

Method: 提出Latent-SFT两阶段框架：第一阶段设计专用注意力掩码指导潜在标记编码器生成潜在标记；第二阶段丢弃编码器，直接训练LLM自主生成潜在标记进行推理，使用KL和CE损失优化。

Result: 在GSM8k上匹配显式SFT性能，推理链减少达4倍，优于先前潜在方法。在Math500和AIME24上，基于词汇概率的潜在推理明显超越基于隐藏状态的方法。
...
TL;DR: 提出了一种模态组合感知框架，通过偏好损失和组合正则化目标来缓解统一编码器在对比学习中容易学习模态捷径的问题，从而提升分布偏移下的检索鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 尽管统一编码器在多模态大语言模型中具有灵活性，但传统对比学习训练的统一编码器容易学习模态捷径，导致在分布偏移下鲁棒性较差。

Method: 使用偏好损失强制多模态嵌入优于单模态对应物，同时通过组合正则化目标将多模态嵌入与其单模态部分组成的原型对齐，显式建模组合表示与其单模态对应物之间的结构关系。

Result: 在多个基准测试中，该方法在分布外检索方面取得了提升，证明了模态组合感知是使用MLLMs作为统一编码器时实现鲁棒组合多模态检索的有效原则。

Conclusion: 模态组合感知框架能有效缓解统一编码器中的模态捷径问题，提升多模态检索在分布偏移下的鲁棒性。
...
TL;DR: 提出了TokenTiming算法，通过动态时间规整(DTW)技术实现通用推测解码，解决了传统推测解码中草稿模型和目标模型必须共享词汇表的限制，实现了1.57倍的加速效果。


<details>
  <summary>Details</summary>
Motivation: 传统推测解码方法存在根本性限制：草稿模型和目标模型必须共享相同的词汇表，这限制了可用草稿模型的选择范围，通常需要从头训练新模型。

Method: 提出TokenTiming算法，通过重新编码草稿标记序列获得新的目标标记序列，然后使用DTW建立映射来传输概率分布以进行推测采样。该方法能够处理不匹配的词汇表，无需重新训练和修改即可与任何现成模型配合使用。
...
TL;DR: 本文通过偏差-方差分解重新定义跨语言差距，提出目标语言响应方差是主要成因，并通过降低方差的方法显著提升了跨语言准确率。


<details>
  <summary>Details</summary>
Motivation: 现有研究将跨语言差距归因于源语言和目标语言潜在表征的差异，但本文提出目标语言响应方差才是主要成因，需要新的理论框架来理解这一现象。

Method: 采用偏差-方差分解形式化跨语言差距，通过实验验证假设，并设计推理时干预措施来控制响应方差，包括简单的提示指令方法。

Result: 实验证明目标语言响应方差是跨语言差距的主要来源，通过降低方差的方法使目标语言准确率提升了20-25%。

Conclusion: 跨语言差距主要源于目标语言响应方差而非表征差异，通过控制方差可有效缩小跨语言性能差距。
...
TL;DR: 提出ParallaxRAG框架，通过多视图空间对称解耦查询和图三元组，实现稳健的检索架构，在知识图谱增强生成中减少幻觉并改进多跳推理。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在语言理解方面表现出色，但经常产生幻觉且在多跳推理方面存在困难。基于知识图谱的检索增强生成方法提供了基础，但大多数方法依赖扁平嵌入和噪声路径探索。

Method: ParallaxRAG框架将查询和图三元组对称解耦到多视图空间，构建稳健的检索架构，明确强制头部多样性同时约束弱相关路径。利用不同注意力头在不同推理阶段专门处理语义关系的特点。

Result: 在WebQSP和CWQ数据集上的实验显示，在统一的、可复现的设置下（BGE-M3 + Llama3.1-8B），实现了有竞争力的检索和问答性能，同时减少了幻觉并具有良好的泛化能力。

Conclusion: 多视图头部专业化是知识基础多跳推理的一个原则性方向，ParallaxRAG通过构建更清晰的子图并指导LLMs进行基于基础的逐步推理，有效改进了多跳推理性能。
...
TL;DR: 提出了KITE基准测试，专门评估韩语大语言模型的指令跟随能力，填补了韩语在开放指令评估方面的空白。


<details>
  <summary>Details</summary>
Motivation: 现有评估主要关注英语模型，忽略了其他语言的语言文化特性。韩语具有独特的语法、形态特征、敬语系统和双数系统，但缺乏专门的开放指令评估基准。

Method: 开发了KITE基准，包含通用和韩语特定指令的评估任务。评估流程结合自动化指标和人工评估，分析模型在不同任务上的表现。

Result: 评估揭示了不同模型之间的性能差异，提供了对模型优势和弱点的深入洞察。

Conclusion: 通过公开KITE数据集和代码，旨在促进文化和语言包容性LLM开发的研究，并激励为其他代表性不足语言开展类似工作。
...
TL;DR: 本文介绍了EvaCun 2025令牌预测任务的提交系统，基于Command-R、Mistral和Aya Expanse等LLM进行微调，使用三种不同提示方法进行预测比较。


<details>
  <summary>Details</summary>
Motivation: 由于对任务领域和语言知识有限，作者希望探索在不进行任务特定调整的情况下，仅使用提供的训练数据来构建有效的令牌预测系统。

Method: 使用三种不同的提示方法对Command-R、Mistral和Aya Expanse等大型语言模型进行微调，直接使用组织者提供的训练数据，不进行预处理或过滤。

Result: 在数据保留集上评估了三种不同提示方法的性能表现，但具体结果未在摘要中详细说明。

Conclusion: 展示了在领域知识有限的情况下，仅使用原始训练数据对LLM进行微调即可构建令牌预测系统的可行性。
...
TL;DR: 该研究通过多义词分析探索乌尔都语诗歌中爱情表达的复杂性，重点关注pyaar、muhabbat和ishq三个看似同义词的细微差别，并通过词嵌入技术进行乌尔都语与英语的对比分析。


<details>
  <summary>Details</summary>
Motivation: 揭示乌尔都语诗歌中爱情词汇的丰富语义层次，这些词汇在英语文学中缺乏直接对应词，展现乌尔都语独特的文化表达方式。

Method: 采用多义词案例研究方法，分析词汇在诗歌中的使用语境，同时使用词嵌入技术对乌尔都语和英语爱情相关词汇进行量化比较和可视化。

Result: 发现了pyaar、muhabbat和ishq之间微妙的情感差异，通过词嵌入可视化展示了这些词汇在语义空间中的独特分布，揭示了乌尔都语爱情表达的文化特异性。

Conclusion: 乌尔都语诗歌通过丰富的词汇层次展现了爱情表达的复杂性，这种文化特异性需要通过深入的语言分析才能充分理解，词嵌入技术为跨语言文化研究提供了有效工具。
...
TL;DR: 提出了一种跨语言双向最大相似度评分(BiMax)方法，用于计算文档间相似度，相比最优传输方法在保持准确率的同时实现了约100倍的速度提升。
...
Method: 开发了跨语言双向最大相似度评分(BiMax)来计算文档间相似度，并发布了名为EmbDA的公开工具。

Result: 在WMT16双语文档对齐任务中，BiMax方法达到了与OT相当的准确率，同时速度提升了约100倍。

Conclusion: BiMax方法在文档对齐任务中实现了准确性和效率的良好平衡，为大规模跨语言文档对齐提供了有效解决方案。
...
TL;DR: 本文介绍了一个新的法语长篇小说核心指代消解语料库，包含三本完整小说，总计超过28.5万个词符，用于评估长文档中的核心指代消解模型。


<details>
  <summary>Details</summary>
Motivation: 当前计算文学研究中核心指代消解受到广泛关注，但完全标注的长文档代表性数据集仍然稀缺，特别是针对复杂长篇文学作品的挑战。

Method: 提出了一个模块化的核心指代消解流程，支持细粒度错误分析，能够有效扩展到长文档处理。

Result: 该方法具有竞争力，能够有效处理长文档，并展示了在推断虚构角色性别方面的实用性。

Conclusion: 该语料库和流程对于文学分析和下游NLP任务都具有重要意义，填补了长文档核心指代消解数据集的空白。
...
TL;DR: HypoSpace是一个评估语言模型生成多种合理假设能力的诊断套件，关注假设的有效性、独特性和覆盖率，揭示了在复杂问题中模型容易产生模式崩溃的问题。


<details>
  <summary>Details</summary>
Motivation: 随着语言模型在科学工作流程中的广泛应用，需要评估它们提出多种解释假设（而非单一正确答案）的能力，因为许多科学问题存在多种机制不同的合理解释。

Method: 将LLMs视为有限假设集的采样器，开发HypoSpace诊断套件，在三个结构化领域（因果图、3D体素重建、布尔遗传交互）中测量三个指标：有效性、独特性和覆盖率。

Result: 研究发现，随着可接受假设空间的扩大，模型的有效性通常保持较高，但独特性和覆盖率会下降，这揭示了仅关注正确性的指标无法发现的模式崩溃问题。

Conclusion: HypoSpace为显式探索和覆盖可接受解释空间的方法提供了一个受控的探测工具，而非排行榜，有助于更全面地评估语言模型的科学推理能力。
...
TL;DR: 使用大型语言模型作为动态知识库生成背景上下文，通过四种方法将上下文整合到仇恨言论检测分类器中，在文本和多模态设置中分别获得最高3和6个F1分数的提升。


<details>
  <summary>Details</summary>
Motivation: 利用LLMs生成背景上下文来增强仇恨言论检测，特别是针对隐含仇恨言论和仇恨表情包，因为传统方法在处理这些需要背景知识的案例时效果有限。

Method: 提出两种上下文生成策略（命名实体聚焦和全文提示），比较四种上下文整合方法：文本拼接、嵌入拼接、分层transformer融合和LLM驱动的文本增强。

Result: 在文本Latent Hatred数据集和多模态MAMI数据集上实验表明，上下文信息和整合方法都很关键，从零上下文基线到最佳系统（基于嵌入拼接）分别获得3和6个F1分数提升。

Conclusion: LLM生成的上下文能显著提升仇恨言论检测性能，嵌入拼接是最有效的整合方法，证明了背景知识对检测隐含仇恨内容的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 现有推理模型虽然性能强大，但检索和推理token都导致高昂的计算成本，需要更高效的解决方案。

Method: 开发了动态调整检索文档长度的检索增强推理模型，使用成本感知优势函数通过强化学习训练，探索了内存和延迟约束下的实现方案。

Result: 在7个公开问答数据集上评估，模型延迟降低16-20%，准确率平均提升5%，在提升效率的同时不损失效果。

Conclusion: 提出的成本感知检索增强推理框架能显著提升效率，同时保持甚至改善模型性能，为高效推理提供了有效解决方案。
...
TL;DR: 本文对掩码扩散语言模型(DLMs)的注意力机制进行实证分析，发现其存在动态变化的注意力下沉现象，且与自回归模型(ARMs)相比，DLMs对注意力下沉的移除具有更强的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 虽然掩码扩散语言模型作为自回归模型的替代方案显示出前景，但其内部工作机制尚未得到充分探索，特别是注意力模式的特征。

Method: 对DLM注意力模式进行实证分析，重点关注注意力下沉现象，并与自回归模型进行对比研究。
...
Method: 提出自动互评框架，让LLM通过自玩和同行评审相互评估输出，使用博弈论投票算法聚合同行评审结果，并与人类投票行为进行系统比较。
...
TL;DR: 提出了一种无需参考翻译的机器翻译质量评估方法，通过逐段翻译和NLP混洗测试来验证翻译器，特别适用于动物语言翻译等无参考翻译的场景。


<details>
  <summary>Details</summary>
Motivation: 解决如何验证AI动物语言翻译器的问题，避免直接交互或观察的需求，在安全性、伦理和成本方面具有优势。

Method: 使用逐段翻译结合经典NLP混洗测试，通过比较顺序翻译与随机排列翻译的合理性来评估翻译质量。

Result: 在数据稀缺的人类语言和构造语言上的概念验证实验表明，该方法与基于参考翻译的标准评估高度相关。

Conclusion: 交互在翻译学习的早期阶段可能既不必要也不高效，提出的无参考评估方法在数据稀缺场景下具有潜在实用价值。
...
TL;DR: 该论文通过构建一个单层Transformer玩具模型，揭示了语言模型中线性真值子空间形成的具体机制，发现真值编码可以在事实陈述共现的数据分布中自然涌现。


<details>
  <summary>Details</summary>
Motivation: 现有研究表明大语言模型存在区分真假陈述的线性子空间，但其形成机制尚不明确，需要构建透明模型来揭示具体形成路径。

Method: 设计一个透明的单层Transformer玩具模型，在事实陈述共现的数据分布设置下进行端到端训练，观察真值子空间的形成过程。

Result: 在玩具模型中观察到两阶段学习动态：网络先快速记忆个体事实关联，然后经过更长时间学习线性区分真假陈述，从而降低语言建模损失。

Conclusion: 研究提供了线性真值表征在语言模型中涌现的机制性证明和实证动机，解释了真值子空间如何及为何形成。
...
<details>
  <summary>Details</summary>
Motivation: 当前学术项目网站在有效传播研究方面存在不足，现有方法如直接LLM生成、模板或HTML转换难以产生具有布局感知和交互性的网站，且缺乏全面的评估套件。

Method: 提出Paper2Web基准数据集，包含基于规则的指标（连通性、完整性）和人类验证的LLM-as-a-Judge评估（交互性、美观性、信息性），以及PaperQuiz用于衡量论文级知识保留。同时开发PWAgent自主管道，通过MCP工具迭代优化内容和布局。

Result: 实验表明PWAgent在学术网页生成方面大幅优于端到端基线方法（如基于模板的网页和arXiv/alphaXiv版本），同时保持低成本，实现了帕累托最优。

Conclusion: Paper2Web为学术网页生成提供了全面的评估框架，PWAgent能够有效将科学论文转换为高质量的交互式多媒体学术主页，显著提升研究传播效果。
...
Method: 采用混合词典-模糊-Transformer框架，结合基于规则的启发式方法、上下文深度学习和模糊逻辑，通过VADER初始情感估计、DistilBERT置信度调整和模糊推理系统生成连续情感分数。

Result: 在四个领域特定数据集上的评估显示，该方法与用户评分更一致，能更好识别情感极端值，减少误分类，定量和定性分析均证实了模型的鲁棒性和效率。
...
TL;DR: 该论文研究了基于语音大语言模型的槽位填充任务，通过建立经验上限、识别性能差距，并提出训练数据、架构和训练策略的改进来缩小与上限的差距。


<details>
  <summary>Details</summary>
Motivation: 传统槽位填充采用语音识别和自然语言理解的级联方法，而语音大语言模型提供了更统一、生成式和指令跟随的方式，具有零样本能力和泛化到未见槽位标签的潜力。

Method: 创建任务的经验上限，识别性能、鲁棒性和泛化差距，改进训练数据、架构和训练策略。
...
TL;DR: 提出了ORBIT框架，通过基于量规的增量强化学习来提升LLM在开放式高风险医疗对话中的表现，在HealthBench-Hard基准上取得了显著改进。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在数学和代码等有明确奖励函数的领域表现良好，但在开放式领域如医疗咨询中，由于奖励模糊、主观且依赖上下文，缺乏稳健的奖励函数，限制了RL策略的应用。

Method: ORBIT框架结合合成对话生成和动态量规创建，使用量规指导增量强化学习过程，不依赖外部医学知识或手动规则，而是通过量规引导反馈来塑造学习。

Result: 在Qwen3-4B-Instruct模型上实施，仅使用2k样本就将HealthBench-Hard基准性能从7.0提升至27.2，达到该规模模型的最先进水平。

Conclusion: 量规驱动的强化学习能在多样化咨询场景中实现一致性能提升，超越简单的数值改进，证明基于量规的反馈是推进LLM在复杂开放式任务中的可扩展策略。
...
TL;DR: PolySkill框架通过将技能的抽象目标与具体实现解耦，使智能体能够学习可泛化和组合的技能，在网页交互任务中显著提升技能复用率和成功率。


<details>
  <summary>Details</summary>
Motivation: 现有方法学习的技能通常过度专门化于单一网站，缺乏泛化能力。需要开发能够学习可跨网站泛化技能的框架。

Method: 受软件工程中多态性启发，将技能的抽象目标（做什么）与具体实现（如何做）解耦，使智能体能够学习通用技能。

Result: 在已见网站上技能复用率提高1.7倍；在Mind2Web和未见网站上成功率分别提升9.4%和13.9%，同时步骤减少20%以上；在自探索设置中能学习跨网站通用技能。
...
### [142] [OpenEstimate: Evaluating LLMs on Reasoning Under Uncertainty with Real-World Data](https://arxiv.org/abs/2510.15096)
*Alana Renda,Jillian Ross,Michael Cafarella,Jacob Andreas*

Main category: cs.AI

TL;DR: OpenEstimate是一个用于评估语言模型在不确定性下进行数值估计和概率推理能力的多领域基准测试，发现前沿语言模型产生的先验分布往往不准确且过于自信。
...
Method: 构建了OpenEstimate基准测试，通过数值估计任务评估语言模型合成背景信息并表达概率先验的能力，评估这些先验的准确性和校准度。

Result: 在六个前沿语言模型上的测试显示，模型产生的先验分布通常不准确且过于自信，性能仅随不确定性提取方式有轻微改善，但不受采样策略、推理努力或提示设计的影响。
...
### [143] [Procedural Game Level Design with Deep Reinforcement Learning](https://arxiv.org/abs/2510.15120)
*Miraç Buğra Özkan*

Main category: cs.AI

TL;DR: 提出了一种基于深度强化学习的程序化关卡设计方法，使用Unity 3D环境和两个智能体（蜂鸟和浮岛）协同工作，实现自主游戏关卡生成和解决。


<details>
  <summary>Details</summary>
Motivation: 程序化内容生成在游戏开发中越来越受欢迎，可以减少人工工作量并创建动态、可重玩的游戏环境。本研究旨在探索深度强化学习在自主游戏关卡设计中的应用潜力。

Method: 使用Unity ML-Agents工具包，采用PPO算法训练两个智能体：蜂鸟智能体作为解决者学习导航和收集花朵，浮岛智能体负责根据障碍物位置和蜂鸟初始状态生成花朵布局。

Result: 该方法产生了有效的智能体行为，并在各种环境配置中展现出鲁棒的泛化能力。两个智能体之间的交互导致了涌现行为。

Conclusion: 这项工作展示了深度强化学习在使智能体能够生成和解决虚拟环境内容方面的潜力，为AI在创意游戏开发过程中的贡献开辟了新途径。
...
### [144] [Towards Error Centric Intelligence I, Beyond Observational Learning](https://arxiv.org/abs/2510.15128)
*Marcus A. Thomas*

Main category: cs.AI

TL;DR: 该论文认为AGI进展受理论限制而非数据或规模限制，提出因果力学框架，强调假设空间变化作为核心操作，通过模块化干预和因果机制分离等原则实现错误发现和修正。
...
Method: 提出因果力学框架，将假设空间变化作为首要操作，引入局部性和自主性原则、独立因果机制和组合自主性原则，提供可操作的诊断方法。

Result: 建立了将不可达错误转化为可达错误并进行修正的系统框架，为AGI发展提供了理论基础和方法论指导。

Conclusion: AGI发展需要理论突破，因果力学框架通过强调假设空间变化和错误修正机制，为解决理论限制提供了可行路径。
...
### [145] [HugAgent: Evaluating LLMs in Simulating Human-Like Individual Reasoning on Open-Ended Tasks](https://arxiv.org/abs/2510.15144)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型虽然能大规模近似人类响应，但偏向于群体共识，忽略了推理风格和信念轨迹的个体差异，需要开发能捕捉个体推理演化的模型。

Method: 采用双轨设计：合成轨道用于规模化和系统性压力测试，人类轨道用于生态有效的"出声"推理数据收集，评估模型是否能捕捉人们如何推理而不仅仅是相信什么。

Result: 实验显示最先进的LLMs在适应个体推理方面仍存在持续差距，HugAgent成为首个可扩展的基准测试，用于对齐机器推理与人类思维的个体性。

Conclusion: HugAgent为推进机器推理与人类思维个体性对齐提供了首个可扩展基准，其基准和聊天机器人已开源。
...
### [146] [WELD: A Large-Scale Longitudinal Dataset of Emotional Dynamics for Ubiquitous Affective Computing](https://arxiv.org/abs/2510.15221)
*Xiao Sun*

Main category: cs.AI

TL;DR: 提出了一个包含733,651个面部表情记录的大型纵向工作场所情绪数据集，覆盖38名员工30.5个月的数据，包含7种情绪概率和32个扩展情绪指标，验证了数据质量并展示了情绪分类和效价预测的高性能。


<details>
  <summary>Details</summary>
Motivation: 解决真实工作环境中大规模、纵向情绪识别数据集稀缺的问题，特别是在自然环境中收集长期情绪数据的需求。

Method: 使用深度学习面部表情识别技术，在真实办公环境中收集员工面部表情数据，计算7种基本情绪概率和32个扩展情绪指标，包括效价、唤醒度、波动性等。

Result: 数据集成功复现了已知心理学模式（周末效应、昼夜节律），员工离职预测AUC达到1.0，基线实验情绪分类准确率91.2%，效价预测R²=0.84。
...
### [147] [From Checklists to Clusters: A Homeostatic Account of AGI Evaluation](https://arxiv.org/abs/2510.15236)
*Brett Reynolds*

Main category: cs.AI

TL;DR: 本文提出AGI评估应使用因果中心性加权和集群稳定性指数，而非对称权重和快照测试，以更好衡量通用智能的持久性和稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前AGI评估使用对称权重和快照分数，无法区分持久能力与脆弱表现，且忽视了不同领域在智能中的重要性差异。

Method: 提出两个评估扩展：基于CHC理论的中心性优先分数，以及包含配置文件持久性、持久学习和错误纠正的集群稳定性指数家族。

Result: 这些扩展保持了多领域广度，同时减少了脆弱性和博弈行为，提供了更可靠的AGI评估方法。

Conclusion: 通用智能应被视为稳态属性集群，评估应关注能力在扰动下的持久共现，提出的方法为实验室提供了可实施的黑盒协议。
...
### [148] [Multi-dimensional Data Analysis and Applications Basing on LLM Agents and Knowledge Graph Interactions](https://arxiv.org/abs/2510.15258)
...
### [149] [Experience-Driven Exploration for Efficient API-Free AI Agents](https://arxiv.org/abs/2510.15259)
*Chenwei Tang,Jingyu Xing,Xinyu Liu,Zizhou Wang,Jiawei Du,Liangli Zhen,Jiancheng Lv*

Main category: cs.AI

TL;DR: KG-Agent是一个经验驱动的学习框架，通过将像素级GUI交互构建为状态-动作知识图谱来解决API缺失环境下的效率瓶颈问题。


<details>
  <summary>Details</summary>
Motivation: 现有软件大多缺乏可访问的API，导致LLM智能体只能通过像素级GUI进行操作，面临效率低下、短视决策和试错成本高的问题。

Method: 构建状态-动作知识图谱(SA-KG)，将功能相似但视觉不同的GUI状态连接起来，设计基于图拓扑的混合内在奖励机制，结合状态价值奖励和新颖性奖励。

Result: 在Civilization V和Slay the Spire两个复杂GUI决策环境中，相比最先进方法在探索效率和策略深度方面有显著提升。

Conclusion: KG-Agent通过结构化经验表示和混合奖励机制，有效解决了API缺失环境下智能体的探索效率和长期规划问题。
...
### [150] [AUGUSTUS: An LLM-Driven Multimodal Agent System with Contextualized User Memory](https://arxiv.org/abs/2510.15261)
*Jitesh Jain,Shubham Maheshwari,Ning Yu,Wen-mei Hwu,Humphrey Shi*

Main category: cs.AI

TL;DR: AUGUSTUS是一个多模态智能体系统，受人类记忆启发，通过语义标签和图结构的多模态上下文记忆实现高效概念驱动检索，在ImageNet分类任务中比传统多模态RAG快3.5倍，并在MSC基准上超越MemGPT。


<details>
  <summary>Details</summary>
Motivation: 现有智能体系统主要存储文本信息，忽视了多模态信号的重要性。受人类记忆的多模态特性启发，作者希望开发一个符合认知科学中人类记忆理念的多模态智能体系统。

Method: 系统包含4个循环阶段：编码（理解输入）、存储记忆（保存重要信息）、检索（从记忆中搜索相关上下文）、行动（执行任务）。不同于使用向量数据库的传统系统，提出将信息概念化为语义标签，并将标签与其上下文关联存储在图形结构的多模态上下文记忆中，实现高效的概念驱动检索。

Result: 在ImageNet分类任务中比传统多模态RAG方法快3.5倍，在MSC基准测试中表现优于MemGPT。

Conclusion: AUGUSTUS系统通过引入多模态记忆和概念驱动的检索机制，显著提升了智能体系统的性能和效率，验证了多模态记忆在智能体系统中的重要性。
...
### [151] [WebGen-V Bench: Structured Representation for Enhancing Visual Design in LLM-based Web Generation and Evaluation](https://arxiv.org/abs/2510.15306)
*Kuang-Da Wang,Zhao Wang,Yotaro Shimose,Wei-Yao Wang,Shingo Takamatsu*

Main category: cs.AI

TL;DR: WebGen-V是一个用于指令到HTML生成的基准框架，通过智能爬虫收集真实网页数据，采用结构化分段表示，并提供细粒度多模态评估协议。


<details>
  <summary>Details</summary>
Motivation: 利用LLM在编码和多模态理解方面的进展，提升指令到HTML生成任务的数据质量和评估粒度，解决现有基准的局限性。

Method: 提出三个关键创新：1)可扩展的智能爬虫框架持续收集真实网页；2)结构化分段数据表示，整合元数据、局部UI截图和JSON格式资源；3)分段级多模态评估协议，对齐文本、布局和视觉组件。
...
### [152] [VERITAS: Leveraging Vision Priors and Expert Fusion to Improve Multimodal Data](https://arxiv.org/abs/2510.15317)
*Tingqiao Xu,Ziru Zeng,Jiayu Chen*

Main category: cs.AI

TL;DR: VERITAS是一个通过整合视觉先验和多模态模型来提升SFT数据质量的系统，使用统计方法融合多个LMM的评估结果，训练轻量级批评模型，显著提升多模态模型在文本丰富和细粒度推理任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前SFT数据增强方法存在事实错误和幻觉问题，主要由于视觉感知不足。需要系统性地整合视觉先验和多模态模型来提升数据质量。

Method: 使用视觉识别模型和OCR提取结构化视觉先验，结合三个LMM评估原始答案，统计融合得到高置信度共识分数，训练轻量级批评模型，各LMM基于批评生成新答案并选择最优答案。

Result: 在六个多模态基准测试中，使用VERITAS处理数据的模型性能持续优于使用原始数据的模型，特别是在文本丰富和细粒度推理任务上。批评模型能力接近最先进LMM但效率显著更高。

Conclusion: VERITAS能有效提升SFT数据质量，增强多模态模型性能，同时提供高效的数据优化解决方案，推动多模态数据优化研究。
...
### [153] [Towards Flash Thinking via Decoupled Advantage Policy Optimization](https://arxiv.org/abs/2510.15374)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习算法虽然提高了模型准确性，但存在响应过长和过度思考问题，导致推理延迟和计算消耗增加，特别是在需要最少推理的简单任务中。
...
Conclusion: DEPO框架有效解决了大推理模型在简单任务中的低效推理问题，在保持准确性的同时显著降低了响应长度和计算消耗。
...
### [154] [Advancing Routing-Awareness in Analog ICs Floorplanning](https://arxiv.org/abs/2510.15387)
*Davide Basso,Luca Bortolussi,Mirjana Videnovic-Misic,Husni Habal*

Main category: cs.AI

TL;DR: 提出基于强化学习和关系图卷积神经网络的自动布局引擎，通过提高网格分辨率、精确引脚信息集成和动态布线资源估计，在模拟环境中相比现有学习技术减少13.8%死区空间、40.6%线长和提升73.4%布线成功率


<details>
  <summary>Details</summary>
Motivation: 解决模拟集成电路布局中由于电气约束和特定问题要求导致的机器学习应用受限问题，满足布局工程师对路由感知布局解决方案的需求

Method: 使用强化学习和关系图卷积神经网络开发自动布局引擎，结合高网格分辨率、精确引脚信息集成和动态布线资源估计技术

Result: 在模拟环境中相比现有学习技术实现：13.8%死区空间减少、40.6%线长减少、73.4%布线成功率提升

Conclusion: 该方法能够平衡布线效率和面积效率，达到工业标准，为模拟集成电路提供有效的路由感知布局解决方案
...
### [155] [Corrigibility Transformation: Constructing Goals That Accept Updates](https://arxiv.org/abs/2510.15395)
*Rubi Hudson*

Main category: cs.AI

TL;DR: 论文提出了可修正性目标的概念，通过转换方法使任何可修正的目标都能在不牺牲性能的情况下变得可修正，并在网格世界实验中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: AI训练过程中，部分学习的目标往往会激励AI避免进一步的更新，这对安全性构成威胁。可修正性允许纠正错误和适应人类偏好变化，是重要的安全属性。

Method: 形式化定义可修正性，引入转换方法构造任何可修正目标的修正版本，通过条件奖励预测实现，并可递归扩展到新创建的代理。

Result: 两个网格世界实验证明这些可修正目标可以有效学习，并产生期望的行为。

Conclusion: 可修正性目标可以在保持性能的同时确保AI不会抵抗训练更新，为AI安全提供了重要解决方案。
...
### [156] [MARS: Reinforcing Multi-Agent Reasoning of LLMs through Self-Play in Strategic Games](https://arxiv.org/abs/2510.15414)
...
<details>
  <summary>Details</summary>
Motivation: 虽然强化学习在单智能体任务中有效，但在多轮次、多智能体场景中的扩展仍未被充分探索，主要面临长期信用分配和智能体特定优势估计的挑战。
...
### [157] [Adaptive Minds: Empowering Agents with LoRA-as-Tools](https://arxiv.org/abs/2510.15416)
*Pavan C Shekar,Ashwanth Krishnan*

Main category: cs.AI

TL;DR: Adaptive Minds是一个将LoRA适配器作为领域专用工具使用的智能体系统，让基础LLM能够分析查询并动态选择最相关的LoRA工具，实现按需切换领域专家。


<details>
  <summary>Details</summary>
Motivation: 传统方法依赖单一微调模型或基于规则的固定路由，缺乏灵活性和适应性。需要一种能够动态选择领域专业知识的方法，同时保持对话能力。

Method: 使用LangGraph进行工作流管理，让基础LLM作为语义路由器分析每个查询，动态选择最相关的LoRA适配器工具，结合多智能体编排和参数高效微调。

Result: 系统能够提供准确、专业的响应，同时保持对话能力，支持API和Web界面，完全开源，为领域自适应AI助手提供了可扩展基础。

Conclusion: Adaptive Minds通过将LoRA适配器作为工具使用，结合动态路由和多智能体编排，实现了灵活高效的领域自适应AI系统，为专业AI助手提供了可扩展的解决方案。
...
### [158] [Taming the Judge: Deconflicting AI Feedback for Stable Reinforcement Learning](https://arxiv.org/abs/2510.15514)
...
Method: 开发了冲突检测率(CDR)来量化判断冲突，并提出了去冲突图奖励(DGR)框架，通过构建偏好图、转换为无冲突有向无环图(DAG)来生成逻辑一致的奖励信号。
...
### [159] [Hypergraph Contrastive Sensor Fusion for Multimodal Fault Diagnosis in Induction Motors](https://arxiv.org/abs/2510.15547)
*Usman Ali,Ali Zia,Waqas Ali,Umer Ramzan,Abdul Rehman,Muhammad Tayyab Chaudhry,Wei Xiang*

Main category: cs.AI

TL;DR: 提出MM-HCAN多模态超图对比注意力网络，用于电机故障诊断，通过超图拓扑和对比学习融合多模态传感器数据，实现高精度多故障诊断和强泛化能力。
...
Result: 在三个真实基准测试中达到99.82%的准确率，具有强大的跨域泛化能力和噪声鲁棒性，适用于实际部署。
...
### [160] [JudgeSQL: Reasoning over SQL Candidates with Weighted Consensus Tournament](https://arxiv.org/abs/2510.15560)
*Jiayuan Bai,Xuan-guang Pan,Chongyang Tao,Shuai Ma*

Main category: cs.AI

TL;DR: JudgeSQL是一个通过结构化推理和加权共识锦标赛机制改进SQL候选查询选择的框架，解决了现有方法在语义区分和推理链稳定性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 现有文本到SQL任务中的查询选择方法（如自一致性或最佳N解码）仅提供浅层信号，容易产生不一致评分、脆弱推理链，且无法捕捉密切相关的SQL候选之间的细粒度语义差异。

Method: 开发基于推理的SQL判断模型，通过可验证奖励引导的强化学习提炼推理轨迹；构建加权共识锦标赛机制，整合显式推理偏好和隐式生成器置信度。
...
### [161] [Context-aware deep learning using individualized prior information reduces false positives in disease risk prediction and longitudinal health assessment](https://arxiv.org/abs/2510.15591)
...
<details>
  <summary>Details</summary>
Motivation: 医疗中的时间背景对于评估患者健康状况随时间变化至关重要，特别是在历史就诊数据有限且频率不固定的情况下。

Method: 模型首先使用最近一次就诊的医疗数据估计初始疾病风险，然后通过消化先前收集的影像学和/或临床生物标志物信息来优化评估。

Result: 整合历史背景将假阳性转为真阴性，假阳性率从51%降至33%（整合3次影像检查），进一步降至24%（加入临床数据）；预测5年内风险时假阳性率从64%降至9%。

Conclusion: 随时间收集的信息提供了相关背景，可提高医疗风险预测的特异性，为扩大低基线风险人群的纵向健康监测提供可能，实现早期检测和改善健康结果。
...
### [162] [Unleashing Scientific Reasoning for Bio-experimental Protocol Generation via Structured Component-based Reward Mechanism](https://arxiv.org/abs/2510.15600)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型生成的科学实验协议往往不完整或不一致，限制了在可重复科学研究中的实用性。

Method: 引入SciRecipe数据集，提出"草图-填充"范式分离分析、结构和表达，采用结构化组件奖励机制评估步骤粒度、动作顺序和语义保真度，并通过分阶段知识到行动过程训练Thoth模型。

Result: Thoth在多个基准测试中持续超越专有和开源LLM，在步骤对齐、逻辑排序和语义准确性方面实现显著改进。

Conclusion: 该方法为构建可靠的科学助手铺平了道路，能够将知识与实验执行有效连接。
...
### [163] [Build Your Personalized Research Group: A Multiagent Framework for Continual and Interactive Science Automation](https://arxiv.org/abs/2510.15624)
*Ed Li,Junyu Ren,Xintian Pan,Cat Yan,Chuanhao Li,Dirk Bergemann,Zhuoran Yang*

Main category: cs.AI

TL;DR: 提出了一个名为freephdlabor的开源多智能体框架，通过动态工作流和模块化架构实现自适应科学研究自动化，解决了现有系统工作流僵化和上下文管理不足的问题。
...
Method: 采用完全动态工作流（由实时智能体推理决定）和模块化架构，支持用户自定义智能体。提供自动上下文压缩、基于工作空间的通信、跨会话内存持久化和非阻塞人工干预等基础设施。
...
### [164] [Direct Preference Optimization with Unobserved Preference Heterogeneity: The Necessity of Ternary Preferences](https://arxiv.org/abs/2510.15716)
...
Result: 证明了三个或更多响应的排名能确保可识别性，建立了异质偏好下的公平对齐方法。

Conclusion: 为生成模型对齐中的多样用户公平性和个性化建立了理论和算法框架。
...
### [165] [Invoice Information Extraction: Methods and Performance Evaluation](https://arxiv.org/abs/2510.15727)
*Sai Yashwant,Anurag Dubey,Praneeth Paikray,Gantala Thulsiram*

Main category: cs.AI

TL;DR: 提出从发票文档中提取结构化信息的方法，并设计了一套评估指标来衡量提取数据的准确性。


<details>
  <summary>Details</summary>
Motivation: 需要标准化评估发票信息提取方法的效果，比较不同方法的性能优劣。

Method: 使用Docling和LlamaCloud服务对扫描或数字发票进行预处理，提取关键字段如发票号、日期、总金额和供应商信息。

Result: 建立了包含字段级精度、一致性检查失败率和完全匹配准确率的评估框架。

Conclusion: 提出的评估指标为比较不同提取方法提供了标准化方式，并能揭示各字段提取性能的优缺点。
...
### [166] [AURA: An Agent Autonomy Risk Assessment Framework](https://arxiv.org/abs/2510.15739)
...
<details>
  <summary>Details</summary>
Motivation: 随着自主智能AI系统在组织中的采用增加，对齐、治理和风险管理方面的持续挑战威胁着大规模部署。

Method: AURA引入基于gamma的风险评分方法，在风险评估准确性与计算效率之间取得平衡，提供交互式流程来评分、评估和减轻运行单个或多个AI代理的风险。

Result: 该框架为人机协同监督而设计，提供代理到人的通信机制，可与现有协议和工具互操作，支持负责任和透明的智能AI采用。

Conclusion: AURA在平衡计算资源的同时提供强大的风险检测和缓解能力，使其成为企业环境中大规模、可治理智能AI的关键推动者。
...
### [167] [Towards Relaxed Multimodal Inputs for Gait-based Parkinson's Disease Assessment](https://arxiv.org/abs/2510.15748)
...
Result: 在三个公共数据集上的实验表明，TRIP在异步设置下比最佳基线分别提升了16.48、6.89和11.55个百分点，在同步设置下提升了4.86和2.30个百分点。
...
### [168] [Preliminary Quantitative Study on Explainability and Trust in AI Systems](https://arxiv.org/abs/2510.15769)
*Allen Daniel Sunny*

Main category: cs.AI

TL;DR: 研究通过贷款审批模拟实验发现，交互式解释能增强用户信任和参与度，解释的清晰度和相关性是信任的关键决定因素


<details>
  <summary>Details</summary>
Motivation: 大型AI模型在关键领域的应用引发了对信任和透明度的迫切关注，需要研究可解释性与用户信任之间的关系
...
### [169] [Self-evolving expertise in complex non-verifiable subject domains: dialogue as implicit meta-RL](https://arxiv.org/abs/2510.15772)
*Richard M. Bailey*

Main category: cs.AI

TL;DR: Dialectica框架通过结构化对话、记忆和自我反思机制，使AI代理在复杂问题中发展专业知识，显著提升了在Elo评分和Bradley-Terry-Davidson能力等指标上的表现。


<details>
  <summary>Details</summary>
Motivation: 解决LLM在复杂多维度问题中缺乏通过经验发展专业知识的内生机制的问题，特别是在涉及非可验证结果和异质性影响的"邪恶问题"领域。

Method: 提出Dialectica框架，让代理在定义主题上进行结构化对话，配备记忆、自我反思和政策约束的上下文编辑功能，将讨论视为隐式元强化学习过程。

Result: 在两种模型架构上的实验表明，启用基于反思的上下文编辑的代理在Elo评分、标准化Bradley-Terry-Davidson能力和AlphaRank质量上均优于基线版本。

Conclusion: 对话驱动的上下文演化是在开放非可验证领域中实现目标专业知识放大的实用路径，定量和定性证据一致支持这一结论。
...
### [170] [Demo: Guide-RAG: Evidence-Driven Corpus Curation for Retrieval-Augmented Generation in Long COVID](https://arxiv.org/abs/2510.15782)
...
### [171] [PokeeResearch: Effective Deep Research via Reinforcement Learning from AI Feedback and Robust Reasoning Scaffold](https://arxiv.org/abs/2510.15862)
*Yi Wan,Jiuqi Wang,Liam Li,Jinsong Liu,Ruihao Zhu,Zheqing Zhu*

Main category: cs.AI

TL;DR: PokeeResearch-7B是一个7B参数的深度研究代理，通过统一的强化学习框架构建，在10个深度研究基准测试中实现了7B规模代理的最先进性能。
...
Method: 采用无标注的AI反馈强化学习(RLAIF)框架训练，使用基于LLM的奖励信号优化策略，结合思维链驱动的多调用推理框架，实现自我验证和从工具故障中自适应恢复。

Result: 在10个流行的深度研究基准测试中，PokeeResearch-7B在7B规模的深度研究代理中实现了最先进的性能。

Conclusion: 精心设计的强化学习和推理框架可以产生高效、有韧性和研究级的AI代理，该模型和推理代码已在MIT许可下开源。
...
### [172] [Fix False Transparency by Noise Guided Splatting](https://arxiv.org/abs/2510.15736)
*Aly El Hakie,Yiren Lu,Yu Yin,Michael Jenkins,Yehe Liu*

Main category: cs.GR

TL;DR: 本文提出了NGS方法，通过向物体体积中注入不透明噪声高斯来减少3D高斯泼溅（3DGS）重建中出现的虚假透明表面问题，并提出了基于透射率的量化评估指标。


<details>
  <summary>Details</summary>
Motivation: 3DGS重建的不透明物体经常出现虚假透明表面，导致在交互式观看时背景和内部图案不一致。这个问题源于3DGS优化过程中的病态问题，缺乏对表面不透明度的显式约束。

Method: NGS策略通过在训练期间向物体体积中注入不透明噪声高斯，鼓励表面高斯采用更高的不透明度，只需对现有泼溅过程进行最小修改。

Result: 实验表明NGS显著减少了虚假透明度，同时在标准渲染指标上保持竞争力。

Conclusion: NGS方法有效解决了3DGS中的虚假透明伪影问题，证明了其整体有效性。
